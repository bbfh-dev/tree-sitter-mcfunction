/* Automatically @generated by tree-sitter v0.25.10 */

#include "tree_sitter/parser.h"

#if defined(__GNUC__) || defined(__clang__)
#pragma GCC diagnostic ignored "-Wmissing-field-initializers"
#endif

#ifdef _MSC_VER
#pragma optimize("", off)
#elif defined(__clang__)
#pragma clang optimize off
#elif defined(__GNUC__)
#pragma GCC optimize ("O0")
#endif

#define LANGUAGE_VERSION 15
#define STATE_COUNT 254
#define LARGE_STATE_COUNT 20
#define SYMBOL_COUNT 324
#define ALIAS_COUNT 0
#define TOKEN_COUNT 288
#define EXTERNAL_TOKEN_COUNT 0
#define FIELD_COUNT 0
#define MAX_ALIAS_SEQUENCE_LENGTH 6
#define MAX_RESERVED_WORD_SET_SIZE 0
#define PRODUCTION_ID_COUNT 1
#define SUPERTYPE_COUNT 0

enum ts_symbol_identifiers {
  sym__newline = 1,
  aux_sym__space_token1 = 2,
  sym_backslash = 3,
  aux_sym__statement_token1 = 4,
  sym_comment = 5,
  anon_sym_execute = 6,
  anon_sym_run = 7,
  sym_macro_indicator = 8,
  anon_sym_advancement = 9,
  anon_sym_attribute = 10,
  anon_sym_ban = 11,
  anon_sym_ban_DASHip = 12,
  anon_sym_banlist = 13,
  anon_sym_bossbar = 14,
  anon_sym_clear = 15,
  anon_sym_clone = 16,
  anon_sym_damage = 17,
  anon_sym_data = 18,
  anon_sym_datapack = 19,
  anon_sym_debug = 20,
  anon_sym_defaultgamemode = 21,
  anon_sym_deop = 22,
  anon_sym_dialog = 23,
  anon_sym_difficulty = 24,
  anon_sym_effect = 25,
  anon_sym_enchant = 26,
  anon_sym_experience = 27,
  anon_sym_fetchprofile = 28,
  anon_sym_fill = 29,
  anon_sym_fillbiome = 30,
  anon_sym_forceload = 31,
  anon_sym_function = 32,
  anon_sym_gamemode = 33,
  anon_sym_gamerule = 34,
  anon_sym_give = 35,
  anon_sym_help = 36,
  anon_sym_item = 37,
  anon_sym_jfr = 38,
  anon_sym_kick = 39,
  anon_sym_kill = 40,
  anon_sym_list = 41,
  anon_sym_locate = 42,
  anon_sym_loot = 43,
  anon_sym_me = 44,
  anon_sym_msg = 45,
  anon_sym_op = 46,
  anon_sym_pardon = 47,
  anon_sym_pardon_DASHip = 48,
  anon_sym_particle = 49,
  anon_sym_perf = 50,
  anon_sym_place = 51,
  anon_sym_playsound = 52,
  anon_sym_publish = 53,
  anon_sym_random = 54,
  anon_sym_recipe = 55,
  anon_sym_reload = 56,
  anon_sym_return = 57,
  anon_sym_ride = 58,
  anon_sym_rotate = 59,
  anon_sym_save_DASHall = 60,
  anon_sym_save_DASHoff = 61,
  anon_sym_save_DASHon = 62,
  anon_sym_say = 63,
  anon_sym_schedule = 64,
  anon_sym_scoreboard = 65,
  anon_sym_seed = 66,
  anon_sym_setblock = 67,
  anon_sym_setidletimeout = 68,
  anon_sym_setworldspawn = 69,
  anon_sym_spawnpoint = 70,
  anon_sym_spectate = 71,
  anon_sym_spreadplayers = 72,
  anon_sym_stop = 73,
  anon_sym_stopsound = 74,
  anon_sym_summon = 75,
  anon_sym_swing = 76,
  anon_sym_tag = 77,
  anon_sym_team = 78,
  anon_sym_teammsg = 79,
  anon_sym_teleport = 80,
  anon_sym_tell = 81,
  anon_sym_tellraw = 82,
  anon_sym_test = 83,
  anon_sym_time = 84,
  anon_sym_title = 85,
  anon_sym_tm = 86,
  anon_sym_tp = 87,
  anon_sym_transfer = 88,
  anon_sym_trigger = 89,
  anon_sym_version = 90,
  anon_sym_w = 91,
  anon_sym_waypoint = 92,
  anon_sym_weather = 93,
  anon_sym_whitelist = 94,
  anon_sym_worldborder = 95,
  anon_sym_xp = 96,
  anon_sym_align = 97,
  anon_sym_all = 98,
  anon_sym_anchored = 99,
  anon_sym_as = 100,
  anon_sym_at = 101,
  anon_sym_attacker = 102,
  anon_sym_biome = 103,
  anon_sym_block = 104,
  anon_sym_blocks = 105,
  anon_sym_controller = 106,
  anon_sym_dimension = 107,
  anon_sym_entity = 108,
  anon_sym_eyes = 109,
  anon_sym_facing = 110,
  anon_sym_feet = 111,
  anon_sym_if = 112,
  anon_sym_in = 113,
  anon_sym_items = 114,
  anon_sym_leasher = 115,
  anon_sym_loaded = 116,
  anon_sym_masked = 117,
  anon_sym_matches = 118,
  anon_sym_max = 119,
  anon_sym_on = 120,
  anon_sym_origin = 121,
  anon_sym_over = 122,
  anon_sym_owner = 123,
  anon_sym_passengers = 124,
  anon_sym_positioned = 125,
  anon_sym_predicate = 126,
  anon_sym_result = 127,
  anon_sym_rotated = 128,
  anon_sym_score = 129,
  anon_sym_storage = 130,
  anon_sym_store = 131,
  anon_sym_success = 132,
  anon_sym_target = 133,
  anon_sym_unless = 134,
  anon_sym_value = 135,
  anon_sym_vehicle = 136,
  anon_sym_add = 137,
  anon_sym_add_multiplied_base = 138,
  anon_sym_add_multiplied_total = 139,
  anon_sym_add_value = 140,
  anon_sym_amount = 141,
  anon_sym_append = 142,
  anon_sym_base = 143,
  anon_sym_buffer = 144,
  anon_sym_by = 145,
  anon_sym_center = 146,
  anon_sym_color = 147,
  anon_sym_create = 148,
  anon_sym_day = 149,
  anon_sym_daytime = 150,
  anon_sym_destroy = 151,
  anon_sym_disable = 152,
  anon_sym_dismount = 153,
  anon_sym_distance = 154,
  anon_sym_enable = 155,
  anon_sym_everything = 156,
  anon_sym_export = 157,
  anon_sym_exportclosest = 158,
  anon_sym_exportthat = 159,
  anon_sym_exportthese = 160,
  anon_sym_fail = 161,
  anon_sym_feature = 162,
  anon_sym_fish = 163,
  anon_sym_flush = 164,
  anon_sym_force = 165,
  anon_sym_from = 166,
  anon_sym_gametime = 167,
  anon_sym_get = 168,
  anon_sym_grant = 169,
  anon_sym_hollow = 170,
  anon_sym_id = 171,
  anon_sym_insert = 172,
  anon_sym_ips = 173,
  anon_sym_jigsaw = 174,
  anon_sym_join = 175,
  anon_sym_keep = 176,
  anon_sym_leave = 177,
  anon_sym_levels = 178,
  anon_sym_mainhand = 179,
  anon_sym_merge = 180,
  anon_sym_midnight = 181,
  anon_sym_mine = 182,
  anon_sym_modifier = 183,
  anon_sym_modify = 184,
  anon_sym_mount = 185,
  anon_sym_move = 186,
  anon_sym_name = 187,
  anon_sym_night = 188,
  anon_sym_noon = 189,
  anon_sym_normal = 190,
  anon_sym_objectives = 191,
  anon_sym_off = 192,
  anon_sym_offhand = 193,
  anon_sym_only = 194,
  anon_sym_outline = 195,
  anon_sym_players = 196,
  anon_sym_poi = 197,
  anon_sym_points = 198,
  anon_sym_pos = 199,
  anon_sym_prepend = 200,
  anon_sym_query = 201,
  anon_sym_rain = 202,
  anon_sym_remove = 203,
  anon_sym_replace = 204,
  anon_sym_reset = 205,
  anon_sym_resetclosest = 206,
  anon_sym_resetthat = 207,
  anon_sym_resetthese = 208,
  anon_sym_revoke = 209,
  anon_sym_roll = 210,
  anon_sym_runclosest = 211,
  anon_sym_runfailed = 212,
  anon_sym_runthat = 213,
  anon_sym_runthese = 214,
  anon_sym_set = 215,
  anon_sym_setdisplay = 216,
  anon_sym_show = 217,
  anon_sym_spawn = 218,
  anon_sym_start = 219,
  anon_sym_strict = 220,
  anon_sym_string = 221,
  anon_sym_structure = 222,
  anon_sym_style = 223,
  anon_sym_take = 224,
  anon_sym_template = 225,
  anon_sym_through = 226,
  anon_sym_thunder = 227,
  anon_sym_times = 228,
  anon_sym_to = 229,
  anon_sym_under = 230,
  anon_sym_until = 231,
  anon_sym_uuids = 232,
  anon_sym_verify = 233,
  anon_sym_visible = 234,
  anon_sym_warning = 235,
  anon_sym_with = 236,
  aux_sym_selector_token1 = 237,
  aux_sym_selector_token2 = 238,
  sym_selector_identifier = 239,
  sym_query_identifier = 240,
  anon_sym_EQ = 241,
  anon_sym_EQ_BANG = 242,
  aux_sym_selector_query_token1 = 243,
  anon_sym_TILDE = 244,
  anon_sym_CARET = 245,
  sym_heightmap = 246,
  sym_resource = 247,
  anon_sym_DASH = 248,
  aux_sym_scale_token1 = 249,
  aux_sym_scale_token2 = 250,
  anon_sym_DOLLAR_LPAREN = 251,
  aux_sym_macro_token1 = 252,
  anon_sym_RPAREN = 253,
  anon_sym_PERCENT_LBRACK = 254,
  aux_sym_substitution_token1 = 255,
  sym_type = 256,
  anon_sym_PLUS_EQ = 257,
  anon_sym_DASH_EQ = 258,
  anon_sym_STAR_EQ = 259,
  anon_sym_SLASH_EQ = 260,
  anon_sym_PERCENT_EQ = 261,
  anon_sym_GT_LT = 262,
  anon_sym_LT = 263,
  anon_sym_GT = 264,
  aux_sym_slot_token1 = 265,
  aux_sym_slot_token2 = 266,
  aux_sym_slot_token3 = 267,
  aux_sym_slot_token4 = 268,
  aux_sym_slot_token5 = 269,
  aux_sym_slot_token6 = 270,
  aux_sym_slot_token7 = 271,
  anon_sym_contents = 272,
  anon_sym_player_DOTcursor = 273,
  aux_sym_slot_token8 = 274,
  aux_sym_word_token1 = 275,
  anon_sym_DQUOTE = 276,
  aux_sym_string_token1 = 277,
  anon_sym_SQUOTE = 278,
  aux_sym_string_token2 = 279,
  sym_boolean = 280,
  sym_range = 281,
  aux_sym_nbt_identifier_token1 = 282,
  anon_sym_LBRACE = 283,
  anon_sym_COLON = 284,
  anon_sym_RBRACE = 285,
  anon_sym_LBRACK = 286,
  anon_sym_RBRACK = 287,
  sym_source_file = 288,
  sym__space = 289,
  sym__statement = 290,
  sym_command = 291,
  sym__execute_command = 292,
  sym__subcommand = 293,
  sym__generic_command = 294,
  sym_command_identifier = 295,
  sym_subcommand_identifier = 296,
  sym_argument = 297,
  sym_selector = 298,
  sym_selector_query = 299,
  sym_position = 300,
  sym_rotation = 301,
  sym_scale = 302,
  sym_macro = 303,
  sym_substitution = 304,
  sym_operator = 305,
  sym_slot = 306,
  sym_word = 307,
  sym_string = 308,
  sym__nbt_element = 309,
  sym_nbt_identifier = 310,
  sym_nbt_compound = 311,
  sym_nbt_array = 312,
  sym_selector_compound = 313,
  sym_item_compound = 314,
  aux_sym_source_file_repeat1 = 315,
  aux_sym__execute_command_repeat1 = 316,
  aux_sym__generic_command_repeat1 = 317,
  aux_sym_selector_repeat1 = 318,
  aux_sym_selector_query_repeat1 = 319,
  aux_sym_nbt_compound_repeat1 = 320,
  aux_sym_nbt_array_repeat1 = 321,
  aux_sym_selector_compound_repeat1 = 322,
  aux_sym_item_compound_repeat1 = 323,
};

static const char * const ts_symbol_names[] = {
  [ts_builtin_sym_end] = "end",
  [sym__newline] = "_newline",
  [aux_sym__space_token1] = "_space_token1",
  [sym_backslash] = "backslash",
  [aux_sym__statement_token1] = "_statement_token1",
  [sym_comment] = "comment",
  [anon_sym_execute] = "execute",
  [anon_sym_run] = "run",
  [sym_macro_indicator] = "macro_indicator",
  [anon_sym_advancement] = "advancement",
  [anon_sym_attribute] = "attribute",
  [anon_sym_ban] = "ban",
  [anon_sym_ban_DASHip] = "ban-ip",
  [anon_sym_banlist] = "banlist",
  [anon_sym_bossbar] = "bossbar",
  [anon_sym_clear] = "clear",
  [anon_sym_clone] = "clone",
  [anon_sym_damage] = "damage",
  [anon_sym_data] = "data",
  [anon_sym_datapack] = "datapack",
  [anon_sym_debug] = "debug",
  [anon_sym_defaultgamemode] = "defaultgamemode",
  [anon_sym_deop] = "deop",
  [anon_sym_dialog] = "dialog",
  [anon_sym_difficulty] = "difficulty",
  [anon_sym_effect] = "effect",
  [anon_sym_enchant] = "enchant",
  [anon_sym_experience] = "experience",
  [anon_sym_fetchprofile] = "fetchprofile",
  [anon_sym_fill] = "fill",
  [anon_sym_fillbiome] = "fillbiome",
  [anon_sym_forceload] = "forceload",
  [anon_sym_function] = "function",
  [anon_sym_gamemode] = "gamemode",
  [anon_sym_gamerule] = "gamerule",
  [anon_sym_give] = "give",
  [anon_sym_help] = "help",
  [anon_sym_item] = "item",
  [anon_sym_jfr] = "jfr",
  [anon_sym_kick] = "kick",
  [anon_sym_kill] = "kill",
  [anon_sym_list] = "list",
  [anon_sym_locate] = "locate",
  [anon_sym_loot] = "loot",
  [anon_sym_me] = "me",
  [anon_sym_msg] = "msg",
  [anon_sym_op] = "op",
  [anon_sym_pardon] = "pardon",
  [anon_sym_pardon_DASHip] = "pardon-ip",
  [anon_sym_particle] = "particle",
  [anon_sym_perf] = "perf",
  [anon_sym_place] = "place",
  [anon_sym_playsound] = "playsound",
  [anon_sym_publish] = "publish",
  [anon_sym_random] = "random",
  [anon_sym_recipe] = "recipe",
  [anon_sym_reload] = "reload",
  [anon_sym_return] = "return",
  [anon_sym_ride] = "ride",
  [anon_sym_rotate] = "rotate",
  [anon_sym_save_DASHall] = "save-all",
  [anon_sym_save_DASHoff] = "save-off",
  [anon_sym_save_DASHon] = "save-on",
  [anon_sym_say] = "say",
  [anon_sym_schedule] = "schedule",
  [anon_sym_scoreboard] = "scoreboard",
  [anon_sym_seed] = "seed",
  [anon_sym_setblock] = "setblock",
  [anon_sym_setidletimeout] = "setidletimeout",
  [anon_sym_setworldspawn] = "setworldspawn",
  [anon_sym_spawnpoint] = "spawnpoint",
  [anon_sym_spectate] = "spectate",
  [anon_sym_spreadplayers] = "spreadplayers",
  [anon_sym_stop] = "stop",
  [anon_sym_stopsound] = "stopsound",
  [anon_sym_summon] = "summon",
  [anon_sym_swing] = "swing",
  [anon_sym_tag] = "tag",
  [anon_sym_team] = "team",
  [anon_sym_teammsg] = "teammsg",
  [anon_sym_teleport] = "teleport",
  [anon_sym_tell] = "tell",
  [anon_sym_tellraw] = "tellraw",
  [anon_sym_test] = "test",
  [anon_sym_time] = "time",
  [anon_sym_title] = "title",
  [anon_sym_tm] = "tm",
  [anon_sym_tp] = "tp",
  [anon_sym_transfer] = "transfer",
  [anon_sym_trigger] = "trigger",
  [anon_sym_version] = "version",
  [anon_sym_w] = "w",
  [anon_sym_waypoint] = "waypoint",
  [anon_sym_weather] = "weather",
  [anon_sym_whitelist] = "whitelist",
  [anon_sym_worldborder] = "worldborder",
  [anon_sym_xp] = "xp",
  [anon_sym_align] = "align",
  [anon_sym_all] = "all",
  [anon_sym_anchored] = "anchored",
  [anon_sym_as] = "as",
  [anon_sym_at] = "at",
  [anon_sym_attacker] = "attacker",
  [anon_sym_biome] = "biome",
  [anon_sym_block] = "block",
  [anon_sym_blocks] = "blocks",
  [anon_sym_controller] = "controller",
  [anon_sym_dimension] = "dimension",
  [anon_sym_entity] = "entity",
  [anon_sym_eyes] = "eyes",
  [anon_sym_facing] = "facing",
  [anon_sym_feet] = "feet",
  [anon_sym_if] = "if",
  [anon_sym_in] = "in",
  [anon_sym_items] = "items",
  [anon_sym_leasher] = "leasher",
  [anon_sym_loaded] = "loaded",
  [anon_sym_masked] = "masked",
  [anon_sym_matches] = "matches",
  [anon_sym_max] = "max",
  [anon_sym_on] = "on",
  [anon_sym_origin] = "origin",
  [anon_sym_over] = "over",
  [anon_sym_owner] = "owner",
  [anon_sym_passengers] = "passengers",
  [anon_sym_positioned] = "positioned",
  [anon_sym_predicate] = "predicate",
  [anon_sym_result] = "result",
  [anon_sym_rotated] = "rotated",
  [anon_sym_score] = "score",
  [anon_sym_storage] = "storage",
  [anon_sym_store] = "store",
  [anon_sym_success] = "success",
  [anon_sym_target] = "target",
  [anon_sym_unless] = "unless",
  [anon_sym_value] = "value",
  [anon_sym_vehicle] = "vehicle",
  [anon_sym_add] = "add",
  [anon_sym_add_multiplied_base] = "add_multiplied_base",
  [anon_sym_add_multiplied_total] = "add_multiplied_total",
  [anon_sym_add_value] = "add_value",
  [anon_sym_amount] = "amount",
  [anon_sym_append] = "append",
  [anon_sym_base] = "base",
  [anon_sym_buffer] = "buffer",
  [anon_sym_by] = "by",
  [anon_sym_center] = "center",
  [anon_sym_color] = "color",
  [anon_sym_create] = "create",
  [anon_sym_day] = "day",
  [anon_sym_daytime] = "daytime",
  [anon_sym_destroy] = "destroy",
  [anon_sym_disable] = "disable",
  [anon_sym_dismount] = "dismount",
  [anon_sym_distance] = "distance",
  [anon_sym_enable] = "enable",
  [anon_sym_everything] = "everything",
  [anon_sym_export] = "export",
  [anon_sym_exportclosest] = "exportclosest",
  [anon_sym_exportthat] = "exportthat",
  [anon_sym_exportthese] = "exportthese",
  [anon_sym_fail] = "fail",
  [anon_sym_feature] = "feature",
  [anon_sym_fish] = "fish",
  [anon_sym_flush] = "flush",
  [anon_sym_force] = "force",
  [anon_sym_from] = "from",
  [anon_sym_gametime] = "gametime",
  [anon_sym_get] = "get",
  [anon_sym_grant] = "grant",
  [anon_sym_hollow] = "hollow",
  [anon_sym_id] = "id",
  [anon_sym_insert] = "insert",
  [anon_sym_ips] = "ips",
  [anon_sym_jigsaw] = "jigsaw",
  [anon_sym_join] = "join",
  [anon_sym_keep] = "keep",
  [anon_sym_leave] = "leave",
  [anon_sym_levels] = "levels",
  [anon_sym_mainhand] = "mainhand",
  [anon_sym_merge] = "merge",
  [anon_sym_midnight] = "midnight",
  [anon_sym_mine] = "mine",
  [anon_sym_modifier] = "modifier",
  [anon_sym_modify] = "modify",
  [anon_sym_mount] = "mount",
  [anon_sym_move] = "move",
  [anon_sym_name] = "name",
  [anon_sym_night] = "night",
  [anon_sym_noon] = "noon",
  [anon_sym_normal] = "normal",
  [anon_sym_objectives] = "objectives",
  [anon_sym_off] = "off",
  [anon_sym_offhand] = "offhand",
  [anon_sym_only] = "only",
  [anon_sym_outline] = "outline",
  [anon_sym_players] = "players",
  [anon_sym_poi] = "poi",
  [anon_sym_points] = "points",
  [anon_sym_pos] = "pos",
  [anon_sym_prepend] = "prepend",
  [anon_sym_query] = "query",
  [anon_sym_rain] = "rain",
  [anon_sym_remove] = "remove",
  [anon_sym_replace] = "replace",
  [anon_sym_reset] = "reset",
  [anon_sym_resetclosest] = "resetclosest",
  [anon_sym_resetthat] = "resetthat",
  [anon_sym_resetthese] = "resetthese",
  [anon_sym_revoke] = "revoke",
  [anon_sym_roll] = "roll",
  [anon_sym_runclosest] = "runclosest",
  [anon_sym_runfailed] = "runfailed",
  [anon_sym_runthat] = "runthat",
  [anon_sym_runthese] = "runthese",
  [anon_sym_set] = "set",
  [anon_sym_setdisplay] = "setdisplay",
  [anon_sym_show] = "show",
  [anon_sym_spawn] = "spawn",
  [anon_sym_start] = "start",
  [anon_sym_strict] = "strict",
  [anon_sym_string] = "string",
  [anon_sym_structure] = "structure",
  [anon_sym_style] = "style",
  [anon_sym_take] = "take",
  [anon_sym_template] = "template",
  [anon_sym_through] = "through",
  [anon_sym_thunder] = "thunder",
  [anon_sym_times] = "times",
  [anon_sym_to] = "to",
  [anon_sym_under] = "under",
  [anon_sym_until] = "until",
  [anon_sym_uuids] = "uuids",
  [anon_sym_verify] = "verify",
  [anon_sym_visible] = "visible",
  [anon_sym_warning] = "warning",
  [anon_sym_with] = "with",
  [aux_sym_selector_token1] = "selector_token1",
  [aux_sym_selector_token2] = "selector_token2",
  [sym_selector_identifier] = "selector_identifier",
  [sym_query_identifier] = "query_identifier",
  [anon_sym_EQ] = "=",
  [anon_sym_EQ_BANG] = "=!",
  [aux_sym_selector_query_token1] = "selector_query_token1",
  [anon_sym_TILDE] = "~",
  [anon_sym_CARET] = "^",
  [sym_heightmap] = "heightmap",
  [sym_resource] = "resource",
  [anon_sym_DASH] = "-",
  [aux_sym_scale_token1] = "scale_token1",
  [aux_sym_scale_token2] = "scale_token2",
  [anon_sym_DOLLAR_LPAREN] = "$(",
  [aux_sym_macro_token1] = "macro_token1",
  [anon_sym_RPAREN] = ")",
  [anon_sym_PERCENT_LBRACK] = "%[",
  [aux_sym_substitution_token1] = "substitution_token1",
  [sym_type] = "type",
  [anon_sym_PLUS_EQ] = "+=",
  [anon_sym_DASH_EQ] = "-=",
  [anon_sym_STAR_EQ] = "*=",
  [anon_sym_SLASH_EQ] = "/=",
  [anon_sym_PERCENT_EQ] = "%=",
  [anon_sym_GT_LT] = "><",
  [anon_sym_LT] = "<",
  [anon_sym_GT] = ">",
  [aux_sym_slot_token1] = "slot_token1",
  [aux_sym_slot_token2] = "slot_token2",
  [aux_sym_slot_token3] = "slot_token3",
  [aux_sym_slot_token4] = "slot_token4",
  [aux_sym_slot_token5] = "slot_token5",
  [aux_sym_slot_token6] = "slot_token6",
  [aux_sym_slot_token7] = "slot_token7",
  [anon_sym_contents] = "contents",
  [anon_sym_player_DOTcursor] = "player.cursor",
  [aux_sym_slot_token8] = "slot_token8",
  [aux_sym_word_token1] = "word_token1",
  [anon_sym_DQUOTE] = "\"",
  [aux_sym_string_token1] = "string_token1",
  [anon_sym_SQUOTE] = "'",
  [aux_sym_string_token2] = "string_token2",
  [sym_boolean] = "boolean",
  [sym_range] = "range",
  [aux_sym_nbt_identifier_token1] = "nbt_identifier_token1",
  [anon_sym_LBRACE] = "{",
  [anon_sym_COLON] = ":",
  [anon_sym_RBRACE] = "}",
  [anon_sym_LBRACK] = "[",
  [anon_sym_RBRACK] = "]",
  [sym_source_file] = "source_file",
  [sym__space] = "_space",
  [sym__statement] = "_statement",
  [sym_command] = "command",
  [sym__execute_command] = "_execute_command",
  [sym__subcommand] = "_subcommand",
  [sym__generic_command] = "_generic_command",
  [sym_command_identifier] = "command_identifier",
  [sym_subcommand_identifier] = "subcommand_identifier",
  [sym_argument] = "argument",
  [sym_selector] = "selector",
  [sym_selector_query] = "selector_query",
  [sym_position] = "position",
  [sym_rotation] = "rotation",
  [sym_scale] = "scale",
  [sym_macro] = "macro",
  [sym_substitution] = "substitution",
  [sym_operator] = "operator",
  [sym_slot] = "slot",
  [sym_word] = "word",
  [sym_string] = "string",
  [sym__nbt_element] = "_nbt_element",
  [sym_nbt_identifier] = "nbt_identifier",
  [sym_nbt_compound] = "nbt_compound",
  [sym_nbt_array] = "nbt_array",
  [sym_selector_compound] = "selector_compound",
  [sym_item_compound] = "item_compound",
  [aux_sym_source_file_repeat1] = "source_file_repeat1",
  [aux_sym__execute_command_repeat1] = "_execute_command_repeat1",
  [aux_sym__generic_command_repeat1] = "_generic_command_repeat1",
  [aux_sym_selector_repeat1] = "selector_repeat1",
  [aux_sym_selector_query_repeat1] = "selector_query_repeat1",
  [aux_sym_nbt_compound_repeat1] = "nbt_compound_repeat1",
  [aux_sym_nbt_array_repeat1] = "nbt_array_repeat1",
  [aux_sym_selector_compound_repeat1] = "selector_compound_repeat1",
  [aux_sym_item_compound_repeat1] = "item_compound_repeat1",
};

static const TSSymbol ts_symbol_map[] = {
  [ts_builtin_sym_end] = ts_builtin_sym_end,
  [sym__newline] = sym__newline,
  [aux_sym__space_token1] = aux_sym__space_token1,
  [sym_backslash] = sym_backslash,
  [aux_sym__statement_token1] = aux_sym__statement_token1,
  [sym_comment] = sym_comment,
  [anon_sym_execute] = anon_sym_execute,
  [anon_sym_run] = anon_sym_run,
  [sym_macro_indicator] = sym_macro_indicator,
  [anon_sym_advancement] = anon_sym_advancement,
  [anon_sym_attribute] = anon_sym_attribute,
  [anon_sym_ban] = anon_sym_ban,
  [anon_sym_ban_DASHip] = anon_sym_ban_DASHip,
  [anon_sym_banlist] = anon_sym_banlist,
  [anon_sym_bossbar] = anon_sym_bossbar,
  [anon_sym_clear] = anon_sym_clear,
  [anon_sym_clone] = anon_sym_clone,
  [anon_sym_damage] = anon_sym_damage,
  [anon_sym_data] = anon_sym_data,
  [anon_sym_datapack] = anon_sym_datapack,
  [anon_sym_debug] = anon_sym_debug,
  [anon_sym_defaultgamemode] = anon_sym_defaultgamemode,
  [anon_sym_deop] = anon_sym_deop,
  [anon_sym_dialog] = anon_sym_dialog,
  [anon_sym_difficulty] = anon_sym_difficulty,
  [anon_sym_effect] = anon_sym_effect,
  [anon_sym_enchant] = anon_sym_enchant,
  [anon_sym_experience] = anon_sym_experience,
  [anon_sym_fetchprofile] = anon_sym_fetchprofile,
  [anon_sym_fill] = anon_sym_fill,
  [anon_sym_fillbiome] = anon_sym_fillbiome,
  [anon_sym_forceload] = anon_sym_forceload,
  [anon_sym_function] = anon_sym_function,
  [anon_sym_gamemode] = anon_sym_gamemode,
  [anon_sym_gamerule] = anon_sym_gamerule,
  [anon_sym_give] = anon_sym_give,
  [anon_sym_help] = anon_sym_help,
  [anon_sym_item] = anon_sym_item,
  [anon_sym_jfr] = anon_sym_jfr,
  [anon_sym_kick] = anon_sym_kick,
  [anon_sym_kill] = anon_sym_kill,
  [anon_sym_list] = anon_sym_list,
  [anon_sym_locate] = anon_sym_locate,
  [anon_sym_loot] = anon_sym_loot,
  [anon_sym_me] = anon_sym_me,
  [anon_sym_msg] = anon_sym_msg,
  [anon_sym_op] = anon_sym_op,
  [anon_sym_pardon] = anon_sym_pardon,
  [anon_sym_pardon_DASHip] = anon_sym_pardon_DASHip,
  [anon_sym_particle] = anon_sym_particle,
  [anon_sym_perf] = anon_sym_perf,
  [anon_sym_place] = anon_sym_place,
  [anon_sym_playsound] = anon_sym_playsound,
  [anon_sym_publish] = anon_sym_publish,
  [anon_sym_random] = anon_sym_random,
  [anon_sym_recipe] = anon_sym_recipe,
  [anon_sym_reload] = anon_sym_reload,
  [anon_sym_return] = anon_sym_return,
  [anon_sym_ride] = anon_sym_ride,
  [anon_sym_rotate] = anon_sym_rotate,
  [anon_sym_save_DASHall] = anon_sym_save_DASHall,
  [anon_sym_save_DASHoff] = anon_sym_save_DASHoff,
  [anon_sym_save_DASHon] = anon_sym_save_DASHon,
  [anon_sym_say] = anon_sym_say,
  [anon_sym_schedule] = anon_sym_schedule,
  [anon_sym_scoreboard] = anon_sym_scoreboard,
  [anon_sym_seed] = anon_sym_seed,
  [anon_sym_setblock] = anon_sym_setblock,
  [anon_sym_setidletimeout] = anon_sym_setidletimeout,
  [anon_sym_setworldspawn] = anon_sym_setworldspawn,
  [anon_sym_spawnpoint] = anon_sym_spawnpoint,
  [anon_sym_spectate] = anon_sym_spectate,
  [anon_sym_spreadplayers] = anon_sym_spreadplayers,
  [anon_sym_stop] = anon_sym_stop,
  [anon_sym_stopsound] = anon_sym_stopsound,
  [anon_sym_summon] = anon_sym_summon,
  [anon_sym_swing] = anon_sym_swing,
  [anon_sym_tag] = anon_sym_tag,
  [anon_sym_team] = anon_sym_team,
  [anon_sym_teammsg] = anon_sym_teammsg,
  [anon_sym_teleport] = anon_sym_teleport,
  [anon_sym_tell] = anon_sym_tell,
  [anon_sym_tellraw] = anon_sym_tellraw,
  [anon_sym_test] = anon_sym_test,
  [anon_sym_time] = anon_sym_time,
  [anon_sym_title] = anon_sym_title,
  [anon_sym_tm] = anon_sym_tm,
  [anon_sym_tp] = anon_sym_tp,
  [anon_sym_transfer] = anon_sym_transfer,
  [anon_sym_trigger] = anon_sym_trigger,
  [anon_sym_version] = anon_sym_version,
  [anon_sym_w] = anon_sym_w,
  [anon_sym_waypoint] = anon_sym_waypoint,
  [anon_sym_weather] = anon_sym_weather,
  [anon_sym_whitelist] = anon_sym_whitelist,
  [anon_sym_worldborder] = anon_sym_worldborder,
  [anon_sym_xp] = anon_sym_xp,
  [anon_sym_align] = anon_sym_align,
  [anon_sym_all] = anon_sym_all,
  [anon_sym_anchored] = anon_sym_anchored,
  [anon_sym_as] = anon_sym_as,
  [anon_sym_at] = anon_sym_at,
  [anon_sym_attacker] = anon_sym_attacker,
  [anon_sym_biome] = anon_sym_biome,
  [anon_sym_block] = anon_sym_block,
  [anon_sym_blocks] = anon_sym_blocks,
  [anon_sym_controller] = anon_sym_controller,
  [anon_sym_dimension] = anon_sym_dimension,
  [anon_sym_entity] = anon_sym_entity,
  [anon_sym_eyes] = anon_sym_eyes,
  [anon_sym_facing] = anon_sym_facing,
  [anon_sym_feet] = anon_sym_feet,
  [anon_sym_if] = anon_sym_if,
  [anon_sym_in] = anon_sym_in,
  [anon_sym_items] = anon_sym_items,
  [anon_sym_leasher] = anon_sym_leasher,
  [anon_sym_loaded] = anon_sym_loaded,
  [anon_sym_masked] = anon_sym_masked,
  [anon_sym_matches] = anon_sym_matches,
  [anon_sym_max] = anon_sym_max,
  [anon_sym_on] = anon_sym_on,
  [anon_sym_origin] = anon_sym_origin,
  [anon_sym_over] = anon_sym_over,
  [anon_sym_owner] = anon_sym_owner,
  [anon_sym_passengers] = anon_sym_passengers,
  [anon_sym_positioned] = anon_sym_positioned,
  [anon_sym_predicate] = anon_sym_predicate,
  [anon_sym_result] = anon_sym_result,
  [anon_sym_rotated] = anon_sym_rotated,
  [anon_sym_score] = anon_sym_score,
  [anon_sym_storage] = anon_sym_storage,
  [anon_sym_store] = anon_sym_store,
  [anon_sym_success] = anon_sym_success,
  [anon_sym_target] = anon_sym_target,
  [anon_sym_unless] = anon_sym_unless,
  [anon_sym_value] = anon_sym_value,
  [anon_sym_vehicle] = anon_sym_vehicle,
  [anon_sym_add] = anon_sym_add,
  [anon_sym_add_multiplied_base] = anon_sym_add_multiplied_base,
  [anon_sym_add_multiplied_total] = anon_sym_add_multiplied_total,
  [anon_sym_add_value] = anon_sym_add_value,
  [anon_sym_amount] = anon_sym_amount,
  [anon_sym_append] = anon_sym_append,
  [anon_sym_base] = anon_sym_base,
  [anon_sym_buffer] = anon_sym_buffer,
  [anon_sym_by] = anon_sym_by,
  [anon_sym_center] = anon_sym_center,
  [anon_sym_color] = anon_sym_color,
  [anon_sym_create] = anon_sym_create,
  [anon_sym_day] = anon_sym_day,
  [anon_sym_daytime] = anon_sym_daytime,
  [anon_sym_destroy] = anon_sym_destroy,
  [anon_sym_disable] = anon_sym_disable,
  [anon_sym_dismount] = anon_sym_dismount,
  [anon_sym_distance] = anon_sym_distance,
  [anon_sym_enable] = anon_sym_enable,
  [anon_sym_everything] = anon_sym_everything,
  [anon_sym_export] = anon_sym_export,
  [anon_sym_exportclosest] = anon_sym_exportclosest,
  [anon_sym_exportthat] = anon_sym_exportthat,
  [anon_sym_exportthese] = anon_sym_exportthese,
  [anon_sym_fail] = anon_sym_fail,
  [anon_sym_feature] = anon_sym_feature,
  [anon_sym_fish] = anon_sym_fish,
  [anon_sym_flush] = anon_sym_flush,
  [anon_sym_force] = anon_sym_force,
  [anon_sym_from] = anon_sym_from,
  [anon_sym_gametime] = anon_sym_gametime,
  [anon_sym_get] = anon_sym_get,
  [anon_sym_grant] = anon_sym_grant,
  [anon_sym_hollow] = anon_sym_hollow,
  [anon_sym_id] = anon_sym_id,
  [anon_sym_insert] = anon_sym_insert,
  [anon_sym_ips] = anon_sym_ips,
  [anon_sym_jigsaw] = anon_sym_jigsaw,
  [anon_sym_join] = anon_sym_join,
  [anon_sym_keep] = anon_sym_keep,
  [anon_sym_leave] = anon_sym_leave,
  [anon_sym_levels] = anon_sym_levels,
  [anon_sym_mainhand] = anon_sym_mainhand,
  [anon_sym_merge] = anon_sym_merge,
  [anon_sym_midnight] = anon_sym_midnight,
  [anon_sym_mine] = anon_sym_mine,
  [anon_sym_modifier] = anon_sym_modifier,
  [anon_sym_modify] = anon_sym_modify,
  [anon_sym_mount] = anon_sym_mount,
  [anon_sym_move] = anon_sym_move,
  [anon_sym_name] = anon_sym_name,
  [anon_sym_night] = anon_sym_night,
  [anon_sym_noon] = anon_sym_noon,
  [anon_sym_normal] = anon_sym_normal,
  [anon_sym_objectives] = anon_sym_objectives,
  [anon_sym_off] = anon_sym_off,
  [anon_sym_offhand] = anon_sym_offhand,
  [anon_sym_only] = anon_sym_only,
  [anon_sym_outline] = anon_sym_outline,
  [anon_sym_players] = anon_sym_players,
  [anon_sym_poi] = anon_sym_poi,
  [anon_sym_points] = anon_sym_points,
  [anon_sym_pos] = anon_sym_pos,
  [anon_sym_prepend] = anon_sym_prepend,
  [anon_sym_query] = anon_sym_query,
  [anon_sym_rain] = anon_sym_rain,
  [anon_sym_remove] = anon_sym_remove,
  [anon_sym_replace] = anon_sym_replace,
  [anon_sym_reset] = anon_sym_reset,
  [anon_sym_resetclosest] = anon_sym_resetclosest,
  [anon_sym_resetthat] = anon_sym_resetthat,
  [anon_sym_resetthese] = anon_sym_resetthese,
  [anon_sym_revoke] = anon_sym_revoke,
  [anon_sym_roll] = anon_sym_roll,
  [anon_sym_runclosest] = anon_sym_runclosest,
  [anon_sym_runfailed] = anon_sym_runfailed,
  [anon_sym_runthat] = anon_sym_runthat,
  [anon_sym_runthese] = anon_sym_runthese,
  [anon_sym_set] = anon_sym_set,
  [anon_sym_setdisplay] = anon_sym_setdisplay,
  [anon_sym_show] = anon_sym_show,
  [anon_sym_spawn] = anon_sym_spawn,
  [anon_sym_start] = anon_sym_start,
  [anon_sym_strict] = anon_sym_strict,
  [anon_sym_string] = anon_sym_string,
  [anon_sym_structure] = anon_sym_structure,
  [anon_sym_style] = anon_sym_style,
  [anon_sym_take] = anon_sym_take,
  [anon_sym_template] = anon_sym_template,
  [anon_sym_through] = anon_sym_through,
  [anon_sym_thunder] = anon_sym_thunder,
  [anon_sym_times] = anon_sym_times,
  [anon_sym_to] = anon_sym_to,
  [anon_sym_under] = anon_sym_under,
  [anon_sym_until] = anon_sym_until,
  [anon_sym_uuids] = anon_sym_uuids,
  [anon_sym_verify] = anon_sym_verify,
  [anon_sym_visible] = anon_sym_visible,
  [anon_sym_warning] = anon_sym_warning,
  [anon_sym_with] = anon_sym_with,
  [aux_sym_selector_token1] = aux_sym_selector_token1,
  [aux_sym_selector_token2] = aux_sym_selector_token2,
  [sym_selector_identifier] = sym_selector_identifier,
  [sym_query_identifier] = sym_query_identifier,
  [anon_sym_EQ] = anon_sym_EQ,
  [anon_sym_EQ_BANG] = anon_sym_EQ_BANG,
  [aux_sym_selector_query_token1] = aux_sym_selector_query_token1,
  [anon_sym_TILDE] = anon_sym_TILDE,
  [anon_sym_CARET] = anon_sym_CARET,
  [sym_heightmap] = sym_heightmap,
  [sym_resource] = sym_resource,
  [anon_sym_DASH] = anon_sym_DASH,
  [aux_sym_scale_token1] = aux_sym_scale_token1,
  [aux_sym_scale_token2] = aux_sym_scale_token2,
  [anon_sym_DOLLAR_LPAREN] = anon_sym_DOLLAR_LPAREN,
  [aux_sym_macro_token1] = aux_sym_macro_token1,
  [anon_sym_RPAREN] = anon_sym_RPAREN,
  [anon_sym_PERCENT_LBRACK] = anon_sym_PERCENT_LBRACK,
  [aux_sym_substitution_token1] = aux_sym_substitution_token1,
  [sym_type] = sym_type,
  [anon_sym_PLUS_EQ] = anon_sym_PLUS_EQ,
  [anon_sym_DASH_EQ] = anon_sym_DASH_EQ,
  [anon_sym_STAR_EQ] = anon_sym_STAR_EQ,
  [anon_sym_SLASH_EQ] = anon_sym_SLASH_EQ,
  [anon_sym_PERCENT_EQ] = anon_sym_PERCENT_EQ,
  [anon_sym_GT_LT] = anon_sym_GT_LT,
  [anon_sym_LT] = anon_sym_LT,
  [anon_sym_GT] = anon_sym_GT,
  [aux_sym_slot_token1] = aux_sym_slot_token1,
  [aux_sym_slot_token2] = aux_sym_slot_token2,
  [aux_sym_slot_token3] = aux_sym_slot_token3,
  [aux_sym_slot_token4] = aux_sym_slot_token4,
  [aux_sym_slot_token5] = aux_sym_slot_token5,
  [aux_sym_slot_token6] = aux_sym_slot_token6,
  [aux_sym_slot_token7] = aux_sym_slot_token7,
  [anon_sym_contents] = anon_sym_contents,
  [anon_sym_player_DOTcursor] = anon_sym_player_DOTcursor,
  [aux_sym_slot_token8] = aux_sym_slot_token8,
  [aux_sym_word_token1] = aux_sym_word_token1,
  [anon_sym_DQUOTE] = anon_sym_DQUOTE,
  [aux_sym_string_token1] = aux_sym_string_token1,
  [anon_sym_SQUOTE] = anon_sym_SQUOTE,
  [aux_sym_string_token2] = aux_sym_string_token2,
  [sym_boolean] = sym_boolean,
  [sym_range] = sym_range,
  [aux_sym_nbt_identifier_token1] = aux_sym_nbt_identifier_token1,
  [anon_sym_LBRACE] = anon_sym_LBRACE,
  [anon_sym_COLON] = anon_sym_COLON,
  [anon_sym_RBRACE] = anon_sym_RBRACE,
  [anon_sym_LBRACK] = anon_sym_LBRACK,
  [anon_sym_RBRACK] = anon_sym_RBRACK,
  [sym_source_file] = sym_source_file,
  [sym__space] = sym__space,
  [sym__statement] = sym__statement,
  [sym_command] = sym_command,
  [sym__execute_command] = sym__execute_command,
  [sym__subcommand] = sym__subcommand,
  [sym__generic_command] = sym__generic_command,
  [sym_command_identifier] = sym_command_identifier,
  [sym_subcommand_identifier] = sym_subcommand_identifier,
  [sym_argument] = sym_argument,
  [sym_selector] = sym_selector,
  [sym_selector_query] = sym_selector_query,
  [sym_position] = sym_position,
  [sym_rotation] = sym_rotation,
  [sym_scale] = sym_scale,
  [sym_macro] = sym_macro,
  [sym_substitution] = sym_substitution,
  [sym_operator] = sym_operator,
  [sym_slot] = sym_slot,
  [sym_word] = sym_word,
  [sym_string] = sym_string,
  [sym__nbt_element] = sym__nbt_element,
  [sym_nbt_identifier] = sym_nbt_identifier,
  [sym_nbt_compound] = sym_nbt_compound,
  [sym_nbt_array] = sym_nbt_array,
  [sym_selector_compound] = sym_selector_compound,
  [sym_item_compound] = sym_item_compound,
  [aux_sym_source_file_repeat1] = aux_sym_source_file_repeat1,
  [aux_sym__execute_command_repeat1] = aux_sym__execute_command_repeat1,
  [aux_sym__generic_command_repeat1] = aux_sym__generic_command_repeat1,
  [aux_sym_selector_repeat1] = aux_sym_selector_repeat1,
  [aux_sym_selector_query_repeat1] = aux_sym_selector_query_repeat1,
  [aux_sym_nbt_compound_repeat1] = aux_sym_nbt_compound_repeat1,
  [aux_sym_nbt_array_repeat1] = aux_sym_nbt_array_repeat1,
  [aux_sym_selector_compound_repeat1] = aux_sym_selector_compound_repeat1,
  [aux_sym_item_compound_repeat1] = aux_sym_item_compound_repeat1,
};

static const TSSymbolMetadata ts_symbol_metadata[] = {
  [ts_builtin_sym_end] = {
    .visible = false,
    .named = true,
  },
  [sym__newline] = {
    .visible = false,
    .named = true,
  },
  [aux_sym__space_token1] = {
    .visible = false,
    .named = false,
  },
  [sym_backslash] = {
    .visible = true,
    .named = true,
  },
  [aux_sym__statement_token1] = {
    .visible = false,
    .named = false,
  },
  [sym_comment] = {
    .visible = true,
    .named = true,
  },
  [anon_sym_execute] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_run] = {
    .visible = true,
    .named = false,
  },
  [sym_macro_indicator] = {
    .visible = true,
    .named = true,
  },
  [anon_sym_advancement] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_attribute] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_ban] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_ban_DASHip] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_banlist] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_bossbar] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_clear] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_clone] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_damage] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_data] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_datapack] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_debug] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_defaultgamemode] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_deop] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_dialog] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_difficulty] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_effect] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_enchant] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_experience] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_fetchprofile] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_fill] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_fillbiome] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_forceload] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_function] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_gamemode] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_gamerule] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_give] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_help] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_item] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_jfr] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_kick] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_kill] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_list] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_locate] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_loot] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_me] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_msg] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_op] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_pardon] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_pardon_DASHip] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_particle] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_perf] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_place] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_playsound] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_publish] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_random] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_recipe] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_reload] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_return] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_ride] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_rotate] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_save_DASHall] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_save_DASHoff] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_save_DASHon] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_say] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_schedule] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_scoreboard] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_seed] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_setblock] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_setidletimeout] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_setworldspawn] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_spawnpoint] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_spectate] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_spreadplayers] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_stop] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_stopsound] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_summon] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_swing] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_tag] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_team] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_teammsg] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_teleport] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_tell] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_tellraw] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_test] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_time] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_title] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_tm] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_tp] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_transfer] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_trigger] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_version] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_w] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_waypoint] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_weather] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_whitelist] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_worldborder] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_xp] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_align] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_all] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_anchored] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_as] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_at] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_attacker] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_biome] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_block] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_blocks] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_controller] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_dimension] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_entity] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_eyes] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_facing] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_feet] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_if] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_in] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_items] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_leasher] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_loaded] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_masked] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_matches] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_max] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_on] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_origin] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_over] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_owner] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_passengers] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_positioned] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_predicate] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_result] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_rotated] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_score] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_storage] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_store] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_success] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_target] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_unless] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_value] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_vehicle] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_add] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_add_multiplied_base] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_add_multiplied_total] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_add_value] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_amount] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_append] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_base] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_buffer] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_by] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_center] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_color] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_create] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_day] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_daytime] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_destroy] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_disable] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_dismount] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_distance] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_enable] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_everything] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_export] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_exportclosest] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_exportthat] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_exportthese] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_fail] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_feature] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_fish] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_flush] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_force] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_from] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_gametime] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_get] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_grant] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_hollow] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_id] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_insert] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_ips] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_jigsaw] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_join] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_keep] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_leave] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_levels] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_mainhand] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_merge] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_midnight] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_mine] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_modifier] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_modify] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_mount] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_move] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_name] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_night] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_noon] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_normal] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_objectives] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_off] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_offhand] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_only] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_outline] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_players] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_poi] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_points] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_pos] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_prepend] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_query] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_rain] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_remove] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_replace] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_reset] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_resetclosest] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_resetthat] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_resetthese] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_revoke] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_roll] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_runclosest] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_runfailed] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_runthat] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_runthese] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_set] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_setdisplay] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_show] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_spawn] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_start] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_strict] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_string] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_structure] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_style] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_take] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_template] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_through] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_thunder] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_times] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_to] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_under] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_until] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_uuids] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_verify] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_visible] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_warning] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_with] = {
    .visible = true,
    .named = false,
  },
  [aux_sym_selector_token1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_selector_token2] = {
    .visible = false,
    .named = false,
  },
  [sym_selector_identifier] = {
    .visible = true,
    .named = true,
  },
  [sym_query_identifier] = {
    .visible = true,
    .named = true,
  },
  [anon_sym_EQ] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_EQ_BANG] = {
    .visible = true,
    .named = false,
  },
  [aux_sym_selector_query_token1] = {
    .visible = false,
    .named = false,
  },
  [anon_sym_TILDE] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_CARET] = {
    .visible = true,
    .named = false,
  },
  [sym_heightmap] = {
    .visible = true,
    .named = true,
  },
  [sym_resource] = {
    .visible = true,
    .named = true,
  },
  [anon_sym_DASH] = {
    .visible = true,
    .named = false,
  },
  [aux_sym_scale_token1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_scale_token2] = {
    .visible = false,
    .named = false,
  },
  [anon_sym_DOLLAR_LPAREN] = {
    .visible = true,
    .named = false,
  },
  [aux_sym_macro_token1] = {
    .visible = false,
    .named = false,
  },
  [anon_sym_RPAREN] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_PERCENT_LBRACK] = {
    .visible = true,
    .named = false,
  },
  [aux_sym_substitution_token1] = {
    .visible = false,
    .named = false,
  },
  [sym_type] = {
    .visible = true,
    .named = true,
  },
  [anon_sym_PLUS_EQ] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_DASH_EQ] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_STAR_EQ] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_SLASH_EQ] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_PERCENT_EQ] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_GT_LT] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_LT] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_GT] = {
    .visible = true,
    .named = false,
  },
  [aux_sym_slot_token1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_slot_token2] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_slot_token3] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_slot_token4] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_slot_token5] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_slot_token6] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_slot_token7] = {
    .visible = false,
    .named = false,
  },
  [anon_sym_contents] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_player_DOTcursor] = {
    .visible = true,
    .named = false,
  },
  [aux_sym_slot_token8] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_word_token1] = {
    .visible = false,
    .named = false,
  },
  [anon_sym_DQUOTE] = {
    .visible = true,
    .named = false,
  },
  [aux_sym_string_token1] = {
    .visible = false,
    .named = false,
  },
  [anon_sym_SQUOTE] = {
    .visible = true,
    .named = false,
  },
  [aux_sym_string_token2] = {
    .visible = false,
    .named = false,
  },
  [sym_boolean] = {
    .visible = true,
    .named = true,
  },
  [sym_range] = {
    .visible = true,
    .named = true,
  },
  [aux_sym_nbt_identifier_token1] = {
    .visible = false,
    .named = false,
  },
  [anon_sym_LBRACE] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_COLON] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_RBRACE] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_LBRACK] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_RBRACK] = {
    .visible = true,
    .named = false,
  },
  [sym_source_file] = {
    .visible = true,
    .named = true,
  },
  [sym__space] = {
    .visible = false,
    .named = true,
  },
  [sym__statement] = {
    .visible = false,
    .named = true,
  },
  [sym_command] = {
    .visible = true,
    .named = true,
  },
  [sym__execute_command] = {
    .visible = false,
    .named = true,
  },
  [sym__subcommand] = {
    .visible = false,
    .named = true,
  },
  [sym__generic_command] = {
    .visible = false,
    .named = true,
  },
  [sym_command_identifier] = {
    .visible = true,
    .named = true,
  },
  [sym_subcommand_identifier] = {
    .visible = true,
    .named = true,
  },
  [sym_argument] = {
    .visible = true,
    .named = true,
  },
  [sym_selector] = {
    .visible = true,
    .named = true,
  },
  [sym_selector_query] = {
    .visible = true,
    .named = true,
  },
  [sym_position] = {
    .visible = true,
    .named = true,
  },
  [sym_rotation] = {
    .visible = true,
    .named = true,
  },
  [sym_scale] = {
    .visible = true,
    .named = true,
  },
  [sym_macro] = {
    .visible = true,
    .named = true,
  },
  [sym_substitution] = {
    .visible = true,
    .named = true,
  },
  [sym_operator] = {
    .visible = true,
    .named = true,
  },
  [sym_slot] = {
    .visible = true,
    .named = true,
  },
  [sym_word] = {
    .visible = true,
    .named = true,
  },
  [sym_string] = {
    .visible = true,
    .named = true,
  },
  [sym__nbt_element] = {
    .visible = false,
    .named = true,
  },
  [sym_nbt_identifier] = {
    .visible = true,
    .named = true,
  },
  [sym_nbt_compound] = {
    .visible = true,
    .named = true,
  },
  [sym_nbt_array] = {
    .visible = true,
    .named = true,
  },
  [sym_selector_compound] = {
    .visible = true,
    .named = true,
  },
  [sym_item_compound] = {
    .visible = true,
    .named = true,
  },
  [aux_sym_source_file_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym__execute_command_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym__generic_command_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_selector_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_selector_query_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_nbt_compound_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_nbt_array_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_selector_compound_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_item_compound_repeat1] = {
    .visible = false,
    .named = false,
  },
};

static const TSSymbol ts_alias_sequences[PRODUCTION_ID_COUNT][MAX_ALIAS_SEQUENCE_LENGTH] = {
  [0] = {0},
};

static const uint16_t ts_non_terminal_alias_map[] = {
  0,
};

static const TSStateId ts_primary_state_ids[STATE_COUNT] = {
  [0] = 0,
  [1] = 1,
  [2] = 2,
  [3] = 3,
  [4] = 3,
  [5] = 5,
  [6] = 6,
  [7] = 7,
  [8] = 8,
  [9] = 9,
  [10] = 10,
  [11] = 11,
  [12] = 12,
  [13] = 13,
  [14] = 14,
  [15] = 15,
  [16] = 16,
  [17] = 17,
  [18] = 3,
  [19] = 3,
  [20] = 20,
  [21] = 21,
  [22] = 22,
  [23] = 23,
  [24] = 24,
  [25] = 25,
  [26] = 3,
  [27] = 27,
  [28] = 27,
  [29] = 27,
  [30] = 30,
  [31] = 31,
  [32] = 30,
  [33] = 30,
  [34] = 27,
  [35] = 30,
  [36] = 30,
  [37] = 27,
  [38] = 38,
  [39] = 39,
  [40] = 40,
  [41] = 41,
  [42] = 42,
  [43] = 43,
  [44] = 44,
  [45] = 45,
  [46] = 46,
  [47] = 25,
  [48] = 48,
  [49] = 24,
  [50] = 50,
  [51] = 51,
  [52] = 52,
  [53] = 53,
  [54] = 54,
  [55] = 55,
  [56] = 56,
  [57] = 57,
  [58] = 58,
  [59] = 59,
  [60] = 60,
  [61] = 61,
  [62] = 62,
  [63] = 3,
  [64] = 64,
  [65] = 65,
  [66] = 56,
  [67] = 67,
  [68] = 67,
  [69] = 69,
  [70] = 70,
  [71] = 71,
  [72] = 70,
  [73] = 73,
  [74] = 74,
  [75] = 74,
  [76] = 76,
  [77] = 77,
  [78] = 78,
  [79] = 79,
  [80] = 80,
  [81] = 78,
  [82] = 77,
  [83] = 52,
  [84] = 51,
  [85] = 78,
  [86] = 78,
  [87] = 80,
  [88] = 80,
  [89] = 80,
  [90] = 78,
  [91] = 59,
  [92] = 57,
  [93] = 60,
  [94] = 58,
  [95] = 61,
  [96] = 62,
  [97] = 97,
  [98] = 98,
  [99] = 99,
  [100] = 100,
  [101] = 101,
  [102] = 24,
  [103] = 103,
  [104] = 52,
  [105] = 51,
  [106] = 55,
  [107] = 107,
  [108] = 97,
  [109] = 109,
  [110] = 110,
  [111] = 111,
  [112] = 112,
  [113] = 113,
  [114] = 114,
  [115] = 115,
  [116] = 109,
  [117] = 101,
  [118] = 24,
  [119] = 119,
  [120] = 120,
  [121] = 121,
  [122] = 112,
  [123] = 123,
  [124] = 124,
  [125] = 125,
  [126] = 60,
  [127] = 61,
  [128] = 128,
  [129] = 129,
  [130] = 124,
  [131] = 58,
  [132] = 132,
  [133] = 62,
  [134] = 134,
  [135] = 135,
  [136] = 24,
  [137] = 128,
  [138] = 138,
  [139] = 55,
  [140] = 140,
  [141] = 141,
  [142] = 56,
  [143] = 143,
  [144] = 144,
  [145] = 57,
  [146] = 56,
  [147] = 59,
  [148] = 148,
  [149] = 149,
  [150] = 150,
  [151] = 151,
  [152] = 152,
  [153] = 153,
  [154] = 51,
  [155] = 155,
  [156] = 69,
  [157] = 25,
  [158] = 150,
  [159] = 52,
  [160] = 51,
  [161] = 56,
  [162] = 162,
  [163] = 56,
  [164] = 153,
  [165] = 52,
  [166] = 162,
  [167] = 167,
  [168] = 168,
  [169] = 169,
  [170] = 170,
  [171] = 171,
  [172] = 59,
  [173] = 173,
  [174] = 62,
  [175] = 175,
  [176] = 176,
  [177] = 177,
  [178] = 167,
  [179] = 57,
  [180] = 60,
  [181] = 61,
  [182] = 60,
  [183] = 58,
  [184] = 62,
  [185] = 185,
  [186] = 186,
  [187] = 175,
  [188] = 188,
  [189] = 185,
  [190] = 168,
  [191] = 191,
  [192] = 61,
  [193] = 193,
  [194] = 194,
  [195] = 57,
  [196] = 171,
  [197] = 197,
  [198] = 198,
  [199] = 188,
  [200] = 200,
  [201] = 177,
  [202] = 58,
  [203] = 197,
  [204] = 198,
  [205] = 205,
  [206] = 206,
  [207] = 207,
  [208] = 208,
  [209] = 209,
  [210] = 210,
  [211] = 211,
  [212] = 212,
  [213] = 213,
  [214] = 214,
  [215] = 215,
  [216] = 216,
  [217] = 217,
  [218] = 218,
  [219] = 218,
  [220] = 220,
  [221] = 221,
  [222] = 214,
  [223] = 221,
  [224] = 213,
  [225] = 225,
  [226] = 218,
  [227] = 214,
  [228] = 218,
  [229] = 221,
  [230] = 214,
  [231] = 221,
  [232] = 232,
  [233] = 218,
  [234] = 234,
  [235] = 221,
  [236] = 214,
  [237] = 237,
  [238] = 238,
  [239] = 221,
  [240] = 214,
  [241] = 213,
  [242] = 225,
  [243] = 213,
  [244] = 225,
  [245] = 245,
  [246] = 225,
  [247] = 213,
  [248] = 225,
  [249] = 213,
  [250] = 225,
  [251] = 251,
  [252] = 252,
  [253] = 253,
};

static bool ts_lex(TSLexer *lexer, TSStateId state) {
  START_LEXER();
  eof = lexer->eof(lexer);
  switch (state) {
    case 0:
      if (eof) ADVANCE(899);
      ADVANCE_MAP(
        '\n', 901,
        '\r', 3,
        ' ', 904,
        '"', 2068,
        '#', 908,
        '$', 914,
        '%', 511,
        '\'', 2073,
        ')', 1333,
        '*', 512,
        '+', 513,
        '-', 1307,
        '.', 31,
        '/', 514,
        '0', 1316,
        '1', 1316,
        ':', 2085,
        '<', 1345,
        '=', 1297,
        '>', 1346,
        '@', 896,
        '[', 2087,
        '\\', 5,
        ']', 2088,
        '^', 1301,
        'a', 100,
        'b', 41,
        'c', 123,
        'e', 213,
        'f', 1322,
        'g', 42,
        'i', 101,
        'j', 210,
        'k', 167,
        'l', 124,
        'n', 61,
        'o', 76,
        'p', 43,
        'q', 496,
        'r', 44,
        't', 1323,
        'u', 319,
        'v', 48,
        'w', 1043,
        'x', 376,
        '{', 2084,
        '}', 2086,
        '~', 1300,
        '\t', 516,
        0x0b, 516,
        '\f', 516,
        '_', 510,
        'y', 510,
        'z', 510,
        'd', 1324,
        'h', 1324,
        'm', 1324,
        's', 1324,
      );
      if (('2' <= lookahead && lookahead <= '9')) ADVANCE(1317);
      END_STATE();
    case 1:
      ADVANCE_MAP(
        '\n', 901,
        '\r', 3,
        ' ', 904,
        '"', 2068,
        '$', 25,
        '%', 515,
        '\'', 2073,
        '=', 1296,
        '[', 2087,
        '\\', 5,
        '\t', 516,
        0x0b, 516,
        '\f', 516,
      );
      if (('!' <= lookahead && lookahead <= '#') ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 2:
      ADVANCE_MAP(
        '\n', 901,
        '\r', 3,
        ' ', 904,
        '$', 25,
        '%', 515,
        '-', 1306,
        '.', 889,
        '[', 1290,
        '\\', 5,
        '\t', 516,
        0x0b, 516,
        '\f', 516,
        'd', 1321,
        'f', 1321,
        'h', 1321,
        'm', 1321,
        's', 1321,
        't', 1321,
      );
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1311);
      END_STATE();
    case 3:
      if (lookahead == '\n') ADVANCE(901);
      if (lookahead == '\\') ADVANCE(5);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(516);
      END_STATE();
    case 4:
      if (lookahead == '\n') ADVANCE(905);
      END_STATE();
    case 5:
      if (lookahead == '\n') ADVANCE(905);
      if (lookahead == '\r') ADVANCE(4);
      END_STATE();
    case 6:
      if (lookahead == '\n') ADVANCE(900);
      END_STATE();
    case 7:
      ADVANCE_MAP(
        '\n', 900,
        '\r', 6,
        '#', 908,
        '$', 914,
        '%', 515,
        'a', 581,
        'b', 525,
        'c', 701,
        'd', 519,
        'e', 653,
        'f', 628,
        'g', 520,
        'h', 625,
        'i', 853,
        'j', 654,
        'k', 678,
        'l', 680,
        'm', 600,
        'o', 787,
        'p', 533,
        'r', 531,
        's', 521,
        't', 528,
        'v', 643,
        'w', 1044,
        'x', 788,
      );
      END_STATE();
    case 8:
      ADVANCE_MAP(
        ' ', 904,
        '"', 2068,
        '$', 25,
        '%', 511,
        '\'', 2073,
        '*', 512,
        '+', 2053,
        '-', 1309,
        '.', 2066,
        '/', 514,
        '0', 1313,
        '1', 1313,
        '<', 1345,
        '=', 1297,
        '>', 1346,
        '@', 896,
        '[', 2087,
        '\\', 5,
        '^', 1301,
        'a', 1713,
        'b', 1666,
        'c', 1857,
        'd', 1429,
        'e', 1810,
        'f', 1398,
        'h', 1821,
        'i', 1621,
        'l', 1590,
        'm', 1399,
        'o', 1476,
        'p', 1419,
        'r', 1585,
        's', 1484,
        't', 1452,
        'u', 1814,
        'v', 1403,
        'w', 1561,
        '{', 2084,
        '~', 1300,
      );
      if (('\t' <= lookahead && lookahead <= '\r')) ADVANCE(516);
      if (('2' <= lookahead && lookahead <= '9')) ADVANCE(1314);
      if (lookahead == '_' ||
          ('g' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (('!' <= lookahead && lookahead <= '#') ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 9:
      ADVANCE_MAP(
        ' ', 904,
        '"', 2068,
        '$', 25,
        '%', 511,
        '\'', 2073,
        '*', 512,
        '+', 2053,
        '-', 1309,
        '.', 2066,
        '/', 514,
        '0', 1313,
        '1', 1313,
        '<', 1345,
        '=', 1296,
        '>', 1346,
        '@', 896,
        '[', 2087,
        '\\', 5,
        '^', 1301,
        'a', 1489,
        'b', 1392,
        'c', 1515,
        'd', 1393,
        'e', 1767,
        'f', 1394,
        'g', 1401,
        'h', 1818,
        'i', 1490,
        'j', 1662,
        'k', 1560,
        'l', 1516,
        'm', 1395,
        'n', 1434,
        'o', 1456,
        'p', 1714,
        'q', 2028,
        'r', 1432,
        's', 1559,
        't', 1396,
        'u', 1768,
        'v', 1402,
        'w', 1406,
        '{', 2084,
        '~', 1300,
      );
      if (lookahead == '_' ||
          ('x' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (('\t' <= lookahead && lookahead <= '\r')) ADVANCE(516);
      if (('2' <= lookahead && lookahead <= '9')) ADVANCE(1314);
      if (('!' <= lookahead && lookahead <= '#') ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 10:
      ADVANCE_MAP(
        ' ', 904,
        '"', 2068,
        '$', 25,
        '%', 511,
        '\'', 2073,
        '*', 512,
        '+', 2053,
        '-', 1309,
        '.', 2066,
        '/', 514,
        '0', 1313,
        '1', 1313,
        '<', 1345,
        '=', 1296,
        '>', 1346,
        '@', 896,
        '[', 2087,
        '\\', 5,
        '^', 1301,
        'a', 1872,
        'b', 2049,
        'c', 1858,
        'd', 1822,
        'e', 1812,
        'f', 1422,
        'h', 1821,
        'i', 1782,
        'l', 1841,
        'm', 1860,
        'o', 1477,
        'p', 1749,
        's', 1659,
        't', 1915,
        'v', 1688,
        'w', 1561,
        '{', 2084,
        '~', 1300,
      );
      if (('\t' <= lookahead && lookahead <= '\r')) ADVANCE(516);
      if (('2' <= lookahead && lookahead <= '9')) ADVANCE(1314);
      if (lookahead == '_' ||
          ('g' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (('!' <= lookahead && lookahead <= '#') ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 11:
      ADVANCE_MAP(
        ' ', 904,
        '"', 2068,
        '$', 25,
        '%', 515,
        '\'', 2073,
        '-', 1308,
        '.', 31,
        '0', 1316,
        '1', 1316,
        ':', 2085,
        '=', 1297,
        '[', 2087,
        '\\', 5,
        'f', 546,
        't', 814,
        '{', 2084,
      );
      if (('\t' <= lookahead && lookahead <= '\r')) ADVANCE(516);
      if (('2' <= lookahead && lookahead <= '9')) ADVANCE(1317);
      END_STATE();
    case 12:
      ADVANCE_MAP(
        ' ', 904,
        '"', 2068,
        '$', 25,
        '%', 515,
        '\'', 2073,
        '-', 1308,
        '.', 31,
        '0', 1316,
        '1', 1316,
        ':', 2085,
        '=', 1296,
        '[', 2087,
        '\\', 5,
        'f', 546,
        't', 814,
        '{', 2084,
      );
      if (('\t' <= lookahead && lookahead <= '\r')) ADVANCE(516);
      if (('2' <= lookahead && lookahead <= '9')) ADVANCE(1317);
      END_STATE();
    case 13:
      ADVANCE_MAP(
        ' ', 903,
        '"', 2068,
        '$', 914,
        '%', 515,
        '\'', 2073,
        ',', 1299,
        '-', 1308,
        '.', 31,
        '0', 1316,
        '1', 1316,
        '[', 2087,
        '\\', 5,
        ']', 2088,
        'a', 581,
        'b', 525,
        'c', 701,
        'd', 519,
        'e', 653,
        'f', 547,
        'g', 520,
        'h', 625,
        'i', 853,
        'j', 654,
        'k', 678,
        'l', 680,
        'm', 600,
        'o', 787,
        'p', 533,
        'r', 531,
        's', 521,
        't', 527,
        'v', 643,
        'w', 1044,
        'x', 788,
        '{', 2084,
      );
      if (('\t' <= lookahead && lookahead <= '\r')) ADVANCE(26);
      if (('2' <= lookahead && lookahead <= '9')) ADVANCE(1317);
      END_STATE();
    case 14:
      ADVANCE_MAP(
        ' ', 903,
        '"', 2068,
        '$', 25,
        '%', 515,
        '\'', 2073,
        ',', 1299,
        '-', 1308,
        '.', 31,
        '0', 1316,
        '1', 1316,
        '[', 2087,
        '\\', 5,
        ']', 2088,
        'f', 1326,
        't', 1327,
        '{', 2084,
        'd', 1321,
        'h', 1321,
        'm', 1321,
        's', 1321,
      );
      if (('\t' <= lookahead && lookahead <= '\r')) ADVANCE(26);
      if (('2' <= lookahead && lookahead <= '9')) ADVANCE(1317);
      END_STATE();
    case 15:
      ADVANCE_MAP(
        ' ', 903,
        '"', 2068,
        '$', 25,
        '%', 515,
        '\'', 2073,
        ',', 1299,
        '=', 1296,
        '\\', 5,
        ']', 2088,
        '}', 2086,
      );
      if (('\t' <= lookahead && lookahead <= '\r')) ADVANCE(26);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2082);
      if (('!' <= lookahead && lookahead <= '#') ||
          ('+' <= lookahead && lookahead <= '.') ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2083);
      END_STATE();
    case 16:
      ADVANCE_MAP(
        '"', 2068,
        '#', 2067,
        '$', 25,
        '%', 515,
        '\'', 2073,
        ',', 1299,
        '-', 1295,
        '[', 2087,
        ']', 1291,
      );
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(27);
      if (lookahead == '!' ||
          ('+' <= lookahead && lookahead <= '.') ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1293);
      END_STATE();
    case 17:
      ADVANCE_MAP(
        '"', 2068,
        '$', 25,
        '%', 511,
        '\'', 2073,
        '*', 512,
        '+', 2053,
        '-', 1309,
        '.', 2066,
        '/', 514,
        '0', 1313,
        '1', 1313,
        '<', 1345,
        '>', 1346,
        '@', 896,
        '[', 2087,
        '^', 1301,
        'a', 1713,
        'b', 1666,
        'c', 1857,
        'd', 1429,
        'e', 1810,
        'f', 1398,
        'h', 1821,
        'i', 1621,
        'l', 1590,
        'm', 1399,
        'o', 1476,
        'p', 1419,
        'r', 1586,
        's', 1484,
        't', 1452,
        'u', 1814,
        'v', 1403,
        'w', 1561,
        '{', 2084,
        '~', 1300,
      );
      if (('2' <= lookahead && lookahead <= '9')) ADVANCE(1314);
      if (lookahead == '_' ||
          ('g' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (('!' <= lookahead && lookahead <= '#') ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 18:
      ADVANCE_MAP(
        '"', 2068,
        '$', 25,
        '%', 515,
        '\'', 2073,
        ',', 1299,
        ']', 2088,
        '}', 2086,
        'd', 1325,
        'f', 1325,
        'h', 1325,
        'm', 1325,
        's', 1325,
        't', 1325,
      );
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(27);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2082);
      if (('!' <= lookahead && lookahead <= '#') ||
          ('+' <= lookahead && lookahead <= '.') ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2083);
      END_STATE();
    case 19:
      ADVANCE_MAP(
        '"', 2068,
        '$', 25,
        '%', 515,
        '\'', 2073,
        ',', 1299,
        '}', 2086,
        'd', 1328,
        'f', 1328,
        'h', 1328,
        'm', 1328,
        's', 1328,
        't', 1328,
      );
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(27);
      if (('!' <= lookahead && lookahead <= '#') ||
          ('+' <= lookahead && lookahead <= '.') ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2083);
      END_STATE();
    case 20:
      if (lookahead == '"') ADVANCE(2068);
      if (lookahead == '$') ADVANCE(25);
      if (lookahead == '%') ADVANCE(515);
      if (lookahead == '\'') ADVANCE(2073);
      if (lookahead == ',') ADVANCE(1299);
      if (lookahead == '}') ADVANCE(2086);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(27);
      if (('!' <= lookahead && lookahead <= '#') ||
          ('+' <= lookahead && lookahead <= '.') ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2083);
      END_STATE();
    case 21:
      if (lookahead == '$') ADVANCE(25);
      if (lookahead == '%') ADVANCE(515);
      if (lookahead == ',') ADVANCE(1299);
      if (lookahead == '-') ADVANCE(1310);
      if (lookahead == '.') ADVANCE(1294);
      if (lookahead == ']') ADVANCE(1291);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(27);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1315);
      if (lookahead == '!' ||
          lookahead == '+' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1295);
      END_STATE();
    case 22:
      ADVANCE_MAP(
        '$', 25,
        '%', 515,
        ',', 1299,
        '[', 1290,
        ']', 1291,
        'd', 1295,
        'f', 1295,
        'h', 1295,
        'm', 1295,
        's', 1295,
        't', 1295,
      );
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(27);
      if (lookahead == '!' ||
          ('+' <= lookahead && lookahead <= '.') ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1295);
      END_STATE();
    case 23:
      if (lookahead == '$') ADVANCE(25);
      if (lookahead == '%') ADVANCE(515);
      if (('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1332);
      END_STATE();
    case 24:
      if (lookahead == '$') ADVANCE(25);
      if (lookahead == '%') ADVANCE(515);
      if (('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(517);
      END_STATE();
    case 25:
      if (lookahead == '(') ADVANCE(1329);
      END_STATE();
    case 26:
      if (lookahead == ',') ADVANCE(1299);
      if (lookahead == '\\') ADVANCE(5);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(26);
      END_STATE();
    case 27:
      if (lookahead == ',') ADVANCE(1299);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(27);
      END_STATE();
    case 28:
      if (lookahead == '-') ADVANCE(890);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(2081);
      END_STATE();
    case 29:
      if (lookahead == '-') ADVANCE(551);
      END_STATE();
    case 30:
      if (lookahead == '.') ADVANCE(28);
      END_STATE();
    case 31:
      if (lookahead == '.') ADVANCE(28);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1318);
      END_STATE();
    case 32:
      if (lookahead == '.') ADVANCE(2080);
      END_STATE();
    case 33:
      if (lookahead == '.') ADVANCE(563);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(1223);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 34:
      if (lookahead == '.') ADVANCE(891);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 35:
      if (lookahead == '.') ADVANCE(32);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(35);
      END_STATE();
    case 36:
      if (lookahead == '.') ADVANCE(892);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 37:
      if (lookahead == '.') ADVANCE(893);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 38:
      if (lookahead == '.') ADVANCE(894);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 39:
      if (lookahead == '.') ADVANCE(895);
      END_STATE();
    case 40:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_') ADVANCE(79);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 41:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(320);
      if (lookahead == 'i') ADVANCE(370);
      if (lookahead == 'l') ADVANCE(354);
      if (lookahead == 'o') ADVANCE(424);
      if (lookahead == 'u') ADVANCE(216);
      if (lookahead == 'y') ADVANCE(1142);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 42:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(313);
      if (lookahead == 'e') ADVANCE(445);
      if (lookahead == 'i') ADVANCE(500);
      if (lookahead == 'r') ADVANCE(60);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 43:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(383);
      if (lookahead == 'e') ADVANCE(401);
      if (lookahead == 'l') ADVANCE(46);
      if (lookahead == 'o') ADVANCE(239);
      if (lookahead == 'r') ADVANCE(125);
      if (lookahead == 'u') ADVANCE(77);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 44:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(248);
      if (lookahead == 'e') ADVANCE(89);
      if (lookahead == 'i') ADVANCE(118);
      if (lookahead == 'o') ADVANCE(285);
      if (lookahead == 'u') ADVANCE(321);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 45:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(425);
      if (lookahead == 'v') ADVANCE(197);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 46:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(92);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 47:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(503);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 48:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(301);
      if (lookahead == 'e') ADVANCE(229);
      if (lookahead == 'i') ADVANCE(300);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 49:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(224);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 50:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(385);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 51:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(105);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 52:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(332);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 53:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(335);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 54:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(280);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 55:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(281);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 56:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(392);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 57:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(459);
      if (lookahead == 'e') ADVANCE(437);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 58:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(461);
      if (lookahead == 'e') ADVANCE(438);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 59:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(463);
      if (lookahead == 'e') ADVANCE(439);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 60:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(337);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 61:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(315);
      if (lookahead == 'i') ADVANCE(220);
      if (lookahead == 'o') ADVANCE(357);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 62:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(78);
      if (lookahead == 'c') ADVANCE(238);
      if (lookahead == 'd') ADVANCE(173);
      if (lookahead == 't') ADVANCE(251);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 63:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(119);
      if (lookahead == 'c') ADVANCE(69);
      if (lookahead == 'o') ADVANCE(448);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 64:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(88);
      if (lookahead == 'r') ADVANCE(241);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 65:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(488);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 66:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(94);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 67:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(268);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 68:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(478);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 69:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(479);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 70:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(480);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 71:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(270);
      if (lookahead == 'e') ADVANCE(342);
      if (lookahead == 'r') ADVANCE(362);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 72:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(343);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 73:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(440);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 74:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(483);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 75:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(304);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 76:
      ADVANCE_MAP(
        ':', 897,
        'b', 272,
        'f', 211,
        'n', 1096,
        'p', 981,
        'r', 244,
        'u', 468,
        'v', 169,
        'w', 350,
      );
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 77:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'b') ADVANCE(289);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 78:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'b') ADVANCE(294);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 79:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'b') ADVANCE(73);
      if (lookahead == 't') ADVANCE(368);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 80:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'b') ADVANCE(56);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 81:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'b') ADVANCE(296);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 82:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'b') ADVANCE(367);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 83:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'b') ADVANCE(499);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 84:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'c') ADVANCE(233);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 85:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'c') ADVANCE(273);
      if (lookahead == 'l') ADVANCE(277);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 86:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'c') ADVANCE(274);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 87:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'c') ADVANCE(498);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 88:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'c') ADVANCE(276);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 89:
      ADVANCE_MAP(
        ':', 897,
        'c', 259,
        'l', 360,
        'm', 369,
        'p', 287,
        's', 182,
        't', 489,
        'v', 355,
      );
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 90:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'c') ADVANCE(231);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 91:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'c') ADVANCE(453);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 92:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'c') ADVANCE(135);
      if (lookahead == 'y') ADVANCE(188);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 93:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'c') ADVANCE(205);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 94:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'c') ADVANCE(146);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 95:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'c') ADVANCE(157);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 96:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'c') ADVANCE(295);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 97:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'c') ADVANCE(475);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 98:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'c') ADVANCE(298);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 99:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'c') ADVANCE(74);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 100:
      ADVANCE_MAP(
        ':', 897,
        'd', 102,
        'l', 240,
        'm', 353,
        'n', 84,
        'p', 378,
        's', 1061,
        't', 1063,
      );
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 101:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'd') ADVANCE(1181);
      if (lookahead == 'f') ADVANCE(1083);
      if (lookahead == 'n') ADVANCE(1085);
      if (lookahead == 'p') ADVANCE(414);
      if (lookahead == 't') ADVANCE(161);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 102:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'd') ADVANCE(1126);
      if (lookahead == 'v') ADVANCE(52);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 103:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'd') ADVANCE(1136);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 104:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'd') ADVANCE(1091);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 105:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'd') ADVANCE(1000);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 106:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'd') ADVANCE(1217);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 107:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'd') ADVANCE(1231);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 108:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'd') ADVANCE(1059);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 109:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'd') ADVANCE(992);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 110:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'd') ADVANCE(1255);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 111:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'd') ADVANCE(1107);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 112:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'd') ADVANCE(40);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 113:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'd') ADVANCE(82);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 114:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'd') ADVANCE(361);
      if (lookahead == 't') ADVANCE(271);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 115:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'd') ADVANCE(416);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 116:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'd') ADVANCE(356);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 117:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'd') ADVANCE(249);
      if (lookahead == 'p') ADVANCE(185);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 118:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'd') ADVANCE(130);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 119:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'd') ADVANCE(172);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 120:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'd') ADVANCE(149);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 121:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'd') ADVANCE(183);
      if (lookahead == 'l') ADVANCE(196);
      if (lookahead == 't') ADVANCE(254);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 122:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'd') ADVANCE(195);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 123:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(330);
      if (lookahead == 'l') ADVANCE(168);
      if (lookahead == 'o') ADVANCE(282);
      if (lookahead == 'r') ADVANCE(170);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 124:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(45);
      if (lookahead == 'i') ADVANCE(426);
      if (lookahead == 'o') ADVANCE(63);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 125:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(117);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 126:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(1138);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 127:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(314);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 128:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(959);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 129:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(1205);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 130:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(1005);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 131:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(2079);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 132:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(1068);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 133:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(931);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 134:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(1193);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 135:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(990);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 136:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(1122);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 137:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(1148);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 138:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(1156);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 139:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(974);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 140:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(998);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 141:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(1237);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 142:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(1249);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 143:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(1007);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 144:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(910);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 145:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(1221);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 146:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(1239);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 147:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(1124);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 148:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(1284);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 149:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(955);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 150:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(957);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 151:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(1174);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 152:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(986);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 153:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(1259);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 154:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(1132);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 155:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(918);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 156:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(1109);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 157:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(947);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 158:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(1247);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 159:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(1166);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 160:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(1128);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 161:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(309);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 162:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(377);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 163:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(87);
      if (lookahead == 'p') ADVANCE(175);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 164:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(91);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 165:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(399);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 166:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(415);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 167:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(162);
      if (lookahead == 'i') ADVANCE(85);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 168:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(50);
      if (lookahead == 'o') ADVANCE(340);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 169:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(384);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 170:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(68);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 171:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(400);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 172:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(104);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 173:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(411);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 174:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(331);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 175:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(405);
      if (lookahead == 'o') ADVANCE(408);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 176:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(108);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 177:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(387);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 178:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(110);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 179:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(111);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 180:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(421);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 181:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(352);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 182:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(451);
      if (lookahead == 'u') ADVANCE(292);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 183:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(388);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 184:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(112);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 185:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(336);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 186:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(389);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 187:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(390);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 188:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(391);
      if (lookahead == 's') ADVANCE(358);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 189:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(393);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 190:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(394);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 191:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(395);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 192:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(396);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 193:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(406);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 194:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(397);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 195:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(398);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 196:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(429);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 197:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(286);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 198:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(97);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 199:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(306);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 200:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(348);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 201:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(409);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 202:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(412);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 203:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(341);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 204:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(433);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 205:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(318);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 206:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(434);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 207:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(435);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 208:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(436);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 209:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'e') ADVANCE(345);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 210:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'f') ADVANCE(382);
      if (lookahead == 'i') ADVANCE(221);
      if (lookahead == 'o') ADVANCE(243);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 211:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'f') ADVANCE(1215);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 212:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'f') ADVANCE(988);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 213:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'f') ADVANCE(215);
      if (lookahead == 'n') ADVANCE(62);
      if (lookahead == 'v') ADVANCE(165);
      if (lookahead == 'x') ADVANCE(163);
      if (lookahead == 'y') ADVANCE(166);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 214:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'f') ADVANCE(507);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 215:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'f') ADVANCE(164);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 216:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'f') ADVANCE(217);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 217:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'f') ADVANCE(186);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 218:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'g') ADVANCE(1286);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 219:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'g') ADVANCE(1158);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 220:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'g') ADVANCE(230);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 221:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'g') ADVANCE(427);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 222:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'g') ADVANCE(325);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 223:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'g') ADVANCE(253);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 224:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'g') ADVANCE(192);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 225:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'g') ADVANCE(193);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 226:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'h') ADVANCE(1288);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 227:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'h') ADVANCE(57);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 228:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'h') ADVANCE(994);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 229:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'h') ADVANCE(246);
      if (lookahead == 'r') ADVANCE(247);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 230:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'h') ADVANCE(450);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 231:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'h') ADVANCE(204);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 232:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'h') ADVANCE(58);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 233:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'h') ADVANCE(363);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 234:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'h') ADVANCE(59);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 235:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'h') ADVANCE(257);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 236:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'h') ADVANCE(189);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 237:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'h') ADVANCE(190);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 238:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'h') ADVANCE(72);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 239:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(1225);
      if (lookahead == 's') ADVANCE(1229);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 240:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(222);
      if (lookahead == 'l') ADVANCE(1057);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 241:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(83);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 242:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(115);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 243:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(322);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 244:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(223);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 245:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(381);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 246:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(96);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 247:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(214);
      if (lookahead == 's') ADVANCE(261);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 248:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(324);
      if (lookahead == 'n') ADVANCE(116);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 249:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(99);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 250:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(477);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 251:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(470);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 252:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(428);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 253:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(326);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 254:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(279);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 255:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(373);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 256:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(333);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 257:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(334);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 258:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(184);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 259:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(380);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 260:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(431);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 261:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(365);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 262:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(502);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 263:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(81);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 264:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(346);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 265:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(432);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 266:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(317);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 267:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(200);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 268:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(299);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 269:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(344);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 270:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(351);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 271:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(98);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 272:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'j') ADVANCE(198);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 273:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'k') ADVANCE(967);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 274:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'k') ADVANCE(1070);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 275:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'k') ADVANCE(142);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 276:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'k') ADVANCE(191);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 277:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(969);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 278:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(1251);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 279:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(1278);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 280:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(1211);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 281:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(1130);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 282:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(359);
      if (lookahead == 'n') ADVANCE(446);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 283:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(113);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 284:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(430);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 285:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(278);
      if (lookahead == 't') ADVANCE(70);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 286:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(417);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 287:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(66);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 288:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(49);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 289:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(252);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 290:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(372);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 291:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(305);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 292:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(456);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 293:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(258);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 294:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(138);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 295:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(147);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 296:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(148);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 297:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(150);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 298:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(152);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 299:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(178);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 300:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(288);
      if (lookahead == 's') ADVANCE(263);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 301:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(492);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 302:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(264);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 303:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(476);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 304:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(493);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 305:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(194);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 306:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(265);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 307:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(374);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 308:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(375);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 309:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'm') ADVANCE(963);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 310:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'm') ADVANCE(996);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 311:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'm') ADVANCE(494);
      if (lookahead == 'v') ADVANCE(75);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 312:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'm') ADVANCE(54);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 313:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'm') ADVANCE(127);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 314:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'm') ADVANCE(371);
      if (lookahead == 'r') ADVANCE(497);
      if (lookahead == 't') ADVANCE(266);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 315:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'm') ADVANCE(129);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 316:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'm') ADVANCE(132);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 317:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'm') ADVANCE(151);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 318:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'm') ADVANCE(209);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 319:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(121);
      if (lookahead == 'u') ADVANCE(242);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 320:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(919);
      if (lookahead == 's') ADVANCE(126);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 321:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(911);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 322:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(1189);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 323:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(1209);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 324:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(1235);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 325:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(1055);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 326:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(1099);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 327:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(983);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 328:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(1003);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 329:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(1042);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 330:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(487);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 331:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(103);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 332:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(93);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 333:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(218);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 334:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(219);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 335:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(106);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 336:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(107);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 337:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(449);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 338:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(109);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 339:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(452);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 340:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(133);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 341:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(485);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 342:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(473);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 343:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(458);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 344:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(460);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 345:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(465);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 346:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(145);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 347:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(179);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 348:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(95);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 349:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(256);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 350:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(177);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 351:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(202);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 352:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(225);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 353:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'o') ADVANCE(495);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 354:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'o') ADVANCE(86);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 355:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'o') ADVANCE(275);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 356:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'o') ADVANCE(310);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 357:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'o') ADVANCE(323);
      if (lookahead == 'r') ADVANCE(312);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 358:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'o') ADVANCE(491);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 359:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'o') ADVANCE(386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 360:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'o') ADVANCE(51);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 361:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'o') ADVANCE(327);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 362:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'o') ADVANCE(291);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 363:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'o') ADVANCE(410);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 364:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'o') ADVANCE(269);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 365:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'o') ADVANCE(329);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 366:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'o') ADVANCE(402);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 367:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'o') ADVANCE(413);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 368:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'o') ADVANCE(484);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 369:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'o') ADVANCE(501);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 370:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'o') ADVANCE(316);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 371:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'o') ADVANCE(120);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 372:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'o') ADVANCE(442);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 373:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'o') ADVANCE(347);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 374:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'o') ADVANCE(443);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 375:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'o') ADVANCE(444);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 376:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'p') ADVANCE(1054);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 377:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'p') ADVANCE(1191);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 378:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'p') ADVANCE(174);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 379:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'p') ADVANCE(364);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 380:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'p') ADVANCE(140);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 381:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'p') ADVANCE(293);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 382:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(965);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 383:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(114);
      if (lookahead == 's') ADVANCE(441);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 384:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(1101);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 385:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(928);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 386:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(1146);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 387:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(1103);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 388:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(1276);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 389:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(1140);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 390:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(1144);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 391:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(33);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 392:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(925);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 393:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(1089);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 394:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(1048);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 395:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(1066);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 396:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(34);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 397:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(1074);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 398:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(1052);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 399:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(509);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 400:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(505);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 401:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(212);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 402:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(508);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 403:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(349);
      if (lookahead == 'y') ADVANCE(379);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 404:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(283);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 405:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(267);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 406:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(422);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 407:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(328);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 408:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(454);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 409:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(455);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 410:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(176);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 411:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(90);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 412:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(36);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 413:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(122);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 414:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(1185);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 415:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(1079);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 416:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(1280);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 417:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(1195);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 418:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(1227);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 419:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(1120);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 420:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(1362);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 421:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(1213);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 422:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(1105);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 423:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(80);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 424:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(423);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 425:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(236);
      if (lookahead == 'v') ADVANCE(134);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 426:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(447);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 427:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(47);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 428:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(228);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 429:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(419);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 430:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(131);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 431:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(457);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 432:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(462);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 433:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(486);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 434:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(464);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 435:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(466);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 436:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(467);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 437:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(153);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 438:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(158);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 439:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(159);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 440:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(160);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 441:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(181);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 442:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(206);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 443:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(207);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 444:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(208);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 445:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(1176);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 446:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(71);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 447:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(971);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 448:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(977);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 449:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(1178);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 450:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(1207);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 451:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(1241);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 452:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(1134);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 453:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(943);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 454:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(1160);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 455:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(1183);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 456:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(1111);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 457:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(923);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 458:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(945);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 459:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(1257);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 460:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(1046);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 461:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(1245);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 462:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(1050);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 463:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(1164);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 464:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(1253);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 465:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(916);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 466:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(1243);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 467:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(1162);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 468:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(302);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 469:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(226);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 470:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(506);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 471:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(235);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 472:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(418);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 473:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(420);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 474:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(255);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 475:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(262);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 476:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(245);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 477:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(199);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 478:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(137);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 479:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(139);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 480:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(143);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 481:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(144);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 482:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(155);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 483:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(156);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 484:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(55);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 485:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(366);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 486:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(38);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 487:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(187);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 488:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(237);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 489:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'u') ADVANCE(407);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 490:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'u') ADVANCE(131);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 491:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'u') ADVANCE(338);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 492:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'u') ADVANCE(136);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 493:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'u') ADVANCE(154);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 494:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'u') ADVANCE(303);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 495:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'u') ADVANCE(339);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 496:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'u') ADVANCE(171);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 497:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'u') ADVANCE(297);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 498:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'u') ADVANCE(481);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 499:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'u') ADVANCE(482);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 500:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'v') ADVANCE(128);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 501:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'v') ADVANCE(141);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 502:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'v') ADVANCE(180);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 503:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'w') ADVANCE(1187);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 504:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'y') ADVANCE(1219);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 505:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'y') ADVANCE(1233);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 506:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'y') ADVANCE(1077);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 507:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'y') ADVANCE(1282);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 508:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'y') ADVANCE(37);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 509:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'y') ADVANCE(471);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 510:
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 511:
      if (lookahead == '=') ADVANCE(1343);
      if (lookahead == '[') ADVANCE(1334);
      END_STATE();
    case 512:
      if (lookahead == '=') ADVANCE(1341);
      END_STATE();
    case 513:
      if (lookahead == '=') ADVANCE(1339);
      END_STATE();
    case 514:
      if (lookahead == '=') ADVANCE(1342);
      END_STATE();
    case 515:
      if (lookahead == '[') ADVANCE(1334);
      END_STATE();
    case 516:
      if (lookahead == '\\') ADVANCE(5);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(516);
      END_STATE();
    case 517:
      if (lookahead == ']') ADVANCE(1337);
      if (('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(517);
      END_STATE();
    case 518:
      if (lookahead == 'a') ADVANCE(650);
      END_STATE();
    case 519:
      if (lookahead == 'a') ADVANCE(729);
      if (lookahead == 'e') ADVANCE(556);
      if (lookahead == 'i') ADVANCE(532);
      END_STATE();
    case 520:
      if (lookahead == 'a') ADVANCE(730);
      if (lookahead == 'i') ADVANCE(881);
      END_STATE();
    case 521:
      if (lookahead == 'a') ADVANCE(882);
      if (lookahead == 'c') ADVANCE(673);
      if (lookahead == 'e') ADVANCE(627);
      if (lookahead == 'p') ADVANCE(523);
      if (lookahead == 't') ADVANCE(765);
      if (lookahead == 'u') ADVANCE(728);
      if (lookahead == 'w') ADVANCE(688);
      END_STATE();
    case 522:
      if (lookahead == 'a') ADVANCE(572);
      END_STATE();
    case 523:
      if (lookahead == 'a') ADVANCE(884);
      if (lookahead == 'e') ADVANCE(580);
      if (lookahead == 'r') ADVANCE(635);
      END_STATE();
    case 524:
      if (lookahead == 'a') ADVANCE(935);
      END_STATE();
    case 525:
      if (lookahead == 'a') ADVANCE(740);
      if (lookahead == 'o') ADVANCE(828);
      END_STATE();
    case 526:
      if (lookahead == 'a') ADVANCE(883);
      END_STATE();
    case 527:
      if (lookahead == 'a') ADVANCE(661);
      if (lookahead == 'e') ADVANCE(535);
      if (lookahead == 'i') ADVANCE(732);
      if (lookahead == 'm') ADVANCE(1037);
      if (lookahead == 'p') ADVANCE(1038);
      if (lookahead == 'r') ADVANCE(537);
      END_STATE();
    case 528:
      if (lookahead == 'a') ADVANCE(661);
      if (lookahead == 'e') ADVANCE(535);
      if (lookahead == 'i') ADVANCE(732);
      if (lookahead == 'm') ADVANCE(1037);
      if (lookahead == 'p') ADVANCE(1038);
      if (lookahead == 'r') ADVANCE(536);
      END_STATE();
    case 529:
      if (lookahead == 'a') ADVANCE(888);
      END_STATE();
    case 530:
      if (lookahead == 'a') ADVANCE(869);
      END_STATE();
    case 531:
      if (lookahead == 'a') ADVANCE(747);
      if (lookahead == 'e') ADVANCE(577);
      if (lookahead == 'i') ADVANCE(591);
      if (lookahead == 'o') ADVANCE(865);
      END_STATE();
    case 532:
      if (lookahead == 'a') ADVANCE(705);
      if (lookahead == 'f') ADVANCE(657);
      END_STATE();
    case 533:
      if (lookahead == 'a') ADVANCE(809);
      if (lookahead == 'e') ADVANCE(812);
      if (lookahead == 'l') ADVANCE(522);
      if (lookahead == 'u') ADVANCE(557);
      END_STATE();
    case 534:
      if (lookahead == 'a') ADVANCE(668);
      END_STATE();
    case 535:
      if (lookahead == 'a') ADVANCE(726);
      if (lookahead == 'l') ADVANCE(633);
      if (lookahead == 's') ADVANCE(841);
      END_STATE();
    case 536:
      if (lookahead == 'a') ADVANCE(749);
      if (lookahead == 'i') ADVANCE(669);
      END_STATE();
    case 537:
      if (lookahead == 'a') ADVANCE(749);
      if (lookahead == 'i') ADVANCE(669);
      if (lookahead == 'u') ADVANCE(599);
      END_STATE();
    case 538:
      if (lookahead == 'a') ADVANCE(583);
      END_STATE();
    case 539:
      if (lookahead == 'a') ADVANCE(755);
      END_STATE();
    case 540:
      if (lookahead == 'a') ADVANCE(593);
      END_STATE();
    case 541:
      if (lookahead == 'a') ADVANCE(854);
      END_STATE();
    case 542:
      if (lookahead == 'a') ADVANCE(803);
      END_STATE();
    case 543:
      if (lookahead == 'a') ADVANCE(584);
      END_STATE();
    case 544:
      if (lookahead == 'a') ADVANCE(567);
      END_STATE();
    case 545:
      if (lookahead == 'a') ADVANCE(804);
      END_STATE();
    case 546:
      if (lookahead == 'a') ADVANCE(712);
      END_STATE();
    case 547:
      if (lookahead == 'a') ADVANCE(712);
      if (lookahead == 'e') ADVANCE(855);
      if (lookahead == 'i') ADVANCE(706);
      if (lookahead == 'o') ADVANCE(811);
      if (lookahead == 'u') ADVANCE(748);
      END_STATE();
    case 548:
      if (lookahead == 'a') ADVANCE(820);
      END_STATE();
    case 549:
      if (lookahead == 'a') ADVANCE(885);
      END_STATE();
    case 550:
      if (lookahead == 'a') ADVANCE(859);
      END_STATE();
    case 551:
      if (lookahead == 'a') ADVANCE(714);
      if (lookahead == 'o') ADVANCE(656);
      END_STATE();
    case 552:
      if (lookahead == 'a') ADVANCE(751);
      END_STATE();
    case 553:
      if (lookahead == 'a') ADVANCE(860);
      END_STATE();
    case 554:
      if (lookahead == 'a') ADVANCE(736);
      END_STATE();
    case 555:
      if (lookahead == 'a') ADVANCE(862);
      END_STATE();
    case 556:
      if (lookahead == 'b') ADVANCE(868);
      if (lookahead == 'f') ADVANCE(530);
      if (lookahead == 'o') ADVANCE(789);
      END_STATE();
    case 557:
      if (lookahead == 'b') ADVANCE(709);
      END_STATE();
    case 558:
      if (lookahead == 'b') ADVANCE(711);
      if (lookahead == 'i') ADVANCE(596);
      if (lookahead == 'w') ADVANCE(780);
      END_STATE();
    case 559:
      if (lookahead == 'b') ADVANCE(545);
      END_STATE();
    case 560:
      if (lookahead == 'b') ADVANCE(782);
      END_STATE();
    case 561:
      if (lookahead == 'b') ADVANCE(776);
      END_STATE();
    case 562:
      if (lookahead == 'b') ADVANCE(879);
      END_STATE();
    case 563:
      if (lookahead == 'c') ADVANCE(799);
      END_STATE();
    case 564:
      if (lookahead == 'c') ADVANCE(672);
      END_STATE();
    case 565:
      if (lookahead == 'c') ADVANCE(698);
      if (lookahead == 'l') ADVANCE(703);
      END_STATE();
    case 566:
      if (lookahead == 'c') ADVANCE(674);
      END_STATE();
    case 567:
      if (lookahead == 'c') ADVANCE(699);
      END_STATE();
    case 568:
      if (lookahead == 'c') ADVANCE(700);
      END_STATE();
    case 569:
      if (lookahead == 'c') ADVANCE(877);
      END_STATE();
    case 570:
      if (lookahead == 'c') ADVANCE(550);
      if (lookahead == 'o') ADVANCE(840);
      END_STATE();
    case 571:
      if (lookahead == 'c') ADVANCE(842);
      END_STATE();
    case 572:
      if (lookahead == 'c') ADVANCE(607);
      if (lookahead == 'y') ADVANCE(831);
      END_STATE();
    case 573:
      if (lookahead == 'c') ADVANCE(644);
      END_STATE();
    case 574:
      if (lookahead == 'c') ADVANCE(621);
      END_STATE();
    case 575:
      if (lookahead == 'c') ADVANCE(866);
      END_STATE();
    case 576:
      if (lookahead == 'c') ADVANCE(636);
      END_STATE();
    case 577:
      if (lookahead == 'c') ADVANCE(683);
      if (lookahead == 'l') ADVANCE(768);
      if (lookahead == 't') ADVANCE(870);
      END_STATE();
    case 578:
      if (lookahead == 'c') ADVANCE(720);
      END_STATE();
    case 579:
      if (lookahead == 'c') ADVANCE(875);
      END_STATE();
    case 580:
      if (lookahead == 'c') ADVANCE(867);
      END_STATE();
    case 581:
      if (lookahead == 'd') ADVANCE(880);
      if (lookahead == 't') ADVANCE(856);
      END_STATE();
    case 582:
      if (lookahead == 'd') ADVANCE(1014);
      END_STATE();
    case 583:
      if (lookahead == 'd') ADVANCE(999);
      END_STATE();
    case 584:
      if (lookahead == 'd') ADVANCE(951);
      END_STATE();
    case 585:
      if (lookahead == 'd') ADVANCE(991);
      END_STATE();
    case 586:
      if (lookahead == 'd') ADVANCE(1023);
      END_STATE();
    case 587:
      if (lookahead == 'd') ADVANCE(1013);
      END_STATE();
    case 588:
      if (lookahead == 'd') ADVANCE(767);
      if (lookahead == 't') ADVANCE(684);
      END_STATE();
    case 589:
      if (lookahead == 'd') ADVANCE(561);
      END_STATE();
    case 590:
      if (lookahead == 'd') ADVANCE(763);
      END_STATE();
    case 591:
      if (lookahead == 'd') ADVANCE(603);
      END_STATE();
    case 592:
      if (lookahead == 'd') ADVANCE(835);
      END_STATE();
    case 593:
      if (lookahead == 'd') ADVANCE(794);
      END_STATE();
    case 594:
      if (lookahead == 'd') ADVANCE(614);
      END_STATE();
    case 595:
      if (lookahead == 'd') ADVANCE(623);
      END_STATE();
    case 596:
      if (lookahead == 'd') ADVANCE(718);
      END_STATE();
    case 597:
      if (lookahead == 'd') ADVANCE(642);
      END_STATE();
    case 598:
      if (lookahead == 'd') ADVANCE(878);
      END_STATE();
    case 599:
      if (lookahead == 'e') ADVANCE(2078);
      END_STATE();
    case 600:
      if (lookahead == 'e') ADVANCE(978);
      if (lookahead == 's') ADVANCE(660);
      END_STATE();
    case 601:
      if (lookahead == 'e') ADVANCE(731);
      END_STATE();
    case 602:
      if (lookahead == 'e') ADVANCE(958);
      END_STATE();
    case 603:
      if (lookahead == 'e') ADVANCE(1004);
      END_STATE();
    case 604:
      if (lookahead == 'e') ADVANCE(29);
      END_STATE();
    case 605:
      if (lookahead == 'e') ADVANCE(1034);
      END_STATE();
    case 606:
      if (lookahead == 'e') ADVANCE(930);
      END_STATE();
    case 607:
      if (lookahead == 'e') ADVANCE(989);
      END_STATE();
    case 608:
      if (lookahead == 'e') ADVANCE(1036);
      END_STATE();
    case 609:
      if (lookahead == 'e') ADVANCE(932);
      END_STATE();
    case 610:
      if (lookahead == 'e') ADVANCE(973);
      END_STATE();
    case 611:
      if (lookahead == 'e') ADVANCE(997);
      END_STATE();
    case 612:
      if (lookahead == 'e') ADVANCE(1006);
      END_STATE();
    case 613:
      if (lookahead == 'e') ADVANCE(909);
      END_STATE();
    case 614:
      if (lookahead == 'e') ADVANCE(954);
      END_STATE();
    case 615:
      if (lookahead == 'e') ADVANCE(956);
      END_STATE();
    case 616:
      if (lookahead == 'e') ADVANCE(985);
      END_STATE();
    case 617:
      if (lookahead == 'e') ADVANCE(1012);
      END_STATE();
    case 618:
      if (lookahead == 'e') ADVANCE(1019);
      END_STATE();
    case 619:
      if (lookahead == 'e') ADVANCE(917);
      END_STATE();
    case 620:
      if (lookahead == 'e') ADVANCE(950);
      END_STATE();
    case 621:
      if (lookahead == 'e') ADVANCE(946);
      END_STATE();
    case 622:
      if (lookahead == 'e') ADVANCE(948);
      END_STATE();
    case 623:
      if (lookahead == 'e') ADVANCE(938);
      END_STATE();
    case 624:
      if (lookahead == 'e') ADVANCE(569);
      if (lookahead == 'p') ADVANCE(634);
      END_STATE();
    case 625:
      if (lookahead == 'e') ADVANCE(707);
      END_STATE();
    case 626:
      if (lookahead == 'e') ADVANCE(725);
      END_STATE();
    case 627:
      if (lookahead == 'e') ADVANCE(582);
      if (lookahead == 't') ADVANCE(558);
      END_STATE();
    case 628:
      if (lookahead == 'e') ADVANCE(855);
      if (lookahead == 'i') ADVANCE(706);
      if (lookahead == 'o') ADVANCE(811);
      if (lookahead == 'u') ADVANCE(748);
      END_STATE();
    case 629:
      if (lookahead == 'e') ADVANCE(560);
      END_STATE();
    case 630:
      if (lookahead == 'e') ADVANCE(598);
      END_STATE();
    case 631:
      if (lookahead == 'e') ADVANCE(542);
      if (lookahead == 'o') ADVANCE(754);
      END_STATE();
    case 632:
      if (lookahead == 'e') ADVANCE(571);
      END_STATE();
    case 633:
      if (lookahead == 'e') ADVANCE(797);
      if (lookahead == 'l') ADVANCE(1031);
      END_STATE();
    case 634:
      if (lookahead == 'e') ADVANCE(818);
      END_STATE();
    case 635:
      if (lookahead == 'e') ADVANCE(540);
      END_STATE();
    case 636:
      if (lookahead == 'e') ADVANCE(723);
      END_STATE();
    case 637:
      if (lookahead == 'e') ADVANCE(764);
      END_STATE();
    case 638:
      if (lookahead == 'e') ADVANCE(724);
      END_STATE();
    case 639:
      if (lookahead == 'e') ADVANCE(805);
      END_STATE();
    case 640:
      if (lookahead == 'e') ADVANCE(806);
      END_STATE();
    case 641:
      if (lookahead == 'e') ADVANCE(807);
      END_STATE();
    case 642:
      if (lookahead == 'e') ADVANCE(808);
      END_STATE();
    case 643:
      if (lookahead == 'e') ADVANCE(813);
      END_STATE();
    case 644:
      if (lookahead == 'e') ADVANCE(733);
      END_STATE();
    case 645:
      if (lookahead == 'e') ADVANCE(738);
      END_STATE();
    case 646:
      if (lookahead == 'e') ADVANCE(816);
      END_STATE();
    case 647:
      if (lookahead == 'e') ADVANCE(858);
      END_STATE();
    case 648:
      if (lookahead == 'e') ADVANCE(759);
      END_STATE();
    case 649:
      if (lookahead == 'e') ADVANCE(758);
      END_STATE();
    case 650:
      if (lookahead == 'f') ADVANCE(851);
      END_STATE();
    case 651:
      if (lookahead == 'f') ADVANCE(987);
      END_STATE();
    case 652:
      if (lookahead == 'f') ADVANCE(1009);
      END_STATE();
    case 653:
      if (lookahead == 'f') ADVANCE(655);
      if (lookahead == 'n') ADVANCE(564);
      if (lookahead == 'x') ADVANCE(624);
      END_STATE();
    case 654:
      if (lookahead == 'f') ADVANCE(802);
      END_STATE();
    case 655:
      if (lookahead == 'f') ADVANCE(632);
      END_STATE();
    case 656:
      if (lookahead == 'f') ADVANCE(652);
      if (lookahead == 'n') ADVANCE(1010);
      END_STATE();
    case 657:
      if (lookahead == 'f') ADVANCE(689);
      END_STATE();
    case 658:
      if (lookahead == 'f') ADVANCE(697);
      END_STATE();
    case 659:
      if (lookahead == 'f') ADVANCE(641);
      END_STATE();
    case 660:
      if (lookahead == 'g') ADVANCE(979);
      END_STATE();
    case 661:
      if (lookahead == 'g') ADVANCE(1027);
      END_STATE();
    case 662:
      if (lookahead == 'g') ADVANCE(937);
      END_STATE();
    case 663:
      if (lookahead == 'g') ADVANCE(1026);
      END_STATE();
    case 664:
      if (lookahead == 'g') ADVANCE(940);
      END_STATE();
    case 665:
      if (lookahead == 'g') ADVANCE(1029);
      END_STATE();
    case 666:
      if (lookahead == 'g') ADVANCE(39);
      END_STATE();
    case 667:
      if (lookahead == 'g') ADVANCE(554);
      END_STATE();
    case 668:
      if (lookahead == 'g') ADVANCE(609);
      END_STATE();
    case 669:
      if (lookahead == 'g') ADVANCE(670);
      END_STATE();
    case 670:
      if (lookahead == 'g') ADVANCE(639);
      END_STATE();
    case 671:
      if (lookahead == 'h') ADVANCE(993);
      END_STATE();
    case 672:
      if (lookahead == 'h') ADVANCE(539);
      END_STATE();
    case 673:
      if (lookahead == 'h') ADVANCE(630);
      if (lookahead == 'o') ADVANCE(823);
      END_STATE();
    case 674:
      if (lookahead == 'h') ADVANCE(793);
      END_STATE();
    case 675:
      if (lookahead == 'h') ADVANCE(640);
      END_STATE();
    case 676:
      if (lookahead == 'i') ADVANCE(785);
      END_STATE();
    case 677:
      if (lookahead == 'i') ADVANCE(739);
      END_STATE();
    case 678:
      if (lookahead == 'i') ADVANCE(565);
      END_STATE();
    case 679:
      if (lookahead == 'i') ADVANCE(786);
      END_STATE();
    case 680:
      if (lookahead == 'i') ADVANCE(830);
      if (lookahead == 'o') ADVANCE(570);
      END_STATE();
    case 681:
      if (lookahead == 'i') ADVANCE(562);
      END_STATE();
    case 682:
      if (lookahead == 'i') ADVANCE(829);
      END_STATE();
    case 683:
      if (lookahead == 'i') ADVANCE(796);
      END_STATE();
    case 684:
      if (lookahead == 'i') ADVANCE(578);
      END_STATE();
    case 685:
      if (lookahead == 'i') ADVANCE(864);
      END_STATE();
    case 686:
      if (lookahead == 'i') ADVANCE(781);
      END_STATE();
    case 687:
      if (lookahead == 'i') ADVANCE(648);
      END_STATE();
    case 688:
      if (lookahead == 'i') ADVANCE(750);
      END_STATE();
    case 689:
      if (lookahead == 'i') ADVANCE(579);
      END_STATE();
    case 690:
      if (lookahead == 'i') ADVANCE(834);
      END_STATE();
    case 691:
      if (lookahead == 'i') ADVANCE(756);
      END_STATE();
    case 692:
      if (lookahead == 'i') ADVANCE(836);
      END_STATE();
    case 693:
      if (lookahead == 'i') ADVANCE(771);
      END_STATE();
    case 694:
      if (lookahead == 'i') ADVANCE(757);
      END_STATE();
    case 695:
      if (lookahead == 'i') ADVANCE(773);
      END_STATE();
    case 696:
      if (lookahead == 'i') ADVANCE(735);
      END_STATE();
    case 697:
      if (lookahead == 'i') ADVANCE(722);
      END_STATE();
    case 698:
      if (lookahead == 'k') ADVANCE(966);
      END_STATE();
    case 699:
      if (lookahead == 'k') ADVANCE(936);
      END_STATE();
    case 700:
      if (lookahead == 'k') ADVANCE(1015);
      END_STATE();
    case 701:
      if (lookahead == 'l') ADVANCE(631);
      END_STATE();
    case 702:
      if (lookahead == 'l') ADVANCE(949);
      END_STATE();
    case 703:
      if (lookahead == 'l') ADVANCE(968);
      END_STATE();
    case 704:
      if (lookahead == 'l') ADVANCE(1008);
      END_STATE();
    case 705:
      if (lookahead == 'l') ADVANCE(766);
      END_STATE();
    case 706:
      if (lookahead == 'l') ADVANCE(702);
      END_STATE();
    case 707:
      if (lookahead == 'l') ADVANCE(790);
      END_STATE();
    case 708:
      if (lookahead == 'l') ADVANCE(589);
      END_STATE();
    case 709:
      if (lookahead == 'l') ADVANCE(682);
      END_STATE();
    case 710:
      if (lookahead == 'l') ADVANCE(592);
      END_STATE();
    case 711:
      if (lookahead == 'l') ADVANCE(770);
      END_STATE();
    case 712:
      if (lookahead == 'l') ADVANCE(837);
      END_STATE();
    case 713:
      if (lookahead == 'l') ADVANCE(857);
      END_STATE();
    case 714:
      if (lookahead == 'l') ADVANCE(704);
      END_STATE();
    case 715:
      if (lookahead == 'l') ADVANCE(608);
      END_STATE();
    case 716:
      if (lookahead == 'l') ADVANCE(529);
      END_STATE();
    case 717:
      if (lookahead == 'l') ADVANCE(852);
      END_STATE();
    case 718:
      if (lookahead == 'l') ADVANCE(647);
      END_STATE();
    case 719:
      if (lookahead == 'l') ADVANCE(615);
      END_STATE();
    case 720:
      if (lookahead == 'l') ADVANCE(616);
      END_STATE();
    case 721:
      if (lookahead == 'l') ADVANCE(617);
      END_STATE();
    case 722:
      if (lookahead == 'l') ADVANCE(622);
      END_STATE();
    case 723:
      if (lookahead == 'l') ADVANCE(774);
      END_STATE();
    case 724:
      if (lookahead == 'l') ADVANCE(692);
      END_STATE();
    case 725:
      if (lookahead == 'm') ADVANCE(962);
      END_STATE();
    case 726:
      if (lookahead == 'm') ADVANCE(1028);
      END_STATE();
    case 727:
      if (lookahead == 'm') ADVANCE(995);
      END_STATE();
    case 728:
      if (lookahead == 'm') ADVANCE(737);
      END_STATE();
    case 729:
      if (lookahead == 'm') ADVANCE(534);
      if (lookahead == 't') ADVANCE(524);
      END_STATE();
    case 730:
      if (lookahead == 'm') ADVANCE(601);
      END_STATE();
    case 731:
      if (lookahead == 'm') ADVANCE(777);
      if (lookahead == 'r') ADVANCE(876);
      END_STATE();
    case 732:
      if (lookahead == 'm') ADVANCE(605);
      if (lookahead == 't') ADVANCE(715);
      END_STATE();
    case 733:
      if (lookahead == 'm') ADVANCE(649);
      END_STATE();
    case 734:
      if (lookahead == 'm') ADVANCE(620);
      END_STATE();
    case 735:
      if (lookahead == 'm') ADVANCE(637);
      END_STATE();
    case 736:
      if (lookahead == 'm') ADVANCE(645);
      END_STATE();
    case 737:
      if (lookahead == 'm') ADVANCE(769);
      END_STATE();
    case 738:
      if (lookahead == 'm') ADVANCE(783);
      END_STATE();
    case 739:
      if (lookahead == 'n') ADVANCE(666);
      END_STATE();
    case 740:
      if (lookahead == 'n') ADVANCE(920);
      END_STATE();
    case 741:
      if (lookahead == 'n') ADVANCE(982);
      END_STATE();
    case 742:
      if (lookahead == 'n') ADVANCE(1002);
      END_STATE();
    case 743:
      if (lookahead == 'n') ADVANCE(1024);
      END_STATE();
    case 744:
      if (lookahead == 'n') ADVANCE(1041);
      END_STATE();
    case 745:
      if (lookahead == 'n') ADVANCE(952);
      END_STATE();
    case 746:
      if (lookahead == 'n') ADVANCE(1017);
      END_STATE();
    case 747:
      if (lookahead == 'n') ADVANCE(590);
      END_STATE();
    case 748:
      if (lookahead == 'n') ADVANCE(575);
      END_STATE();
    case 749:
      if (lookahead == 'n') ADVANCE(838);
      END_STATE();
    case 750:
      if (lookahead == 'n') ADVANCE(663);
      END_STATE();
    case 751:
      if (lookahead == 'n') ADVANCE(573);
      END_STATE();
    case 752:
      if (lookahead == 'n') ADVANCE(585);
      END_STATE();
    case 753:
      if (lookahead == 'n') ADVANCE(586);
      END_STATE();
    case 754:
      if (lookahead == 'n') ADVANCE(606);
      END_STATE();
    case 755:
      if (lookahead == 'n') ADVANCE(844);
      END_STATE();
    case 756:
      if (lookahead == 'n') ADVANCE(846);
      END_STATE();
    case 757:
      if (lookahead == 'n') ADVANCE(848);
      END_STATE();
    case 758:
      if (lookahead == 'n') ADVANCE(849);
      END_STATE();
    case 759:
      if (lookahead == 'n') ADVANCE(574);
      END_STATE();
    case 760:
      if (lookahead == 'n') ADVANCE(798);
      END_STATE();
    case 761:
      if (lookahead == 'o') ADVANCE(801);
      END_STATE();
    case 762:
      if (lookahead == 'o') ADVANCE(871);
      END_STATE();
    case 763:
      if (lookahead == 'o') ADVANCE(727);
      END_STATE();
    case 764:
      if (lookahead == 'o') ADVANCE(873);
      END_STATE();
    case 765:
      if (lookahead == 'o') ADVANCE(791);
      END_STATE();
    case 766:
      if (lookahead == 'o') ADVANCE(664);
      END_STATE();
    case 767:
      if (lookahead == 'o') ADVANCE(741);
      END_STATE();
    case 768:
      if (lookahead == 'o') ADVANCE(538);
      END_STATE();
    case 769:
      if (lookahead == 'o') ADVANCE(743);
      END_STATE();
    case 770:
      if (lookahead == 'o') ADVANCE(568);
      END_STATE();
    case 771:
      if (lookahead == 'o') ADVANCE(744);
      END_STATE();
    case 772:
      if (lookahead == 'o') ADVANCE(691);
      END_STATE();
    case 773:
      if (lookahead == 'o') ADVANCE(745);
      END_STATE();
    case 774:
      if (lookahead == 'o') ADVANCE(543);
      END_STATE();
    case 775:
      if (lookahead == 'o') ADVANCE(822);
      END_STATE();
    case 776:
      if (lookahead == 'o') ADVANCE(824);
      END_STATE();
    case 777:
      if (lookahead == 'o') ADVANCE(594);
      END_STATE();
    case 778:
      if (lookahead == 'o') ADVANCE(658);
      END_STATE();
    case 779:
      if (lookahead == 'o') ADVANCE(872);
      END_STATE();
    case 780:
      if (lookahead == 'o') ADVANCE(821);
      END_STATE();
    case 781:
      if (lookahead == 'o') ADVANCE(734);
      END_STATE();
    case 782:
      if (lookahead == 'o') ADVANCE(548);
      END_STATE();
    case 783:
      if (lookahead == 'o') ADVANCE(595);
      END_STATE();
    case 784:
      if (lookahead == 'o') ADVANCE(694);
      END_STATE();
    case 785:
      if (lookahead == 'p') ADVANCE(921);
      END_STATE();
    case 786:
      if (lookahead == 'p') ADVANCE(984);
      END_STATE();
    case 787:
      if (lookahead == 'p') ADVANCE(980);
      END_STATE();
    case 788:
      if (lookahead == 'p') ADVANCE(1053);
      END_STATE();
    case 789:
      if (lookahead == 'p') ADVANCE(939);
      END_STATE();
    case 790:
      if (lookahead == 'p') ADVANCE(961);
      END_STATE();
    case 791:
      if (lookahead == 'p') ADVANCE(1022);
      END_STATE();
    case 792:
      if (lookahead == 'p') ADVANCE(772);
      END_STATE();
    case 793:
      if (lookahead == 'p') ADVANCE(819);
      END_STATE();
    case 794:
      if (lookahead == 'p') ADVANCE(716);
      END_STATE();
    case 795:
      if (lookahead == 'p') ADVANCE(549);
      END_STATE();
    case 796:
      if (lookahead == 'p') ADVANCE(611);
      END_STATE();
    case 797:
      if (lookahead == 'p') ADVANCE(775);
      END_STATE();
    case 798:
      if (lookahead == 'p') ADVANCE(784);
      END_STATE();
    case 799:
      if (lookahead == 'r') ADVANCE(518);
      if (lookahead == 'u') ADVANCE(800);
      END_STATE();
    case 800:
      if (lookahead == 'r') ADVANCE(825);
      END_STATE();
    case 801:
      if (lookahead == 'r') ADVANCE(1364);
      END_STATE();
    case 802:
      if (lookahead == 'r') ADVANCE(964);
      END_STATE();
    case 803:
      if (lookahead == 'r') ADVANCE(927);
      END_STATE();
    case 804:
      if (lookahead == 'r') ADVANCE(924);
      END_STATE();
    case 805:
      if (lookahead == 'r') ADVANCE(1040);
      END_STATE();
    case 806:
      if (lookahead == 'r') ADVANCE(1047);
      END_STATE();
    case 807:
      if (lookahead == 'r') ADVANCE(1039);
      END_STATE();
    case 808:
      if (lookahead == 'r') ADVANCE(1051);
      END_STATE();
    case 809:
      if (lookahead == 'r') ADVANCE(588);
      END_STATE();
    case 810:
      if (lookahead == 'r') ADVANCE(681);
      END_STATE();
    case 811:
      if (lookahead == 'r') ADVANCE(576);
      END_STATE();
    case 812:
      if (lookahead == 'r') ADVANCE(651);
      END_STATE();
    case 813:
      if (lookahead == 'r') ADVANCE(833);
      END_STATE();
    case 814:
      if (lookahead == 'r') ADVANCE(874);
      END_STATE();
    case 815:
      if (lookahead == 'r') ADVANCE(708);
      END_STATE();
    case 816:
      if (lookahead == 'r') ADVANCE(826);
      END_STATE();
    case 817:
      if (lookahead == 'r') ADVANCE(742);
      END_STATE();
    case 818:
      if (lookahead == 'r') ADVANCE(687);
      END_STATE();
    case 819:
      if (lookahead == 'r') ADVANCE(778);
      END_STATE();
    case 820:
      if (lookahead == 'r') ADVANCE(587);
      END_STATE();
    case 821:
      if (lookahead == 'r') ADVANCE(710);
      END_STATE();
    case 822:
      if (lookahead == 'r') ADVANCE(845);
      END_STATE();
    case 823:
      if (lookahead == 'r') ADVANCE(629);
      END_STATE();
    case 824:
      if (lookahead == 'r') ADVANCE(597);
      END_STATE();
    case 825:
      if (lookahead == 's') ADVANCE(761);
      END_STATE();
    case 826:
      if (lookahead == 's') ADVANCE(1020);
      END_STATE();
    case 827:
      if (lookahead == 's') ADVANCE(559);
      END_STATE();
    case 828:
      if (lookahead == 's') ADVANCE(827);
      END_STATE();
    case 829:
      if (lookahead == 's') ADVANCE(671);
      END_STATE();
    case 830:
      if (lookahead == 's') ADVANCE(839);
      END_STATE();
    case 831:
      if (lookahead == 's') ADVANCE(762);
      END_STATE();
    case 832:
      if (lookahead == 's') ADVANCE(665);
      END_STATE();
    case 833:
      if (lookahead == 's') ADVANCE(693);
      END_STATE();
    case 834:
      if (lookahead == 's') ADVANCE(843);
      END_STATE();
    case 835:
      if (lookahead == 's') ADVANCE(795);
      END_STATE();
    case 836:
      if (lookahead == 's') ADVANCE(847);
      END_STATE();
    case 837:
      if (lookahead == 's') ADVANCE(599);
      END_STATE();
    case 838:
      if (lookahead == 's') ADVANCE(659);
      END_STATE();
    case 839:
      if (lookahead == 't') ADVANCE(970);
      END_STATE();
    case 840:
      if (lookahead == 't') ADVANCE(976);
      END_STATE();
    case 841:
      if (lookahead == 't') ADVANCE(1033);
      END_STATE();
    case 842:
      if (lookahead == 't') ADVANCE(942);
      END_STATE();
    case 843:
      if (lookahead == 't') ADVANCE(922);
      END_STATE();
    case 844:
      if (lookahead == 't') ADVANCE(944);
      END_STATE();
    case 845:
      if (lookahead == 't') ADVANCE(1030);
      END_STATE();
    case 846:
      if (lookahead == 't') ADVANCE(1045);
      END_STATE();
    case 847:
      if (lookahead == 't') ADVANCE(1049);
      END_STATE();
    case 848:
      if (lookahead == 't') ADVANCE(1018);
      END_STATE();
    case 849:
      if (lookahead == 't') ADVANCE(915);
      END_STATE();
    case 850:
      if (lookahead == 't') ADVANCE(1016);
      END_STATE();
    case 851:
      if (lookahead == 't') ADVANCE(677);
      END_STATE();
    case 852:
      if (lookahead == 't') ADVANCE(886);
      END_STATE();
    case 853:
      if (lookahead == 't') ADVANCE(626);
      END_STATE();
    case 854:
      if (lookahead == 't') ADVANCE(675);
      END_STATE();
    case 855:
      if (lookahead == 't') ADVANCE(566);
      END_STATE();
    case 856:
      if (lookahead == 't') ADVANCE(810);
      END_STATE();
    case 857:
      if (lookahead == 't') ADVANCE(667);
      END_STATE();
    case 858:
      if (lookahead == 't') ADVANCE(696);
      END_STATE();
    case 859:
      if (lookahead == 't') ADVANCE(610);
      END_STATE();
    case 860:
      if (lookahead == 't') ADVANCE(612);
      END_STATE();
    case 861:
      if (lookahead == 't') ADVANCE(613);
      END_STATE();
    case 862:
      if (lookahead == 't') ADVANCE(618);
      END_STATE();
    case 863:
      if (lookahead == 't') ADVANCE(619);
      END_STATE();
    case 864:
      if (lookahead == 't') ADVANCE(638);
      END_STATE();
    case 865:
      if (lookahead == 't') ADVANCE(553);
      END_STATE();
    case 866:
      if (lookahead == 't') ADVANCE(695);
      END_STATE();
    case 867:
      if (lookahead == 't') ADVANCE(555);
      END_STATE();
    case 868:
      if (lookahead == 'u') ADVANCE(662);
      END_STATE();
    case 869:
      if (lookahead == 'u') ADVANCE(713);
      END_STATE();
    case 870:
      if (lookahead == 'u') ADVANCE(817);
      END_STATE();
    case 871:
      if (lookahead == 'u') ADVANCE(752);
      END_STATE();
    case 872:
      if (lookahead == 'u') ADVANCE(753);
      END_STATE();
    case 873:
      if (lookahead == 'u') ADVANCE(850);
      END_STATE();
    case 874:
      if (lookahead == 'u') ADVANCE(599);
      END_STATE();
    case 875:
      if (lookahead == 'u') ADVANCE(717);
      END_STATE();
    case 876:
      if (lookahead == 'u') ADVANCE(719);
      END_STATE();
    case 877:
      if (lookahead == 'u') ADVANCE(861);
      END_STATE();
    case 878:
      if (lookahead == 'u') ADVANCE(721);
      END_STATE();
    case 879:
      if (lookahead == 'u') ADVANCE(863);
      END_STATE();
    case 880:
      if (lookahead == 'v') ADVANCE(552);
      END_STATE();
    case 881:
      if (lookahead == 'v') ADVANCE(602);
      END_STATE();
    case 882:
      if (lookahead == 'v') ADVANCE(604);
      if (lookahead == 'y') ADVANCE(1011);
      END_STATE();
    case 883:
      if (lookahead == 'w') ADVANCE(1032);
      END_STATE();
    case 884:
      if (lookahead == 'w') ADVANCE(760);
      END_STATE();
    case 885:
      if (lookahead == 'w') ADVANCE(746);
      END_STATE();
    case 886:
      if (lookahead == 'y') ADVANCE(941);
      END_STATE();
    case 887:
      if (lookahead == 'y') ADVANCE(792);
      END_STATE();
    case 888:
      if (lookahead == 'y') ADVANCE(646);
      END_STATE();
    case 889:
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1318);
      END_STATE();
    case 890:
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(2081);
      END_STATE();
    case 891:
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1356);
      END_STATE();
    case 892:
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1348);
      END_STATE();
    case 893:
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1352);
      END_STATE();
    case 894:
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1354);
      END_STATE();
    case 895:
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1361);
      END_STATE();
    case 896:
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(1292);
      END_STATE();
    case 897:
      if (lookahead == '/' ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1305);
      END_STATE();
    case 898:
      if (eof) ADVANCE(899);
      ADVANCE_MAP(
        '\n', 902,
        '\r', 906,
        '#', 908,
        '$', 914,
        '%', 515,
        '.', 889,
        'a', 581,
        'b', 525,
        'c', 701,
        'd', 519,
        'e', 653,
        'f', 628,
        'g', 520,
        'h', 625,
        'i', 853,
        'j', 654,
        'k', 678,
        'l', 680,
        'm', 600,
        'o', 787,
        'p', 533,
        'r', 531,
        's', 521,
        't', 528,
        'v', 643,
        'w', 1044,
        'x', 788,
        '\t', 907,
        0x0b, 907,
        '\f', 907,
        ' ', 907,
      );
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1311);
      END_STATE();
    case 899:
      ACCEPT_TOKEN(ts_builtin_sym_end);
      END_STATE();
    case 900:
      ACCEPT_TOKEN(sym__newline);
      END_STATE();
    case 901:
      ACCEPT_TOKEN(sym__newline);
      if (lookahead == '\\') ADVANCE(5);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(516);
      END_STATE();
    case 902:
      ACCEPT_TOKEN(sym__newline);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(907);
      END_STATE();
    case 903:
      ACCEPT_TOKEN(aux_sym__space_token1);
      if (lookahead == ',') ADVANCE(1299);
      if (lookahead == '\\') ADVANCE(5);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(26);
      END_STATE();
    case 904:
      ACCEPT_TOKEN(aux_sym__space_token1);
      if (lookahead == '\\') ADVANCE(5);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(516);
      END_STATE();
    case 905:
      ACCEPT_TOKEN(sym_backslash);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(905);
      END_STATE();
    case 906:
      ACCEPT_TOKEN(aux_sym__statement_token1);
      if (lookahead == '\n') ADVANCE(902);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(907);
      END_STATE();
    case 907:
      ACCEPT_TOKEN(aux_sym__statement_token1);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(907);
      END_STATE();
    case 908:
      ACCEPT_TOKEN(sym_comment);
      if (lookahead != 0 &&
          lookahead != '\n' &&
          lookahead != '\r') ADVANCE(908);
      END_STATE();
    case 909:
      ACCEPT_TOKEN(anon_sym_execute);
      END_STATE();
    case 910:
      ACCEPT_TOKEN(anon_sym_execute);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 911:
      ACCEPT_TOKEN(anon_sym_run);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'c') ADVANCE(290);
      if (lookahead == 'f') ADVANCE(67);
      if (lookahead == 't') ADVANCE(227);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 912:
      ACCEPT_TOKEN(anon_sym_run);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1723);
      if (lookahead == 'f') ADVANCE(1439);
      if (lookahead == 't') ADVANCE(1649);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 913:
      ACCEPT_TOKEN(anon_sym_run);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 914:
      ACCEPT_TOKEN(sym_macro_indicator);
      if (lookahead == '(') ADVANCE(1329);
      END_STATE();
    case 915:
      ACCEPT_TOKEN(anon_sym_advancement);
      END_STATE();
    case 916:
      ACCEPT_TOKEN(anon_sym_advancement);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 917:
      ACCEPT_TOKEN(anon_sym_attribute);
      END_STATE();
    case 918:
      ACCEPT_TOKEN(anon_sym_attribute);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 919:
      ACCEPT_TOKEN(anon_sym_ban);
      if (lookahead == '-') ADVANCE(676);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(260);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 920:
      ACCEPT_TOKEN(anon_sym_ban);
      if (lookahead == '-') ADVANCE(676);
      if (lookahead == 'l') ADVANCE(690);
      END_STATE();
    case 921:
      ACCEPT_TOKEN(anon_sym_ban_DASHip);
      END_STATE();
    case 922:
      ACCEPT_TOKEN(anon_sym_banlist);
      END_STATE();
    case 923:
      ACCEPT_TOKEN(anon_sym_banlist);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 924:
      ACCEPT_TOKEN(anon_sym_bossbar);
      END_STATE();
    case 925:
      ACCEPT_TOKEN(anon_sym_bossbar);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 926:
      ACCEPT_TOKEN(anon_sym_bossbar);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 927:
      ACCEPT_TOKEN(anon_sym_clear);
      END_STATE();
    case 928:
      ACCEPT_TOKEN(anon_sym_clear);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 929:
      ACCEPT_TOKEN(anon_sym_clear);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 930:
      ACCEPT_TOKEN(anon_sym_clone);
      END_STATE();
    case 931:
      ACCEPT_TOKEN(anon_sym_clone);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 932:
      ACCEPT_TOKEN(anon_sym_damage);
      END_STATE();
    case 933:
      ACCEPT_TOKEN(anon_sym_damage);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 934:
      ACCEPT_TOKEN(anon_sym_data);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 935:
      ACCEPT_TOKEN(anon_sym_data);
      if (lookahead == 'p') ADVANCE(544);
      END_STATE();
    case 936:
      ACCEPT_TOKEN(anon_sym_datapack);
      END_STATE();
    case 937:
      ACCEPT_TOKEN(anon_sym_debug);
      END_STATE();
    case 938:
      ACCEPT_TOKEN(anon_sym_defaultgamemode);
      END_STATE();
    case 939:
      ACCEPT_TOKEN(anon_sym_deop);
      END_STATE();
    case 940:
      ACCEPT_TOKEN(anon_sym_dialog);
      END_STATE();
    case 941:
      ACCEPT_TOKEN(anon_sym_difficulty);
      END_STATE();
    case 942:
      ACCEPT_TOKEN(anon_sym_effect);
      END_STATE();
    case 943:
      ACCEPT_TOKEN(anon_sym_effect);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 944:
      ACCEPT_TOKEN(anon_sym_enchant);
      END_STATE();
    case 945:
      ACCEPT_TOKEN(anon_sym_enchant);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 946:
      ACCEPT_TOKEN(anon_sym_experience);
      END_STATE();
    case 947:
      ACCEPT_TOKEN(anon_sym_experience);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 948:
      ACCEPT_TOKEN(anon_sym_fetchprofile);
      END_STATE();
    case 949:
      ACCEPT_TOKEN(anon_sym_fill);
      if (lookahead == 'b') ADVANCE(686);
      END_STATE();
    case 950:
      ACCEPT_TOKEN(anon_sym_fillbiome);
      END_STATE();
    case 951:
      ACCEPT_TOKEN(anon_sym_forceload);
      END_STATE();
    case 952:
      ACCEPT_TOKEN(anon_sym_function);
      END_STATE();
    case 953:
      ACCEPT_TOKEN(anon_sym_function);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 954:
      ACCEPT_TOKEN(anon_sym_gamemode);
      END_STATE();
    case 955:
      ACCEPT_TOKEN(anon_sym_gamemode);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 956:
      ACCEPT_TOKEN(anon_sym_gamerule);
      END_STATE();
    case 957:
      ACCEPT_TOKEN(anon_sym_gamerule);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 958:
      ACCEPT_TOKEN(anon_sym_give);
      END_STATE();
    case 959:
      ACCEPT_TOKEN(anon_sym_give);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 960:
      ACCEPT_TOKEN(anon_sym_give);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 961:
      ACCEPT_TOKEN(anon_sym_help);
      END_STATE();
    case 962:
      ACCEPT_TOKEN(anon_sym_item);
      END_STATE();
    case 963:
      ACCEPT_TOKEN(anon_sym_item);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(1087);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 964:
      ACCEPT_TOKEN(anon_sym_jfr);
      END_STATE();
    case 965:
      ACCEPT_TOKEN(anon_sym_jfr);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 966:
      ACCEPT_TOKEN(anon_sym_kick);
      END_STATE();
    case 967:
      ACCEPT_TOKEN(anon_sym_kick);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 968:
      ACCEPT_TOKEN(anon_sym_kill);
      END_STATE();
    case 969:
      ACCEPT_TOKEN(anon_sym_kill);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 970:
      ACCEPT_TOKEN(anon_sym_list);
      END_STATE();
    case 971:
      ACCEPT_TOKEN(anon_sym_list);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 972:
      ACCEPT_TOKEN(anon_sym_list);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 973:
      ACCEPT_TOKEN(anon_sym_locate);
      END_STATE();
    case 974:
      ACCEPT_TOKEN(anon_sym_locate);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 975:
      ACCEPT_TOKEN(anon_sym_locate);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 976:
      ACCEPT_TOKEN(anon_sym_loot);
      END_STATE();
    case 977:
      ACCEPT_TOKEN(anon_sym_loot);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 978:
      ACCEPT_TOKEN(anon_sym_me);
      END_STATE();
    case 979:
      ACCEPT_TOKEN(anon_sym_msg);
      END_STATE();
    case 980:
      ACCEPT_TOKEN(anon_sym_op);
      END_STATE();
    case 981:
      ACCEPT_TOKEN(anon_sym_op);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 982:
      ACCEPT_TOKEN(anon_sym_pardon);
      if (lookahead == '-') ADVANCE(679);
      END_STATE();
    case 983:
      ACCEPT_TOKEN(anon_sym_pardon);
      if (lookahead == '-') ADVANCE(679);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 984:
      ACCEPT_TOKEN(anon_sym_pardon_DASHip);
      END_STATE();
    case 985:
      ACCEPT_TOKEN(anon_sym_particle);
      END_STATE();
    case 986:
      ACCEPT_TOKEN(anon_sym_particle);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 987:
      ACCEPT_TOKEN(anon_sym_perf);
      END_STATE();
    case 988:
      ACCEPT_TOKEN(anon_sym_perf);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 989:
      ACCEPT_TOKEN(anon_sym_place);
      END_STATE();
    case 990:
      ACCEPT_TOKEN(anon_sym_place);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 991:
      ACCEPT_TOKEN(anon_sym_playsound);
      END_STATE();
    case 992:
      ACCEPT_TOKEN(anon_sym_playsound);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 993:
      ACCEPT_TOKEN(anon_sym_publish);
      END_STATE();
    case 994:
      ACCEPT_TOKEN(anon_sym_publish);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 995:
      ACCEPT_TOKEN(anon_sym_random);
      END_STATE();
    case 996:
      ACCEPT_TOKEN(anon_sym_random);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 997:
      ACCEPT_TOKEN(anon_sym_recipe);
      END_STATE();
    case 998:
      ACCEPT_TOKEN(anon_sym_recipe);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 999:
      ACCEPT_TOKEN(anon_sym_reload);
      END_STATE();
    case 1000:
      ACCEPT_TOKEN(anon_sym_reload);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1001:
      ACCEPT_TOKEN(anon_sym_reload);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1002:
      ACCEPT_TOKEN(anon_sym_return);
      END_STATE();
    case 1003:
      ACCEPT_TOKEN(anon_sym_return);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1004:
      ACCEPT_TOKEN(anon_sym_ride);
      END_STATE();
    case 1005:
      ACCEPT_TOKEN(anon_sym_ride);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1006:
      ACCEPT_TOKEN(anon_sym_rotate);
      END_STATE();
    case 1007:
      ACCEPT_TOKEN(anon_sym_rotate);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'd') ADVANCE(1113);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1008:
      ACCEPT_TOKEN(anon_sym_save_DASHall);
      END_STATE();
    case 1009:
      ACCEPT_TOKEN(anon_sym_save_DASHoff);
      END_STATE();
    case 1010:
      ACCEPT_TOKEN(anon_sym_save_DASHon);
      END_STATE();
    case 1011:
      ACCEPT_TOKEN(anon_sym_say);
      END_STATE();
    case 1012:
      ACCEPT_TOKEN(anon_sym_schedule);
      END_STATE();
    case 1013:
      ACCEPT_TOKEN(anon_sym_scoreboard);
      END_STATE();
    case 1014:
      ACCEPT_TOKEN(anon_sym_seed);
      END_STATE();
    case 1015:
      ACCEPT_TOKEN(anon_sym_setblock);
      END_STATE();
    case 1016:
      ACCEPT_TOKEN(anon_sym_setidletimeout);
      END_STATE();
    case 1017:
      ACCEPT_TOKEN(anon_sym_setworldspawn);
      END_STATE();
    case 1018:
      ACCEPT_TOKEN(anon_sym_spawnpoint);
      END_STATE();
    case 1019:
      ACCEPT_TOKEN(anon_sym_spectate);
      END_STATE();
    case 1020:
      ACCEPT_TOKEN(anon_sym_spreadplayers);
      END_STATE();
    case 1021:
      ACCEPT_TOKEN(anon_sym_stop);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1022:
      ACCEPT_TOKEN(anon_sym_stop);
      if (lookahead == 's') ADVANCE(779);
      END_STATE();
    case 1023:
      ACCEPT_TOKEN(anon_sym_stopsound);
      END_STATE();
    case 1024:
      ACCEPT_TOKEN(anon_sym_summon);
      END_STATE();
    case 1025:
      ACCEPT_TOKEN(anon_sym_summon);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1026:
      ACCEPT_TOKEN(anon_sym_swing);
      END_STATE();
    case 1027:
      ACCEPT_TOKEN(anon_sym_tag);
      END_STATE();
    case 1028:
      ACCEPT_TOKEN(anon_sym_team);
      if (lookahead == 'm') ADVANCE(832);
      END_STATE();
    case 1029:
      ACCEPT_TOKEN(anon_sym_teammsg);
      END_STATE();
    case 1030:
      ACCEPT_TOKEN(anon_sym_teleport);
      END_STATE();
    case 1031:
      ACCEPT_TOKEN(anon_sym_tell);
      if (lookahead == 'r') ADVANCE(526);
      END_STATE();
    case 1032:
      ACCEPT_TOKEN(anon_sym_tellraw);
      END_STATE();
    case 1033:
      ACCEPT_TOKEN(anon_sym_test);
      END_STATE();
    case 1034:
      ACCEPT_TOKEN(anon_sym_time);
      END_STATE();
    case 1035:
      ACCEPT_TOKEN(anon_sym_time);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1274);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1036:
      ACCEPT_TOKEN(anon_sym_title);
      END_STATE();
    case 1037:
      ACCEPT_TOKEN(anon_sym_tm);
      END_STATE();
    case 1038:
      ACCEPT_TOKEN(anon_sym_tp);
      END_STATE();
    case 1039:
      ACCEPT_TOKEN(anon_sym_transfer);
      END_STATE();
    case 1040:
      ACCEPT_TOKEN(anon_sym_trigger);
      END_STATE();
    case 1041:
      ACCEPT_TOKEN(anon_sym_version);
      END_STATE();
    case 1042:
      ACCEPT_TOKEN(anon_sym_version);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1043:
      ACCEPT_TOKEN(anon_sym_w);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(403);
      if (lookahead == 'e') ADVANCE(65);
      if (lookahead == 'h') ADVANCE(250);
      if (lookahead == 'i') ADVANCE(469);
      if (lookahead == 'o') ADVANCE(404);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1044:
      ACCEPT_TOKEN(anon_sym_w);
      if (lookahead == 'a') ADVANCE(887);
      if (lookahead == 'e') ADVANCE(541);
      if (lookahead == 'h') ADVANCE(685);
      if (lookahead == 'o') ADVANCE(815);
      END_STATE();
    case 1045:
      ACCEPT_TOKEN(anon_sym_waypoint);
      END_STATE();
    case 1046:
      ACCEPT_TOKEN(anon_sym_waypoint);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1047:
      ACCEPT_TOKEN(anon_sym_weather);
      END_STATE();
    case 1048:
      ACCEPT_TOKEN(anon_sym_weather);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1049:
      ACCEPT_TOKEN(anon_sym_whitelist);
      END_STATE();
    case 1050:
      ACCEPT_TOKEN(anon_sym_whitelist);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1051:
      ACCEPT_TOKEN(anon_sym_worldborder);
      END_STATE();
    case 1052:
      ACCEPT_TOKEN(anon_sym_worldborder);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1053:
      ACCEPT_TOKEN(anon_sym_xp);
      END_STATE();
    case 1054:
      ACCEPT_TOKEN(anon_sym_xp);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1055:
      ACCEPT_TOKEN(anon_sym_align);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1056:
      ACCEPT_TOKEN(anon_sym_align);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1057:
      ACCEPT_TOKEN(anon_sym_all);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1058:
      ACCEPT_TOKEN(anon_sym_all);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1059:
      ACCEPT_TOKEN(anon_sym_anchored);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1060:
      ACCEPT_TOKEN(anon_sym_anchored);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1061:
      ACCEPT_TOKEN(anon_sym_as);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1062:
      ACCEPT_TOKEN(anon_sym_as);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1063:
      ACCEPT_TOKEN(anon_sym_at);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 't') ADVANCE(64);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1064:
      ACCEPT_TOKEN(anon_sym_at);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1407);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1065:
      ACCEPT_TOKEN(anon_sym_at);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1066:
      ACCEPT_TOKEN(anon_sym_attacker);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1067:
      ACCEPT_TOKEN(anon_sym_attacker);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1068:
      ACCEPT_TOKEN(anon_sym_biome);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1069:
      ACCEPT_TOKEN(anon_sym_biome);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1070:
      ACCEPT_TOKEN(anon_sym_block);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(1072);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1071:
      ACCEPT_TOKEN(anon_sym_block);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1073);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1072:
      ACCEPT_TOKEN(anon_sym_blocks);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1073:
      ACCEPT_TOKEN(anon_sym_blocks);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1074:
      ACCEPT_TOKEN(anon_sym_controller);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1075:
      ACCEPT_TOKEN(anon_sym_controller);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1076:
      ACCEPT_TOKEN(anon_sym_dimension);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1077:
      ACCEPT_TOKEN(anon_sym_entity);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1078:
      ACCEPT_TOKEN(anon_sym_entity);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1079:
      ACCEPT_TOKEN(anon_sym_eyes);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1080:
      ACCEPT_TOKEN(anon_sym_eyes);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1081:
      ACCEPT_TOKEN(anon_sym_facing);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1082:
      ACCEPT_TOKEN(anon_sym_feet);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1083:
      ACCEPT_TOKEN(anon_sym_if);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1084:
      ACCEPT_TOKEN(anon_sym_if);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1085:
      ACCEPT_TOKEN(anon_sym_in);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 's') ADVANCE(201);
      if (lookahead == 'v') ADVANCE(203);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1086:
      ACCEPT_TOKEN(anon_sym_in);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1338);
      if (lookahead == 'v') ADVANCE(1615);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1087:
      ACCEPT_TOKEN(anon_sym_items);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1088:
      ACCEPT_TOKEN(anon_sym_items);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1089:
      ACCEPT_TOKEN(anon_sym_leasher);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1090:
      ACCEPT_TOKEN(anon_sym_leasher);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1091:
      ACCEPT_TOKEN(anon_sym_loaded);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1092:
      ACCEPT_TOKEN(anon_sym_loaded);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1093:
      ACCEPT_TOKEN(anon_sym_masked);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1094:
      ACCEPT_TOKEN(anon_sym_matches);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1095:
      ACCEPT_TOKEN(anon_sym_max);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1096:
      ACCEPT_TOKEN(anon_sym_on);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'l') ADVANCE(504);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1097:
      ACCEPT_TOKEN(anon_sym_on);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(2041);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1098:
      ACCEPT_TOKEN(anon_sym_on);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1099:
      ACCEPT_TOKEN(anon_sym_origin);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1100:
      ACCEPT_TOKEN(anon_sym_origin);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1101:
      ACCEPT_TOKEN(anon_sym_over);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1102:
      ACCEPT_TOKEN(anon_sym_over);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1103:
      ACCEPT_TOKEN(anon_sym_owner);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1104:
      ACCEPT_TOKEN(anon_sym_owner);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1105:
      ACCEPT_TOKEN(anon_sym_passengers);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1106:
      ACCEPT_TOKEN(anon_sym_passengers);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1107:
      ACCEPT_TOKEN(anon_sym_positioned);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1108:
      ACCEPT_TOKEN(anon_sym_positioned);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1109:
      ACCEPT_TOKEN(anon_sym_predicate);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1110:
      ACCEPT_TOKEN(anon_sym_predicate);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1111:
      ACCEPT_TOKEN(anon_sym_result);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1112:
      ACCEPT_TOKEN(anon_sym_result);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1113:
      ACCEPT_TOKEN(anon_sym_rotated);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1114:
      ACCEPT_TOKEN(anon_sym_rotated);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1115:
      ACCEPT_TOKEN(anon_sym_score);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1116:
      ACCEPT_TOKEN(anon_sym_storage);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1117:
      ACCEPT_TOKEN(anon_sym_store);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1118:
      ACCEPT_TOKEN(anon_sym_success);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1119:
      ACCEPT_TOKEN(anon_sym_target);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1120:
      ACCEPT_TOKEN(anon_sym_unless);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1121:
      ACCEPT_TOKEN(anon_sym_unless);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1122:
      ACCEPT_TOKEN(anon_sym_value);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1123:
      ACCEPT_TOKEN(anon_sym_value);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1124:
      ACCEPT_TOKEN(anon_sym_vehicle);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1125:
      ACCEPT_TOKEN(anon_sym_vehicle);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1126:
      ACCEPT_TOKEN(anon_sym_add);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_') ADVANCE(311);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1127:
      ACCEPT_TOKEN(anon_sym_add);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_') ADVANCE(1754);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1128:
      ACCEPT_TOKEN(anon_sym_add_multiplied_base);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1129:
      ACCEPT_TOKEN(anon_sym_add_multiplied_base);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1130:
      ACCEPT_TOKEN(anon_sym_add_multiplied_total);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1131:
      ACCEPT_TOKEN(anon_sym_add_multiplied_total);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1132:
      ACCEPT_TOKEN(anon_sym_add_value);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1133:
      ACCEPT_TOKEN(anon_sym_add_value);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1134:
      ACCEPT_TOKEN(anon_sym_amount);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1135:
      ACCEPT_TOKEN(anon_sym_amount);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1136:
      ACCEPT_TOKEN(anon_sym_append);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1137:
      ACCEPT_TOKEN(anon_sym_append);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1138:
      ACCEPT_TOKEN(anon_sym_base);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1139:
      ACCEPT_TOKEN(anon_sym_base);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1140:
      ACCEPT_TOKEN(anon_sym_buffer);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1141:
      ACCEPT_TOKEN(anon_sym_buffer);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1142:
      ACCEPT_TOKEN(anon_sym_by);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1143:
      ACCEPT_TOKEN(anon_sym_by);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1517);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1144:
      ACCEPT_TOKEN(anon_sym_center);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1145:
      ACCEPT_TOKEN(anon_sym_center);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1146:
      ACCEPT_TOKEN(anon_sym_color);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1147:
      ACCEPT_TOKEN(anon_sym_color);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1148:
      ACCEPT_TOKEN(anon_sym_create);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1149:
      ACCEPT_TOKEN(anon_sym_create);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1150:
      ACCEPT_TOKEN(anon_sym_day);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1692);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1151:
      ACCEPT_TOKEN(anon_sym_daytime);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1152:
      ACCEPT_TOKEN(anon_sym_destroy);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1153:
      ACCEPT_TOKEN(anon_sym_disable);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1154:
      ACCEPT_TOKEN(anon_sym_dismount);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1155:
      ACCEPT_TOKEN(anon_sym_distance);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1156:
      ACCEPT_TOKEN(anon_sym_enable);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1157:
      ACCEPT_TOKEN(anon_sym_enable);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1158:
      ACCEPT_TOKEN(anon_sym_everything);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1159:
      ACCEPT_TOKEN(anon_sym_everything);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1160:
      ACCEPT_TOKEN(anon_sym_export);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'c') ADVANCE(308);
      if (lookahead == 't') ADVANCE(234);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1161:
      ACCEPT_TOKEN(anon_sym_export);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1751);
      if (lookahead == 't') ADVANCE(1656);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1162:
      ACCEPT_TOKEN(anon_sym_exportclosest);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1163:
      ACCEPT_TOKEN(anon_sym_exportclosest);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1164:
      ACCEPT_TOKEN(anon_sym_exportthat);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1165:
      ACCEPT_TOKEN(anon_sym_exportthat);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1166:
      ACCEPT_TOKEN(anon_sym_exportthese);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1167:
      ACCEPT_TOKEN(anon_sym_exportthese);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1168:
      ACCEPT_TOKEN(anon_sym_fail);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1169:
      ACCEPT_TOKEN(anon_sym_feature);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1170:
      ACCEPT_TOKEN(anon_sym_fish);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1171:
      ACCEPT_TOKEN(anon_sym_flush);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1172:
      ACCEPT_TOKEN(anon_sym_force);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1173:
      ACCEPT_TOKEN(anon_sym_from);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1174:
      ACCEPT_TOKEN(anon_sym_gametime);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1175:
      ACCEPT_TOKEN(anon_sym_gametime);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1176:
      ACCEPT_TOKEN(anon_sym_get);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1177:
      ACCEPT_TOKEN(anon_sym_get);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1178:
      ACCEPT_TOKEN(anon_sym_grant);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1179:
      ACCEPT_TOKEN(anon_sym_grant);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1180:
      ACCEPT_TOKEN(anon_sym_hollow);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1181:
      ACCEPT_TOKEN(anon_sym_id);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1182:
      ACCEPT_TOKEN(anon_sym_id);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1183:
      ACCEPT_TOKEN(anon_sym_insert);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1184:
      ACCEPT_TOKEN(anon_sym_insert);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1185:
      ACCEPT_TOKEN(anon_sym_ips);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1186:
      ACCEPT_TOKEN(anon_sym_ips);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1187:
      ACCEPT_TOKEN(anon_sym_jigsaw);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1188:
      ACCEPT_TOKEN(anon_sym_jigsaw);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1189:
      ACCEPT_TOKEN(anon_sym_join);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1190:
      ACCEPT_TOKEN(anon_sym_join);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1191:
      ACCEPT_TOKEN(anon_sym_keep);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1192:
      ACCEPT_TOKEN(anon_sym_keep);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1193:
      ACCEPT_TOKEN(anon_sym_leave);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1194:
      ACCEPT_TOKEN(anon_sym_leave);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1195:
      ACCEPT_TOKEN(anon_sym_levels);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1196:
      ACCEPT_TOKEN(anon_sym_levels);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1197:
      ACCEPT_TOKEN(anon_sym_mainhand);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1198:
      ACCEPT_TOKEN(anon_sym_merge);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1199:
      ACCEPT_TOKEN(anon_sym_midnight);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1200:
      ACCEPT_TOKEN(anon_sym_mine);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1201:
      ACCEPT_TOKEN(anon_sym_modifier);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1202:
      ACCEPT_TOKEN(anon_sym_modify);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1203:
      ACCEPT_TOKEN(anon_sym_mount);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1204:
      ACCEPT_TOKEN(anon_sym_move);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1205:
      ACCEPT_TOKEN(anon_sym_name);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1206:
      ACCEPT_TOKEN(anon_sym_name);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1207:
      ACCEPT_TOKEN(anon_sym_night);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1208:
      ACCEPT_TOKEN(anon_sym_night);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1209:
      ACCEPT_TOKEN(anon_sym_noon);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1210:
      ACCEPT_TOKEN(anon_sym_noon);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1211:
      ACCEPT_TOKEN(anon_sym_normal);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1212:
      ACCEPT_TOKEN(anon_sym_normal);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1213:
      ACCEPT_TOKEN(anon_sym_objectives);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1214:
      ACCEPT_TOKEN(anon_sym_objectives);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1215:
      ACCEPT_TOKEN(anon_sym_off);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'h') ADVANCE(53);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1216:
      ACCEPT_TOKEN(anon_sym_off);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'h') ADVANCE(1418);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1217:
      ACCEPT_TOKEN(anon_sym_offhand);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1218:
      ACCEPT_TOKEN(anon_sym_offhand);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1219:
      ACCEPT_TOKEN(anon_sym_only);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1220:
      ACCEPT_TOKEN(anon_sym_only);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1221:
      ACCEPT_TOKEN(anon_sym_outline);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1222:
      ACCEPT_TOKEN(anon_sym_outline);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1223:
      ACCEPT_TOKEN(anon_sym_players);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1224:
      ACCEPT_TOKEN(anon_sym_players);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1225:
      ACCEPT_TOKEN(anon_sym_poi);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'n') ADVANCE(472);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1226:
      ACCEPT_TOKEN(anon_sym_poi);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(2001);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1227:
      ACCEPT_TOKEN(anon_sym_points);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1228:
      ACCEPT_TOKEN(anon_sym_points);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1229:
      ACCEPT_TOKEN(anon_sym_pos);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'i') ADVANCE(474);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1230:
      ACCEPT_TOKEN(anon_sym_pos);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1231:
      ACCEPT_TOKEN(anon_sym_prepend);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1232:
      ACCEPT_TOKEN(anon_sym_prepend);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1233:
      ACCEPT_TOKEN(anon_sym_query);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1234:
      ACCEPT_TOKEN(anon_sym_query);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1235:
      ACCEPT_TOKEN(anon_sym_rain);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1236:
      ACCEPT_TOKEN(anon_sym_rain);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1237:
      ACCEPT_TOKEN(anon_sym_remove);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1238:
      ACCEPT_TOKEN(anon_sym_remove);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1239:
      ACCEPT_TOKEN(anon_sym_replace);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1240:
      ACCEPT_TOKEN(anon_sym_replace);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1241:
      ACCEPT_TOKEN(anon_sym_reset);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'c') ADVANCE(307);
      if (lookahead == 't') ADVANCE(232);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1242:
      ACCEPT_TOKEN(anon_sym_reset);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1750);
      if (lookahead == 't') ADVANCE(1655);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1243:
      ACCEPT_TOKEN(anon_sym_resetclosest);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1244:
      ACCEPT_TOKEN(anon_sym_resetclosest);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1245:
      ACCEPT_TOKEN(anon_sym_resetthat);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1246:
      ACCEPT_TOKEN(anon_sym_resetthat);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1247:
      ACCEPT_TOKEN(anon_sym_resetthese);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1248:
      ACCEPT_TOKEN(anon_sym_resetthese);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1249:
      ACCEPT_TOKEN(anon_sym_revoke);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1250:
      ACCEPT_TOKEN(anon_sym_revoke);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1251:
      ACCEPT_TOKEN(anon_sym_roll);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1252:
      ACCEPT_TOKEN(anon_sym_roll);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1253:
      ACCEPT_TOKEN(anon_sym_runclosest);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1254:
      ACCEPT_TOKEN(anon_sym_runclosest);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1255:
      ACCEPT_TOKEN(anon_sym_runfailed);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1256:
      ACCEPT_TOKEN(anon_sym_runfailed);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1257:
      ACCEPT_TOKEN(anon_sym_runthat);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1258:
      ACCEPT_TOKEN(anon_sym_runthat);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1259:
      ACCEPT_TOKEN(anon_sym_runthese);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1260:
      ACCEPT_TOKEN(anon_sym_runthese);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1261:
      ACCEPT_TOKEN(anon_sym_set);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1674);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1262:
      ACCEPT_TOKEN(anon_sym_setdisplay);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1263:
      ACCEPT_TOKEN(anon_sym_show);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1264:
      ACCEPT_TOKEN(anon_sym_spawn);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1265:
      ACCEPT_TOKEN(anon_sym_start);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1266:
      ACCEPT_TOKEN(anon_sym_strict);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1267:
      ACCEPT_TOKEN(anon_sym_string);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1268:
      ACCEPT_TOKEN(anon_sym_structure);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1269:
      ACCEPT_TOKEN(anon_sym_style);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1270:
      ACCEPT_TOKEN(anon_sym_take);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1271:
      ACCEPT_TOKEN(anon_sym_template);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1272:
      ACCEPT_TOKEN(anon_sym_through);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1273:
      ACCEPT_TOKEN(anon_sym_thunder);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1274:
      ACCEPT_TOKEN(anon_sym_times);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1275:
      ACCEPT_TOKEN(anon_sym_to);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1276:
      ACCEPT_TOKEN(anon_sym_under);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1277:
      ACCEPT_TOKEN(anon_sym_under);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1278:
      ACCEPT_TOKEN(anon_sym_until);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1279:
      ACCEPT_TOKEN(anon_sym_until);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1280:
      ACCEPT_TOKEN(anon_sym_uuids);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1281:
      ACCEPT_TOKEN(anon_sym_uuids);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1282:
      ACCEPT_TOKEN(anon_sym_verify);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1283:
      ACCEPT_TOKEN(anon_sym_verify);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1284:
      ACCEPT_TOKEN(anon_sym_visible);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1285:
      ACCEPT_TOKEN(anon_sym_visible);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1286:
      ACCEPT_TOKEN(anon_sym_warning);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1287:
      ACCEPT_TOKEN(anon_sym_warning);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1288:
      ACCEPT_TOKEN(anon_sym_with);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1289:
      ACCEPT_TOKEN(anon_sym_with);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1290:
      ACCEPT_TOKEN(aux_sym_selector_token1);
      END_STATE();
    case 1291:
      ACCEPT_TOKEN(aux_sym_selector_token2);
      END_STATE();
    case 1292:
      ACCEPT_TOKEN(sym_selector_identifier);
      END_STATE();
    case 1293:
      ACCEPT_TOKEN(sym_query_identifier);
      if (lookahead == '#') ADVANCE(2067);
      if (lookahead == '-') ADVANCE(1295);
      if (lookahead == '!' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1293);
      END_STATE();
    case 1294:
      ACCEPT_TOKEN(sym_query_identifier);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1320);
      if (lookahead == '!' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1295);
      END_STATE();
    case 1295:
      ACCEPT_TOKEN(sym_query_identifier);
      if (lookahead == '!' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1295);
      END_STATE();
    case 1296:
      ACCEPT_TOKEN(anon_sym_EQ);
      END_STATE();
    case 1297:
      ACCEPT_TOKEN(anon_sym_EQ);
      if (lookahead == '!') ADVANCE(1298);
      END_STATE();
    case 1298:
      ACCEPT_TOKEN(anon_sym_EQ_BANG);
      END_STATE();
    case 1299:
      ACCEPT_TOKEN(aux_sym_selector_query_token1);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(1299);
      END_STATE();
    case 1300:
      ACCEPT_TOKEN(anon_sym_TILDE);
      END_STATE();
    case 1301:
      ACCEPT_TOKEN(anon_sym_CARET);
      END_STATE();
    case 1302:
      ACCEPT_TOKEN(sym_heightmap);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_') ADVANCE(1801);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1303:
      ACCEPT_TOKEN(sym_heightmap);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1304:
      ACCEPT_TOKEN(sym_resource);
      if (lookahead == '-' ||
          lookahead == '/') ADVANCE(1305);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      if (lookahead == '+' ||
          ('.' <= lookahead && lookahead <= '9') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1304);
      END_STATE();
    case 1305:
      ACCEPT_TOKEN(sym_resource);
      if (lookahead == '+' ||
          ('-' <= lookahead && lookahead <= '9') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1305);
      END_STATE();
    case 1306:
      ACCEPT_TOKEN(anon_sym_DASH);
      END_STATE();
    case 1307:
      ACCEPT_TOKEN(anon_sym_DASH);
      if (lookahead == '.') ADVANCE(30);
      if (lookahead == '=') ADVANCE(1340);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(35);
      END_STATE();
    case 1308:
      ACCEPT_TOKEN(anon_sym_DASH);
      if (lookahead == '.') ADVANCE(30);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(35);
      END_STATE();
    case 1309:
      ACCEPT_TOKEN(anon_sym_DASH);
      if (lookahead == '=') ADVANCE(1340);
      END_STATE();
    case 1310:
      ACCEPT_TOKEN(anon_sym_DASH);
      if (lookahead == '!' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1295);
      END_STATE();
    case 1311:
      ACCEPT_TOKEN(aux_sym_scale_token1);
      if (lookahead == '.') ADVANCE(1318);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1311);
      END_STATE();
    case 1312:
      ACCEPT_TOKEN(aux_sym_scale_token1);
      if (lookahead == '.') ADVANCE(2080);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1318);
      END_STATE();
    case 1313:
      ACCEPT_TOKEN(aux_sym_scale_token1);
      if (lookahead == '.') ADVANCE(1319);
      if (lookahead == 'b') ADVANCE(2067);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1314);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 1314:
      ACCEPT_TOKEN(aux_sym_scale_token1);
      if (lookahead == '.') ADVANCE(1319);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1314);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 1315:
      ACCEPT_TOKEN(aux_sym_scale_token1);
      if (lookahead == '.') ADVANCE(1320);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1315);
      if (lookahead == '!' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1295);
      END_STATE();
    case 1316:
      ACCEPT_TOKEN(aux_sym_scale_token1);
      if (lookahead == '.') ADVANCE(1312);
      if (lookahead == 'b') ADVANCE(2078);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1317);
      END_STATE();
    case 1317:
      ACCEPT_TOKEN(aux_sym_scale_token1);
      if (lookahead == '.') ADVANCE(1312);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1317);
      END_STATE();
    case 1318:
      ACCEPT_TOKEN(aux_sym_scale_token1);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1318);
      END_STATE();
    case 1319:
      ACCEPT_TOKEN(aux_sym_scale_token1);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1319);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 1320:
      ACCEPT_TOKEN(aux_sym_scale_token1);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1320);
      if (lookahead == '!' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1295);
      END_STATE();
    case 1321:
      ACCEPT_TOKEN(aux_sym_scale_token2);
      END_STATE();
    case 1322:
      ACCEPT_TOKEN(aux_sym_scale_token2);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'a') ADVANCE(284);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1323:
      ACCEPT_TOKEN(aux_sym_scale_token2);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == 'r') ADVANCE(490);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1324:
      ACCEPT_TOKEN(aux_sym_scale_token2);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1325:
      ACCEPT_TOKEN(aux_sym_scale_token2);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2082);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2083);
      END_STATE();
    case 1326:
      ACCEPT_TOKEN(aux_sym_scale_token2);
      if (lookahead == 'a') ADVANCE(712);
      END_STATE();
    case 1327:
      ACCEPT_TOKEN(aux_sym_scale_token2);
      if (lookahead == 'r') ADVANCE(874);
      END_STATE();
    case 1328:
      ACCEPT_TOKEN(aux_sym_scale_token2);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2083);
      END_STATE();
    case 1329:
      ACCEPT_TOKEN(anon_sym_DOLLAR_LPAREN);
      END_STATE();
    case 1330:
      ACCEPT_TOKEN(anon_sym_DOLLAR_LPAREN);
      if (lookahead != 0 &&
          lookahead != '"') ADVANCE(2072);
      END_STATE();
    case 1331:
      ACCEPT_TOKEN(anon_sym_DOLLAR_LPAREN);
      if (lookahead != 0 &&
          lookahead != '\'') ADVANCE(2077);
      END_STATE();
    case 1332:
      ACCEPT_TOKEN(aux_sym_macro_token1);
      if (('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1332);
      END_STATE();
    case 1333:
      ACCEPT_TOKEN(anon_sym_RPAREN);
      END_STATE();
    case 1334:
      ACCEPT_TOKEN(anon_sym_PERCENT_LBRACK);
      END_STATE();
    case 1335:
      ACCEPT_TOKEN(anon_sym_PERCENT_LBRACK);
      if (lookahead != 0 &&
          lookahead != '"') ADVANCE(2072);
      END_STATE();
    case 1336:
      ACCEPT_TOKEN(anon_sym_PERCENT_LBRACK);
      if (lookahead != 0 &&
          lookahead != '\'') ADVANCE(2077);
      END_STATE();
    case 1337:
      ACCEPT_TOKEN(aux_sym_substitution_token1);
      END_STATE();
    case 1338:
      ACCEPT_TOKEN(sym_type);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1339:
      ACCEPT_TOKEN(anon_sym_PLUS_EQ);
      END_STATE();
    case 1340:
      ACCEPT_TOKEN(anon_sym_DASH_EQ);
      END_STATE();
    case 1341:
      ACCEPT_TOKEN(anon_sym_STAR_EQ);
      END_STATE();
    case 1342:
      ACCEPT_TOKEN(anon_sym_SLASH_EQ);
      END_STATE();
    case 1343:
      ACCEPT_TOKEN(anon_sym_PERCENT_EQ);
      END_STATE();
    case 1344:
      ACCEPT_TOKEN(anon_sym_GT_LT);
      END_STATE();
    case 1345:
      ACCEPT_TOKEN(anon_sym_LT);
      END_STATE();
    case 1346:
      ACCEPT_TOKEN(anon_sym_GT);
      if (lookahead == '<') ADVANCE(1344);
      END_STATE();
    case 1347:
      ACCEPT_TOKEN(aux_sym_slot_token1);
      if (lookahead == '*') ADVANCE(1348);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1347);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 1348:
      ACCEPT_TOKEN(aux_sym_slot_token1);
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1348);
      END_STATE();
    case 1349:
      ACCEPT_TOKEN(aux_sym_slot_token2);
      if (lookahead == '*') ADVANCE(1350);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1349);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 1350:
      ACCEPT_TOKEN(aux_sym_slot_token2);
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1350);
      END_STATE();
    case 1351:
      ACCEPT_TOKEN(aux_sym_slot_token3);
      if (lookahead == '*') ADVANCE(1352);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1351);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 1352:
      ACCEPT_TOKEN(aux_sym_slot_token3);
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1352);
      END_STATE();
    case 1353:
      ACCEPT_TOKEN(aux_sym_slot_token4);
      if (lookahead == '*') ADVANCE(1354);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1353);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 1354:
      ACCEPT_TOKEN(aux_sym_slot_token4);
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1354);
      END_STATE();
    case 1355:
      ACCEPT_TOKEN(aux_sym_slot_token5);
      if (lookahead == '*') ADVANCE(1356);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1355);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 1356:
      ACCEPT_TOKEN(aux_sym_slot_token5);
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1356);
      END_STATE();
    case 1357:
      ACCEPT_TOKEN(aux_sym_slot_token6);
      if (lookahead == '*') ADVANCE(1357);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1359);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(1367);
      END_STATE();
    case 1358:
      ACCEPT_TOKEN(aux_sym_slot_token6);
      if (lookahead == '*') ADVANCE(1359);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1358);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 1359:
      ACCEPT_TOKEN(aux_sym_slot_token6);
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1359);
      END_STATE();
    case 1360:
      ACCEPT_TOKEN(aux_sym_slot_token7);
      if (lookahead == '*') ADVANCE(1361);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1360);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 1361:
      ACCEPT_TOKEN(aux_sym_slot_token7);
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1361);
      END_STATE();
    case 1362:
      ACCEPT_TOKEN(anon_sym_contents);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 1363:
      ACCEPT_TOKEN(anon_sym_contents);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1364:
      ACCEPT_TOKEN(anon_sym_player_DOTcursor);
      END_STATE();
    case 1365:
      ACCEPT_TOKEN(anon_sym_player_DOTcursor);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 1366:
      ACCEPT_TOKEN(aux_sym_slot_token8);
      if (lookahead == '*') ADVANCE(1367);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(1366);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_') ADVANCE(2067);
      END_STATE();
    case 1367:
      ACCEPT_TOKEN(aux_sym_slot_token8);
      if (lookahead == '*' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1367);
      END_STATE();
    case 1368:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '*') ADVANCE(1356);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1355);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 1369:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '*') ADVANCE(1348);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1347);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 1370:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '*') ADVANCE(1352);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1351);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 1371:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '*') ADVANCE(1354);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1353);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 1372:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '*') ADVANCE(1361);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1360);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 1373:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '*') ADVANCE(1367);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(1366);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_') ADVANCE(2067);
      END_STATE();
    case 1374:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '*') ADVANCE(1357);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1358);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(1366);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_') ADVANCE(2067);
      END_STATE();
    case 1375:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '*') ADVANCE(1350);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1349);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 1376:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '.') ADVANCE(1373);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1377:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '.') ADVANCE(1374);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1378:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '.') ADVANCE(1375);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1379:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '.') ADVANCE(2055);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1224);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1380:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '.') ADVANCE(2055);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1381:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '.') ADVANCE(1368);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1382:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '.') ADVANCE(1369);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1383:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '.') ADVANCE(1370);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1384:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '.') ADVANCE(1371);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1385:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '.') ADVANCE(1372);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 1386:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '/') ADVANCE(1305);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1304);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          ('.' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1387:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_') ADVANCE(1458);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1388:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_') ADVANCE(1624);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1389:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_') ADVANCE(1954);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1390:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_') ADVANCE(1744);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1391:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_') ADVANCE(1457);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1392:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1936);
      if (lookahead == 'i') ADVANCE(1853);
      if (lookahead == 'l') ADVANCE(1819);
      if (lookahead == 'u') ADVANCE(1622);
      if (lookahead == 'y') ADVANCE(1143);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1393:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1756);
      if (lookahead == 'e') ADVANCE(1935);
      if (lookahead == 'i') ADVANCE(1919);
      if (lookahead == 'o') ADVANCE(2020);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1394:
      ACCEPT_TOKEN(aux_sym_word_token1);
      ADVANCE_MAP(
        ':', 1386,
        'a', 1479,
        'e', 1411,
        'i', 1920,
        'l', 1847,
        'o', 1889,
        'r', 1825,
        'u', 1785,
      );
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1395:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1670);
      if (lookahead == 'e') ADVANCE(1893);
      if (lookahead == 'i') ADVANCE(1506);
      if (lookahead == 'o') ADVANCE(1504);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1396:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1703);
      if (lookahead == 'e') ADVANCE(1753);
      if (lookahead == 'h') ADVANCE(1916);
      if (lookahead == 'i') ADVANCE(1760);
      if (lookahead == 'o') ADVANCE(1275);
      if (lookahead == 'r') ADVANCE(2023);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1397:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(2040);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1398:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1480);
      if (lookahead == 'e') ADVANCE(1605);
      if (lookahead == 'l') ADVANCE(1848);
      if (lookahead == 'u') ADVANCE(1785);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1399:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1956);
      if (lookahead == 'o') ADVANCE(1995);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1400:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(934);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1401:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1757);
      if (lookahead == 'e') ADVANCE(1966);
      if (lookahead == 'i') ADVANCE(2033);
      if (lookahead == 'r') ADVANCE(1430);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1402:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1743);
      if (lookahead == 'e') ADVANCE(1899);
      if (lookahead == 'i') ADVANCE(1740);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1403:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1743);
      if (lookahead == 'e') ADVANCE(1650);
      if (lookahead == 'i') ADVANCE(1741);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1404:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(2050);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1405:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(2039);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1406:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1903);
      if (lookahead == 'e') ADVANCE(1433);
      if (lookahead == 'i') ADVANCE(1993);
      if (lookahead == 'o') ADVANCE(1902);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1407:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1482);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1408:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1493);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1409:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1643);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1410:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1907);
      if (lookahead == 'o') ADVANCE(1864);
      if (lookahead == 'r') ADVANCE(1665);
      if (lookahead == 'y') ADVANCE(1729);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1411:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1994);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1412:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(2046);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1413:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1874);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1414:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(2007);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1415:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1968);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1416:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1774);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1417:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1711);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1418:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1792);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1419:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1959);
      if (lookahead == 'l') ADVANCE(1454);
      if (lookahead == 'o') ADVANCE(1953);
      if (lookahead == 'r') ADVANCE(1571);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1420:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1892);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1421:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1712);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1422:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1738);
      if (lookahead == 'l') ADVANCE(1848);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1423:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1939);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1424:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1795);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1425:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1981);
      if (lookahead == 'e') ADVANCE(1949);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1426:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1885);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1427:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1984);
      if (lookahead == 'e') ADVANCE(1950);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1428:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1985);
      if (lookahead == 'e') ADVANCE(1951);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1429:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(2006);
      if (lookahead == 'i') ADVANCE(1765);
      if (lookahead == 'o') ADVANCE(2020);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1430:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1799);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1431:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(2034);
      if (lookahead == 'v') ADVANCE(1568);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1432:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1673);
      if (lookahead == 'e') ADVANCE(1718);
      if (lookahead == 'o') ADVANCE(1717);
      if (lookahead == 'u') ADVANCE(1769);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1433:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1869);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1434:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1759);
      if (lookahead == 'i') ADVANCE(1633);
      if (lookahead == 'o') ADVANCE(1828);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1435:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1811);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1436:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1473);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1437:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1511);
      if (lookahead == 'n') ADVANCE(1627);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1438:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1638);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1439:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1695);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1440:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(2008);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1441:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1639);
      if (lookahead == 'e') ADVANCE(1117);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1442:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1639);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1443:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(2009);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1444:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1475);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1445:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(2010);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1446:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(2037);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1447:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(2011);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1448:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1683);
      if (lookahead == 'e') ADVANCE(1805);
      if (lookahead == 'r') ADVANCE(1850);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1449:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1683);
      if (lookahead == 'e') ADVANCE(1805);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1450:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1952);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1451:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1462);
      if (lookahead == 'd') ADVANCE(1608);
      if (lookahead == 't') ADVANCE(1675);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1452:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1918);
      if (lookahead == 'r') ADVANCE(2023);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1453:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1745);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1454:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(2051);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1455:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'a') ADVANCE(1463);
      if (lookahead == 'm') ADVANCE(1859);
      if (lookahead == 't') ADVANCE(1435);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1456:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'b') ADVANCE(1701);
      if (lookahead == 'c') ADVANCE(1566);
      if (lookahead == 'f') ADVANCE(1619);
      if (lookahead == 'n') ADVANCE(1097);
      if (lookahead == 'u') ADVANCE(1996);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1457:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'b') ADVANCE(1739);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1458:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'b') ADVANCE(1450);
      if (lookahead == 't') ADVANCE(1842);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1459:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'b') ADVANCE(1420);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1460:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'b') ADVANCE(1719);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1461:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'b') ADVANCE(1426);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1462:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'b') ADVANCE(1731);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1463:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'b') ADVANCE(1732);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1464:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'b') ADVANCE(1733);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1465:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1702);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1466:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1706);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1467:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1653);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1468:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1660);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1469:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(2017);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1470:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1526);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1471:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(2003);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1472:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1980);
      if (lookahead == 'n') ADVANCE(1629);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1473:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1541);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1474:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1544);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1475:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1552);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1476:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1566);
      if (lookahead == 'n') ADVANCE(1098);
      if (lookahead == 'r') ADVANCE(1679);
      if (lookahead == 'v') ADVANCE(1596);
      if (lookahead == 'w') ADVANCE(1816);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1477:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1566);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1478:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1654);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1479:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1678);
      if (lookahead == 'i') ADVANCE(1708);
      if (lookahead == 'l') ADVANCE(1941);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1480:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1678);
      if (lookahead == 'l') ADVANCE(1941);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1481:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1440);
      if (lookahead == 'n') ADVANCE(1627);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1482:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1707);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1483:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1488);
      if (lookahead == 'm') ADVANCE(1766);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1484:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1851);
      if (lookahead == 'h') ADVANCE(1854);
      if (lookahead == 't') ADVANCE(1838);
      if (lookahead == 'u') ADVANCE(1483);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1485:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1447);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1486:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1737);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1487:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(2016);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1488:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'c') ADVANCE(1613);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1489:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1491);
      if (lookahead == 'm') ADVANCE(1824);
      if (lookahead == 'p') ADVANCE(1865);
      if (lookahead == 'r') ADVANCE(1755);
      if (lookahead == 't') ADVANCE(1065);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1490:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1182);
      if (lookahead == 'n') ADVANCE(1957);
      if (lookahead == 'p') ADVANCE(1921);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1491:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1127);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1492:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1137);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1493:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1001);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1494:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1218);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1495:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1232);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1496:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1197);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1497:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1256);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1498:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1092);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1499:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1093);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1500:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1114);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1501:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1060);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1502:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1108);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1503:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1389);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1504:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1667);
      if (lookahead == 't') ADVANCE(1677);
      if (lookahead == 'u') ADVANCE(1800);
      if (lookahead == 'v') ADVANCE(1521);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1505:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1387);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1506:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1798);
      if (lookahead == 'n') ADVANCE(1520);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1507:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1922);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1508:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1608);
      if (lookahead == 't') ADVANCE(1675);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1509:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1608);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1510:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1672);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1511:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1572);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1512:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1573);
      if (lookahead == 't') ADVANCE(1680);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1513:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'd') ADVANCE(1588);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1514:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(2052);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1515:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1788);
      if (lookahead == 'l') ADVANCE(1563);
      if (lookahead == 'o') ADVANCE(1742);
      if (lookahead == 'r') ADVANCE(1607);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1516:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1431);
      if (lookahead == 'i') ADVANCE(1937);
      if (lookahead == 'o') ADVANCE(1481);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1517:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1338);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1518:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1139);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1519:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(960);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1520:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1200);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1521:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1204);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1522:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1206);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1523:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1270);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1524:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1035);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1525:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1069);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1526:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1172);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1527:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1194);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1528:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1198);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1529:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1269);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1530:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1123);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1531:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1149);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1532:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(933);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1533:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1157);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1534:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(975);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1535:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1238);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1536:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1250);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1537:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1151);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1538:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1153);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1539:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1169);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1540:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1222);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1541:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1240);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1542:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1116);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1543:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1285);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1544:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1155);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1545:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1175);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1546:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1260);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1547:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1271);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1548:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1133);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1549:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1268);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1550:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1248);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1551:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1167);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1552:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1303);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1553:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1129);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1554:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1115);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1555:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1125);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1556:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1110);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1557:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1377);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1558:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1863);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1559:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1969);
      if (lookahead == 'h') ADVANCE(1820);
      if (lookahead == 'p') ADVANCE(1397);
      if (lookahead == 't') ADVANCE(1410);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1560:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1558);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1561:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1433);
      if (lookahead == 'o') ADVANCE(1902);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1562:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1762);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1563:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1413);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1564:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1890);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1565:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1895);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1566:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1416);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1567:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1497);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1568:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1720);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1569:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1505);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1570:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1787);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1571:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1510);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1572:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1498);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1573:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1876);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1574:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1499);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1575:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1926);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1576:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1877);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1577:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(2018);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1578:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1500);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1579:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1927);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1580:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1878);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1581:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1501);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1582:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1502);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1583:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1793);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1584:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1879);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1585:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1961);
      if (lookahead == 'o') ADVANCE(2014);
      if (lookahead == 'u') ADVANCE(1777);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1586:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1961);
      if (lookahead == 'o') ADVANCE(2014);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1587:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1446);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1588:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1880);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1589:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1975);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1590:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1423);
      if (lookahead == 'o') ADVANCE(1437);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1591:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1928);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1592:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1881);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1593:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1896);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1594:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1595:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1931);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1596:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1883);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1597:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1884);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1598:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1901);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1599:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1803);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1600:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1886);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1601:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1817);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1602:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1887);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1603:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1910);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1604:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1888);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1605:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1990);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1606:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1992);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1607:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1414);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1608:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1894);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1609:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1471);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1610:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1943);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1611:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1945);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1612:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1909);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1613:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1944);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1614:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1946);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1615:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1804);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1616:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1947);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1617:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1948);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1618:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'e') ADVANCE(1868);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1619:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'f') ADVANCE(1216);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1620:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'f') ADVANCE(1697);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1621:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'f') ADVANCE(1084);
      if (lookahead == 'n') ADVANCE(1086);
      if (lookahead == 't') ADVANCE(1562);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1622:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'f') ADVANCE(1625);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1623:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'f') ADVANCE(2044);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1624:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'f') ADVANCE(1725);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1625:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'f') ADVANCE(1576);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1626:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'f') ADVANCE(1444);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1627:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'g') ADVANCE(1338);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1628:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'g') ADVANCE(1081);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1629:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'g') ADVANCE(1267);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1630:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'g') ADVANCE(1287);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1631:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'g') ADVANCE(1159);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1632:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'g') ADVANCE(1302);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1633:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'g') ADVANCE(1651);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1634:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'g') ADVANCE(1938);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1635:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'g') ADVANCE(1648);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1636:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'g') ADVANCE(1528);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1637:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'g') ADVANCE(1778);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1638:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'g') ADVANCE(1532);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1639:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'g') ADVANCE(1542);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1640:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'g') ADVANCE(1652);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1641:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'g') ADVANCE(1606);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1642:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'g') ADVANCE(1686);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1643:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'g') ADVANCE(1593);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1644:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'g') ADVANCE(1603);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1645:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'h') ADVANCE(1170);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1646:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'h') ADVANCE(1289);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1647:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'h') ADVANCE(1171);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1648:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'h') ADVANCE(1272);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1649:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'h') ADVANCE(1425);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1650:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'h') ADVANCE(1671);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1651:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'h') ADVANCE(1974);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1652:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'h') ADVANCE(1983);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1653:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'h') ADVANCE(1611);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1654:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'h') ADVANCE(1595);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1655:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'h') ADVANCE(1427);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1656:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'h') ADVANCE(1428);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1657:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'h') ADVANCE(1424);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1658:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'h') ADVANCE(1684);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1659:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'h') ADVANCE(1854);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1660:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'h') ADVANCE(1855);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1661:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'h') ADVANCE(1600);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1662:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1634);
      if (lookahead == 'o') ADVANCE(1668);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1663:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1226);
      if (lookahead == 's') ADVANCE(1230);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1664:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1507);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1665:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1472);
      if (lookahead == 'u') ADVANCE(1487);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1666:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1853);
      if (lookahead == 'l') ADVANCE(1819);
      if (lookahead == 'o') ADVANCE(1940);
      if (lookahead == 'y') ADVANCE(2000);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1667:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1620);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1668:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1770);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1669:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1623);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1670:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1790);
      if (lookahead == 'x') ADVANCE(1095);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1671:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1486);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1672:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1485);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1673:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1772);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1674:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1955);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1675:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1997);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1676:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1637);
      if (lookahead == 'l') ADVANCE(1058);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1677:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1833);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1678:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1789);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1679:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1642);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1680:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1710);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1681:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1807);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1682:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1794);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1683:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1815);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1684:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1796);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1685:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1797);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1686:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1779);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1687:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1569);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1688:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1741);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1689:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1867);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1690:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1836);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1691:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(2036);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1692:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1763);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1693:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1846);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1694:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1764);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1695:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1734);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1696:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1840);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1697:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1592);
      if (lookahead == 'y') ADVANCE(1202);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1698:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1640);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1699:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(1464);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1700:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'i') ADVANCE(2019);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1701:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'j') ADVANCE(1609);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1702:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'k') ADVANCE(1071);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1703:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'k') ADVANCE(1523);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1704:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'k') ADVANCE(1536);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1705:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'k') ADVANCE(1574);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1706:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'k') ADVANCE(1685);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1707:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'k') ADVANCE(1602);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1708:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1168);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1709:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1252);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1710:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1279);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1711:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1212);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1712:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1131);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1713:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1676);
      if (lookahead == 'n') ADVANCE(1468);
      if (lookahead == 'r') ADVANCE(1755);
      if (lookahead == 's') ADVANCE(1062);
      if (lookahead == 't') ADVANCE(1064);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1714:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1404);
      if (lookahead == 'o') ADVANCE(1663);
      if (lookahead == 'r') ADVANCE(1618);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1715:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1503);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1716:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1721);
      if (lookahead == 'r') ADVANCE(1942);
      if (lookahead == 't') ADVANCE(1459);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1717:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1709);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1718:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1832);
      if (lookahead == 'm') ADVANCE(1849);
      if (lookahead == 'p') ADVANCE(1722);
      if (lookahead == 's') ADVANCE(1589);
      if (lookahead == 'v') ADVANCE(1843);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1719:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1517);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1720:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1923);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1721:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1823);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1722:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1436);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1723:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1852);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1724:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1687);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1725:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1856);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1726:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1409);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1727:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1412);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1728:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1748);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1729:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1529);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1730:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(2005);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1731:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1533);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1732:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1538);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1733:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1543);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1734:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1567);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1735:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1991);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1736:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1610);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1737:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1555);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1738:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1941);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1739:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1844);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1740:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1726);
      if (lookahead == 's') ADVANCE(1699);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1741:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1726);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1742:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1831);
      if (lookahead == 'n') ADVANCE(1970);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1743:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(2025);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1744:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1587);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1745:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(2026);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1746:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1681);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1747:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1443);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1748:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1604);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1749:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1454);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1750:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1861);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1751:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'l') ADVANCE(1862);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1752:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'm') ADVANCE(1173);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1753:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'm') ADVANCE(1866);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1754:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'm') ADVANCE(2021);
      if (lookahead == 'v') ADVANCE(1453);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1755:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'm') ADVANCE(1830);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1756:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'm') ADVANCE(1438);
      if (lookahead == 'y') ADVANCE(1150);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1757:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'm') ADVANCE(1577);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1758:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'm') ADVANCE(1417);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1759:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'm') ADVANCE(1522);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1760:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'm') ADVANCE(1524);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1761:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'm') ADVANCE(1525);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1762:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'm') ADVANCE(1929);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1763:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'm') ADVANCE(1537);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1764:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'm') ADVANCE(1545);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1765:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'm') ADVANCE(1599);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1766:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'm') ADVANCE(1839);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1767:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1451);
      if (lookahead == 'v') ADVANCE(1564);
      if (lookahead == 'x') ADVANCE(1871);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1768:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1512);
      if (lookahead == 'u') ADVANCE(1664);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1769:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(912);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1770:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1190);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1771:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1210);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1772:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1236);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1773:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1376);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1774:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1388);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1775:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1264);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1776:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(953);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1777:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(913);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1778:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1056);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1779:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1100);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1780:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1025);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1781:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1076);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1782:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1967);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1783:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1513);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1784:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1627);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1785:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1469);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1786:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1391);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1787:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1492);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1788:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(2015);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1789:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1628);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1790:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1657);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1791:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1970);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1792:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1494);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1793:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1495);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1794:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1630);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1795:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1496);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1796:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1631);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1797:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1632);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1798:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1698);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1799:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1972);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1800:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1973);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1801:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1827);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1802:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1977);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1803:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1964);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1804:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(2013);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1805:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(2004);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1806:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1982);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1807:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1540);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1808:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1989);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1809:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1582);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1810:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1508);
      if (lookahead == 'y') ADVANCE(1591);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1811:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1474);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1812:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1509);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1813:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1682);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1814:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1736);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1815:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1594);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1816:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1597);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1817:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'n') ADVANCE(1644);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1818:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1716);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1819:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1465);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1820:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1904);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1821:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1917);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1822:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(2020);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1823:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(2038);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1824:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(2031);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1825:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1752);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1826:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(2027);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1827:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1390);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1828:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1771);
      if (lookahead == 'r') ADVANCE(1758);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1829:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(2045);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1830:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1873);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1831:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1875);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1832:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1408);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1833:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1786);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1834:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1773);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1835:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1900);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1836:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1776);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1837:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1838:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1891);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1839:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1780);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1840:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1781);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1841:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1784);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1842:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(2012);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1843:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1704);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1844:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1466);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1845:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1908);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1846:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1809);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1847:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1415);
      if (lookahead == 'u') ADVANCE(1934);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1848:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1415);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1849:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(2035);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1850:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1728);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1851:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1913);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1852:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1962);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1853:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1761);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1854:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1905);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1855:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1914);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1856:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1837);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1857:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1808);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1858:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1791);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1859:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(2032);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1860:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1995);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1861:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1963);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1862:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'o') ADVANCE(1965);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1863:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'p') ADVANCE(1192);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1864:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'p') ADVANCE(1021);
      if (lookahead == 'r') ADVANCE(1442);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1865:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'p') ADVANCE(1570);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1866:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'p') ADVANCE(1747);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1867:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'p') ADVANCE(1724);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1868:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'p') ADVANCE(1583);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1869:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'p') ADVANCE(1834);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1870:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'p') ADVANCE(1727);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1871:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'p') ADVANCE(1845);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1872:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1755);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1873:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1376);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1874:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(929);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1875:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1147);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1876:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1277);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1877:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1141);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1878:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1145);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1879:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1379);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1880:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1273);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1881:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1201);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1882:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1303);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1883:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1102);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1884:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1104);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1885:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(926);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1886:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1090);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1887:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1067);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1888:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1075);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1889:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1470);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1890:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(2048);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1891:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1441);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1892:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1378);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1893:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1636);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1894:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1467);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1895:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(2042);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1896:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1381);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1897:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1382);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1898:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1626);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1899:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1669);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1900:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(2047);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1901:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1380);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1902:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1715);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1903:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1813);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1904:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1968);
      if (lookahead == 'w') ADVANCE(1263);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1905:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1968);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1906:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1829);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1907:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1976);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1908:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1978);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1909:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1979);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1910:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1933);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1911:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1539);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1912:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1549);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1913:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1554);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1914:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1581);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1915:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(2023);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1916:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1826);
      if (lookahead == 'u') ADVANCE(1783);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1917:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1942);
      if (lookahead == 't') ADVANCE(1459);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1918:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'r') ADVANCE(1641);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1919:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1455);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1920:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1645);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1921:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1186);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1922:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1281);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1923:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1196);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1924:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1228);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1925:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1363);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1926:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1214);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1927:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1303);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1928:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1080);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1929:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1088);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1930:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1121);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1931:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1094);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1932:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1118);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1933:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1106);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1934:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1647);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1935:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(2002);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1936:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1518);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1937:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1971);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1938:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1405);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1939:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1661);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1940:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1960);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1941:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1514);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1942:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1557);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1943:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1930);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1944:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1932);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1945:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1998);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1946:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1986);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1947:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1987);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1948:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1988);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1949:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1546);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1950:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1550);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1951:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1551);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1952:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1553);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1953:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1700);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1954:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(2024);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1955:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1870);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1956:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1705);
      if (lookahead == 't') ADVANCE(1478);
      if (lookahead == 'x') ADVANCE(1095);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1957:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1612);
      if (lookahead == 't') ADVANCE(1338);
      if (lookahead == 'v') ADVANCE(1615);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1958:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1601);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1959:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1958);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1960:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1461);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1961:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(2029);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1962:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1614);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1963:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1616);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1964:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1696);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1965:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 's') ADVANCE(1617);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1966:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1177);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1967:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1338);
      if (lookahead == 'v') ADVANCE(1615);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1968:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1338);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1969:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1261);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1970:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1449);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1971:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(972);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1972:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1179);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1973:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1203);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1974:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1208);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1975:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1242);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1976:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1265);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1977:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1135);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1978:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1161);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1979:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1184);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1980:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1266);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1981:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1258);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1982:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1154);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1983:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1199);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1984:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1246);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1985:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1165);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1986:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1254);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1987:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1244);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1988:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1163);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1989:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1448);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1990:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1082);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1991:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1112);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1992:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1119);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1993:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1646);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1994:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(2022);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1995:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1677);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1996:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1746);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1997:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(2043);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1998:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1384);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 1999:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1658);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2000:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1517);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2001:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1924);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2002:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1906);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2003:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1691);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2004:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1925);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2005:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1689);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2006:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1400);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2007:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1531);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2008:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1534);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2009:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1547);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2010:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1578);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2011:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1556);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2012:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1421);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2013:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1835);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2014:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1445);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2015:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1580);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2016:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(2030);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2017:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1690);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2018:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1694);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2019:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 't') ADVANCE(1693);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2020:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'u') ADVANCE(1460);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2021:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'u') ADVANCE(1730);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2022:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'u') ADVANCE(1911);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2023:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'u') ADVANCE(1514);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2024:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'u') ADVANCE(1898);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2025:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'u') ADVANCE(1530);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2026:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'u') ADVANCE(1548);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2027:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'u') ADVANCE(1635);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2028:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'u') ADVANCE(1565);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2029:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'u') ADVANCE(1735);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2030:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'u') ADVANCE(1912);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2031:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'u') ADVANCE(1802);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2032:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'u') ADVANCE(1806);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2033:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'v') ADVANCE(1519);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2034:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'v') ADVANCE(1527);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2035:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'v') ADVANCE(1535);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2036:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'v') ADVANCE(1575);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2037:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'v') ADVANCE(1579);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2038:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'w') ADVANCE(1180);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2039:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'w') ADVANCE(1188);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2040:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'w') ADVANCE(1775);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2041:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'y') ADVANCE(1220);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2042:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'y') ADVANCE(1234);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2043:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'y') ADVANCE(1078);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2044:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'y') ADVANCE(1283);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2045:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'y') ADVANCE(1152);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2046:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'y') ADVANCE(1262);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2047:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'y') ADVANCE(1383);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2048:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'y') ADVANCE(1999);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2049:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'y') ADVANCE(2000);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2050:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'y') ADVANCE(1584);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2051:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == 'y') ADVANCE(1598);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2052:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2052);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2067);
      END_STATE();
    case 2053:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '=') ADVANCE(1339);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 2054:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 'a') ADVANCE(2056);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 2055:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 'c') ADVANCE(2061);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 2056:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 'f') ADVANCE(2065);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 2057:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 'g') ADVANCE(1385);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 2058:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 'i') ADVANCE(2059);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 2059:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 'n') ADVANCE(2057);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 2060:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 'o') ADVANCE(2063);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 2061:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 'r') ADVANCE(2054);
      if (lookahead == 'u') ADVANCE(2062);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 2062:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 'r') ADVANCE(2064);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 2063:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 'r') ADVANCE(1365);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 2064:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 's') ADVANCE(2060);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 2065:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 't') ADVANCE(2058);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 2066:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1319);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 2067:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2067);
      END_STATE();
    case 2068:
      ACCEPT_TOKEN(anon_sym_DQUOTE);
      END_STATE();
    case 2069:
      ACCEPT_TOKEN(aux_sym_string_token1);
      if (lookahead == '$') ADVANCE(2070);
      if (lookahead == '%') ADVANCE(2071);
      if (lookahead != 0 &&
          lookahead != '"') ADVANCE(2072);
      END_STATE();
    case 2070:
      ACCEPT_TOKEN(aux_sym_string_token1);
      if (lookahead == '(') ADVANCE(1330);
      if (lookahead != 0 &&
          lookahead != '"') ADVANCE(2072);
      END_STATE();
    case 2071:
      ACCEPT_TOKEN(aux_sym_string_token1);
      if (lookahead == '[') ADVANCE(1335);
      if (lookahead != 0 &&
          lookahead != '"') ADVANCE(2072);
      END_STATE();
    case 2072:
      ACCEPT_TOKEN(aux_sym_string_token1);
      if (lookahead != 0 &&
          lookahead != '"') ADVANCE(2072);
      END_STATE();
    case 2073:
      ACCEPT_TOKEN(anon_sym_SQUOTE);
      END_STATE();
    case 2074:
      ACCEPT_TOKEN(aux_sym_string_token2);
      if (lookahead == '$') ADVANCE(2075);
      if (lookahead == '%') ADVANCE(2076);
      if (lookahead != 0 &&
          lookahead != '\'') ADVANCE(2077);
      END_STATE();
    case 2075:
      ACCEPT_TOKEN(aux_sym_string_token2);
      if (lookahead == '(') ADVANCE(1331);
      if (lookahead != 0 &&
          lookahead != '\'' &&
          lookahead != '(') ADVANCE(2077);
      END_STATE();
    case 2076:
      ACCEPT_TOKEN(aux_sym_string_token2);
      if (lookahead == '[') ADVANCE(1336);
      if (lookahead != 0 &&
          lookahead != '\'') ADVANCE(2077);
      END_STATE();
    case 2077:
      ACCEPT_TOKEN(aux_sym_string_token2);
      if (lookahead != 0 &&
          lookahead != '\'') ADVANCE(2077);
      END_STATE();
    case 2078:
      ACCEPT_TOKEN(sym_boolean);
      END_STATE();
    case 2079:
      ACCEPT_TOKEN(sym_boolean);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(510);
      END_STATE();
    case 2080:
      ACCEPT_TOKEN(sym_range);
      if (lookahead == '-') ADVANCE(2081);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(2081);
      END_STATE();
    case 2081:
      ACCEPT_TOKEN(sym_range);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(2081);
      END_STATE();
    case 2082:
      ACCEPT_TOKEN(aux_sym_nbt_identifier_token1);
      if (lookahead == ':') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2082);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2083);
      END_STATE();
    case 2083:
      ACCEPT_TOKEN(aux_sym_nbt_identifier_token1);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2083);
      END_STATE();
    case 2084:
      ACCEPT_TOKEN(anon_sym_LBRACE);
      END_STATE();
    case 2085:
      ACCEPT_TOKEN(anon_sym_COLON);
      END_STATE();
    case 2086:
      ACCEPT_TOKEN(anon_sym_RBRACE);
      END_STATE();
    case 2087:
      ACCEPT_TOKEN(anon_sym_LBRACK);
      END_STATE();
    case 2088:
      ACCEPT_TOKEN(anon_sym_RBRACK);
      END_STATE();
    default:
      return false;
  }
}

static const TSLexerMode ts_lex_modes[STATE_COUNT] = {
  [0] = {.lex_state = 0},
  [1] = {.lex_state = 898},
  [2] = {.lex_state = 9},
  [3] = {.lex_state = 9},
  [4] = {.lex_state = 13},
  [5] = {.lex_state = 898},
  [6] = {.lex_state = 898},
  [7] = {.lex_state = 7},
  [8] = {.lex_state = 898},
  [9] = {.lex_state = 898},
  [10] = {.lex_state = 898},
  [11] = {.lex_state = 898},
  [12] = {.lex_state = 898},
  [13] = {.lex_state = 898},
  [14] = {.lex_state = 898},
  [15] = {.lex_state = 8},
  [16] = {.lex_state = 8},
  [17] = {.lex_state = 17},
  [18] = {.lex_state = 8},
  [19] = {.lex_state = 17},
  [20] = {.lex_state = 10},
  [21] = {.lex_state = 10},
  [22] = {.lex_state = 10},
  [23] = {.lex_state = 10},
  [24] = {.lex_state = 10},
  [25] = {.lex_state = 10},
  [26] = {.lex_state = 10},
  [27] = {.lex_state = 13},
  [28] = {.lex_state = 13},
  [29] = {.lex_state = 13},
  [30] = {.lex_state = 13},
  [31] = {.lex_state = 13},
  [32] = {.lex_state = 13},
  [33] = {.lex_state = 13},
  [34] = {.lex_state = 13},
  [35] = {.lex_state = 13},
  [36] = {.lex_state = 13},
  [37] = {.lex_state = 13},
  [38] = {.lex_state = 11},
  [39] = {.lex_state = 11},
  [40] = {.lex_state = 11},
  [41] = {.lex_state = 11},
  [42] = {.lex_state = 11},
  [43] = {.lex_state = 11},
  [44] = {.lex_state = 11},
  [45] = {.lex_state = 11},
  [46] = {.lex_state = 11},
  [47] = {.lex_state = 11},
  [48] = {.lex_state = 11},
  [49] = {.lex_state = 11},
  [50] = {.lex_state = 11},
  [51] = {.lex_state = 14},
  [52] = {.lex_state = 14},
  [53] = {.lex_state = 11},
  [54] = {.lex_state = 13},
  [55] = {.lex_state = 13},
  [56] = {.lex_state = 13},
  [57] = {.lex_state = 13},
  [58] = {.lex_state = 13},
  [59] = {.lex_state = 13},
  [60] = {.lex_state = 13},
  [61] = {.lex_state = 13},
  [62] = {.lex_state = 13},
  [63] = {.lex_state = 12},
  [64] = {.lex_state = 15},
  [65] = {.lex_state = 15},
  [66] = {.lex_state = 15},
  [67] = {.lex_state = 1},
  [68] = {.lex_state = 16},
  [69] = {.lex_state = 15},
  [70] = {.lex_state = 15},
  [71] = {.lex_state = 15},
  [72] = {.lex_state = 15},
  [73] = {.lex_state = 15},
  [74] = {.lex_state = 15},
  [75] = {.lex_state = 15},
  [76] = {.lex_state = 15},
  [77] = {.lex_state = 16},
  [78] = {.lex_state = 20},
  [79] = {.lex_state = 20},
  [80] = {.lex_state = 20},
  [81] = {.lex_state = 20},
  [82] = {.lex_state = 1},
  [83] = {.lex_state = 18},
  [84] = {.lex_state = 18},
  [85] = {.lex_state = 20},
  [86] = {.lex_state = 20},
  [87] = {.lex_state = 20},
  [88] = {.lex_state = 20},
  [89] = {.lex_state = 20},
  [90] = {.lex_state = 20},
  [91] = {.lex_state = 15},
  [92] = {.lex_state = 15},
  [93] = {.lex_state = 15},
  [94] = {.lex_state = 15},
  [95] = {.lex_state = 15},
  [96] = {.lex_state = 15},
  [97] = {.lex_state = 2},
  [98] = {.lex_state = 15},
  [99] = {.lex_state = 0},
  [100] = {.lex_state = 15},
  [101] = {.lex_state = 16},
  [102] = {.lex_state = 0},
  [103] = {.lex_state = 12},
  [104] = {.lex_state = 19},
  [105] = {.lex_state = 19},
  [106] = {.lex_state = 15},
  [107] = {.lex_state = 15},
  [108] = {.lex_state = 21},
  [109] = {.lex_state = 21},
  [110] = {.lex_state = 15},
  [111] = {.lex_state = 15},
  [112] = {.lex_state = 1},
  [113] = {.lex_state = 15},
  [114] = {.lex_state = 15},
  [115] = {.lex_state = 15},
  [116] = {.lex_state = 2},
  [117] = {.lex_state = 1},
  [118] = {.lex_state = 12},
  [119] = {.lex_state = 0},
  [120] = {.lex_state = 12},
  [121] = {.lex_state = 15},
  [122] = {.lex_state = 16},
  [123] = {.lex_state = 9},
  [124] = {.lex_state = 16},
  [125] = {.lex_state = 9},
  [126] = {.lex_state = 20},
  [127] = {.lex_state = 20},
  [128] = {.lex_state = 16},
  [129] = {.lex_state = 0},
  [130] = {.lex_state = 1},
  [131] = {.lex_state = 20},
  [132] = {.lex_state = 9},
  [133] = {.lex_state = 20},
  [134] = {.lex_state = 0},
  [135] = {.lex_state = 20},
  [136] = {.lex_state = 9},
  [137] = {.lex_state = 1},
  [138] = {.lex_state = 0},
  [139] = {.lex_state = 20},
  [140] = {.lex_state = 20},
  [141] = {.lex_state = 0},
  [142] = {.lex_state = 20},
  [143] = {.lex_state = 20},
  [144] = {.lex_state = 20},
  [145] = {.lex_state = 20},
  [146] = {.lex_state = 1},
  [147] = {.lex_state = 20},
  [148] = {.lex_state = 0},
  [149] = {.lex_state = 0},
  [150] = {.lex_state = 22},
  [151] = {.lex_state = 0},
  [152] = {.lex_state = 16},
  [153] = {.lex_state = 16},
  [154] = {.lex_state = 2},
  [155] = {.lex_state = 0},
  [156] = {.lex_state = 12},
  [157] = {.lex_state = 12},
  [158] = {.lex_state = 2},
  [159] = {.lex_state = 22},
  [160] = {.lex_state = 22},
  [161] = {.lex_state = 16},
  [162] = {.lex_state = 16},
  [163] = {.lex_state = 12},
  [164] = {.lex_state = 16},
  [165] = {.lex_state = 2},
  [166] = {.lex_state = 16},
  [167] = {.lex_state = 16},
  [168] = {.lex_state = 16},
  [169] = {.lex_state = 16},
  [170] = {.lex_state = 0},
  [171] = {.lex_state = 0},
  [172] = {.lex_state = 0},
  [173] = {.lex_state = 0},
  [174] = {.lex_state = 0},
  [175] = {.lex_state = 16},
  [176] = {.lex_state = 16},
  [177] = {.lex_state = 16},
  [178] = {.lex_state = 0},
  [179] = {.lex_state = 16},
  [180] = {.lex_state = 16},
  [181] = {.lex_state = 16},
  [182] = {.lex_state = 0},
  [183] = {.lex_state = 16},
  [184] = {.lex_state = 16},
  [185] = {.lex_state = 16},
  [186] = {.lex_state = 0},
  [187] = {.lex_state = 0},
  [188] = {.lex_state = 16},
  [189] = {.lex_state = 0},
  [190] = {.lex_state = 0},
  [191] = {.lex_state = 16},
  [192] = {.lex_state = 0},
  [193] = {.lex_state = 0},
  [194] = {.lex_state = 16},
  [195] = {.lex_state = 0},
  [196] = {.lex_state = 16},
  [197] = {.lex_state = 0},
  [198] = {.lex_state = 0},
  [199] = {.lex_state = 0},
  [200] = {.lex_state = 0},
  [201] = {.lex_state = 0},
  [202] = {.lex_state = 0},
  [203] = {.lex_state = 16},
  [204] = {.lex_state = 16},
  [205] = {.lex_state = 16},
  [206] = {.lex_state = 16},
  [207] = {.lex_state = 0},
  [208] = {.lex_state = 16},
  [209] = {.lex_state = 0},
  [210] = {.lex_state = 0},
  [211] = {.lex_state = 16},
  [212] = {.lex_state = 16},
  [213] = {.lex_state = 2069},
  [214] = {.lex_state = 0},
  [215] = {.lex_state = 7},
  [216] = {.lex_state = 7},
  [217] = {.lex_state = 7},
  [218] = {.lex_state = 898},
  [219] = {.lex_state = 898},
  [220] = {.lex_state = 7},
  [221] = {.lex_state = 0},
  [222] = {.lex_state = 0},
  [223] = {.lex_state = 0},
  [224] = {.lex_state = 2069},
  [225] = {.lex_state = 2074},
  [226] = {.lex_state = 898},
  [227] = {.lex_state = 0},
  [228] = {.lex_state = 898},
  [229] = {.lex_state = 0},
  [230] = {.lex_state = 0},
  [231] = {.lex_state = 0},
  [232] = {.lex_state = 0},
  [233] = {.lex_state = 898},
  [234] = {.lex_state = 7},
  [235] = {.lex_state = 0},
  [236] = {.lex_state = 0},
  [237] = {.lex_state = 23},
  [238] = {.lex_state = 7},
  [239] = {.lex_state = 0},
  [240] = {.lex_state = 0},
  [241] = {.lex_state = 2069},
  [242] = {.lex_state = 2074},
  [243] = {.lex_state = 2069},
  [244] = {.lex_state = 2074},
  [245] = {.lex_state = 24},
  [246] = {.lex_state = 2074},
  [247] = {.lex_state = 2069},
  [248] = {.lex_state = 2074},
  [249] = {.lex_state = 2069},
  [250] = {.lex_state = 2074},
  [251] = {.lex_state = 0},
  [252] = {(TSStateId)(-1),},
  [253] = {(TSStateId)(-1),},
};

static const uint16_t ts_parse_table[LARGE_STATE_COUNT][SYMBOL_COUNT] = {
  [STATE(0)] = {
    [sym_macro] = STATE(0),
    [sym_substitution] = STATE(0),
    [ts_builtin_sym_end] = ACTIONS(1),
    [sym__newline] = ACTIONS(1),
    [aux_sym__space_token1] = ACTIONS(1),
    [sym_backslash] = ACTIONS(1),
    [sym_comment] = ACTIONS(1),
    [anon_sym_execute] = ACTIONS(1),
    [anon_sym_run] = ACTIONS(1),
    [sym_macro_indicator] = ACTIONS(1),
    [anon_sym_advancement] = ACTIONS(1),
    [anon_sym_attribute] = ACTIONS(1),
    [anon_sym_ban] = ACTIONS(1),
    [anon_sym_ban_DASHip] = ACTIONS(1),
    [anon_sym_banlist] = ACTIONS(1),
    [anon_sym_bossbar] = ACTIONS(1),
    [anon_sym_clear] = ACTIONS(1),
    [anon_sym_clone] = ACTIONS(1),
    [anon_sym_effect] = ACTIONS(1),
    [anon_sym_enchant] = ACTIONS(1),
    [anon_sym_experience] = ACTIONS(1),
    [anon_sym_gamemode] = ACTIONS(1),
    [anon_sym_gamerule] = ACTIONS(1),
    [anon_sym_give] = ACTIONS(1),
    [anon_sym_item] = ACTIONS(1),
    [anon_sym_jfr] = ACTIONS(1),
    [anon_sym_kick] = ACTIONS(1),
    [anon_sym_kill] = ACTIONS(1),
    [anon_sym_list] = ACTIONS(1),
    [anon_sym_locate] = ACTIONS(1),
    [anon_sym_loot] = ACTIONS(1),
    [anon_sym_op] = ACTIONS(1),
    [anon_sym_pardon] = ACTIONS(1),
    [anon_sym_pardon_DASHip] = ACTIONS(1),
    [anon_sym_particle] = ACTIONS(1),
    [anon_sym_perf] = ACTIONS(1),
    [anon_sym_place] = ACTIONS(1),
    [anon_sym_playsound] = ACTIONS(1),
    [anon_sym_publish] = ACTIONS(1),
    [anon_sym_random] = ACTIONS(1),
    [anon_sym_recipe] = ACTIONS(1),
    [anon_sym_reload] = ACTIONS(1),
    [anon_sym_return] = ACTIONS(1),
    [anon_sym_ride] = ACTIONS(1),
    [anon_sym_rotate] = ACTIONS(1),
    [anon_sym_version] = ACTIONS(1),
    [anon_sym_w] = ACTIONS(1),
    [anon_sym_waypoint] = ACTIONS(1),
    [anon_sym_weather] = ACTIONS(1),
    [anon_sym_whitelist] = ACTIONS(1),
    [anon_sym_worldborder] = ACTIONS(1),
    [anon_sym_xp] = ACTIONS(1),
    [anon_sym_align] = ACTIONS(1),
    [anon_sym_all] = ACTIONS(1),
    [anon_sym_anchored] = ACTIONS(1),
    [anon_sym_as] = ACTIONS(1),
    [anon_sym_at] = ACTIONS(1),
    [anon_sym_attacker] = ACTIONS(1),
    [anon_sym_biome] = ACTIONS(1),
    [anon_sym_block] = ACTIONS(1),
    [anon_sym_blocks] = ACTIONS(1),
    [anon_sym_controller] = ACTIONS(1),
    [anon_sym_entity] = ACTIONS(1),
    [anon_sym_eyes] = ACTIONS(1),
    [anon_sym_if] = ACTIONS(1),
    [anon_sym_in] = ACTIONS(1),
    [anon_sym_items] = ACTIONS(1),
    [anon_sym_leasher] = ACTIONS(1),
    [anon_sym_loaded] = ACTIONS(1),
    [anon_sym_on] = ACTIONS(1),
    [anon_sym_origin] = ACTIONS(1),
    [anon_sym_over] = ACTIONS(1),
    [anon_sym_owner] = ACTIONS(1),
    [anon_sym_passengers] = ACTIONS(1),
    [anon_sym_positioned] = ACTIONS(1),
    [anon_sym_predicate] = ACTIONS(1),
    [anon_sym_result] = ACTIONS(1),
    [anon_sym_rotated] = ACTIONS(1),
    [anon_sym_unless] = ACTIONS(1),
    [anon_sym_value] = ACTIONS(1),
    [anon_sym_vehicle] = ACTIONS(1),
    [anon_sym_add] = ACTIONS(1),
    [anon_sym_add_multiplied_base] = ACTIONS(1),
    [anon_sym_add_multiplied_total] = ACTIONS(1),
    [anon_sym_add_value] = ACTIONS(1),
    [anon_sym_amount] = ACTIONS(1),
    [anon_sym_append] = ACTIONS(1),
    [anon_sym_base] = ACTIONS(1),
    [anon_sym_buffer] = ACTIONS(1),
    [anon_sym_by] = ACTIONS(1),
    [anon_sym_center] = ACTIONS(1),
    [anon_sym_color] = ACTIONS(1),
    [anon_sym_create] = ACTIONS(1),
    [anon_sym_enable] = ACTIONS(1),
    [anon_sym_everything] = ACTIONS(1),
    [anon_sym_export] = ACTIONS(1),
    [anon_sym_exportclosest] = ACTIONS(1),
    [anon_sym_exportthat] = ACTIONS(1),
    [anon_sym_exportthese] = ACTIONS(1),
    [anon_sym_gametime] = ACTIONS(1),
    [anon_sym_get] = ACTIONS(1),
    [anon_sym_grant] = ACTIONS(1),
    [anon_sym_id] = ACTIONS(1),
    [anon_sym_insert] = ACTIONS(1),
    [anon_sym_ips] = ACTIONS(1),
    [anon_sym_jigsaw] = ACTIONS(1),
    [anon_sym_join] = ACTIONS(1),
    [anon_sym_keep] = ACTIONS(1),
    [anon_sym_leave] = ACTIONS(1),
    [anon_sym_levels] = ACTIONS(1),
    [anon_sym_name] = ACTIONS(1),
    [anon_sym_night] = ACTIONS(1),
    [anon_sym_noon] = ACTIONS(1),
    [anon_sym_normal] = ACTIONS(1),
    [anon_sym_objectives] = ACTIONS(1),
    [anon_sym_off] = ACTIONS(1),
    [anon_sym_offhand] = ACTIONS(1),
    [anon_sym_only] = ACTIONS(1),
    [anon_sym_outline] = ACTIONS(1),
    [anon_sym_players] = ACTIONS(1),
    [anon_sym_poi] = ACTIONS(1),
    [anon_sym_points] = ACTIONS(1),
    [anon_sym_pos] = ACTIONS(1),
    [anon_sym_prepend] = ACTIONS(1),
    [anon_sym_query] = ACTIONS(1),
    [anon_sym_rain] = ACTIONS(1),
    [anon_sym_remove] = ACTIONS(1),
    [anon_sym_replace] = ACTIONS(1),
    [anon_sym_reset] = ACTIONS(1),
    [anon_sym_resetclosest] = ACTIONS(1),
    [anon_sym_resetthat] = ACTIONS(1),
    [anon_sym_resetthese] = ACTIONS(1),
    [anon_sym_revoke] = ACTIONS(1),
    [anon_sym_roll] = ACTIONS(1),
    [anon_sym_runclosest] = ACTIONS(1),
    [anon_sym_runfailed] = ACTIONS(1),
    [anon_sym_runthat] = ACTIONS(1),
    [anon_sym_runthese] = ACTIONS(1),
    [anon_sym_under] = ACTIONS(1),
    [anon_sym_until] = ACTIONS(1),
    [anon_sym_uuids] = ACTIONS(1),
    [anon_sym_verify] = ACTIONS(1),
    [anon_sym_visible] = ACTIONS(1),
    [anon_sym_warning] = ACTIONS(1),
    [anon_sym_with] = ACTIONS(1),
    [aux_sym_selector_token1] = ACTIONS(1),
    [aux_sym_selector_token2] = ACTIONS(1),
    [sym_selector_identifier] = ACTIONS(1),
    [anon_sym_EQ] = ACTIONS(1),
    [anon_sym_EQ_BANG] = ACTIONS(1),
    [anon_sym_TILDE] = ACTIONS(1),
    [anon_sym_CARET] = ACTIONS(1),
    [sym_resource] = ACTIONS(1),
    [anon_sym_DASH] = ACTIONS(1),
    [aux_sym_scale_token1] = ACTIONS(1),
    [aux_sym_scale_token2] = ACTIONS(1),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [anon_sym_RPAREN] = ACTIONS(1),
    [anon_sym_PERCENT_LBRACK] = ACTIONS(5),
    [anon_sym_PLUS_EQ] = ACTIONS(1),
    [anon_sym_DASH_EQ] = ACTIONS(1),
    [anon_sym_STAR_EQ] = ACTIONS(1),
    [anon_sym_SLASH_EQ] = ACTIONS(1),
    [anon_sym_PERCENT_EQ] = ACTIONS(1),
    [anon_sym_GT_LT] = ACTIONS(1),
    [anon_sym_LT] = ACTIONS(1),
    [anon_sym_GT] = ACTIONS(1),
    [aux_sym_slot_token1] = ACTIONS(1),
    [aux_sym_slot_token3] = ACTIONS(1),
    [aux_sym_slot_token4] = ACTIONS(1),
    [aux_sym_slot_token5] = ACTIONS(1),
    [aux_sym_slot_token7] = ACTIONS(1),
    [anon_sym_contents] = ACTIONS(1),
    [anon_sym_player_DOTcursor] = ACTIONS(1),
    [anon_sym_DQUOTE] = ACTIONS(1),
    [anon_sym_SQUOTE] = ACTIONS(1),
    [sym_boolean] = ACTIONS(1),
    [sym_range] = ACTIONS(1),
    [anon_sym_LBRACE] = ACTIONS(1),
    [anon_sym_COLON] = ACTIONS(1),
    [anon_sym_RBRACE] = ACTIONS(1),
    [anon_sym_LBRACK] = ACTIONS(1),
    [anon_sym_RBRACK] = ACTIONS(1),
  },
  [STATE(1)] = {
    [sym_source_file] = STATE(251),
    [sym__statement] = STATE(9),
    [sym_command] = STATE(234),
    [sym__execute_command] = STATE(217),
    [sym__generic_command] = STATE(217),
    [sym_command_identifier] = STATE(129),
    [sym_macro] = STATE(1),
    [sym_substitution] = STATE(1),
    [aux_sym_source_file_repeat1] = STATE(5),
    [ts_builtin_sym_end] = ACTIONS(7),
    [sym__newline] = ACTIONS(9),
    [aux_sym__statement_token1] = ACTIONS(11),
    [sym_comment] = ACTIONS(13),
    [anon_sym_execute] = ACTIONS(15),
    [sym_macro_indicator] = ACTIONS(17),
    [anon_sym_advancement] = ACTIONS(19),
    [anon_sym_attribute] = ACTIONS(19),
    [anon_sym_ban] = ACTIONS(21),
    [anon_sym_ban_DASHip] = ACTIONS(19),
    [anon_sym_banlist] = ACTIONS(19),
    [anon_sym_bossbar] = ACTIONS(19),
    [anon_sym_clear] = ACTIONS(19),
    [anon_sym_clone] = ACTIONS(19),
    [anon_sym_damage] = ACTIONS(19),
    [anon_sym_data] = ACTIONS(21),
    [anon_sym_datapack] = ACTIONS(19),
    [anon_sym_debug] = ACTIONS(19),
    [anon_sym_defaultgamemode] = ACTIONS(19),
    [anon_sym_deop] = ACTIONS(19),
    [anon_sym_dialog] = ACTIONS(19),
    [anon_sym_difficulty] = ACTIONS(19),
    [anon_sym_effect] = ACTIONS(19),
    [anon_sym_enchant] = ACTIONS(19),
    [anon_sym_experience] = ACTIONS(19),
    [anon_sym_fetchprofile] = ACTIONS(19),
    [anon_sym_fill] = ACTIONS(21),
    [anon_sym_fillbiome] = ACTIONS(19),
    [anon_sym_forceload] = ACTIONS(19),
    [anon_sym_function] = ACTIONS(19),
    [anon_sym_gamemode] = ACTIONS(19),
    [anon_sym_gamerule] = ACTIONS(19),
    [anon_sym_give] = ACTIONS(19),
    [anon_sym_help] = ACTIONS(19),
    [anon_sym_item] = ACTIONS(19),
    [anon_sym_jfr] = ACTIONS(19),
    [anon_sym_kick] = ACTIONS(19),
    [anon_sym_kill] = ACTIONS(19),
    [anon_sym_list] = ACTIONS(19),
    [anon_sym_locate] = ACTIONS(19),
    [anon_sym_loot] = ACTIONS(19),
    [anon_sym_me] = ACTIONS(19),
    [anon_sym_msg] = ACTIONS(19),
    [anon_sym_op] = ACTIONS(19),
    [anon_sym_pardon] = ACTIONS(21),
    [anon_sym_pardon_DASHip] = ACTIONS(19),
    [anon_sym_particle] = ACTIONS(19),
    [anon_sym_perf] = ACTIONS(19),
    [anon_sym_place] = ACTIONS(19),
    [anon_sym_playsound] = ACTIONS(19),
    [anon_sym_publish] = ACTIONS(19),
    [anon_sym_random] = ACTIONS(19),
    [anon_sym_recipe] = ACTIONS(19),
    [anon_sym_reload] = ACTIONS(19),
    [anon_sym_return] = ACTIONS(19),
    [anon_sym_ride] = ACTIONS(19),
    [anon_sym_rotate] = ACTIONS(19),
    [anon_sym_save_DASHall] = ACTIONS(19),
    [anon_sym_save_DASHoff] = ACTIONS(19),
    [anon_sym_save_DASHon] = ACTIONS(19),
    [anon_sym_say] = ACTIONS(19),
    [anon_sym_schedule] = ACTIONS(19),
    [anon_sym_scoreboard] = ACTIONS(19),
    [anon_sym_seed] = ACTIONS(19),
    [anon_sym_setblock] = ACTIONS(19),
    [anon_sym_setidletimeout] = ACTIONS(19),
    [anon_sym_setworldspawn] = ACTIONS(19),
    [anon_sym_spawnpoint] = ACTIONS(19),
    [anon_sym_spectate] = ACTIONS(19),
    [anon_sym_spreadplayers] = ACTIONS(19),
    [anon_sym_stop] = ACTIONS(21),
    [anon_sym_stopsound] = ACTIONS(19),
    [anon_sym_summon] = ACTIONS(19),
    [anon_sym_swing] = ACTIONS(19),
    [anon_sym_tag] = ACTIONS(19),
    [anon_sym_team] = ACTIONS(21),
    [anon_sym_teammsg] = ACTIONS(19),
    [anon_sym_teleport] = ACTIONS(19),
    [anon_sym_tell] = ACTIONS(21),
    [anon_sym_tellraw] = ACTIONS(19),
    [anon_sym_test] = ACTIONS(19),
    [anon_sym_time] = ACTIONS(19),
    [anon_sym_title] = ACTIONS(19),
    [anon_sym_tm] = ACTIONS(19),
    [anon_sym_tp] = ACTIONS(19),
    [anon_sym_transfer] = ACTIONS(19),
    [anon_sym_trigger] = ACTIONS(19),
    [anon_sym_version] = ACTIONS(19),
    [anon_sym_w] = ACTIONS(21),
    [anon_sym_waypoint] = ACTIONS(19),
    [anon_sym_weather] = ACTIONS(19),
    [anon_sym_whitelist] = ACTIONS(19),
    [anon_sym_worldborder] = ACTIONS(19),
    [anon_sym_xp] = ACTIONS(19),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [anon_sym_PERCENT_LBRACK] = ACTIONS(5),
  },
  [STATE(2)] = {
    [sym_argument] = STATE(170),
    [sym_selector] = STATE(193),
    [sym_position] = STATE(193),
    [sym_rotation] = STATE(193),
    [sym_scale] = STATE(193),
    [sym_macro] = STATE(2),
    [sym_substitution] = STATE(2),
    [sym_operator] = STATE(193),
    [sym_slot] = STATE(193),
    [sym_word] = STATE(193),
    [sym_string] = STATE(193),
    [sym_nbt_compound] = STATE(193),
    [sym_nbt_array] = STATE(193),
    [anon_sym_run] = ACTIONS(23),
    [anon_sym_clear] = ACTIONS(23),
    [anon_sym_damage] = ACTIONS(23),
    [anon_sym_function] = ACTIONS(23),
    [anon_sym_give] = ACTIONS(23),
    [anon_sym_list] = ACTIONS(23),
    [anon_sym_locate] = ACTIONS(23),
    [anon_sym_reload] = ACTIONS(23),
    [anon_sym_stop] = ACTIONS(23),
    [anon_sym_time] = ACTIONS(23),
    [anon_sym_at] = ACTIONS(23),
    [anon_sym_biome] = ACTIONS(23),
    [anon_sym_block] = ACTIONS(23),
    [anon_sym_blocks] = ACTIONS(23),
    [anon_sym_entity] = ACTIONS(23),
    [anon_sym_facing] = ACTIONS(23),
    [anon_sym_max] = ACTIONS(23),
    [anon_sym_on] = ACTIONS(23),
    [anon_sym_storage] = ACTIONS(23),
    [anon_sym_value] = ACTIONS(23),
    [anon_sym_add] = ACTIONS(23),
    [anon_sym_add_multiplied_base] = ACTIONS(23),
    [anon_sym_add_multiplied_total] = ACTIONS(23),
    [anon_sym_add_value] = ACTIONS(23),
    [anon_sym_amount] = ACTIONS(23),
    [anon_sym_append] = ACTIONS(23),
    [anon_sym_base] = ACTIONS(23),
    [anon_sym_buffer] = ACTIONS(23),
    [anon_sym_by] = ACTIONS(23),
    [anon_sym_center] = ACTIONS(23),
    [anon_sym_color] = ACTIONS(23),
    [anon_sym_create] = ACTIONS(23),
    [anon_sym_day] = ACTIONS(23),
    [anon_sym_daytime] = ACTIONS(23),
    [anon_sym_destroy] = ACTIONS(23),
    [anon_sym_disable] = ACTIONS(23),
    [anon_sym_dismount] = ACTIONS(23),
    [anon_sym_distance] = ACTIONS(23),
    [anon_sym_enable] = ACTIONS(23),
    [anon_sym_everything] = ACTIONS(23),
    [anon_sym_export] = ACTIONS(23),
    [anon_sym_exportclosest] = ACTIONS(23),
    [anon_sym_exportthat] = ACTIONS(23),
    [anon_sym_exportthese] = ACTIONS(23),
    [anon_sym_fail] = ACTIONS(23),
    [anon_sym_feature] = ACTIONS(23),
    [anon_sym_fish] = ACTIONS(23),
    [anon_sym_flush] = ACTIONS(23),
    [anon_sym_force] = ACTIONS(23),
    [anon_sym_from] = ACTIONS(23),
    [anon_sym_gametime] = ACTIONS(23),
    [anon_sym_get] = ACTIONS(23),
    [anon_sym_grant] = ACTIONS(23),
    [anon_sym_hollow] = ACTIONS(23),
    [anon_sym_id] = ACTIONS(23),
    [anon_sym_insert] = ACTIONS(23),
    [anon_sym_ips] = ACTIONS(23),
    [anon_sym_jigsaw] = ACTIONS(23),
    [anon_sym_join] = ACTIONS(23),
    [anon_sym_keep] = ACTIONS(23),
    [anon_sym_leave] = ACTIONS(23),
    [anon_sym_levels] = ACTIONS(23),
    [anon_sym_mainhand] = ACTIONS(23),
    [anon_sym_merge] = ACTIONS(23),
    [anon_sym_midnight] = ACTIONS(23),
    [anon_sym_mine] = ACTIONS(23),
    [anon_sym_modifier] = ACTIONS(23),
    [anon_sym_modify] = ACTIONS(23),
    [anon_sym_mount] = ACTIONS(23),
    [anon_sym_move] = ACTIONS(23),
    [anon_sym_name] = ACTIONS(23),
    [anon_sym_night] = ACTIONS(23),
    [anon_sym_noon] = ACTIONS(23),
    [anon_sym_normal] = ACTIONS(23),
    [anon_sym_objectives] = ACTIONS(23),
    [anon_sym_off] = ACTIONS(23),
    [anon_sym_offhand] = ACTIONS(23),
    [anon_sym_only] = ACTIONS(23),
    [anon_sym_outline] = ACTIONS(23),
    [anon_sym_players] = ACTIONS(23),
    [anon_sym_poi] = ACTIONS(23),
    [anon_sym_points] = ACTIONS(23),
    [anon_sym_pos] = ACTIONS(23),
    [anon_sym_prepend] = ACTIONS(23),
    [anon_sym_query] = ACTIONS(23),
    [anon_sym_rain] = ACTIONS(23),
    [anon_sym_remove] = ACTIONS(23),
    [anon_sym_replace] = ACTIONS(23),
    [anon_sym_reset] = ACTIONS(23),
    [anon_sym_resetclosest] = ACTIONS(23),
    [anon_sym_resetthat] = ACTIONS(23),
    [anon_sym_resetthese] = ACTIONS(23),
    [anon_sym_revoke] = ACTIONS(23),
    [anon_sym_roll] = ACTIONS(23),
    [anon_sym_runclosest] = ACTIONS(23),
    [anon_sym_runfailed] = ACTIONS(23),
    [anon_sym_runthat] = ACTIONS(23),
    [anon_sym_runthese] = ACTIONS(23),
    [anon_sym_set] = ACTIONS(23),
    [anon_sym_setdisplay] = ACTIONS(23),
    [anon_sym_show] = ACTIONS(23),
    [anon_sym_spawn] = ACTIONS(23),
    [anon_sym_start] = ACTIONS(23),
    [anon_sym_strict] = ACTIONS(23),
    [anon_sym_string] = ACTIONS(23),
    [anon_sym_structure] = ACTIONS(23),
    [anon_sym_style] = ACTIONS(23),
    [anon_sym_take] = ACTIONS(23),
    [anon_sym_template] = ACTIONS(23),
    [anon_sym_through] = ACTIONS(23),
    [anon_sym_thunder] = ACTIONS(23),
    [anon_sym_times] = ACTIONS(23),
    [anon_sym_to] = ACTIONS(23),
    [anon_sym_under] = ACTIONS(23),
    [anon_sym_until] = ACTIONS(23),
    [anon_sym_uuids] = ACTIONS(23),
    [anon_sym_verify] = ACTIONS(23),
    [anon_sym_visible] = ACTIONS(23),
    [anon_sym_warning] = ACTIONS(23),
    [anon_sym_with] = ACTIONS(23),
    [sym_selector_identifier] = ACTIONS(25),
    [anon_sym_TILDE] = ACTIONS(27),
    [anon_sym_CARET] = ACTIONS(29),
    [sym_heightmap] = ACTIONS(23),
    [sym_resource] = ACTIONS(23),
    [anon_sym_DASH] = ACTIONS(31),
    [aux_sym_scale_token1] = ACTIONS(33),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [anon_sym_PERCENT_LBRACK] = ACTIONS(5),
    [sym_type] = ACTIONS(23),
    [anon_sym_PLUS_EQ] = ACTIONS(35),
    [anon_sym_DASH_EQ] = ACTIONS(35),
    [anon_sym_STAR_EQ] = ACTIONS(35),
    [anon_sym_SLASH_EQ] = ACTIONS(35),
    [anon_sym_PERCENT_EQ] = ACTIONS(35),
    [anon_sym_GT_LT] = ACTIONS(35),
    [anon_sym_LT] = ACTIONS(35),
    [anon_sym_GT] = ACTIONS(37),
    [aux_sym_slot_token1] = ACTIONS(39),
    [aux_sym_slot_token2] = ACTIONS(39),
    [aux_sym_slot_token3] = ACTIONS(39),
    [aux_sym_slot_token4] = ACTIONS(39),
    [aux_sym_slot_token5] = ACTIONS(39),
    [aux_sym_slot_token6] = ACTIONS(39),
    [aux_sym_slot_token7] = ACTIONS(39),
    [anon_sym_contents] = ACTIONS(39),
    [anon_sym_player_DOTcursor] = ACTIONS(39),
    [aux_sym_slot_token8] = ACTIONS(39),
    [aux_sym_word_token1] = ACTIONS(41),
    [anon_sym_DQUOTE] = ACTIONS(43),
    [anon_sym_SQUOTE] = ACTIONS(45),
    [sym_boolean] = ACTIONS(23),
    [anon_sym_LBRACE] = ACTIONS(47),
    [anon_sym_LBRACK] = ACTIONS(49),
  },
  [STATE(3)] = {
    [sym_macro] = STATE(3),
    [sym_substitution] = STATE(3),
    [aux_sym__space_token1] = ACTIONS(51),
    [sym_backslash] = ACTIONS(53),
    [anon_sym_run] = ACTIONS(51),
    [anon_sym_clear] = ACTIONS(51),
    [anon_sym_damage] = ACTIONS(51),
    [anon_sym_function] = ACTIONS(51),
    [anon_sym_give] = ACTIONS(51),
    [anon_sym_list] = ACTIONS(51),
    [anon_sym_locate] = ACTIONS(51),
    [anon_sym_reload] = ACTIONS(51),
    [anon_sym_stop] = ACTIONS(51),
    [anon_sym_time] = ACTIONS(51),
    [anon_sym_at] = ACTIONS(51),
    [anon_sym_biome] = ACTIONS(51),
    [anon_sym_block] = ACTIONS(51),
    [anon_sym_blocks] = ACTIONS(51),
    [anon_sym_entity] = ACTIONS(51),
    [anon_sym_facing] = ACTIONS(51),
    [anon_sym_max] = ACTIONS(51),
    [anon_sym_on] = ACTIONS(51),
    [anon_sym_storage] = ACTIONS(51),
    [anon_sym_value] = ACTIONS(51),
    [anon_sym_add] = ACTIONS(51),
    [anon_sym_add_multiplied_base] = ACTIONS(51),
    [anon_sym_add_multiplied_total] = ACTIONS(51),
    [anon_sym_add_value] = ACTIONS(51),
    [anon_sym_amount] = ACTIONS(51),
    [anon_sym_append] = ACTIONS(51),
    [anon_sym_base] = ACTIONS(51),
    [anon_sym_buffer] = ACTIONS(51),
    [anon_sym_by] = ACTIONS(51),
    [anon_sym_center] = ACTIONS(51),
    [anon_sym_color] = ACTIONS(51),
    [anon_sym_create] = ACTIONS(51),
    [anon_sym_day] = ACTIONS(51),
    [anon_sym_daytime] = ACTIONS(51),
    [anon_sym_destroy] = ACTIONS(51),
    [anon_sym_disable] = ACTIONS(51),
    [anon_sym_dismount] = ACTIONS(51),
    [anon_sym_distance] = ACTIONS(51),
    [anon_sym_enable] = ACTIONS(51),
    [anon_sym_everything] = ACTIONS(51),
    [anon_sym_export] = ACTIONS(51),
    [anon_sym_exportclosest] = ACTIONS(51),
    [anon_sym_exportthat] = ACTIONS(51),
    [anon_sym_exportthese] = ACTIONS(51),
    [anon_sym_fail] = ACTIONS(51),
    [anon_sym_feature] = ACTIONS(51),
    [anon_sym_fish] = ACTIONS(51),
    [anon_sym_flush] = ACTIONS(51),
    [anon_sym_force] = ACTIONS(51),
    [anon_sym_from] = ACTIONS(51),
    [anon_sym_gametime] = ACTIONS(51),
    [anon_sym_get] = ACTIONS(51),
    [anon_sym_grant] = ACTIONS(51),
    [anon_sym_hollow] = ACTIONS(51),
    [anon_sym_id] = ACTIONS(51),
    [anon_sym_insert] = ACTIONS(51),
    [anon_sym_ips] = ACTIONS(51),
    [anon_sym_jigsaw] = ACTIONS(51),
    [anon_sym_join] = ACTIONS(51),
    [anon_sym_keep] = ACTIONS(51),
    [anon_sym_leave] = ACTIONS(51),
    [anon_sym_levels] = ACTIONS(51),
    [anon_sym_mainhand] = ACTIONS(51),
    [anon_sym_merge] = ACTIONS(51),
    [anon_sym_midnight] = ACTIONS(51),
    [anon_sym_mine] = ACTIONS(51),
    [anon_sym_modifier] = ACTIONS(51),
    [anon_sym_modify] = ACTIONS(51),
    [anon_sym_mount] = ACTIONS(51),
    [anon_sym_move] = ACTIONS(51),
    [anon_sym_name] = ACTIONS(51),
    [anon_sym_night] = ACTIONS(51),
    [anon_sym_noon] = ACTIONS(51),
    [anon_sym_normal] = ACTIONS(51),
    [anon_sym_objectives] = ACTIONS(51),
    [anon_sym_off] = ACTIONS(51),
    [anon_sym_offhand] = ACTIONS(51),
    [anon_sym_only] = ACTIONS(51),
    [anon_sym_outline] = ACTIONS(51),
    [anon_sym_players] = ACTIONS(51),
    [anon_sym_poi] = ACTIONS(51),
    [anon_sym_points] = ACTIONS(51),
    [anon_sym_pos] = ACTIONS(51),
    [anon_sym_prepend] = ACTIONS(51),
    [anon_sym_query] = ACTIONS(51),
    [anon_sym_rain] = ACTIONS(51),
    [anon_sym_remove] = ACTIONS(51),
    [anon_sym_replace] = ACTIONS(51),
    [anon_sym_reset] = ACTIONS(51),
    [anon_sym_resetclosest] = ACTIONS(51),
    [anon_sym_resetthat] = ACTIONS(51),
    [anon_sym_resetthese] = ACTIONS(51),
    [anon_sym_revoke] = ACTIONS(51),
    [anon_sym_roll] = ACTIONS(51),
    [anon_sym_runclosest] = ACTIONS(51),
    [anon_sym_runfailed] = ACTIONS(51),
    [anon_sym_runthat] = ACTIONS(51),
    [anon_sym_runthese] = ACTIONS(51),
    [anon_sym_set] = ACTIONS(51),
    [anon_sym_setdisplay] = ACTIONS(51),
    [anon_sym_show] = ACTIONS(51),
    [anon_sym_spawn] = ACTIONS(51),
    [anon_sym_start] = ACTIONS(51),
    [anon_sym_strict] = ACTIONS(51),
    [anon_sym_string] = ACTIONS(51),
    [anon_sym_structure] = ACTIONS(51),
    [anon_sym_style] = ACTIONS(51),
    [anon_sym_take] = ACTIONS(51),
    [anon_sym_template] = ACTIONS(51),
    [anon_sym_through] = ACTIONS(51),
    [anon_sym_thunder] = ACTIONS(51),
    [anon_sym_times] = ACTIONS(51),
    [anon_sym_to] = ACTIONS(51),
    [anon_sym_under] = ACTIONS(51),
    [anon_sym_until] = ACTIONS(51),
    [anon_sym_uuids] = ACTIONS(51),
    [anon_sym_verify] = ACTIONS(51),
    [anon_sym_visible] = ACTIONS(51),
    [anon_sym_warning] = ACTIONS(51),
    [anon_sym_with] = ACTIONS(51),
    [sym_selector_identifier] = ACTIONS(53),
    [anon_sym_EQ] = ACTIONS(53),
    [anon_sym_TILDE] = ACTIONS(53),
    [anon_sym_CARET] = ACTIONS(53),
    [sym_heightmap] = ACTIONS(51),
    [sym_resource] = ACTIONS(51),
    [anon_sym_DASH] = ACTIONS(51),
    [aux_sym_scale_token1] = ACTIONS(51),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [anon_sym_PERCENT_LBRACK] = ACTIONS(5),
    [sym_type] = ACTIONS(51),
    [anon_sym_PLUS_EQ] = ACTIONS(53),
    [anon_sym_DASH_EQ] = ACTIONS(53),
    [anon_sym_STAR_EQ] = ACTIONS(53),
    [anon_sym_SLASH_EQ] = ACTIONS(53),
    [anon_sym_PERCENT_EQ] = ACTIONS(53),
    [anon_sym_GT_LT] = ACTIONS(53),
    [anon_sym_LT] = ACTIONS(53),
    [anon_sym_GT] = ACTIONS(51),
    [aux_sym_slot_token1] = ACTIONS(51),
    [aux_sym_slot_token2] = ACTIONS(51),
    [aux_sym_slot_token3] = ACTIONS(51),
    [aux_sym_slot_token4] = ACTIONS(51),
    [aux_sym_slot_token5] = ACTIONS(51),
    [aux_sym_slot_token6] = ACTIONS(51),
    [aux_sym_slot_token7] = ACTIONS(51),
    [anon_sym_contents] = ACTIONS(51),
    [anon_sym_player_DOTcursor] = ACTIONS(51),
    [aux_sym_slot_token8] = ACTIONS(51),
    [aux_sym_word_token1] = ACTIONS(51),
    [anon_sym_DQUOTE] = ACTIONS(53),
    [anon_sym_SQUOTE] = ACTIONS(53),
    [sym_boolean] = ACTIONS(51),
    [anon_sym_LBRACE] = ACTIONS(53),
    [anon_sym_LBRACK] = ACTIONS(53),
  },
  [STATE(4)] = {
    [sym_macro] = STATE(4),
    [sym_substitution] = STATE(4),
    [aux_sym__space_token1] = ACTIONS(51),
    [sym_backslash] = ACTIONS(53),
    [anon_sym_execute] = ACTIONS(53),
    [sym_macro_indicator] = ACTIONS(51),
    [anon_sym_advancement] = ACTIONS(53),
    [anon_sym_attribute] = ACTIONS(53),
    [anon_sym_ban] = ACTIONS(51),
    [anon_sym_ban_DASHip] = ACTIONS(53),
    [anon_sym_banlist] = ACTIONS(53),
    [anon_sym_bossbar] = ACTIONS(53),
    [anon_sym_clear] = ACTIONS(53),
    [anon_sym_clone] = ACTIONS(53),
    [anon_sym_damage] = ACTIONS(53),
    [anon_sym_data] = ACTIONS(51),
    [anon_sym_datapack] = ACTIONS(53),
    [anon_sym_debug] = ACTIONS(53),
    [anon_sym_defaultgamemode] = ACTIONS(53),
    [anon_sym_deop] = ACTIONS(53),
    [anon_sym_dialog] = ACTIONS(53),
    [anon_sym_difficulty] = ACTIONS(53),
    [anon_sym_effect] = ACTIONS(53),
    [anon_sym_enchant] = ACTIONS(53),
    [anon_sym_experience] = ACTIONS(53),
    [anon_sym_fetchprofile] = ACTIONS(53),
    [anon_sym_fill] = ACTIONS(51),
    [anon_sym_fillbiome] = ACTIONS(53),
    [anon_sym_forceload] = ACTIONS(53),
    [anon_sym_function] = ACTIONS(53),
    [anon_sym_gamemode] = ACTIONS(53),
    [anon_sym_gamerule] = ACTIONS(53),
    [anon_sym_give] = ACTIONS(53),
    [anon_sym_help] = ACTIONS(53),
    [anon_sym_item] = ACTIONS(53),
    [anon_sym_jfr] = ACTIONS(53),
    [anon_sym_kick] = ACTIONS(53),
    [anon_sym_kill] = ACTIONS(53),
    [anon_sym_list] = ACTIONS(53),
    [anon_sym_locate] = ACTIONS(53),
    [anon_sym_loot] = ACTIONS(53),
    [anon_sym_me] = ACTIONS(53),
    [anon_sym_msg] = ACTIONS(53),
    [anon_sym_op] = ACTIONS(53),
    [anon_sym_pardon] = ACTIONS(51),
    [anon_sym_pardon_DASHip] = ACTIONS(53),
    [anon_sym_particle] = ACTIONS(53),
    [anon_sym_perf] = ACTIONS(53),
    [anon_sym_place] = ACTIONS(53),
    [anon_sym_playsound] = ACTIONS(53),
    [anon_sym_publish] = ACTIONS(53),
    [anon_sym_random] = ACTIONS(53),
    [anon_sym_recipe] = ACTIONS(53),
    [anon_sym_reload] = ACTIONS(53),
    [anon_sym_return] = ACTIONS(53),
    [anon_sym_ride] = ACTIONS(53),
    [anon_sym_rotate] = ACTIONS(53),
    [anon_sym_save_DASHall] = ACTIONS(53),
    [anon_sym_save_DASHoff] = ACTIONS(53),
    [anon_sym_save_DASHon] = ACTIONS(53),
    [anon_sym_say] = ACTIONS(53),
    [anon_sym_schedule] = ACTIONS(53),
    [anon_sym_scoreboard] = ACTIONS(53),
    [anon_sym_seed] = ACTIONS(53),
    [anon_sym_setblock] = ACTIONS(53),
    [anon_sym_setidletimeout] = ACTIONS(53),
    [anon_sym_setworldspawn] = ACTIONS(53),
    [anon_sym_spawnpoint] = ACTIONS(53),
    [anon_sym_spectate] = ACTIONS(53),
    [anon_sym_spreadplayers] = ACTIONS(53),
    [anon_sym_stop] = ACTIONS(51),
    [anon_sym_stopsound] = ACTIONS(53),
    [anon_sym_summon] = ACTIONS(53),
    [anon_sym_swing] = ACTIONS(53),
    [anon_sym_tag] = ACTIONS(53),
    [anon_sym_team] = ACTIONS(51),
    [anon_sym_teammsg] = ACTIONS(53),
    [anon_sym_teleport] = ACTIONS(53),
    [anon_sym_tell] = ACTIONS(51),
    [anon_sym_tellraw] = ACTIONS(53),
    [anon_sym_test] = ACTIONS(53),
    [anon_sym_time] = ACTIONS(53),
    [anon_sym_title] = ACTIONS(53),
    [anon_sym_tm] = ACTIONS(53),
    [anon_sym_tp] = ACTIONS(53),
    [anon_sym_transfer] = ACTIONS(53),
    [anon_sym_trigger] = ACTIONS(53),
    [anon_sym_version] = ACTIONS(53),
    [anon_sym_w] = ACTIONS(51),
    [anon_sym_waypoint] = ACTIONS(53),
    [anon_sym_weather] = ACTIONS(53),
    [anon_sym_whitelist] = ACTIONS(53),
    [anon_sym_worldborder] = ACTIONS(53),
    [anon_sym_xp] = ACTIONS(53),
    [aux_sym_selector_query_token1] = ACTIONS(53),
    [anon_sym_DASH] = ACTIONS(51),
    [aux_sym_scale_token1] = ACTIONS(51),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [anon_sym_PERCENT_LBRACK] = ACTIONS(5),
    [anon_sym_DQUOTE] = ACTIONS(53),
    [anon_sym_SQUOTE] = ACTIONS(53),
    [sym_boolean] = ACTIONS(53),
    [sym_range] = ACTIONS(53),
    [anon_sym_LBRACE] = ACTIONS(53),
    [anon_sym_LBRACK] = ACTIONS(53),
    [anon_sym_RBRACK] = ACTIONS(53),
  },
  [STATE(5)] = {
    [sym__statement] = STATE(9),
    [sym_command] = STATE(234),
    [sym__execute_command] = STATE(217),
    [sym__generic_command] = STATE(217),
    [sym_command_identifier] = STATE(129),
    [sym_macro] = STATE(5),
    [sym_substitution] = STATE(5),
    [aux_sym_source_file_repeat1] = STATE(6),
    [ts_builtin_sym_end] = ACTIONS(55),
    [sym__newline] = ACTIONS(9),
    [aux_sym__statement_token1] = ACTIONS(11),
    [sym_comment] = ACTIONS(13),
    [anon_sym_execute] = ACTIONS(15),
    [sym_macro_indicator] = ACTIONS(17),
    [anon_sym_advancement] = ACTIONS(19),
    [anon_sym_attribute] = ACTIONS(19),
    [anon_sym_ban] = ACTIONS(21),
    [anon_sym_ban_DASHip] = ACTIONS(19),
    [anon_sym_banlist] = ACTIONS(19),
    [anon_sym_bossbar] = ACTIONS(19),
    [anon_sym_clear] = ACTIONS(19),
    [anon_sym_clone] = ACTIONS(19),
    [anon_sym_damage] = ACTIONS(19),
    [anon_sym_data] = ACTIONS(21),
    [anon_sym_datapack] = ACTIONS(19),
    [anon_sym_debug] = ACTIONS(19),
    [anon_sym_defaultgamemode] = ACTIONS(19),
    [anon_sym_deop] = ACTIONS(19),
    [anon_sym_dialog] = ACTIONS(19),
    [anon_sym_difficulty] = ACTIONS(19),
    [anon_sym_effect] = ACTIONS(19),
    [anon_sym_enchant] = ACTIONS(19),
    [anon_sym_experience] = ACTIONS(19),
    [anon_sym_fetchprofile] = ACTIONS(19),
    [anon_sym_fill] = ACTIONS(21),
    [anon_sym_fillbiome] = ACTIONS(19),
    [anon_sym_forceload] = ACTIONS(19),
    [anon_sym_function] = ACTIONS(19),
    [anon_sym_gamemode] = ACTIONS(19),
    [anon_sym_gamerule] = ACTIONS(19),
    [anon_sym_give] = ACTIONS(19),
    [anon_sym_help] = ACTIONS(19),
    [anon_sym_item] = ACTIONS(19),
    [anon_sym_jfr] = ACTIONS(19),
    [anon_sym_kick] = ACTIONS(19),
    [anon_sym_kill] = ACTIONS(19),
    [anon_sym_list] = ACTIONS(19),
    [anon_sym_locate] = ACTIONS(19),
    [anon_sym_loot] = ACTIONS(19),
    [anon_sym_me] = ACTIONS(19),
    [anon_sym_msg] = ACTIONS(19),
    [anon_sym_op] = ACTIONS(19),
    [anon_sym_pardon] = ACTIONS(21),
    [anon_sym_pardon_DASHip] = ACTIONS(19),
    [anon_sym_particle] = ACTIONS(19),
    [anon_sym_perf] = ACTIONS(19),
    [anon_sym_place] = ACTIONS(19),
    [anon_sym_playsound] = ACTIONS(19),
    [anon_sym_publish] = ACTIONS(19),
    [anon_sym_random] = ACTIONS(19),
    [anon_sym_recipe] = ACTIONS(19),
    [anon_sym_reload] = ACTIONS(19),
    [anon_sym_return] = ACTIONS(19),
    [anon_sym_ride] = ACTIONS(19),
    [anon_sym_rotate] = ACTIONS(19),
    [anon_sym_save_DASHall] = ACTIONS(19),
    [anon_sym_save_DASHoff] = ACTIONS(19),
    [anon_sym_save_DASHon] = ACTIONS(19),
    [anon_sym_say] = ACTIONS(19),
    [anon_sym_schedule] = ACTIONS(19),
    [anon_sym_scoreboard] = ACTIONS(19),
    [anon_sym_seed] = ACTIONS(19),
    [anon_sym_setblock] = ACTIONS(19),
    [anon_sym_setidletimeout] = ACTIONS(19),
    [anon_sym_setworldspawn] = ACTIONS(19),
    [anon_sym_spawnpoint] = ACTIONS(19),
    [anon_sym_spectate] = ACTIONS(19),
    [anon_sym_spreadplayers] = ACTIONS(19),
    [anon_sym_stop] = ACTIONS(21),
    [anon_sym_stopsound] = ACTIONS(19),
    [anon_sym_summon] = ACTIONS(19),
    [anon_sym_swing] = ACTIONS(19),
    [anon_sym_tag] = ACTIONS(19),
    [anon_sym_team] = ACTIONS(21),
    [anon_sym_teammsg] = ACTIONS(19),
    [anon_sym_teleport] = ACTIONS(19),
    [anon_sym_tell] = ACTIONS(21),
    [anon_sym_tellraw] = ACTIONS(19),
    [anon_sym_test] = ACTIONS(19),
    [anon_sym_time] = ACTIONS(19),
    [anon_sym_title] = ACTIONS(19),
    [anon_sym_tm] = ACTIONS(19),
    [anon_sym_tp] = ACTIONS(19),
    [anon_sym_transfer] = ACTIONS(19),
    [anon_sym_trigger] = ACTIONS(19),
    [anon_sym_version] = ACTIONS(19),
    [anon_sym_w] = ACTIONS(21),
    [anon_sym_waypoint] = ACTIONS(19),
    [anon_sym_weather] = ACTIONS(19),
    [anon_sym_whitelist] = ACTIONS(19),
    [anon_sym_worldborder] = ACTIONS(19),
    [anon_sym_xp] = ACTIONS(19),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [anon_sym_PERCENT_LBRACK] = ACTIONS(5),
  },
  [STATE(6)] = {
    [sym__statement] = STATE(9),
    [sym_command] = STATE(234),
    [sym__execute_command] = STATE(217),
    [sym__generic_command] = STATE(217),
    [sym_command_identifier] = STATE(129),
    [sym_macro] = STATE(6),
    [sym_substitution] = STATE(6),
    [aux_sym_source_file_repeat1] = STATE(6),
    [ts_builtin_sym_end] = ACTIONS(57),
    [sym__newline] = ACTIONS(59),
    [aux_sym__statement_token1] = ACTIONS(62),
    [sym_comment] = ACTIONS(65),
    [anon_sym_execute] = ACTIONS(68),
    [sym_macro_indicator] = ACTIONS(71),
    [anon_sym_advancement] = ACTIONS(74),
    [anon_sym_attribute] = ACTIONS(74),
    [anon_sym_ban] = ACTIONS(77),
    [anon_sym_ban_DASHip] = ACTIONS(74),
    [anon_sym_banlist] = ACTIONS(74),
    [anon_sym_bossbar] = ACTIONS(74),
    [anon_sym_clear] = ACTIONS(74),
    [anon_sym_clone] = ACTIONS(74),
    [anon_sym_damage] = ACTIONS(74),
    [anon_sym_data] = ACTIONS(77),
    [anon_sym_datapack] = ACTIONS(74),
    [anon_sym_debug] = ACTIONS(74),
    [anon_sym_defaultgamemode] = ACTIONS(74),
    [anon_sym_deop] = ACTIONS(74),
    [anon_sym_dialog] = ACTIONS(74),
    [anon_sym_difficulty] = ACTIONS(74),
    [anon_sym_effect] = ACTIONS(74),
    [anon_sym_enchant] = ACTIONS(74),
    [anon_sym_experience] = ACTIONS(74),
    [anon_sym_fetchprofile] = ACTIONS(74),
    [anon_sym_fill] = ACTIONS(77),
    [anon_sym_fillbiome] = ACTIONS(74),
    [anon_sym_forceload] = ACTIONS(74),
    [anon_sym_function] = ACTIONS(74),
    [anon_sym_gamemode] = ACTIONS(74),
    [anon_sym_gamerule] = ACTIONS(74),
    [anon_sym_give] = ACTIONS(74),
    [anon_sym_help] = ACTIONS(74),
    [anon_sym_item] = ACTIONS(74),
    [anon_sym_jfr] = ACTIONS(74),
    [anon_sym_kick] = ACTIONS(74),
    [anon_sym_kill] = ACTIONS(74),
    [anon_sym_list] = ACTIONS(74),
    [anon_sym_locate] = ACTIONS(74),
    [anon_sym_loot] = ACTIONS(74),
    [anon_sym_me] = ACTIONS(74),
    [anon_sym_msg] = ACTIONS(74),
    [anon_sym_op] = ACTIONS(74),
    [anon_sym_pardon] = ACTIONS(77),
    [anon_sym_pardon_DASHip] = ACTIONS(74),
    [anon_sym_particle] = ACTIONS(74),
    [anon_sym_perf] = ACTIONS(74),
    [anon_sym_place] = ACTIONS(74),
    [anon_sym_playsound] = ACTIONS(74),
    [anon_sym_publish] = ACTIONS(74),
    [anon_sym_random] = ACTIONS(74),
    [anon_sym_recipe] = ACTIONS(74),
    [anon_sym_reload] = ACTIONS(74),
    [anon_sym_return] = ACTIONS(74),
    [anon_sym_ride] = ACTIONS(74),
    [anon_sym_rotate] = ACTIONS(74),
    [anon_sym_save_DASHall] = ACTIONS(74),
    [anon_sym_save_DASHoff] = ACTIONS(74),
    [anon_sym_save_DASHon] = ACTIONS(74),
    [anon_sym_say] = ACTIONS(74),
    [anon_sym_schedule] = ACTIONS(74),
    [anon_sym_scoreboard] = ACTIONS(74),
    [anon_sym_seed] = ACTIONS(74),
    [anon_sym_setblock] = ACTIONS(74),
    [anon_sym_setidletimeout] = ACTIONS(74),
    [anon_sym_setworldspawn] = ACTIONS(74),
    [anon_sym_spawnpoint] = ACTIONS(74),
    [anon_sym_spectate] = ACTIONS(74),
    [anon_sym_spreadplayers] = ACTIONS(74),
    [anon_sym_stop] = ACTIONS(77),
    [anon_sym_stopsound] = ACTIONS(74),
    [anon_sym_summon] = ACTIONS(74),
    [anon_sym_swing] = ACTIONS(74),
    [anon_sym_tag] = ACTIONS(74),
    [anon_sym_team] = ACTIONS(77),
    [anon_sym_teammsg] = ACTIONS(74),
    [anon_sym_teleport] = ACTIONS(74),
    [anon_sym_tell] = ACTIONS(77),
    [anon_sym_tellraw] = ACTIONS(74),
    [anon_sym_test] = ACTIONS(74),
    [anon_sym_time] = ACTIONS(74),
    [anon_sym_title] = ACTIONS(74),
    [anon_sym_tm] = ACTIONS(74),
    [anon_sym_tp] = ACTIONS(74),
    [anon_sym_transfer] = ACTIONS(74),
    [anon_sym_trigger] = ACTIONS(74),
    [anon_sym_version] = ACTIONS(74),
    [anon_sym_w] = ACTIONS(77),
    [anon_sym_waypoint] = ACTIONS(74),
    [anon_sym_weather] = ACTIONS(74),
    [anon_sym_whitelist] = ACTIONS(74),
    [anon_sym_worldborder] = ACTIONS(74),
    [anon_sym_xp] = ACTIONS(74),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [anon_sym_PERCENT_LBRACK] = ACTIONS(5),
  },
  [STATE(7)] = {
    [sym_command] = STATE(220),
    [sym__execute_command] = STATE(217),
    [sym__generic_command] = STATE(217),
    [sym_command_identifier] = STATE(129),
    [sym_macro] = STATE(7),
    [sym_substitution] = STATE(7),
    [sym__newline] = ACTIONS(80),
    [sym_comment] = ACTIONS(82),
    [anon_sym_execute] = ACTIONS(15),
    [sym_macro_indicator] = ACTIONS(17),
    [anon_sym_advancement] = ACTIONS(19),
    [anon_sym_attribute] = ACTIONS(19),
    [anon_sym_ban] = ACTIONS(21),
    [anon_sym_ban_DASHip] = ACTIONS(19),
    [anon_sym_banlist] = ACTIONS(19),
    [anon_sym_bossbar] = ACTIONS(19),
    [anon_sym_clear] = ACTIONS(19),
    [anon_sym_clone] = ACTIONS(19),
    [anon_sym_damage] = ACTIONS(19),
    [anon_sym_data] = ACTIONS(21),
    [anon_sym_datapack] = ACTIONS(19),
    [anon_sym_debug] = ACTIONS(19),
    [anon_sym_defaultgamemode] = ACTIONS(19),
    [anon_sym_deop] = ACTIONS(19),
    [anon_sym_dialog] = ACTIONS(19),
    [anon_sym_difficulty] = ACTIONS(19),
    [anon_sym_effect] = ACTIONS(19),
    [anon_sym_enchant] = ACTIONS(19),
    [anon_sym_experience] = ACTIONS(19),
    [anon_sym_fetchprofile] = ACTIONS(19),
    [anon_sym_fill] = ACTIONS(21),
    [anon_sym_fillbiome] = ACTIONS(19),
    [anon_sym_forceload] = ACTIONS(19),
    [anon_sym_function] = ACTIONS(19),
    [anon_sym_gamemode] = ACTIONS(19),
    [anon_sym_gamerule] = ACTIONS(19),
    [anon_sym_give] = ACTIONS(19),
    [anon_sym_help] = ACTIONS(19),
    [anon_sym_item] = ACTIONS(19),
    [anon_sym_jfr] = ACTIONS(19),
    [anon_sym_kick] = ACTIONS(19),
    [anon_sym_kill] = ACTIONS(19),
    [anon_sym_list] = ACTIONS(19),
    [anon_sym_locate] = ACTIONS(19),
    [anon_sym_loot] = ACTIONS(19),
    [anon_sym_me] = ACTIONS(19),
    [anon_sym_msg] = ACTIONS(19),
    [anon_sym_op] = ACTIONS(19),
    [anon_sym_pardon] = ACTIONS(21),
    [anon_sym_pardon_DASHip] = ACTIONS(19),
    [anon_sym_particle] = ACTIONS(19),
    [anon_sym_perf] = ACTIONS(19),
    [anon_sym_place] = ACTIONS(19),
    [anon_sym_playsound] = ACTIONS(19),
    [anon_sym_publish] = ACTIONS(19),
    [anon_sym_random] = ACTIONS(19),
    [anon_sym_recipe] = ACTIONS(19),
    [anon_sym_reload] = ACTIONS(19),
    [anon_sym_return] = ACTIONS(19),
    [anon_sym_ride] = ACTIONS(19),
    [anon_sym_rotate] = ACTIONS(19),
    [anon_sym_save_DASHall] = ACTIONS(19),
    [anon_sym_save_DASHoff] = ACTIONS(19),
    [anon_sym_save_DASHon] = ACTIONS(19),
    [anon_sym_say] = ACTIONS(19),
    [anon_sym_schedule] = ACTIONS(19),
    [anon_sym_scoreboard] = ACTIONS(19),
    [anon_sym_seed] = ACTIONS(19),
    [anon_sym_setblock] = ACTIONS(19),
    [anon_sym_setidletimeout] = ACTIONS(19),
    [anon_sym_setworldspawn] = ACTIONS(19),
    [anon_sym_spawnpoint] = ACTIONS(19),
    [anon_sym_spectate] = ACTIONS(19),
    [anon_sym_spreadplayers] = ACTIONS(19),
    [anon_sym_stop] = ACTIONS(21),
    [anon_sym_stopsound] = ACTIONS(19),
    [anon_sym_summon] = ACTIONS(19),
    [anon_sym_swing] = ACTIONS(19),
    [anon_sym_tag] = ACTIONS(19),
    [anon_sym_team] = ACTIONS(21),
    [anon_sym_teammsg] = ACTIONS(19),
    [anon_sym_teleport] = ACTIONS(19),
    [anon_sym_tell] = ACTIONS(21),
    [anon_sym_tellraw] = ACTIONS(19),
    [anon_sym_test] = ACTIONS(19),
    [anon_sym_time] = ACTIONS(19),
    [anon_sym_title] = ACTIONS(19),
    [anon_sym_tm] = ACTIONS(19),
    [anon_sym_tp] = ACTIONS(19),
    [anon_sym_transfer] = ACTIONS(19),
    [anon_sym_trigger] = ACTIONS(19),
    [anon_sym_version] = ACTIONS(19),
    [anon_sym_w] = ACTIONS(21),
    [anon_sym_waypoint] = ACTIONS(19),
    [anon_sym_weather] = ACTIONS(19),
    [anon_sym_whitelist] = ACTIONS(19),
    [anon_sym_worldborder] = ACTIONS(19),
    [anon_sym_xp] = ACTIONS(19),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [anon_sym_PERCENT_LBRACK] = ACTIONS(5),
  },
  [STATE(8)] = {
    [sym_macro] = STATE(8),
    [sym_substitution] = STATE(8),
    [ts_builtin_sym_end] = ACTIONS(84),
    [sym__newline] = ACTIONS(86),
    [aux_sym__statement_token1] = ACTIONS(86),
    [sym_comment] = ACTIONS(84),
    [anon_sym_execute] = ACTIONS(84),
    [sym_macro_indicator] = ACTIONS(86),
    [anon_sym_advancement] = ACTIONS(84),
    [anon_sym_attribute] = ACTIONS(84),
    [anon_sym_ban] = ACTIONS(86),
    [anon_sym_ban_DASHip] = ACTIONS(84),
    [anon_sym_banlist] = ACTIONS(84),
    [anon_sym_bossbar] = ACTIONS(84),
    [anon_sym_clear] = ACTIONS(84),
    [anon_sym_clone] = ACTIONS(84),
    [anon_sym_damage] = ACTIONS(84),
    [anon_sym_data] = ACTIONS(86),
    [anon_sym_datapack] = ACTIONS(84),
    [anon_sym_debug] = ACTIONS(84),
    [anon_sym_defaultgamemode] = ACTIONS(84),
    [anon_sym_deop] = ACTIONS(84),
    [anon_sym_dialog] = ACTIONS(84),
    [anon_sym_difficulty] = ACTIONS(84),
    [anon_sym_effect] = ACTIONS(84),
    [anon_sym_enchant] = ACTIONS(84),
    [anon_sym_experience] = ACTIONS(84),
    [anon_sym_fetchprofile] = ACTIONS(84),
    [anon_sym_fill] = ACTIONS(86),
    [anon_sym_fillbiome] = ACTIONS(84),
    [anon_sym_forceload] = ACTIONS(84),
    [anon_sym_function] = ACTIONS(84),
    [anon_sym_gamemode] = ACTIONS(84),
    [anon_sym_gamerule] = ACTIONS(84),
    [anon_sym_give] = ACTIONS(84),
    [anon_sym_help] = ACTIONS(84),
    [anon_sym_item] = ACTIONS(84),
    [anon_sym_jfr] = ACTIONS(84),
    [anon_sym_kick] = ACTIONS(84),
    [anon_sym_kill] = ACTIONS(84),
    [anon_sym_list] = ACTIONS(84),
    [anon_sym_locate] = ACTIONS(84),
    [anon_sym_loot] = ACTIONS(84),
    [anon_sym_me] = ACTIONS(84),
    [anon_sym_msg] = ACTIONS(84),
    [anon_sym_op] = ACTIONS(84),
    [anon_sym_pardon] = ACTIONS(86),
    [anon_sym_pardon_DASHip] = ACTIONS(84),
    [anon_sym_particle] = ACTIONS(84),
    [anon_sym_perf] = ACTIONS(84),
    [anon_sym_place] = ACTIONS(84),
    [anon_sym_playsound] = ACTIONS(84),
    [anon_sym_publish] = ACTIONS(84),
    [anon_sym_random] = ACTIONS(84),
    [anon_sym_recipe] = ACTIONS(84),
    [anon_sym_reload] = ACTIONS(84),
    [anon_sym_return] = ACTIONS(84),
    [anon_sym_ride] = ACTIONS(84),
    [anon_sym_rotate] = ACTIONS(84),
    [anon_sym_save_DASHall] = ACTIONS(84),
    [anon_sym_save_DASHoff] = ACTIONS(84),
    [anon_sym_save_DASHon] = ACTIONS(84),
    [anon_sym_say] = ACTIONS(84),
    [anon_sym_schedule] = ACTIONS(84),
    [anon_sym_scoreboard] = ACTIONS(84),
    [anon_sym_seed] = ACTIONS(84),
    [anon_sym_setblock] = ACTIONS(84),
    [anon_sym_setidletimeout] = ACTIONS(84),
    [anon_sym_setworldspawn] = ACTIONS(84),
    [anon_sym_spawnpoint] = ACTIONS(84),
    [anon_sym_spectate] = ACTIONS(84),
    [anon_sym_spreadplayers] = ACTIONS(84),
    [anon_sym_stop] = ACTIONS(86),
    [anon_sym_stopsound] = ACTIONS(84),
    [anon_sym_summon] = ACTIONS(84),
    [anon_sym_swing] = ACTIONS(84),
    [anon_sym_tag] = ACTIONS(84),
    [anon_sym_team] = ACTIONS(86),
    [anon_sym_teammsg] = ACTIONS(84),
    [anon_sym_teleport] = ACTIONS(84),
    [anon_sym_tell] = ACTIONS(86),
    [anon_sym_tellraw] = ACTIONS(84),
    [anon_sym_test] = ACTIONS(84),
    [anon_sym_time] = ACTIONS(84),
    [anon_sym_title] = ACTIONS(84),
    [anon_sym_tm] = ACTIONS(84),
    [anon_sym_tp] = ACTIONS(84),
    [anon_sym_transfer] = ACTIONS(84),
    [anon_sym_trigger] = ACTIONS(84),
    [anon_sym_version] = ACTIONS(84),
    [anon_sym_w] = ACTIONS(86),
    [anon_sym_waypoint] = ACTIONS(84),
    [anon_sym_weather] = ACTIONS(84),
    [anon_sym_whitelist] = ACTIONS(84),
    [anon_sym_worldborder] = ACTIONS(84),
    [anon_sym_xp] = ACTIONS(84),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [anon_sym_PERCENT_LBRACK] = ACTIONS(5),
  },
  [STATE(9)] = {
    [sym_macro] = STATE(9),
    [sym_substitution] = STATE(9),
    [ts_builtin_sym_end] = ACTIONS(88),
    [sym__newline] = ACTIONS(90),
    [aux_sym__statement_token1] = ACTIONS(90),
    [sym_comment] = ACTIONS(88),
    [anon_sym_execute] = ACTIONS(88),
    [sym_macro_indicator] = ACTIONS(90),
    [anon_sym_advancement] = ACTIONS(88),
    [anon_sym_attribute] = ACTIONS(88),
    [anon_sym_ban] = ACTIONS(90),
    [anon_sym_ban_DASHip] = ACTIONS(88),
    [anon_sym_banlist] = ACTIONS(88),
    [anon_sym_bossbar] = ACTIONS(88),
    [anon_sym_clear] = ACTIONS(88),
    [anon_sym_clone] = ACTIONS(88),
    [anon_sym_damage] = ACTIONS(88),
    [anon_sym_data] = ACTIONS(90),
    [anon_sym_datapack] = ACTIONS(88),
    [anon_sym_debug] = ACTIONS(88),
    [anon_sym_defaultgamemode] = ACTIONS(88),
    [anon_sym_deop] = ACTIONS(88),
    [anon_sym_dialog] = ACTIONS(88),
    [anon_sym_difficulty] = ACTIONS(88),
    [anon_sym_effect] = ACTIONS(88),
    [anon_sym_enchant] = ACTIONS(88),
    [anon_sym_experience] = ACTIONS(88),
    [anon_sym_fetchprofile] = ACTIONS(88),
    [anon_sym_fill] = ACTIONS(90),
    [anon_sym_fillbiome] = ACTIONS(88),
    [anon_sym_forceload] = ACTIONS(88),
    [anon_sym_function] = ACTIONS(88),
    [anon_sym_gamemode] = ACTIONS(88),
    [anon_sym_gamerule] = ACTIONS(88),
    [anon_sym_give] = ACTIONS(88),
    [anon_sym_help] = ACTIONS(88),
    [anon_sym_item] = ACTIONS(88),
    [anon_sym_jfr] = ACTIONS(88),
    [anon_sym_kick] = ACTIONS(88),
    [anon_sym_kill] = ACTIONS(88),
    [anon_sym_list] = ACTIONS(88),
    [anon_sym_locate] = ACTIONS(88),
    [anon_sym_loot] = ACTIONS(88),
    [anon_sym_me] = ACTIONS(88),
    [anon_sym_msg] = ACTIONS(88),
    [anon_sym_op] = ACTIONS(88),
    [anon_sym_pardon] = ACTIONS(90),
    [anon_sym_pardon_DASHip] = ACTIONS(88),
    [anon_sym_particle] = ACTIONS(88),
    [anon_sym_perf] = ACTIONS(88),
    [anon_sym_place] = ACTIONS(88),
    [anon_sym_playsound] = ACTIONS(88),
    [anon_sym_publish] = ACTIONS(88),
    [anon_sym_random] = ACTIONS(88),
    [anon_sym_recipe] = ACTIONS(88),
    [anon_sym_reload] = ACTIONS(88),
    [anon_sym_return] = ACTIONS(88),
    [anon_sym_ride] = ACTIONS(88),
    [anon_sym_rotate] = ACTIONS(88),
    [anon_sym_save_DASHall] = ACTIONS(88),
    [anon_sym_save_DASHoff] = ACTIONS(88),
    [anon_sym_save_DASHon] = ACTIONS(88),
    [anon_sym_say] = ACTIONS(88),
    [anon_sym_schedule] = ACTIONS(88),
    [anon_sym_scoreboard] = ACTIONS(88),
    [anon_sym_seed] = ACTIONS(88),
    [anon_sym_setblock] = ACTIONS(88),
    [anon_sym_setidletimeout] = ACTIONS(88),
    [anon_sym_setworldspawn] = ACTIONS(88),
    [anon_sym_spawnpoint] = ACTIONS(88),
    [anon_sym_spectate] = ACTIONS(88),
    [anon_sym_spreadplayers] = ACTIONS(88),
    [anon_sym_stop] = ACTIONS(90),
    [anon_sym_stopsound] = ACTIONS(88),
    [anon_sym_summon] = ACTIONS(88),
    [anon_sym_swing] = ACTIONS(88),
    [anon_sym_tag] = ACTIONS(88),
    [anon_sym_team] = ACTIONS(90),
    [anon_sym_teammsg] = ACTIONS(88),
    [anon_sym_teleport] = ACTIONS(88),
    [anon_sym_tell] = ACTIONS(90),
    [anon_sym_tellraw] = ACTIONS(88),
    [anon_sym_test] = ACTIONS(88),
    [anon_sym_time] = ACTIONS(88),
    [anon_sym_title] = ACTIONS(88),
    [anon_sym_tm] = ACTIONS(88),
    [anon_sym_tp] = ACTIONS(88),
    [anon_sym_transfer] = ACTIONS(88),
    [anon_sym_trigger] = ACTIONS(88),
    [anon_sym_version] = ACTIONS(88),
    [anon_sym_w] = ACTIONS(90),
    [anon_sym_waypoint] = ACTIONS(88),
    [anon_sym_weather] = ACTIONS(88),
    [anon_sym_whitelist] = ACTIONS(88),
    [anon_sym_worldborder] = ACTIONS(88),
    [anon_sym_xp] = ACTIONS(88),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [anon_sym_PERCENT_LBRACK] = ACTIONS(5),
  },
  [STATE(10)] = {
    [sym_macro] = STATE(10),
    [sym_substitution] = STATE(10),
    [ts_builtin_sym_end] = ACTIONS(92),
    [sym__newline] = ACTIONS(94),
    [aux_sym__statement_token1] = ACTIONS(94),
    [sym_comment] = ACTIONS(92),
    [anon_sym_execute] = ACTIONS(92),
    [sym_macro_indicator] = ACTIONS(94),
    [anon_sym_advancement] = ACTIONS(92),
    [anon_sym_attribute] = ACTIONS(92),
    [anon_sym_ban] = ACTIONS(94),
    [anon_sym_ban_DASHip] = ACTIONS(92),
    [anon_sym_banlist] = ACTIONS(92),
    [anon_sym_bossbar] = ACTIONS(92),
    [anon_sym_clear] = ACTIONS(92),
    [anon_sym_clone] = ACTIONS(92),
    [anon_sym_damage] = ACTIONS(92),
    [anon_sym_data] = ACTIONS(94),
    [anon_sym_datapack] = ACTIONS(92),
    [anon_sym_debug] = ACTIONS(92),
    [anon_sym_defaultgamemode] = ACTIONS(92),
    [anon_sym_deop] = ACTIONS(92),
    [anon_sym_dialog] = ACTIONS(92),
    [anon_sym_difficulty] = ACTIONS(92),
    [anon_sym_effect] = ACTIONS(92),
    [anon_sym_enchant] = ACTIONS(92),
    [anon_sym_experience] = ACTIONS(92),
    [anon_sym_fetchprofile] = ACTIONS(92),
    [anon_sym_fill] = ACTIONS(94),
    [anon_sym_fillbiome] = ACTIONS(92),
    [anon_sym_forceload] = ACTIONS(92),
    [anon_sym_function] = ACTIONS(92),
    [anon_sym_gamemode] = ACTIONS(92),
    [anon_sym_gamerule] = ACTIONS(92),
    [anon_sym_give] = ACTIONS(92),
    [anon_sym_help] = ACTIONS(92),
    [anon_sym_item] = ACTIONS(92),
    [anon_sym_jfr] = ACTIONS(92),
    [anon_sym_kick] = ACTIONS(92),
    [anon_sym_kill] = ACTIONS(92),
    [anon_sym_list] = ACTIONS(92),
    [anon_sym_locate] = ACTIONS(92),
    [anon_sym_loot] = ACTIONS(92),
    [anon_sym_me] = ACTIONS(92),
    [anon_sym_msg] = ACTIONS(92),
    [anon_sym_op] = ACTIONS(92),
    [anon_sym_pardon] = ACTIONS(94),
    [anon_sym_pardon_DASHip] = ACTIONS(92),
    [anon_sym_particle] = ACTIONS(92),
    [anon_sym_perf] = ACTIONS(92),
    [anon_sym_place] = ACTIONS(92),
    [anon_sym_playsound] = ACTIONS(92),
    [anon_sym_publish] = ACTIONS(92),
    [anon_sym_random] = ACTIONS(92),
    [anon_sym_recipe] = ACTIONS(92),
    [anon_sym_reload] = ACTIONS(92),
    [anon_sym_return] = ACTIONS(92),
    [anon_sym_ride] = ACTIONS(92),
    [anon_sym_rotate] = ACTIONS(92),
    [anon_sym_save_DASHall] = ACTIONS(92),
    [anon_sym_save_DASHoff] = ACTIONS(92),
    [anon_sym_save_DASHon] = ACTIONS(92),
    [anon_sym_say] = ACTIONS(92),
    [anon_sym_schedule] = ACTIONS(92),
    [anon_sym_scoreboard] = ACTIONS(92),
    [anon_sym_seed] = ACTIONS(92),
    [anon_sym_setblock] = ACTIONS(92),
    [anon_sym_setidletimeout] = ACTIONS(92),
    [anon_sym_setworldspawn] = ACTIONS(92),
    [anon_sym_spawnpoint] = ACTIONS(92),
    [anon_sym_spectate] = ACTIONS(92),
    [anon_sym_spreadplayers] = ACTIONS(92),
    [anon_sym_stop] = ACTIONS(94),
    [anon_sym_stopsound] = ACTIONS(92),
    [anon_sym_summon] = ACTIONS(92),
    [anon_sym_swing] = ACTIONS(92),
    [anon_sym_tag] = ACTIONS(92),
    [anon_sym_team] = ACTIONS(94),
    [anon_sym_teammsg] = ACTIONS(92),
    [anon_sym_teleport] = ACTIONS(92),
    [anon_sym_tell] = ACTIONS(94),
    [anon_sym_tellraw] = ACTIONS(92),
    [anon_sym_test] = ACTIONS(92),
    [anon_sym_time] = ACTIONS(92),
    [anon_sym_title] = ACTIONS(92),
    [anon_sym_tm] = ACTIONS(92),
    [anon_sym_tp] = ACTIONS(92),
    [anon_sym_transfer] = ACTIONS(92),
    [anon_sym_trigger] = ACTIONS(92),
    [anon_sym_version] = ACTIONS(92),
    [anon_sym_w] = ACTIONS(94),
    [anon_sym_waypoint] = ACTIONS(92),
    [anon_sym_weather] = ACTIONS(92),
    [anon_sym_whitelist] = ACTIONS(92),
    [anon_sym_worldborder] = ACTIONS(92),
    [anon_sym_xp] = ACTIONS(92),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [anon_sym_PERCENT_LBRACK] = ACTIONS(5),
  },
  [STATE(11)] = {
    [sym_macro] = STATE(11),
    [sym_substitution] = STATE(11),
    [ts_builtin_sym_end] = ACTIONS(96),
    [sym__newline] = ACTIONS(98),
    [aux_sym__statement_token1] = ACTIONS(98),
    [sym_comment] = ACTIONS(96),
    [anon_sym_execute] = ACTIONS(96),
    [sym_macro_indicator] = ACTIONS(98),
    [anon_sym_advancement] = ACTIONS(96),
    [anon_sym_attribute] = ACTIONS(96),
    [anon_sym_ban] = ACTIONS(98),
    [anon_sym_ban_DASHip] = ACTIONS(96),
    [anon_sym_banlist] = ACTIONS(96),
    [anon_sym_bossbar] = ACTIONS(96),
    [anon_sym_clear] = ACTIONS(96),
    [anon_sym_clone] = ACTIONS(96),
    [anon_sym_damage] = ACTIONS(96),
    [anon_sym_data] = ACTIONS(98),
    [anon_sym_datapack] = ACTIONS(96),
    [anon_sym_debug] = ACTIONS(96),
    [anon_sym_defaultgamemode] = ACTIONS(96),
    [anon_sym_deop] = ACTIONS(96),
    [anon_sym_dialog] = ACTIONS(96),
    [anon_sym_difficulty] = ACTIONS(96),
    [anon_sym_effect] = ACTIONS(96),
    [anon_sym_enchant] = ACTIONS(96),
    [anon_sym_experience] = ACTIONS(96),
    [anon_sym_fetchprofile] = ACTIONS(96),
    [anon_sym_fill] = ACTIONS(98),
    [anon_sym_fillbiome] = ACTIONS(96),
    [anon_sym_forceload] = ACTIONS(96),
    [anon_sym_function] = ACTIONS(96),
    [anon_sym_gamemode] = ACTIONS(96),
    [anon_sym_gamerule] = ACTIONS(96),
    [anon_sym_give] = ACTIONS(96),
    [anon_sym_help] = ACTIONS(96),
    [anon_sym_item] = ACTIONS(96),
    [anon_sym_jfr] = ACTIONS(96),
    [anon_sym_kick] = ACTIONS(96),
    [anon_sym_kill] = ACTIONS(96),
    [anon_sym_list] = ACTIONS(96),
    [anon_sym_locate] = ACTIONS(96),
    [anon_sym_loot] = ACTIONS(96),
    [anon_sym_me] = ACTIONS(96),
    [anon_sym_msg] = ACTIONS(96),
    [anon_sym_op] = ACTIONS(96),
    [anon_sym_pardon] = ACTIONS(98),
    [anon_sym_pardon_DASHip] = ACTIONS(96),
    [anon_sym_particle] = ACTIONS(96),
    [anon_sym_perf] = ACTIONS(96),
    [anon_sym_place] = ACTIONS(96),
    [anon_sym_playsound] = ACTIONS(96),
    [anon_sym_publish] = ACTIONS(96),
    [anon_sym_random] = ACTIONS(96),
    [anon_sym_recipe] = ACTIONS(96),
    [anon_sym_reload] = ACTIONS(96),
    [anon_sym_return] = ACTIONS(96),
    [anon_sym_ride] = ACTIONS(96),
    [anon_sym_rotate] = ACTIONS(96),
    [anon_sym_save_DASHall] = ACTIONS(96),
    [anon_sym_save_DASHoff] = ACTIONS(96),
    [anon_sym_save_DASHon] = ACTIONS(96),
    [anon_sym_say] = ACTIONS(96),
    [anon_sym_schedule] = ACTIONS(96),
    [anon_sym_scoreboard] = ACTIONS(96),
    [anon_sym_seed] = ACTIONS(96),
    [anon_sym_setblock] = ACTIONS(96),
    [anon_sym_setidletimeout] = ACTIONS(96),
    [anon_sym_setworldspawn] = ACTIONS(96),
    [anon_sym_spawnpoint] = ACTIONS(96),
    [anon_sym_spectate] = ACTIONS(96),
    [anon_sym_spreadplayers] = ACTIONS(96),
    [anon_sym_stop] = ACTIONS(98),
    [anon_sym_stopsound] = ACTIONS(96),
    [anon_sym_summon] = ACTIONS(96),
    [anon_sym_swing] = ACTIONS(96),
    [anon_sym_tag] = ACTIONS(96),
    [anon_sym_team] = ACTIONS(98),
    [anon_sym_teammsg] = ACTIONS(96),
    [anon_sym_teleport] = ACTIONS(96),
    [anon_sym_tell] = ACTIONS(98),
    [anon_sym_tellraw] = ACTIONS(96),
    [anon_sym_test] = ACTIONS(96),
    [anon_sym_time] = ACTIONS(96),
    [anon_sym_title] = ACTIONS(96),
    [anon_sym_tm] = ACTIONS(96),
    [anon_sym_tp] = ACTIONS(96),
    [anon_sym_transfer] = ACTIONS(96),
    [anon_sym_trigger] = ACTIONS(96),
    [anon_sym_version] = ACTIONS(96),
    [anon_sym_w] = ACTIONS(98),
    [anon_sym_waypoint] = ACTIONS(96),
    [anon_sym_weather] = ACTIONS(96),
    [anon_sym_whitelist] = ACTIONS(96),
    [anon_sym_worldborder] = ACTIONS(96),
    [anon_sym_xp] = ACTIONS(96),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [anon_sym_PERCENT_LBRACK] = ACTIONS(5),
  },
  [STATE(12)] = {
    [sym_command] = STATE(238),
    [sym__execute_command] = STATE(217),
    [sym__generic_command] = STATE(217),
    [sym_command_identifier] = STATE(129),
    [sym_macro] = STATE(12),
    [sym_substitution] = STATE(12),
    [anon_sym_execute] = ACTIONS(15),
    [sym_macro_indicator] = ACTIONS(17),
    [anon_sym_advancement] = ACTIONS(19),
    [anon_sym_attribute] = ACTIONS(19),
    [anon_sym_ban] = ACTIONS(21),
    [anon_sym_ban_DASHip] = ACTIONS(19),
    [anon_sym_banlist] = ACTIONS(19),
    [anon_sym_bossbar] = ACTIONS(19),
    [anon_sym_clear] = ACTIONS(19),
    [anon_sym_clone] = ACTIONS(19),
    [anon_sym_damage] = ACTIONS(19),
    [anon_sym_data] = ACTIONS(21),
    [anon_sym_datapack] = ACTIONS(19),
    [anon_sym_debug] = ACTIONS(19),
    [anon_sym_defaultgamemode] = ACTIONS(19),
    [anon_sym_deop] = ACTIONS(19),
    [anon_sym_dialog] = ACTIONS(19),
    [anon_sym_difficulty] = ACTIONS(19),
    [anon_sym_effect] = ACTIONS(19),
    [anon_sym_enchant] = ACTIONS(19),
    [anon_sym_experience] = ACTIONS(19),
    [anon_sym_fetchprofile] = ACTIONS(19),
    [anon_sym_fill] = ACTIONS(21),
    [anon_sym_fillbiome] = ACTIONS(19),
    [anon_sym_forceload] = ACTIONS(19),
    [anon_sym_function] = ACTIONS(19),
    [anon_sym_gamemode] = ACTIONS(19),
    [anon_sym_gamerule] = ACTIONS(19),
    [anon_sym_give] = ACTIONS(19),
    [anon_sym_help] = ACTIONS(19),
    [anon_sym_item] = ACTIONS(19),
    [anon_sym_jfr] = ACTIONS(19),
    [anon_sym_kick] = ACTIONS(19),
    [anon_sym_kill] = ACTIONS(19),
    [anon_sym_list] = ACTIONS(19),
    [anon_sym_locate] = ACTIONS(19),
    [anon_sym_loot] = ACTIONS(19),
    [anon_sym_me] = ACTIONS(19),
    [anon_sym_msg] = ACTIONS(19),
    [anon_sym_op] = ACTIONS(19),
    [anon_sym_pardon] = ACTIONS(21),
    [anon_sym_pardon_DASHip] = ACTIONS(19),
    [anon_sym_particle] = ACTIONS(19),
    [anon_sym_perf] = ACTIONS(19),
    [anon_sym_place] = ACTIONS(19),
    [anon_sym_playsound] = ACTIONS(19),
    [anon_sym_publish] = ACTIONS(19),
    [anon_sym_random] = ACTIONS(19),
    [anon_sym_recipe] = ACTIONS(19),
    [anon_sym_reload] = ACTIONS(19),
    [anon_sym_return] = ACTIONS(19),
    [anon_sym_ride] = ACTIONS(19),
    [anon_sym_rotate] = ACTIONS(19),
    [anon_sym_save_DASHall] = ACTIONS(19),
    [anon_sym_save_DASHoff] = ACTIONS(19),
    [anon_sym_save_DASHon] = ACTIONS(19),
    [anon_sym_say] = ACTIONS(19),
    [anon_sym_schedule] = ACTIONS(19),
    [anon_sym_scoreboard] = ACTIONS(19),
    [anon_sym_seed] = ACTIONS(19),
    [anon_sym_setblock] = ACTIONS(19),
    [anon_sym_setidletimeout] = ACTIONS(19),
    [anon_sym_setworldspawn] = ACTIONS(19),
    [anon_sym_spawnpoint] = ACTIONS(19),
    [anon_sym_spectate] = ACTIONS(19),
    [anon_sym_spreadplayers] = ACTIONS(19),
    [anon_sym_stop] = ACTIONS(21),
    [anon_sym_stopsound] = ACTIONS(19),
    [anon_sym_summon] = ACTIONS(19),
    [anon_sym_swing] = ACTIONS(19),
    [anon_sym_tag] = ACTIONS(19),
    [anon_sym_team] = ACTIONS(21),
    [anon_sym_teammsg] = ACTIONS(19),
    [anon_sym_teleport] = ACTIONS(19),
    [anon_sym_tell] = ACTIONS(21),
    [anon_sym_tellraw] = ACTIONS(19),
    [anon_sym_test] = ACTIONS(19),
    [anon_sym_time] = ACTIONS(19),
    [anon_sym_title] = ACTIONS(19),
    [anon_sym_tm] = ACTIONS(19),
    [anon_sym_tp] = ACTIONS(19),
    [anon_sym_transfer] = ACTIONS(19),
    [anon_sym_trigger] = ACTIONS(19),
    [anon_sym_version] = ACTIONS(19),
    [anon_sym_w] = ACTIONS(21),
    [anon_sym_waypoint] = ACTIONS(19),
    [anon_sym_weather] = ACTIONS(19),
    [anon_sym_whitelist] = ACTIONS(19),
    [anon_sym_worldborder] = ACTIONS(19),
    [anon_sym_xp] = ACTIONS(19),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [anon_sym_PERCENT_LBRACK] = ACTIONS(5),
  },
  [STATE(13)] = {
    [sym_command] = STATE(216),
    [sym__execute_command] = STATE(217),
    [sym__generic_command] = STATE(217),
    [sym_command_identifier] = STATE(129),
    [sym_macro] = STATE(13),
    [sym_substitution] = STATE(13),
    [anon_sym_execute] = ACTIONS(15),
    [sym_macro_indicator] = ACTIONS(17),
    [anon_sym_advancement] = ACTIONS(19),
    [anon_sym_attribute] = ACTIONS(19),
    [anon_sym_ban] = ACTIONS(21),
    [anon_sym_ban_DASHip] = ACTIONS(19),
    [anon_sym_banlist] = ACTIONS(19),
    [anon_sym_bossbar] = ACTIONS(19),
    [anon_sym_clear] = ACTIONS(19),
    [anon_sym_clone] = ACTIONS(19),
    [anon_sym_damage] = ACTIONS(19),
    [anon_sym_data] = ACTIONS(21),
    [anon_sym_datapack] = ACTIONS(19),
    [anon_sym_debug] = ACTIONS(19),
    [anon_sym_defaultgamemode] = ACTIONS(19),
    [anon_sym_deop] = ACTIONS(19),
    [anon_sym_dialog] = ACTIONS(19),
    [anon_sym_difficulty] = ACTIONS(19),
    [anon_sym_effect] = ACTIONS(19),
    [anon_sym_enchant] = ACTIONS(19),
    [anon_sym_experience] = ACTIONS(19),
    [anon_sym_fetchprofile] = ACTIONS(19),
    [anon_sym_fill] = ACTIONS(21),
    [anon_sym_fillbiome] = ACTIONS(19),
    [anon_sym_forceload] = ACTIONS(19),
    [anon_sym_function] = ACTIONS(19),
    [anon_sym_gamemode] = ACTIONS(19),
    [anon_sym_gamerule] = ACTIONS(19),
    [anon_sym_give] = ACTIONS(19),
    [anon_sym_help] = ACTIONS(19),
    [anon_sym_item] = ACTIONS(19),
    [anon_sym_jfr] = ACTIONS(19),
    [anon_sym_kick] = ACTIONS(19),
    [anon_sym_kill] = ACTIONS(19),
    [anon_sym_list] = ACTIONS(19),
    [anon_sym_locate] = ACTIONS(19),
    [anon_sym_loot] = ACTIONS(19),
    [anon_sym_me] = ACTIONS(19),
    [anon_sym_msg] = ACTIONS(19),
    [anon_sym_op] = ACTIONS(19),
    [anon_sym_pardon] = ACTIONS(21),
    [anon_sym_pardon_DASHip] = ACTIONS(19),
    [anon_sym_particle] = ACTIONS(19),
    [anon_sym_perf] = ACTIONS(19),
    [anon_sym_place] = ACTIONS(19),
    [anon_sym_playsound] = ACTIONS(19),
    [anon_sym_publish] = ACTIONS(19),
    [anon_sym_random] = ACTIONS(19),
    [anon_sym_recipe] = ACTIONS(19),
    [anon_sym_reload] = ACTIONS(19),
    [anon_sym_return] = ACTIONS(19),
    [anon_sym_ride] = ACTIONS(19),
    [anon_sym_rotate] = ACTIONS(19),
    [anon_sym_save_DASHall] = ACTIONS(19),
    [anon_sym_save_DASHoff] = ACTIONS(19),
    [anon_sym_save_DASHon] = ACTIONS(19),
    [anon_sym_say] = ACTIONS(19),
    [anon_sym_schedule] = ACTIONS(19),
    [anon_sym_scoreboard] = ACTIONS(19),
    [anon_sym_seed] = ACTIONS(19),
    [anon_sym_setblock] = ACTIONS(19),
    [anon_sym_setidletimeout] = ACTIONS(19),
    [anon_sym_setworldspawn] = ACTIONS(19),
    [anon_sym_spawnpoint] = ACTIONS(19),
    [anon_sym_spectate] = ACTIONS(19),
    [anon_sym_spreadplayers] = ACTIONS(19),
    [anon_sym_stop] = ACTIONS(21),
    [anon_sym_stopsound] = ACTIONS(19),
    [anon_sym_summon] = ACTIONS(19),
    [anon_sym_swing] = ACTIONS(19),
    [anon_sym_tag] = ACTIONS(19),
    [anon_sym_team] = ACTIONS(21),
    [anon_sym_teammsg] = ACTIONS(19),
    [anon_sym_teleport] = ACTIONS(19),
    [anon_sym_tell] = ACTIONS(21),
    [anon_sym_tellraw] = ACTIONS(19),
    [anon_sym_test] = ACTIONS(19),
    [anon_sym_time] = ACTIONS(19),
    [anon_sym_title] = ACTIONS(19),
    [anon_sym_tm] = ACTIONS(19),
    [anon_sym_tp] = ACTIONS(19),
    [anon_sym_transfer] = ACTIONS(19),
    [anon_sym_trigger] = ACTIONS(19),
    [anon_sym_version] = ACTIONS(19),
    [anon_sym_w] = ACTIONS(21),
    [anon_sym_waypoint] = ACTIONS(19),
    [anon_sym_weather] = ACTIONS(19),
    [anon_sym_whitelist] = ACTIONS(19),
    [anon_sym_worldborder] = ACTIONS(19),
    [anon_sym_xp] = ACTIONS(19),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [anon_sym_PERCENT_LBRACK] = ACTIONS(5),
  },
  [STATE(14)] = {
    [sym__execute_command] = STATE(215),
    [sym__generic_command] = STATE(215),
    [sym_command_identifier] = STATE(129),
    [sym_macro] = STATE(14),
    [sym_substitution] = STATE(14),
    [anon_sym_execute] = ACTIONS(15),
    [anon_sym_advancement] = ACTIONS(19),
    [anon_sym_attribute] = ACTIONS(19),
    [anon_sym_ban] = ACTIONS(21),
    [anon_sym_ban_DASHip] = ACTIONS(19),
    [anon_sym_banlist] = ACTIONS(19),
    [anon_sym_bossbar] = ACTIONS(19),
    [anon_sym_clear] = ACTIONS(19),
    [anon_sym_clone] = ACTIONS(19),
    [anon_sym_damage] = ACTIONS(19),
    [anon_sym_data] = ACTIONS(21),
    [anon_sym_datapack] = ACTIONS(19),
    [anon_sym_debug] = ACTIONS(19),
    [anon_sym_defaultgamemode] = ACTIONS(19),
    [anon_sym_deop] = ACTIONS(19),
    [anon_sym_dialog] = ACTIONS(19),
    [anon_sym_difficulty] = ACTIONS(19),
    [anon_sym_effect] = ACTIONS(19),
    [anon_sym_enchant] = ACTIONS(19),
    [anon_sym_experience] = ACTIONS(19),
    [anon_sym_fetchprofile] = ACTIONS(19),
    [anon_sym_fill] = ACTIONS(21),
    [anon_sym_fillbiome] = ACTIONS(19),
    [anon_sym_forceload] = ACTIONS(19),
    [anon_sym_function] = ACTIONS(19),
    [anon_sym_gamemode] = ACTIONS(19),
    [anon_sym_gamerule] = ACTIONS(19),
    [anon_sym_give] = ACTIONS(19),
    [anon_sym_help] = ACTIONS(19),
    [anon_sym_item] = ACTIONS(19),
    [anon_sym_jfr] = ACTIONS(19),
    [anon_sym_kick] = ACTIONS(19),
    [anon_sym_kill] = ACTIONS(19),
    [anon_sym_list] = ACTIONS(19),
    [anon_sym_locate] = ACTIONS(19),
    [anon_sym_loot] = ACTIONS(19),
    [anon_sym_me] = ACTIONS(19),
    [anon_sym_msg] = ACTIONS(19),
    [anon_sym_op] = ACTIONS(19),
    [anon_sym_pardon] = ACTIONS(21),
    [anon_sym_pardon_DASHip] = ACTIONS(19),
    [anon_sym_particle] = ACTIONS(19),
    [anon_sym_perf] = ACTIONS(19),
    [anon_sym_place] = ACTIONS(19),
    [anon_sym_playsound] = ACTIONS(19),
    [anon_sym_publish] = ACTIONS(19),
    [anon_sym_random] = ACTIONS(19),
    [anon_sym_recipe] = ACTIONS(19),
    [anon_sym_reload] = ACTIONS(19),
    [anon_sym_return] = ACTIONS(19),
    [anon_sym_ride] = ACTIONS(19),
    [anon_sym_rotate] = ACTIONS(19),
    [anon_sym_save_DASHall] = ACTIONS(19),
    [anon_sym_save_DASHoff] = ACTIONS(19),
    [anon_sym_save_DASHon] = ACTIONS(19),
    [anon_sym_say] = ACTIONS(19),
    [anon_sym_schedule] = ACTIONS(19),
    [anon_sym_scoreboard] = ACTIONS(19),
    [anon_sym_seed] = ACTIONS(19),
    [anon_sym_setblock] = ACTIONS(19),
    [anon_sym_setidletimeout] = ACTIONS(19),
    [anon_sym_setworldspawn] = ACTIONS(19),
    [anon_sym_spawnpoint] = ACTIONS(19),
    [anon_sym_spectate] = ACTIONS(19),
    [anon_sym_spreadplayers] = ACTIONS(19),
    [anon_sym_stop] = ACTIONS(21),
    [anon_sym_stopsound] = ACTIONS(19),
    [anon_sym_summon] = ACTIONS(19),
    [anon_sym_swing] = ACTIONS(19),
    [anon_sym_tag] = ACTIONS(19),
    [anon_sym_team] = ACTIONS(21),
    [anon_sym_teammsg] = ACTIONS(19),
    [anon_sym_teleport] = ACTIONS(19),
    [anon_sym_tell] = ACTIONS(21),
    [anon_sym_tellraw] = ACTIONS(19),
    [anon_sym_test] = ACTIONS(19),
    [anon_sym_time] = ACTIONS(19),
    [anon_sym_title] = ACTIONS(19),
    [anon_sym_tm] = ACTIONS(19),
    [anon_sym_tp] = ACTIONS(19),
    [anon_sym_transfer] = ACTIONS(19),
    [anon_sym_trigger] = ACTIONS(19),
    [anon_sym_version] = ACTIONS(19),
    [anon_sym_w] = ACTIONS(21),
    [anon_sym_waypoint] = ACTIONS(19),
    [anon_sym_weather] = ACTIONS(19),
    [anon_sym_whitelist] = ACTIONS(19),
    [anon_sym_worldborder] = ACTIONS(19),
    [anon_sym_xp] = ACTIONS(19),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [anon_sym_PERCENT_LBRACK] = ACTIONS(5),
  },
  [STATE(15)] = {
    [sym__subcommand] = STATE(209),
    [sym_subcommand_identifier] = STATE(210),
    [sym_selector] = STATE(210),
    [sym_position] = STATE(210),
    [sym_rotation] = STATE(210),
    [sym_scale] = STATE(210),
    [sym_macro] = STATE(15),
    [sym_substitution] = STATE(15),
    [sym_operator] = STATE(210),
    [sym_slot] = STATE(210),
    [sym_word] = STATE(210),
    [sym_string] = STATE(210),
    [sym_nbt_compound] = STATE(210),
    [sym_nbt_array] = STATE(210),
    [anon_sym_run] = ACTIONS(100),
    [anon_sym_bossbar] = ACTIONS(102),
    [anon_sym_data] = ACTIONS(102),
    [anon_sym_function] = ACTIONS(102),
    [anon_sym_summon] = ACTIONS(102),
    [anon_sym_align] = ACTIONS(102),
    [anon_sym_all] = ACTIONS(102),
    [anon_sym_anchored] = ACTIONS(102),
    [anon_sym_as] = ACTIONS(102),
    [anon_sym_at] = ACTIONS(102),
    [anon_sym_attacker] = ACTIONS(102),
    [anon_sym_biome] = ACTIONS(102),
    [anon_sym_block] = ACTIONS(102),
    [anon_sym_blocks] = ACTIONS(102),
    [anon_sym_controller] = ACTIONS(102),
    [anon_sym_dimension] = ACTIONS(102),
    [anon_sym_entity] = ACTIONS(102),
    [anon_sym_eyes] = ACTIONS(102),
    [anon_sym_facing] = ACTIONS(102),
    [anon_sym_feet] = ACTIONS(102),
    [anon_sym_if] = ACTIONS(102),
    [anon_sym_in] = ACTIONS(102),
    [anon_sym_items] = ACTIONS(102),
    [anon_sym_leasher] = ACTIONS(102),
    [anon_sym_loaded] = ACTIONS(102),
    [anon_sym_masked] = ACTIONS(102),
    [anon_sym_matches] = ACTIONS(102),
    [anon_sym_max] = ACTIONS(102),
    [anon_sym_on] = ACTIONS(102),
    [anon_sym_origin] = ACTIONS(102),
    [anon_sym_over] = ACTIONS(102),
    [anon_sym_owner] = ACTIONS(102),
    [anon_sym_passengers] = ACTIONS(102),
    [anon_sym_positioned] = ACTIONS(102),
    [anon_sym_predicate] = ACTIONS(102),
    [anon_sym_result] = ACTIONS(102),
    [anon_sym_rotated] = ACTIONS(102),
    [anon_sym_score] = ACTIONS(102),
    [anon_sym_storage] = ACTIONS(102),
    [anon_sym_store] = ACTIONS(102),
    [anon_sym_success] = ACTIONS(102),
    [anon_sym_target] = ACTIONS(102),
    [anon_sym_unless] = ACTIONS(102),
    [anon_sym_value] = ACTIONS(102),
    [anon_sym_vehicle] = ACTIONS(102),
    [sym_selector_identifier] = ACTIONS(25),
    [anon_sym_TILDE] = ACTIONS(27),
    [anon_sym_CARET] = ACTIONS(29),
    [sym_heightmap] = ACTIONS(104),
    [sym_resource] = ACTIONS(104),
    [anon_sym_DASH] = ACTIONS(31),
    [aux_sym_scale_token1] = ACTIONS(33),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [anon_sym_PERCENT_LBRACK] = ACTIONS(5),
    [sym_type] = ACTIONS(104),
    [anon_sym_PLUS_EQ] = ACTIONS(35),
    [anon_sym_DASH_EQ] = ACTIONS(35),
    [anon_sym_STAR_EQ] = ACTIONS(35),
    [anon_sym_SLASH_EQ] = ACTIONS(35),
    [anon_sym_PERCENT_EQ] = ACTIONS(35),
    [anon_sym_GT_LT] = ACTIONS(35),
    [anon_sym_LT] = ACTIONS(35),
    [anon_sym_GT] = ACTIONS(37),
    [aux_sym_slot_token1] = ACTIONS(39),
    [aux_sym_slot_token2] = ACTIONS(39),
    [aux_sym_slot_token3] = ACTIONS(39),
    [aux_sym_slot_token4] = ACTIONS(39),
    [aux_sym_slot_token5] = ACTIONS(39),
    [aux_sym_slot_token6] = ACTIONS(39),
    [aux_sym_slot_token7] = ACTIONS(39),
    [anon_sym_contents] = ACTIONS(39),
    [anon_sym_player_DOTcursor] = ACTIONS(39),
    [aux_sym_slot_token8] = ACTIONS(39),
    [aux_sym_word_token1] = ACTIONS(41),
    [anon_sym_DQUOTE] = ACTIONS(43),
    [anon_sym_SQUOTE] = ACTIONS(45),
    [sym_boolean] = ACTIONS(104),
    [anon_sym_LBRACE] = ACTIONS(47),
    [anon_sym_LBRACK] = ACTIONS(49),
  },
  [STATE(16)] = {
    [sym__subcommand] = STATE(209),
    [sym_subcommand_identifier] = STATE(210),
    [sym_selector] = STATE(210),
    [sym_position] = STATE(210),
    [sym_rotation] = STATE(210),
    [sym_scale] = STATE(210),
    [sym_macro] = STATE(16),
    [sym_substitution] = STATE(16),
    [sym_operator] = STATE(210),
    [sym_slot] = STATE(210),
    [sym_word] = STATE(210),
    [sym_string] = STATE(210),
    [sym_nbt_compound] = STATE(210),
    [sym_nbt_array] = STATE(210),
    [anon_sym_run] = ACTIONS(106),
    [anon_sym_bossbar] = ACTIONS(102),
    [anon_sym_data] = ACTIONS(102),
    [anon_sym_function] = ACTIONS(102),
    [anon_sym_summon] = ACTIONS(102),
    [anon_sym_align] = ACTIONS(102),
    [anon_sym_all] = ACTIONS(102),
    [anon_sym_anchored] = ACTIONS(102),
    [anon_sym_as] = ACTIONS(102),
    [anon_sym_at] = ACTIONS(102),
    [anon_sym_attacker] = ACTIONS(102),
    [anon_sym_biome] = ACTIONS(102),
    [anon_sym_block] = ACTIONS(102),
    [anon_sym_blocks] = ACTIONS(102),
    [anon_sym_controller] = ACTIONS(102),
    [anon_sym_dimension] = ACTIONS(102),
    [anon_sym_entity] = ACTIONS(102),
    [anon_sym_eyes] = ACTIONS(102),
    [anon_sym_facing] = ACTIONS(102),
    [anon_sym_feet] = ACTIONS(102),
    [anon_sym_if] = ACTIONS(102),
    [anon_sym_in] = ACTIONS(102),
    [anon_sym_items] = ACTIONS(102),
    [anon_sym_leasher] = ACTIONS(102),
    [anon_sym_loaded] = ACTIONS(102),
    [anon_sym_masked] = ACTIONS(102),
    [anon_sym_matches] = ACTIONS(102),
    [anon_sym_max] = ACTIONS(102),
    [anon_sym_on] = ACTIONS(102),
    [anon_sym_origin] = ACTIONS(102),
    [anon_sym_over] = ACTIONS(102),
    [anon_sym_owner] = ACTIONS(102),
    [anon_sym_passengers] = ACTIONS(102),
    [anon_sym_positioned] = ACTIONS(102),
    [anon_sym_predicate] = ACTIONS(102),
    [anon_sym_result] = ACTIONS(102),
    [anon_sym_rotated] = ACTIONS(102),
    [anon_sym_score] = ACTIONS(102),
    [anon_sym_storage] = ACTIONS(102),
    [anon_sym_store] = ACTIONS(102),
    [anon_sym_success] = ACTIONS(102),
    [anon_sym_target] = ACTIONS(102),
    [anon_sym_unless] = ACTIONS(102),
    [anon_sym_value] = ACTIONS(102),
    [anon_sym_vehicle] = ACTIONS(102),
    [sym_selector_identifier] = ACTIONS(25),
    [anon_sym_TILDE] = ACTIONS(27),
    [anon_sym_CARET] = ACTIONS(29),
    [sym_heightmap] = ACTIONS(104),
    [sym_resource] = ACTIONS(104),
    [anon_sym_DASH] = ACTIONS(31),
    [aux_sym_scale_token1] = ACTIONS(33),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [anon_sym_PERCENT_LBRACK] = ACTIONS(5),
    [sym_type] = ACTIONS(104),
    [anon_sym_PLUS_EQ] = ACTIONS(35),
    [anon_sym_DASH_EQ] = ACTIONS(35),
    [anon_sym_STAR_EQ] = ACTIONS(35),
    [anon_sym_SLASH_EQ] = ACTIONS(35),
    [anon_sym_PERCENT_EQ] = ACTIONS(35),
    [anon_sym_GT_LT] = ACTIONS(35),
    [anon_sym_LT] = ACTIONS(35),
    [anon_sym_GT] = ACTIONS(37),
    [aux_sym_slot_token1] = ACTIONS(39),
    [aux_sym_slot_token2] = ACTIONS(39),
    [aux_sym_slot_token3] = ACTIONS(39),
    [aux_sym_slot_token4] = ACTIONS(39),
    [aux_sym_slot_token5] = ACTIONS(39),
    [aux_sym_slot_token6] = ACTIONS(39),
    [aux_sym_slot_token7] = ACTIONS(39),
    [anon_sym_contents] = ACTIONS(39),
    [anon_sym_player_DOTcursor] = ACTIONS(39),
    [aux_sym_slot_token8] = ACTIONS(39),
    [aux_sym_word_token1] = ACTIONS(41),
    [anon_sym_DQUOTE] = ACTIONS(43),
    [anon_sym_SQUOTE] = ACTIONS(45),
    [sym_boolean] = ACTIONS(104),
    [anon_sym_LBRACE] = ACTIONS(47),
    [anon_sym_LBRACK] = ACTIONS(49),
  },
  [STATE(17)] = {
    [sym__subcommand] = STATE(209),
    [sym_subcommand_identifier] = STATE(210),
    [sym_selector] = STATE(210),
    [sym_position] = STATE(210),
    [sym_rotation] = STATE(210),
    [sym_scale] = STATE(210),
    [sym_macro] = STATE(17),
    [sym_substitution] = STATE(17),
    [sym_operator] = STATE(210),
    [sym_slot] = STATE(210),
    [sym_word] = STATE(210),
    [sym_string] = STATE(210),
    [sym_nbt_compound] = STATE(210),
    [sym_nbt_array] = STATE(210),
    [anon_sym_bossbar] = ACTIONS(102),
    [anon_sym_data] = ACTIONS(102),
    [anon_sym_function] = ACTIONS(102),
    [anon_sym_summon] = ACTIONS(102),
    [anon_sym_align] = ACTIONS(102),
    [anon_sym_all] = ACTIONS(102),
    [anon_sym_anchored] = ACTIONS(102),
    [anon_sym_as] = ACTIONS(102),
    [anon_sym_at] = ACTIONS(102),
    [anon_sym_attacker] = ACTIONS(102),
    [anon_sym_biome] = ACTIONS(102),
    [anon_sym_block] = ACTIONS(102),
    [anon_sym_blocks] = ACTIONS(102),
    [anon_sym_controller] = ACTIONS(102),
    [anon_sym_dimension] = ACTIONS(102),
    [anon_sym_entity] = ACTIONS(102),
    [anon_sym_eyes] = ACTIONS(102),
    [anon_sym_facing] = ACTIONS(102),
    [anon_sym_feet] = ACTIONS(102),
    [anon_sym_if] = ACTIONS(102),
    [anon_sym_in] = ACTIONS(102),
    [anon_sym_items] = ACTIONS(102),
    [anon_sym_leasher] = ACTIONS(102),
    [anon_sym_loaded] = ACTIONS(102),
    [anon_sym_masked] = ACTIONS(102),
    [anon_sym_matches] = ACTIONS(102),
    [anon_sym_max] = ACTIONS(102),
    [anon_sym_on] = ACTIONS(102),
    [anon_sym_origin] = ACTIONS(102),
    [anon_sym_over] = ACTIONS(102),
    [anon_sym_owner] = ACTIONS(102),
    [anon_sym_passengers] = ACTIONS(102),
    [anon_sym_positioned] = ACTIONS(102),
    [anon_sym_predicate] = ACTIONS(102),
    [anon_sym_result] = ACTIONS(102),
    [anon_sym_rotated] = ACTIONS(102),
    [anon_sym_score] = ACTIONS(102),
    [anon_sym_storage] = ACTIONS(102),
    [anon_sym_store] = ACTIONS(102),
    [anon_sym_success] = ACTIONS(102),
    [anon_sym_target] = ACTIONS(102),
    [anon_sym_unless] = ACTIONS(102),
    [anon_sym_value] = ACTIONS(102),
    [anon_sym_vehicle] = ACTIONS(102),
    [sym_selector_identifier] = ACTIONS(25),
    [anon_sym_TILDE] = ACTIONS(27),
    [anon_sym_CARET] = ACTIONS(29),
    [sym_heightmap] = ACTIONS(104),
    [sym_resource] = ACTIONS(104),
    [anon_sym_DASH] = ACTIONS(31),
    [aux_sym_scale_token1] = ACTIONS(33),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [anon_sym_PERCENT_LBRACK] = ACTIONS(5),
    [sym_type] = ACTIONS(104),
    [anon_sym_PLUS_EQ] = ACTIONS(35),
    [anon_sym_DASH_EQ] = ACTIONS(35),
    [anon_sym_STAR_EQ] = ACTIONS(35),
    [anon_sym_SLASH_EQ] = ACTIONS(35),
    [anon_sym_PERCENT_EQ] = ACTIONS(35),
    [anon_sym_GT_LT] = ACTIONS(35),
    [anon_sym_LT] = ACTIONS(35),
    [anon_sym_GT] = ACTIONS(37),
    [aux_sym_slot_token1] = ACTIONS(39),
    [aux_sym_slot_token2] = ACTIONS(39),
    [aux_sym_slot_token3] = ACTIONS(39),
    [aux_sym_slot_token4] = ACTIONS(39),
    [aux_sym_slot_token5] = ACTIONS(39),
    [aux_sym_slot_token6] = ACTIONS(39),
    [aux_sym_slot_token7] = ACTIONS(39),
    [anon_sym_contents] = ACTIONS(39),
    [anon_sym_player_DOTcursor] = ACTIONS(39),
    [aux_sym_slot_token8] = ACTIONS(39),
    [aux_sym_word_token1] = ACTIONS(41),
    [anon_sym_DQUOTE] = ACTIONS(43),
    [anon_sym_SQUOTE] = ACTIONS(45),
    [sym_boolean] = ACTIONS(104),
    [anon_sym_LBRACE] = ACTIONS(47),
    [anon_sym_LBRACK] = ACTIONS(49),
  },
  [STATE(18)] = {
    [sym_macro] = STATE(18),
    [sym_substitution] = STATE(18),
    [aux_sym__space_token1] = ACTIONS(51),
    [sym_backslash] = ACTIONS(53),
    [anon_sym_run] = ACTIONS(51),
    [anon_sym_bossbar] = ACTIONS(51),
    [anon_sym_data] = ACTIONS(51),
    [anon_sym_function] = ACTIONS(51),
    [anon_sym_summon] = ACTIONS(51),
    [anon_sym_align] = ACTIONS(51),
    [anon_sym_all] = ACTIONS(51),
    [anon_sym_anchored] = ACTIONS(51),
    [anon_sym_as] = ACTIONS(51),
    [anon_sym_at] = ACTIONS(51),
    [anon_sym_attacker] = ACTIONS(51),
    [anon_sym_biome] = ACTIONS(51),
    [anon_sym_block] = ACTIONS(51),
    [anon_sym_blocks] = ACTIONS(51),
    [anon_sym_controller] = ACTIONS(51),
    [anon_sym_dimension] = ACTIONS(51),
    [anon_sym_entity] = ACTIONS(51),
    [anon_sym_eyes] = ACTIONS(51),
    [anon_sym_facing] = ACTIONS(51),
    [anon_sym_feet] = ACTIONS(51),
    [anon_sym_if] = ACTIONS(51),
    [anon_sym_in] = ACTIONS(51),
    [anon_sym_items] = ACTIONS(51),
    [anon_sym_leasher] = ACTIONS(51),
    [anon_sym_loaded] = ACTIONS(51),
    [anon_sym_masked] = ACTIONS(51),
    [anon_sym_matches] = ACTIONS(51),
    [anon_sym_max] = ACTIONS(51),
    [anon_sym_on] = ACTIONS(51),
    [anon_sym_origin] = ACTIONS(51),
    [anon_sym_over] = ACTIONS(51),
    [anon_sym_owner] = ACTIONS(51),
    [anon_sym_passengers] = ACTIONS(51),
    [anon_sym_positioned] = ACTIONS(51),
    [anon_sym_predicate] = ACTIONS(51),
    [anon_sym_result] = ACTIONS(51),
    [anon_sym_rotated] = ACTIONS(51),
    [anon_sym_score] = ACTIONS(51),
    [anon_sym_storage] = ACTIONS(51),
    [anon_sym_store] = ACTIONS(51),
    [anon_sym_success] = ACTIONS(51),
    [anon_sym_target] = ACTIONS(51),
    [anon_sym_unless] = ACTIONS(51),
    [anon_sym_value] = ACTIONS(51),
    [anon_sym_vehicle] = ACTIONS(51),
    [sym_selector_identifier] = ACTIONS(53),
    [anon_sym_EQ] = ACTIONS(51),
    [anon_sym_EQ_BANG] = ACTIONS(53),
    [anon_sym_TILDE] = ACTIONS(53),
    [anon_sym_CARET] = ACTIONS(53),
    [sym_heightmap] = ACTIONS(51),
    [sym_resource] = ACTIONS(51),
    [anon_sym_DASH] = ACTIONS(51),
    [aux_sym_scale_token1] = ACTIONS(51),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [anon_sym_PERCENT_LBRACK] = ACTIONS(5),
    [sym_type] = ACTIONS(51),
    [anon_sym_PLUS_EQ] = ACTIONS(53),
    [anon_sym_DASH_EQ] = ACTIONS(53),
    [anon_sym_STAR_EQ] = ACTIONS(53),
    [anon_sym_SLASH_EQ] = ACTIONS(53),
    [anon_sym_PERCENT_EQ] = ACTIONS(53),
    [anon_sym_GT_LT] = ACTIONS(53),
    [anon_sym_LT] = ACTIONS(53),
    [anon_sym_GT] = ACTIONS(51),
    [aux_sym_slot_token1] = ACTIONS(51),
    [aux_sym_slot_token2] = ACTIONS(51),
    [aux_sym_slot_token3] = ACTIONS(51),
    [aux_sym_slot_token4] = ACTIONS(51),
    [aux_sym_slot_token5] = ACTIONS(51),
    [aux_sym_slot_token6] = ACTIONS(51),
    [aux_sym_slot_token7] = ACTIONS(51),
    [anon_sym_contents] = ACTIONS(51),
    [anon_sym_player_DOTcursor] = ACTIONS(51),
    [aux_sym_slot_token8] = ACTIONS(51),
    [aux_sym_word_token1] = ACTIONS(51),
    [anon_sym_DQUOTE] = ACTIONS(53),
    [anon_sym_SQUOTE] = ACTIONS(53),
    [sym_boolean] = ACTIONS(51),
    [anon_sym_LBRACE] = ACTIONS(53),
    [anon_sym_LBRACK] = ACTIONS(53),
  },
  [STATE(19)] = {
    [sym_macro] = STATE(19),
    [sym_substitution] = STATE(19),
    [anon_sym_bossbar] = ACTIONS(51),
    [anon_sym_data] = ACTIONS(51),
    [anon_sym_function] = ACTIONS(51),
    [anon_sym_summon] = ACTIONS(51),
    [anon_sym_align] = ACTIONS(51),
    [anon_sym_all] = ACTIONS(51),
    [anon_sym_anchored] = ACTIONS(51),
    [anon_sym_as] = ACTIONS(51),
    [anon_sym_at] = ACTIONS(51),
    [anon_sym_attacker] = ACTIONS(51),
    [anon_sym_biome] = ACTIONS(51),
    [anon_sym_block] = ACTIONS(51),
    [anon_sym_blocks] = ACTIONS(51),
    [anon_sym_controller] = ACTIONS(51),
    [anon_sym_dimension] = ACTIONS(51),
    [anon_sym_entity] = ACTIONS(51),
    [anon_sym_eyes] = ACTIONS(51),
    [anon_sym_facing] = ACTIONS(51),
    [anon_sym_feet] = ACTIONS(51),
    [anon_sym_if] = ACTIONS(51),
    [anon_sym_in] = ACTIONS(51),
    [anon_sym_items] = ACTIONS(51),
    [anon_sym_leasher] = ACTIONS(51),
    [anon_sym_loaded] = ACTIONS(51),
    [anon_sym_masked] = ACTIONS(51),
    [anon_sym_matches] = ACTIONS(51),
    [anon_sym_max] = ACTIONS(51),
    [anon_sym_on] = ACTIONS(51),
    [anon_sym_origin] = ACTIONS(51),
    [anon_sym_over] = ACTIONS(51),
    [anon_sym_owner] = ACTIONS(51),
    [anon_sym_passengers] = ACTIONS(51),
    [anon_sym_positioned] = ACTIONS(51),
    [anon_sym_predicate] = ACTIONS(51),
    [anon_sym_result] = ACTIONS(51),
    [anon_sym_rotated] = ACTIONS(51),
    [anon_sym_score] = ACTIONS(51),
    [anon_sym_storage] = ACTIONS(51),
    [anon_sym_store] = ACTIONS(51),
    [anon_sym_success] = ACTIONS(51),
    [anon_sym_target] = ACTIONS(51),
    [anon_sym_unless] = ACTIONS(51),
    [anon_sym_value] = ACTIONS(51),
    [anon_sym_vehicle] = ACTIONS(51),
    [sym_selector_identifier] = ACTIONS(53),
    [anon_sym_TILDE] = ACTIONS(53),
    [anon_sym_CARET] = ACTIONS(53),
    [sym_heightmap] = ACTIONS(51),
    [sym_resource] = ACTIONS(51),
    [anon_sym_DASH] = ACTIONS(51),
    [aux_sym_scale_token1] = ACTIONS(51),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [anon_sym_PERCENT_LBRACK] = ACTIONS(5),
    [sym_type] = ACTIONS(51),
    [anon_sym_PLUS_EQ] = ACTIONS(53),
    [anon_sym_DASH_EQ] = ACTIONS(53),
    [anon_sym_STAR_EQ] = ACTIONS(53),
    [anon_sym_SLASH_EQ] = ACTIONS(53),
    [anon_sym_PERCENT_EQ] = ACTIONS(53),
    [anon_sym_GT_LT] = ACTIONS(53),
    [anon_sym_LT] = ACTIONS(53),
    [anon_sym_GT] = ACTIONS(51),
    [aux_sym_slot_token1] = ACTIONS(51),
    [aux_sym_slot_token2] = ACTIONS(51),
    [aux_sym_slot_token3] = ACTIONS(51),
    [aux_sym_slot_token4] = ACTIONS(51),
    [aux_sym_slot_token5] = ACTIONS(51),
    [aux_sym_slot_token6] = ACTIONS(51),
    [aux_sym_slot_token7] = ACTIONS(51),
    [anon_sym_contents] = ACTIONS(51),
    [anon_sym_player_DOTcursor] = ACTIONS(51),
    [aux_sym_slot_token8] = ACTIONS(51),
    [aux_sym_word_token1] = ACTIONS(51),
    [anon_sym_DQUOTE] = ACTIONS(53),
    [anon_sym_SQUOTE] = ACTIONS(53),
    [sym_boolean] = ACTIONS(51),
    [anon_sym_LBRACE] = ACTIONS(53),
    [anon_sym_LBRACK] = ACTIONS(53),
  },
};

static const uint16_t ts_small_parse_table[] = {
  [0] = 22,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(108), 1,
      aux_sym__space_token1,
    ACTIONS(110), 1,
      sym_backslash,
    ACTIONS(112), 1,
      sym_selector_identifier,
    ACTIONS(114), 1,
      anon_sym_TILDE,
    ACTIONS(116), 1,
      anon_sym_CARET,
    ACTIONS(120), 1,
      anon_sym_DASH,
    ACTIONS(122), 1,
      aux_sym_scale_token1,
    ACTIONS(126), 1,
      anon_sym_GT,
    ACTIONS(130), 1,
      aux_sym_word_token1,
    ACTIONS(132), 1,
      anon_sym_DQUOTE,
    ACTIONS(134), 1,
      anon_sym_SQUOTE,
    ACTIONS(136), 1,
      anon_sym_LBRACE,
    ACTIONS(138), 1,
      anon_sym_LBRACK,
    STATE(24), 1,
      aux_sym_selector_query_repeat1,
    STATE(25), 1,
      sym__space,
    STATE(20), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(118), 4,
      sym_heightmap,
      sym_resource,
      sym_type,
      sym_boolean,
    ACTIONS(124), 7,
      anon_sym_PLUS_EQ,
      anon_sym_DASH_EQ,
      anon_sym_STAR_EQ,
      anon_sym_SLASH_EQ,
      anon_sym_PERCENT_EQ,
      anon_sym_GT_LT,
      anon_sym_LT,
    ACTIONS(128), 10,
      aux_sym_slot_token1,
      aux_sym_slot_token2,
      aux_sym_slot_token3,
      aux_sym_slot_token4,
      aux_sym_slot_token5,
      aux_sym_slot_token6,
      aux_sym_slot_token7,
      anon_sym_contents,
      anon_sym_player_DOTcursor,
      aux_sym_slot_token8,
    STATE(194), 11,
      sym_selector,
      sym_position,
      sym_rotation,
      sym_scale,
      sym_operator,
      sym_slot,
      sym_word,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
      sym_selector_compound,
  [96] = 22,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(108), 1,
      aux_sym__space_token1,
    ACTIONS(110), 1,
      sym_backslash,
    ACTIONS(112), 1,
      sym_selector_identifier,
    ACTIONS(114), 1,
      anon_sym_TILDE,
    ACTIONS(116), 1,
      anon_sym_CARET,
    ACTIONS(120), 1,
      anon_sym_DASH,
    ACTIONS(122), 1,
      aux_sym_scale_token1,
    ACTIONS(126), 1,
      anon_sym_GT,
    ACTIONS(130), 1,
      aux_sym_word_token1,
    ACTIONS(132), 1,
      anon_sym_DQUOTE,
    ACTIONS(134), 1,
      anon_sym_SQUOTE,
    ACTIONS(136), 1,
      anon_sym_LBRACE,
    ACTIONS(138), 1,
      anon_sym_LBRACK,
    STATE(24), 1,
      aux_sym_selector_query_repeat1,
    STATE(25), 1,
      sym__space,
    STATE(21), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(140), 4,
      sym_heightmap,
      sym_resource,
      sym_type,
      sym_boolean,
    ACTIONS(124), 7,
      anon_sym_PLUS_EQ,
      anon_sym_DASH_EQ,
      anon_sym_STAR_EQ,
      anon_sym_SLASH_EQ,
      anon_sym_PERCENT_EQ,
      anon_sym_GT_LT,
      anon_sym_LT,
    ACTIONS(128), 10,
      aux_sym_slot_token1,
      aux_sym_slot_token2,
      aux_sym_slot_token3,
      aux_sym_slot_token4,
      aux_sym_slot_token5,
      aux_sym_slot_token6,
      aux_sym_slot_token7,
      anon_sym_contents,
      anon_sym_player_DOTcursor,
      aux_sym_slot_token8,
    STATE(205), 11,
      sym_selector,
      sym_position,
      sym_rotation,
      sym_scale,
      sym_operator,
      sym_slot,
      sym_word,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
      sym_selector_compound,
  [192] = 22,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(108), 1,
      aux_sym__space_token1,
    ACTIONS(110), 1,
      sym_backslash,
    ACTIONS(112), 1,
      sym_selector_identifier,
    ACTIONS(114), 1,
      anon_sym_TILDE,
    ACTIONS(116), 1,
      anon_sym_CARET,
    ACTIONS(120), 1,
      anon_sym_DASH,
    ACTIONS(122), 1,
      aux_sym_scale_token1,
    ACTIONS(126), 1,
      anon_sym_GT,
    ACTIONS(130), 1,
      aux_sym_word_token1,
    ACTIONS(132), 1,
      anon_sym_DQUOTE,
    ACTIONS(134), 1,
      anon_sym_SQUOTE,
    ACTIONS(136), 1,
      anon_sym_LBRACE,
    ACTIONS(138), 1,
      anon_sym_LBRACK,
    STATE(20), 1,
      aux_sym_selector_query_repeat1,
    STATE(25), 1,
      sym__space,
    STATE(22), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(140), 4,
      sym_heightmap,
      sym_resource,
      sym_type,
      sym_boolean,
    ACTIONS(124), 7,
      anon_sym_PLUS_EQ,
      anon_sym_DASH_EQ,
      anon_sym_STAR_EQ,
      anon_sym_SLASH_EQ,
      anon_sym_PERCENT_EQ,
      anon_sym_GT_LT,
      anon_sym_LT,
    ACTIONS(128), 10,
      aux_sym_slot_token1,
      aux_sym_slot_token2,
      aux_sym_slot_token3,
      aux_sym_slot_token4,
      aux_sym_slot_token5,
      aux_sym_slot_token6,
      aux_sym_slot_token7,
      anon_sym_contents,
      anon_sym_player_DOTcursor,
      aux_sym_slot_token8,
    STATE(205), 11,
      sym_selector,
      sym_position,
      sym_rotation,
      sym_scale,
      sym_operator,
      sym_slot,
      sym_word,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
      sym_selector_compound,
  [288] = 22,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(108), 1,
      aux_sym__space_token1,
    ACTIONS(110), 1,
      sym_backslash,
    ACTIONS(112), 1,
      sym_selector_identifier,
    ACTIONS(114), 1,
      anon_sym_TILDE,
    ACTIONS(116), 1,
      anon_sym_CARET,
    ACTIONS(120), 1,
      anon_sym_DASH,
    ACTIONS(122), 1,
      aux_sym_scale_token1,
    ACTIONS(126), 1,
      anon_sym_GT,
    ACTIONS(130), 1,
      aux_sym_word_token1,
    ACTIONS(132), 1,
      anon_sym_DQUOTE,
    ACTIONS(134), 1,
      anon_sym_SQUOTE,
    ACTIONS(136), 1,
      anon_sym_LBRACE,
    ACTIONS(138), 1,
      anon_sym_LBRACK,
    STATE(21), 1,
      aux_sym_selector_query_repeat1,
    STATE(25), 1,
      sym__space,
    STATE(23), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(142), 4,
      sym_heightmap,
      sym_resource,
      sym_type,
      sym_boolean,
    ACTIONS(124), 7,
      anon_sym_PLUS_EQ,
      anon_sym_DASH_EQ,
      anon_sym_STAR_EQ,
      anon_sym_SLASH_EQ,
      anon_sym_PERCENT_EQ,
      anon_sym_GT_LT,
      anon_sym_LT,
    ACTIONS(128), 10,
      aux_sym_slot_token1,
      aux_sym_slot_token2,
      aux_sym_slot_token3,
      aux_sym_slot_token4,
      aux_sym_slot_token5,
      aux_sym_slot_token6,
      aux_sym_slot_token7,
      anon_sym_contents,
      anon_sym_player_DOTcursor,
      aux_sym_slot_token8,
    STATE(169), 11,
      sym_selector,
      sym_position,
      sym_rotation,
      sym_scale,
      sym_operator,
      sym_slot,
      sym_word,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
      sym_selector_compound,
  [384] = 8,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(144), 1,
      aux_sym__space_token1,
    ACTIONS(147), 1,
      sym_backslash,
    STATE(25), 1,
      sym__space,
    STATE(24), 3,
      sym_macro,
      sym_substitution,
      aux_sym_selector_query_repeat1,
    ACTIONS(150), 14,
      sym_selector_identifier,
      anon_sym_TILDE,
      anon_sym_CARET,
      anon_sym_PLUS_EQ,
      anon_sym_DASH_EQ,
      anon_sym_STAR_EQ,
      anon_sym_SLASH_EQ,
      anon_sym_PERCENT_EQ,
      anon_sym_GT_LT,
      anon_sym_LT,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
    ACTIONS(152), 18,
      sym_heightmap,
      sym_resource,
      anon_sym_DASH,
      aux_sym_scale_token1,
      sym_type,
      anon_sym_GT,
      aux_sym_slot_token1,
      aux_sym_slot_token2,
      aux_sym_slot_token3,
      aux_sym_slot_token4,
      aux_sym_slot_token5,
      aux_sym_slot_token6,
      aux_sym_slot_token7,
      anon_sym_contents,
      anon_sym_player_DOTcursor,
      aux_sym_slot_token8,
      aux_sym_word_token1,
      sym_boolean,
  [441] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(25), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(156), 16,
      sym_backslash,
      sym_selector_identifier,
      anon_sym_EQ,
      anon_sym_TILDE,
      anon_sym_CARET,
      anon_sym_PLUS_EQ,
      anon_sym_DASH_EQ,
      anon_sym_STAR_EQ,
      anon_sym_SLASH_EQ,
      anon_sym_PERCENT_EQ,
      anon_sym_GT_LT,
      anon_sym_LT,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
    ACTIONS(154), 19,
      aux_sym__space_token1,
      sym_heightmap,
      sym_resource,
      anon_sym_DASH,
      aux_sym_scale_token1,
      sym_type,
      anon_sym_GT,
      aux_sym_slot_token1,
      aux_sym_slot_token2,
      aux_sym_slot_token3,
      aux_sym_slot_token4,
      aux_sym_slot_token5,
      aux_sym_slot_token6,
      aux_sym_slot_token7,
      anon_sym_contents,
      anon_sym_player_DOTcursor,
      aux_sym_slot_token8,
      aux_sym_word_token1,
      sym_boolean,
  [491] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(26), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(53), 15,
      sym_backslash,
      sym_selector_identifier,
      anon_sym_TILDE,
      anon_sym_CARET,
      anon_sym_PLUS_EQ,
      anon_sym_DASH_EQ,
      anon_sym_STAR_EQ,
      anon_sym_SLASH_EQ,
      anon_sym_PERCENT_EQ,
      anon_sym_GT_LT,
      anon_sym_LT,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
    ACTIONS(51), 19,
      aux_sym__space_token1,
      sym_heightmap,
      sym_resource,
      anon_sym_DASH,
      aux_sym_scale_token1,
      sym_type,
      anon_sym_GT,
      aux_sym_slot_token1,
      aux_sym_slot_token2,
      aux_sym_slot_token3,
      aux_sym_slot_token4,
      aux_sym_slot_token5,
      aux_sym_slot_token6,
      aux_sym_slot_token7,
      anon_sym_contents,
      anon_sym_player_DOTcursor,
      aux_sym_slot_token8,
      aux_sym_word_token1,
      sym_boolean,
  [540] = 17,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(158), 1,
      aux_sym__space_token1,
    ACTIONS(160), 1,
      sym_backslash,
    ACTIONS(162), 1,
      aux_sym_selector_query_token1,
    ACTIONS(164), 1,
      anon_sym_DASH,
    ACTIONS(166), 1,
      aux_sym_scale_token1,
    ACTIONS(168), 1,
      anon_sym_DQUOTE,
    ACTIONS(170), 1,
      anon_sym_SQUOTE,
    ACTIONS(174), 1,
      anon_sym_LBRACE,
    ACTIONS(176), 1,
      anon_sym_LBRACK,
    ACTIONS(178), 1,
      anon_sym_RBRACK,
    STATE(31), 1,
      aux_sym_nbt_array_repeat1,
    ACTIONS(172), 2,
      sym_boolean,
      sym_range,
    STATE(27), 2,
      sym_macro,
      sym_substitution,
    STATE(54), 2,
      sym__space,
      sym__nbt_element,
    STATE(55), 4,
      sym_scale,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
  [598] = 17,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(158), 1,
      aux_sym__space_token1,
    ACTIONS(160), 1,
      sym_backslash,
    ACTIONS(162), 1,
      aux_sym_selector_query_token1,
    ACTIONS(164), 1,
      anon_sym_DASH,
    ACTIONS(166), 1,
      aux_sym_scale_token1,
    ACTIONS(168), 1,
      anon_sym_DQUOTE,
    ACTIONS(170), 1,
      anon_sym_SQUOTE,
    ACTIONS(174), 1,
      anon_sym_LBRACE,
    ACTIONS(176), 1,
      anon_sym_LBRACK,
    ACTIONS(180), 1,
      anon_sym_RBRACK,
    STATE(31), 1,
      aux_sym_nbt_array_repeat1,
    ACTIONS(172), 2,
      sym_boolean,
      sym_range,
    STATE(28), 2,
      sym_macro,
      sym_substitution,
    STATE(54), 2,
      sym__space,
      sym__nbt_element,
    STATE(55), 4,
      sym_scale,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
  [656] = 17,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(158), 1,
      aux_sym__space_token1,
    ACTIONS(160), 1,
      sym_backslash,
    ACTIONS(162), 1,
      aux_sym_selector_query_token1,
    ACTIONS(164), 1,
      anon_sym_DASH,
    ACTIONS(166), 1,
      aux_sym_scale_token1,
    ACTIONS(168), 1,
      anon_sym_DQUOTE,
    ACTIONS(170), 1,
      anon_sym_SQUOTE,
    ACTIONS(174), 1,
      anon_sym_LBRACE,
    ACTIONS(176), 1,
      anon_sym_LBRACK,
    ACTIONS(182), 1,
      anon_sym_RBRACK,
    STATE(31), 1,
      aux_sym_nbt_array_repeat1,
    ACTIONS(172), 2,
      sym_boolean,
      sym_range,
    STATE(29), 2,
      sym_macro,
      sym_substitution,
    STATE(54), 2,
      sym__space,
      sym__nbt_element,
    STATE(55), 4,
      sym_scale,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
  [714] = 17,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(158), 1,
      aux_sym__space_token1,
    ACTIONS(160), 1,
      sym_backslash,
    ACTIONS(162), 1,
      aux_sym_selector_query_token1,
    ACTIONS(164), 1,
      anon_sym_DASH,
    ACTIONS(166), 1,
      aux_sym_scale_token1,
    ACTIONS(168), 1,
      anon_sym_DQUOTE,
    ACTIONS(170), 1,
      anon_sym_SQUOTE,
    ACTIONS(174), 1,
      anon_sym_LBRACE,
    ACTIONS(176), 1,
      anon_sym_LBRACK,
    ACTIONS(184), 1,
      anon_sym_RBRACK,
    STATE(29), 1,
      aux_sym_nbt_array_repeat1,
    ACTIONS(172), 2,
      sym_boolean,
      sym_range,
    STATE(30), 2,
      sym_macro,
      sym_substitution,
    STATE(54), 2,
      sym__space,
      sym__nbt_element,
    STATE(55), 4,
      sym_scale,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
  [772] = 16,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(186), 1,
      aux_sym__space_token1,
    ACTIONS(189), 1,
      sym_backslash,
    ACTIONS(192), 1,
      aux_sym_selector_query_token1,
    ACTIONS(195), 1,
      anon_sym_DASH,
    ACTIONS(198), 1,
      aux_sym_scale_token1,
    ACTIONS(201), 1,
      anon_sym_DQUOTE,
    ACTIONS(204), 1,
      anon_sym_SQUOTE,
    ACTIONS(210), 1,
      anon_sym_LBRACE,
    ACTIONS(213), 1,
      anon_sym_LBRACK,
    ACTIONS(216), 1,
      anon_sym_RBRACK,
    ACTIONS(207), 2,
      sym_boolean,
      sym_range,
    STATE(54), 2,
      sym__space,
      sym__nbt_element,
    STATE(31), 3,
      sym_macro,
      sym_substitution,
      aux_sym_nbt_array_repeat1,
    STATE(55), 4,
      sym_scale,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
  [828] = 17,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(158), 1,
      aux_sym__space_token1,
    ACTIONS(160), 1,
      sym_backslash,
    ACTIONS(162), 1,
      aux_sym_selector_query_token1,
    ACTIONS(164), 1,
      anon_sym_DASH,
    ACTIONS(166), 1,
      aux_sym_scale_token1,
    ACTIONS(168), 1,
      anon_sym_DQUOTE,
    ACTIONS(170), 1,
      anon_sym_SQUOTE,
    ACTIONS(174), 1,
      anon_sym_LBRACE,
    ACTIONS(176), 1,
      anon_sym_LBRACK,
    ACTIONS(218), 1,
      anon_sym_RBRACK,
    STATE(28), 1,
      aux_sym_nbt_array_repeat1,
    ACTIONS(172), 2,
      sym_boolean,
      sym_range,
    STATE(32), 2,
      sym_macro,
      sym_substitution,
    STATE(54), 2,
      sym__space,
      sym__nbt_element,
    STATE(55), 4,
      sym_scale,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
  [886] = 17,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(158), 1,
      aux_sym__space_token1,
    ACTIONS(160), 1,
      sym_backslash,
    ACTIONS(162), 1,
      aux_sym_selector_query_token1,
    ACTIONS(164), 1,
      anon_sym_DASH,
    ACTIONS(166), 1,
      aux_sym_scale_token1,
    ACTIONS(168), 1,
      anon_sym_DQUOTE,
    ACTIONS(170), 1,
      anon_sym_SQUOTE,
    ACTIONS(174), 1,
      anon_sym_LBRACE,
    ACTIONS(176), 1,
      anon_sym_LBRACK,
    ACTIONS(220), 1,
      anon_sym_RBRACK,
    STATE(37), 1,
      aux_sym_nbt_array_repeat1,
    ACTIONS(172), 2,
      sym_boolean,
      sym_range,
    STATE(33), 2,
      sym_macro,
      sym_substitution,
    STATE(54), 2,
      sym__space,
      sym__nbt_element,
    STATE(55), 4,
      sym_scale,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
  [944] = 17,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(158), 1,
      aux_sym__space_token1,
    ACTIONS(160), 1,
      sym_backslash,
    ACTIONS(162), 1,
      aux_sym_selector_query_token1,
    ACTIONS(164), 1,
      anon_sym_DASH,
    ACTIONS(166), 1,
      aux_sym_scale_token1,
    ACTIONS(168), 1,
      anon_sym_DQUOTE,
    ACTIONS(170), 1,
      anon_sym_SQUOTE,
    ACTIONS(174), 1,
      anon_sym_LBRACE,
    ACTIONS(176), 1,
      anon_sym_LBRACK,
    ACTIONS(222), 1,
      anon_sym_RBRACK,
    STATE(31), 1,
      aux_sym_nbt_array_repeat1,
    ACTIONS(172), 2,
      sym_boolean,
      sym_range,
    STATE(34), 2,
      sym_macro,
      sym_substitution,
    STATE(54), 2,
      sym__space,
      sym__nbt_element,
    STATE(55), 4,
      sym_scale,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
  [1002] = 17,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(158), 1,
      aux_sym__space_token1,
    ACTIONS(160), 1,
      sym_backslash,
    ACTIONS(162), 1,
      aux_sym_selector_query_token1,
    ACTIONS(164), 1,
      anon_sym_DASH,
    ACTIONS(166), 1,
      aux_sym_scale_token1,
    ACTIONS(168), 1,
      anon_sym_DQUOTE,
    ACTIONS(170), 1,
      anon_sym_SQUOTE,
    ACTIONS(174), 1,
      anon_sym_LBRACE,
    ACTIONS(176), 1,
      anon_sym_LBRACK,
    ACTIONS(224), 1,
      anon_sym_RBRACK,
    STATE(27), 1,
      aux_sym_nbt_array_repeat1,
    ACTIONS(172), 2,
      sym_boolean,
      sym_range,
    STATE(35), 2,
      sym_macro,
      sym_substitution,
    STATE(54), 2,
      sym__space,
      sym__nbt_element,
    STATE(55), 4,
      sym_scale,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
  [1060] = 17,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(158), 1,
      aux_sym__space_token1,
    ACTIONS(160), 1,
      sym_backslash,
    ACTIONS(162), 1,
      aux_sym_selector_query_token1,
    ACTIONS(164), 1,
      anon_sym_DASH,
    ACTIONS(166), 1,
      aux_sym_scale_token1,
    ACTIONS(168), 1,
      anon_sym_DQUOTE,
    ACTIONS(170), 1,
      anon_sym_SQUOTE,
    ACTIONS(174), 1,
      anon_sym_LBRACE,
    ACTIONS(176), 1,
      anon_sym_LBRACK,
    ACTIONS(226), 1,
      anon_sym_RBRACK,
    STATE(34), 1,
      aux_sym_nbt_array_repeat1,
    ACTIONS(172), 2,
      sym_boolean,
      sym_range,
    STATE(36), 2,
      sym_macro,
      sym_substitution,
    STATE(54), 2,
      sym__space,
      sym__nbt_element,
    STATE(55), 4,
      sym_scale,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
  [1118] = 17,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(158), 1,
      aux_sym__space_token1,
    ACTIONS(160), 1,
      sym_backslash,
    ACTIONS(162), 1,
      aux_sym_selector_query_token1,
    ACTIONS(164), 1,
      anon_sym_DASH,
    ACTIONS(166), 1,
      aux_sym_scale_token1,
    ACTIONS(168), 1,
      anon_sym_DQUOTE,
    ACTIONS(170), 1,
      anon_sym_SQUOTE,
    ACTIONS(174), 1,
      anon_sym_LBRACE,
    ACTIONS(176), 1,
      anon_sym_LBRACK,
    ACTIONS(228), 1,
      anon_sym_RBRACK,
    STATE(31), 1,
      aux_sym_nbt_array_repeat1,
    ACTIONS(172), 2,
      sym_boolean,
      sym_range,
    STATE(37), 2,
      sym_macro,
      sym_substitution,
    STATE(54), 2,
      sym__space,
      sym__nbt_element,
    STATE(55), 4,
      sym_scale,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
  [1176] = 16,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(230), 1,
      aux_sym__space_token1,
    ACTIONS(232), 1,
      sym_backslash,
    ACTIONS(234), 1,
      anon_sym_DASH,
    ACTIONS(236), 1,
      aux_sym_scale_token1,
    ACTIONS(238), 1,
      anon_sym_DQUOTE,
    ACTIONS(240), 1,
      anon_sym_SQUOTE,
    ACTIONS(244), 1,
      anon_sym_LBRACE,
    ACTIONS(246), 1,
      anon_sym_LBRACK,
    STATE(40), 1,
      aux_sym_selector_query_repeat1,
    STATE(47), 1,
      sym__space,
    STATE(144), 1,
      sym__nbt_element,
    ACTIONS(242), 2,
      sym_boolean,
      sym_range,
    STATE(38), 2,
      sym_macro,
      sym_substitution,
    STATE(139), 4,
      sym_scale,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
  [1230] = 16,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(230), 1,
      aux_sym__space_token1,
    ACTIONS(232), 1,
      sym_backslash,
    ACTIONS(234), 1,
      anon_sym_DASH,
    ACTIONS(236), 1,
      aux_sym_scale_token1,
    ACTIONS(238), 1,
      anon_sym_DQUOTE,
    ACTIONS(240), 1,
      anon_sym_SQUOTE,
    ACTIONS(244), 1,
      anon_sym_LBRACE,
    ACTIONS(246), 1,
      anon_sym_LBRACK,
    STATE(47), 1,
      sym__space,
    STATE(49), 1,
      aux_sym_selector_query_repeat1,
    STATE(144), 1,
      sym__nbt_element,
    ACTIONS(242), 2,
      sym_boolean,
      sym_range,
    STATE(39), 2,
      sym_macro,
      sym_substitution,
    STATE(139), 4,
      sym_scale,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
  [1284] = 16,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(230), 1,
      aux_sym__space_token1,
    ACTIONS(232), 1,
      sym_backslash,
    ACTIONS(234), 1,
      anon_sym_DASH,
    ACTIONS(236), 1,
      aux_sym_scale_token1,
    ACTIONS(238), 1,
      anon_sym_DQUOTE,
    ACTIONS(240), 1,
      anon_sym_SQUOTE,
    ACTIONS(244), 1,
      anon_sym_LBRACE,
    ACTIONS(246), 1,
      anon_sym_LBRACK,
    STATE(47), 1,
      sym__space,
    STATE(49), 1,
      aux_sym_selector_query_repeat1,
    STATE(135), 1,
      sym__nbt_element,
    ACTIONS(242), 2,
      sym_boolean,
      sym_range,
    STATE(40), 2,
      sym_macro,
      sym_substitution,
    STATE(139), 4,
      sym_scale,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
  [1338] = 16,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(230), 1,
      aux_sym__space_token1,
    ACTIONS(232), 1,
      sym_backslash,
    ACTIONS(234), 1,
      anon_sym_DASH,
    ACTIONS(236), 1,
      aux_sym_scale_token1,
    ACTIONS(238), 1,
      anon_sym_DQUOTE,
    ACTIONS(240), 1,
      anon_sym_SQUOTE,
    ACTIONS(244), 1,
      anon_sym_LBRACE,
    ACTIONS(246), 1,
      anon_sym_LBRACK,
    STATE(39), 1,
      aux_sym_selector_query_repeat1,
    STATE(47), 1,
      sym__space,
    STATE(140), 1,
      sym__nbt_element,
    ACTIONS(242), 2,
      sym_boolean,
      sym_range,
    STATE(41), 2,
      sym_macro,
      sym_substitution,
    STATE(139), 4,
      sym_scale,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
  [1392] = 16,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(230), 1,
      aux_sym__space_token1,
    ACTIONS(232), 1,
      sym_backslash,
    ACTIONS(248), 1,
      anon_sym_DASH,
    ACTIONS(250), 1,
      aux_sym_scale_token1,
    ACTIONS(252), 1,
      anon_sym_DQUOTE,
    ACTIONS(254), 1,
      anon_sym_SQUOTE,
    ACTIONS(258), 1,
      anon_sym_LBRACE,
    ACTIONS(260), 1,
      anon_sym_LBRACK,
    STATE(47), 1,
      sym__space,
    STATE(49), 1,
      aux_sym_selector_query_repeat1,
    STATE(107), 1,
      sym__nbt_element,
    ACTIONS(256), 2,
      sym_boolean,
      sym_range,
    STATE(42), 2,
      sym_macro,
      sym_substitution,
    STATE(106), 4,
      sym_scale,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
  [1446] = 16,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(230), 1,
      aux_sym__space_token1,
    ACTIONS(232), 1,
      sym_backslash,
    ACTIONS(248), 1,
      anon_sym_DASH,
    ACTIONS(250), 1,
      aux_sym_scale_token1,
    ACTIONS(252), 1,
      anon_sym_DQUOTE,
    ACTIONS(254), 1,
      anon_sym_SQUOTE,
    ACTIONS(258), 1,
      anon_sym_LBRACE,
    ACTIONS(260), 1,
      anon_sym_LBRACK,
    STATE(42), 1,
      aux_sym_selector_query_repeat1,
    STATE(47), 1,
      sym__space,
    STATE(111), 1,
      sym__nbt_element,
    ACTIONS(256), 2,
      sym_boolean,
      sym_range,
    STATE(43), 2,
      sym_macro,
      sym_substitution,
    STATE(106), 4,
      sym_scale,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
  [1500] = 16,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(230), 1,
      aux_sym__space_token1,
    ACTIONS(232), 1,
      sym_backslash,
    ACTIONS(248), 1,
      anon_sym_DASH,
    ACTIONS(250), 1,
      aux_sym_scale_token1,
    ACTIONS(252), 1,
      anon_sym_DQUOTE,
    ACTIONS(254), 1,
      anon_sym_SQUOTE,
    ACTIONS(258), 1,
      anon_sym_LBRACE,
    ACTIONS(260), 1,
      anon_sym_LBRACK,
    STATE(45), 1,
      aux_sym_selector_query_repeat1,
    STATE(47), 1,
      sym__space,
    STATE(113), 1,
      sym__nbt_element,
    ACTIONS(256), 2,
      sym_boolean,
      sym_range,
    STATE(44), 2,
      sym_macro,
      sym_substitution,
    STATE(106), 4,
      sym_scale,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
  [1554] = 16,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(230), 1,
      aux_sym__space_token1,
    ACTIONS(232), 1,
      sym_backslash,
    ACTIONS(248), 1,
      anon_sym_DASH,
    ACTIONS(250), 1,
      aux_sym_scale_token1,
    ACTIONS(252), 1,
      anon_sym_DQUOTE,
    ACTIONS(254), 1,
      anon_sym_SQUOTE,
    ACTIONS(258), 1,
      anon_sym_LBRACE,
    ACTIONS(260), 1,
      anon_sym_LBRACK,
    STATE(47), 1,
      sym__space,
    STATE(49), 1,
      aux_sym_selector_query_repeat1,
    STATE(111), 1,
      sym__nbt_element,
    ACTIONS(256), 2,
      sym_boolean,
      sym_range,
    STATE(45), 2,
      sym_macro,
      sym_substitution,
    STATE(106), 4,
      sym_scale,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
  [1608] = 13,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(230), 1,
      aux_sym__space_token1,
    ACTIONS(232), 1,
      sym_backslash,
    ACTIONS(248), 1,
      anon_sym_DASH,
    ACTIONS(250), 1,
      aux_sym_scale_token1,
    ACTIONS(258), 1,
      anon_sym_LBRACE,
    ACTIONS(260), 1,
      anon_sym_LBRACK,
    STATE(47), 1,
      sym__space,
    STATE(48), 1,
      aux_sym_selector_query_repeat1,
    ACTIONS(262), 2,
      sym_boolean,
      sym_range,
    STATE(46), 2,
      sym_macro,
      sym_substitution,
    STATE(115), 3,
      sym_scale,
      sym_nbt_compound,
      sym_nbt_array,
  [1652] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(47), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(154), 4,
      aux_sym__space_token1,
      anon_sym_EQ,
      anon_sym_DASH,
      aux_sym_scale_token1,
    ACTIONS(156), 9,
      sym_backslash,
      anon_sym_EQ_BANG,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      sym_boolean,
      sym_range,
      anon_sym_LBRACE,
      anon_sym_COLON,
      anon_sym_LBRACK,
  [1680] = 13,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(230), 1,
      aux_sym__space_token1,
    ACTIONS(232), 1,
      sym_backslash,
    ACTIONS(248), 1,
      anon_sym_DASH,
    ACTIONS(250), 1,
      aux_sym_scale_token1,
    ACTIONS(258), 1,
      anon_sym_LBRACE,
    ACTIONS(260), 1,
      anon_sym_LBRACK,
    STATE(47), 1,
      sym__space,
    STATE(49), 1,
      aux_sym_selector_query_repeat1,
    ACTIONS(264), 2,
      sym_boolean,
      sym_range,
    STATE(48), 2,
      sym_macro,
      sym_substitution,
    STATE(114), 3,
      sym_scale,
      sym_nbt_compound,
      sym_nbt_array,
  [1724] = 8,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(266), 1,
      aux_sym__space_token1,
    ACTIONS(269), 1,
      sym_backslash,
    STATE(47), 1,
      sym__space,
    ACTIONS(152), 2,
      anon_sym_DASH,
      aux_sym_scale_token1,
    STATE(49), 3,
      sym_macro,
      sym_substitution,
      aux_sym_selector_query_repeat1,
    ACTIONS(150), 7,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      sym_boolean,
      sym_range,
      anon_sym_LBRACE,
      anon_sym_COLON,
      anon_sym_LBRACK,
  [1758] = 13,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(230), 1,
      aux_sym__space_token1,
    ACTIONS(232), 1,
      sym_backslash,
    ACTIONS(248), 1,
      anon_sym_DASH,
    ACTIONS(250), 1,
      aux_sym_scale_token1,
    ACTIONS(258), 1,
      anon_sym_LBRACE,
    ACTIONS(260), 1,
      anon_sym_LBRACK,
    STATE(47), 1,
      sym__space,
    STATE(53), 1,
      aux_sym_selector_query_repeat1,
    ACTIONS(264), 2,
      sym_boolean,
      sym_range,
    STATE(50), 2,
      sym_macro,
      sym_substitution,
    STATE(114), 3,
      sym_scale,
      sym_nbt_compound,
      sym_nbt_array,
  [1802] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(276), 1,
      aux_sym_scale_token2,
    STATE(51), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(272), 3,
      aux_sym__space_token1,
      anon_sym_DASH,
      aux_sym_scale_token1,
    ACTIONS(274), 9,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      sym_boolean,
      sym_range,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
  [1832] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(282), 1,
      aux_sym_scale_token2,
    STATE(52), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(278), 3,
      aux_sym__space_token1,
      anon_sym_DASH,
      aux_sym_scale_token1,
    ACTIONS(280), 9,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      sym_boolean,
      sym_range,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
  [1862] = 13,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(230), 1,
      aux_sym__space_token1,
    ACTIONS(232), 1,
      sym_backslash,
    ACTIONS(248), 1,
      anon_sym_DASH,
    ACTIONS(250), 1,
      aux_sym_scale_token1,
    ACTIONS(258), 1,
      anon_sym_LBRACE,
    ACTIONS(260), 1,
      anon_sym_LBRACK,
    STATE(47), 1,
      sym__space,
    STATE(49), 1,
      aux_sym_selector_query_repeat1,
    ACTIONS(284), 2,
      sym_boolean,
      sym_range,
    STATE(53), 2,
      sym_macro,
      sym_substitution,
    STATE(98), 3,
      sym_scale,
      sym_nbt_compound,
      sym_nbt_array,
  [1906] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(54), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(286), 3,
      aux_sym__space_token1,
      anon_sym_DASH,
      aux_sym_scale_token1,
    ACTIONS(288), 9,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      sym_boolean,
      sym_range,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
  [1933] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(55), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(290), 3,
      aux_sym__space_token1,
      anon_sym_DASH,
      aux_sym_scale_token1,
    ACTIONS(292), 9,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      sym_boolean,
      sym_range,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
  [1960] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(56), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(294), 3,
      aux_sym__space_token1,
      anon_sym_DASH,
      aux_sym_scale_token1,
    ACTIONS(296), 9,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      sym_boolean,
      sym_range,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
  [1987] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(57), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(272), 3,
      aux_sym__space_token1,
      anon_sym_DASH,
      aux_sym_scale_token1,
    ACTIONS(274), 9,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      sym_boolean,
      sym_range,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
  [2014] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(58), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(298), 3,
      aux_sym__space_token1,
      anon_sym_DASH,
      aux_sym_scale_token1,
    ACTIONS(300), 9,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      sym_boolean,
      sym_range,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
  [2041] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(59), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(302), 3,
      aux_sym__space_token1,
      anon_sym_DASH,
      aux_sym_scale_token1,
    ACTIONS(304), 9,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      sym_boolean,
      sym_range,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
  [2068] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(60), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(306), 3,
      aux_sym__space_token1,
      anon_sym_DASH,
      aux_sym_scale_token1,
    ACTIONS(308), 9,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      sym_boolean,
      sym_range,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
  [2095] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(61), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(310), 3,
      aux_sym__space_token1,
      anon_sym_DASH,
      aux_sym_scale_token1,
    ACTIONS(312), 9,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      sym_boolean,
      sym_range,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
  [2122] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(62), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(314), 3,
      aux_sym__space_token1,
      anon_sym_DASH,
      aux_sym_scale_token1,
    ACTIONS(316), 9,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      sym_boolean,
      sym_range,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
  [2149] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(63), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(51), 3,
      aux_sym__space_token1,
      anon_sym_DASH,
      aux_sym_scale_token1,
    ACTIONS(53), 9,
      sym_backslash,
      anon_sym_EQ,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      sym_boolean,
      sym_range,
      anon_sym_LBRACE,
      anon_sym_COLON,
      anon_sym_LBRACK,
  [2176] = 10,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(318), 1,
      aux_sym__space_token1,
    ACTIONS(320), 1,
      sym_backslash,
    ACTIONS(322), 1,
      anon_sym_EQ,
    ACTIONS(326), 1,
      aux_sym_nbt_identifier_token1,
    STATE(25), 1,
      sym__space,
    STATE(132), 1,
      aux_sym_selector_query_repeat1,
    STATE(64), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(324), 5,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACK,
  [2212] = 13,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(328), 1,
      aux_sym_selector_query_token1,
    ACTIONS(330), 1,
      sym_resource,
    ACTIONS(332), 1,
      anon_sym_DQUOTE,
    ACTIONS(334), 1,
      anon_sym_SQUOTE,
    ACTIONS(336), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(338), 1,
      anon_sym_RBRACE,
    STATE(76), 1,
      aux_sym_selector_compound_repeat1,
    STATE(90), 1,
      aux_sym_nbt_compound_repeat1,
    STATE(103), 1,
      sym_nbt_identifier,
    STATE(156), 1,
      sym_string,
    STATE(65), 2,
      sym_macro,
      sym_substitution,
  [2253] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(294), 2,
      aux_sym__space_token1,
      aux_sym_nbt_identifier_token1,
    STATE(66), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(296), 8,
      sym_backslash,
      anon_sym_EQ,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
      anon_sym_RBRACK,
  [2278] = 12,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(43), 1,
      anon_sym_DQUOTE,
    ACTIONS(45), 1,
      anon_sym_SQUOTE,
    ACTIONS(342), 1,
      sym_backslash,
    ACTIONS(344), 1,
      aux_sym_word_token1,
    ACTIONS(346), 1,
      anon_sym_LBRACK,
    STATE(82), 1,
      sym_item_compound,
    STATE(137), 1,
      sym_string,
    STATE(201), 1,
      sym_word,
    ACTIONS(340), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(67), 2,
      sym_macro,
      sym_substitution,
  [2317] = 12,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(130), 1,
      aux_sym_word_token1,
    ACTIONS(132), 1,
      anon_sym_DQUOTE,
    ACTIONS(134), 1,
      anon_sym_SQUOTE,
    ACTIONS(340), 1,
      sym_query_identifier,
    ACTIONS(348), 1,
      anon_sym_LBRACK,
    STATE(77), 1,
      sym_item_compound,
    STATE(128), 1,
      sym_string,
    STATE(177), 1,
      sym_word,
    ACTIONS(342), 2,
      aux_sym_selector_token2,
      aux_sym_selector_query_token1,
    STATE(68), 2,
      sym_macro,
      sym_substitution,
  [2356] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(350), 2,
      aux_sym__space_token1,
      aux_sym_nbt_identifier_token1,
    STATE(69), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(352), 7,
      sym_backslash,
      anon_sym_EQ,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACK,
  [2380] = 12,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(252), 1,
      anon_sym_DQUOTE,
    ACTIONS(254), 1,
      anon_sym_SQUOTE,
    ACTIONS(354), 1,
      aux_sym_selector_query_token1,
    ACTIONS(356), 1,
      sym_resource,
    ACTIONS(358), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(360), 1,
      anon_sym_RBRACK,
    STATE(64), 1,
      sym_nbt_identifier,
    STATE(69), 1,
      sym_string,
    STATE(75), 1,
      aux_sym_item_compound_repeat1,
    STATE(70), 2,
      sym_macro,
      sym_substitution,
  [2418] = 11,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(362), 1,
      aux_sym_selector_query_token1,
    ACTIONS(365), 1,
      sym_resource,
    ACTIONS(368), 1,
      anon_sym_DQUOTE,
    ACTIONS(371), 1,
      anon_sym_SQUOTE,
    ACTIONS(374), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(377), 1,
      anon_sym_RBRACK,
    STATE(64), 1,
      sym_nbt_identifier,
    STATE(69), 1,
      sym_string,
    STATE(71), 3,
      sym_macro,
      sym_substitution,
      aux_sym_item_compound_repeat1,
  [2454] = 12,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(252), 1,
      anon_sym_DQUOTE,
    ACTIONS(254), 1,
      anon_sym_SQUOTE,
    ACTIONS(354), 1,
      aux_sym_selector_query_token1,
    ACTIONS(356), 1,
      sym_resource,
    ACTIONS(358), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(379), 1,
      anon_sym_RBRACK,
    STATE(64), 1,
      sym_nbt_identifier,
    STATE(69), 1,
      sym_string,
    STATE(74), 1,
      aux_sym_item_compound_repeat1,
    STATE(72), 2,
      sym_macro,
      sym_substitution,
  [2492] = 11,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(381), 1,
      aux_sym_selector_query_token1,
    ACTIONS(384), 1,
      sym_resource,
    ACTIONS(387), 1,
      anon_sym_DQUOTE,
    ACTIONS(390), 1,
      anon_sym_SQUOTE,
    ACTIONS(393), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(396), 1,
      anon_sym_RBRACE,
    STATE(125), 1,
      sym_nbt_identifier,
    STATE(156), 1,
      sym_string,
    STATE(73), 3,
      sym_macro,
      sym_substitution,
      aux_sym_selector_compound_repeat1,
  [2528] = 12,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(252), 1,
      anon_sym_DQUOTE,
    ACTIONS(254), 1,
      anon_sym_SQUOTE,
    ACTIONS(354), 1,
      aux_sym_selector_query_token1,
    ACTIONS(356), 1,
      sym_resource,
    ACTIONS(358), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(398), 1,
      anon_sym_RBRACK,
    STATE(64), 1,
      sym_nbt_identifier,
    STATE(69), 1,
      sym_string,
    STATE(71), 1,
      aux_sym_item_compound_repeat1,
    STATE(74), 2,
      sym_macro,
      sym_substitution,
  [2566] = 12,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(252), 1,
      anon_sym_DQUOTE,
    ACTIONS(254), 1,
      anon_sym_SQUOTE,
    ACTIONS(354), 1,
      aux_sym_selector_query_token1,
    ACTIONS(356), 1,
      sym_resource,
    ACTIONS(358), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(400), 1,
      anon_sym_RBRACK,
    STATE(64), 1,
      sym_nbt_identifier,
    STATE(69), 1,
      sym_string,
    STATE(71), 1,
      aux_sym_item_compound_repeat1,
    STATE(75), 2,
      sym_macro,
      sym_substitution,
  [2604] = 12,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(43), 1,
      anon_sym_DQUOTE,
    ACTIONS(45), 1,
      anon_sym_SQUOTE,
    ACTIONS(330), 1,
      sym_resource,
    ACTIONS(336), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(402), 1,
      aux_sym_selector_query_token1,
    ACTIONS(404), 1,
      anon_sym_RBRACE,
    STATE(73), 1,
      aux_sym_selector_compound_repeat1,
    STATE(125), 1,
      sym_nbt_identifier,
    STATE(156), 1,
      sym_string,
    STATE(76), 2,
      sym_macro,
      sym_substitution,
  [2642] = 10,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(130), 1,
      aux_sym_word_token1,
    ACTIONS(132), 1,
      anon_sym_DQUOTE,
    ACTIONS(134), 1,
      anon_sym_SQUOTE,
    ACTIONS(408), 1,
      sym_query_identifier,
    STATE(124), 1,
      sym_string,
    STATE(188), 1,
      sym_word,
    ACTIONS(406), 2,
      aux_sym_selector_token2,
      aux_sym_selector_query_token1,
    STATE(77), 2,
      sym_macro,
      sym_substitution,
  [2675] = 11,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(332), 1,
      anon_sym_DQUOTE,
    ACTIONS(334), 1,
      anon_sym_SQUOTE,
    ACTIONS(410), 1,
      aux_sym_selector_query_token1,
    ACTIONS(412), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(414), 1,
      anon_sym_RBRACE,
    STATE(79), 1,
      aux_sym_nbt_compound_repeat1,
    STATE(134), 1,
      sym_nbt_identifier,
    STATE(156), 1,
      sym_string,
    STATE(78), 2,
      sym_macro,
      sym_substitution,
  [2710] = 10,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(416), 1,
      aux_sym_selector_query_token1,
    ACTIONS(419), 1,
      anon_sym_DQUOTE,
    ACTIONS(422), 1,
      anon_sym_SQUOTE,
    ACTIONS(425), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(428), 1,
      anon_sym_RBRACE,
    STATE(134), 1,
      sym_nbt_identifier,
    STATE(156), 1,
      sym_string,
    STATE(79), 3,
      sym_macro,
      sym_substitution,
      aux_sym_nbt_compound_repeat1,
  [2743] = 11,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(332), 1,
      anon_sym_DQUOTE,
    ACTIONS(334), 1,
      anon_sym_SQUOTE,
    ACTIONS(410), 1,
      aux_sym_selector_query_token1,
    ACTIONS(412), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(430), 1,
      anon_sym_RBRACE,
    STATE(81), 1,
      aux_sym_nbt_compound_repeat1,
    STATE(134), 1,
      sym_nbt_identifier,
    STATE(156), 1,
      sym_string,
    STATE(80), 2,
      sym_macro,
      sym_substitution,
  [2778] = 11,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(332), 1,
      anon_sym_DQUOTE,
    ACTIONS(334), 1,
      anon_sym_SQUOTE,
    ACTIONS(410), 1,
      aux_sym_selector_query_token1,
    ACTIONS(412), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(432), 1,
      anon_sym_RBRACE,
    STATE(79), 1,
      aux_sym_nbt_compound_repeat1,
    STATE(134), 1,
      sym_nbt_identifier,
    STATE(156), 1,
      sym_string,
    STATE(81), 2,
      sym_macro,
      sym_substitution,
  [2813] = 10,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(43), 1,
      anon_sym_DQUOTE,
    ACTIONS(45), 1,
      anon_sym_SQUOTE,
    ACTIONS(344), 1,
      aux_sym_word_token1,
    ACTIONS(406), 1,
      sym_backslash,
    STATE(130), 1,
      sym_string,
    STATE(199), 1,
      sym_word,
    ACTIONS(408), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(82), 2,
      sym_macro,
      sym_substitution,
  [2846] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(278), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(434), 1,
      aux_sym_scale_token2,
    STATE(83), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(280), 6,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
      anon_sym_RBRACK,
  [2871] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(272), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(436), 1,
      aux_sym_scale_token2,
    STATE(84), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(274), 6,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
      anon_sym_RBRACK,
  [2896] = 11,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(332), 1,
      anon_sym_DQUOTE,
    ACTIONS(334), 1,
      anon_sym_SQUOTE,
    ACTIONS(410), 1,
      aux_sym_selector_query_token1,
    ACTIONS(412), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(438), 1,
      anon_sym_RBRACE,
    STATE(79), 1,
      aux_sym_nbt_compound_repeat1,
    STATE(134), 1,
      sym_nbt_identifier,
    STATE(156), 1,
      sym_string,
    STATE(85), 2,
      sym_macro,
      sym_substitution,
  [2931] = 11,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(332), 1,
      anon_sym_DQUOTE,
    ACTIONS(334), 1,
      anon_sym_SQUOTE,
    ACTIONS(410), 1,
      aux_sym_selector_query_token1,
    ACTIONS(412), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(440), 1,
      anon_sym_RBRACE,
    STATE(79), 1,
      aux_sym_nbt_compound_repeat1,
    STATE(134), 1,
      sym_nbt_identifier,
    STATE(156), 1,
      sym_string,
    STATE(86), 2,
      sym_macro,
      sym_substitution,
  [2966] = 11,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(332), 1,
      anon_sym_DQUOTE,
    ACTIONS(334), 1,
      anon_sym_SQUOTE,
    ACTIONS(410), 1,
      aux_sym_selector_query_token1,
    ACTIONS(412), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(442), 1,
      anon_sym_RBRACE,
    STATE(85), 1,
      aux_sym_nbt_compound_repeat1,
    STATE(134), 1,
      sym_nbt_identifier,
    STATE(156), 1,
      sym_string,
    STATE(87), 2,
      sym_macro,
      sym_substitution,
  [3001] = 11,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(332), 1,
      anon_sym_DQUOTE,
    ACTIONS(334), 1,
      anon_sym_SQUOTE,
    ACTIONS(410), 1,
      aux_sym_selector_query_token1,
    ACTIONS(412), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(444), 1,
      anon_sym_RBRACE,
    STATE(78), 1,
      aux_sym_nbt_compound_repeat1,
    STATE(134), 1,
      sym_nbt_identifier,
    STATE(156), 1,
      sym_string,
    STATE(88), 2,
      sym_macro,
      sym_substitution,
  [3036] = 11,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(332), 1,
      anon_sym_DQUOTE,
    ACTIONS(334), 1,
      anon_sym_SQUOTE,
    ACTIONS(410), 1,
      aux_sym_selector_query_token1,
    ACTIONS(412), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(446), 1,
      anon_sym_RBRACE,
    STATE(86), 1,
      aux_sym_nbt_compound_repeat1,
    STATE(134), 1,
      sym_nbt_identifier,
    STATE(156), 1,
      sym_string,
    STATE(89), 2,
      sym_macro,
      sym_substitution,
  [3071] = 11,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(332), 1,
      anon_sym_DQUOTE,
    ACTIONS(334), 1,
      anon_sym_SQUOTE,
    ACTIONS(410), 1,
      aux_sym_selector_query_token1,
    ACTIONS(412), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(448), 1,
      anon_sym_RBRACE,
    STATE(79), 1,
      aux_sym_nbt_compound_repeat1,
    STATE(134), 1,
      sym_nbt_identifier,
    STATE(156), 1,
      sym_string,
    STATE(90), 2,
      sym_macro,
      sym_substitution,
  [3106] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(302), 1,
      aux_sym_nbt_identifier_token1,
    STATE(91), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(304), 6,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
      anon_sym_RBRACK,
  [3128] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(272), 1,
      aux_sym_nbt_identifier_token1,
    STATE(92), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(274), 6,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
      anon_sym_RBRACK,
  [3150] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(306), 1,
      aux_sym_nbt_identifier_token1,
    STATE(93), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(308), 6,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
      anon_sym_RBRACK,
  [3172] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(298), 1,
      aux_sym_nbt_identifier_token1,
    STATE(94), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(300), 6,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
      anon_sym_RBRACK,
  [3194] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(310), 1,
      aux_sym_nbt_identifier_token1,
    STATE(95), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(312), 6,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
      anon_sym_RBRACK,
  [3216] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(314), 1,
      aux_sym_nbt_identifier_token1,
    STATE(96), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(316), 6,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
      anon_sym_RBRACK,
  [3238] = 8,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(452), 1,
      sym_backslash,
    ACTIONS(454), 1,
      anon_sym_DASH,
    ACTIONS(456), 1,
      aux_sym_scale_token1,
    STATE(178), 1,
      sym_scale,
    ACTIONS(450), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(97), 2,
      sym_macro,
      sym_substitution,
  [3265] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(460), 1,
      aux_sym_nbt_identifier_token1,
    STATE(98), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(458), 5,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACK,
  [3286] = 9,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(462), 1,
      aux_sym__space_token1,
    ACTIONS(464), 1,
      sym_backslash,
    ACTIONS(466), 1,
      anon_sym_EQ,
    ACTIONS(468), 1,
      anon_sym_EQ_BANG,
    STATE(47), 1,
      sym__space,
    STATE(102), 1,
      aux_sym_selector_query_repeat1,
    STATE(99), 2,
      sym_macro,
      sym_substitution,
  [3315] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(473), 1,
      sym_resource,
    ACTIONS(475), 1,
      aux_sym_nbt_identifier_token1,
    STATE(100), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(470), 4,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
  [3338] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(480), 2,
      sym_query_identifier,
      aux_sym_word_token1,
    STATE(101), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(478), 4,
      aux_sym_selector_token2,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
  [3359] = 8,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(150), 1,
      anon_sym_EQ_BANG,
    ACTIONS(152), 1,
      anon_sym_EQ,
    ACTIONS(482), 1,
      aux_sym__space_token1,
    ACTIONS(485), 1,
      sym_backslash,
    STATE(47), 1,
      sym__space,
    STATE(102), 3,
      sym_macro,
      sym_substitution,
      aux_sym_selector_query_repeat1,
  [3386] = 9,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(230), 1,
      aux_sym__space_token1,
    ACTIONS(232), 1,
      sym_backslash,
    ACTIONS(488), 1,
      anon_sym_EQ,
    ACTIONS(490), 1,
      anon_sym_COLON,
    STATE(120), 1,
      aux_sym_selector_query_repeat1,
    STATE(157), 1,
      sym__space,
    STATE(103), 2,
      sym_macro,
      sym_substitution,
  [3415] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(278), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(492), 1,
      aux_sym_scale_token2,
    STATE(104), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(280), 4,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
  [3438] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(272), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(494), 1,
      aux_sym_scale_token2,
    STATE(105), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(274), 4,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
  [3461] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(290), 1,
      aux_sym_nbt_identifier_token1,
    STATE(106), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(292), 5,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
  [3482] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(498), 1,
      aux_sym_nbt_identifier_token1,
    STATE(107), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(496), 5,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
  [3503] = 8,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(120), 1,
      anon_sym_DASH,
    ACTIONS(122), 1,
      aux_sym_scale_token1,
    ACTIONS(450), 1,
      sym_query_identifier,
    STATE(167), 1,
      sym_scale,
    ACTIONS(452), 2,
      aux_sym_selector_token2,
      aux_sym_selector_query_token1,
    STATE(108), 2,
      sym_macro,
      sym_substitution,
  [3530] = 8,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(120), 1,
      anon_sym_DASH,
    ACTIONS(122), 1,
      aux_sym_scale_token1,
    ACTIONS(502), 1,
      sym_query_identifier,
    STATE(185), 1,
      sym_scale,
    ACTIONS(500), 2,
      aux_sym_selector_token2,
      aux_sym_selector_query_token1,
    STATE(109), 2,
      sym_macro,
      sym_substitution,
  [3557] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(326), 1,
      aux_sym_nbt_identifier_token1,
    STATE(110), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(324), 5,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACK,
  [3578] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(506), 1,
      aux_sym_nbt_identifier_token1,
    STATE(111), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(504), 5,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
  [3599] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(508), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(112), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(510), 4,
      sym_backslash,
      aux_sym_word_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
  [3620] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(514), 1,
      aux_sym_nbt_identifier_token1,
    STATE(113), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(512), 5,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
  [3641] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(518), 1,
      aux_sym_nbt_identifier_token1,
    STATE(114), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(516), 5,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACK,
  [3662] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(522), 1,
      aux_sym_nbt_identifier_token1,
    STATE(115), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(520), 5,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACK,
  [3683] = 8,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(454), 1,
      anon_sym_DASH,
    ACTIONS(456), 1,
      aux_sym_scale_token1,
    ACTIONS(500), 1,
      sym_backslash,
    STATE(189), 1,
      sym_scale,
    ACTIONS(502), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(116), 2,
      sym_macro,
      sym_substitution,
  [3710] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(480), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(117), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(478), 4,
      sym_backslash,
      aux_sym_word_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
  [3731] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(266), 1,
      aux_sym__space_token1,
    ACTIONS(269), 1,
      sym_backslash,
    STATE(157), 1,
      sym__space,
    ACTIONS(150), 2,
      anon_sym_EQ,
      anon_sym_COLON,
    STATE(118), 3,
      sym_macro,
      sym_substitution,
      aux_sym_selector_query_repeat1,
  [3756] = 9,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(462), 1,
      aux_sym__space_token1,
    ACTIONS(464), 1,
      sym_backslash,
    ACTIONS(524), 1,
      anon_sym_EQ,
    ACTIONS(526), 1,
      anon_sym_EQ_BANG,
    STATE(47), 1,
      sym__space,
    STATE(99), 1,
      aux_sym_selector_query_repeat1,
    STATE(119), 2,
      sym_macro,
      sym_substitution,
  [3785] = 9,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(230), 1,
      aux_sym__space_token1,
    ACTIONS(232), 1,
      sym_backslash,
    ACTIONS(528), 1,
      anon_sym_EQ,
    ACTIONS(530), 1,
      anon_sym_COLON,
    STATE(118), 1,
      aux_sym_selector_query_repeat1,
    STATE(157), 1,
      sym__space,
    STATE(120), 2,
      sym_macro,
      sym_substitution,
  [3814] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(532), 1,
      aux_sym_nbt_identifier_token1,
    STATE(121), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(473), 5,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
  [3835] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(508), 2,
      sym_query_identifier,
      aux_sym_word_token1,
    STATE(122), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(510), 4,
      aux_sym_selector_token2,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
  [3856] = 8,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(318), 1,
      aux_sym__space_token1,
    ACTIONS(320), 1,
      sym_backslash,
    ACTIONS(528), 1,
      anon_sym_EQ,
    STATE(25), 1,
      sym__space,
    STATE(136), 1,
      aux_sym_selector_query_repeat1,
    STATE(123), 2,
      sym_macro,
      sym_substitution,
  [3882] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(130), 1,
      aux_sym_word_token1,
    ACTIONS(536), 1,
      sym_query_identifier,
    STATE(168), 1,
      sym_word,
    ACTIONS(534), 2,
      aux_sym_selector_token2,
      aux_sym_selector_query_token1,
    STATE(124), 2,
      sym_macro,
      sym_substitution,
  [3906] = 8,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(318), 1,
      aux_sym__space_token1,
    ACTIONS(320), 1,
      sym_backslash,
    ACTIONS(488), 1,
      anon_sym_EQ,
    STATE(25), 1,
      sym__space,
    STATE(123), 1,
      aux_sym_selector_query_repeat1,
    STATE(125), 2,
      sym_macro,
      sym_substitution,
  [3932] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(126), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(308), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [3950] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(127), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(312), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [3968] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(130), 1,
      aux_sym_word_token1,
    ACTIONS(408), 1,
      sym_query_identifier,
    STATE(188), 1,
      sym_word,
    ACTIONS(406), 2,
      aux_sym_selector_token2,
      aux_sym_selector_query_token1,
    STATE(128), 2,
      sym_macro,
      sym_substitution,
  [3992] = 8,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(318), 1,
      aux_sym__space_token1,
    ACTIONS(320), 1,
      sym_backslash,
    ACTIONS(538), 1,
      sym__newline,
    STATE(2), 1,
      sym__space,
    STATE(138), 1,
      aux_sym__generic_command_repeat1,
    STATE(129), 2,
      sym_macro,
      sym_substitution,
  [4018] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(344), 1,
      aux_sym_word_token1,
    ACTIONS(534), 1,
      sym_backslash,
    STATE(190), 1,
      sym_word,
    ACTIONS(536), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(130), 2,
      sym_macro,
      sym_substitution,
  [4042] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(131), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(300), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [4060] = 8,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(318), 1,
      aux_sym__space_token1,
    ACTIONS(320), 1,
      sym_backslash,
    ACTIONS(540), 1,
      anon_sym_EQ,
    STATE(25), 1,
      sym__space,
    STATE(136), 1,
      aux_sym_selector_query_repeat1,
    STATE(132), 2,
      sym_macro,
      sym_substitution,
  [4086] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(133), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(316), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [4104] = 8,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(230), 1,
      aux_sym__space_token1,
    ACTIONS(232), 1,
      sym_backslash,
    ACTIONS(490), 1,
      anon_sym_COLON,
    STATE(47), 1,
      sym__space,
    STATE(148), 1,
      aux_sym_selector_query_repeat1,
    STATE(134), 2,
      sym_macro,
      sym_substitution,
  [4130] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(135), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(542), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [4148] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(150), 1,
      anon_sym_EQ,
    ACTIONS(544), 1,
      aux_sym__space_token1,
    ACTIONS(547), 1,
      sym_backslash,
    STATE(25), 1,
      sym__space,
    STATE(136), 3,
      sym_macro,
      sym_substitution,
      aux_sym_selector_query_repeat1,
  [4172] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(344), 1,
      aux_sym_word_token1,
    ACTIONS(406), 1,
      sym_backslash,
    STATE(199), 1,
      sym_word,
    ACTIONS(408), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(137), 2,
      sym_macro,
      sym_substitution,
  [4196] = 8,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(318), 1,
      aux_sym__space_token1,
    ACTIONS(320), 1,
      sym_backslash,
    ACTIONS(550), 1,
      sym__newline,
    STATE(2), 1,
      sym__space,
    STATE(141), 1,
      aux_sym__generic_command_repeat1,
    STATE(138), 2,
      sym_macro,
      sym_substitution,
  [4222] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(139), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(292), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [4240] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(140), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(552), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [4258] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(554), 1,
      sym__newline,
    ACTIONS(556), 1,
      aux_sym__space_token1,
    ACTIONS(559), 1,
      sym_backslash,
    STATE(2), 1,
      sym__space,
    STATE(141), 3,
      sym_macro,
      sym_substitution,
      aux_sym__generic_command_repeat1,
  [4282] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(142), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(296), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [4300] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(143), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(562), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [4318] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(144), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(564), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [4336] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(145), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(274), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [4354] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(294), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(146), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(296), 3,
      sym_backslash,
      anon_sym_EQ,
      aux_sym_word_token1,
  [4374] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(147), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(304), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [4392] = 8,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(230), 1,
      aux_sym__space_token1,
    ACTIONS(232), 1,
      sym_backslash,
    ACTIONS(530), 1,
      anon_sym_COLON,
    STATE(47), 1,
      sym__space,
    STATE(49), 1,
      aux_sym_selector_query_repeat1,
    STATE(148), 2,
      sym_macro,
      sym_substitution,
  [4418] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(462), 1,
      aux_sym__space_token1,
    ACTIONS(464), 1,
      sym_backslash,
    STATE(16), 1,
      sym__space,
    STATE(151), 1,
      aux_sym__execute_command_repeat1,
    STATE(149), 2,
      sym_macro,
      sym_substitution,
  [4441] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(566), 1,
      aux_sym_selector_token1,
    STATE(150), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(568), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4460] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(570), 1,
      aux_sym__space_token1,
    ACTIONS(573), 1,
      sym_backslash,
    STATE(17), 1,
      sym__space,
    STATE(151), 3,
      sym_macro,
      sym_substitution,
      aux_sym__execute_command_repeat1,
  [4481] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(576), 1,
      aux_sym_selector_token2,
    ACTIONS(578), 1,
      sym_query_identifier,
    STATE(208), 1,
      sym_selector_query,
    STATE(152), 3,
      sym_macro,
      sym_substitution,
      aux_sym_selector_repeat1,
  [4502] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(581), 1,
      aux_sym_selector_token2,
    ACTIONS(583), 1,
      sym_query_identifier,
    STATE(162), 1,
      aux_sym_selector_repeat1,
    STATE(208), 1,
      sym_selector_query,
    STATE(153), 2,
      sym_macro,
      sym_substitution,
  [4525] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(274), 1,
      sym_backslash,
    ACTIONS(585), 1,
      aux_sym_scale_token2,
    ACTIONS(272), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(154), 2,
      sym_macro,
      sym_substitution,
  [4546] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(462), 1,
      aux_sym__space_token1,
    ACTIONS(464), 1,
      sym_backslash,
    STATE(15), 1,
      sym__space,
    STATE(149), 1,
      aux_sym__execute_command_repeat1,
    STATE(155), 2,
      sym_macro,
      sym_substitution,
  [4569] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(350), 1,
      aux_sym__space_token1,
    STATE(156), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(352), 3,
      sym_backslash,
      anon_sym_EQ,
      anon_sym_COLON,
  [4588] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(154), 1,
      aux_sym__space_token1,
    STATE(157), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(156), 3,
      sym_backslash,
      anon_sym_EQ,
      anon_sym_COLON,
  [4607] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(568), 1,
      sym_backslash,
    ACTIONS(589), 1,
      aux_sym_selector_token1,
    ACTIONS(587), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(158), 2,
      sym_macro,
      sym_substitution,
  [4628] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(591), 1,
      aux_sym_scale_token2,
    STATE(159), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(280), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4647] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(593), 1,
      aux_sym_scale_token2,
    STATE(160), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(274), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4666] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(294), 2,
      sym_query_identifier,
      aux_sym_word_token1,
    ACTIONS(296), 2,
      aux_sym_selector_token2,
      aux_sym_selector_query_token1,
    STATE(161), 2,
      sym_macro,
      sym_substitution,
  [4685] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(583), 1,
      sym_query_identifier,
    ACTIONS(595), 1,
      aux_sym_selector_token2,
    STATE(152), 1,
      aux_sym_selector_repeat1,
    STATE(208), 1,
      sym_selector_query,
    STATE(162), 2,
      sym_macro,
      sym_substitution,
  [4708] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(294), 1,
      aux_sym__space_token1,
    STATE(163), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(296), 3,
      sym_backslash,
      anon_sym_EQ,
      anon_sym_COLON,
  [4727] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(583), 1,
      sym_query_identifier,
    ACTIONS(597), 1,
      aux_sym_selector_token2,
    STATE(166), 1,
      aux_sym_selector_repeat1,
    STATE(208), 1,
      sym_selector_query,
    STATE(164), 2,
      sym_macro,
      sym_substitution,
  [4750] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(280), 1,
      sym_backslash,
    ACTIONS(599), 1,
      aux_sym_scale_token2,
    ACTIONS(278), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(165), 2,
      sym_macro,
      sym_substitution,
  [4771] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(583), 1,
      sym_query_identifier,
    ACTIONS(601), 1,
      aux_sym_selector_token2,
    STATE(152), 1,
      aux_sym_selector_repeat1,
    STATE(208), 1,
      sym_selector_query,
    STATE(166), 2,
      sym_macro,
      sym_substitution,
  [4794] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(167), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(603), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4810] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(168), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(605), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4826] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(609), 1,
      aux_sym_selector_query_token1,
    ACTIONS(607), 2,
      aux_sym_selector_token2,
      sym_query_identifier,
    STATE(169), 2,
      sym_macro,
      sym_substitution,
  [4844] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(611), 1,
      sym_backslash,
    ACTIONS(554), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(170), 2,
      sym_macro,
      sym_substitution,
  [4862] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(615), 1,
      sym_backslash,
    ACTIONS(613), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(171), 2,
      sym_macro,
      sym_substitution,
  [4880] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(304), 1,
      sym_backslash,
    ACTIONS(302), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(172), 2,
      sym_macro,
      sym_substitution,
  [4898] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(158), 1,
      aux_sym__space_token1,
    ACTIONS(160), 1,
      sym_backslash,
    STATE(13), 1,
      sym__space,
    STATE(173), 2,
      sym_macro,
      sym_substitution,
  [4918] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(316), 1,
      sym_backslash,
    ACTIONS(314), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(174), 2,
      sym_macro,
      sym_substitution,
  [4936] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(175), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(617), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4952] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(176), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(619), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4968] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(177), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(406), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4984] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(603), 1,
      sym_backslash,
    ACTIONS(622), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(178), 2,
      sym_macro,
      sym_substitution,
  [5002] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(179), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(274), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [5018] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(180), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(308), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [5034] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(181), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(312), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [5050] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(308), 1,
      sym_backslash,
    ACTIONS(306), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(182), 2,
      sym_macro,
      sym_substitution,
  [5068] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(183), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(300), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [5084] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(184), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(316), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [5100] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(185), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(624), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [5116] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(158), 1,
      aux_sym__space_token1,
    ACTIONS(160), 1,
      sym_backslash,
    STATE(12), 1,
      sym__space,
    STATE(186), 2,
      sym_macro,
      sym_substitution,
  [5136] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(617), 1,
      sym_backslash,
    ACTIONS(626), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(187), 2,
      sym_macro,
      sym_substitution,
  [5154] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(188), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(534), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [5170] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(624), 1,
      sym_backslash,
    ACTIONS(628), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(189), 2,
      sym_macro,
      sym_substitution,
  [5188] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(605), 1,
      sym_backslash,
    ACTIONS(630), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(190), 2,
      sym_macro,
      sym_substitution,
  [5206] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(191), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(632), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [5222] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(312), 1,
      sym_backslash,
    ACTIONS(310), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(192), 2,
      sym_macro,
      sym_substitution,
  [5240] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(636), 1,
      sym_backslash,
    ACTIONS(634), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(193), 2,
      sym_macro,
      sym_substitution,
  [5258] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(640), 1,
      aux_sym_selector_query_token1,
    ACTIONS(638), 2,
      aux_sym_selector_token2,
      sym_query_identifier,
    STATE(194), 2,
      sym_macro,
      sym_substitution,
  [5276] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(274), 1,
      sym_backslash,
    ACTIONS(272), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(195), 2,
      sym_macro,
      sym_substitution,
  [5294] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(196), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(615), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [5310] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(644), 1,
      sym_backslash,
    ACTIONS(642), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(197), 2,
      sym_macro,
      sym_substitution,
  [5328] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(648), 1,
      sym_backslash,
    ACTIONS(646), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(198), 2,
      sym_macro,
      sym_substitution,
  [5346] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(534), 1,
      sym_backslash,
    ACTIONS(536), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(199), 2,
      sym_macro,
      sym_substitution,
  [5364] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(652), 1,
      sym_backslash,
    ACTIONS(650), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(200), 2,
      sym_macro,
      sym_substitution,
  [5382] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(406), 1,
      sym_backslash,
    ACTIONS(408), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(201), 2,
      sym_macro,
      sym_substitution,
  [5400] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(300), 1,
      sym_backslash,
    ACTIONS(298), 2,
      sym__newline,
      aux_sym__space_token1,
    STATE(202), 2,
      sym_macro,
      sym_substitution,
  [5418] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(203), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(644), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [5434] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    STATE(204), 2,
      sym_macro,
      sym_substitution,
    ACTIONS(648), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [5450] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(656), 1,
      aux_sym_selector_query_token1,
    ACTIONS(654), 2,
      aux_sym_selector_token2,
      sym_query_identifier,
    STATE(205), 2,
      sym_macro,
      sym_substitution,
  [5468] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(654), 2,
      aux_sym_selector_token2,
      sym_query_identifier,
    STATE(206), 2,
      sym_macro,
      sym_substitution,
  [5483] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(658), 1,
      aux_sym__space_token1,
    ACTIONS(660), 1,
      sym_backslash,
    STATE(207), 2,
      sym_macro,
      sym_substitution,
  [5500] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(662), 2,
      aux_sym_selector_token2,
      sym_query_identifier,
    STATE(208), 2,
      sym_macro,
      sym_substitution,
  [5515] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(664), 1,
      aux_sym__space_token1,
    ACTIONS(666), 1,
      sym_backslash,
    STATE(209), 2,
      sym_macro,
      sym_substitution,
  [5532] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(668), 1,
      aux_sym__space_token1,
    ACTIONS(670), 1,
      sym_backslash,
    STATE(210), 2,
      sym_macro,
      sym_substitution,
  [5549] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(638), 2,
      aux_sym_selector_token2,
      sym_query_identifier,
    STATE(211), 2,
      sym_macro,
      sym_substitution,
  [5564] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(672), 2,
      aux_sym_selector_token2,
      sym_query_identifier,
    STATE(212), 2,
      sym_macro,
      sym_substitution,
  [5579] = 4,
    ACTIONS(674), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(676), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(678), 1,
      aux_sym_string_token1,
    STATE(213), 2,
      sym_macro,
      sym_substitution,
  [5593] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(680), 1,
      anon_sym_SQUOTE,
    STATE(214), 2,
      sym_macro,
      sym_substitution,
  [5607] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(682), 1,
      sym__newline,
    STATE(215), 2,
      sym_macro,
      sym_substitution,
  [5621] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(684), 1,
      sym__newline,
    STATE(216), 2,
      sym_macro,
      sym_substitution,
  [5635] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(686), 1,
      sym__newline,
    STATE(217), 2,
      sym_macro,
      sym_substitution,
  [5649] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(688), 1,
      aux_sym_scale_token1,
    STATE(218), 2,
      sym_macro,
      sym_substitution,
  [5663] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(690), 1,
      aux_sym_scale_token1,
    STATE(219), 2,
      sym_macro,
      sym_substitution,
  [5677] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(692), 1,
      sym__newline,
    STATE(220), 2,
      sym_macro,
      sym_substitution,
  [5691] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(694), 1,
      anon_sym_DQUOTE,
    STATE(221), 2,
      sym_macro,
      sym_substitution,
  [5705] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(694), 1,
      anon_sym_SQUOTE,
    STATE(222), 2,
      sym_macro,
      sym_substitution,
  [5719] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(696), 1,
      anon_sym_DQUOTE,
    STATE(223), 2,
      sym_macro,
      sym_substitution,
  [5733] = 4,
    ACTIONS(674), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(676), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(698), 1,
      aux_sym_string_token1,
    STATE(224), 2,
      sym_macro,
      sym_substitution,
  [5747] = 4,
    ACTIONS(674), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(676), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(700), 1,
      aux_sym_string_token2,
    STATE(225), 2,
      sym_macro,
      sym_substitution,
  [5761] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(702), 1,
      aux_sym_scale_token1,
    STATE(226), 2,
      sym_macro,
      sym_substitution,
  [5775] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(696), 1,
      anon_sym_SQUOTE,
    STATE(227), 2,
      sym_macro,
      sym_substitution,
  [5789] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(704), 1,
      aux_sym_scale_token1,
    STATE(228), 2,
      sym_macro,
      sym_substitution,
  [5803] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(706), 1,
      anon_sym_DQUOTE,
    STATE(229), 2,
      sym_macro,
      sym_substitution,
  [5817] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(706), 1,
      anon_sym_SQUOTE,
    STATE(230), 2,
      sym_macro,
      sym_substitution,
  [5831] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(680), 1,
      anon_sym_DQUOTE,
    STATE(231), 2,
      sym_macro,
      sym_substitution,
  [5845] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(708), 1,
      anon_sym_RPAREN,
    STATE(232), 2,
      sym_macro,
      sym_substitution,
  [5859] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(710), 1,
      aux_sym_scale_token1,
    STATE(233), 2,
      sym_macro,
      sym_substitution,
  [5873] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(80), 1,
      sym__newline,
    STATE(234), 2,
      sym_macro,
      sym_substitution,
  [5887] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(712), 1,
      anon_sym_DQUOTE,
    STATE(235), 2,
      sym_macro,
      sym_substitution,
  [5901] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(712), 1,
      anon_sym_SQUOTE,
    STATE(236), 2,
      sym_macro,
      sym_substitution,
  [5915] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(714), 1,
      aux_sym_macro_token1,
    STATE(237), 2,
      sym_macro,
      sym_substitution,
  [5929] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(716), 1,
      sym__newline,
    STATE(238), 2,
      sym_macro,
      sym_substitution,
  [5943] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(718), 1,
      anon_sym_DQUOTE,
    STATE(239), 2,
      sym_macro,
      sym_substitution,
  [5957] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(718), 1,
      anon_sym_SQUOTE,
    STATE(240), 2,
      sym_macro,
      sym_substitution,
  [5971] = 4,
    ACTIONS(674), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(676), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(720), 1,
      aux_sym_string_token1,
    STATE(241), 2,
      sym_macro,
      sym_substitution,
  [5985] = 4,
    ACTIONS(674), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(676), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(722), 1,
      aux_sym_string_token2,
    STATE(242), 2,
      sym_macro,
      sym_substitution,
  [5999] = 4,
    ACTIONS(674), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(676), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(724), 1,
      aux_sym_string_token1,
    STATE(243), 2,
      sym_macro,
      sym_substitution,
  [6013] = 4,
    ACTIONS(674), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(676), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(726), 1,
      aux_sym_string_token2,
    STATE(244), 2,
      sym_macro,
      sym_substitution,
  [6027] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(728), 1,
      aux_sym_substitution_token1,
    STATE(245), 2,
      sym_macro,
      sym_substitution,
  [6041] = 4,
    ACTIONS(674), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(676), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(730), 1,
      aux_sym_string_token2,
    STATE(246), 2,
      sym_macro,
      sym_substitution,
  [6055] = 4,
    ACTIONS(674), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(676), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(732), 1,
      aux_sym_string_token1,
    STATE(247), 2,
      sym_macro,
      sym_substitution,
  [6069] = 4,
    ACTIONS(674), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(676), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(734), 1,
      aux_sym_string_token2,
    STATE(248), 2,
      sym_macro,
      sym_substitution,
  [6083] = 4,
    ACTIONS(674), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(676), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(736), 1,
      aux_sym_string_token1,
    STATE(249), 2,
      sym_macro,
      sym_substitution,
  [6097] = 4,
    ACTIONS(674), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(676), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(738), 1,
      aux_sym_string_token2,
    STATE(250), 2,
      sym_macro,
      sym_substitution,
  [6111] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(5), 1,
      anon_sym_PERCENT_LBRACK,
    ACTIONS(740), 1,
      ts_builtin_sym_end,
    STATE(251), 2,
      sym_macro,
      sym_substitution,
  [6125] = 1,
    ACTIONS(742), 1,
      ts_builtin_sym_end,
  [6129] = 1,
    ACTIONS(744), 1,
      ts_builtin_sym_end,
};

static const uint32_t ts_small_parse_table_map[] = {
  [SMALL_STATE(20)] = 0,
  [SMALL_STATE(21)] = 96,
  [SMALL_STATE(22)] = 192,
  [SMALL_STATE(23)] = 288,
  [SMALL_STATE(24)] = 384,
  [SMALL_STATE(25)] = 441,
  [SMALL_STATE(26)] = 491,
  [SMALL_STATE(27)] = 540,
  [SMALL_STATE(28)] = 598,
  [SMALL_STATE(29)] = 656,
  [SMALL_STATE(30)] = 714,
  [SMALL_STATE(31)] = 772,
  [SMALL_STATE(32)] = 828,
  [SMALL_STATE(33)] = 886,
  [SMALL_STATE(34)] = 944,
  [SMALL_STATE(35)] = 1002,
  [SMALL_STATE(36)] = 1060,
  [SMALL_STATE(37)] = 1118,
  [SMALL_STATE(38)] = 1176,
  [SMALL_STATE(39)] = 1230,
  [SMALL_STATE(40)] = 1284,
  [SMALL_STATE(41)] = 1338,
  [SMALL_STATE(42)] = 1392,
  [SMALL_STATE(43)] = 1446,
  [SMALL_STATE(44)] = 1500,
  [SMALL_STATE(45)] = 1554,
  [SMALL_STATE(46)] = 1608,
  [SMALL_STATE(47)] = 1652,
  [SMALL_STATE(48)] = 1680,
  [SMALL_STATE(49)] = 1724,
  [SMALL_STATE(50)] = 1758,
  [SMALL_STATE(51)] = 1802,
  [SMALL_STATE(52)] = 1832,
  [SMALL_STATE(53)] = 1862,
  [SMALL_STATE(54)] = 1906,
  [SMALL_STATE(55)] = 1933,
  [SMALL_STATE(56)] = 1960,
  [SMALL_STATE(57)] = 1987,
  [SMALL_STATE(58)] = 2014,
  [SMALL_STATE(59)] = 2041,
  [SMALL_STATE(60)] = 2068,
  [SMALL_STATE(61)] = 2095,
  [SMALL_STATE(62)] = 2122,
  [SMALL_STATE(63)] = 2149,
  [SMALL_STATE(64)] = 2176,
  [SMALL_STATE(65)] = 2212,
  [SMALL_STATE(66)] = 2253,
  [SMALL_STATE(67)] = 2278,
  [SMALL_STATE(68)] = 2317,
  [SMALL_STATE(69)] = 2356,
  [SMALL_STATE(70)] = 2380,
  [SMALL_STATE(71)] = 2418,
  [SMALL_STATE(72)] = 2454,
  [SMALL_STATE(73)] = 2492,
  [SMALL_STATE(74)] = 2528,
  [SMALL_STATE(75)] = 2566,
  [SMALL_STATE(76)] = 2604,
  [SMALL_STATE(77)] = 2642,
  [SMALL_STATE(78)] = 2675,
  [SMALL_STATE(79)] = 2710,
  [SMALL_STATE(80)] = 2743,
  [SMALL_STATE(81)] = 2778,
  [SMALL_STATE(82)] = 2813,
  [SMALL_STATE(83)] = 2846,
  [SMALL_STATE(84)] = 2871,
  [SMALL_STATE(85)] = 2896,
  [SMALL_STATE(86)] = 2931,
  [SMALL_STATE(87)] = 2966,
  [SMALL_STATE(88)] = 3001,
  [SMALL_STATE(89)] = 3036,
  [SMALL_STATE(90)] = 3071,
  [SMALL_STATE(91)] = 3106,
  [SMALL_STATE(92)] = 3128,
  [SMALL_STATE(93)] = 3150,
  [SMALL_STATE(94)] = 3172,
  [SMALL_STATE(95)] = 3194,
  [SMALL_STATE(96)] = 3216,
  [SMALL_STATE(97)] = 3238,
  [SMALL_STATE(98)] = 3265,
  [SMALL_STATE(99)] = 3286,
  [SMALL_STATE(100)] = 3315,
  [SMALL_STATE(101)] = 3338,
  [SMALL_STATE(102)] = 3359,
  [SMALL_STATE(103)] = 3386,
  [SMALL_STATE(104)] = 3415,
  [SMALL_STATE(105)] = 3438,
  [SMALL_STATE(106)] = 3461,
  [SMALL_STATE(107)] = 3482,
  [SMALL_STATE(108)] = 3503,
  [SMALL_STATE(109)] = 3530,
  [SMALL_STATE(110)] = 3557,
  [SMALL_STATE(111)] = 3578,
  [SMALL_STATE(112)] = 3599,
  [SMALL_STATE(113)] = 3620,
  [SMALL_STATE(114)] = 3641,
  [SMALL_STATE(115)] = 3662,
  [SMALL_STATE(116)] = 3683,
  [SMALL_STATE(117)] = 3710,
  [SMALL_STATE(118)] = 3731,
  [SMALL_STATE(119)] = 3756,
  [SMALL_STATE(120)] = 3785,
  [SMALL_STATE(121)] = 3814,
  [SMALL_STATE(122)] = 3835,
  [SMALL_STATE(123)] = 3856,
  [SMALL_STATE(124)] = 3882,
  [SMALL_STATE(125)] = 3906,
  [SMALL_STATE(126)] = 3932,
  [SMALL_STATE(127)] = 3950,
  [SMALL_STATE(128)] = 3968,
  [SMALL_STATE(129)] = 3992,
  [SMALL_STATE(130)] = 4018,
  [SMALL_STATE(131)] = 4042,
  [SMALL_STATE(132)] = 4060,
  [SMALL_STATE(133)] = 4086,
  [SMALL_STATE(134)] = 4104,
  [SMALL_STATE(135)] = 4130,
  [SMALL_STATE(136)] = 4148,
  [SMALL_STATE(137)] = 4172,
  [SMALL_STATE(138)] = 4196,
  [SMALL_STATE(139)] = 4222,
  [SMALL_STATE(140)] = 4240,
  [SMALL_STATE(141)] = 4258,
  [SMALL_STATE(142)] = 4282,
  [SMALL_STATE(143)] = 4300,
  [SMALL_STATE(144)] = 4318,
  [SMALL_STATE(145)] = 4336,
  [SMALL_STATE(146)] = 4354,
  [SMALL_STATE(147)] = 4374,
  [SMALL_STATE(148)] = 4392,
  [SMALL_STATE(149)] = 4418,
  [SMALL_STATE(150)] = 4441,
  [SMALL_STATE(151)] = 4460,
  [SMALL_STATE(152)] = 4481,
  [SMALL_STATE(153)] = 4502,
  [SMALL_STATE(154)] = 4525,
  [SMALL_STATE(155)] = 4546,
  [SMALL_STATE(156)] = 4569,
  [SMALL_STATE(157)] = 4588,
  [SMALL_STATE(158)] = 4607,
  [SMALL_STATE(159)] = 4628,
  [SMALL_STATE(160)] = 4647,
  [SMALL_STATE(161)] = 4666,
  [SMALL_STATE(162)] = 4685,
  [SMALL_STATE(163)] = 4708,
  [SMALL_STATE(164)] = 4727,
  [SMALL_STATE(165)] = 4750,
  [SMALL_STATE(166)] = 4771,
  [SMALL_STATE(167)] = 4794,
  [SMALL_STATE(168)] = 4810,
  [SMALL_STATE(169)] = 4826,
  [SMALL_STATE(170)] = 4844,
  [SMALL_STATE(171)] = 4862,
  [SMALL_STATE(172)] = 4880,
  [SMALL_STATE(173)] = 4898,
  [SMALL_STATE(174)] = 4918,
  [SMALL_STATE(175)] = 4936,
  [SMALL_STATE(176)] = 4952,
  [SMALL_STATE(177)] = 4968,
  [SMALL_STATE(178)] = 4984,
  [SMALL_STATE(179)] = 5002,
  [SMALL_STATE(180)] = 5018,
  [SMALL_STATE(181)] = 5034,
  [SMALL_STATE(182)] = 5050,
  [SMALL_STATE(183)] = 5068,
  [SMALL_STATE(184)] = 5084,
  [SMALL_STATE(185)] = 5100,
  [SMALL_STATE(186)] = 5116,
  [SMALL_STATE(187)] = 5136,
  [SMALL_STATE(188)] = 5154,
  [SMALL_STATE(189)] = 5170,
  [SMALL_STATE(190)] = 5188,
  [SMALL_STATE(191)] = 5206,
  [SMALL_STATE(192)] = 5222,
  [SMALL_STATE(193)] = 5240,
  [SMALL_STATE(194)] = 5258,
  [SMALL_STATE(195)] = 5276,
  [SMALL_STATE(196)] = 5294,
  [SMALL_STATE(197)] = 5310,
  [SMALL_STATE(198)] = 5328,
  [SMALL_STATE(199)] = 5346,
  [SMALL_STATE(200)] = 5364,
  [SMALL_STATE(201)] = 5382,
  [SMALL_STATE(202)] = 5400,
  [SMALL_STATE(203)] = 5418,
  [SMALL_STATE(204)] = 5434,
  [SMALL_STATE(205)] = 5450,
  [SMALL_STATE(206)] = 5468,
  [SMALL_STATE(207)] = 5483,
  [SMALL_STATE(208)] = 5500,
  [SMALL_STATE(209)] = 5515,
  [SMALL_STATE(210)] = 5532,
  [SMALL_STATE(211)] = 5549,
  [SMALL_STATE(212)] = 5564,
  [SMALL_STATE(213)] = 5579,
  [SMALL_STATE(214)] = 5593,
  [SMALL_STATE(215)] = 5607,
  [SMALL_STATE(216)] = 5621,
  [SMALL_STATE(217)] = 5635,
  [SMALL_STATE(218)] = 5649,
  [SMALL_STATE(219)] = 5663,
  [SMALL_STATE(220)] = 5677,
  [SMALL_STATE(221)] = 5691,
  [SMALL_STATE(222)] = 5705,
  [SMALL_STATE(223)] = 5719,
  [SMALL_STATE(224)] = 5733,
  [SMALL_STATE(225)] = 5747,
  [SMALL_STATE(226)] = 5761,
  [SMALL_STATE(227)] = 5775,
  [SMALL_STATE(228)] = 5789,
  [SMALL_STATE(229)] = 5803,
  [SMALL_STATE(230)] = 5817,
  [SMALL_STATE(231)] = 5831,
  [SMALL_STATE(232)] = 5845,
  [SMALL_STATE(233)] = 5859,
  [SMALL_STATE(234)] = 5873,
  [SMALL_STATE(235)] = 5887,
  [SMALL_STATE(236)] = 5901,
  [SMALL_STATE(237)] = 5915,
  [SMALL_STATE(238)] = 5929,
  [SMALL_STATE(239)] = 5943,
  [SMALL_STATE(240)] = 5957,
  [SMALL_STATE(241)] = 5971,
  [SMALL_STATE(242)] = 5985,
  [SMALL_STATE(243)] = 5999,
  [SMALL_STATE(244)] = 6013,
  [SMALL_STATE(245)] = 6027,
  [SMALL_STATE(246)] = 6041,
  [SMALL_STATE(247)] = 6055,
  [SMALL_STATE(248)] = 6069,
  [SMALL_STATE(249)] = 6083,
  [SMALL_STATE(250)] = 6097,
  [SMALL_STATE(251)] = 6111,
  [SMALL_STATE(252)] = 6125,
  [SMALL_STATE(253)] = 6129,
};

static const TSParseActionEntry ts_parse_actions[] = {
  [0] = {.entry = {.count = 0, .reusable = false}},
  [1] = {.entry = {.count = 1, .reusable = false}}, RECOVER(),
  [3] = {.entry = {.count = 1, .reusable = true}}, SHIFT(237),
  [5] = {.entry = {.count = 1, .reusable = true}}, SHIFT(245),
  [7] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_source_file, 0, 0, 0),
  [9] = {.entry = {.count = 1, .reusable = false}}, SHIFT(10),
  [11] = {.entry = {.count = 1, .reusable = false}}, SHIFT(7),
  [13] = {.entry = {.count = 1, .reusable = true}}, SHIFT(234),
  [15] = {.entry = {.count = 1, .reusable = true}}, SHIFT(155),
  [17] = {.entry = {.count = 1, .reusable = false}}, SHIFT(14),
  [19] = {.entry = {.count = 1, .reusable = true}}, SHIFT(200),
  [21] = {.entry = {.count = 1, .reusable = false}}, SHIFT(200),
  [23] = {.entry = {.count = 1, .reusable = false}}, SHIFT(193),
  [25] = {.entry = {.count = 1, .reusable = true}}, SHIFT(158),
  [27] = {.entry = {.count = 1, .reusable = true}}, SHIFT(97),
  [29] = {.entry = {.count = 1, .reusable = true}}, SHIFT(116),
  [31] = {.entry = {.count = 1, .reusable = false}}, SHIFT(218),
  [33] = {.entry = {.count = 1, .reusable = false}}, SHIFT(165),
  [35] = {.entry = {.count = 1, .reusable = true}}, SHIFT(197),
  [37] = {.entry = {.count = 1, .reusable = false}}, SHIFT(197),
  [39] = {.entry = {.count = 1, .reusable = false}}, SHIFT(198),
  [41] = {.entry = {.count = 1, .reusable = false}}, SHIFT(67),
  [43] = {.entry = {.count = 1, .reusable = true}}, SHIFT(224),
  [45] = {.entry = {.count = 1, .reusable = true}}, SHIFT(225),
  [47] = {.entry = {.count = 1, .reusable = true}}, SHIFT(80),
  [49] = {.entry = {.count = 1, .reusable = true}}, SHIFT(30),
  [51] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym__space, 1, 0, 0),
  [53] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym__space, 1, 0, 0),
  [55] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_source_file, 1, 0, 0),
  [57] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0),
  [59] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(10),
  [62] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(7),
  [65] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(234),
  [68] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(155),
  [71] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(14),
  [74] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(200),
  [77] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(200),
  [80] = {.entry = {.count = 1, .reusable = true}}, SHIFT(8),
  [82] = {.entry = {.count = 1, .reusable = true}}, SHIFT(220),
  [84] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym__statement, 2, 0, 0),
  [86] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym__statement, 2, 0, 0),
  [88] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 1, 0, 0),
  [90] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_source_file_repeat1, 1, 0, 0),
  [92] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym__statement, 1, 0, 0),
  [94] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym__statement, 1, 0, 0),
  [96] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym__statement, 3, 0, 0),
  [98] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym__statement, 3, 0, 0),
  [100] = {.entry = {.count = 1, .reusable = false}}, SHIFT(173),
  [102] = {.entry = {.count = 1, .reusable = false}}, SHIFT(207),
  [104] = {.entry = {.count = 1, .reusable = false}}, SHIFT(210),
  [106] = {.entry = {.count = 1, .reusable = false}}, SHIFT(186),
  [108] = {.entry = {.count = 1, .reusable = false}}, SHIFT(26),
  [110] = {.entry = {.count = 1, .reusable = true}}, SHIFT(26),
  [112] = {.entry = {.count = 1, .reusable = true}}, SHIFT(150),
  [114] = {.entry = {.count = 1, .reusable = true}}, SHIFT(108),
  [116] = {.entry = {.count = 1, .reusable = true}}, SHIFT(109),
  [118] = {.entry = {.count = 1, .reusable = false}}, SHIFT(194),
  [120] = {.entry = {.count = 1, .reusable = false}}, SHIFT(226),
  [122] = {.entry = {.count = 1, .reusable = false}}, SHIFT(159),
  [124] = {.entry = {.count = 1, .reusable = true}}, SHIFT(203),
  [126] = {.entry = {.count = 1, .reusable = false}}, SHIFT(203),
  [128] = {.entry = {.count = 1, .reusable = false}}, SHIFT(204),
  [130] = {.entry = {.count = 1, .reusable = false}}, SHIFT(68),
  [132] = {.entry = {.count = 1, .reusable = true}}, SHIFT(249),
  [134] = {.entry = {.count = 1, .reusable = true}}, SHIFT(250),
  [136] = {.entry = {.count = 1, .reusable = true}}, SHIFT(65),
  [138] = {.entry = {.count = 1, .reusable = true}}, SHIFT(33),
  [140] = {.entry = {.count = 1, .reusable = false}}, SHIFT(205),
  [142] = {.entry = {.count = 1, .reusable = false}}, SHIFT(169),
  [144] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_selector_query_repeat1, 2, 0, 0), SHIFT_REPEAT(26),
  [147] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_selector_query_repeat1, 2, 0, 0), SHIFT_REPEAT(26),
  [150] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_selector_query_repeat1, 2, 0, 0),
  [152] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_selector_query_repeat1, 2, 0, 0),
  [154] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_selector_query_repeat1, 1, 0, 0),
  [156] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_selector_query_repeat1, 1, 0, 0),
  [158] = {.entry = {.count = 1, .reusable = false}}, SHIFT(4),
  [160] = {.entry = {.count = 1, .reusable = true}}, SHIFT(4),
  [162] = {.entry = {.count = 1, .reusable = true}}, SHIFT(54),
  [164] = {.entry = {.count = 1, .reusable = false}}, SHIFT(228),
  [166] = {.entry = {.count = 1, .reusable = false}}, SHIFT(52),
  [168] = {.entry = {.count = 1, .reusable = true}}, SHIFT(243),
  [170] = {.entry = {.count = 1, .reusable = true}}, SHIFT(244),
  [172] = {.entry = {.count = 1, .reusable = true}}, SHIFT(55),
  [174] = {.entry = {.count = 1, .reusable = true}}, SHIFT(87),
  [176] = {.entry = {.count = 1, .reusable = true}}, SHIFT(35),
  [178] = {.entry = {.count = 1, .reusable = true}}, SHIFT(62),
  [180] = {.entry = {.count = 1, .reusable = true}}, SHIFT(96),
  [182] = {.entry = {.count = 1, .reusable = true}}, SHIFT(174),
  [184] = {.entry = {.count = 1, .reusable = true}}, SHIFT(182),
  [186] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_nbt_array_repeat1, 2, 0, 0), SHIFT_REPEAT(4),
  [189] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_nbt_array_repeat1, 2, 0, 0), SHIFT_REPEAT(4),
  [192] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_nbt_array_repeat1, 2, 0, 0), SHIFT_REPEAT(54),
  [195] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_nbt_array_repeat1, 2, 0, 0), SHIFT_REPEAT(228),
  [198] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_nbt_array_repeat1, 2, 0, 0), SHIFT_REPEAT(52),
  [201] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_nbt_array_repeat1, 2, 0, 0), SHIFT_REPEAT(243),
  [204] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_nbt_array_repeat1, 2, 0, 0), SHIFT_REPEAT(244),
  [207] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_nbt_array_repeat1, 2, 0, 0), SHIFT_REPEAT(55),
  [210] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_nbt_array_repeat1, 2, 0, 0), SHIFT_REPEAT(87),
  [213] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_nbt_array_repeat1, 2, 0, 0), SHIFT_REPEAT(35),
  [216] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_nbt_array_repeat1, 2, 0, 0),
  [218] = {.entry = {.count = 1, .reusable = true}}, SHIFT(93),
  [220] = {.entry = {.count = 1, .reusable = true}}, SHIFT(180),
  [222] = {.entry = {.count = 1, .reusable = true}}, SHIFT(133),
  [224] = {.entry = {.count = 1, .reusable = true}}, SHIFT(60),
  [226] = {.entry = {.count = 1, .reusable = true}}, SHIFT(126),
  [228] = {.entry = {.count = 1, .reusable = true}}, SHIFT(184),
  [230] = {.entry = {.count = 1, .reusable = false}}, SHIFT(63),
  [232] = {.entry = {.count = 1, .reusable = true}}, SHIFT(63),
  [234] = {.entry = {.count = 1, .reusable = false}}, SHIFT(219),
  [236] = {.entry = {.count = 1, .reusable = false}}, SHIFT(104),
  [238] = {.entry = {.count = 1, .reusable = true}}, SHIFT(247),
  [240] = {.entry = {.count = 1, .reusable = true}}, SHIFT(248),
  [242] = {.entry = {.count = 1, .reusable = true}}, SHIFT(139),
  [244] = {.entry = {.count = 1, .reusable = true}}, SHIFT(89),
  [246] = {.entry = {.count = 1, .reusable = true}}, SHIFT(36),
  [248] = {.entry = {.count = 1, .reusable = false}}, SHIFT(233),
  [250] = {.entry = {.count = 1, .reusable = false}}, SHIFT(83),
  [252] = {.entry = {.count = 1, .reusable = true}}, SHIFT(213),
  [254] = {.entry = {.count = 1, .reusable = true}}, SHIFT(246),
  [256] = {.entry = {.count = 1, .reusable = true}}, SHIFT(106),
  [258] = {.entry = {.count = 1, .reusable = true}}, SHIFT(88),
  [260] = {.entry = {.count = 1, .reusable = true}}, SHIFT(32),
  [262] = {.entry = {.count = 1, .reusable = true}}, SHIFT(115),
  [264] = {.entry = {.count = 1, .reusable = true}}, SHIFT(114),
  [266] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_selector_query_repeat1, 2, 0, 0), SHIFT_REPEAT(63),
  [269] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_selector_query_repeat1, 2, 0, 0), SHIFT_REPEAT(63),
  [272] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_scale, 2, 0, 0),
  [274] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_scale, 2, 0, 0),
  [276] = {.entry = {.count = 1, .reusable = false}}, SHIFT(61),
  [278] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_scale, 1, 0, 0),
  [280] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_scale, 1, 0, 0),
  [282] = {.entry = {.count = 1, .reusable = false}}, SHIFT(57),
  [284] = {.entry = {.count = 1, .reusable = true}}, SHIFT(98),
  [286] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_nbt_array_repeat1, 1, 0, 0),
  [288] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_nbt_array_repeat1, 1, 0, 0),
  [290] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym__nbt_element, 1, 0, 0),
  [292] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym__nbt_element, 1, 0, 0),
  [294] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_string, 3, 0, 0),
  [296] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_string, 3, 0, 0),
  [298] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_nbt_compound, 3, 0, 0),
  [300] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_nbt_compound, 3, 0, 0),
  [302] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_nbt_compound, 2, 0, 0),
  [304] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_nbt_compound, 2, 0, 0),
  [306] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_nbt_array, 2, 0, 0),
  [308] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_nbt_array, 2, 0, 0),
  [310] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_scale, 3, 0, 0),
  [312] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_scale, 3, 0, 0),
  [314] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_nbt_array, 3, 0, 0),
  [316] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_nbt_array, 3, 0, 0),
  [318] = {.entry = {.count = 1, .reusable = false}}, SHIFT(3),
  [320] = {.entry = {.count = 1, .reusable = true}}, SHIFT(3),
  [322] = {.entry = {.count = 1, .reusable = true}}, SHIFT(46),
  [324] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_item_compound_repeat1, 1, 0, 0),
  [326] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_item_compound_repeat1, 1, 0, 0),
  [328] = {.entry = {.count = 1, .reusable = true}}, SHIFT(100),
  [330] = {.entry = {.count = 1, .reusable = true}}, SHIFT(125),
  [332] = {.entry = {.count = 1, .reusable = true}}, SHIFT(241),
  [334] = {.entry = {.count = 1, .reusable = true}}, SHIFT(242),
  [336] = {.entry = {.count = 1, .reusable = false}}, SHIFT(156),
  [338] = {.entry = {.count = 1, .reusable = true}}, SHIFT(176),
  [340] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_word, 1, 0, 0),
  [342] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_word, 1, 0, 0),
  [344] = {.entry = {.count = 1, .reusable = true}}, SHIFT(67),
  [346] = {.entry = {.count = 1, .reusable = true}}, SHIFT(70),
  [348] = {.entry = {.count = 1, .reusable = true}}, SHIFT(72),
  [350] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_nbt_identifier, 1, 0, 0),
  [352] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_nbt_identifier, 1, 0, 0),
  [354] = {.entry = {.count = 1, .reusable = true}}, SHIFT(110),
  [356] = {.entry = {.count = 1, .reusable = true}}, SHIFT(64),
  [358] = {.entry = {.count = 1, .reusable = false}}, SHIFT(69),
  [360] = {.entry = {.count = 1, .reusable = true}}, SHIFT(112),
  [362] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_item_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(110),
  [365] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_item_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(64),
  [368] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_item_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(213),
  [371] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_item_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(246),
  [374] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_item_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(69),
  [377] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_item_compound_repeat1, 2, 0, 0),
  [379] = {.entry = {.count = 1, .reusable = true}}, SHIFT(122),
  [381] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_selector_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(121),
  [384] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_selector_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(125),
  [387] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_selector_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(224),
  [390] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_selector_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(225),
  [393] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_selector_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(156),
  [396] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_selector_compound_repeat1, 2, 0, 0),
  [398] = {.entry = {.count = 1, .reusable = true}}, SHIFT(101),
  [400] = {.entry = {.count = 1, .reusable = true}}, SHIFT(117),
  [402] = {.entry = {.count = 1, .reusable = true}}, SHIFT(121),
  [404] = {.entry = {.count = 1, .reusable = true}}, SHIFT(191),
  [406] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_word, 2, 0, 0),
  [408] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_word, 2, 0, 0),
  [410] = {.entry = {.count = 1, .reusable = true}}, SHIFT(143),
  [412] = {.entry = {.count = 1, .reusable = true}}, SHIFT(156),
  [414] = {.entry = {.count = 1, .reusable = true}}, SHIFT(94),
  [416] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_nbt_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(143),
  [419] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_nbt_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(241),
  [422] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_nbt_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(242),
  [425] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_nbt_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(156),
  [428] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_nbt_compound_repeat1, 2, 0, 0),
  [430] = {.entry = {.count = 1, .reusable = true}}, SHIFT(172),
  [432] = {.entry = {.count = 1, .reusable = true}}, SHIFT(202),
  [434] = {.entry = {.count = 1, .reusable = false}}, SHIFT(92),
  [436] = {.entry = {.count = 1, .reusable = false}}, SHIFT(95),
  [438] = {.entry = {.count = 1, .reusable = true}}, SHIFT(58),
  [440] = {.entry = {.count = 1, .reusable = true}}, SHIFT(131),
  [442] = {.entry = {.count = 1, .reusable = true}}, SHIFT(59),
  [444] = {.entry = {.count = 1, .reusable = true}}, SHIFT(91),
  [446] = {.entry = {.count = 1, .reusable = true}}, SHIFT(147),
  [448] = {.entry = {.count = 1, .reusable = true}}, SHIFT(183),
  [450] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_position, 1, 0, 0),
  [452] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_position, 1, 0, 0),
  [454] = {.entry = {.count = 1, .reusable = true}}, SHIFT(218),
  [456] = {.entry = {.count = 1, .reusable = true}}, SHIFT(165),
  [458] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_item_compound_repeat1, 5, 0, 0),
  [460] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_item_compound_repeat1, 5, 0, 0),
  [462] = {.entry = {.count = 1, .reusable = false}}, SHIFT(18),
  [464] = {.entry = {.count = 1, .reusable = true}}, SHIFT(18),
  [466] = {.entry = {.count = 1, .reusable = false}}, SHIFT(22),
  [468] = {.entry = {.count = 1, .reusable = true}}, SHIFT(22),
  [470] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_nbt_compound_repeat1, 1, 0, 0), REDUCE(aux_sym_selector_compound_repeat1, 1, 0, 0),
  [473] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_selector_compound_repeat1, 1, 0, 0),
  [475] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_nbt_compound_repeat1, 1, 0, 0), REDUCE(aux_sym_selector_compound_repeat1, 1, 0, 0),
  [478] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_item_compound, 3, 0, 0),
  [480] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_item_compound, 3, 0, 0),
  [482] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_selector_query_repeat1, 2, 0, 0), SHIFT_REPEAT(18),
  [485] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_selector_query_repeat1, 2, 0, 0), SHIFT_REPEAT(18),
  [488] = {.entry = {.count = 1, .reusable = true}}, SHIFT(44),
  [490] = {.entry = {.count = 1, .reusable = true}}, SHIFT(41),
  [492] = {.entry = {.count = 1, .reusable = false}}, SHIFT(145),
  [494] = {.entry = {.count = 1, .reusable = false}}, SHIFT(127),
  [496] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_selector_compound_repeat1, 5, 0, 0),
  [498] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_selector_compound_repeat1, 5, 0, 0),
  [500] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_rotation, 1, 0, 0),
  [502] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_rotation, 1, 0, 0),
  [504] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_selector_compound_repeat1, 4, 0, 0),
  [506] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_selector_compound_repeat1, 4, 0, 0),
  [508] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_item_compound, 2, 0, 0),
  [510] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_item_compound, 2, 0, 0),
  [512] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_selector_compound_repeat1, 3, 0, 0),
  [514] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_selector_compound_repeat1, 3, 0, 0),
  [516] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_item_compound_repeat1, 4, 0, 0),
  [518] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_item_compound_repeat1, 4, 0, 0),
  [520] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_item_compound_repeat1, 3, 0, 0),
  [522] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_item_compound_repeat1, 3, 0, 0),
  [524] = {.entry = {.count = 1, .reusable = false}}, SHIFT(23),
  [526] = {.entry = {.count = 1, .reusable = true}}, SHIFT(23),
  [528] = {.entry = {.count = 1, .reusable = true}}, SHIFT(43),
  [530] = {.entry = {.count = 1, .reusable = true}}, SHIFT(38),
  [532] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_selector_compound_repeat1, 1, 0, 0),
  [534] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_word, 3, 0, 0),
  [536] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_word, 3, 0, 0),
  [538] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym__generic_command, 1, 0, 0),
  [540] = {.entry = {.count = 1, .reusable = true}}, SHIFT(50),
  [542] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_nbt_compound_repeat1, 5, 0, 0),
  [544] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_selector_query_repeat1, 2, 0, 0), SHIFT_REPEAT(3),
  [547] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_selector_query_repeat1, 2, 0, 0), SHIFT_REPEAT(3),
  [550] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym__generic_command, 2, 0, 0),
  [552] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_nbt_compound_repeat1, 3, 0, 0),
  [554] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym__generic_command_repeat1, 2, 0, 0),
  [556] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym__generic_command_repeat1, 2, 0, 0), SHIFT_REPEAT(3),
  [559] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym__generic_command_repeat1, 2, 0, 0), SHIFT_REPEAT(3),
  [562] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_nbt_compound_repeat1, 1, 0, 0),
  [564] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_nbt_compound_repeat1, 4, 0, 0),
  [566] = {.entry = {.count = 1, .reusable = true}}, SHIFT(164),
  [568] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_selector, 1, 0, 0),
  [570] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym__execute_command_repeat1, 2, 0, 0), SHIFT_REPEAT(19),
  [573] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym__execute_command_repeat1, 2, 0, 0), SHIFT_REPEAT(19),
  [576] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_selector_repeat1, 2, 0, 0),
  [578] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_selector_repeat1, 2, 0, 0), SHIFT_REPEAT(119),
  [581] = {.entry = {.count = 1, .reusable = true}}, SHIFT(187),
  [583] = {.entry = {.count = 1, .reusable = true}}, SHIFT(119),
  [585] = {.entry = {.count = 1, .reusable = true}}, SHIFT(192),
  [587] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_selector, 1, 0, 0),
  [589] = {.entry = {.count = 1, .reusable = true}}, SHIFT(153),
  [591] = {.entry = {.count = 1, .reusable = false}}, SHIFT(179),
  [593] = {.entry = {.count = 1, .reusable = false}}, SHIFT(181),
  [595] = {.entry = {.count = 1, .reusable = true}}, SHIFT(171),
  [597] = {.entry = {.count = 1, .reusable = true}}, SHIFT(175),
  [599] = {.entry = {.count = 1, .reusable = true}}, SHIFT(195),
  [601] = {.entry = {.count = 1, .reusable = true}}, SHIFT(196),
  [603] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_position, 2, 0, 0),
  [605] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_word, 4, 0, 0),
  [607] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_selector_query, 3, 0, 0),
  [609] = {.entry = {.count = 1, .reusable = true}}, SHIFT(206),
  [611] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym__generic_command_repeat1, 2, 0, 0),
  [613] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_selector, 4, 0, 0),
  [615] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_selector, 4, 0, 0),
  [617] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_selector, 3, 0, 0),
  [619] = {.entry = {.count = 2, .reusable = true}}, REDUCE(sym_nbt_compound, 2, 0, 0), REDUCE(sym_selector_compound, 2, 0, 0),
  [622] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_position, 2, 0, 0),
  [624] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_rotation, 2, 0, 0),
  [626] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_selector, 3, 0, 0),
  [628] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_rotation, 2, 0, 0),
  [630] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_word, 4, 0, 0),
  [632] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_selector_compound, 3, 0, 0),
  [634] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_argument, 1, 0, 0),
  [636] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_argument, 1, 0, 0),
  [638] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_selector_query, 5, 0, 0),
  [640] = {.entry = {.count = 1, .reusable = true}}, SHIFT(212),
  [642] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_operator, 1, 0, 0),
  [644] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_operator, 1, 0, 0),
  [646] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_slot, 1, 0, 0),
  [648] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_slot, 1, 0, 0),
  [650] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_command_identifier, 1, 0, 0),
  [652] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_command_identifier, 1, 0, 0),
  [654] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_selector_query, 4, 0, 0),
  [656] = {.entry = {.count = 1, .reusable = true}}, SHIFT(211),
  [658] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_subcommand_identifier, 1, 0, 0),
  [660] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_subcommand_identifier, 1, 0, 0),
  [662] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_selector_repeat1, 1, 0, 0),
  [664] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym__execute_command_repeat1, 2, 0, 0),
  [666] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym__execute_command_repeat1, 2, 0, 0),
  [668] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym__subcommand, 1, 0, 0),
  [670] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym__subcommand, 1, 0, 0),
  [672] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_selector_query, 6, 0, 0),
  [674] = {.entry = {.count = 1, .reusable = false}}, SHIFT(237),
  [676] = {.entry = {.count = 1, .reusable = false}}, SHIFT(245),
  [678] = {.entry = {.count = 1, .reusable = false}}, SHIFT(229),
  [680] = {.entry = {.count = 1, .reusable = true}}, SHIFT(163),
  [682] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_command, 2, 0, 0),
  [684] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym__execute_command, 5, 0, 0),
  [686] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_command, 1, 0, 0),
  [688] = {.entry = {.count = 1, .reusable = true}}, SHIFT(154),
  [690] = {.entry = {.count = 1, .reusable = true}}, SHIFT(105),
  [692] = {.entry = {.count = 1, .reusable = true}}, SHIFT(11),
  [694] = {.entry = {.count = 1, .reusable = true}}, SHIFT(56),
  [696] = {.entry = {.count = 1, .reusable = true}}, SHIFT(146),
  [698] = {.entry = {.count = 1, .reusable = false}}, SHIFT(223),
  [700] = {.entry = {.count = 1, .reusable = false}}, SHIFT(227),
  [702] = {.entry = {.count = 1, .reusable = true}}, SHIFT(160),
  [704] = {.entry = {.count = 1, .reusable = true}}, SHIFT(51),
  [706] = {.entry = {.count = 1, .reusable = true}}, SHIFT(66),
  [708] = {.entry = {.count = 1, .reusable = true}}, SHIFT(253),
  [710] = {.entry = {.count = 1, .reusable = true}}, SHIFT(84),
  [712] = {.entry = {.count = 1, .reusable = true}}, SHIFT(142),
  [714] = {.entry = {.count = 1, .reusable = true}}, SHIFT(232),
  [716] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym__execute_command, 6, 0, 0),
  [718] = {.entry = {.count = 1, .reusable = true}}, SHIFT(161),
  [720] = {.entry = {.count = 1, .reusable = false}}, SHIFT(231),
  [722] = {.entry = {.count = 1, .reusable = false}}, SHIFT(214),
  [724] = {.entry = {.count = 1, .reusable = false}}, SHIFT(221),
  [726] = {.entry = {.count = 1, .reusable = false}}, SHIFT(222),
  [728] = {.entry = {.count = 1, .reusable = true}}, SHIFT(252),
  [730] = {.entry = {.count = 1, .reusable = false}}, SHIFT(230),
  [732] = {.entry = {.count = 1, .reusable = false}}, SHIFT(235),
  [734] = {.entry = {.count = 1, .reusable = false}}, SHIFT(236),
  [736] = {.entry = {.count = 1, .reusable = false}}, SHIFT(239),
  [738] = {.entry = {.count = 1, .reusable = false}}, SHIFT(240),
  [740] = {.entry = {.count = 1, .reusable = true}},  ACCEPT_INPUT(),
  [742] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_substitution, 2, 0, 0),
  [744] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_macro, 3, 0, 0),
};

#ifdef __cplusplus
extern "C" {
#endif
#ifdef TREE_SITTER_HIDE_SYMBOLS
#define TS_PUBLIC
#elif defined(_WIN32)
#define TS_PUBLIC __declspec(dllexport)
#else
#define TS_PUBLIC __attribute__((visibility("default")))
#endif

TS_PUBLIC const TSLanguage *tree_sitter_mcfunction(void) {
  static const TSLanguage language = {
    .abi_version = LANGUAGE_VERSION,
    .symbol_count = SYMBOL_COUNT,
    .alias_count = ALIAS_COUNT,
    .token_count = TOKEN_COUNT,
    .external_token_count = EXTERNAL_TOKEN_COUNT,
    .state_count = STATE_COUNT,
    .large_state_count = LARGE_STATE_COUNT,
    .production_id_count = PRODUCTION_ID_COUNT,
    .supertype_count = SUPERTYPE_COUNT,
    .field_count = FIELD_COUNT,
    .max_alias_sequence_length = MAX_ALIAS_SEQUENCE_LENGTH,
    .parse_table = &ts_parse_table[0][0],
    .small_parse_table = ts_small_parse_table,
    .small_parse_table_map = ts_small_parse_table_map,
    .parse_actions = ts_parse_actions,
    .symbol_names = ts_symbol_names,
    .symbol_metadata = ts_symbol_metadata,
    .public_symbol_map = ts_symbol_map,
    .alias_map = ts_non_terminal_alias_map,
    .alias_sequences = &ts_alias_sequences[0][0],
    .lex_modes = (const void*)ts_lex_modes,
    .lex_fn = ts_lex,
    .primary_state_ids = ts_primary_state_ids,
    .name = "mcfunction",
    .max_reserved_word_set_size = 0,
    .metadata = {
      .major_version = 1,
      .minor_version = 0,
      .patch_version = 0,
    },
  };
  return &language;
}
#ifdef __cplusplus
}
#endif
