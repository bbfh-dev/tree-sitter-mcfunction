/* Automatically @generated by tree-sitter v0.25.10 */

#include "tree_sitter/parser.h"

#if defined(__GNUC__) || defined(__clang__)
#pragma GCC diagnostic ignored "-Wmissing-field-initializers"
#endif

#ifdef _MSC_VER
#pragma optimize("", off)
#elif defined(__clang__)
#pragma clang optimize off
#elif defined(__GNUC__)
#pragma GCC optimize ("O0")
#endif

#define LANGUAGE_VERSION 15
#define STATE_COUNT 257
#define LARGE_STATE_COUNT 20
#define SYMBOL_COUNT 322
#define ALIAS_COUNT 0
#define TOKEN_COUNT 287
#define EXTERNAL_TOKEN_COUNT 0
#define FIELD_COUNT 0
#define MAX_ALIAS_SEQUENCE_LENGTH 6
#define MAX_RESERVED_WORD_SET_SIZE 0
#define PRODUCTION_ID_COUNT 1
#define SUPERTYPE_COUNT 0

enum ts_symbol_identifiers {
  sym__newline = 1,
  aux_sym__space_token1 = 2,
  sym_backslash = 3,
  aux_sym__statement_token1 = 4,
  sym_comment = 5,
  anon_sym_execute = 6,
  anon_sym_run = 7,
  sym_macro_indicator = 8,
  anon_sym_advancement = 9,
  anon_sym_attribute = 10,
  anon_sym_ban = 11,
  anon_sym_ban_DASHip = 12,
  anon_sym_banlist = 13,
  anon_sym_bossbar = 14,
  anon_sym_clear = 15,
  anon_sym_clone = 16,
  anon_sym_damage = 17,
  anon_sym_data = 18,
  anon_sym_datapack = 19,
  anon_sym_debug = 20,
  anon_sym_defaultgamemode = 21,
  anon_sym_deop = 22,
  anon_sym_dialog = 23,
  anon_sym_difficulty = 24,
  anon_sym_effect = 25,
  anon_sym_enchant = 26,
  anon_sym_experience = 27,
  anon_sym_fetchprofile = 28,
  anon_sym_fill = 29,
  anon_sym_fillbiome = 30,
  anon_sym_forceload = 31,
  anon_sym_function = 32,
  anon_sym_gamemode = 33,
  anon_sym_gamerule = 34,
  anon_sym_give = 35,
  anon_sym_help = 36,
  anon_sym_item = 37,
  anon_sym_jfr = 38,
  anon_sym_kick = 39,
  anon_sym_kill = 40,
  anon_sym_list = 41,
  anon_sym_locate = 42,
  anon_sym_loot = 43,
  anon_sym_me = 44,
  anon_sym_msg = 45,
  anon_sym_op = 46,
  anon_sym_pardon = 47,
  anon_sym_pardon_DASHip = 48,
  anon_sym_particle = 49,
  anon_sym_perf = 50,
  anon_sym_place = 51,
  anon_sym_playsound = 52,
  anon_sym_publish = 53,
  anon_sym_random = 54,
  anon_sym_recipe = 55,
  anon_sym_reload = 56,
  anon_sym_return = 57,
  anon_sym_ride = 58,
  anon_sym_rotate = 59,
  anon_sym_save_DASHall = 60,
  anon_sym_save_DASHoff = 61,
  anon_sym_save_DASHon = 62,
  anon_sym_say = 63,
  anon_sym_schedule = 64,
  anon_sym_scoreboard = 65,
  anon_sym_seed = 66,
  anon_sym_setblock = 67,
  anon_sym_setidletimeout = 68,
  anon_sym_setworldspawn = 69,
  anon_sym_spawnpoint = 70,
  anon_sym_spectate = 71,
  anon_sym_spreadplayers = 72,
  anon_sym_stop = 73,
  anon_sym_stopsound = 74,
  anon_sym_summon = 75,
  anon_sym_swing = 76,
  anon_sym_tag = 77,
  anon_sym_team = 78,
  anon_sym_teammsg = 79,
  anon_sym_teleport = 80,
  anon_sym_tell = 81,
  anon_sym_tellraw = 82,
  anon_sym_test = 83,
  anon_sym_time = 84,
  anon_sym_title = 85,
  anon_sym_tm = 86,
  anon_sym_tp = 87,
  anon_sym_transfer = 88,
  anon_sym_trigger = 89,
  anon_sym_version = 90,
  anon_sym_w = 91,
  anon_sym_waypoint = 92,
  anon_sym_weather = 93,
  anon_sym_whitelist = 94,
  anon_sym_worldborder = 95,
  anon_sym_xp = 96,
  anon_sym_align = 97,
  anon_sym_all = 98,
  anon_sym_anchored = 99,
  anon_sym_as = 100,
  anon_sym_at = 101,
  anon_sym_attacker = 102,
  anon_sym_biome = 103,
  anon_sym_block = 104,
  anon_sym_blocks = 105,
  anon_sym_controller = 106,
  anon_sym_dimension = 107,
  anon_sym_entity = 108,
  anon_sym_eyes = 109,
  anon_sym_facing = 110,
  anon_sym_feet = 111,
  anon_sym_if = 112,
  anon_sym_in = 113,
  anon_sym_items = 114,
  anon_sym_leasher = 115,
  anon_sym_loaded = 116,
  anon_sym_masked = 117,
  anon_sym_matches = 118,
  anon_sym_max = 119,
  anon_sym_on = 120,
  anon_sym_origin = 121,
  anon_sym_over = 122,
  anon_sym_owner = 123,
  anon_sym_passengers = 124,
  anon_sym_positioned = 125,
  anon_sym_predicate = 126,
  anon_sym_result = 127,
  anon_sym_rotated = 128,
  anon_sym_score = 129,
  anon_sym_storage = 130,
  anon_sym_store = 131,
  anon_sym_success = 132,
  anon_sym_target = 133,
  anon_sym_unless = 134,
  anon_sym_value = 135,
  anon_sym_vehicle = 136,
  anon_sym_add = 137,
  anon_sym_add_multiplied_base = 138,
  anon_sym_add_multiplied_total = 139,
  anon_sym_add_value = 140,
  anon_sym_amount = 141,
  anon_sym_append = 142,
  anon_sym_base = 143,
  anon_sym_buffer = 144,
  anon_sym_by = 145,
  anon_sym_center = 146,
  anon_sym_color = 147,
  anon_sym_create = 148,
  anon_sym_day = 149,
  anon_sym_daytime = 150,
  anon_sym_destroy = 151,
  anon_sym_disable = 152,
  anon_sym_dismount = 153,
  anon_sym_distance = 154,
  anon_sym_enable = 155,
  anon_sym_everything = 156,
  anon_sym_export = 157,
  anon_sym_exportclosest = 158,
  anon_sym_exportthat = 159,
  anon_sym_exportthese = 160,
  anon_sym_fail = 161,
  anon_sym_feature = 162,
  anon_sym_fish = 163,
  anon_sym_flush = 164,
  anon_sym_force = 165,
  anon_sym_from = 166,
  anon_sym_gametime = 167,
  anon_sym_get = 168,
  anon_sym_grant = 169,
  anon_sym_hollow = 170,
  anon_sym_id = 171,
  anon_sym_insert = 172,
  anon_sym_ips = 173,
  anon_sym_jigsaw = 174,
  anon_sym_join = 175,
  anon_sym_keep = 176,
  anon_sym_leave = 177,
  anon_sym_levels = 178,
  anon_sym_mainhand = 179,
  anon_sym_merge = 180,
  anon_sym_midnight = 181,
  anon_sym_mine = 182,
  anon_sym_modifier = 183,
  anon_sym_modify = 184,
  anon_sym_mount = 185,
  anon_sym_move = 186,
  anon_sym_name = 187,
  anon_sym_night = 188,
  anon_sym_noon = 189,
  anon_sym_normal = 190,
  anon_sym_off = 191,
  anon_sym_offhand = 192,
  anon_sym_only = 193,
  anon_sym_outline = 194,
  anon_sym_players = 195,
  anon_sym_poi = 196,
  anon_sym_points = 197,
  anon_sym_pos = 198,
  anon_sym_prepend = 199,
  anon_sym_query = 200,
  anon_sym_rain = 201,
  anon_sym_remove = 202,
  anon_sym_replace = 203,
  anon_sym_reset = 204,
  anon_sym_resetclosest = 205,
  anon_sym_resetthat = 206,
  anon_sym_resetthese = 207,
  anon_sym_revoke = 208,
  anon_sym_roll = 209,
  anon_sym_runclosest = 210,
  anon_sym_runfailed = 211,
  anon_sym_runthat = 212,
  anon_sym_runthese = 213,
  anon_sym_set = 214,
  anon_sym_setdisplay = 215,
  anon_sym_show = 216,
  anon_sym_spawn = 217,
  anon_sym_start = 218,
  anon_sym_strict = 219,
  anon_sym_string = 220,
  anon_sym_structure = 221,
  anon_sym_style = 222,
  anon_sym_take = 223,
  anon_sym_template = 224,
  anon_sym_through = 225,
  anon_sym_thunder = 226,
  anon_sym_times = 227,
  anon_sym_to = 228,
  anon_sym_under = 229,
  anon_sym_until = 230,
  anon_sym_uuids = 231,
  anon_sym_verify = 232,
  anon_sym_visible = 233,
  anon_sym_warning = 234,
  anon_sym_with = 235,
  aux_sym_selector_token1 = 236,
  aux_sym_selector_token2 = 237,
  sym_selector_identifier = 238,
  sym_query_identifier = 239,
  anon_sym_EQ = 240,
  anon_sym_EQ_BANG = 241,
  aux_sym_selector_query_token1 = 242,
  anon_sym_TILDE = 243,
  anon_sym_CARET = 244,
  sym_heightmap = 245,
  sym_resource = 246,
  aux_sym_scale_token1 = 247,
  aux_sym_scale_token2 = 248,
  aux_sym_scale_token3 = 249,
  aux_sym_scale_token4 = 250,
  aux_sym_scale_token5 = 251,
  anon_sym_DOLLAR_LPAREN = 252,
  aux_sym_macro_token1 = 253,
  anon_sym_RPAREN = 254,
  sym_type = 255,
  anon_sym_PLUS_EQ = 256,
  anon_sym_DASH_EQ = 257,
  anon_sym_STAR_EQ = 258,
  anon_sym_SLASH_EQ = 259,
  anon_sym_PERCENT_EQ = 260,
  anon_sym_GT_LT = 261,
  anon_sym_LT = 262,
  anon_sym_GT = 263,
  aux_sym_slot_token1 = 264,
  aux_sym_slot_token2 = 265,
  aux_sym_slot_token3 = 266,
  aux_sym_slot_token4 = 267,
  aux_sym_slot_token5 = 268,
  aux_sym_slot_token6 = 269,
  aux_sym_slot_token7 = 270,
  anon_sym_contents = 271,
  anon_sym_player_DOTcursor = 272,
  aux_sym_slot_token8 = 273,
  aux_sym_word_token1 = 274,
  anon_sym_DQUOTE = 275,
  aux_sym_string_token1 = 276,
  anon_sym_SQUOTE = 277,
  aux_sym_string_token2 = 278,
  sym_boolean = 279,
  sym_range = 280,
  aux_sym_nbt_identifier_token1 = 281,
  anon_sym_LBRACE = 282,
  anon_sym_COLON = 283,
  anon_sym_RBRACE = 284,
  anon_sym_LBRACK = 285,
  anon_sym_RBRACK = 286,
  sym_source_file = 287,
  sym__space = 288,
  sym__statement = 289,
  sym_command = 290,
  sym__execute_command = 291,
  sym__subcommand = 292,
  sym__generic_command = 293,
  sym_command_identifier = 294,
  sym_subcommand_identifier = 295,
  sym_argument = 296,
  sym_selector = 297,
  sym_selector_query = 298,
  sym_position = 299,
  sym_rotation = 300,
  sym_scale = 301,
  sym_macro = 302,
  sym_operator = 303,
  sym_slot = 304,
  sym_word = 305,
  sym_string = 306,
  sym__nbt_element = 307,
  sym_nbt_identifier = 308,
  sym_nbt_compound = 309,
  sym_nbt_array = 310,
  sym_selector_compound = 311,
  sym_item_compound = 312,
  aux_sym_source_file_repeat1 = 313,
  aux_sym__execute_command_repeat1 = 314,
  aux_sym__generic_command_repeat1 = 315,
  aux_sym_selector_repeat1 = 316,
  aux_sym_selector_query_repeat1 = 317,
  aux_sym_nbt_compound_repeat1 = 318,
  aux_sym_nbt_array_repeat1 = 319,
  aux_sym_selector_compound_repeat1 = 320,
  aux_sym_item_compound_repeat1 = 321,
};

static const char * const ts_symbol_names[] = {
  [ts_builtin_sym_end] = "end",
  [sym__newline] = "_newline",
  [aux_sym__space_token1] = "_space_token1",
  [sym_backslash] = "backslash",
  [aux_sym__statement_token1] = "_statement_token1",
  [sym_comment] = "comment",
  [anon_sym_execute] = "execute",
  [anon_sym_run] = "run",
  [sym_macro_indicator] = "macro_indicator",
  [anon_sym_advancement] = "advancement",
  [anon_sym_attribute] = "attribute",
  [anon_sym_ban] = "ban",
  [anon_sym_ban_DASHip] = "ban-ip",
  [anon_sym_banlist] = "banlist",
  [anon_sym_bossbar] = "bossbar",
  [anon_sym_clear] = "clear",
  [anon_sym_clone] = "clone",
  [anon_sym_damage] = "damage",
  [anon_sym_data] = "data",
  [anon_sym_datapack] = "datapack",
  [anon_sym_debug] = "debug",
  [anon_sym_defaultgamemode] = "defaultgamemode",
  [anon_sym_deop] = "deop",
  [anon_sym_dialog] = "dialog",
  [anon_sym_difficulty] = "difficulty",
  [anon_sym_effect] = "effect",
  [anon_sym_enchant] = "enchant",
  [anon_sym_experience] = "experience",
  [anon_sym_fetchprofile] = "fetchprofile",
  [anon_sym_fill] = "fill",
  [anon_sym_fillbiome] = "fillbiome",
  [anon_sym_forceload] = "forceload",
  [anon_sym_function] = "function",
  [anon_sym_gamemode] = "gamemode",
  [anon_sym_gamerule] = "gamerule",
  [anon_sym_give] = "give",
  [anon_sym_help] = "help",
  [anon_sym_item] = "item",
  [anon_sym_jfr] = "jfr",
  [anon_sym_kick] = "kick",
  [anon_sym_kill] = "kill",
  [anon_sym_list] = "list",
  [anon_sym_locate] = "locate",
  [anon_sym_loot] = "loot",
  [anon_sym_me] = "me",
  [anon_sym_msg] = "msg",
  [anon_sym_op] = "op",
  [anon_sym_pardon] = "pardon",
  [anon_sym_pardon_DASHip] = "pardon-ip",
  [anon_sym_particle] = "particle",
  [anon_sym_perf] = "perf",
  [anon_sym_place] = "place",
  [anon_sym_playsound] = "playsound",
  [anon_sym_publish] = "publish",
  [anon_sym_random] = "random",
  [anon_sym_recipe] = "recipe",
  [anon_sym_reload] = "reload",
  [anon_sym_return] = "return",
  [anon_sym_ride] = "ride",
  [anon_sym_rotate] = "rotate",
  [anon_sym_save_DASHall] = "save-all",
  [anon_sym_save_DASHoff] = "save-off",
  [anon_sym_save_DASHon] = "save-on",
  [anon_sym_say] = "say",
  [anon_sym_schedule] = "schedule",
  [anon_sym_scoreboard] = "scoreboard",
  [anon_sym_seed] = "seed",
  [anon_sym_setblock] = "setblock",
  [anon_sym_setidletimeout] = "setidletimeout",
  [anon_sym_setworldspawn] = "setworldspawn",
  [anon_sym_spawnpoint] = "spawnpoint",
  [anon_sym_spectate] = "spectate",
  [anon_sym_spreadplayers] = "spreadplayers",
  [anon_sym_stop] = "stop",
  [anon_sym_stopsound] = "stopsound",
  [anon_sym_summon] = "summon",
  [anon_sym_swing] = "swing",
  [anon_sym_tag] = "tag",
  [anon_sym_team] = "team",
  [anon_sym_teammsg] = "teammsg",
  [anon_sym_teleport] = "teleport",
  [anon_sym_tell] = "tell",
  [anon_sym_tellraw] = "tellraw",
  [anon_sym_test] = "test",
  [anon_sym_time] = "time",
  [anon_sym_title] = "title",
  [anon_sym_tm] = "tm",
  [anon_sym_tp] = "tp",
  [anon_sym_transfer] = "transfer",
  [anon_sym_trigger] = "trigger",
  [anon_sym_version] = "version",
  [anon_sym_w] = "w",
  [anon_sym_waypoint] = "waypoint",
  [anon_sym_weather] = "weather",
  [anon_sym_whitelist] = "whitelist",
  [anon_sym_worldborder] = "worldborder",
  [anon_sym_xp] = "xp",
  [anon_sym_align] = "align",
  [anon_sym_all] = "all",
  [anon_sym_anchored] = "anchored",
  [anon_sym_as] = "as",
  [anon_sym_at] = "at",
  [anon_sym_attacker] = "attacker",
  [anon_sym_biome] = "biome",
  [anon_sym_block] = "block",
  [anon_sym_blocks] = "blocks",
  [anon_sym_controller] = "controller",
  [anon_sym_dimension] = "dimension",
  [anon_sym_entity] = "entity",
  [anon_sym_eyes] = "eyes",
  [anon_sym_facing] = "facing",
  [anon_sym_feet] = "feet",
  [anon_sym_if] = "if",
  [anon_sym_in] = "in",
  [anon_sym_items] = "items",
  [anon_sym_leasher] = "leasher",
  [anon_sym_loaded] = "loaded",
  [anon_sym_masked] = "masked",
  [anon_sym_matches] = "matches",
  [anon_sym_max] = "max",
  [anon_sym_on] = "on",
  [anon_sym_origin] = "origin",
  [anon_sym_over] = "over",
  [anon_sym_owner] = "owner",
  [anon_sym_passengers] = "passengers",
  [anon_sym_positioned] = "positioned",
  [anon_sym_predicate] = "predicate",
  [anon_sym_result] = "result",
  [anon_sym_rotated] = "rotated",
  [anon_sym_score] = "score",
  [anon_sym_storage] = "storage",
  [anon_sym_store] = "store",
  [anon_sym_success] = "success",
  [anon_sym_target] = "target",
  [anon_sym_unless] = "unless",
  [anon_sym_value] = "value",
  [anon_sym_vehicle] = "vehicle",
  [anon_sym_add] = "add",
  [anon_sym_add_multiplied_base] = "add_multiplied_base",
  [anon_sym_add_multiplied_total] = "add_multiplied_total",
  [anon_sym_add_value] = "add_value",
  [anon_sym_amount] = "amount",
  [anon_sym_append] = "append",
  [anon_sym_base] = "base",
  [anon_sym_buffer] = "buffer",
  [anon_sym_by] = "by",
  [anon_sym_center] = "center",
  [anon_sym_color] = "color",
  [anon_sym_create] = "create",
  [anon_sym_day] = "day",
  [anon_sym_daytime] = "daytime",
  [anon_sym_destroy] = "destroy",
  [anon_sym_disable] = "disable",
  [anon_sym_dismount] = "dismount",
  [anon_sym_distance] = "distance",
  [anon_sym_enable] = "enable",
  [anon_sym_everything] = "everything",
  [anon_sym_export] = "export",
  [anon_sym_exportclosest] = "exportclosest",
  [anon_sym_exportthat] = "exportthat",
  [anon_sym_exportthese] = "exportthese",
  [anon_sym_fail] = "fail",
  [anon_sym_feature] = "feature",
  [anon_sym_fish] = "fish",
  [anon_sym_flush] = "flush",
  [anon_sym_force] = "force",
  [anon_sym_from] = "from",
  [anon_sym_gametime] = "gametime",
  [anon_sym_get] = "get",
  [anon_sym_grant] = "grant",
  [anon_sym_hollow] = "hollow",
  [anon_sym_id] = "id",
  [anon_sym_insert] = "insert",
  [anon_sym_ips] = "ips",
  [anon_sym_jigsaw] = "jigsaw",
  [anon_sym_join] = "join",
  [anon_sym_keep] = "keep",
  [anon_sym_leave] = "leave",
  [anon_sym_levels] = "levels",
  [anon_sym_mainhand] = "mainhand",
  [anon_sym_merge] = "merge",
  [anon_sym_midnight] = "midnight",
  [anon_sym_mine] = "mine",
  [anon_sym_modifier] = "modifier",
  [anon_sym_modify] = "modify",
  [anon_sym_mount] = "mount",
  [anon_sym_move] = "move",
  [anon_sym_name] = "name",
  [anon_sym_night] = "night",
  [anon_sym_noon] = "noon",
  [anon_sym_normal] = "normal",
  [anon_sym_off] = "off",
  [anon_sym_offhand] = "offhand",
  [anon_sym_only] = "only",
  [anon_sym_outline] = "outline",
  [anon_sym_players] = "players",
  [anon_sym_poi] = "poi",
  [anon_sym_points] = "points",
  [anon_sym_pos] = "pos",
  [anon_sym_prepend] = "prepend",
  [anon_sym_query] = "query",
  [anon_sym_rain] = "rain",
  [anon_sym_remove] = "remove",
  [anon_sym_replace] = "replace",
  [anon_sym_reset] = "reset",
  [anon_sym_resetclosest] = "resetclosest",
  [anon_sym_resetthat] = "resetthat",
  [anon_sym_resetthese] = "resetthese",
  [anon_sym_revoke] = "revoke",
  [anon_sym_roll] = "roll",
  [anon_sym_runclosest] = "runclosest",
  [anon_sym_runfailed] = "runfailed",
  [anon_sym_runthat] = "runthat",
  [anon_sym_runthese] = "runthese",
  [anon_sym_set] = "set",
  [anon_sym_setdisplay] = "setdisplay",
  [anon_sym_show] = "show",
  [anon_sym_spawn] = "spawn",
  [anon_sym_start] = "start",
  [anon_sym_strict] = "strict",
  [anon_sym_string] = "string",
  [anon_sym_structure] = "structure",
  [anon_sym_style] = "style",
  [anon_sym_take] = "take",
  [anon_sym_template] = "template",
  [anon_sym_through] = "through",
  [anon_sym_thunder] = "thunder",
  [anon_sym_times] = "times",
  [anon_sym_to] = "to",
  [anon_sym_under] = "under",
  [anon_sym_until] = "until",
  [anon_sym_uuids] = "uuids",
  [anon_sym_verify] = "verify",
  [anon_sym_visible] = "visible",
  [anon_sym_warning] = "warning",
  [anon_sym_with] = "with",
  [aux_sym_selector_token1] = "selector_token1",
  [aux_sym_selector_token2] = "selector_token2",
  [sym_selector_identifier] = "selector_identifier",
  [sym_query_identifier] = "query_identifier",
  [anon_sym_EQ] = "=",
  [anon_sym_EQ_BANG] = "=!",
  [aux_sym_selector_query_token1] = "selector_query_token1",
  [anon_sym_TILDE] = "~",
  [anon_sym_CARET] = "^",
  [sym_heightmap] = "heightmap",
  [sym_resource] = "resource",
  [aux_sym_scale_token1] = "scale_token1",
  [aux_sym_scale_token2] = "scale_token2",
  [aux_sym_scale_token3] = "scale_token3",
  [aux_sym_scale_token4] = "scale_token4",
  [aux_sym_scale_token5] = "scale_token5",
  [anon_sym_DOLLAR_LPAREN] = "$(",
  [aux_sym_macro_token1] = "macro_token1",
  [anon_sym_RPAREN] = ")",
  [sym_type] = "type",
  [anon_sym_PLUS_EQ] = "+=",
  [anon_sym_DASH_EQ] = "-=",
  [anon_sym_STAR_EQ] = "*=",
  [anon_sym_SLASH_EQ] = "/=",
  [anon_sym_PERCENT_EQ] = "%=",
  [anon_sym_GT_LT] = "><",
  [anon_sym_LT] = "<",
  [anon_sym_GT] = ">",
  [aux_sym_slot_token1] = "slot_token1",
  [aux_sym_slot_token2] = "slot_token2",
  [aux_sym_slot_token3] = "slot_token3",
  [aux_sym_slot_token4] = "slot_token4",
  [aux_sym_slot_token5] = "slot_token5",
  [aux_sym_slot_token6] = "slot_token6",
  [aux_sym_slot_token7] = "slot_token7",
  [anon_sym_contents] = "contents",
  [anon_sym_player_DOTcursor] = "player.cursor",
  [aux_sym_slot_token8] = "slot_token8",
  [aux_sym_word_token1] = "word_token1",
  [anon_sym_DQUOTE] = "\"",
  [aux_sym_string_token1] = "string_token1",
  [anon_sym_SQUOTE] = "'",
  [aux_sym_string_token2] = "string_token2",
  [sym_boolean] = "boolean",
  [sym_range] = "range",
  [aux_sym_nbt_identifier_token1] = "nbt_identifier_token1",
  [anon_sym_LBRACE] = "{",
  [anon_sym_COLON] = ":",
  [anon_sym_RBRACE] = "}",
  [anon_sym_LBRACK] = "[",
  [anon_sym_RBRACK] = "]",
  [sym_source_file] = "source_file",
  [sym__space] = "_space",
  [sym__statement] = "_statement",
  [sym_command] = "command",
  [sym__execute_command] = "_execute_command",
  [sym__subcommand] = "_subcommand",
  [sym__generic_command] = "_generic_command",
  [sym_command_identifier] = "command_identifier",
  [sym_subcommand_identifier] = "subcommand_identifier",
  [sym_argument] = "argument",
  [sym_selector] = "selector",
  [sym_selector_query] = "selector_query",
  [sym_position] = "position",
  [sym_rotation] = "rotation",
  [sym_scale] = "scale",
  [sym_macro] = "macro",
  [sym_operator] = "operator",
  [sym_slot] = "slot",
  [sym_word] = "word",
  [sym_string] = "string",
  [sym__nbt_element] = "_nbt_element",
  [sym_nbt_identifier] = "nbt_identifier",
  [sym_nbt_compound] = "nbt_compound",
  [sym_nbt_array] = "nbt_array",
  [sym_selector_compound] = "selector_compound",
  [sym_item_compound] = "item_compound",
  [aux_sym_source_file_repeat1] = "source_file_repeat1",
  [aux_sym__execute_command_repeat1] = "_execute_command_repeat1",
  [aux_sym__generic_command_repeat1] = "_generic_command_repeat1",
  [aux_sym_selector_repeat1] = "selector_repeat1",
  [aux_sym_selector_query_repeat1] = "selector_query_repeat1",
  [aux_sym_nbt_compound_repeat1] = "nbt_compound_repeat1",
  [aux_sym_nbt_array_repeat1] = "nbt_array_repeat1",
  [aux_sym_selector_compound_repeat1] = "selector_compound_repeat1",
  [aux_sym_item_compound_repeat1] = "item_compound_repeat1",
};

static const TSSymbol ts_symbol_map[] = {
  [ts_builtin_sym_end] = ts_builtin_sym_end,
  [sym__newline] = sym__newline,
  [aux_sym__space_token1] = aux_sym__space_token1,
  [sym_backslash] = sym_backslash,
  [aux_sym__statement_token1] = aux_sym__statement_token1,
  [sym_comment] = sym_comment,
  [anon_sym_execute] = anon_sym_execute,
  [anon_sym_run] = anon_sym_run,
  [sym_macro_indicator] = sym_macro_indicator,
  [anon_sym_advancement] = anon_sym_advancement,
  [anon_sym_attribute] = anon_sym_attribute,
  [anon_sym_ban] = anon_sym_ban,
  [anon_sym_ban_DASHip] = anon_sym_ban_DASHip,
  [anon_sym_banlist] = anon_sym_banlist,
  [anon_sym_bossbar] = anon_sym_bossbar,
  [anon_sym_clear] = anon_sym_clear,
  [anon_sym_clone] = anon_sym_clone,
  [anon_sym_damage] = anon_sym_damage,
  [anon_sym_data] = anon_sym_data,
  [anon_sym_datapack] = anon_sym_datapack,
  [anon_sym_debug] = anon_sym_debug,
  [anon_sym_defaultgamemode] = anon_sym_defaultgamemode,
  [anon_sym_deop] = anon_sym_deop,
  [anon_sym_dialog] = anon_sym_dialog,
  [anon_sym_difficulty] = anon_sym_difficulty,
  [anon_sym_effect] = anon_sym_effect,
  [anon_sym_enchant] = anon_sym_enchant,
  [anon_sym_experience] = anon_sym_experience,
  [anon_sym_fetchprofile] = anon_sym_fetchprofile,
  [anon_sym_fill] = anon_sym_fill,
  [anon_sym_fillbiome] = anon_sym_fillbiome,
  [anon_sym_forceload] = anon_sym_forceload,
  [anon_sym_function] = anon_sym_function,
  [anon_sym_gamemode] = anon_sym_gamemode,
  [anon_sym_gamerule] = anon_sym_gamerule,
  [anon_sym_give] = anon_sym_give,
  [anon_sym_help] = anon_sym_help,
  [anon_sym_item] = anon_sym_item,
  [anon_sym_jfr] = anon_sym_jfr,
  [anon_sym_kick] = anon_sym_kick,
  [anon_sym_kill] = anon_sym_kill,
  [anon_sym_list] = anon_sym_list,
  [anon_sym_locate] = anon_sym_locate,
  [anon_sym_loot] = anon_sym_loot,
  [anon_sym_me] = anon_sym_me,
  [anon_sym_msg] = anon_sym_msg,
  [anon_sym_op] = anon_sym_op,
  [anon_sym_pardon] = anon_sym_pardon,
  [anon_sym_pardon_DASHip] = anon_sym_pardon_DASHip,
  [anon_sym_particle] = anon_sym_particle,
  [anon_sym_perf] = anon_sym_perf,
  [anon_sym_place] = anon_sym_place,
  [anon_sym_playsound] = anon_sym_playsound,
  [anon_sym_publish] = anon_sym_publish,
  [anon_sym_random] = anon_sym_random,
  [anon_sym_recipe] = anon_sym_recipe,
  [anon_sym_reload] = anon_sym_reload,
  [anon_sym_return] = anon_sym_return,
  [anon_sym_ride] = anon_sym_ride,
  [anon_sym_rotate] = anon_sym_rotate,
  [anon_sym_save_DASHall] = anon_sym_save_DASHall,
  [anon_sym_save_DASHoff] = anon_sym_save_DASHoff,
  [anon_sym_save_DASHon] = anon_sym_save_DASHon,
  [anon_sym_say] = anon_sym_say,
  [anon_sym_schedule] = anon_sym_schedule,
  [anon_sym_scoreboard] = anon_sym_scoreboard,
  [anon_sym_seed] = anon_sym_seed,
  [anon_sym_setblock] = anon_sym_setblock,
  [anon_sym_setidletimeout] = anon_sym_setidletimeout,
  [anon_sym_setworldspawn] = anon_sym_setworldspawn,
  [anon_sym_spawnpoint] = anon_sym_spawnpoint,
  [anon_sym_spectate] = anon_sym_spectate,
  [anon_sym_spreadplayers] = anon_sym_spreadplayers,
  [anon_sym_stop] = anon_sym_stop,
  [anon_sym_stopsound] = anon_sym_stopsound,
  [anon_sym_summon] = anon_sym_summon,
  [anon_sym_swing] = anon_sym_swing,
  [anon_sym_tag] = anon_sym_tag,
  [anon_sym_team] = anon_sym_team,
  [anon_sym_teammsg] = anon_sym_teammsg,
  [anon_sym_teleport] = anon_sym_teleport,
  [anon_sym_tell] = anon_sym_tell,
  [anon_sym_tellraw] = anon_sym_tellraw,
  [anon_sym_test] = anon_sym_test,
  [anon_sym_time] = anon_sym_time,
  [anon_sym_title] = anon_sym_title,
  [anon_sym_tm] = anon_sym_tm,
  [anon_sym_tp] = anon_sym_tp,
  [anon_sym_transfer] = anon_sym_transfer,
  [anon_sym_trigger] = anon_sym_trigger,
  [anon_sym_version] = anon_sym_version,
  [anon_sym_w] = anon_sym_w,
  [anon_sym_waypoint] = anon_sym_waypoint,
  [anon_sym_weather] = anon_sym_weather,
  [anon_sym_whitelist] = anon_sym_whitelist,
  [anon_sym_worldborder] = anon_sym_worldborder,
  [anon_sym_xp] = anon_sym_xp,
  [anon_sym_align] = anon_sym_align,
  [anon_sym_all] = anon_sym_all,
  [anon_sym_anchored] = anon_sym_anchored,
  [anon_sym_as] = anon_sym_as,
  [anon_sym_at] = anon_sym_at,
  [anon_sym_attacker] = anon_sym_attacker,
  [anon_sym_biome] = anon_sym_biome,
  [anon_sym_block] = anon_sym_block,
  [anon_sym_blocks] = anon_sym_blocks,
  [anon_sym_controller] = anon_sym_controller,
  [anon_sym_dimension] = anon_sym_dimension,
  [anon_sym_entity] = anon_sym_entity,
  [anon_sym_eyes] = anon_sym_eyes,
  [anon_sym_facing] = anon_sym_facing,
  [anon_sym_feet] = anon_sym_feet,
  [anon_sym_if] = anon_sym_if,
  [anon_sym_in] = anon_sym_in,
  [anon_sym_items] = anon_sym_items,
  [anon_sym_leasher] = anon_sym_leasher,
  [anon_sym_loaded] = anon_sym_loaded,
  [anon_sym_masked] = anon_sym_masked,
  [anon_sym_matches] = anon_sym_matches,
  [anon_sym_max] = anon_sym_max,
  [anon_sym_on] = anon_sym_on,
  [anon_sym_origin] = anon_sym_origin,
  [anon_sym_over] = anon_sym_over,
  [anon_sym_owner] = anon_sym_owner,
  [anon_sym_passengers] = anon_sym_passengers,
  [anon_sym_positioned] = anon_sym_positioned,
  [anon_sym_predicate] = anon_sym_predicate,
  [anon_sym_result] = anon_sym_result,
  [anon_sym_rotated] = anon_sym_rotated,
  [anon_sym_score] = anon_sym_score,
  [anon_sym_storage] = anon_sym_storage,
  [anon_sym_store] = anon_sym_store,
  [anon_sym_success] = anon_sym_success,
  [anon_sym_target] = anon_sym_target,
  [anon_sym_unless] = anon_sym_unless,
  [anon_sym_value] = anon_sym_value,
  [anon_sym_vehicle] = anon_sym_vehicle,
  [anon_sym_add] = anon_sym_add,
  [anon_sym_add_multiplied_base] = anon_sym_add_multiplied_base,
  [anon_sym_add_multiplied_total] = anon_sym_add_multiplied_total,
  [anon_sym_add_value] = anon_sym_add_value,
  [anon_sym_amount] = anon_sym_amount,
  [anon_sym_append] = anon_sym_append,
  [anon_sym_base] = anon_sym_base,
  [anon_sym_buffer] = anon_sym_buffer,
  [anon_sym_by] = anon_sym_by,
  [anon_sym_center] = anon_sym_center,
  [anon_sym_color] = anon_sym_color,
  [anon_sym_create] = anon_sym_create,
  [anon_sym_day] = anon_sym_day,
  [anon_sym_daytime] = anon_sym_daytime,
  [anon_sym_destroy] = anon_sym_destroy,
  [anon_sym_disable] = anon_sym_disable,
  [anon_sym_dismount] = anon_sym_dismount,
  [anon_sym_distance] = anon_sym_distance,
  [anon_sym_enable] = anon_sym_enable,
  [anon_sym_everything] = anon_sym_everything,
  [anon_sym_export] = anon_sym_export,
  [anon_sym_exportclosest] = anon_sym_exportclosest,
  [anon_sym_exportthat] = anon_sym_exportthat,
  [anon_sym_exportthese] = anon_sym_exportthese,
  [anon_sym_fail] = anon_sym_fail,
  [anon_sym_feature] = anon_sym_feature,
  [anon_sym_fish] = anon_sym_fish,
  [anon_sym_flush] = anon_sym_flush,
  [anon_sym_force] = anon_sym_force,
  [anon_sym_from] = anon_sym_from,
  [anon_sym_gametime] = anon_sym_gametime,
  [anon_sym_get] = anon_sym_get,
  [anon_sym_grant] = anon_sym_grant,
  [anon_sym_hollow] = anon_sym_hollow,
  [anon_sym_id] = anon_sym_id,
  [anon_sym_insert] = anon_sym_insert,
  [anon_sym_ips] = anon_sym_ips,
  [anon_sym_jigsaw] = anon_sym_jigsaw,
  [anon_sym_join] = anon_sym_join,
  [anon_sym_keep] = anon_sym_keep,
  [anon_sym_leave] = anon_sym_leave,
  [anon_sym_levels] = anon_sym_levels,
  [anon_sym_mainhand] = anon_sym_mainhand,
  [anon_sym_merge] = anon_sym_merge,
  [anon_sym_midnight] = anon_sym_midnight,
  [anon_sym_mine] = anon_sym_mine,
  [anon_sym_modifier] = anon_sym_modifier,
  [anon_sym_modify] = anon_sym_modify,
  [anon_sym_mount] = anon_sym_mount,
  [anon_sym_move] = anon_sym_move,
  [anon_sym_name] = anon_sym_name,
  [anon_sym_night] = anon_sym_night,
  [anon_sym_noon] = anon_sym_noon,
  [anon_sym_normal] = anon_sym_normal,
  [anon_sym_off] = anon_sym_off,
  [anon_sym_offhand] = anon_sym_offhand,
  [anon_sym_only] = anon_sym_only,
  [anon_sym_outline] = anon_sym_outline,
  [anon_sym_players] = anon_sym_players,
  [anon_sym_poi] = anon_sym_poi,
  [anon_sym_points] = anon_sym_points,
  [anon_sym_pos] = anon_sym_pos,
  [anon_sym_prepend] = anon_sym_prepend,
  [anon_sym_query] = anon_sym_query,
  [anon_sym_rain] = anon_sym_rain,
  [anon_sym_remove] = anon_sym_remove,
  [anon_sym_replace] = anon_sym_replace,
  [anon_sym_reset] = anon_sym_reset,
  [anon_sym_resetclosest] = anon_sym_resetclosest,
  [anon_sym_resetthat] = anon_sym_resetthat,
  [anon_sym_resetthese] = anon_sym_resetthese,
  [anon_sym_revoke] = anon_sym_revoke,
  [anon_sym_roll] = anon_sym_roll,
  [anon_sym_runclosest] = anon_sym_runclosest,
  [anon_sym_runfailed] = anon_sym_runfailed,
  [anon_sym_runthat] = anon_sym_runthat,
  [anon_sym_runthese] = anon_sym_runthese,
  [anon_sym_set] = anon_sym_set,
  [anon_sym_setdisplay] = anon_sym_setdisplay,
  [anon_sym_show] = anon_sym_show,
  [anon_sym_spawn] = anon_sym_spawn,
  [anon_sym_start] = anon_sym_start,
  [anon_sym_strict] = anon_sym_strict,
  [anon_sym_string] = anon_sym_string,
  [anon_sym_structure] = anon_sym_structure,
  [anon_sym_style] = anon_sym_style,
  [anon_sym_take] = anon_sym_take,
  [anon_sym_template] = anon_sym_template,
  [anon_sym_through] = anon_sym_through,
  [anon_sym_thunder] = anon_sym_thunder,
  [anon_sym_times] = anon_sym_times,
  [anon_sym_to] = anon_sym_to,
  [anon_sym_under] = anon_sym_under,
  [anon_sym_until] = anon_sym_until,
  [anon_sym_uuids] = anon_sym_uuids,
  [anon_sym_verify] = anon_sym_verify,
  [anon_sym_visible] = anon_sym_visible,
  [anon_sym_warning] = anon_sym_warning,
  [anon_sym_with] = anon_sym_with,
  [aux_sym_selector_token1] = aux_sym_selector_token1,
  [aux_sym_selector_token2] = aux_sym_selector_token2,
  [sym_selector_identifier] = sym_selector_identifier,
  [sym_query_identifier] = sym_query_identifier,
  [anon_sym_EQ] = anon_sym_EQ,
  [anon_sym_EQ_BANG] = anon_sym_EQ_BANG,
  [aux_sym_selector_query_token1] = aux_sym_selector_query_token1,
  [anon_sym_TILDE] = anon_sym_TILDE,
  [anon_sym_CARET] = anon_sym_CARET,
  [sym_heightmap] = sym_heightmap,
  [sym_resource] = sym_resource,
  [aux_sym_scale_token1] = aux_sym_scale_token1,
  [aux_sym_scale_token2] = aux_sym_scale_token2,
  [aux_sym_scale_token3] = aux_sym_scale_token3,
  [aux_sym_scale_token4] = aux_sym_scale_token4,
  [aux_sym_scale_token5] = aux_sym_scale_token5,
  [anon_sym_DOLLAR_LPAREN] = anon_sym_DOLLAR_LPAREN,
  [aux_sym_macro_token1] = aux_sym_macro_token1,
  [anon_sym_RPAREN] = anon_sym_RPAREN,
  [sym_type] = sym_type,
  [anon_sym_PLUS_EQ] = anon_sym_PLUS_EQ,
  [anon_sym_DASH_EQ] = anon_sym_DASH_EQ,
  [anon_sym_STAR_EQ] = anon_sym_STAR_EQ,
  [anon_sym_SLASH_EQ] = anon_sym_SLASH_EQ,
  [anon_sym_PERCENT_EQ] = anon_sym_PERCENT_EQ,
  [anon_sym_GT_LT] = anon_sym_GT_LT,
  [anon_sym_LT] = anon_sym_LT,
  [anon_sym_GT] = anon_sym_GT,
  [aux_sym_slot_token1] = aux_sym_slot_token1,
  [aux_sym_slot_token2] = aux_sym_slot_token2,
  [aux_sym_slot_token3] = aux_sym_slot_token3,
  [aux_sym_slot_token4] = aux_sym_slot_token4,
  [aux_sym_slot_token5] = aux_sym_slot_token5,
  [aux_sym_slot_token6] = aux_sym_slot_token6,
  [aux_sym_slot_token7] = aux_sym_slot_token7,
  [anon_sym_contents] = anon_sym_contents,
  [anon_sym_player_DOTcursor] = anon_sym_player_DOTcursor,
  [aux_sym_slot_token8] = aux_sym_slot_token8,
  [aux_sym_word_token1] = aux_sym_word_token1,
  [anon_sym_DQUOTE] = anon_sym_DQUOTE,
  [aux_sym_string_token1] = aux_sym_string_token1,
  [anon_sym_SQUOTE] = anon_sym_SQUOTE,
  [aux_sym_string_token2] = aux_sym_string_token2,
  [sym_boolean] = sym_boolean,
  [sym_range] = sym_range,
  [aux_sym_nbt_identifier_token1] = aux_sym_nbt_identifier_token1,
  [anon_sym_LBRACE] = anon_sym_LBRACE,
  [anon_sym_COLON] = anon_sym_COLON,
  [anon_sym_RBRACE] = anon_sym_RBRACE,
  [anon_sym_LBRACK] = anon_sym_LBRACK,
  [anon_sym_RBRACK] = anon_sym_RBRACK,
  [sym_source_file] = sym_source_file,
  [sym__space] = sym__space,
  [sym__statement] = sym__statement,
  [sym_command] = sym_command,
  [sym__execute_command] = sym__execute_command,
  [sym__subcommand] = sym__subcommand,
  [sym__generic_command] = sym__generic_command,
  [sym_command_identifier] = sym_command_identifier,
  [sym_subcommand_identifier] = sym_subcommand_identifier,
  [sym_argument] = sym_argument,
  [sym_selector] = sym_selector,
  [sym_selector_query] = sym_selector_query,
  [sym_position] = sym_position,
  [sym_rotation] = sym_rotation,
  [sym_scale] = sym_scale,
  [sym_macro] = sym_macro,
  [sym_operator] = sym_operator,
  [sym_slot] = sym_slot,
  [sym_word] = sym_word,
  [sym_string] = sym_string,
  [sym__nbt_element] = sym__nbt_element,
  [sym_nbt_identifier] = sym_nbt_identifier,
  [sym_nbt_compound] = sym_nbt_compound,
  [sym_nbt_array] = sym_nbt_array,
  [sym_selector_compound] = sym_selector_compound,
  [sym_item_compound] = sym_item_compound,
  [aux_sym_source_file_repeat1] = aux_sym_source_file_repeat1,
  [aux_sym__execute_command_repeat1] = aux_sym__execute_command_repeat1,
  [aux_sym__generic_command_repeat1] = aux_sym__generic_command_repeat1,
  [aux_sym_selector_repeat1] = aux_sym_selector_repeat1,
  [aux_sym_selector_query_repeat1] = aux_sym_selector_query_repeat1,
  [aux_sym_nbt_compound_repeat1] = aux_sym_nbt_compound_repeat1,
  [aux_sym_nbt_array_repeat1] = aux_sym_nbt_array_repeat1,
  [aux_sym_selector_compound_repeat1] = aux_sym_selector_compound_repeat1,
  [aux_sym_item_compound_repeat1] = aux_sym_item_compound_repeat1,
};

static const TSSymbolMetadata ts_symbol_metadata[] = {
  [ts_builtin_sym_end] = {
    .visible = false,
    .named = true,
  },
  [sym__newline] = {
    .visible = false,
    .named = true,
  },
  [aux_sym__space_token1] = {
    .visible = false,
    .named = false,
  },
  [sym_backslash] = {
    .visible = true,
    .named = true,
  },
  [aux_sym__statement_token1] = {
    .visible = false,
    .named = false,
  },
  [sym_comment] = {
    .visible = true,
    .named = true,
  },
  [anon_sym_execute] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_run] = {
    .visible = true,
    .named = false,
  },
  [sym_macro_indicator] = {
    .visible = true,
    .named = true,
  },
  [anon_sym_advancement] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_attribute] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_ban] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_ban_DASHip] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_banlist] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_bossbar] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_clear] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_clone] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_damage] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_data] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_datapack] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_debug] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_defaultgamemode] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_deop] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_dialog] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_difficulty] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_effect] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_enchant] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_experience] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_fetchprofile] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_fill] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_fillbiome] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_forceload] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_function] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_gamemode] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_gamerule] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_give] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_help] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_item] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_jfr] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_kick] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_kill] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_list] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_locate] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_loot] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_me] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_msg] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_op] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_pardon] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_pardon_DASHip] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_particle] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_perf] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_place] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_playsound] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_publish] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_random] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_recipe] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_reload] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_return] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_ride] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_rotate] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_save_DASHall] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_save_DASHoff] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_save_DASHon] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_say] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_schedule] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_scoreboard] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_seed] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_setblock] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_setidletimeout] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_setworldspawn] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_spawnpoint] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_spectate] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_spreadplayers] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_stop] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_stopsound] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_summon] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_swing] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_tag] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_team] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_teammsg] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_teleport] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_tell] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_tellraw] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_test] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_time] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_title] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_tm] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_tp] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_transfer] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_trigger] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_version] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_w] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_waypoint] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_weather] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_whitelist] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_worldborder] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_xp] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_align] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_all] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_anchored] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_as] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_at] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_attacker] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_biome] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_block] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_blocks] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_controller] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_dimension] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_entity] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_eyes] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_facing] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_feet] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_if] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_in] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_items] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_leasher] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_loaded] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_masked] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_matches] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_max] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_on] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_origin] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_over] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_owner] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_passengers] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_positioned] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_predicate] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_result] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_rotated] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_score] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_storage] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_store] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_success] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_target] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_unless] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_value] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_vehicle] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_add] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_add_multiplied_base] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_add_multiplied_total] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_add_value] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_amount] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_append] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_base] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_buffer] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_by] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_center] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_color] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_create] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_day] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_daytime] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_destroy] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_disable] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_dismount] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_distance] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_enable] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_everything] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_export] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_exportclosest] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_exportthat] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_exportthese] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_fail] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_feature] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_fish] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_flush] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_force] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_from] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_gametime] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_get] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_grant] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_hollow] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_id] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_insert] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_ips] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_jigsaw] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_join] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_keep] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_leave] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_levels] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_mainhand] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_merge] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_midnight] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_mine] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_modifier] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_modify] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_mount] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_move] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_name] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_night] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_noon] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_normal] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_off] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_offhand] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_only] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_outline] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_players] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_poi] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_points] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_pos] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_prepend] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_query] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_rain] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_remove] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_replace] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_reset] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_resetclosest] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_resetthat] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_resetthese] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_revoke] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_roll] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_runclosest] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_runfailed] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_runthat] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_runthese] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_set] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_setdisplay] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_show] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_spawn] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_start] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_strict] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_string] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_structure] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_style] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_take] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_template] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_through] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_thunder] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_times] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_to] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_under] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_until] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_uuids] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_verify] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_visible] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_warning] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_with] = {
    .visible = true,
    .named = false,
  },
  [aux_sym_selector_token1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_selector_token2] = {
    .visible = false,
    .named = false,
  },
  [sym_selector_identifier] = {
    .visible = true,
    .named = true,
  },
  [sym_query_identifier] = {
    .visible = true,
    .named = true,
  },
  [anon_sym_EQ] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_EQ_BANG] = {
    .visible = true,
    .named = false,
  },
  [aux_sym_selector_query_token1] = {
    .visible = false,
    .named = false,
  },
  [anon_sym_TILDE] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_CARET] = {
    .visible = true,
    .named = false,
  },
  [sym_heightmap] = {
    .visible = true,
    .named = true,
  },
  [sym_resource] = {
    .visible = true,
    .named = true,
  },
  [aux_sym_scale_token1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_scale_token2] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_scale_token3] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_scale_token4] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_scale_token5] = {
    .visible = false,
    .named = false,
  },
  [anon_sym_DOLLAR_LPAREN] = {
    .visible = true,
    .named = false,
  },
  [aux_sym_macro_token1] = {
    .visible = false,
    .named = false,
  },
  [anon_sym_RPAREN] = {
    .visible = true,
    .named = false,
  },
  [sym_type] = {
    .visible = true,
    .named = true,
  },
  [anon_sym_PLUS_EQ] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_DASH_EQ] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_STAR_EQ] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_SLASH_EQ] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_PERCENT_EQ] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_GT_LT] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_LT] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_GT] = {
    .visible = true,
    .named = false,
  },
  [aux_sym_slot_token1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_slot_token2] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_slot_token3] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_slot_token4] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_slot_token5] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_slot_token6] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_slot_token7] = {
    .visible = false,
    .named = false,
  },
  [anon_sym_contents] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_player_DOTcursor] = {
    .visible = true,
    .named = false,
  },
  [aux_sym_slot_token8] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_word_token1] = {
    .visible = false,
    .named = false,
  },
  [anon_sym_DQUOTE] = {
    .visible = true,
    .named = false,
  },
  [aux_sym_string_token1] = {
    .visible = false,
    .named = false,
  },
  [anon_sym_SQUOTE] = {
    .visible = true,
    .named = false,
  },
  [aux_sym_string_token2] = {
    .visible = false,
    .named = false,
  },
  [sym_boolean] = {
    .visible = true,
    .named = true,
  },
  [sym_range] = {
    .visible = true,
    .named = true,
  },
  [aux_sym_nbt_identifier_token1] = {
    .visible = false,
    .named = false,
  },
  [anon_sym_LBRACE] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_COLON] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_RBRACE] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_LBRACK] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_RBRACK] = {
    .visible = true,
    .named = false,
  },
  [sym_source_file] = {
    .visible = true,
    .named = true,
  },
  [sym__space] = {
    .visible = false,
    .named = true,
  },
  [sym__statement] = {
    .visible = false,
    .named = true,
  },
  [sym_command] = {
    .visible = true,
    .named = true,
  },
  [sym__execute_command] = {
    .visible = false,
    .named = true,
  },
  [sym__subcommand] = {
    .visible = false,
    .named = true,
  },
  [sym__generic_command] = {
    .visible = false,
    .named = true,
  },
  [sym_command_identifier] = {
    .visible = true,
    .named = true,
  },
  [sym_subcommand_identifier] = {
    .visible = true,
    .named = true,
  },
  [sym_argument] = {
    .visible = true,
    .named = true,
  },
  [sym_selector] = {
    .visible = true,
    .named = true,
  },
  [sym_selector_query] = {
    .visible = true,
    .named = true,
  },
  [sym_position] = {
    .visible = true,
    .named = true,
  },
  [sym_rotation] = {
    .visible = true,
    .named = true,
  },
  [sym_scale] = {
    .visible = true,
    .named = true,
  },
  [sym_macro] = {
    .visible = true,
    .named = true,
  },
  [sym_operator] = {
    .visible = true,
    .named = true,
  },
  [sym_slot] = {
    .visible = true,
    .named = true,
  },
  [sym_word] = {
    .visible = true,
    .named = true,
  },
  [sym_string] = {
    .visible = true,
    .named = true,
  },
  [sym__nbt_element] = {
    .visible = false,
    .named = true,
  },
  [sym_nbt_identifier] = {
    .visible = true,
    .named = true,
  },
  [sym_nbt_compound] = {
    .visible = true,
    .named = true,
  },
  [sym_nbt_array] = {
    .visible = true,
    .named = true,
  },
  [sym_selector_compound] = {
    .visible = true,
    .named = true,
  },
  [sym_item_compound] = {
    .visible = true,
    .named = true,
  },
  [aux_sym_source_file_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym__execute_command_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym__generic_command_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_selector_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_selector_query_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_nbt_compound_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_nbt_array_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_selector_compound_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_item_compound_repeat1] = {
    .visible = false,
    .named = false,
  },
};

static const TSSymbol ts_alias_sequences[PRODUCTION_ID_COUNT][MAX_ALIAS_SEQUENCE_LENGTH] = {
  [0] = {0},
};

static const uint16_t ts_non_terminal_alias_map[] = {
  0,
};

static const TSStateId ts_primary_state_ids[STATE_COUNT] = {
  [0] = 0,
  [1] = 1,
  [2] = 2,
  [3] = 3,
  [4] = 3,
  [5] = 5,
  [6] = 6,
  [7] = 7,
  [8] = 8,
  [9] = 9,
  [10] = 10,
  [11] = 11,
  [12] = 12,
  [13] = 13,
  [14] = 14,
  [15] = 15,
  [16] = 16,
  [17] = 17,
  [18] = 3,
  [19] = 3,
  [20] = 20,
  [21] = 21,
  [22] = 22,
  [23] = 23,
  [24] = 24,
  [25] = 25,
  [26] = 3,
  [27] = 27,
  [28] = 28,
  [29] = 28,
  [30] = 30,
  [31] = 28,
  [32] = 27,
  [33] = 27,
  [34] = 27,
  [35] = 28,
  [36] = 28,
  [37] = 27,
  [38] = 38,
  [39] = 39,
  [40] = 40,
  [41] = 41,
  [42] = 42,
  [43] = 43,
  [44] = 44,
  [45] = 45,
  [46] = 46,
  [47] = 25,
  [48] = 48,
  [49] = 49,
  [50] = 50,
  [51] = 51,
  [52] = 52,
  [53] = 53,
  [54] = 54,
  [55] = 55,
  [56] = 56,
  [57] = 57,
  [58] = 58,
  [59] = 59,
  [60] = 3,
  [61] = 24,
  [62] = 62,
  [63] = 63,
  [64] = 64,
  [65] = 3,
  [66] = 25,
  [67] = 24,
  [68] = 68,
  [69] = 54,
  [70] = 49,
  [71] = 71,
  [72] = 72,
  [73] = 72,
  [74] = 74,
  [75] = 75,
  [76] = 76,
  [77] = 77,
  [78] = 77,
  [79] = 74,
  [80] = 80,
  [81] = 81,
  [82] = 82,
  [83] = 83,
  [84] = 81,
  [85] = 85,
  [86] = 83,
  [87] = 87,
  [88] = 87,
  [89] = 89,
  [90] = 46,
  [91] = 48,
  [92] = 85,
  [93] = 89,
  [94] = 85,
  [95] = 89,
  [96] = 85,
  [97] = 89,
  [98] = 89,
  [99] = 53,
  [100] = 51,
  [101] = 50,
  [102] = 55,
  [103] = 58,
  [104] = 57,
  [105] = 105,
  [106] = 106,
  [107] = 107,
  [108] = 108,
  [109] = 24,
  [110] = 46,
  [111] = 48,
  [112] = 59,
  [113] = 113,
  [114] = 114,
  [115] = 108,
  [116] = 116,
  [117] = 117,
  [118] = 118,
  [119] = 119,
  [120] = 120,
  [121] = 121,
  [122] = 122,
  [123] = 123,
  [124] = 107,
  [125] = 125,
  [126] = 24,
  [127] = 50,
  [128] = 128,
  [129] = 129,
  [130] = 130,
  [131] = 131,
  [132] = 132,
  [133] = 133,
  [134] = 59,
  [135] = 133,
  [136] = 24,
  [137] = 137,
  [138] = 138,
  [139] = 51,
  [140] = 140,
  [141] = 53,
  [142] = 54,
  [143] = 55,
  [144] = 58,
  [145] = 57,
  [146] = 49,
  [147] = 54,
  [148] = 148,
  [149] = 149,
  [150] = 150,
  [151] = 24,
  [152] = 152,
  [153] = 153,
  [154] = 137,
  [155] = 155,
  [156] = 156,
  [157] = 49,
  [158] = 46,
  [159] = 54,
  [160] = 160,
  [161] = 54,
  [162] = 160,
  [163] = 163,
  [164] = 163,
  [165] = 165,
  [166] = 165,
  [167] = 167,
  [168] = 46,
  [169] = 48,
  [170] = 48,
  [171] = 171,
  [172] = 172,
  [173] = 173,
  [174] = 173,
  [175] = 175,
  [176] = 176,
  [177] = 175,
  [178] = 178,
  [179] = 176,
  [180] = 180,
  [181] = 51,
  [182] = 182,
  [183] = 50,
  [184] = 57,
  [185] = 51,
  [186] = 186,
  [187] = 182,
  [188] = 50,
  [189] = 55,
  [190] = 58,
  [191] = 57,
  [192] = 192,
  [193] = 178,
  [194] = 194,
  [195] = 195,
  [196] = 196,
  [197] = 195,
  [198] = 198,
  [199] = 194,
  [200] = 200,
  [201] = 201,
  [202] = 202,
  [203] = 195,
  [204] = 55,
  [205] = 205,
  [206] = 205,
  [207] = 195,
  [208] = 208,
  [209] = 209,
  [210] = 195,
  [211] = 198,
  [212] = 58,
  [213] = 53,
  [214] = 214,
  [215] = 208,
  [216] = 216,
  [217] = 217,
  [218] = 218,
  [219] = 219,
  [220] = 220,
  [221] = 221,
  [222] = 222,
  [223] = 223,
  [224] = 224,
  [225] = 225,
  [226] = 226,
  [227] = 223,
  [228] = 228,
  [229] = 225,
  [230] = 225,
  [231] = 231,
  [232] = 232,
  [233] = 225,
  [234] = 226,
  [235] = 226,
  [236] = 236,
  [237] = 237,
  [238] = 238,
  [239] = 239,
  [240] = 225,
  [241] = 226,
  [242] = 242,
  [243] = 225,
  [244] = 226,
  [245] = 245,
  [246] = 223,
  [247] = 245,
  [248] = 245,
  [249] = 245,
  [250] = 223,
  [251] = 245,
  [252] = 223,
  [253] = 245,
  [254] = 223,
  [255] = 226,
  [256] = 256,
};

static bool ts_lex(TSLexer *lexer, TSStateId state) {
  START_LEXER();
  eof = lexer->eof(lexer);
  switch (state) {
    case 0:
      if (eof) ADVANCE(884);
      ADVANCE_MAP(
        '\n', 886,
        '\r', 3,
        ' ', 889,
        '"', 2045,
        '#', 893,
        '$', 899,
        '%', 501,
        '\'', 2049,
        ')', 1322,
        '*', 502,
        '+', 503,
        '-', 1291,
        '.', 29,
        '/', 504,
        '0', 1295,
        '1', 1295,
        ':', 2075,
        '<', 1330,
        '=', 1281,
        '>', 1331,
        '@', 881,
        '[', 2077,
        '\\', 5,
        ']', 2078,
        '^', 1285,
        'a', 96,
        'b', 39,
        'c', 119,
        'e', 207,
        'f', 1311,
        'g', 40,
        'i', 97,
        'j', 204,
        'k', 163,
        'l', 120,
        'n', 59,
        'o', 208,
        'p', 41,
        'q', 487,
        'r', 42,
        't', 1312,
        'u', 312,
        'v', 46,
        'w', 1028,
        'x', 369,
        '{', 2074,
        '}', 2076,
        '~', 1284,
        '\t', 505,
        0x0b, 505,
        '\f', 505,
        '_', 500,
        'y', 500,
        'z', 500,
        'd', 1313,
        'h', 1313,
        'm', 1313,
        's', 1313,
      );
      if (('2' <= lookahead && lookahead <= '9')) ADVANCE(1296);
      END_STATE();
    case 1:
      ADVANCE_MAP(
        '\n', 886,
        '\r', 3,
        ' ', 889,
        '"', 2045,
        '$', 23,
        '\'', 2049,
        '=', 1280,
        '[', 2077,
        '\\', 5,
        '\t', 505,
        0x0b, 505,
        '\f', 505,
      );
      if (('!' <= lookahead && lookahead <= '#') ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 2:
      ADVANCE_MAP(
        '\n', 886,
        '\r', 3,
        ' ', 889,
        '$', 23,
        '-', 1290,
        '.', 874,
        '[', 1273,
        '\\', 5,
        '\t', 505,
        0x0b, 505,
        '\f', 505,
        'd', 1310,
        'f', 1310,
        'h', 1310,
        'm', 1310,
        's', 1310,
        't', 1310,
      );
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1297);
      END_STATE();
    case 3:
      if (lookahead == '\n') ADVANCE(886);
      if (lookahead == '\\') ADVANCE(5);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(505);
      END_STATE();
    case 4:
      if (lookahead == '\n') ADVANCE(890);
      END_STATE();
    case 5:
      if (lookahead == '\n') ADVANCE(890);
      if (lookahead == '\r') ADVANCE(4);
      END_STATE();
    case 6:
      if (lookahead == '\n') ADVANCE(885);
      END_STATE();
    case 7:
      ADVANCE_MAP(
        '\n', 885,
        '\r', 6,
        '#', 893,
        '$', 899,
        'a', 568,
        'b', 513,
        'c', 688,
        'd', 507,
        'e', 640,
        'f', 615,
        'g', 508,
        'h', 612,
        'i', 839,
        'j', 641,
        'k', 665,
        'l', 667,
        'm', 587,
        'o', 774,
        'p', 521,
        'r', 519,
        's', 509,
        't', 516,
        'v', 630,
        'w', 1029,
        'x', 775,
      );
      END_STATE();
    case 8:
      ADVANCE_MAP(
        ' ', 889,
        '"', 2045,
        '$', 23,
        '%', 501,
        '\'', 2049,
        '*', 502,
        '+', 2030,
        '-', 1294,
        '.', 2043,
        '/', 504,
        '0', 1298,
        '1', 1298,
        '<', 1330,
        '=', 1281,
        '>', 1331,
        '@', 881,
        '[', 2077,
        '\\', 5,
        '^', 1285,
        'a', 1693,
        'b', 1648,
        'c', 1837,
        'd', 1414,
        'e', 1790,
        'f', 1383,
        'h', 1801,
        'i', 1603,
        'l', 1574,
        'm', 1384,
        'o', 1460,
        'p', 1403,
        'r', 1566,
        's', 1468,
        't', 1437,
        'u', 1794,
        'v', 1388,
        'w', 1545,
        '{', 2074,
        '~', 1284,
      );
      if (('\t' <= lookahead && lookahead <= '\r')) ADVANCE(505);
      if (('2' <= lookahead && lookahead <= '9')) ADVANCE(1299);
      if (lookahead == '_' ||
          ('g' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (('!' <= lookahead && lookahead <= '#') ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 9:
      ADVANCE_MAP(
        ' ', 889,
        '"', 2045,
        '$', 23,
        '%', 501,
        '\'', 2049,
        '*', 502,
        '+', 2030,
        '-', 1294,
        '.', 2043,
        '/', 504,
        '0', 1298,
        '1', 1298,
        '<', 1330,
        '=', 1280,
        '>', 1331,
        '@', 881,
        '[', 2077,
        '\\', 5,
        '^', 1285,
        'a', 1473,
        'b', 1377,
        'c', 1499,
        'd', 1378,
        'e', 1747,
        'f', 1379,
        'g', 1386,
        'h', 1798,
        'i', 1474,
        'j', 1644,
        'k', 1544,
        'l', 1500,
        'm', 1380,
        'n', 1419,
        'o', 1459,
        'p', 1694,
        'q', 2006,
        'r', 1417,
        's', 1543,
        't', 1381,
        'u', 1748,
        'v', 1387,
        'w', 1391,
        '{', 2074,
        '~', 1284,
      );
      if (lookahead == '_' ||
          ('x' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (('\t' <= lookahead && lookahead <= '\r')) ADVANCE(505);
      if (('2' <= lookahead && lookahead <= '9')) ADVANCE(1299);
      if (('!' <= lookahead && lookahead <= '#') ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 10:
      ADVANCE_MAP(
        ' ', 889,
        '"', 2045,
        '$', 23,
        '%', 501,
        '\'', 2049,
        '*', 502,
        '+', 2030,
        '-', 1294,
        '.', 2043,
        '/', 504,
        '0', 1298,
        '1', 1298,
        '<', 1330,
        '=', 1280,
        '>', 1331,
        '@', 881,
        '[', 2077,
        '\\', 5,
        '^', 1285,
        'a', 1852,
        'b', 2026,
        'c', 1838,
        'd', 1802,
        'e', 1792,
        'f', 1408,
        'h', 1801,
        'i', 1762,
        'l', 1821,
        'm', 1840,
        'o', 1461,
        'p', 1729,
        's', 1641,
        't', 1895,
        'v', 1670,
        'w', 1545,
        '{', 2074,
        '~', 1284,
      );
      if (('\t' <= lookahead && lookahead <= '\r')) ADVANCE(505);
      if (('2' <= lookahead && lookahead <= '9')) ADVANCE(1299);
      if (lookahead == '_' ||
          ('g' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (('!' <= lookahead && lookahead <= '#') ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 11:
      ADVANCE_MAP(
        ' ', 889,
        '"', 2045,
        '$', 23,
        '\'', 2049,
        '-', 1293,
        '.', 2061,
        '0', 1300,
        '1', 1300,
        ':', 2075,
        '=', 1281,
        '[', 2077,
        '\\', 5,
        'f', 2066,
        't', 2069,
        '{', 2074,
      );
      if (('\t' <= lookahead && lookahead <= '\r')) ADVANCE(505);
      if (('2' <= lookahead && lookahead <= '9')) ADVANCE(1301);
      if (('!' <= lookahead && lookahead <= '#') ||
          lookahead == '+' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 12:
      ADVANCE_MAP(
        ' ', 889,
        '$', 899,
        '-', 1292,
        '.', 29,
        '0', 1295,
        '1', 1295,
        ':', 2075,
        '=', 1280,
        '[', 2077,
        '\\', 5,
        'a', 568,
        'b', 513,
        'c', 688,
        'd', 507,
        'e', 640,
        'f', 534,
        'g', 508,
        'h', 612,
        'i', 839,
        'j', 641,
        'k', 665,
        'l', 667,
        'm', 587,
        'o', 774,
        'p', 521,
        'r', 519,
        's', 509,
        't', 515,
        'v', 630,
        'w', 1029,
        'x', 775,
        '{', 2074,
      );
      if (('\t' <= lookahead && lookahead <= '\r')) ADVANCE(505);
      if (('2' <= lookahead && lookahead <= '9')) ADVANCE(1296);
      END_STATE();
    case 13:
      ADVANCE_MAP(
        ' ', 888,
        '"', 2045,
        '$', 23,
        '\'', 2049,
        ',', 1283,
        '-', 1293,
        '.', 2061,
        '0', 1300,
        '1', 1300,
        '[', 2077,
        '\\', 5,
        ']', 2078,
        'f', 2066,
        't', 2069,
        '{', 2074,
      );
      if (('\t' <= lookahead && lookahead <= '\r')) ADVANCE(24);
      if (('2' <= lookahead && lookahead <= '9')) ADVANCE(1301);
      if (('!' <= lookahead && lookahead <= '#') ||
          lookahead == '+' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 14:
      ADVANCE_MAP(
        ' ', 888,
        '"', 2045,
        '$', 23,
        '\'', 2049,
        ',', 1283,
        '-', 1293,
        '.', 2061,
        '0', 1300,
        '1', 1300,
        '[', 2077,
        '\\', 5,
        ']', 2078,
        'f', 1315,
        't', 1316,
        '{', 2074,
        'd', 1317,
        'h', 1317,
        'm', 1317,
        's', 1317,
      );
      if (('\t' <= lookahead && lookahead <= '\r')) ADVANCE(24);
      if (('2' <= lookahead && lookahead <= '9')) ADVANCE(1301);
      if (('!' <= lookahead && lookahead <= '#') ||
          lookahead == '+' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 15:
      ADVANCE_MAP(
        ' ', 888,
        '"', 2045,
        '$', 23,
        '\'', 2049,
        ',', 1283,
        '=', 1280,
        '\\', 5,
        ']', 2078,
        '}', 2076,
      );
      if (('\t' <= lookahead && lookahead <= '\r')) ADVANCE(24);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2065);
      if (('!' <= lookahead && lookahead <= '#') ||
          ('+' <= lookahead && lookahead <= '.') ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2073);
      END_STATE();
    case 16:
      if (lookahead == '"') ADVANCE(2045);
      if (lookahead == '#') ADVANCE(2044);
      if (lookahead == '$') ADVANCE(23);
      if (lookahead == '\'') ADVANCE(2049);
      if (lookahead == ',') ADVANCE(1283);
      if (lookahead == '[') ADVANCE(2077);
      if (lookahead == ']') ADVANCE(1274);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(25);
      if (lookahead == '!' ||
          ('+' <= lookahead && lookahead <= '.') ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1276);
      END_STATE();
    case 17:
      ADVANCE_MAP(
        '"', 2045,
        '$', 23,
        '%', 501,
        '\'', 2049,
        '*', 502,
        '+', 2030,
        '-', 1294,
        '.', 2043,
        '/', 504,
        '0', 1298,
        '1', 1298,
        '<', 1330,
        '>', 1331,
        '@', 881,
        '[', 2077,
        '^', 1285,
        'a', 1693,
        'b', 1648,
        'c', 1837,
        'd', 1414,
        'e', 1790,
        'f', 1383,
        'h', 1801,
        'i', 1603,
        'l', 1574,
        'm', 1384,
        'o', 1460,
        'p', 1403,
        'r', 1567,
        's', 1468,
        't', 1437,
        'u', 1794,
        'v', 1388,
        'w', 1545,
        '{', 2074,
        '~', 1284,
      );
      if (('2' <= lookahead && lookahead <= '9')) ADVANCE(1299);
      if (lookahead == '_' ||
          ('g' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (('!' <= lookahead && lookahead <= '#') ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 18:
      ADVANCE_MAP(
        '"', 2045,
        '$', 23,
        '\'', 2049,
        ',', 1283,
        ']', 2078,
        '}', 2076,
        'd', 1314,
        'f', 1314,
        'h', 1314,
        'm', 1314,
        's', 1314,
        't', 1314,
      );
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(25);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2065);
      if (('!' <= lookahead && lookahead <= '#') ||
          ('+' <= lookahead && lookahead <= '.') ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2073);
      END_STATE();
    case 19:
      ADVANCE_MAP(
        '"', 2045,
        '$', 23,
        '\'', 2049,
        ',', 1283,
        '}', 2076,
        'd', 1317,
        'f', 1317,
        'h', 1317,
        'm', 1317,
        's', 1317,
        't', 1317,
      );
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(25);
      if (('!' <= lookahead && lookahead <= '#') ||
          ('+' <= lookahead && lookahead <= '.') ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 20:
      if (lookahead == '"') ADVANCE(2045);
      if (lookahead == '$') ADVANCE(23);
      if (lookahead == '\'') ADVANCE(2049);
      if (lookahead == ',') ADVANCE(1283);
      if (lookahead == '}') ADVANCE(2076);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(25);
      if (('!' <= lookahead && lookahead <= '#') ||
          ('+' <= lookahead && lookahead <= '.') ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 21:
      ADVANCE_MAP(
        '$', 23,
        ',', 1283,
        '-', 1279,
        '.', 1278,
        '[', 1273,
        ']', 1274,
        'd', 1279,
        'f', 1279,
        'h', 1279,
        'm', 1279,
        's', 1279,
        't', 1279,
      );
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(25);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1277);
      if (lookahead == '!' ||
          lookahead == '+' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1279);
      END_STATE();
    case 22:
      if (lookahead == '$') ADVANCE(23);
      if (('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1321);
      END_STATE();
    case 23:
      if (lookahead == '(') ADVANCE(1318);
      END_STATE();
    case 24:
      if (lookahead == ',') ADVANCE(1283);
      if (lookahead == '\\') ADVANCE(5);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(24);
      END_STATE();
    case 25:
      if (lookahead == ',') ADVANCE(1283);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(25);
      END_STATE();
    case 26:
      if (lookahead == '-') ADVANCE(875);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(2058);
      END_STATE();
    case 27:
      if (lookahead == '-') ADVANCE(538);
      END_STATE();
    case 28:
      if (lookahead == '.') ADVANCE(26);
      END_STATE();
    case 29:
      if (lookahead == '.') ADVANCE(26);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1307);
      END_STATE();
    case 30:
      if (lookahead == '.') ADVANCE(2056);
      END_STATE();
    case 31:
      if (lookahead == '.') ADVANCE(550);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(1206);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 32:
      if (lookahead == '.') ADVANCE(876);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 33:
      if (lookahead == '.') ADVANCE(30);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(33);
      END_STATE();
    case 34:
      if (lookahead == '.') ADVANCE(877);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 35:
      if (lookahead == '.') ADVANCE(878);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 36:
      if (lookahead == '.') ADVANCE(879);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 37:
      if (lookahead == '.') ADVANCE(880);
      END_STATE();
    case 38:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_') ADVANCE(76);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 39:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(313);
      if (lookahead == 'i') ADVANCE(363);
      if (lookahead == 'l') ADVANCE(347);
      if (lookahead == 'o') ADVANCE(416);
      if (lookahead == 'u') ADVANCE(211);
      if (lookahead == 'y') ADVANCE(1127);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 40:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(306);
      if (lookahead == 'e') ADVANCE(437);
      if (lookahead == 'i') ADVANCE(491);
      if (lookahead == 'r') ADVANCE(58);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 41:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(376);
      if (lookahead == 'e') ADVANCE(394);
      if (lookahead == 'l') ADVANCE(44);
      if (lookahead == 'o') ADVANCE(234);
      if (lookahead == 'r') ADVANCE(121);
      if (lookahead == 'u') ADVANCE(74);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 42:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(243);
      if (lookahead == 'e') ADVANCE(86);
      if (lookahead == 'i') ADVANCE(114);
      if (lookahead == 'o') ADVANCE(278);
      if (lookahead == 'u') ADVANCE(314);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 43:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(417);
      if (lookahead == 'v') ADVANCE(192);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 44:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(89);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 45:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(493);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 46:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(295);
      if (lookahead == 'e') ADVANCE(224);
      if (lookahead == 'i') ADVANCE(293);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 47:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(219);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 48:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(378);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 49:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(101);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 50:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(325);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 51:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(328);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 52:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(273);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 53:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(274);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 54:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(385);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 55:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(451);
      if (lookahead == 'e') ADVANCE(429);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 56:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(453);
      if (lookahead == 'e') ADVANCE(430);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 57:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(455);
      if (lookahead == 'e') ADVANCE(431);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 58:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(330);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 59:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(308);
      if (lookahead == 'i') ADVANCE(215);
      if (lookahead == 'o') ADVANCE(350);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 60:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(75);
      if (lookahead == 'c') ADVANCE(233);
      if (lookahead == 'd') ADVANCE(169);
      if (lookahead == 't') ADVANCE(246);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 61:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(115);
      if (lookahead == 'c') ADVANCE(67);
      if (lookahead == 'o') ADVANCE(440);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 62:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(85);
      if (lookahead == 'r') ADVANCE(236);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 63:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(479);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 64:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(91);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 65:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(262);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 66:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(469);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 67:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(470);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 68:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(471);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 69:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(264);
      if (lookahead == 'e') ADVANCE(335);
      if (lookahead == 'r') ADVANCE(355);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 70:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(336);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 71:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(432);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 72:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(474);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 73:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(297);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 74:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'b') ADVANCE(282);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 75:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'b') ADVANCE(287);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 76:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'b') ADVANCE(71);
      if (lookahead == 't') ADVANCE(361);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 77:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'b') ADVANCE(54);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 78:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'b') ADVANCE(289);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 79:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'b') ADVANCE(360);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 80:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'b') ADVANCE(490);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 81:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'c') ADVANCE(228);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 82:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'c') ADVANCE(266);
      if (lookahead == 'l') ADVANCE(270);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 83:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'c') ADVANCE(267);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 84:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'c') ADVANCE(489);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 85:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'c') ADVANCE(269);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 86:
      ADVANCE_MAP(
        ':', 882,
        'c', 254,
        'l', 353,
        'm', 362,
        'p', 280,
        's', 177,
        't', 480,
        'v', 348,
      );
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 87:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'c') ADVANCE(226);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 88:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'c') ADVANCE(445);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 89:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'c') ADVANCE(131);
      if (lookahead == 'y') ADVANCE(183);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 90:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'c') ADVANCE(199);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 91:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'c') ADVANCE(142);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 92:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'c') ADVANCE(153);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 93:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'c') ADVANCE(288);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 94:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'c') ADVANCE(291);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 95:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'c') ADVANCE(72);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 96:
      ADVANCE_MAP(
        ':', 882,
        'd', 98,
        'l', 235,
        'm', 346,
        'n', 81,
        'p', 371,
        's', 1046,
        't', 1048,
      );
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 97:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'd') ADVANCE(1166);
      if (lookahead == 'f') ADVANCE(1068);
      if (lookahead == 'n') ADVANCE(1070);
      if (lookahead == 'p') ADVANCE(407);
      if (lookahead == 't') ADVANCE(157);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 98:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'd') ADVANCE(1111);
      if (lookahead == 'v') ADVANCE(50);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 99:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'd') ADVANCE(1121);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 100:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'd') ADVANCE(1076);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 101:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'd') ADVANCE(985);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 102:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'd') ADVANCE(1200);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 103:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'd') ADVANCE(1214);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 104:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'd') ADVANCE(1044);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 105:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'd') ADVANCE(977);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 106:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'd') ADVANCE(1238);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 107:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'd') ADVANCE(1092);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 108:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'd') ADVANCE(38);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 109:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'd') ADVANCE(79);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 110:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'd') ADVANCE(354);
      if (lookahead == 't') ADVANCE(265);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 111:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'd') ADVANCE(409);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 112:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'd') ADVANCE(349);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 113:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'd') ADVANCE(244);
      if (lookahead == 'p') ADVANCE(180);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 114:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'd') ADVANCE(126);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 115:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'd') ADVANCE(168);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 116:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'd') ADVANCE(145);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 117:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'd') ADVANCE(178);
      if (lookahead == 'l') ADVANCE(191);
      if (lookahead == 't') ADVANCE(249);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 118:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'd') ADVANCE(190);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 119:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(323);
      if (lookahead == 'l') ADVANCE(164);
      if (lookahead == 'o') ADVANCE(275);
      if (lookahead == 'r') ADVANCE(166);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 120:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(43);
      if (lookahead == 'i') ADVANCE(418);
      if (lookahead == 'o') ADVANCE(61);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 121:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(113);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 122:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(1123);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 123:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(307);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 124:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(944);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 125:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(1190);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 126:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(990);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 127:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(2054);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 128:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(1053);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 129:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(916);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 130:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(1178);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 131:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(975);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 132:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(1107);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 133:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(1133);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 134:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(1141);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 135:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(959);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 136:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(983);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 137:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(1220);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 138:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(1232);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 139:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(992);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 140:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(895);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 141:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(1204);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 142:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(1222);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 143:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(1109);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 144:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(1267);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 145:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(940);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 146:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(942);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 147:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(1159);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 148:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(971);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 149:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(1242);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 150:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(1117);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 151:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(903);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 152:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(1094);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 153:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(932);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 154:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(1230);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 155:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(1151);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 156:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(1113);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 157:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(302);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 158:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(370);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 159:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(84);
      if (lookahead == 'p') ADVANCE(171);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 160:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(88);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 161:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(392);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 162:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(408);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 163:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(158);
      if (lookahead == 'i') ADVANCE(82);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 164:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(48);
      if (lookahead == 'o') ADVANCE(333);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 165:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(377);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 166:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(66);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 167:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(393);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 168:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(100);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 169:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(404);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 170:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(324);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 171:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(399);
      if (lookahead == 'o') ADVANCE(401);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 172:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(104);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 173:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(380);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 174:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(106);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 175:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(107);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 176:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(345);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 177:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(443);
      if (lookahead == 'u') ADVANCE(286);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 178:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(381);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 179:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(108);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 180:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(329);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 181:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(382);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 182:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(383);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 183:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(384);
      if (lookahead == 's') ADVANCE(351);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 184:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(386);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 185:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(387);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 186:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(388);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 187:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(389);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 188:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(398);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 189:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(390);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 190:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(391);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 191:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(421);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 192:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(279);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 193:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(299);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 194:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(341);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 195:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(402);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 196:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(405);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 197:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(334);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 198:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(425);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 199:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(311);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 200:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(426);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 201:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(427);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 202:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(428);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 203:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'e') ADVANCE(338);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 204:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'f') ADVANCE(375);
      if (lookahead == 'i') ADVANCE(216);
      if (lookahead == 'o') ADVANCE(238);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 205:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'f') ADVANCE(1198);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 206:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'f') ADVANCE(973);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 207:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'f') ADVANCE(210);
      if (lookahead == 'n') ADVANCE(60);
      if (lookahead == 'v') ADVANCE(161);
      if (lookahead == 'x') ADVANCE(159);
      if (lookahead == 'y') ADVANCE(162);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 208:
      ADVANCE_MAP(
        ':', 882,
        'f', 205,
        'n', 1081,
        'p', 966,
        'r', 239,
        'u', 460,
        'v', 165,
        'w', 343,
      );
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 209:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'f') ADVANCE(497);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 210:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'f') ADVANCE(160);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 211:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'f') ADVANCE(212);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 212:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'f') ADVANCE(181);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 213:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'g') ADVANCE(1269);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 214:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'g') ADVANCE(1143);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 215:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'g') ADVANCE(225);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 216:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'g') ADVANCE(419);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 217:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'g') ADVANCE(318);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 218:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'g') ADVANCE(248);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 219:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'g') ADVANCE(187);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 220:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'g') ADVANCE(188);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 221:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'h') ADVANCE(1271);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 222:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'h') ADVANCE(55);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 223:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'h') ADVANCE(979);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 224:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'h') ADVANCE(241);
      if (lookahead == 'r') ADVANCE(242);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 225:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'h') ADVANCE(442);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 226:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'h') ADVANCE(198);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 227:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'h') ADVANCE(56);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 228:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'h') ADVANCE(356);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 229:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'h') ADVANCE(57);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 230:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'h') ADVANCE(252);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 231:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'h') ADVANCE(184);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 232:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'h') ADVANCE(185);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 233:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'h') ADVANCE(70);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 234:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(1208);
      if (lookahead == 's') ADVANCE(1212);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 235:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(217);
      if (lookahead == 'l') ADVANCE(1042);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 236:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(80);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 237:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(111);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 238:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(315);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 239:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(218);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 240:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(374);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 241:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(93);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 242:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(209);
      if (lookahead == 's') ADVANCE(256);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 243:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(317);
      if (lookahead == 'n') ADVANCE(112);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 244:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(95);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 245:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(468);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 246:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(462);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 247:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(420);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 248:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(319);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 249:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(272);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 250:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(366);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 251:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(326);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 252:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(327);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 253:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(179);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 254:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(373);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 255:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(423);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 256:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(358);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 257:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(78);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 258:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(339);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 259:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(424);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 260:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(310);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 261:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(194);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 262:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(292);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 263:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(337);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 264:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(344);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 265:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(94);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 266:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'k') ADVANCE(952);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 267:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'k') ADVANCE(1055);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 268:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'k') ADVANCE(138);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 269:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'k') ADVANCE(186);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 270:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(954);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 271:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(1234);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 272:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(1261);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 273:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(1196);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 274:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(1115);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 275:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(352);
      if (lookahead == 'n') ADVANCE(438);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 276:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(109);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 277:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(422);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 278:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(271);
      if (lookahead == 't') ADVANCE(68);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 279:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(410);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 280:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(64);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 281:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(47);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 282:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(247);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 283:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(365);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 284:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(298);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 285:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(253);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 286:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(448);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 287:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(134);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 288:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(143);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 289:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(144);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 290:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(146);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 291:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(148);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 292:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(174);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 293:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(281);
      if (lookahead == 's') ADVANCE(257);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 294:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(467);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 295:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(483);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 296:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(258);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 297:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(484);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 298:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(189);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 299:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(259);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 300:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(367);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 301:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(368);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 302:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'm') ADVANCE(948);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 303:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'm') ADVANCE(981);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 304:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'm') ADVANCE(485);
      if (lookahead == 'v') ADVANCE(73);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 305:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'm') ADVANCE(52);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 306:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'm') ADVANCE(123);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 307:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'm') ADVANCE(364);
      if (lookahead == 'r') ADVANCE(488);
      if (lookahead == 't') ADVANCE(260);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 308:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'm') ADVANCE(125);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 309:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'm') ADVANCE(128);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 310:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'm') ADVANCE(147);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 311:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'm') ADVANCE(203);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 312:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(117);
      if (lookahead == 'u') ADVANCE(237);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 313:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(904);
      if (lookahead == 's') ADVANCE(122);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 314:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(896);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 315:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(1174);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 316:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(1194);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 317:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(1218);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 318:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(1040);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 319:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(1084);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 320:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(968);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 321:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(988);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 322:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(1027);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 323:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(478);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 324:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(99);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 325:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(90);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 326:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(213);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 327:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(214);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 328:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(102);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 329:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(103);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 330:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(441);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 331:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(105);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 332:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(444);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 333:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(129);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 334:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(476);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 335:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(465);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 336:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(450);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 337:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(452);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 338:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(457);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 339:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(141);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 340:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(175);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 341:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(92);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 342:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(251);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 343:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(173);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 344:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(196);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 345:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(220);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 346:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'o') ADVANCE(486);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 347:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'o') ADVANCE(83);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 348:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'o') ADVANCE(268);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 349:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'o') ADVANCE(303);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 350:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'o') ADVANCE(316);
      if (lookahead == 'r') ADVANCE(305);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 351:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'o') ADVANCE(482);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 352:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'o') ADVANCE(379);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 353:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'o') ADVANCE(49);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 354:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'o') ADVANCE(320);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 355:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'o') ADVANCE(284);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 356:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'o') ADVANCE(403);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 357:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'o') ADVANCE(263);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 358:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'o') ADVANCE(322);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 359:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'o') ADVANCE(395);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 360:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'o') ADVANCE(406);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 361:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'o') ADVANCE(475);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 362:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'o') ADVANCE(492);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 363:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'o') ADVANCE(309);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 364:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'o') ADVANCE(116);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 365:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'o') ADVANCE(434);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 366:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'o') ADVANCE(340);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 367:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'o') ADVANCE(435);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 368:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'o') ADVANCE(436);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 369:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'p') ADVANCE(1039);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 370:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'p') ADVANCE(1176);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 371:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'p') ADVANCE(170);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 372:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'p') ADVANCE(357);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 373:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'p') ADVANCE(136);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 374:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'p') ADVANCE(285);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 375:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(950);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 376:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(110);
      if (lookahead == 's') ADVANCE(433);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 377:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(1086);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 378:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(913);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 379:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(1131);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 380:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(1088);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 381:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(1259);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 382:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(1125);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 383:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(1129);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 384:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(31);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 385:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(910);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 386:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(1074);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 387:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(1033);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 388:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(1051);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 389:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(32);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 390:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(1059);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 391:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(1037);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 392:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(499);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 393:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(495);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 394:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(206);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 395:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(498);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 396:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(342);
      if (lookahead == 'y') ADVANCE(372);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 397:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(276);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 398:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(414);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 399:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(261);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 400:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(321);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 401:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(446);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 402:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(447);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 403:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(172);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 404:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(87);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 405:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(34);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 406:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(118);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 407:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(1170);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 408:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(1064);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 409:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(1263);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 410:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(1180);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 411:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(1210);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 412:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(1105);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 413:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(1347);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 414:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(1090);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 415:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(77);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 416:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(415);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 417:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(231);
      if (lookahead == 'v') ADVANCE(130);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 418:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(439);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 419:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(45);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 420:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(223);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 421:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(412);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 422:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(127);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 423:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(449);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 424:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(454);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 425:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(477);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 426:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(456);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 427:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(458);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 428:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(459);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 429:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(149);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 430:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(154);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 431:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(155);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 432:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(156);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 433:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(176);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 434:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(200);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 435:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(201);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 436:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(202);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 437:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(1161);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 438:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(69);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 439:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(956);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 440:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(962);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 441:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(1163);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 442:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(1192);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 443:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(1224);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 444:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(1119);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 445:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(928);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 446:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(1145);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 447:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(1168);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 448:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(1096);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 449:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(908);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 450:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(930);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 451:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(1240);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 452:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(1031);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 453:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(1228);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 454:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(1035);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 455:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(1149);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 456:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(1236);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 457:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(901);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 458:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(1226);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 459:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(1147);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 460:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(296);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 461:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(221);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 462:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(496);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 463:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(230);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 464:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(411);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 465:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(413);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 466:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(250);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 467:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(240);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 468:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(193);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 469:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(133);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 470:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(135);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 471:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(139);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 472:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(140);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 473:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(151);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 474:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(152);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 475:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(53);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 476:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(359);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 477:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(36);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 478:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(182);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 479:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(232);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 480:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'u') ADVANCE(400);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 481:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'u') ADVANCE(127);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 482:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'u') ADVANCE(331);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 483:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'u') ADVANCE(132);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 484:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'u') ADVANCE(150);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 485:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'u') ADVANCE(294);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 486:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'u') ADVANCE(332);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 487:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'u') ADVANCE(167);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 488:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'u') ADVANCE(290);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 489:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'u') ADVANCE(472);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 490:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'u') ADVANCE(473);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 491:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'v') ADVANCE(124);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 492:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'v') ADVANCE(137);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 493:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'w') ADVANCE(1172);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 494:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'y') ADVANCE(1202);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 495:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'y') ADVANCE(1216);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 496:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'y') ADVANCE(1062);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 497:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'y') ADVANCE(1265);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 498:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'y') ADVANCE(35);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 499:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'y') ADVANCE(463);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 500:
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 501:
      if (lookahead == '=') ADVANCE(1328);
      END_STATE();
    case 502:
      if (lookahead == '=') ADVANCE(1326);
      END_STATE();
    case 503:
      if (lookahead == '=') ADVANCE(1324);
      END_STATE();
    case 504:
      if (lookahead == '=') ADVANCE(1327);
      END_STATE();
    case 505:
      if (lookahead == '\\') ADVANCE(5);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(505);
      END_STATE();
    case 506:
      if (lookahead == 'a') ADVANCE(637);
      END_STATE();
    case 507:
      if (lookahead == 'a') ADVANCE(716);
      if (lookahead == 'e') ADVANCE(543);
      if (lookahead == 'i') ADVANCE(520);
      END_STATE();
    case 508:
      if (lookahead == 'a') ADVANCE(717);
      if (lookahead == 'i') ADVANCE(866);
      END_STATE();
    case 509:
      if (lookahead == 'a') ADVANCE(867);
      if (lookahead == 'c') ADVANCE(660);
      if (lookahead == 'e') ADVANCE(614);
      if (lookahead == 'p') ADVANCE(511);
      if (lookahead == 't') ADVANCE(752);
      if (lookahead == 'u') ADVANCE(715);
      if (lookahead == 'w') ADVANCE(675);
      END_STATE();
    case 510:
      if (lookahead == 'a') ADVANCE(559);
      END_STATE();
    case 511:
      if (lookahead == 'a') ADVANCE(869);
      if (lookahead == 'e') ADVANCE(567);
      if (lookahead == 'r') ADVANCE(622);
      END_STATE();
    case 512:
      if (lookahead == 'a') ADVANCE(920);
      END_STATE();
    case 513:
      if (lookahead == 'a') ADVANCE(727);
      if (lookahead == 'o') ADVANCE(814);
      END_STATE();
    case 514:
      if (lookahead == 'a') ADVANCE(868);
      END_STATE();
    case 515:
      if (lookahead == 'a') ADVANCE(648);
      if (lookahead == 'e') ADVANCE(523);
      if (lookahead == 'i') ADVANCE(719);
      if (lookahead == 'm') ADVANCE(1022);
      if (lookahead == 'p') ADVANCE(1023);
      if (lookahead == 'r') ADVANCE(525);
      END_STATE();
    case 516:
      if (lookahead == 'a') ADVANCE(648);
      if (lookahead == 'e') ADVANCE(523);
      if (lookahead == 'i') ADVANCE(719);
      if (lookahead == 'm') ADVANCE(1022);
      if (lookahead == 'p') ADVANCE(1023);
      if (lookahead == 'r') ADVANCE(524);
      END_STATE();
    case 517:
      if (lookahead == 'a') ADVANCE(873);
      END_STATE();
    case 518:
      if (lookahead == 'a') ADVANCE(855);
      END_STATE();
    case 519:
      if (lookahead == 'a') ADVANCE(734);
      if (lookahead == 'e') ADVANCE(564);
      if (lookahead == 'i') ADVANCE(578);
      if (lookahead == 'o') ADVANCE(851);
      END_STATE();
    case 520:
      if (lookahead == 'a') ADVANCE(692);
      if (lookahead == 'f') ADVANCE(644);
      END_STATE();
    case 521:
      if (lookahead == 'a') ADVANCE(796);
      if (lookahead == 'e') ADVANCE(799);
      if (lookahead == 'l') ADVANCE(510);
      if (lookahead == 'u') ADVANCE(544);
      END_STATE();
    case 522:
      if (lookahead == 'a') ADVANCE(655);
      END_STATE();
    case 523:
      if (lookahead == 'a') ADVANCE(713);
      if (lookahead == 'l') ADVANCE(620);
      if (lookahead == 's') ADVANCE(827);
      END_STATE();
    case 524:
      if (lookahead == 'a') ADVANCE(736);
      if (lookahead == 'i') ADVANCE(656);
      END_STATE();
    case 525:
      if (lookahead == 'a') ADVANCE(736);
      if (lookahead == 'i') ADVANCE(656);
      if (lookahead == 'u') ADVANCE(586);
      END_STATE();
    case 526:
      if (lookahead == 'a') ADVANCE(570);
      END_STATE();
    case 527:
      if (lookahead == 'a') ADVANCE(742);
      END_STATE();
    case 528:
      if (lookahead == 'a') ADVANCE(580);
      END_STATE();
    case 529:
      if (lookahead == 'a') ADVANCE(840);
      END_STATE();
    case 530:
      if (lookahead == 'a') ADVANCE(790);
      END_STATE();
    case 531:
      if (lookahead == 'a') ADVANCE(571);
      END_STATE();
    case 532:
      if (lookahead == 'a') ADVANCE(554);
      END_STATE();
    case 533:
      if (lookahead == 'a') ADVANCE(791);
      END_STATE();
    case 534:
      if (lookahead == 'a') ADVANCE(699);
      if (lookahead == 'e') ADVANCE(841);
      if (lookahead == 'i') ADVANCE(693);
      if (lookahead == 'o') ADVANCE(798);
      if (lookahead == 'u') ADVANCE(735);
      END_STATE();
    case 535:
      if (lookahead == 'a') ADVANCE(806);
      END_STATE();
    case 536:
      if (lookahead == 'a') ADVANCE(870);
      END_STATE();
    case 537:
      if (lookahead == 'a') ADVANCE(845);
      END_STATE();
    case 538:
      if (lookahead == 'a') ADVANCE(701);
      if (lookahead == 'o') ADVANCE(643);
      END_STATE();
    case 539:
      if (lookahead == 'a') ADVANCE(738);
      END_STATE();
    case 540:
      if (lookahead == 'a') ADVANCE(846);
      END_STATE();
    case 541:
      if (lookahead == 'a') ADVANCE(723);
      END_STATE();
    case 542:
      if (lookahead == 'a') ADVANCE(848);
      END_STATE();
    case 543:
      if (lookahead == 'b') ADVANCE(854);
      if (lookahead == 'f') ADVANCE(518);
      if (lookahead == 'o') ADVANCE(776);
      END_STATE();
    case 544:
      if (lookahead == 'b') ADVANCE(696);
      END_STATE();
    case 545:
      if (lookahead == 'b') ADVANCE(698);
      if (lookahead == 'i') ADVANCE(583);
      if (lookahead == 'w') ADVANCE(767);
      END_STATE();
    case 546:
      if (lookahead == 'b') ADVANCE(533);
      END_STATE();
    case 547:
      if (lookahead == 'b') ADVANCE(769);
      END_STATE();
    case 548:
      if (lookahead == 'b') ADVANCE(763);
      END_STATE();
    case 549:
      if (lookahead == 'b') ADVANCE(864);
      END_STATE();
    case 550:
      if (lookahead == 'c') ADVANCE(786);
      END_STATE();
    case 551:
      if (lookahead == 'c') ADVANCE(659);
      END_STATE();
    case 552:
      if (lookahead == 'c') ADVANCE(685);
      if (lookahead == 'l') ADVANCE(690);
      END_STATE();
    case 553:
      if (lookahead == 'c') ADVANCE(661);
      END_STATE();
    case 554:
      if (lookahead == 'c') ADVANCE(686);
      END_STATE();
    case 555:
      if (lookahead == 'c') ADVANCE(687);
      END_STATE();
    case 556:
      if (lookahead == 'c') ADVANCE(862);
      END_STATE();
    case 557:
      if (lookahead == 'c') ADVANCE(537);
      if (lookahead == 'o') ADVANCE(826);
      END_STATE();
    case 558:
      if (lookahead == 'c') ADVANCE(828);
      END_STATE();
    case 559:
      if (lookahead == 'c') ADVANCE(594);
      if (lookahead == 'y') ADVANCE(817);
      END_STATE();
    case 560:
      if (lookahead == 'c') ADVANCE(631);
      END_STATE();
    case 561:
      if (lookahead == 'c') ADVANCE(608);
      END_STATE();
    case 562:
      if (lookahead == 'c') ADVANCE(852);
      END_STATE();
    case 563:
      if (lookahead == 'c') ADVANCE(623);
      END_STATE();
    case 564:
      if (lookahead == 'c') ADVANCE(670);
      if (lookahead == 'l') ADVANCE(755);
      if (lookahead == 't') ADVANCE(856);
      END_STATE();
    case 565:
      if (lookahead == 'c') ADVANCE(707);
      END_STATE();
    case 566:
      if (lookahead == 'c') ADVANCE(860);
      END_STATE();
    case 567:
      if (lookahead == 'c') ADVANCE(853);
      END_STATE();
    case 568:
      if (lookahead == 'd') ADVANCE(865);
      if (lookahead == 't') ADVANCE(842);
      END_STATE();
    case 569:
      if (lookahead == 'd') ADVANCE(999);
      END_STATE();
    case 570:
      if (lookahead == 'd') ADVANCE(984);
      END_STATE();
    case 571:
      if (lookahead == 'd') ADVANCE(936);
      END_STATE();
    case 572:
      if (lookahead == 'd') ADVANCE(976);
      END_STATE();
    case 573:
      if (lookahead == 'd') ADVANCE(1008);
      END_STATE();
    case 574:
      if (lookahead == 'd') ADVANCE(998);
      END_STATE();
    case 575:
      if (lookahead == 'd') ADVANCE(754);
      if (lookahead == 't') ADVANCE(671);
      END_STATE();
    case 576:
      if (lookahead == 'd') ADVANCE(548);
      END_STATE();
    case 577:
      if (lookahead == 'd') ADVANCE(750);
      END_STATE();
    case 578:
      if (lookahead == 'd') ADVANCE(590);
      END_STATE();
    case 579:
      if (lookahead == 'd') ADVANCE(821);
      END_STATE();
    case 580:
      if (lookahead == 'd') ADVANCE(781);
      END_STATE();
    case 581:
      if (lookahead == 'd') ADVANCE(601);
      END_STATE();
    case 582:
      if (lookahead == 'd') ADVANCE(610);
      END_STATE();
    case 583:
      if (lookahead == 'd') ADVANCE(705);
      END_STATE();
    case 584:
      if (lookahead == 'd') ADVANCE(629);
      END_STATE();
    case 585:
      if (lookahead == 'd') ADVANCE(863);
      END_STATE();
    case 586:
      if (lookahead == 'e') ADVANCE(2053);
      END_STATE();
    case 587:
      if (lookahead == 'e') ADVANCE(963);
      if (lookahead == 's') ADVANCE(647);
      END_STATE();
    case 588:
      if (lookahead == 'e') ADVANCE(718);
      END_STATE();
    case 589:
      if (lookahead == 'e') ADVANCE(943);
      END_STATE();
    case 590:
      if (lookahead == 'e') ADVANCE(989);
      END_STATE();
    case 591:
      if (lookahead == 'e') ADVANCE(27);
      END_STATE();
    case 592:
      if (lookahead == 'e') ADVANCE(1019);
      END_STATE();
    case 593:
      if (lookahead == 'e') ADVANCE(915);
      END_STATE();
    case 594:
      if (lookahead == 'e') ADVANCE(974);
      END_STATE();
    case 595:
      if (lookahead == 'e') ADVANCE(1021);
      END_STATE();
    case 596:
      if (lookahead == 'e') ADVANCE(917);
      END_STATE();
    case 597:
      if (lookahead == 'e') ADVANCE(958);
      END_STATE();
    case 598:
      if (lookahead == 'e') ADVANCE(982);
      END_STATE();
    case 599:
      if (lookahead == 'e') ADVANCE(991);
      END_STATE();
    case 600:
      if (lookahead == 'e') ADVANCE(894);
      END_STATE();
    case 601:
      if (lookahead == 'e') ADVANCE(939);
      END_STATE();
    case 602:
      if (lookahead == 'e') ADVANCE(941);
      END_STATE();
    case 603:
      if (lookahead == 'e') ADVANCE(970);
      END_STATE();
    case 604:
      if (lookahead == 'e') ADVANCE(997);
      END_STATE();
    case 605:
      if (lookahead == 'e') ADVANCE(1004);
      END_STATE();
    case 606:
      if (lookahead == 'e') ADVANCE(902);
      END_STATE();
    case 607:
      if (lookahead == 'e') ADVANCE(935);
      END_STATE();
    case 608:
      if (lookahead == 'e') ADVANCE(931);
      END_STATE();
    case 609:
      if (lookahead == 'e') ADVANCE(933);
      END_STATE();
    case 610:
      if (lookahead == 'e') ADVANCE(923);
      END_STATE();
    case 611:
      if (lookahead == 'e') ADVANCE(556);
      if (lookahead == 'p') ADVANCE(621);
      END_STATE();
    case 612:
      if (lookahead == 'e') ADVANCE(694);
      END_STATE();
    case 613:
      if (lookahead == 'e') ADVANCE(712);
      END_STATE();
    case 614:
      if (lookahead == 'e') ADVANCE(569);
      if (lookahead == 't') ADVANCE(545);
      END_STATE();
    case 615:
      if (lookahead == 'e') ADVANCE(841);
      if (lookahead == 'i') ADVANCE(693);
      if (lookahead == 'o') ADVANCE(798);
      if (lookahead == 'u') ADVANCE(735);
      END_STATE();
    case 616:
      if (lookahead == 'e') ADVANCE(547);
      END_STATE();
    case 617:
      if (lookahead == 'e') ADVANCE(585);
      END_STATE();
    case 618:
      if (lookahead == 'e') ADVANCE(530);
      if (lookahead == 'o') ADVANCE(741);
      END_STATE();
    case 619:
      if (lookahead == 'e') ADVANCE(558);
      END_STATE();
    case 620:
      if (lookahead == 'e') ADVANCE(784);
      if (lookahead == 'l') ADVANCE(1016);
      END_STATE();
    case 621:
      if (lookahead == 'e') ADVANCE(804);
      END_STATE();
    case 622:
      if (lookahead == 'e') ADVANCE(528);
      END_STATE();
    case 623:
      if (lookahead == 'e') ADVANCE(710);
      END_STATE();
    case 624:
      if (lookahead == 'e') ADVANCE(751);
      END_STATE();
    case 625:
      if (lookahead == 'e') ADVANCE(711);
      END_STATE();
    case 626:
      if (lookahead == 'e') ADVANCE(792);
      END_STATE();
    case 627:
      if (lookahead == 'e') ADVANCE(793);
      END_STATE();
    case 628:
      if (lookahead == 'e') ADVANCE(794);
      END_STATE();
    case 629:
      if (lookahead == 'e') ADVANCE(795);
      END_STATE();
    case 630:
      if (lookahead == 'e') ADVANCE(800);
      END_STATE();
    case 631:
      if (lookahead == 'e') ADVANCE(720);
      END_STATE();
    case 632:
      if (lookahead == 'e') ADVANCE(725);
      END_STATE();
    case 633:
      if (lookahead == 'e') ADVANCE(802);
      END_STATE();
    case 634:
      if (lookahead == 'e') ADVANCE(844);
      END_STATE();
    case 635:
      if (lookahead == 'e') ADVANCE(746);
      END_STATE();
    case 636:
      if (lookahead == 'e') ADVANCE(745);
      END_STATE();
    case 637:
      if (lookahead == 'f') ADVANCE(837);
      END_STATE();
    case 638:
      if (lookahead == 'f') ADVANCE(972);
      END_STATE();
    case 639:
      if (lookahead == 'f') ADVANCE(994);
      END_STATE();
    case 640:
      if (lookahead == 'f') ADVANCE(642);
      if (lookahead == 'n') ADVANCE(551);
      if (lookahead == 'x') ADVANCE(611);
      END_STATE();
    case 641:
      if (lookahead == 'f') ADVANCE(789);
      END_STATE();
    case 642:
      if (lookahead == 'f') ADVANCE(619);
      END_STATE();
    case 643:
      if (lookahead == 'f') ADVANCE(639);
      if (lookahead == 'n') ADVANCE(995);
      END_STATE();
    case 644:
      if (lookahead == 'f') ADVANCE(676);
      END_STATE();
    case 645:
      if (lookahead == 'f') ADVANCE(684);
      END_STATE();
    case 646:
      if (lookahead == 'f') ADVANCE(628);
      END_STATE();
    case 647:
      if (lookahead == 'g') ADVANCE(964);
      END_STATE();
    case 648:
      if (lookahead == 'g') ADVANCE(1012);
      END_STATE();
    case 649:
      if (lookahead == 'g') ADVANCE(922);
      END_STATE();
    case 650:
      if (lookahead == 'g') ADVANCE(1011);
      END_STATE();
    case 651:
      if (lookahead == 'g') ADVANCE(925);
      END_STATE();
    case 652:
      if (lookahead == 'g') ADVANCE(1014);
      END_STATE();
    case 653:
      if (lookahead == 'g') ADVANCE(37);
      END_STATE();
    case 654:
      if (lookahead == 'g') ADVANCE(541);
      END_STATE();
    case 655:
      if (lookahead == 'g') ADVANCE(596);
      END_STATE();
    case 656:
      if (lookahead == 'g') ADVANCE(657);
      END_STATE();
    case 657:
      if (lookahead == 'g') ADVANCE(626);
      END_STATE();
    case 658:
      if (lookahead == 'h') ADVANCE(978);
      END_STATE();
    case 659:
      if (lookahead == 'h') ADVANCE(527);
      END_STATE();
    case 660:
      if (lookahead == 'h') ADVANCE(617);
      if (lookahead == 'o') ADVANCE(809);
      END_STATE();
    case 661:
      if (lookahead == 'h') ADVANCE(780);
      END_STATE();
    case 662:
      if (lookahead == 'h') ADVANCE(627);
      END_STATE();
    case 663:
      if (lookahead == 'i') ADVANCE(772);
      END_STATE();
    case 664:
      if (lookahead == 'i') ADVANCE(726);
      END_STATE();
    case 665:
      if (lookahead == 'i') ADVANCE(552);
      END_STATE();
    case 666:
      if (lookahead == 'i') ADVANCE(773);
      END_STATE();
    case 667:
      if (lookahead == 'i') ADVANCE(816);
      if (lookahead == 'o') ADVANCE(557);
      END_STATE();
    case 668:
      if (lookahead == 'i') ADVANCE(549);
      END_STATE();
    case 669:
      if (lookahead == 'i') ADVANCE(815);
      END_STATE();
    case 670:
      if (lookahead == 'i') ADVANCE(783);
      END_STATE();
    case 671:
      if (lookahead == 'i') ADVANCE(565);
      END_STATE();
    case 672:
      if (lookahead == 'i') ADVANCE(850);
      END_STATE();
    case 673:
      if (lookahead == 'i') ADVANCE(768);
      END_STATE();
    case 674:
      if (lookahead == 'i') ADVANCE(635);
      END_STATE();
    case 675:
      if (lookahead == 'i') ADVANCE(737);
      END_STATE();
    case 676:
      if (lookahead == 'i') ADVANCE(566);
      END_STATE();
    case 677:
      if (lookahead == 'i') ADVANCE(820);
      END_STATE();
    case 678:
      if (lookahead == 'i') ADVANCE(743);
      END_STATE();
    case 679:
      if (lookahead == 'i') ADVANCE(822);
      END_STATE();
    case 680:
      if (lookahead == 'i') ADVANCE(758);
      END_STATE();
    case 681:
      if (lookahead == 'i') ADVANCE(744);
      END_STATE();
    case 682:
      if (lookahead == 'i') ADVANCE(760);
      END_STATE();
    case 683:
      if (lookahead == 'i') ADVANCE(722);
      END_STATE();
    case 684:
      if (lookahead == 'i') ADVANCE(709);
      END_STATE();
    case 685:
      if (lookahead == 'k') ADVANCE(951);
      END_STATE();
    case 686:
      if (lookahead == 'k') ADVANCE(921);
      END_STATE();
    case 687:
      if (lookahead == 'k') ADVANCE(1000);
      END_STATE();
    case 688:
      if (lookahead == 'l') ADVANCE(618);
      END_STATE();
    case 689:
      if (lookahead == 'l') ADVANCE(934);
      END_STATE();
    case 690:
      if (lookahead == 'l') ADVANCE(953);
      END_STATE();
    case 691:
      if (lookahead == 'l') ADVANCE(993);
      END_STATE();
    case 692:
      if (lookahead == 'l') ADVANCE(753);
      END_STATE();
    case 693:
      if (lookahead == 'l') ADVANCE(689);
      END_STATE();
    case 694:
      if (lookahead == 'l') ADVANCE(777);
      END_STATE();
    case 695:
      if (lookahead == 'l') ADVANCE(576);
      END_STATE();
    case 696:
      if (lookahead == 'l') ADVANCE(669);
      END_STATE();
    case 697:
      if (lookahead == 'l') ADVANCE(579);
      END_STATE();
    case 698:
      if (lookahead == 'l') ADVANCE(757);
      END_STATE();
    case 699:
      if (lookahead == 'l') ADVANCE(823);
      END_STATE();
    case 700:
      if (lookahead == 'l') ADVANCE(843);
      END_STATE();
    case 701:
      if (lookahead == 'l') ADVANCE(691);
      END_STATE();
    case 702:
      if (lookahead == 'l') ADVANCE(595);
      END_STATE();
    case 703:
      if (lookahead == 'l') ADVANCE(517);
      END_STATE();
    case 704:
      if (lookahead == 'l') ADVANCE(838);
      END_STATE();
    case 705:
      if (lookahead == 'l') ADVANCE(634);
      END_STATE();
    case 706:
      if (lookahead == 'l') ADVANCE(602);
      END_STATE();
    case 707:
      if (lookahead == 'l') ADVANCE(603);
      END_STATE();
    case 708:
      if (lookahead == 'l') ADVANCE(604);
      END_STATE();
    case 709:
      if (lookahead == 'l') ADVANCE(609);
      END_STATE();
    case 710:
      if (lookahead == 'l') ADVANCE(761);
      END_STATE();
    case 711:
      if (lookahead == 'l') ADVANCE(679);
      END_STATE();
    case 712:
      if (lookahead == 'm') ADVANCE(947);
      END_STATE();
    case 713:
      if (lookahead == 'm') ADVANCE(1013);
      END_STATE();
    case 714:
      if (lookahead == 'm') ADVANCE(980);
      END_STATE();
    case 715:
      if (lookahead == 'm') ADVANCE(724);
      END_STATE();
    case 716:
      if (lookahead == 'm') ADVANCE(522);
      if (lookahead == 't') ADVANCE(512);
      END_STATE();
    case 717:
      if (lookahead == 'm') ADVANCE(588);
      END_STATE();
    case 718:
      if (lookahead == 'm') ADVANCE(764);
      if (lookahead == 'r') ADVANCE(861);
      END_STATE();
    case 719:
      if (lookahead == 'm') ADVANCE(592);
      if (lookahead == 't') ADVANCE(702);
      END_STATE();
    case 720:
      if (lookahead == 'm') ADVANCE(636);
      END_STATE();
    case 721:
      if (lookahead == 'm') ADVANCE(607);
      END_STATE();
    case 722:
      if (lookahead == 'm') ADVANCE(624);
      END_STATE();
    case 723:
      if (lookahead == 'm') ADVANCE(632);
      END_STATE();
    case 724:
      if (lookahead == 'm') ADVANCE(756);
      END_STATE();
    case 725:
      if (lookahead == 'm') ADVANCE(770);
      END_STATE();
    case 726:
      if (lookahead == 'n') ADVANCE(653);
      END_STATE();
    case 727:
      if (lookahead == 'n') ADVANCE(905);
      END_STATE();
    case 728:
      if (lookahead == 'n') ADVANCE(967);
      END_STATE();
    case 729:
      if (lookahead == 'n') ADVANCE(987);
      END_STATE();
    case 730:
      if (lookahead == 'n') ADVANCE(1009);
      END_STATE();
    case 731:
      if (lookahead == 'n') ADVANCE(1026);
      END_STATE();
    case 732:
      if (lookahead == 'n') ADVANCE(937);
      END_STATE();
    case 733:
      if (lookahead == 'n') ADVANCE(1002);
      END_STATE();
    case 734:
      if (lookahead == 'n') ADVANCE(577);
      END_STATE();
    case 735:
      if (lookahead == 'n') ADVANCE(562);
      END_STATE();
    case 736:
      if (lookahead == 'n') ADVANCE(824);
      END_STATE();
    case 737:
      if (lookahead == 'n') ADVANCE(650);
      END_STATE();
    case 738:
      if (lookahead == 'n') ADVANCE(560);
      END_STATE();
    case 739:
      if (lookahead == 'n') ADVANCE(572);
      END_STATE();
    case 740:
      if (lookahead == 'n') ADVANCE(573);
      END_STATE();
    case 741:
      if (lookahead == 'n') ADVANCE(593);
      END_STATE();
    case 742:
      if (lookahead == 'n') ADVANCE(830);
      END_STATE();
    case 743:
      if (lookahead == 'n') ADVANCE(832);
      END_STATE();
    case 744:
      if (lookahead == 'n') ADVANCE(834);
      END_STATE();
    case 745:
      if (lookahead == 'n') ADVANCE(835);
      END_STATE();
    case 746:
      if (lookahead == 'n') ADVANCE(561);
      END_STATE();
    case 747:
      if (lookahead == 'n') ADVANCE(785);
      END_STATE();
    case 748:
      if (lookahead == 'o') ADVANCE(788);
      END_STATE();
    case 749:
      if (lookahead == 'o') ADVANCE(857);
      END_STATE();
    case 750:
      if (lookahead == 'o') ADVANCE(714);
      END_STATE();
    case 751:
      if (lookahead == 'o') ADVANCE(859);
      END_STATE();
    case 752:
      if (lookahead == 'o') ADVANCE(778);
      END_STATE();
    case 753:
      if (lookahead == 'o') ADVANCE(651);
      END_STATE();
    case 754:
      if (lookahead == 'o') ADVANCE(728);
      END_STATE();
    case 755:
      if (lookahead == 'o') ADVANCE(526);
      END_STATE();
    case 756:
      if (lookahead == 'o') ADVANCE(730);
      END_STATE();
    case 757:
      if (lookahead == 'o') ADVANCE(555);
      END_STATE();
    case 758:
      if (lookahead == 'o') ADVANCE(731);
      END_STATE();
    case 759:
      if (lookahead == 'o') ADVANCE(678);
      END_STATE();
    case 760:
      if (lookahead == 'o') ADVANCE(732);
      END_STATE();
    case 761:
      if (lookahead == 'o') ADVANCE(531);
      END_STATE();
    case 762:
      if (lookahead == 'o') ADVANCE(808);
      END_STATE();
    case 763:
      if (lookahead == 'o') ADVANCE(810);
      END_STATE();
    case 764:
      if (lookahead == 'o') ADVANCE(581);
      END_STATE();
    case 765:
      if (lookahead == 'o') ADVANCE(645);
      END_STATE();
    case 766:
      if (lookahead == 'o') ADVANCE(858);
      END_STATE();
    case 767:
      if (lookahead == 'o') ADVANCE(807);
      END_STATE();
    case 768:
      if (lookahead == 'o') ADVANCE(721);
      END_STATE();
    case 769:
      if (lookahead == 'o') ADVANCE(535);
      END_STATE();
    case 770:
      if (lookahead == 'o') ADVANCE(582);
      END_STATE();
    case 771:
      if (lookahead == 'o') ADVANCE(681);
      END_STATE();
    case 772:
      if (lookahead == 'p') ADVANCE(906);
      END_STATE();
    case 773:
      if (lookahead == 'p') ADVANCE(969);
      END_STATE();
    case 774:
      if (lookahead == 'p') ADVANCE(965);
      END_STATE();
    case 775:
      if (lookahead == 'p') ADVANCE(1038);
      END_STATE();
    case 776:
      if (lookahead == 'p') ADVANCE(924);
      END_STATE();
    case 777:
      if (lookahead == 'p') ADVANCE(946);
      END_STATE();
    case 778:
      if (lookahead == 'p') ADVANCE(1007);
      END_STATE();
    case 779:
      if (lookahead == 'p') ADVANCE(759);
      END_STATE();
    case 780:
      if (lookahead == 'p') ADVANCE(805);
      END_STATE();
    case 781:
      if (lookahead == 'p') ADVANCE(703);
      END_STATE();
    case 782:
      if (lookahead == 'p') ADVANCE(536);
      END_STATE();
    case 783:
      if (lookahead == 'p') ADVANCE(598);
      END_STATE();
    case 784:
      if (lookahead == 'p') ADVANCE(762);
      END_STATE();
    case 785:
      if (lookahead == 'p') ADVANCE(771);
      END_STATE();
    case 786:
      if (lookahead == 'r') ADVANCE(506);
      if (lookahead == 'u') ADVANCE(787);
      END_STATE();
    case 787:
      if (lookahead == 'r') ADVANCE(811);
      END_STATE();
    case 788:
      if (lookahead == 'r') ADVANCE(1349);
      END_STATE();
    case 789:
      if (lookahead == 'r') ADVANCE(949);
      END_STATE();
    case 790:
      if (lookahead == 'r') ADVANCE(912);
      END_STATE();
    case 791:
      if (lookahead == 'r') ADVANCE(909);
      END_STATE();
    case 792:
      if (lookahead == 'r') ADVANCE(1025);
      END_STATE();
    case 793:
      if (lookahead == 'r') ADVANCE(1032);
      END_STATE();
    case 794:
      if (lookahead == 'r') ADVANCE(1024);
      END_STATE();
    case 795:
      if (lookahead == 'r') ADVANCE(1036);
      END_STATE();
    case 796:
      if (lookahead == 'r') ADVANCE(575);
      END_STATE();
    case 797:
      if (lookahead == 'r') ADVANCE(668);
      END_STATE();
    case 798:
      if (lookahead == 'r') ADVANCE(563);
      END_STATE();
    case 799:
      if (lookahead == 'r') ADVANCE(638);
      END_STATE();
    case 800:
      if (lookahead == 'r') ADVANCE(819);
      END_STATE();
    case 801:
      if (lookahead == 'r') ADVANCE(695);
      END_STATE();
    case 802:
      if (lookahead == 'r') ADVANCE(812);
      END_STATE();
    case 803:
      if (lookahead == 'r') ADVANCE(729);
      END_STATE();
    case 804:
      if (lookahead == 'r') ADVANCE(674);
      END_STATE();
    case 805:
      if (lookahead == 'r') ADVANCE(765);
      END_STATE();
    case 806:
      if (lookahead == 'r') ADVANCE(574);
      END_STATE();
    case 807:
      if (lookahead == 'r') ADVANCE(697);
      END_STATE();
    case 808:
      if (lookahead == 'r') ADVANCE(831);
      END_STATE();
    case 809:
      if (lookahead == 'r') ADVANCE(616);
      END_STATE();
    case 810:
      if (lookahead == 'r') ADVANCE(584);
      END_STATE();
    case 811:
      if (lookahead == 's') ADVANCE(748);
      END_STATE();
    case 812:
      if (lookahead == 's') ADVANCE(1005);
      END_STATE();
    case 813:
      if (lookahead == 's') ADVANCE(546);
      END_STATE();
    case 814:
      if (lookahead == 's') ADVANCE(813);
      END_STATE();
    case 815:
      if (lookahead == 's') ADVANCE(658);
      END_STATE();
    case 816:
      if (lookahead == 's') ADVANCE(825);
      END_STATE();
    case 817:
      if (lookahead == 's') ADVANCE(749);
      END_STATE();
    case 818:
      if (lookahead == 's') ADVANCE(652);
      END_STATE();
    case 819:
      if (lookahead == 's') ADVANCE(680);
      END_STATE();
    case 820:
      if (lookahead == 's') ADVANCE(829);
      END_STATE();
    case 821:
      if (lookahead == 's') ADVANCE(782);
      END_STATE();
    case 822:
      if (lookahead == 's') ADVANCE(833);
      END_STATE();
    case 823:
      if (lookahead == 's') ADVANCE(586);
      END_STATE();
    case 824:
      if (lookahead == 's') ADVANCE(646);
      END_STATE();
    case 825:
      if (lookahead == 't') ADVANCE(955);
      END_STATE();
    case 826:
      if (lookahead == 't') ADVANCE(961);
      END_STATE();
    case 827:
      if (lookahead == 't') ADVANCE(1018);
      END_STATE();
    case 828:
      if (lookahead == 't') ADVANCE(927);
      END_STATE();
    case 829:
      if (lookahead == 't') ADVANCE(907);
      END_STATE();
    case 830:
      if (lookahead == 't') ADVANCE(929);
      END_STATE();
    case 831:
      if (lookahead == 't') ADVANCE(1015);
      END_STATE();
    case 832:
      if (lookahead == 't') ADVANCE(1030);
      END_STATE();
    case 833:
      if (lookahead == 't') ADVANCE(1034);
      END_STATE();
    case 834:
      if (lookahead == 't') ADVANCE(1003);
      END_STATE();
    case 835:
      if (lookahead == 't') ADVANCE(900);
      END_STATE();
    case 836:
      if (lookahead == 't') ADVANCE(1001);
      END_STATE();
    case 837:
      if (lookahead == 't') ADVANCE(664);
      END_STATE();
    case 838:
      if (lookahead == 't') ADVANCE(871);
      END_STATE();
    case 839:
      if (lookahead == 't') ADVANCE(613);
      END_STATE();
    case 840:
      if (lookahead == 't') ADVANCE(662);
      END_STATE();
    case 841:
      if (lookahead == 't') ADVANCE(553);
      END_STATE();
    case 842:
      if (lookahead == 't') ADVANCE(797);
      END_STATE();
    case 843:
      if (lookahead == 't') ADVANCE(654);
      END_STATE();
    case 844:
      if (lookahead == 't') ADVANCE(683);
      END_STATE();
    case 845:
      if (lookahead == 't') ADVANCE(597);
      END_STATE();
    case 846:
      if (lookahead == 't') ADVANCE(599);
      END_STATE();
    case 847:
      if (lookahead == 't') ADVANCE(600);
      END_STATE();
    case 848:
      if (lookahead == 't') ADVANCE(605);
      END_STATE();
    case 849:
      if (lookahead == 't') ADVANCE(606);
      END_STATE();
    case 850:
      if (lookahead == 't') ADVANCE(625);
      END_STATE();
    case 851:
      if (lookahead == 't') ADVANCE(540);
      END_STATE();
    case 852:
      if (lookahead == 't') ADVANCE(682);
      END_STATE();
    case 853:
      if (lookahead == 't') ADVANCE(542);
      END_STATE();
    case 854:
      if (lookahead == 'u') ADVANCE(649);
      END_STATE();
    case 855:
      if (lookahead == 'u') ADVANCE(700);
      END_STATE();
    case 856:
      if (lookahead == 'u') ADVANCE(803);
      END_STATE();
    case 857:
      if (lookahead == 'u') ADVANCE(739);
      END_STATE();
    case 858:
      if (lookahead == 'u') ADVANCE(740);
      END_STATE();
    case 859:
      if (lookahead == 'u') ADVANCE(836);
      END_STATE();
    case 860:
      if (lookahead == 'u') ADVANCE(704);
      END_STATE();
    case 861:
      if (lookahead == 'u') ADVANCE(706);
      END_STATE();
    case 862:
      if (lookahead == 'u') ADVANCE(847);
      END_STATE();
    case 863:
      if (lookahead == 'u') ADVANCE(708);
      END_STATE();
    case 864:
      if (lookahead == 'u') ADVANCE(849);
      END_STATE();
    case 865:
      if (lookahead == 'v') ADVANCE(539);
      END_STATE();
    case 866:
      if (lookahead == 'v') ADVANCE(589);
      END_STATE();
    case 867:
      if (lookahead == 'v') ADVANCE(591);
      if (lookahead == 'y') ADVANCE(996);
      END_STATE();
    case 868:
      if (lookahead == 'w') ADVANCE(1017);
      END_STATE();
    case 869:
      if (lookahead == 'w') ADVANCE(747);
      END_STATE();
    case 870:
      if (lookahead == 'w') ADVANCE(733);
      END_STATE();
    case 871:
      if (lookahead == 'y') ADVANCE(926);
      END_STATE();
    case 872:
      if (lookahead == 'y') ADVANCE(779);
      END_STATE();
    case 873:
      if (lookahead == 'y') ADVANCE(633);
      END_STATE();
    case 874:
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1307);
      END_STATE();
    case 875:
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(2058);
      END_STATE();
    case 876:
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1341);
      END_STATE();
    case 877:
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1333);
      END_STATE();
    case 878:
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1337);
      END_STATE();
    case 879:
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1339);
      END_STATE();
    case 880:
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1346);
      END_STATE();
    case 881:
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(1275);
      END_STATE();
    case 882:
      if (lookahead == '/' ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1289);
      END_STATE();
    case 883:
      if (eof) ADVANCE(884);
      ADVANCE_MAP(
        '\n', 887,
        '\r', 891,
        '#', 893,
        '$', 899,
        '.', 874,
        'a', 568,
        'b', 513,
        'c', 688,
        'd', 507,
        'e', 640,
        'f', 615,
        'g', 508,
        'h', 612,
        'i', 839,
        'j', 641,
        'k', 665,
        'l', 667,
        'm', 587,
        'o', 774,
        'p', 521,
        'r', 519,
        's', 509,
        't', 516,
        'v', 630,
        'w', 1029,
        'x', 775,
        '\t', 892,
        0x0b, 892,
        '\f', 892,
        ' ', 892,
      );
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1297);
      END_STATE();
    case 884:
      ACCEPT_TOKEN(ts_builtin_sym_end);
      END_STATE();
    case 885:
      ACCEPT_TOKEN(sym__newline);
      END_STATE();
    case 886:
      ACCEPT_TOKEN(sym__newline);
      if (lookahead == '\\') ADVANCE(5);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(505);
      END_STATE();
    case 887:
      ACCEPT_TOKEN(sym__newline);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(892);
      END_STATE();
    case 888:
      ACCEPT_TOKEN(aux_sym__space_token1);
      if (lookahead == ',') ADVANCE(1283);
      if (lookahead == '\\') ADVANCE(5);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(24);
      END_STATE();
    case 889:
      ACCEPT_TOKEN(aux_sym__space_token1);
      if (lookahead == '\\') ADVANCE(5);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(505);
      END_STATE();
    case 890:
      ACCEPT_TOKEN(sym_backslash);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(890);
      END_STATE();
    case 891:
      ACCEPT_TOKEN(aux_sym__statement_token1);
      if (lookahead == '\n') ADVANCE(887);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(892);
      END_STATE();
    case 892:
      ACCEPT_TOKEN(aux_sym__statement_token1);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(892);
      END_STATE();
    case 893:
      ACCEPT_TOKEN(sym_comment);
      if (lookahead != 0 &&
          lookahead != '\n' &&
          lookahead != '\r') ADVANCE(893);
      END_STATE();
    case 894:
      ACCEPT_TOKEN(anon_sym_execute);
      END_STATE();
    case 895:
      ACCEPT_TOKEN(anon_sym_execute);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 896:
      ACCEPT_TOKEN(anon_sym_run);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'c') ADVANCE(283);
      if (lookahead == 'f') ADVANCE(65);
      if (lookahead == 't') ADVANCE(222);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 897:
      ACCEPT_TOKEN(anon_sym_run);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1704);
      if (lookahead == 'f') ADVANCE(1424);
      if (lookahead == 't') ADVANCE(1631);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 898:
      ACCEPT_TOKEN(anon_sym_run);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 899:
      ACCEPT_TOKEN(sym_macro_indicator);
      if (lookahead == '(') ADVANCE(1318);
      END_STATE();
    case 900:
      ACCEPT_TOKEN(anon_sym_advancement);
      END_STATE();
    case 901:
      ACCEPT_TOKEN(anon_sym_advancement);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 902:
      ACCEPT_TOKEN(anon_sym_attribute);
      END_STATE();
    case 903:
      ACCEPT_TOKEN(anon_sym_attribute);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 904:
      ACCEPT_TOKEN(anon_sym_ban);
      if (lookahead == '-') ADVANCE(663);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(255);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 905:
      ACCEPT_TOKEN(anon_sym_ban);
      if (lookahead == '-') ADVANCE(663);
      if (lookahead == 'l') ADVANCE(677);
      END_STATE();
    case 906:
      ACCEPT_TOKEN(anon_sym_ban_DASHip);
      END_STATE();
    case 907:
      ACCEPT_TOKEN(anon_sym_banlist);
      END_STATE();
    case 908:
      ACCEPT_TOKEN(anon_sym_banlist);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 909:
      ACCEPT_TOKEN(anon_sym_bossbar);
      END_STATE();
    case 910:
      ACCEPT_TOKEN(anon_sym_bossbar);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 911:
      ACCEPT_TOKEN(anon_sym_bossbar);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 912:
      ACCEPT_TOKEN(anon_sym_clear);
      END_STATE();
    case 913:
      ACCEPT_TOKEN(anon_sym_clear);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 914:
      ACCEPT_TOKEN(anon_sym_clear);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 915:
      ACCEPT_TOKEN(anon_sym_clone);
      END_STATE();
    case 916:
      ACCEPT_TOKEN(anon_sym_clone);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 917:
      ACCEPT_TOKEN(anon_sym_damage);
      END_STATE();
    case 918:
      ACCEPT_TOKEN(anon_sym_damage);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 919:
      ACCEPT_TOKEN(anon_sym_data);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 920:
      ACCEPT_TOKEN(anon_sym_data);
      if (lookahead == 'p') ADVANCE(532);
      END_STATE();
    case 921:
      ACCEPT_TOKEN(anon_sym_datapack);
      END_STATE();
    case 922:
      ACCEPT_TOKEN(anon_sym_debug);
      END_STATE();
    case 923:
      ACCEPT_TOKEN(anon_sym_defaultgamemode);
      END_STATE();
    case 924:
      ACCEPT_TOKEN(anon_sym_deop);
      END_STATE();
    case 925:
      ACCEPT_TOKEN(anon_sym_dialog);
      END_STATE();
    case 926:
      ACCEPT_TOKEN(anon_sym_difficulty);
      END_STATE();
    case 927:
      ACCEPT_TOKEN(anon_sym_effect);
      END_STATE();
    case 928:
      ACCEPT_TOKEN(anon_sym_effect);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 929:
      ACCEPT_TOKEN(anon_sym_enchant);
      END_STATE();
    case 930:
      ACCEPT_TOKEN(anon_sym_enchant);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 931:
      ACCEPT_TOKEN(anon_sym_experience);
      END_STATE();
    case 932:
      ACCEPT_TOKEN(anon_sym_experience);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 933:
      ACCEPT_TOKEN(anon_sym_fetchprofile);
      END_STATE();
    case 934:
      ACCEPT_TOKEN(anon_sym_fill);
      if (lookahead == 'b') ADVANCE(673);
      END_STATE();
    case 935:
      ACCEPT_TOKEN(anon_sym_fillbiome);
      END_STATE();
    case 936:
      ACCEPT_TOKEN(anon_sym_forceload);
      END_STATE();
    case 937:
      ACCEPT_TOKEN(anon_sym_function);
      END_STATE();
    case 938:
      ACCEPT_TOKEN(anon_sym_function);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 939:
      ACCEPT_TOKEN(anon_sym_gamemode);
      END_STATE();
    case 940:
      ACCEPT_TOKEN(anon_sym_gamemode);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 941:
      ACCEPT_TOKEN(anon_sym_gamerule);
      END_STATE();
    case 942:
      ACCEPT_TOKEN(anon_sym_gamerule);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 943:
      ACCEPT_TOKEN(anon_sym_give);
      END_STATE();
    case 944:
      ACCEPT_TOKEN(anon_sym_give);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 945:
      ACCEPT_TOKEN(anon_sym_give);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 946:
      ACCEPT_TOKEN(anon_sym_help);
      END_STATE();
    case 947:
      ACCEPT_TOKEN(anon_sym_item);
      END_STATE();
    case 948:
      ACCEPT_TOKEN(anon_sym_item);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(1072);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 949:
      ACCEPT_TOKEN(anon_sym_jfr);
      END_STATE();
    case 950:
      ACCEPT_TOKEN(anon_sym_jfr);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 951:
      ACCEPT_TOKEN(anon_sym_kick);
      END_STATE();
    case 952:
      ACCEPT_TOKEN(anon_sym_kick);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 953:
      ACCEPT_TOKEN(anon_sym_kill);
      END_STATE();
    case 954:
      ACCEPT_TOKEN(anon_sym_kill);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 955:
      ACCEPT_TOKEN(anon_sym_list);
      END_STATE();
    case 956:
      ACCEPT_TOKEN(anon_sym_list);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 957:
      ACCEPT_TOKEN(anon_sym_list);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 958:
      ACCEPT_TOKEN(anon_sym_locate);
      END_STATE();
    case 959:
      ACCEPT_TOKEN(anon_sym_locate);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 960:
      ACCEPT_TOKEN(anon_sym_locate);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 961:
      ACCEPT_TOKEN(anon_sym_loot);
      END_STATE();
    case 962:
      ACCEPT_TOKEN(anon_sym_loot);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 963:
      ACCEPT_TOKEN(anon_sym_me);
      END_STATE();
    case 964:
      ACCEPT_TOKEN(anon_sym_msg);
      END_STATE();
    case 965:
      ACCEPT_TOKEN(anon_sym_op);
      END_STATE();
    case 966:
      ACCEPT_TOKEN(anon_sym_op);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 967:
      ACCEPT_TOKEN(anon_sym_pardon);
      if (lookahead == '-') ADVANCE(666);
      END_STATE();
    case 968:
      ACCEPT_TOKEN(anon_sym_pardon);
      if (lookahead == '-') ADVANCE(666);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 969:
      ACCEPT_TOKEN(anon_sym_pardon_DASHip);
      END_STATE();
    case 970:
      ACCEPT_TOKEN(anon_sym_particle);
      END_STATE();
    case 971:
      ACCEPT_TOKEN(anon_sym_particle);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 972:
      ACCEPT_TOKEN(anon_sym_perf);
      END_STATE();
    case 973:
      ACCEPT_TOKEN(anon_sym_perf);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 974:
      ACCEPT_TOKEN(anon_sym_place);
      END_STATE();
    case 975:
      ACCEPT_TOKEN(anon_sym_place);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 976:
      ACCEPT_TOKEN(anon_sym_playsound);
      END_STATE();
    case 977:
      ACCEPT_TOKEN(anon_sym_playsound);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 978:
      ACCEPT_TOKEN(anon_sym_publish);
      END_STATE();
    case 979:
      ACCEPT_TOKEN(anon_sym_publish);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 980:
      ACCEPT_TOKEN(anon_sym_random);
      END_STATE();
    case 981:
      ACCEPT_TOKEN(anon_sym_random);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 982:
      ACCEPT_TOKEN(anon_sym_recipe);
      END_STATE();
    case 983:
      ACCEPT_TOKEN(anon_sym_recipe);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 984:
      ACCEPT_TOKEN(anon_sym_reload);
      END_STATE();
    case 985:
      ACCEPT_TOKEN(anon_sym_reload);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 986:
      ACCEPT_TOKEN(anon_sym_reload);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 987:
      ACCEPT_TOKEN(anon_sym_return);
      END_STATE();
    case 988:
      ACCEPT_TOKEN(anon_sym_return);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 989:
      ACCEPT_TOKEN(anon_sym_ride);
      END_STATE();
    case 990:
      ACCEPT_TOKEN(anon_sym_ride);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 991:
      ACCEPT_TOKEN(anon_sym_rotate);
      END_STATE();
    case 992:
      ACCEPT_TOKEN(anon_sym_rotate);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'd') ADVANCE(1098);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 993:
      ACCEPT_TOKEN(anon_sym_save_DASHall);
      END_STATE();
    case 994:
      ACCEPT_TOKEN(anon_sym_save_DASHoff);
      END_STATE();
    case 995:
      ACCEPT_TOKEN(anon_sym_save_DASHon);
      END_STATE();
    case 996:
      ACCEPT_TOKEN(anon_sym_say);
      END_STATE();
    case 997:
      ACCEPT_TOKEN(anon_sym_schedule);
      END_STATE();
    case 998:
      ACCEPT_TOKEN(anon_sym_scoreboard);
      END_STATE();
    case 999:
      ACCEPT_TOKEN(anon_sym_seed);
      END_STATE();
    case 1000:
      ACCEPT_TOKEN(anon_sym_setblock);
      END_STATE();
    case 1001:
      ACCEPT_TOKEN(anon_sym_setidletimeout);
      END_STATE();
    case 1002:
      ACCEPT_TOKEN(anon_sym_setworldspawn);
      END_STATE();
    case 1003:
      ACCEPT_TOKEN(anon_sym_spawnpoint);
      END_STATE();
    case 1004:
      ACCEPT_TOKEN(anon_sym_spectate);
      END_STATE();
    case 1005:
      ACCEPT_TOKEN(anon_sym_spreadplayers);
      END_STATE();
    case 1006:
      ACCEPT_TOKEN(anon_sym_stop);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1007:
      ACCEPT_TOKEN(anon_sym_stop);
      if (lookahead == 's') ADVANCE(766);
      END_STATE();
    case 1008:
      ACCEPT_TOKEN(anon_sym_stopsound);
      END_STATE();
    case 1009:
      ACCEPT_TOKEN(anon_sym_summon);
      END_STATE();
    case 1010:
      ACCEPT_TOKEN(anon_sym_summon);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1011:
      ACCEPT_TOKEN(anon_sym_swing);
      END_STATE();
    case 1012:
      ACCEPT_TOKEN(anon_sym_tag);
      END_STATE();
    case 1013:
      ACCEPT_TOKEN(anon_sym_team);
      if (lookahead == 'm') ADVANCE(818);
      END_STATE();
    case 1014:
      ACCEPT_TOKEN(anon_sym_teammsg);
      END_STATE();
    case 1015:
      ACCEPT_TOKEN(anon_sym_teleport);
      END_STATE();
    case 1016:
      ACCEPT_TOKEN(anon_sym_tell);
      if (lookahead == 'r') ADVANCE(514);
      END_STATE();
    case 1017:
      ACCEPT_TOKEN(anon_sym_tellraw);
      END_STATE();
    case 1018:
      ACCEPT_TOKEN(anon_sym_test);
      END_STATE();
    case 1019:
      ACCEPT_TOKEN(anon_sym_time);
      END_STATE();
    case 1020:
      ACCEPT_TOKEN(anon_sym_time);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1257);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1021:
      ACCEPT_TOKEN(anon_sym_title);
      END_STATE();
    case 1022:
      ACCEPT_TOKEN(anon_sym_tm);
      END_STATE();
    case 1023:
      ACCEPT_TOKEN(anon_sym_tp);
      END_STATE();
    case 1024:
      ACCEPT_TOKEN(anon_sym_transfer);
      END_STATE();
    case 1025:
      ACCEPT_TOKEN(anon_sym_trigger);
      END_STATE();
    case 1026:
      ACCEPT_TOKEN(anon_sym_version);
      END_STATE();
    case 1027:
      ACCEPT_TOKEN(anon_sym_version);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1028:
      ACCEPT_TOKEN(anon_sym_w);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(396);
      if (lookahead == 'e') ADVANCE(63);
      if (lookahead == 'h') ADVANCE(245);
      if (lookahead == 'i') ADVANCE(461);
      if (lookahead == 'o') ADVANCE(397);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1029:
      ACCEPT_TOKEN(anon_sym_w);
      if (lookahead == 'a') ADVANCE(872);
      if (lookahead == 'e') ADVANCE(529);
      if (lookahead == 'h') ADVANCE(672);
      if (lookahead == 'o') ADVANCE(801);
      END_STATE();
    case 1030:
      ACCEPT_TOKEN(anon_sym_waypoint);
      END_STATE();
    case 1031:
      ACCEPT_TOKEN(anon_sym_waypoint);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1032:
      ACCEPT_TOKEN(anon_sym_weather);
      END_STATE();
    case 1033:
      ACCEPT_TOKEN(anon_sym_weather);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1034:
      ACCEPT_TOKEN(anon_sym_whitelist);
      END_STATE();
    case 1035:
      ACCEPT_TOKEN(anon_sym_whitelist);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1036:
      ACCEPT_TOKEN(anon_sym_worldborder);
      END_STATE();
    case 1037:
      ACCEPT_TOKEN(anon_sym_worldborder);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1038:
      ACCEPT_TOKEN(anon_sym_xp);
      END_STATE();
    case 1039:
      ACCEPT_TOKEN(anon_sym_xp);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1040:
      ACCEPT_TOKEN(anon_sym_align);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1041:
      ACCEPT_TOKEN(anon_sym_align);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1042:
      ACCEPT_TOKEN(anon_sym_all);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1043:
      ACCEPT_TOKEN(anon_sym_all);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1044:
      ACCEPT_TOKEN(anon_sym_anchored);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1045:
      ACCEPT_TOKEN(anon_sym_anchored);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1046:
      ACCEPT_TOKEN(anon_sym_as);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1047:
      ACCEPT_TOKEN(anon_sym_as);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1048:
      ACCEPT_TOKEN(anon_sym_at);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 't') ADVANCE(62);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1049:
      ACCEPT_TOKEN(anon_sym_at);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1392);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1050:
      ACCEPT_TOKEN(anon_sym_at);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1051:
      ACCEPT_TOKEN(anon_sym_attacker);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1052:
      ACCEPT_TOKEN(anon_sym_attacker);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1053:
      ACCEPT_TOKEN(anon_sym_biome);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1054:
      ACCEPT_TOKEN(anon_sym_biome);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1055:
      ACCEPT_TOKEN(anon_sym_block);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(1057);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1056:
      ACCEPT_TOKEN(anon_sym_block);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1058);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1057:
      ACCEPT_TOKEN(anon_sym_blocks);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1058:
      ACCEPT_TOKEN(anon_sym_blocks);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1059:
      ACCEPT_TOKEN(anon_sym_controller);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1060:
      ACCEPT_TOKEN(anon_sym_controller);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1061:
      ACCEPT_TOKEN(anon_sym_dimension);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1062:
      ACCEPT_TOKEN(anon_sym_entity);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1063:
      ACCEPT_TOKEN(anon_sym_entity);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1064:
      ACCEPT_TOKEN(anon_sym_eyes);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1065:
      ACCEPT_TOKEN(anon_sym_eyes);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1066:
      ACCEPT_TOKEN(anon_sym_facing);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1067:
      ACCEPT_TOKEN(anon_sym_feet);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1068:
      ACCEPT_TOKEN(anon_sym_if);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1069:
      ACCEPT_TOKEN(anon_sym_if);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1070:
      ACCEPT_TOKEN(anon_sym_in);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 's') ADVANCE(195);
      if (lookahead == 'v') ADVANCE(197);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1071:
      ACCEPT_TOKEN(anon_sym_in);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1323);
      if (lookahead == 'v') ADVANCE(1597);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1072:
      ACCEPT_TOKEN(anon_sym_items);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1073:
      ACCEPT_TOKEN(anon_sym_items);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1074:
      ACCEPT_TOKEN(anon_sym_leasher);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1075:
      ACCEPT_TOKEN(anon_sym_leasher);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1076:
      ACCEPT_TOKEN(anon_sym_loaded);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1077:
      ACCEPT_TOKEN(anon_sym_loaded);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1078:
      ACCEPT_TOKEN(anon_sym_masked);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1079:
      ACCEPT_TOKEN(anon_sym_matches);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1080:
      ACCEPT_TOKEN(anon_sym_max);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1081:
      ACCEPT_TOKEN(anon_sym_on);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'l') ADVANCE(494);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1082:
      ACCEPT_TOKEN(anon_sym_on);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(2018);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1083:
      ACCEPT_TOKEN(anon_sym_on);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1084:
      ACCEPT_TOKEN(anon_sym_origin);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1085:
      ACCEPT_TOKEN(anon_sym_origin);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1086:
      ACCEPT_TOKEN(anon_sym_over);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1087:
      ACCEPT_TOKEN(anon_sym_over);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1088:
      ACCEPT_TOKEN(anon_sym_owner);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1089:
      ACCEPT_TOKEN(anon_sym_owner);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1090:
      ACCEPT_TOKEN(anon_sym_passengers);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1091:
      ACCEPT_TOKEN(anon_sym_passengers);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1092:
      ACCEPT_TOKEN(anon_sym_positioned);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1093:
      ACCEPT_TOKEN(anon_sym_positioned);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1094:
      ACCEPT_TOKEN(anon_sym_predicate);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1095:
      ACCEPT_TOKEN(anon_sym_predicate);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1096:
      ACCEPT_TOKEN(anon_sym_result);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1097:
      ACCEPT_TOKEN(anon_sym_result);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1098:
      ACCEPT_TOKEN(anon_sym_rotated);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1099:
      ACCEPT_TOKEN(anon_sym_rotated);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1100:
      ACCEPT_TOKEN(anon_sym_score);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1101:
      ACCEPT_TOKEN(anon_sym_storage);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1102:
      ACCEPT_TOKEN(anon_sym_store);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1103:
      ACCEPT_TOKEN(anon_sym_success);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1104:
      ACCEPT_TOKEN(anon_sym_target);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1105:
      ACCEPT_TOKEN(anon_sym_unless);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1106:
      ACCEPT_TOKEN(anon_sym_unless);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1107:
      ACCEPT_TOKEN(anon_sym_value);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1108:
      ACCEPT_TOKEN(anon_sym_value);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1109:
      ACCEPT_TOKEN(anon_sym_vehicle);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1110:
      ACCEPT_TOKEN(anon_sym_vehicle);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1111:
      ACCEPT_TOKEN(anon_sym_add);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_') ADVANCE(304);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1112:
      ACCEPT_TOKEN(anon_sym_add);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_') ADVANCE(1734);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1113:
      ACCEPT_TOKEN(anon_sym_add_multiplied_base);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1114:
      ACCEPT_TOKEN(anon_sym_add_multiplied_base);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1115:
      ACCEPT_TOKEN(anon_sym_add_multiplied_total);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1116:
      ACCEPT_TOKEN(anon_sym_add_multiplied_total);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1117:
      ACCEPT_TOKEN(anon_sym_add_value);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1118:
      ACCEPT_TOKEN(anon_sym_add_value);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1119:
      ACCEPT_TOKEN(anon_sym_amount);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1120:
      ACCEPT_TOKEN(anon_sym_amount);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1121:
      ACCEPT_TOKEN(anon_sym_append);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1122:
      ACCEPT_TOKEN(anon_sym_append);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1123:
      ACCEPT_TOKEN(anon_sym_base);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1124:
      ACCEPT_TOKEN(anon_sym_base);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1125:
      ACCEPT_TOKEN(anon_sym_buffer);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1126:
      ACCEPT_TOKEN(anon_sym_buffer);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1127:
      ACCEPT_TOKEN(anon_sym_by);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1128:
      ACCEPT_TOKEN(anon_sym_by);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1501);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1129:
      ACCEPT_TOKEN(anon_sym_center);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1130:
      ACCEPT_TOKEN(anon_sym_center);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1131:
      ACCEPT_TOKEN(anon_sym_color);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1132:
      ACCEPT_TOKEN(anon_sym_color);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1133:
      ACCEPT_TOKEN(anon_sym_create);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1134:
      ACCEPT_TOKEN(anon_sym_create);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1135:
      ACCEPT_TOKEN(anon_sym_day);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1673);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1136:
      ACCEPT_TOKEN(anon_sym_daytime);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1137:
      ACCEPT_TOKEN(anon_sym_destroy);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1138:
      ACCEPT_TOKEN(anon_sym_disable);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1139:
      ACCEPT_TOKEN(anon_sym_dismount);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1140:
      ACCEPT_TOKEN(anon_sym_distance);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1141:
      ACCEPT_TOKEN(anon_sym_enable);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1142:
      ACCEPT_TOKEN(anon_sym_enable);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1143:
      ACCEPT_TOKEN(anon_sym_everything);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1144:
      ACCEPT_TOKEN(anon_sym_everything);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1145:
      ACCEPT_TOKEN(anon_sym_export);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'c') ADVANCE(301);
      if (lookahead == 't') ADVANCE(229);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1146:
      ACCEPT_TOKEN(anon_sym_export);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1731);
      if (lookahead == 't') ADVANCE(1638);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1147:
      ACCEPT_TOKEN(anon_sym_exportclosest);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1148:
      ACCEPT_TOKEN(anon_sym_exportclosest);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1149:
      ACCEPT_TOKEN(anon_sym_exportthat);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1150:
      ACCEPT_TOKEN(anon_sym_exportthat);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1151:
      ACCEPT_TOKEN(anon_sym_exportthese);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1152:
      ACCEPT_TOKEN(anon_sym_exportthese);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1153:
      ACCEPT_TOKEN(anon_sym_fail);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1154:
      ACCEPT_TOKEN(anon_sym_feature);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1155:
      ACCEPT_TOKEN(anon_sym_fish);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1156:
      ACCEPT_TOKEN(anon_sym_flush);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1157:
      ACCEPT_TOKEN(anon_sym_force);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1158:
      ACCEPT_TOKEN(anon_sym_from);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1159:
      ACCEPT_TOKEN(anon_sym_gametime);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1160:
      ACCEPT_TOKEN(anon_sym_gametime);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1161:
      ACCEPT_TOKEN(anon_sym_get);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1162:
      ACCEPT_TOKEN(anon_sym_get);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1163:
      ACCEPT_TOKEN(anon_sym_grant);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1164:
      ACCEPT_TOKEN(anon_sym_grant);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1165:
      ACCEPT_TOKEN(anon_sym_hollow);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1166:
      ACCEPT_TOKEN(anon_sym_id);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1167:
      ACCEPT_TOKEN(anon_sym_id);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1168:
      ACCEPT_TOKEN(anon_sym_insert);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1169:
      ACCEPT_TOKEN(anon_sym_insert);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1170:
      ACCEPT_TOKEN(anon_sym_ips);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1171:
      ACCEPT_TOKEN(anon_sym_ips);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1172:
      ACCEPT_TOKEN(anon_sym_jigsaw);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1173:
      ACCEPT_TOKEN(anon_sym_jigsaw);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1174:
      ACCEPT_TOKEN(anon_sym_join);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1175:
      ACCEPT_TOKEN(anon_sym_join);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1176:
      ACCEPT_TOKEN(anon_sym_keep);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1177:
      ACCEPT_TOKEN(anon_sym_keep);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1178:
      ACCEPT_TOKEN(anon_sym_leave);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1179:
      ACCEPT_TOKEN(anon_sym_leave);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1180:
      ACCEPT_TOKEN(anon_sym_levels);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1181:
      ACCEPT_TOKEN(anon_sym_levels);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1182:
      ACCEPT_TOKEN(anon_sym_mainhand);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1183:
      ACCEPT_TOKEN(anon_sym_merge);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1184:
      ACCEPT_TOKEN(anon_sym_midnight);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1185:
      ACCEPT_TOKEN(anon_sym_mine);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1186:
      ACCEPT_TOKEN(anon_sym_modifier);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1187:
      ACCEPT_TOKEN(anon_sym_modify);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1188:
      ACCEPT_TOKEN(anon_sym_mount);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1189:
      ACCEPT_TOKEN(anon_sym_move);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1190:
      ACCEPT_TOKEN(anon_sym_name);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1191:
      ACCEPT_TOKEN(anon_sym_name);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1192:
      ACCEPT_TOKEN(anon_sym_night);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1193:
      ACCEPT_TOKEN(anon_sym_night);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1194:
      ACCEPT_TOKEN(anon_sym_noon);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1195:
      ACCEPT_TOKEN(anon_sym_noon);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1196:
      ACCEPT_TOKEN(anon_sym_normal);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1197:
      ACCEPT_TOKEN(anon_sym_normal);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1198:
      ACCEPT_TOKEN(anon_sym_off);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'h') ADVANCE(51);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1199:
      ACCEPT_TOKEN(anon_sym_off);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'h') ADVANCE(1404);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1200:
      ACCEPT_TOKEN(anon_sym_offhand);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1201:
      ACCEPT_TOKEN(anon_sym_offhand);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1202:
      ACCEPT_TOKEN(anon_sym_only);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1203:
      ACCEPT_TOKEN(anon_sym_only);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1204:
      ACCEPT_TOKEN(anon_sym_outline);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1205:
      ACCEPT_TOKEN(anon_sym_outline);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1206:
      ACCEPT_TOKEN(anon_sym_players);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1207:
      ACCEPT_TOKEN(anon_sym_players);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1208:
      ACCEPT_TOKEN(anon_sym_poi);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'n') ADVANCE(464);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1209:
      ACCEPT_TOKEN(anon_sym_poi);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1980);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1210:
      ACCEPT_TOKEN(anon_sym_points);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1211:
      ACCEPT_TOKEN(anon_sym_points);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1212:
      ACCEPT_TOKEN(anon_sym_pos);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'i') ADVANCE(466);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1213:
      ACCEPT_TOKEN(anon_sym_pos);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1214:
      ACCEPT_TOKEN(anon_sym_prepend);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1215:
      ACCEPT_TOKEN(anon_sym_prepend);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1216:
      ACCEPT_TOKEN(anon_sym_query);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1217:
      ACCEPT_TOKEN(anon_sym_query);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1218:
      ACCEPT_TOKEN(anon_sym_rain);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1219:
      ACCEPT_TOKEN(anon_sym_rain);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1220:
      ACCEPT_TOKEN(anon_sym_remove);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1221:
      ACCEPT_TOKEN(anon_sym_remove);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1222:
      ACCEPT_TOKEN(anon_sym_replace);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1223:
      ACCEPT_TOKEN(anon_sym_replace);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1224:
      ACCEPT_TOKEN(anon_sym_reset);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'c') ADVANCE(300);
      if (lookahead == 't') ADVANCE(227);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1225:
      ACCEPT_TOKEN(anon_sym_reset);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1730);
      if (lookahead == 't') ADVANCE(1637);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1226:
      ACCEPT_TOKEN(anon_sym_resetclosest);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1227:
      ACCEPT_TOKEN(anon_sym_resetclosest);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1228:
      ACCEPT_TOKEN(anon_sym_resetthat);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1229:
      ACCEPT_TOKEN(anon_sym_resetthat);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1230:
      ACCEPT_TOKEN(anon_sym_resetthese);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1231:
      ACCEPT_TOKEN(anon_sym_resetthese);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1232:
      ACCEPT_TOKEN(anon_sym_revoke);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1233:
      ACCEPT_TOKEN(anon_sym_revoke);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1234:
      ACCEPT_TOKEN(anon_sym_roll);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1235:
      ACCEPT_TOKEN(anon_sym_roll);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1236:
      ACCEPT_TOKEN(anon_sym_runclosest);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1237:
      ACCEPT_TOKEN(anon_sym_runclosest);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1238:
      ACCEPT_TOKEN(anon_sym_runfailed);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1239:
      ACCEPT_TOKEN(anon_sym_runfailed);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1240:
      ACCEPT_TOKEN(anon_sym_runthat);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1241:
      ACCEPT_TOKEN(anon_sym_runthat);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1242:
      ACCEPT_TOKEN(anon_sym_runthese);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1243:
      ACCEPT_TOKEN(anon_sym_runthese);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1244:
      ACCEPT_TOKEN(anon_sym_set);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1656);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1245:
      ACCEPT_TOKEN(anon_sym_setdisplay);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1246:
      ACCEPT_TOKEN(anon_sym_show);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1247:
      ACCEPT_TOKEN(anon_sym_spawn);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1248:
      ACCEPT_TOKEN(anon_sym_start);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1249:
      ACCEPT_TOKEN(anon_sym_strict);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1250:
      ACCEPT_TOKEN(anon_sym_string);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1251:
      ACCEPT_TOKEN(anon_sym_structure);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1252:
      ACCEPT_TOKEN(anon_sym_style);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1253:
      ACCEPT_TOKEN(anon_sym_take);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1254:
      ACCEPT_TOKEN(anon_sym_template);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1255:
      ACCEPT_TOKEN(anon_sym_through);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1256:
      ACCEPT_TOKEN(anon_sym_thunder);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1257:
      ACCEPT_TOKEN(anon_sym_times);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1258:
      ACCEPT_TOKEN(anon_sym_to);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1259:
      ACCEPT_TOKEN(anon_sym_under);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1260:
      ACCEPT_TOKEN(anon_sym_under);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1261:
      ACCEPT_TOKEN(anon_sym_until);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1262:
      ACCEPT_TOKEN(anon_sym_until);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1263:
      ACCEPT_TOKEN(anon_sym_uuids);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1264:
      ACCEPT_TOKEN(anon_sym_uuids);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1265:
      ACCEPT_TOKEN(anon_sym_verify);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1266:
      ACCEPT_TOKEN(anon_sym_verify);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1267:
      ACCEPT_TOKEN(anon_sym_visible);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1268:
      ACCEPT_TOKEN(anon_sym_visible);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1269:
      ACCEPT_TOKEN(anon_sym_warning);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1270:
      ACCEPT_TOKEN(anon_sym_warning);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1271:
      ACCEPT_TOKEN(anon_sym_with);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1272:
      ACCEPT_TOKEN(anon_sym_with);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1273:
      ACCEPT_TOKEN(aux_sym_selector_token1);
      END_STATE();
    case 1274:
      ACCEPT_TOKEN(aux_sym_selector_token2);
      END_STATE();
    case 1275:
      ACCEPT_TOKEN(sym_selector_identifier);
      END_STATE();
    case 1276:
      ACCEPT_TOKEN(sym_query_identifier);
      if (lookahead == '#') ADVANCE(2044);
      if (lookahead == '!' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1276);
      END_STATE();
    case 1277:
      ACCEPT_TOKEN(sym_query_identifier);
      if (lookahead == '.') ADVANCE(1278);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1277);
      if (lookahead == '!' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1279);
      END_STATE();
    case 1278:
      ACCEPT_TOKEN(sym_query_identifier);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1278);
      if (lookahead == '!' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1279);
      END_STATE();
    case 1279:
      ACCEPT_TOKEN(sym_query_identifier);
      if (lookahead == '!' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1279);
      END_STATE();
    case 1280:
      ACCEPT_TOKEN(anon_sym_EQ);
      END_STATE();
    case 1281:
      ACCEPT_TOKEN(anon_sym_EQ);
      if (lookahead == '!') ADVANCE(1282);
      END_STATE();
    case 1282:
      ACCEPT_TOKEN(anon_sym_EQ_BANG);
      END_STATE();
    case 1283:
      ACCEPT_TOKEN(aux_sym_selector_query_token1);
      if (('\t' <= lookahead && lookahead <= '\r') ||
          lookahead == ' ') ADVANCE(1283);
      END_STATE();
    case 1284:
      ACCEPT_TOKEN(anon_sym_TILDE);
      END_STATE();
    case 1285:
      ACCEPT_TOKEN(anon_sym_CARET);
      END_STATE();
    case 1286:
      ACCEPT_TOKEN(sym_heightmap);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_') ADVANCE(1781);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1287:
      ACCEPT_TOKEN(sym_heightmap);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1288:
      ACCEPT_TOKEN(sym_resource);
      if (lookahead == '/') ADVANCE(1289);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      if (lookahead == '+' ||
          ('-' <= lookahead && lookahead <= '9') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1288);
      END_STATE();
    case 1289:
      ACCEPT_TOKEN(sym_resource);
      if (lookahead == '+' ||
          lookahead == '-' ||
          ('/' <= lookahead && lookahead <= '9') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1289);
      END_STATE();
    case 1290:
      ACCEPT_TOKEN(aux_sym_scale_token1);
      END_STATE();
    case 1291:
      ACCEPT_TOKEN(aux_sym_scale_token1);
      if (lookahead == '.') ADVANCE(28);
      if (lookahead == '=') ADVANCE(1325);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(33);
      END_STATE();
    case 1292:
      ACCEPT_TOKEN(aux_sym_scale_token1);
      if (lookahead == '.') ADVANCE(28);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(33);
      END_STATE();
    case 1293:
      ACCEPT_TOKEN(aux_sym_scale_token1);
      if (lookahead == '.') ADVANCE(2062);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(2064);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 1294:
      ACCEPT_TOKEN(aux_sym_scale_token1);
      if (lookahead == '=') ADVANCE(1325);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 1295:
      ACCEPT_TOKEN(aux_sym_scale_token2);
      if (lookahead == '.') ADVANCE(1302);
      if (lookahead == 'b') ADVANCE(2053);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1296);
      END_STATE();
    case 1296:
      ACCEPT_TOKEN(aux_sym_scale_token2);
      if (lookahead == '.') ADVANCE(1302);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1296);
      END_STATE();
    case 1297:
      ACCEPT_TOKEN(aux_sym_scale_token2);
      if (lookahead == '.') ADVANCE(1304);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1297);
      END_STATE();
    case 1298:
      ACCEPT_TOKEN(aux_sym_scale_token2);
      if (lookahead == '.') ADVANCE(1305);
      if (lookahead == 'b') ADVANCE(2044);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1299);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 1299:
      ACCEPT_TOKEN(aux_sym_scale_token2);
      if (lookahead == '.') ADVANCE(1305);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1299);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 1300:
      ACCEPT_TOKEN(aux_sym_scale_token2);
      if (lookahead == '.') ADVANCE(1303);
      if (lookahead == 'b') ADVANCE(2055);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1301);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 1301:
      ACCEPT_TOKEN(aux_sym_scale_token2);
      if (lookahead == '.') ADVANCE(1303);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1301);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 1302:
      ACCEPT_TOKEN(aux_sym_scale_token3);
      if (lookahead == '.') ADVANCE(2056);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1304);
      END_STATE();
    case 1303:
      ACCEPT_TOKEN(aux_sym_scale_token3);
      if (lookahead == '.') ADVANCE(2057);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1306);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 1304:
      ACCEPT_TOKEN(aux_sym_scale_token3);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1304);
      END_STATE();
    case 1305:
      ACCEPT_TOKEN(aux_sym_scale_token3);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1305);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 1306:
      ACCEPT_TOKEN(aux_sym_scale_token3);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1306);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 1307:
      ACCEPT_TOKEN(aux_sym_scale_token4);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1307);
      END_STATE();
    case 1308:
      ACCEPT_TOKEN(aux_sym_scale_token4);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1308);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 1309:
      ACCEPT_TOKEN(aux_sym_scale_token4);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1309);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 1310:
      ACCEPT_TOKEN(aux_sym_scale_token5);
      END_STATE();
    case 1311:
      ACCEPT_TOKEN(aux_sym_scale_token5);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'a') ADVANCE(277);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1312:
      ACCEPT_TOKEN(aux_sym_scale_token5);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == 'r') ADVANCE(481);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1313:
      ACCEPT_TOKEN(aux_sym_scale_token5);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1314:
      ACCEPT_TOKEN(aux_sym_scale_token5);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2065);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2073);
      END_STATE();
    case 1315:
      ACCEPT_TOKEN(aux_sym_scale_token5);
      if (lookahead == 'a') ADVANCE(2068);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 1316:
      ACCEPT_TOKEN(aux_sym_scale_token5);
      if (lookahead == 'r') ADVANCE(2071);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 1317:
      ACCEPT_TOKEN(aux_sym_scale_token5);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 1318:
      ACCEPT_TOKEN(anon_sym_DOLLAR_LPAREN);
      END_STATE();
    case 1319:
      ACCEPT_TOKEN(anon_sym_DOLLAR_LPAREN);
      if (lookahead != 0 &&
          lookahead != '"') ADVANCE(2048);
      END_STATE();
    case 1320:
      ACCEPT_TOKEN(anon_sym_DOLLAR_LPAREN);
      if (lookahead != 0 &&
          lookahead != '\'') ADVANCE(2052);
      END_STATE();
    case 1321:
      ACCEPT_TOKEN(aux_sym_macro_token1);
      if (('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1321);
      END_STATE();
    case 1322:
      ACCEPT_TOKEN(anon_sym_RPAREN);
      END_STATE();
    case 1323:
      ACCEPT_TOKEN(sym_type);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1324:
      ACCEPT_TOKEN(anon_sym_PLUS_EQ);
      END_STATE();
    case 1325:
      ACCEPT_TOKEN(anon_sym_DASH_EQ);
      END_STATE();
    case 1326:
      ACCEPT_TOKEN(anon_sym_STAR_EQ);
      END_STATE();
    case 1327:
      ACCEPT_TOKEN(anon_sym_SLASH_EQ);
      END_STATE();
    case 1328:
      ACCEPT_TOKEN(anon_sym_PERCENT_EQ);
      END_STATE();
    case 1329:
      ACCEPT_TOKEN(anon_sym_GT_LT);
      END_STATE();
    case 1330:
      ACCEPT_TOKEN(anon_sym_LT);
      END_STATE();
    case 1331:
      ACCEPT_TOKEN(anon_sym_GT);
      if (lookahead == '<') ADVANCE(1329);
      END_STATE();
    case 1332:
      ACCEPT_TOKEN(aux_sym_slot_token1);
      if (lookahead == '*') ADVANCE(1333);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1332);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 1333:
      ACCEPT_TOKEN(aux_sym_slot_token1);
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1333);
      END_STATE();
    case 1334:
      ACCEPT_TOKEN(aux_sym_slot_token2);
      if (lookahead == '*') ADVANCE(1335);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1334);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 1335:
      ACCEPT_TOKEN(aux_sym_slot_token2);
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1335);
      END_STATE();
    case 1336:
      ACCEPT_TOKEN(aux_sym_slot_token3);
      if (lookahead == '*') ADVANCE(1337);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1336);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 1337:
      ACCEPT_TOKEN(aux_sym_slot_token3);
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1337);
      END_STATE();
    case 1338:
      ACCEPT_TOKEN(aux_sym_slot_token4);
      if (lookahead == '*') ADVANCE(1339);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1338);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 1339:
      ACCEPT_TOKEN(aux_sym_slot_token4);
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1339);
      END_STATE();
    case 1340:
      ACCEPT_TOKEN(aux_sym_slot_token5);
      if (lookahead == '*') ADVANCE(1341);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1340);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 1341:
      ACCEPT_TOKEN(aux_sym_slot_token5);
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1341);
      END_STATE();
    case 1342:
      ACCEPT_TOKEN(aux_sym_slot_token6);
      if (lookahead == '*') ADVANCE(1342);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1344);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(1352);
      END_STATE();
    case 1343:
      ACCEPT_TOKEN(aux_sym_slot_token6);
      if (lookahead == '*') ADVANCE(1344);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1343);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 1344:
      ACCEPT_TOKEN(aux_sym_slot_token6);
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1344);
      END_STATE();
    case 1345:
      ACCEPT_TOKEN(aux_sym_slot_token7);
      if (lookahead == '*') ADVANCE(1346);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1345);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 1346:
      ACCEPT_TOKEN(aux_sym_slot_token7);
      if (lookahead == '*' ||
          ('0' <= lookahead && lookahead <= '9')) ADVANCE(1346);
      END_STATE();
    case 1347:
      ACCEPT_TOKEN(anon_sym_contents);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 1348:
      ACCEPT_TOKEN(anon_sym_contents);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1349:
      ACCEPT_TOKEN(anon_sym_player_DOTcursor);
      END_STATE();
    case 1350:
      ACCEPT_TOKEN(anon_sym_player_DOTcursor);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 1351:
      ACCEPT_TOKEN(aux_sym_slot_token8);
      if (lookahead == '*') ADVANCE(1352);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(1351);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_') ADVANCE(2044);
      END_STATE();
    case 1352:
      ACCEPT_TOKEN(aux_sym_slot_token8);
      if (lookahead == '*' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1352);
      END_STATE();
    case 1353:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '*') ADVANCE(1341);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1340);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 1354:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '*') ADVANCE(1333);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1332);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 1355:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '*') ADVANCE(1337);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1336);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 1356:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '*') ADVANCE(1339);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1338);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 1357:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '*') ADVANCE(1346);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1345);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 1358:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '*') ADVANCE(1352);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(1351);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_') ADVANCE(2044);
      END_STATE();
    case 1359:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '*') ADVANCE(1342);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1343);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(1351);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_') ADVANCE(2044);
      END_STATE();
    case 1360:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '*') ADVANCE(1335);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1334);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 1361:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '.') ADVANCE(1358);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1362:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '.') ADVANCE(1359);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1363:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '.') ADVANCE(1360);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1364:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '.') ADVANCE(2032);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1207);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1365:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '.') ADVANCE(2032);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1366:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '.') ADVANCE(1353);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1367:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '.') ADVANCE(1354);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1368:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '.') ADVANCE(1355);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1369:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '.') ADVANCE(1356);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1370:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '.') ADVANCE(1357);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 1371:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '/') ADVANCE(1289);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(1288);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          ('-' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1372:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_') ADVANCE(1442);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1373:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_') ADVANCE(1606);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1374:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_') ADVANCE(1933);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1375:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_') ADVANCE(1724);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1376:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_') ADVANCE(1441);
      if (('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1377:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1915);
      if (lookahead == 'i') ADVANCE(1833);
      if (lookahead == 'l') ADVANCE(1799);
      if (lookahead == 'u') ADVANCE(1604);
      if (lookahead == 'y') ADVANCE(1128);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1378:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1736);
      if (lookahead == 'e') ADVANCE(1914);
      if (lookahead == 'i') ADVANCE(1899);
      if (lookahead == 'o') ADVANCE(1998);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1379:
      ACCEPT_TOKEN(aux_sym_word_token1);
      ADVANCE_MAP(
        ':', 1371,
        'a', 1463,
        'e', 1396,
        'i', 1900,
        'l', 1827,
        'o', 1869,
        'r', 1805,
        'u', 1765,
      );
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1380:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1652);
      if (lookahead == 'e') ADVANCE(1873);
      if (lookahead == 'i') ADVANCE(1490);
      if (lookahead == 'o') ADVANCE(1488);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1381:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1683);
      if (lookahead == 'e') ADVANCE(1733);
      if (lookahead == 'h') ADVANCE(1896);
      if (lookahead == 'i') ADVANCE(1740);
      if (lookahead == 'o') ADVANCE(1258);
      if (lookahead == 'r') ADVANCE(2001);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1382:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(2017);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1383:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1464);
      if (lookahead == 'e') ADVANCE(1588);
      if (lookahead == 'l') ADVANCE(1828);
      if (lookahead == 'u') ADVANCE(1765);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1384:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1935);
      if (lookahead == 'o') ADVANCE(1974);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1385:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(919);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1386:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1737);
      if (lookahead == 'e') ADVANCE(1945);
      if (lookahead == 'i') ADVANCE(2011);
      if (lookahead == 'r') ADVANCE(1415);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1387:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1723);
      if (lookahead == 'e') ADVANCE(1879);
      if (lookahead == 'i') ADVANCE(1720);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1388:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1723);
      if (lookahead == 'e') ADVANCE(1632);
      if (lookahead == 'i') ADVANCE(1721);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1389:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(2027);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1390:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(2016);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1391:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1883);
      if (lookahead == 'e') ADVANCE(1418);
      if (lookahead == 'i') ADVANCE(1972);
      if (lookahead == 'o') ADVANCE(1882);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1392:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1466);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1393:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1477);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1394:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1625);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1395:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1887);
      if (lookahead == 'o') ADVANCE(1844);
      if (lookahead == 'r') ADVANCE(1647);
      if (lookahead == 'y') ADVANCE(1709);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1396:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1973);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1397:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(2023);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1398:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1854);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1399:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1985);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1400:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1947);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1401:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1754);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1402:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1691);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1403:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1938);
      if (lookahead == 'l') ADVANCE(1439);
      if (lookahead == 'o') ADVANCE(1932);
      if (lookahead == 'r') ADVANCE(1555);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1404:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1772);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1405:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1872);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1406:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1692);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1407:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1918);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1408:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1718);
      if (lookahead == 'l') ADVANCE(1828);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1409:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1775);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1410:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1960);
      if (lookahead == 'e') ADVANCE(1928);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1411:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1865);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1412:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1963);
      if (lookahead == 'e') ADVANCE(1929);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1413:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1964);
      if (lookahead == 'e') ADVANCE(1930);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1414:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1984);
      if (lookahead == 'i') ADVANCE(1745);
      if (lookahead == 'o') ADVANCE(1998);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1415:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1779);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1416:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(2012);
      if (lookahead == 'v') ADVANCE(1552);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1417:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1655);
      if (lookahead == 'e') ADVANCE(1698);
      if (lookahead == 'o') ADVANCE(1697);
      if (lookahead == 'u') ADVANCE(1749);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1418:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1849);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1419:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1739);
      if (lookahead == 'i') ADVANCE(1615);
      if (lookahead == 'o') ADVANCE(1808);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1420:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1791);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1421:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1456);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1422:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1495);
      if (lookahead == 'n') ADVANCE(1609);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1423:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1619);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1424:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1676);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1425:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1986);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1426:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1621);
      if (lookahead == 'e') ADVANCE(1102);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1427:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1621);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1428:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1987);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1429:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(2014);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1430:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1458);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1431:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1988);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1432:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1989);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1433:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1665);
      if (lookahead == 'e') ADVANCE(1785);
      if (lookahead == 'r') ADVANCE(1830);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1434:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1665);
      if (lookahead == 'e') ADVANCE(1785);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1435:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1931);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1436:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1446);
      if (lookahead == 'd') ADVANCE(1591);
      if (lookahead == 't') ADVANCE(1657);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1437:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1898);
      if (lookahead == 'r') ADVANCE(2001);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1438:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1725);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1439:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(2028);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1440:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'a') ADVANCE(1447);
      if (lookahead == 'm') ADVANCE(1839);
      if (lookahead == 't') ADVANCE(1420);
      if (lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1441:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'b') ADVANCE(1719);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1442:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'b') ADVANCE(1435);
      if (lookahead == 't') ADVANCE(1822);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1443:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'b') ADVANCE(1405);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1444:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'b') ADVANCE(1699);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1445:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'b') ADVANCE(1411);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1446:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'b') ADVANCE(1711);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1447:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'b') ADVANCE(1712);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1448:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'b') ADVANCE(1713);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1449:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1682);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1450:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1686);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1451:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1635);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1452:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1642);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1453:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1995);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1454:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1510);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1455:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1959);
      if (lookahead == 'n') ADVANCE(1611);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1456:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1525);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1457:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1528);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1458:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1536);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1459:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1550);
      if (lookahead == 'f') ADVANCE(1601);
      if (lookahead == 'n') ADVANCE(1082);
      if (lookahead == 'u') ADVANCE(1975);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1460:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1550);
      if (lookahead == 'n') ADVANCE(1083);
      if (lookahead == 'r') ADVANCE(1661);
      if (lookahead == 'v') ADVANCE(1579);
      if (lookahead == 'w') ADVANCE(1796);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1461:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1550);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1462:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1636);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1463:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1660);
      if (lookahead == 'i') ADVANCE(1688);
      if (lookahead == 'l') ADVANCE(1920);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1464:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1660);
      if (lookahead == 'l') ADVANCE(1920);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1465:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1425);
      if (lookahead == 'n') ADVANCE(1609);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1466:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1687);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1467:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1472);
      if (lookahead == 'm') ADVANCE(1746);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1468:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1831);
      if (lookahead == 'h') ADVANCE(1834);
      if (lookahead == 't') ADVANCE(1818);
      if (lookahead == 'u') ADVANCE(1467);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1469:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1432);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1470:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1717);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1471:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1994);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1472:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'c') ADVANCE(1595);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1473:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1475);
      if (lookahead == 'm') ADVANCE(1804);
      if (lookahead == 'p') ADVANCE(1845);
      if (lookahead == 'r') ADVANCE(1735);
      if (lookahead == 't') ADVANCE(1050);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1474:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1167);
      if (lookahead == 'n') ADVANCE(1936);
      if (lookahead == 'p') ADVANCE(1901);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1475:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1112);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1476:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1122);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1477:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(986);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1478:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1201);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1479:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1215);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1480:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1182);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1481:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1239);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1482:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1077);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1483:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1078);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1484:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1099);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1485:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1045);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1486:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1093);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1487:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1374);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1488:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1649);
      if (lookahead == 't') ADVANCE(1659);
      if (lookahead == 'u') ADVANCE(1780);
      if (lookahead == 'v') ADVANCE(1505);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1489:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1372);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1490:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1778);
      if (lookahead == 'n') ADVANCE(1504);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1491:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1902);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1492:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1591);
      if (lookahead == 't') ADVANCE(1657);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1493:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1591);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1494:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1654);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1495:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1556);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1496:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1557);
      if (lookahead == 't') ADVANCE(1662);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1497:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'd') ADVANCE(1571);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1498:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(2029);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1499:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1768);
      if (lookahead == 'l') ADVANCE(1547);
      if (lookahead == 'o') ADVANCE(1722);
      if (lookahead == 'r') ADVANCE(1590);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1500:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1416);
      if (lookahead == 'i') ADVANCE(1916);
      if (lookahead == 'o') ADVANCE(1465);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1501:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1323);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1502:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1124);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1503:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(945);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1504:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1185);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1505:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1189);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1506:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1191);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1507:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1253);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1508:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1020);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1509:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1054);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1510:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1157);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1511:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1179);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1512:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1183);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1513:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1252);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1514:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1108);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1515:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1134);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1516:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(918);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1517:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1142);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1518:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(960);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1519:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1221);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1520:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1233);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1521:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1136);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1522:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1138);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1523:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1154);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1524:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1205);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1525:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1223);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1526:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1101);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1527:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1268);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1528:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1140);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1529:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1160);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1530:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1243);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1531:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1254);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1532:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1118);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1533:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1251);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1534:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1231);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1535:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1152);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1536:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1287);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1537:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1114);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1538:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1100);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1539:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1110);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1540:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1095);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1541:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1362);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1542:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1843);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1543:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1948);
      if (lookahead == 'h') ADVANCE(1800);
      if (lookahead == 'p') ADVANCE(1382);
      if (lookahead == 't') ADVANCE(1395);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1544:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1542);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1545:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1418);
      if (lookahead == 'o') ADVANCE(1882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1546:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1742);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1547:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1398);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1548:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1870);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1549:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1875);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1550:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1401);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1551:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1481);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1552:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1700);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1553:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1489);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1554:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1767);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1555:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1494);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1556:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1482);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1557:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1856);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1558:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1483);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1559:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1906);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1560:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1857);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1561:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1996);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1562:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1484);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1563:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1858);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1564:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1485);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1565:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1486);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1566:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1940);
      if (lookahead == 'o') ADVANCE(1992);
      if (lookahead == 'u') ADVANCE(1757);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1567:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1940);
      if (lookahead == 'o') ADVANCE(1992);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1568:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1773);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1569:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1859);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1570:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1429);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1571:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1860);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1572:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1954);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1573:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1907);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1574:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1407);
      if (lookahead == 'o') ADVANCE(1422);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1575:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1861);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1576:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1876);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1577:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1877);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1578:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1910);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1579:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1863);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1580:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1864);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1581:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1881);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1582:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1783);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1583:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1866);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1584:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1797);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1585:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1867);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1586:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1890);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1587:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1868);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1588:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1969);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1589:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1971);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1590:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1399);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1591:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1874);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1592:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1922);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1593:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1924);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1594:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1889);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1595:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1923);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1596:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1925);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1597:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1784);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1598:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1926);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1599:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1927);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1600:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'e') ADVANCE(1848);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1601:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'f') ADVANCE(1199);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1602:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'f') ADVANCE(1678);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1603:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'f') ADVANCE(1069);
      if (lookahead == 'n') ADVANCE(1071);
      if (lookahead == 't') ADVANCE(1546);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1604:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'f') ADVANCE(1607);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1605:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'f') ADVANCE(2021);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1606:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'f') ADVANCE(1705);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1607:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'f') ADVANCE(1560);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1608:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'f') ADVANCE(1430);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1609:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'g') ADVANCE(1323);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1610:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'g') ADVANCE(1066);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1611:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'g') ADVANCE(1250);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1612:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'g') ADVANCE(1270);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1613:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'g') ADVANCE(1144);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1614:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'g') ADVANCE(1286);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1615:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'g') ADVANCE(1633);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1616:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'g') ADVANCE(1917);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1617:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'g') ADVANCE(1630);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1618:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'g') ADVANCE(1512);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1619:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'g') ADVANCE(1516);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1620:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'g') ADVANCE(1758);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1621:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'g') ADVANCE(1526);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1622:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'g') ADVANCE(1634);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1623:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'g') ADVANCE(1589);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1624:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'g') ADVANCE(1668);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1625:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'g') ADVANCE(1576);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1626:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'g') ADVANCE(1586);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1627:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'h') ADVANCE(1155);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1628:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'h') ADVANCE(1272);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1629:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'h') ADVANCE(1156);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1630:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'h') ADVANCE(1255);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1631:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'h') ADVANCE(1410);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1632:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'h') ADVANCE(1653);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1633:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'h') ADVANCE(1953);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1634:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'h') ADVANCE(1962);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1635:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'h') ADVANCE(1593);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1636:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'h') ADVANCE(1578);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1637:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'h') ADVANCE(1412);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1638:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'h') ADVANCE(1413);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1639:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'h') ADVANCE(1409);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1640:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'h') ADVANCE(1666);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1641:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'h') ADVANCE(1834);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1642:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'h') ADVANCE(1835);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1643:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'h') ADVANCE(1583);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1644:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1616);
      if (lookahead == 'o') ADVANCE(1650);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1645:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1209);
      if (lookahead == 's') ADVANCE(1213);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1646:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1491);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1647:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1455);
      if (lookahead == 'u') ADVANCE(1471);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1648:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1833);
      if (lookahead == 'l') ADVANCE(1799);
      if (lookahead == 'o') ADVANCE(1919);
      if (lookahead == 'y') ADVANCE(1979);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1649:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1602);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1650:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1750);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1651:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1605);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1652:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1770);
      if (lookahead == 'x') ADVANCE(1080);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1653:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1470);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1654:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1469);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1655:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1752);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1656:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1934);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1657:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1976);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1658:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1620);
      if (lookahead == 'l') ADVANCE(1043);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1659:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1813);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1660:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1769);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1661:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1624);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1662:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1690);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1663:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1787);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1664:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1774);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1665:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1795);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1666:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1776);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1667:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1777);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1668:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1759);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1669:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1553);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1670:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1721);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1671:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1847);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1672:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1816);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1673:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1743);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1674:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1826);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1675:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1744);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1676:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1714);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1677:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1820);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1678:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1575);
      if (lookahead == 'y') ADVANCE(1187);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1679:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1622);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1680:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1448);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1681:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'i') ADVANCE(1997);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1682:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'k') ADVANCE(1056);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1683:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'k') ADVANCE(1507);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1684:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'k') ADVANCE(1520);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1685:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'k') ADVANCE(1558);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1686:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'k') ADVANCE(1667);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1687:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'k') ADVANCE(1585);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1688:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1153);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1689:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1235);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1690:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1262);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1691:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1197);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1692:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1116);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1693:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1658);
      if (lookahead == 'n') ADVANCE(1452);
      if (lookahead == 'r') ADVANCE(1735);
      if (lookahead == 's') ADVANCE(1047);
      if (lookahead == 't') ADVANCE(1049);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1694:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1389);
      if (lookahead == 'o') ADVANCE(1645);
      if (lookahead == 'r') ADVANCE(1600);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1695:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1487);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1696:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1701);
      if (lookahead == 'r') ADVANCE(1921);
      if (lookahead == 't') ADVANCE(1443);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1697:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1689);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1698:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1812);
      if (lookahead == 'm') ADVANCE(1829);
      if (lookahead == 'p') ADVANCE(1702);
      if (lookahead == 's') ADVANCE(1572);
      if (lookahead == 'v') ADVANCE(1823);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1699:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1501);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1700:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1903);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1701:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1803);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1702:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1421);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1703:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1669);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1704:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1832);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1705:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1836);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1706:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1394);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1707:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1397);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1708:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1728);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1709:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1513);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1710:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1982);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1711:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1517);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1712:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1522);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1713:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1527);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1714:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1551);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1715:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1970);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1716:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1592);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1717:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1539);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1718:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1920);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1719:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1824);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1720:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1706);
      if (lookahead == 's') ADVANCE(1680);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1721:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1706);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1722:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1811);
      if (lookahead == 'n') ADVANCE(1949);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1723:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(2003);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1724:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1570);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1725:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(2004);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1726:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1663);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1727:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1428);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1728:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1587);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1729:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1439);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1730:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1841);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1731:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'l') ADVANCE(1842);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1732:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'm') ADVANCE(1158);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1733:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'm') ADVANCE(1846);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1734:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'm') ADVANCE(1999);
      if (lookahead == 'v') ADVANCE(1438);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1735:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'm') ADVANCE(1810);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1736:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'm') ADVANCE(1423);
      if (lookahead == 'y') ADVANCE(1135);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1737:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'm') ADVANCE(1561);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1738:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'm') ADVANCE(1402);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1739:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'm') ADVANCE(1506);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1740:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'm') ADVANCE(1508);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1741:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'm') ADVANCE(1509);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1742:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'm') ADVANCE(1908);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1743:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'm') ADVANCE(1521);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1744:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'm') ADVANCE(1529);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1745:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'm') ADVANCE(1582);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1746:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'm') ADVANCE(1819);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1747:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1436);
      if (lookahead == 'v') ADVANCE(1548);
      if (lookahead == 'x') ADVANCE(1851);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1748:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1496);
      if (lookahead == 'u') ADVANCE(1646);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1749:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1750:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1175);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1751:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1195);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1752:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1219);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1753:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1361);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1754:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1373);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1755:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1247);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1756:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(938);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1757:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(898);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1758:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1041);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1759:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1085);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1760:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1010);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1761:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1061);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1762:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1946);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1763:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1497);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1764:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1609);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1765:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1453);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1766:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1376);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1767:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1476);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1768:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1993);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1769:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1610);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1770:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1639);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1771:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1949);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1772:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1478);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1773:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1479);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1774:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1612);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1775:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1480);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1776:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1613);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1777:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1614);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1778:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1679);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1779:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1951);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1780:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1952);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1781:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1807);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1782:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1956);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1783:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1943);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1784:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1991);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1785:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1983);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1786:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1961);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1787:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1524);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1788:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1968);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1789:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1565);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1790:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1492);
      if (lookahead == 'y') ADVANCE(1573);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1791:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1457);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1792:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1493);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1793:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1664);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1794:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1716);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1795:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1577);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1796:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1580);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1797:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'n') ADVANCE(1626);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1798:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1696);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1799:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1449);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1800:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1884);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1801:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1897);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1802:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1998);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1803:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(2015);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1804:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(2009);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1805:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1732);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1806:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(2005);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1807:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1375);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1808:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1751);
      if (lookahead == 'r') ADVANCE(1738);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1809:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(2022);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1810:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1853);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1811:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1855);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1812:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1393);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1813:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1766);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1814:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1753);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1815:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1880);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1816:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1756);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1817:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1862);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1818:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1871);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1819:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1760);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1820:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1761);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1821:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1764);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1822:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1990);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1823:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1684);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1824:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1450);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1825:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1888);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1826:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1789);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1827:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1400);
      if (lookahead == 'u') ADVANCE(1913);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1828:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1400);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1829:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(2013);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1830:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1708);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1831:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1893);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1832:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1941);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1833:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1741);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1834:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1885);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1835:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1894);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1836:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1817);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1837:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1788);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1838:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1771);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1839:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(2010);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1840:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1974);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1841:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1942);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1842:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'o') ADVANCE(1944);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1843:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'p') ADVANCE(1177);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1844:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'p') ADVANCE(1006);
      if (lookahead == 'r') ADVANCE(1427);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1845:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'p') ADVANCE(1554);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1846:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'p') ADVANCE(1727);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1847:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'p') ADVANCE(1703);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1848:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'p') ADVANCE(1568);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1849:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'p') ADVANCE(1814);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1850:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'p') ADVANCE(1707);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1851:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'p') ADVANCE(1825);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1852:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1735);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1853:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1361);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1854:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(914);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1855:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1132);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1856:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1260);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1857:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1126);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1858:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1130);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1859:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1364);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1860:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1256);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1861:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1186);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1862:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1287);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1863:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1087);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1864:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1089);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1865:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(911);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1866:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1075);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1867:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1052);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1868:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1060);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1869:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1454);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1870:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(2025);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1871:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1426);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1872:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1363);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1873:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1618);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1874:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1451);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1875:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(2019);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1876:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1366);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1877:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1367);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1878:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1608);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1879:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1651);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1880:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(2024);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1881:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1365);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1882:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1695);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1883:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1793);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1884:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1947);
      if (lookahead == 'w') ADVANCE(1246);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1885:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1947);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1886:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1809);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1887:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1955);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1888:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1957);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1889:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1958);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1890:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1912);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1891:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1523);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1892:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1533);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1893:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1538);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1894:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1564);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1895:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(2001);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1896:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1806);
      if (lookahead == 'u') ADVANCE(1763);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1897:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1921);
      if (lookahead == 't') ADVANCE(1443);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1898:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'r') ADVANCE(1623);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1899:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1440);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1900:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1627);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1901:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1171);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1902:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1264);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1903:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1181);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1904:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1211);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1905:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1348);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1906:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1287);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1907:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1065);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1908:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1073);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1909:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1106);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1910:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1079);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1911:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1103);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1912:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1091);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1913:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1629);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1914:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1981);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1915:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1502);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1916:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1950);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1917:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1390);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1918:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1643);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1919:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1939);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1920:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1498);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1921:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1541);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1922:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1909);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1923:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1911);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1924:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1977);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1925:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1965);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1926:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1966);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1927:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1967);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1928:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1530);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1929:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1534);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1930:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1535);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1931:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1537);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1932:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1681);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1933:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(2002);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1934:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1850);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1935:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1685);
      if (lookahead == 't') ADVANCE(1462);
      if (lookahead == 'x') ADVANCE(1080);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1936:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1594);
      if (lookahead == 't') ADVANCE(1323);
      if (lookahead == 'v') ADVANCE(1597);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1937:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1584);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1938:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1937);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1939:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1445);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1940:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(2007);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1941:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1596);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1942:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1598);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1943:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1677);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1944:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 's') ADVANCE(1599);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1945:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1162);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1946:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1323);
      if (lookahead == 'v') ADVANCE(1597);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1947:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1323);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1948:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1244);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1949:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1434);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1950:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(957);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1951:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1164);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1952:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1188);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1953:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1193);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1954:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1225);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1955:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1248);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1956:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1120);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1957:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1146);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1958:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1169);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1959:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1249);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1960:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1241);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1961:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1139);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1962:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1184);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1963:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1229);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1964:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1150);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1965:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1237);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1966:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1227);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1967:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1148);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1968:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1433);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1969:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1067);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1970:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1097);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1971:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1104);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1972:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1628);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1973:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(2000);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1974:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1659);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1975:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1726);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1976:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(2020);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1977:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1369);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1978:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1640);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1979:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1501);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1980:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1904);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1981:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1886);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1982:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1671);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1983:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1905);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1984:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1385);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1985:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1515);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1986:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1518);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1987:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1531);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1988:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1562);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1989:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1540);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1990:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1406);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1991:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1815);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1992:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1431);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1993:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1563);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1994:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(2008);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1995:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1672);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1996:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1675);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1997:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 't') ADVANCE(1674);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1998:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'u') ADVANCE(1444);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 1999:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'u') ADVANCE(1710);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2000:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'u') ADVANCE(1891);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2001:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'u') ADVANCE(1498);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2002:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'u') ADVANCE(1878);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2003:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'u') ADVANCE(1514);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2004:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'u') ADVANCE(1532);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2005:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'u') ADVANCE(1617);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2006:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'u') ADVANCE(1549);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2007:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'u') ADVANCE(1715);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2008:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'u') ADVANCE(1892);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2009:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'u') ADVANCE(1782);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2010:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'u') ADVANCE(1786);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2011:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'v') ADVANCE(1503);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2012:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'v') ADVANCE(1511);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2013:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'v') ADVANCE(1519);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2014:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'v') ADVANCE(1559);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2015:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'w') ADVANCE(1165);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2016:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'w') ADVANCE(1173);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2017:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'w') ADVANCE(1755);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2018:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'y') ADVANCE(1203);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2019:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'y') ADVANCE(1217);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2020:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'y') ADVANCE(1063);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2021:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'y') ADVANCE(1266);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2022:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'y') ADVANCE(1137);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2023:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'y') ADVANCE(1245);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2024:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'y') ADVANCE(1368);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2025:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'y') ADVANCE(1978);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2026:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'y') ADVANCE(1979);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2027:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'y') ADVANCE(1569);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2028:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == 'y') ADVANCE(1581);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2029:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == ':') ADVANCE(1371);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2029);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2044);
      END_STATE();
    case 2030:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '=') ADVANCE(1324);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 2031:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 'a') ADVANCE(2033);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 2032:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 'c') ADVANCE(2038);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 2033:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 'f') ADVANCE(2042);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 2034:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 'g') ADVANCE(1370);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 2035:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 'i') ADVANCE(2036);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 2036:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 'n') ADVANCE(2034);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 2037:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 'o') ADVANCE(2040);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 2038:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 'r') ADVANCE(2031);
      if (lookahead == 'u') ADVANCE(2039);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 2039:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 'r') ADVANCE(2041);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 2040:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 'r') ADVANCE(1350);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 2041:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 's') ADVANCE(2037);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 2042:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == 't') ADVANCE(2035);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 2043:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1308);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          lookahead == ':' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 2044:
      ACCEPT_TOKEN(aux_sym_word_token1);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= ':') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2044);
      END_STATE();
    case 2045:
      ACCEPT_TOKEN(anon_sym_DQUOTE);
      END_STATE();
    case 2046:
      ACCEPT_TOKEN(aux_sym_string_token1);
      if (lookahead == '$') ADVANCE(2047);
      if (lookahead != 0 &&
          lookahead != '"') ADVANCE(2048);
      END_STATE();
    case 2047:
      ACCEPT_TOKEN(aux_sym_string_token1);
      if (lookahead == '(') ADVANCE(1319);
      if (lookahead != 0 &&
          lookahead != '"') ADVANCE(2048);
      END_STATE();
    case 2048:
      ACCEPT_TOKEN(aux_sym_string_token1);
      if (lookahead != 0 &&
          lookahead != '"') ADVANCE(2048);
      END_STATE();
    case 2049:
      ACCEPT_TOKEN(anon_sym_SQUOTE);
      END_STATE();
    case 2050:
      ACCEPT_TOKEN(aux_sym_string_token2);
      if (lookahead == '$') ADVANCE(2051);
      if (lookahead != 0 &&
          lookahead != '\'') ADVANCE(2052);
      END_STATE();
    case 2051:
      ACCEPT_TOKEN(aux_sym_string_token2);
      if (lookahead == '(') ADVANCE(1320);
      if (lookahead != 0 &&
          lookahead != '\'' &&
          lookahead != '(') ADVANCE(2052);
      END_STATE();
    case 2052:
      ACCEPT_TOKEN(aux_sym_string_token2);
      if (lookahead != 0 &&
          lookahead != '\'') ADVANCE(2052);
      END_STATE();
    case 2053:
      ACCEPT_TOKEN(sym_boolean);
      END_STATE();
    case 2054:
      ACCEPT_TOKEN(sym_boolean);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(500);
      END_STATE();
    case 2055:
      ACCEPT_TOKEN(sym_boolean);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 2056:
      ACCEPT_TOKEN(sym_range);
      if (lookahead == '-') ADVANCE(2058);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(2058);
      END_STATE();
    case 2057:
      ACCEPT_TOKEN(sym_range);
      if (lookahead == '-') ADVANCE(2059);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(2059);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 2058:
      ACCEPT_TOKEN(sym_range);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(2058);
      END_STATE();
    case 2059:
      ACCEPT_TOKEN(sym_range);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(2059);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 2060:
      ACCEPT_TOKEN(aux_sym_nbt_identifier_token1);
      if (lookahead == '-') ADVANCE(2072);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(2059);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '.' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 2061:
      ACCEPT_TOKEN(aux_sym_nbt_identifier_token1);
      if (lookahead == '.') ADVANCE(2060);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(1309);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 2062:
      ACCEPT_TOKEN(aux_sym_nbt_identifier_token1);
      if (lookahead == '.') ADVANCE(2060);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 2063:
      ACCEPT_TOKEN(aux_sym_nbt_identifier_token1);
      if (lookahead == '.') ADVANCE(2057);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 2064:
      ACCEPT_TOKEN(aux_sym_nbt_identifier_token1);
      if (lookahead == '.') ADVANCE(2063);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(2064);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 2065:
      ACCEPT_TOKEN(aux_sym_nbt_identifier_token1);
      if (lookahead == ':') ADVANCE(882);
      if (lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2065);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z')) ADVANCE(2073);
      END_STATE();
    case 2066:
      ACCEPT_TOKEN(aux_sym_nbt_identifier_token1);
      if (lookahead == 'a') ADVANCE(2068);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 2067:
      ACCEPT_TOKEN(aux_sym_nbt_identifier_token1);
      if (lookahead == 'e') ADVANCE(2055);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 2068:
      ACCEPT_TOKEN(aux_sym_nbt_identifier_token1);
      if (lookahead == 'l') ADVANCE(2070);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 2069:
      ACCEPT_TOKEN(aux_sym_nbt_identifier_token1);
      if (lookahead == 'r') ADVANCE(2071);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 2070:
      ACCEPT_TOKEN(aux_sym_nbt_identifier_token1);
      if (lookahead == 's') ADVANCE(2067);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 2071:
      ACCEPT_TOKEN(aux_sym_nbt_identifier_token1);
      if (lookahead == 'u') ADVANCE(2067);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 2072:
      ACCEPT_TOKEN(aux_sym_nbt_identifier_token1);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(2059);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 2073:
      ACCEPT_TOKEN(aux_sym_nbt_identifier_token1);
      if (lookahead == '!' ||
          lookahead == '#' ||
          lookahead == '+' ||
          lookahead == '-' ||
          lookahead == '.' ||
          ('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(2073);
      END_STATE();
    case 2074:
      ACCEPT_TOKEN(anon_sym_LBRACE);
      END_STATE();
    case 2075:
      ACCEPT_TOKEN(anon_sym_COLON);
      END_STATE();
    case 2076:
      ACCEPT_TOKEN(anon_sym_RBRACE);
      END_STATE();
    case 2077:
      ACCEPT_TOKEN(anon_sym_LBRACK);
      END_STATE();
    case 2078:
      ACCEPT_TOKEN(anon_sym_RBRACK);
      END_STATE();
    default:
      return false;
  }
}

static const TSLexerMode ts_lex_modes[STATE_COUNT] = {
  [0] = {.lex_state = 0},
  [1] = {.lex_state = 883},
  [2] = {.lex_state = 9},
  [3] = {.lex_state = 9},
  [4] = {.lex_state = 12},
  [5] = {.lex_state = 883},
  [6] = {.lex_state = 883},
  [7] = {.lex_state = 7},
  [8] = {.lex_state = 883},
  [9] = {.lex_state = 883},
  [10] = {.lex_state = 883},
  [11] = {.lex_state = 883},
  [12] = {.lex_state = 883},
  [13] = {.lex_state = 883},
  [14] = {.lex_state = 883},
  [15] = {.lex_state = 8},
  [16] = {.lex_state = 8},
  [17] = {.lex_state = 17},
  [18] = {.lex_state = 8},
  [19] = {.lex_state = 17},
  [20] = {.lex_state = 10},
  [21] = {.lex_state = 10},
  [22] = {.lex_state = 10},
  [23] = {.lex_state = 10},
  [24] = {.lex_state = 10},
  [25] = {.lex_state = 10},
  [26] = {.lex_state = 10},
  [27] = {.lex_state = 13},
  [28] = {.lex_state = 13},
  [29] = {.lex_state = 13},
  [30] = {.lex_state = 13},
  [31] = {.lex_state = 13},
  [32] = {.lex_state = 13},
  [33] = {.lex_state = 13},
  [34] = {.lex_state = 13},
  [35] = {.lex_state = 13},
  [36] = {.lex_state = 13},
  [37] = {.lex_state = 13},
  [38] = {.lex_state = 11},
  [39] = {.lex_state = 11},
  [40] = {.lex_state = 11},
  [41] = {.lex_state = 11},
  [42] = {.lex_state = 11},
  [43] = {.lex_state = 11},
  [44] = {.lex_state = 11},
  [45] = {.lex_state = 11},
  [46] = {.lex_state = 14},
  [47] = {.lex_state = 11},
  [48] = {.lex_state = 14},
  [49] = {.lex_state = 13},
  [50] = {.lex_state = 13},
  [51] = {.lex_state = 13},
  [52] = {.lex_state = 12},
  [53] = {.lex_state = 13},
  [54] = {.lex_state = 13},
  [55] = {.lex_state = 13},
  [56] = {.lex_state = 12},
  [57] = {.lex_state = 13},
  [58] = {.lex_state = 13},
  [59] = {.lex_state = 13},
  [60] = {.lex_state = 13},
  [61] = {.lex_state = 11},
  [62] = {.lex_state = 12},
  [63] = {.lex_state = 12},
  [64] = {.lex_state = 13},
  [65] = {.lex_state = 11},
  [66] = {.lex_state = 12},
  [67] = {.lex_state = 12},
  [68] = {.lex_state = 15},
  [69] = {.lex_state = 15},
  [70] = {.lex_state = 15},
  [71] = {.lex_state = 15},
  [72] = {.lex_state = 1},
  [73] = {.lex_state = 16},
  [74] = {.lex_state = 15},
  [75] = {.lex_state = 15},
  [76] = {.lex_state = 15},
  [77] = {.lex_state = 15},
  [78] = {.lex_state = 15},
  [79] = {.lex_state = 15},
  [80] = {.lex_state = 15},
  [81] = {.lex_state = 21},
  [82] = {.lex_state = 20},
  [83] = {.lex_state = 2},
  [84] = {.lex_state = 2},
  [85] = {.lex_state = 20},
  [86] = {.lex_state = 21},
  [87] = {.lex_state = 16},
  [88] = {.lex_state = 1},
  [89] = {.lex_state = 20},
  [90] = {.lex_state = 18},
  [91] = {.lex_state = 18},
  [92] = {.lex_state = 20},
  [93] = {.lex_state = 20},
  [94] = {.lex_state = 20},
  [95] = {.lex_state = 20},
  [96] = {.lex_state = 20},
  [97] = {.lex_state = 20},
  [98] = {.lex_state = 20},
  [99] = {.lex_state = 15},
  [100] = {.lex_state = 15},
  [101] = {.lex_state = 15},
  [102] = {.lex_state = 15},
  [103] = {.lex_state = 15},
  [104] = {.lex_state = 15},
  [105] = {.lex_state = 0},
  [106] = {.lex_state = 15},
  [107] = {.lex_state = 16},
  [108] = {.lex_state = 16},
  [109] = {.lex_state = 0},
  [110] = {.lex_state = 19},
  [111] = {.lex_state = 19},
  [112] = {.lex_state = 15},
  [113] = {.lex_state = 12},
  [114] = {.lex_state = 15},
  [115] = {.lex_state = 1},
  [116] = {.lex_state = 15},
  [117] = {.lex_state = 12},
  [118] = {.lex_state = 0},
  [119] = {.lex_state = 15},
  [120] = {.lex_state = 15},
  [121] = {.lex_state = 15},
  [122] = {.lex_state = 15},
  [123] = {.lex_state = 15},
  [124] = {.lex_state = 1},
  [125] = {.lex_state = 15},
  [126] = {.lex_state = 12},
  [127] = {.lex_state = 20},
  [128] = {.lex_state = 9},
  [129] = {.lex_state = 9},
  [130] = {.lex_state = 0},
  [131] = {.lex_state = 0},
  [132] = {.lex_state = 20},
  [133] = {.lex_state = 16},
  [134] = {.lex_state = 20},
  [135] = {.lex_state = 1},
  [136] = {.lex_state = 0},
  [137] = {.lex_state = 16},
  [138] = {.lex_state = 20},
  [139] = {.lex_state = 20},
  [140] = {.lex_state = 0},
  [141] = {.lex_state = 20},
  [142] = {.lex_state = 1},
  [143] = {.lex_state = 20},
  [144] = {.lex_state = 20},
  [145] = {.lex_state = 20},
  [146] = {.lex_state = 20},
  [147] = {.lex_state = 20},
  [148] = {.lex_state = 20},
  [149] = {.lex_state = 9},
  [150] = {.lex_state = 0},
  [151] = {.lex_state = 9},
  [152] = {.lex_state = 0},
  [153] = {.lex_state = 20},
  [154] = {.lex_state = 1},
  [155] = {.lex_state = 0},
  [156] = {.lex_state = 0},
  [157] = {.lex_state = 12},
  [158] = {.lex_state = 2},
  [159] = {.lex_state = 16},
  [160] = {.lex_state = 21},
  [161] = {.lex_state = 12},
  [162] = {.lex_state = 2},
  [163] = {.lex_state = 16},
  [164] = {.lex_state = 16},
  [165] = {.lex_state = 16},
  [166] = {.lex_state = 16},
  [167] = {.lex_state = 0},
  [168] = {.lex_state = 21},
  [169] = {.lex_state = 21},
  [170] = {.lex_state = 2},
  [171] = {.lex_state = 16},
  [172] = {.lex_state = 0},
  [173] = {.lex_state = 0},
  [174] = {.lex_state = 16},
  [175] = {.lex_state = 16},
  [176] = {.lex_state = 0},
  [177] = {.lex_state = 0},
  [178] = {.lex_state = 16},
  [179] = {.lex_state = 16},
  [180] = {.lex_state = 0},
  [181] = {.lex_state = 0},
  [182] = {.lex_state = 16},
  [183] = {.lex_state = 0},
  [184] = {.lex_state = 0},
  [185] = {.lex_state = 16},
  [186] = {.lex_state = 0},
  [187] = {.lex_state = 0},
  [188] = {.lex_state = 16},
  [189] = {.lex_state = 16},
  [190] = {.lex_state = 16},
  [191] = {.lex_state = 16},
  [192] = {.lex_state = 0},
  [193] = {.lex_state = 0},
  [194] = {.lex_state = 16},
  [195] = {.lex_state = 883},
  [196] = {.lex_state = 16},
  [197] = {.lex_state = 883},
  [198] = {.lex_state = 16},
  [199] = {.lex_state = 0},
  [200] = {.lex_state = 16},
  [201] = {.lex_state = 16},
  [202] = {.lex_state = 0},
  [203] = {.lex_state = 883},
  [204] = {.lex_state = 0},
  [205] = {.lex_state = 0},
  [206] = {.lex_state = 16},
  [207] = {.lex_state = 883},
  [208] = {.lex_state = 16},
  [209] = {.lex_state = 16},
  [210] = {.lex_state = 883},
  [211] = {.lex_state = 0},
  [212] = {.lex_state = 0},
  [213] = {.lex_state = 0},
  [214] = {.lex_state = 16},
  [215] = {.lex_state = 0},
  [216] = {.lex_state = 16},
  [217] = {.lex_state = 0},
  [218] = {.lex_state = 16},
  [219] = {.lex_state = 16},
  [220] = {.lex_state = 0},
  [221] = {.lex_state = 0},
  [222] = {.lex_state = 16},
  [223] = {.lex_state = 2050},
  [224] = {.lex_state = 7},
  [225] = {.lex_state = 0},
  [226] = {.lex_state = 0},
  [227] = {.lex_state = 2050},
  [228] = {.lex_state = 0},
  [229] = {.lex_state = 0},
  [230] = {.lex_state = 0},
  [231] = {.lex_state = 7},
  [232] = {.lex_state = 22},
  [233] = {.lex_state = 0},
  [234] = {.lex_state = 0},
  [235] = {.lex_state = 0},
  [236] = {.lex_state = 7},
  [237] = {.lex_state = 7},
  [238] = {.lex_state = 7},
  [239] = {.lex_state = 0},
  [240] = {.lex_state = 0},
  [241] = {.lex_state = 0},
  [242] = {.lex_state = 7},
  [243] = {.lex_state = 0},
  [244] = {.lex_state = 0},
  [245] = {.lex_state = 2046},
  [246] = {.lex_state = 2050},
  [247] = {.lex_state = 2046},
  [248] = {.lex_state = 2046},
  [249] = {.lex_state = 2046},
  [250] = {.lex_state = 2050},
  [251] = {.lex_state = 2046},
  [252] = {.lex_state = 2050},
  [253] = {.lex_state = 2046},
  [254] = {.lex_state = 2050},
  [255] = {.lex_state = 0},
  [256] = {(TSStateId)(-1),},
};

static const uint16_t ts_parse_table[LARGE_STATE_COUNT][SYMBOL_COUNT] = {
  [STATE(0)] = {
    [sym_macro] = STATE(0),
    [ts_builtin_sym_end] = ACTIONS(1),
    [sym__newline] = ACTIONS(1),
    [aux_sym__space_token1] = ACTIONS(1),
    [sym_backslash] = ACTIONS(1),
    [sym_comment] = ACTIONS(1),
    [anon_sym_execute] = ACTIONS(1),
    [anon_sym_run] = ACTIONS(1),
    [sym_macro_indicator] = ACTIONS(1),
    [anon_sym_advancement] = ACTIONS(1),
    [anon_sym_attribute] = ACTIONS(1),
    [anon_sym_ban] = ACTIONS(1),
    [anon_sym_ban_DASHip] = ACTIONS(1),
    [anon_sym_banlist] = ACTIONS(1),
    [anon_sym_bossbar] = ACTIONS(1),
    [anon_sym_clear] = ACTIONS(1),
    [anon_sym_clone] = ACTIONS(1),
    [anon_sym_effect] = ACTIONS(1),
    [anon_sym_enchant] = ACTIONS(1),
    [anon_sym_experience] = ACTIONS(1),
    [anon_sym_gamemode] = ACTIONS(1),
    [anon_sym_gamerule] = ACTIONS(1),
    [anon_sym_give] = ACTIONS(1),
    [anon_sym_item] = ACTIONS(1),
    [anon_sym_jfr] = ACTIONS(1),
    [anon_sym_kick] = ACTIONS(1),
    [anon_sym_kill] = ACTIONS(1),
    [anon_sym_list] = ACTIONS(1),
    [anon_sym_locate] = ACTIONS(1),
    [anon_sym_loot] = ACTIONS(1),
    [anon_sym_op] = ACTIONS(1),
    [anon_sym_pardon] = ACTIONS(1),
    [anon_sym_pardon_DASHip] = ACTIONS(1),
    [anon_sym_particle] = ACTIONS(1),
    [anon_sym_perf] = ACTIONS(1),
    [anon_sym_place] = ACTIONS(1),
    [anon_sym_playsound] = ACTIONS(1),
    [anon_sym_publish] = ACTIONS(1),
    [anon_sym_random] = ACTIONS(1),
    [anon_sym_recipe] = ACTIONS(1),
    [anon_sym_reload] = ACTIONS(1),
    [anon_sym_return] = ACTIONS(1),
    [anon_sym_ride] = ACTIONS(1),
    [anon_sym_rotate] = ACTIONS(1),
    [anon_sym_version] = ACTIONS(1),
    [anon_sym_w] = ACTIONS(1),
    [anon_sym_waypoint] = ACTIONS(1),
    [anon_sym_weather] = ACTIONS(1),
    [anon_sym_whitelist] = ACTIONS(1),
    [anon_sym_worldborder] = ACTIONS(1),
    [anon_sym_xp] = ACTIONS(1),
    [anon_sym_align] = ACTIONS(1),
    [anon_sym_all] = ACTIONS(1),
    [anon_sym_anchored] = ACTIONS(1),
    [anon_sym_as] = ACTIONS(1),
    [anon_sym_at] = ACTIONS(1),
    [anon_sym_attacker] = ACTIONS(1),
    [anon_sym_biome] = ACTIONS(1),
    [anon_sym_block] = ACTIONS(1),
    [anon_sym_blocks] = ACTIONS(1),
    [anon_sym_controller] = ACTIONS(1),
    [anon_sym_entity] = ACTIONS(1),
    [anon_sym_eyes] = ACTIONS(1),
    [anon_sym_if] = ACTIONS(1),
    [anon_sym_in] = ACTIONS(1),
    [anon_sym_items] = ACTIONS(1),
    [anon_sym_leasher] = ACTIONS(1),
    [anon_sym_loaded] = ACTIONS(1),
    [anon_sym_on] = ACTIONS(1),
    [anon_sym_origin] = ACTIONS(1),
    [anon_sym_over] = ACTIONS(1),
    [anon_sym_owner] = ACTIONS(1),
    [anon_sym_passengers] = ACTIONS(1),
    [anon_sym_positioned] = ACTIONS(1),
    [anon_sym_predicate] = ACTIONS(1),
    [anon_sym_result] = ACTIONS(1),
    [anon_sym_rotated] = ACTIONS(1),
    [anon_sym_unless] = ACTIONS(1),
    [anon_sym_value] = ACTIONS(1),
    [anon_sym_vehicle] = ACTIONS(1),
    [anon_sym_add] = ACTIONS(1),
    [anon_sym_add_multiplied_base] = ACTIONS(1),
    [anon_sym_add_multiplied_total] = ACTIONS(1),
    [anon_sym_add_value] = ACTIONS(1),
    [anon_sym_amount] = ACTIONS(1),
    [anon_sym_append] = ACTIONS(1),
    [anon_sym_base] = ACTIONS(1),
    [anon_sym_buffer] = ACTIONS(1),
    [anon_sym_by] = ACTIONS(1),
    [anon_sym_center] = ACTIONS(1),
    [anon_sym_color] = ACTIONS(1),
    [anon_sym_create] = ACTIONS(1),
    [anon_sym_enable] = ACTIONS(1),
    [anon_sym_everything] = ACTIONS(1),
    [anon_sym_export] = ACTIONS(1),
    [anon_sym_exportclosest] = ACTIONS(1),
    [anon_sym_exportthat] = ACTIONS(1),
    [anon_sym_exportthese] = ACTIONS(1),
    [anon_sym_gametime] = ACTIONS(1),
    [anon_sym_get] = ACTIONS(1),
    [anon_sym_grant] = ACTIONS(1),
    [anon_sym_id] = ACTIONS(1),
    [anon_sym_insert] = ACTIONS(1),
    [anon_sym_ips] = ACTIONS(1),
    [anon_sym_jigsaw] = ACTIONS(1),
    [anon_sym_join] = ACTIONS(1),
    [anon_sym_keep] = ACTIONS(1),
    [anon_sym_leave] = ACTIONS(1),
    [anon_sym_levels] = ACTIONS(1),
    [anon_sym_name] = ACTIONS(1),
    [anon_sym_night] = ACTIONS(1),
    [anon_sym_noon] = ACTIONS(1),
    [anon_sym_normal] = ACTIONS(1),
    [anon_sym_off] = ACTIONS(1),
    [anon_sym_offhand] = ACTIONS(1),
    [anon_sym_only] = ACTIONS(1),
    [anon_sym_outline] = ACTIONS(1),
    [anon_sym_players] = ACTIONS(1),
    [anon_sym_poi] = ACTIONS(1),
    [anon_sym_points] = ACTIONS(1),
    [anon_sym_pos] = ACTIONS(1),
    [anon_sym_prepend] = ACTIONS(1),
    [anon_sym_query] = ACTIONS(1),
    [anon_sym_rain] = ACTIONS(1),
    [anon_sym_remove] = ACTIONS(1),
    [anon_sym_replace] = ACTIONS(1),
    [anon_sym_reset] = ACTIONS(1),
    [anon_sym_resetclosest] = ACTIONS(1),
    [anon_sym_resetthat] = ACTIONS(1),
    [anon_sym_resetthese] = ACTIONS(1),
    [anon_sym_revoke] = ACTIONS(1),
    [anon_sym_roll] = ACTIONS(1),
    [anon_sym_runclosest] = ACTIONS(1),
    [anon_sym_runfailed] = ACTIONS(1),
    [anon_sym_runthat] = ACTIONS(1),
    [anon_sym_runthese] = ACTIONS(1),
    [anon_sym_under] = ACTIONS(1),
    [anon_sym_until] = ACTIONS(1),
    [anon_sym_uuids] = ACTIONS(1),
    [anon_sym_verify] = ACTIONS(1),
    [anon_sym_visible] = ACTIONS(1),
    [anon_sym_warning] = ACTIONS(1),
    [anon_sym_with] = ACTIONS(1),
    [aux_sym_selector_token1] = ACTIONS(1),
    [aux_sym_selector_token2] = ACTIONS(1),
    [sym_selector_identifier] = ACTIONS(1),
    [anon_sym_EQ] = ACTIONS(1),
    [anon_sym_EQ_BANG] = ACTIONS(1),
    [anon_sym_TILDE] = ACTIONS(1),
    [anon_sym_CARET] = ACTIONS(1),
    [sym_resource] = ACTIONS(1),
    [aux_sym_scale_token1] = ACTIONS(1),
    [aux_sym_scale_token2] = ACTIONS(1),
    [aux_sym_scale_token3] = ACTIONS(1),
    [aux_sym_scale_token4] = ACTIONS(1),
    [aux_sym_scale_token5] = ACTIONS(1),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [anon_sym_RPAREN] = ACTIONS(1),
    [anon_sym_PLUS_EQ] = ACTIONS(1),
    [anon_sym_DASH_EQ] = ACTIONS(1),
    [anon_sym_STAR_EQ] = ACTIONS(1),
    [anon_sym_SLASH_EQ] = ACTIONS(1),
    [anon_sym_PERCENT_EQ] = ACTIONS(1),
    [anon_sym_GT_LT] = ACTIONS(1),
    [anon_sym_LT] = ACTIONS(1),
    [anon_sym_GT] = ACTIONS(1),
    [aux_sym_slot_token1] = ACTIONS(1),
    [aux_sym_slot_token3] = ACTIONS(1),
    [aux_sym_slot_token4] = ACTIONS(1),
    [aux_sym_slot_token5] = ACTIONS(1),
    [aux_sym_slot_token7] = ACTIONS(1),
    [anon_sym_contents] = ACTIONS(1),
    [anon_sym_player_DOTcursor] = ACTIONS(1),
    [anon_sym_DQUOTE] = ACTIONS(1),
    [anon_sym_SQUOTE] = ACTIONS(1),
    [sym_boolean] = ACTIONS(1),
    [sym_range] = ACTIONS(1),
    [anon_sym_LBRACE] = ACTIONS(1),
    [anon_sym_COLON] = ACTIONS(1),
    [anon_sym_RBRACE] = ACTIONS(1),
    [anon_sym_LBRACK] = ACTIONS(1),
    [anon_sym_RBRACK] = ACTIONS(1),
  },
  [STATE(1)] = {
    [sym_source_file] = STATE(228),
    [sym__statement] = STATE(10),
    [sym_command] = STATE(237),
    [sym__execute_command] = STATE(236),
    [sym__generic_command] = STATE(236),
    [sym_command_identifier] = STATE(130),
    [sym_macro] = STATE(1),
    [aux_sym_source_file_repeat1] = STATE(5),
    [ts_builtin_sym_end] = ACTIONS(5),
    [sym__newline] = ACTIONS(7),
    [aux_sym__statement_token1] = ACTIONS(9),
    [sym_comment] = ACTIONS(11),
    [anon_sym_execute] = ACTIONS(13),
    [sym_macro_indicator] = ACTIONS(15),
    [anon_sym_advancement] = ACTIONS(17),
    [anon_sym_attribute] = ACTIONS(17),
    [anon_sym_ban] = ACTIONS(19),
    [anon_sym_ban_DASHip] = ACTIONS(17),
    [anon_sym_banlist] = ACTIONS(17),
    [anon_sym_bossbar] = ACTIONS(17),
    [anon_sym_clear] = ACTIONS(17),
    [anon_sym_clone] = ACTIONS(17),
    [anon_sym_damage] = ACTIONS(17),
    [anon_sym_data] = ACTIONS(19),
    [anon_sym_datapack] = ACTIONS(17),
    [anon_sym_debug] = ACTIONS(17),
    [anon_sym_defaultgamemode] = ACTIONS(17),
    [anon_sym_deop] = ACTIONS(17),
    [anon_sym_dialog] = ACTIONS(17),
    [anon_sym_difficulty] = ACTIONS(17),
    [anon_sym_effect] = ACTIONS(17),
    [anon_sym_enchant] = ACTIONS(17),
    [anon_sym_experience] = ACTIONS(17),
    [anon_sym_fetchprofile] = ACTIONS(17),
    [anon_sym_fill] = ACTIONS(19),
    [anon_sym_fillbiome] = ACTIONS(17),
    [anon_sym_forceload] = ACTIONS(17),
    [anon_sym_function] = ACTIONS(17),
    [anon_sym_gamemode] = ACTIONS(17),
    [anon_sym_gamerule] = ACTIONS(17),
    [anon_sym_give] = ACTIONS(17),
    [anon_sym_help] = ACTIONS(17),
    [anon_sym_item] = ACTIONS(17),
    [anon_sym_jfr] = ACTIONS(17),
    [anon_sym_kick] = ACTIONS(17),
    [anon_sym_kill] = ACTIONS(17),
    [anon_sym_list] = ACTIONS(17),
    [anon_sym_locate] = ACTIONS(17),
    [anon_sym_loot] = ACTIONS(17),
    [anon_sym_me] = ACTIONS(17),
    [anon_sym_msg] = ACTIONS(17),
    [anon_sym_op] = ACTIONS(17),
    [anon_sym_pardon] = ACTIONS(19),
    [anon_sym_pardon_DASHip] = ACTIONS(17),
    [anon_sym_particle] = ACTIONS(17),
    [anon_sym_perf] = ACTIONS(17),
    [anon_sym_place] = ACTIONS(17),
    [anon_sym_playsound] = ACTIONS(17),
    [anon_sym_publish] = ACTIONS(17),
    [anon_sym_random] = ACTIONS(17),
    [anon_sym_recipe] = ACTIONS(17),
    [anon_sym_reload] = ACTIONS(17),
    [anon_sym_return] = ACTIONS(17),
    [anon_sym_ride] = ACTIONS(17),
    [anon_sym_rotate] = ACTIONS(17),
    [anon_sym_save_DASHall] = ACTIONS(17),
    [anon_sym_save_DASHoff] = ACTIONS(17),
    [anon_sym_save_DASHon] = ACTIONS(17),
    [anon_sym_say] = ACTIONS(17),
    [anon_sym_schedule] = ACTIONS(17),
    [anon_sym_scoreboard] = ACTIONS(17),
    [anon_sym_seed] = ACTIONS(17),
    [anon_sym_setblock] = ACTIONS(17),
    [anon_sym_setidletimeout] = ACTIONS(17),
    [anon_sym_setworldspawn] = ACTIONS(17),
    [anon_sym_spawnpoint] = ACTIONS(17),
    [anon_sym_spectate] = ACTIONS(17),
    [anon_sym_spreadplayers] = ACTIONS(17),
    [anon_sym_stop] = ACTIONS(19),
    [anon_sym_stopsound] = ACTIONS(17),
    [anon_sym_summon] = ACTIONS(17),
    [anon_sym_swing] = ACTIONS(17),
    [anon_sym_tag] = ACTIONS(17),
    [anon_sym_team] = ACTIONS(19),
    [anon_sym_teammsg] = ACTIONS(17),
    [anon_sym_teleport] = ACTIONS(17),
    [anon_sym_tell] = ACTIONS(19),
    [anon_sym_tellraw] = ACTIONS(17),
    [anon_sym_test] = ACTIONS(17),
    [anon_sym_time] = ACTIONS(17),
    [anon_sym_title] = ACTIONS(17),
    [anon_sym_tm] = ACTIONS(17),
    [anon_sym_tp] = ACTIONS(17),
    [anon_sym_transfer] = ACTIONS(17),
    [anon_sym_trigger] = ACTIONS(17),
    [anon_sym_version] = ACTIONS(17),
    [anon_sym_w] = ACTIONS(19),
    [anon_sym_waypoint] = ACTIONS(17),
    [anon_sym_weather] = ACTIONS(17),
    [anon_sym_whitelist] = ACTIONS(17),
    [anon_sym_worldborder] = ACTIONS(17),
    [anon_sym_xp] = ACTIONS(17),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
  },
  [STATE(2)] = {
    [sym_argument] = STATE(172),
    [sym_selector] = STATE(202),
    [sym_position] = STATE(202),
    [sym_rotation] = STATE(202),
    [sym_scale] = STATE(202),
    [sym_macro] = STATE(2),
    [sym_operator] = STATE(202),
    [sym_slot] = STATE(202),
    [sym_word] = STATE(202),
    [sym_string] = STATE(202),
    [sym_nbt_compound] = STATE(202),
    [sym_nbt_array] = STATE(202),
    [anon_sym_run] = ACTIONS(21),
    [anon_sym_clear] = ACTIONS(21),
    [anon_sym_damage] = ACTIONS(21),
    [anon_sym_function] = ACTIONS(21),
    [anon_sym_give] = ACTIONS(21),
    [anon_sym_list] = ACTIONS(21),
    [anon_sym_locate] = ACTIONS(21),
    [anon_sym_reload] = ACTIONS(21),
    [anon_sym_stop] = ACTIONS(21),
    [anon_sym_time] = ACTIONS(21),
    [anon_sym_at] = ACTIONS(21),
    [anon_sym_biome] = ACTIONS(21),
    [anon_sym_block] = ACTIONS(21),
    [anon_sym_blocks] = ACTIONS(21),
    [anon_sym_entity] = ACTIONS(21),
    [anon_sym_facing] = ACTIONS(21),
    [anon_sym_max] = ACTIONS(21),
    [anon_sym_on] = ACTIONS(21),
    [anon_sym_storage] = ACTIONS(21),
    [anon_sym_value] = ACTIONS(21),
    [anon_sym_add] = ACTIONS(21),
    [anon_sym_add_multiplied_base] = ACTIONS(21),
    [anon_sym_add_multiplied_total] = ACTIONS(21),
    [anon_sym_add_value] = ACTIONS(21),
    [anon_sym_amount] = ACTIONS(21),
    [anon_sym_append] = ACTIONS(21),
    [anon_sym_base] = ACTIONS(21),
    [anon_sym_buffer] = ACTIONS(21),
    [anon_sym_by] = ACTIONS(21),
    [anon_sym_center] = ACTIONS(21),
    [anon_sym_color] = ACTIONS(21),
    [anon_sym_create] = ACTIONS(21),
    [anon_sym_day] = ACTIONS(21),
    [anon_sym_daytime] = ACTIONS(21),
    [anon_sym_destroy] = ACTIONS(21),
    [anon_sym_disable] = ACTIONS(21),
    [anon_sym_dismount] = ACTIONS(21),
    [anon_sym_distance] = ACTIONS(21),
    [anon_sym_enable] = ACTIONS(21),
    [anon_sym_everything] = ACTIONS(21),
    [anon_sym_export] = ACTIONS(21),
    [anon_sym_exportclosest] = ACTIONS(21),
    [anon_sym_exportthat] = ACTIONS(21),
    [anon_sym_exportthese] = ACTIONS(21),
    [anon_sym_fail] = ACTIONS(21),
    [anon_sym_feature] = ACTIONS(21),
    [anon_sym_fish] = ACTIONS(21),
    [anon_sym_flush] = ACTIONS(21),
    [anon_sym_force] = ACTIONS(21),
    [anon_sym_from] = ACTIONS(21),
    [anon_sym_gametime] = ACTIONS(21),
    [anon_sym_get] = ACTIONS(21),
    [anon_sym_grant] = ACTIONS(21),
    [anon_sym_hollow] = ACTIONS(21),
    [anon_sym_id] = ACTIONS(21),
    [anon_sym_insert] = ACTIONS(21),
    [anon_sym_ips] = ACTIONS(21),
    [anon_sym_jigsaw] = ACTIONS(21),
    [anon_sym_join] = ACTIONS(21),
    [anon_sym_keep] = ACTIONS(21),
    [anon_sym_leave] = ACTIONS(21),
    [anon_sym_levels] = ACTIONS(21),
    [anon_sym_mainhand] = ACTIONS(21),
    [anon_sym_merge] = ACTIONS(21),
    [anon_sym_midnight] = ACTIONS(21),
    [anon_sym_mine] = ACTIONS(21),
    [anon_sym_modifier] = ACTIONS(21),
    [anon_sym_modify] = ACTIONS(21),
    [anon_sym_mount] = ACTIONS(21),
    [anon_sym_move] = ACTIONS(21),
    [anon_sym_name] = ACTIONS(21),
    [anon_sym_night] = ACTIONS(21),
    [anon_sym_noon] = ACTIONS(21),
    [anon_sym_normal] = ACTIONS(21),
    [anon_sym_off] = ACTIONS(21),
    [anon_sym_offhand] = ACTIONS(21),
    [anon_sym_only] = ACTIONS(21),
    [anon_sym_outline] = ACTIONS(21),
    [anon_sym_players] = ACTIONS(21),
    [anon_sym_poi] = ACTIONS(21),
    [anon_sym_points] = ACTIONS(21),
    [anon_sym_pos] = ACTIONS(21),
    [anon_sym_prepend] = ACTIONS(21),
    [anon_sym_query] = ACTIONS(21),
    [anon_sym_rain] = ACTIONS(21),
    [anon_sym_remove] = ACTIONS(21),
    [anon_sym_replace] = ACTIONS(21),
    [anon_sym_reset] = ACTIONS(21),
    [anon_sym_resetclosest] = ACTIONS(21),
    [anon_sym_resetthat] = ACTIONS(21),
    [anon_sym_resetthese] = ACTIONS(21),
    [anon_sym_revoke] = ACTIONS(21),
    [anon_sym_roll] = ACTIONS(21),
    [anon_sym_runclosest] = ACTIONS(21),
    [anon_sym_runfailed] = ACTIONS(21),
    [anon_sym_runthat] = ACTIONS(21),
    [anon_sym_runthese] = ACTIONS(21),
    [anon_sym_set] = ACTIONS(21),
    [anon_sym_setdisplay] = ACTIONS(21),
    [anon_sym_show] = ACTIONS(21),
    [anon_sym_spawn] = ACTIONS(21),
    [anon_sym_start] = ACTIONS(21),
    [anon_sym_strict] = ACTIONS(21),
    [anon_sym_string] = ACTIONS(21),
    [anon_sym_structure] = ACTIONS(21),
    [anon_sym_style] = ACTIONS(21),
    [anon_sym_take] = ACTIONS(21),
    [anon_sym_template] = ACTIONS(21),
    [anon_sym_through] = ACTIONS(21),
    [anon_sym_thunder] = ACTIONS(21),
    [anon_sym_times] = ACTIONS(21),
    [anon_sym_to] = ACTIONS(21),
    [anon_sym_under] = ACTIONS(21),
    [anon_sym_until] = ACTIONS(21),
    [anon_sym_uuids] = ACTIONS(21),
    [anon_sym_verify] = ACTIONS(21),
    [anon_sym_visible] = ACTIONS(21),
    [anon_sym_warning] = ACTIONS(21),
    [anon_sym_with] = ACTIONS(21),
    [sym_selector_identifier] = ACTIONS(23),
    [anon_sym_TILDE] = ACTIONS(25),
    [anon_sym_CARET] = ACTIONS(27),
    [sym_heightmap] = ACTIONS(21),
    [sym_resource] = ACTIONS(21),
    [aux_sym_scale_token1] = ACTIONS(29),
    [aux_sym_scale_token2] = ACTIONS(31),
    [aux_sym_scale_token3] = ACTIONS(31),
    [aux_sym_scale_token4] = ACTIONS(31),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [sym_type] = ACTIONS(21),
    [anon_sym_PLUS_EQ] = ACTIONS(33),
    [anon_sym_DASH_EQ] = ACTIONS(33),
    [anon_sym_STAR_EQ] = ACTIONS(33),
    [anon_sym_SLASH_EQ] = ACTIONS(33),
    [anon_sym_PERCENT_EQ] = ACTIONS(33),
    [anon_sym_GT_LT] = ACTIONS(33),
    [anon_sym_LT] = ACTIONS(33),
    [anon_sym_GT] = ACTIONS(35),
    [aux_sym_slot_token1] = ACTIONS(37),
    [aux_sym_slot_token2] = ACTIONS(37),
    [aux_sym_slot_token3] = ACTIONS(37),
    [aux_sym_slot_token4] = ACTIONS(37),
    [aux_sym_slot_token5] = ACTIONS(37),
    [aux_sym_slot_token6] = ACTIONS(37),
    [aux_sym_slot_token7] = ACTIONS(37),
    [anon_sym_contents] = ACTIONS(37),
    [anon_sym_player_DOTcursor] = ACTIONS(37),
    [aux_sym_slot_token8] = ACTIONS(37),
    [aux_sym_word_token1] = ACTIONS(39),
    [anon_sym_DQUOTE] = ACTIONS(41),
    [anon_sym_SQUOTE] = ACTIONS(43),
    [sym_boolean] = ACTIONS(21),
    [anon_sym_LBRACE] = ACTIONS(45),
    [anon_sym_LBRACK] = ACTIONS(47),
  },
  [STATE(3)] = {
    [sym_macro] = STATE(3),
    [aux_sym__space_token1] = ACTIONS(49),
    [sym_backslash] = ACTIONS(51),
    [anon_sym_run] = ACTIONS(49),
    [anon_sym_clear] = ACTIONS(49),
    [anon_sym_damage] = ACTIONS(49),
    [anon_sym_function] = ACTIONS(49),
    [anon_sym_give] = ACTIONS(49),
    [anon_sym_list] = ACTIONS(49),
    [anon_sym_locate] = ACTIONS(49),
    [anon_sym_reload] = ACTIONS(49),
    [anon_sym_stop] = ACTIONS(49),
    [anon_sym_time] = ACTIONS(49),
    [anon_sym_at] = ACTIONS(49),
    [anon_sym_biome] = ACTIONS(49),
    [anon_sym_block] = ACTIONS(49),
    [anon_sym_blocks] = ACTIONS(49),
    [anon_sym_entity] = ACTIONS(49),
    [anon_sym_facing] = ACTIONS(49),
    [anon_sym_max] = ACTIONS(49),
    [anon_sym_on] = ACTIONS(49),
    [anon_sym_storage] = ACTIONS(49),
    [anon_sym_value] = ACTIONS(49),
    [anon_sym_add] = ACTIONS(49),
    [anon_sym_add_multiplied_base] = ACTIONS(49),
    [anon_sym_add_multiplied_total] = ACTIONS(49),
    [anon_sym_add_value] = ACTIONS(49),
    [anon_sym_amount] = ACTIONS(49),
    [anon_sym_append] = ACTIONS(49),
    [anon_sym_base] = ACTIONS(49),
    [anon_sym_buffer] = ACTIONS(49),
    [anon_sym_by] = ACTIONS(49),
    [anon_sym_center] = ACTIONS(49),
    [anon_sym_color] = ACTIONS(49),
    [anon_sym_create] = ACTIONS(49),
    [anon_sym_day] = ACTIONS(49),
    [anon_sym_daytime] = ACTIONS(49),
    [anon_sym_destroy] = ACTIONS(49),
    [anon_sym_disable] = ACTIONS(49),
    [anon_sym_dismount] = ACTIONS(49),
    [anon_sym_distance] = ACTIONS(49),
    [anon_sym_enable] = ACTIONS(49),
    [anon_sym_everything] = ACTIONS(49),
    [anon_sym_export] = ACTIONS(49),
    [anon_sym_exportclosest] = ACTIONS(49),
    [anon_sym_exportthat] = ACTIONS(49),
    [anon_sym_exportthese] = ACTIONS(49),
    [anon_sym_fail] = ACTIONS(49),
    [anon_sym_feature] = ACTIONS(49),
    [anon_sym_fish] = ACTIONS(49),
    [anon_sym_flush] = ACTIONS(49),
    [anon_sym_force] = ACTIONS(49),
    [anon_sym_from] = ACTIONS(49),
    [anon_sym_gametime] = ACTIONS(49),
    [anon_sym_get] = ACTIONS(49),
    [anon_sym_grant] = ACTIONS(49),
    [anon_sym_hollow] = ACTIONS(49),
    [anon_sym_id] = ACTIONS(49),
    [anon_sym_insert] = ACTIONS(49),
    [anon_sym_ips] = ACTIONS(49),
    [anon_sym_jigsaw] = ACTIONS(49),
    [anon_sym_join] = ACTIONS(49),
    [anon_sym_keep] = ACTIONS(49),
    [anon_sym_leave] = ACTIONS(49),
    [anon_sym_levels] = ACTIONS(49),
    [anon_sym_mainhand] = ACTIONS(49),
    [anon_sym_merge] = ACTIONS(49),
    [anon_sym_midnight] = ACTIONS(49),
    [anon_sym_mine] = ACTIONS(49),
    [anon_sym_modifier] = ACTIONS(49),
    [anon_sym_modify] = ACTIONS(49),
    [anon_sym_mount] = ACTIONS(49),
    [anon_sym_move] = ACTIONS(49),
    [anon_sym_name] = ACTIONS(49),
    [anon_sym_night] = ACTIONS(49),
    [anon_sym_noon] = ACTIONS(49),
    [anon_sym_normal] = ACTIONS(49),
    [anon_sym_off] = ACTIONS(49),
    [anon_sym_offhand] = ACTIONS(49),
    [anon_sym_only] = ACTIONS(49),
    [anon_sym_outline] = ACTIONS(49),
    [anon_sym_players] = ACTIONS(49),
    [anon_sym_poi] = ACTIONS(49),
    [anon_sym_points] = ACTIONS(49),
    [anon_sym_pos] = ACTIONS(49),
    [anon_sym_prepend] = ACTIONS(49),
    [anon_sym_query] = ACTIONS(49),
    [anon_sym_rain] = ACTIONS(49),
    [anon_sym_remove] = ACTIONS(49),
    [anon_sym_replace] = ACTIONS(49),
    [anon_sym_reset] = ACTIONS(49),
    [anon_sym_resetclosest] = ACTIONS(49),
    [anon_sym_resetthat] = ACTIONS(49),
    [anon_sym_resetthese] = ACTIONS(49),
    [anon_sym_revoke] = ACTIONS(49),
    [anon_sym_roll] = ACTIONS(49),
    [anon_sym_runclosest] = ACTIONS(49),
    [anon_sym_runfailed] = ACTIONS(49),
    [anon_sym_runthat] = ACTIONS(49),
    [anon_sym_runthese] = ACTIONS(49),
    [anon_sym_set] = ACTIONS(49),
    [anon_sym_setdisplay] = ACTIONS(49),
    [anon_sym_show] = ACTIONS(49),
    [anon_sym_spawn] = ACTIONS(49),
    [anon_sym_start] = ACTIONS(49),
    [anon_sym_strict] = ACTIONS(49),
    [anon_sym_string] = ACTIONS(49),
    [anon_sym_structure] = ACTIONS(49),
    [anon_sym_style] = ACTIONS(49),
    [anon_sym_take] = ACTIONS(49),
    [anon_sym_template] = ACTIONS(49),
    [anon_sym_through] = ACTIONS(49),
    [anon_sym_thunder] = ACTIONS(49),
    [anon_sym_times] = ACTIONS(49),
    [anon_sym_to] = ACTIONS(49),
    [anon_sym_under] = ACTIONS(49),
    [anon_sym_until] = ACTIONS(49),
    [anon_sym_uuids] = ACTIONS(49),
    [anon_sym_verify] = ACTIONS(49),
    [anon_sym_visible] = ACTIONS(49),
    [anon_sym_warning] = ACTIONS(49),
    [anon_sym_with] = ACTIONS(49),
    [sym_selector_identifier] = ACTIONS(51),
    [anon_sym_EQ] = ACTIONS(51),
    [anon_sym_TILDE] = ACTIONS(51),
    [anon_sym_CARET] = ACTIONS(51),
    [sym_heightmap] = ACTIONS(49),
    [sym_resource] = ACTIONS(49),
    [aux_sym_scale_token1] = ACTIONS(49),
    [aux_sym_scale_token2] = ACTIONS(49),
    [aux_sym_scale_token3] = ACTIONS(49),
    [aux_sym_scale_token4] = ACTIONS(49),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [sym_type] = ACTIONS(49),
    [anon_sym_PLUS_EQ] = ACTIONS(51),
    [anon_sym_DASH_EQ] = ACTIONS(51),
    [anon_sym_STAR_EQ] = ACTIONS(51),
    [anon_sym_SLASH_EQ] = ACTIONS(51),
    [anon_sym_PERCENT_EQ] = ACTIONS(51),
    [anon_sym_GT_LT] = ACTIONS(51),
    [anon_sym_LT] = ACTIONS(51),
    [anon_sym_GT] = ACTIONS(49),
    [aux_sym_slot_token1] = ACTIONS(49),
    [aux_sym_slot_token2] = ACTIONS(49),
    [aux_sym_slot_token3] = ACTIONS(49),
    [aux_sym_slot_token4] = ACTIONS(49),
    [aux_sym_slot_token5] = ACTIONS(49),
    [aux_sym_slot_token6] = ACTIONS(49),
    [aux_sym_slot_token7] = ACTIONS(49),
    [anon_sym_contents] = ACTIONS(49),
    [anon_sym_player_DOTcursor] = ACTIONS(49),
    [aux_sym_slot_token8] = ACTIONS(49),
    [aux_sym_word_token1] = ACTIONS(49),
    [anon_sym_DQUOTE] = ACTIONS(51),
    [anon_sym_SQUOTE] = ACTIONS(51),
    [sym_boolean] = ACTIONS(49),
    [anon_sym_LBRACE] = ACTIONS(51),
    [anon_sym_LBRACK] = ACTIONS(51),
  },
  [STATE(4)] = {
    [sym_macro] = STATE(4),
    [aux_sym__space_token1] = ACTIONS(49),
    [sym_backslash] = ACTIONS(51),
    [anon_sym_execute] = ACTIONS(51),
    [sym_macro_indicator] = ACTIONS(49),
    [anon_sym_advancement] = ACTIONS(51),
    [anon_sym_attribute] = ACTIONS(51),
    [anon_sym_ban] = ACTIONS(49),
    [anon_sym_ban_DASHip] = ACTIONS(51),
    [anon_sym_banlist] = ACTIONS(51),
    [anon_sym_bossbar] = ACTIONS(51),
    [anon_sym_clear] = ACTIONS(51),
    [anon_sym_clone] = ACTIONS(51),
    [anon_sym_damage] = ACTIONS(51),
    [anon_sym_data] = ACTIONS(49),
    [anon_sym_datapack] = ACTIONS(51),
    [anon_sym_debug] = ACTIONS(51),
    [anon_sym_defaultgamemode] = ACTIONS(51),
    [anon_sym_deop] = ACTIONS(51),
    [anon_sym_dialog] = ACTIONS(51),
    [anon_sym_difficulty] = ACTIONS(51),
    [anon_sym_effect] = ACTIONS(51),
    [anon_sym_enchant] = ACTIONS(51),
    [anon_sym_experience] = ACTIONS(51),
    [anon_sym_fetchprofile] = ACTIONS(51),
    [anon_sym_fill] = ACTIONS(49),
    [anon_sym_fillbiome] = ACTIONS(51),
    [anon_sym_forceload] = ACTIONS(51),
    [anon_sym_function] = ACTIONS(51),
    [anon_sym_gamemode] = ACTIONS(51),
    [anon_sym_gamerule] = ACTIONS(51),
    [anon_sym_give] = ACTIONS(51),
    [anon_sym_help] = ACTIONS(51),
    [anon_sym_item] = ACTIONS(51),
    [anon_sym_jfr] = ACTIONS(51),
    [anon_sym_kick] = ACTIONS(51),
    [anon_sym_kill] = ACTIONS(51),
    [anon_sym_list] = ACTIONS(51),
    [anon_sym_locate] = ACTIONS(51),
    [anon_sym_loot] = ACTIONS(51),
    [anon_sym_me] = ACTIONS(51),
    [anon_sym_msg] = ACTIONS(51),
    [anon_sym_op] = ACTIONS(51),
    [anon_sym_pardon] = ACTIONS(49),
    [anon_sym_pardon_DASHip] = ACTIONS(51),
    [anon_sym_particle] = ACTIONS(51),
    [anon_sym_perf] = ACTIONS(51),
    [anon_sym_place] = ACTIONS(51),
    [anon_sym_playsound] = ACTIONS(51),
    [anon_sym_publish] = ACTIONS(51),
    [anon_sym_random] = ACTIONS(51),
    [anon_sym_recipe] = ACTIONS(51),
    [anon_sym_reload] = ACTIONS(51),
    [anon_sym_return] = ACTIONS(51),
    [anon_sym_ride] = ACTIONS(51),
    [anon_sym_rotate] = ACTIONS(51),
    [anon_sym_save_DASHall] = ACTIONS(51),
    [anon_sym_save_DASHoff] = ACTIONS(51),
    [anon_sym_save_DASHon] = ACTIONS(51),
    [anon_sym_say] = ACTIONS(51),
    [anon_sym_schedule] = ACTIONS(51),
    [anon_sym_scoreboard] = ACTIONS(51),
    [anon_sym_seed] = ACTIONS(51),
    [anon_sym_setblock] = ACTIONS(51),
    [anon_sym_setidletimeout] = ACTIONS(51),
    [anon_sym_setworldspawn] = ACTIONS(51),
    [anon_sym_spawnpoint] = ACTIONS(51),
    [anon_sym_spectate] = ACTIONS(51),
    [anon_sym_spreadplayers] = ACTIONS(51),
    [anon_sym_stop] = ACTIONS(49),
    [anon_sym_stopsound] = ACTIONS(51),
    [anon_sym_summon] = ACTIONS(51),
    [anon_sym_swing] = ACTIONS(51),
    [anon_sym_tag] = ACTIONS(51),
    [anon_sym_team] = ACTIONS(49),
    [anon_sym_teammsg] = ACTIONS(51),
    [anon_sym_teleport] = ACTIONS(51),
    [anon_sym_tell] = ACTIONS(49),
    [anon_sym_tellraw] = ACTIONS(51),
    [anon_sym_test] = ACTIONS(51),
    [anon_sym_time] = ACTIONS(51),
    [anon_sym_title] = ACTIONS(51),
    [anon_sym_tm] = ACTIONS(51),
    [anon_sym_tp] = ACTIONS(51),
    [anon_sym_transfer] = ACTIONS(51),
    [anon_sym_trigger] = ACTIONS(51),
    [anon_sym_version] = ACTIONS(51),
    [anon_sym_w] = ACTIONS(49),
    [anon_sym_waypoint] = ACTIONS(51),
    [anon_sym_weather] = ACTIONS(51),
    [anon_sym_whitelist] = ACTIONS(51),
    [anon_sym_worldborder] = ACTIONS(51),
    [anon_sym_xp] = ACTIONS(51),
    [anon_sym_EQ] = ACTIONS(51),
    [aux_sym_scale_token1] = ACTIONS(49),
    [aux_sym_scale_token2] = ACTIONS(49),
    [aux_sym_scale_token3] = ACTIONS(49),
    [aux_sym_scale_token4] = ACTIONS(51),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [sym_boolean] = ACTIONS(51),
    [sym_range] = ACTIONS(51),
    [anon_sym_LBRACE] = ACTIONS(51),
    [anon_sym_COLON] = ACTIONS(51),
    [anon_sym_LBRACK] = ACTIONS(51),
  },
  [STATE(5)] = {
    [sym__statement] = STATE(10),
    [sym_command] = STATE(237),
    [sym__execute_command] = STATE(236),
    [sym__generic_command] = STATE(236),
    [sym_command_identifier] = STATE(130),
    [sym_macro] = STATE(5),
    [aux_sym_source_file_repeat1] = STATE(6),
    [ts_builtin_sym_end] = ACTIONS(53),
    [sym__newline] = ACTIONS(7),
    [aux_sym__statement_token1] = ACTIONS(9),
    [sym_comment] = ACTIONS(11),
    [anon_sym_execute] = ACTIONS(13),
    [sym_macro_indicator] = ACTIONS(15),
    [anon_sym_advancement] = ACTIONS(17),
    [anon_sym_attribute] = ACTIONS(17),
    [anon_sym_ban] = ACTIONS(19),
    [anon_sym_ban_DASHip] = ACTIONS(17),
    [anon_sym_banlist] = ACTIONS(17),
    [anon_sym_bossbar] = ACTIONS(17),
    [anon_sym_clear] = ACTIONS(17),
    [anon_sym_clone] = ACTIONS(17),
    [anon_sym_damage] = ACTIONS(17),
    [anon_sym_data] = ACTIONS(19),
    [anon_sym_datapack] = ACTIONS(17),
    [anon_sym_debug] = ACTIONS(17),
    [anon_sym_defaultgamemode] = ACTIONS(17),
    [anon_sym_deop] = ACTIONS(17),
    [anon_sym_dialog] = ACTIONS(17),
    [anon_sym_difficulty] = ACTIONS(17),
    [anon_sym_effect] = ACTIONS(17),
    [anon_sym_enchant] = ACTIONS(17),
    [anon_sym_experience] = ACTIONS(17),
    [anon_sym_fetchprofile] = ACTIONS(17),
    [anon_sym_fill] = ACTIONS(19),
    [anon_sym_fillbiome] = ACTIONS(17),
    [anon_sym_forceload] = ACTIONS(17),
    [anon_sym_function] = ACTIONS(17),
    [anon_sym_gamemode] = ACTIONS(17),
    [anon_sym_gamerule] = ACTIONS(17),
    [anon_sym_give] = ACTIONS(17),
    [anon_sym_help] = ACTIONS(17),
    [anon_sym_item] = ACTIONS(17),
    [anon_sym_jfr] = ACTIONS(17),
    [anon_sym_kick] = ACTIONS(17),
    [anon_sym_kill] = ACTIONS(17),
    [anon_sym_list] = ACTIONS(17),
    [anon_sym_locate] = ACTIONS(17),
    [anon_sym_loot] = ACTIONS(17),
    [anon_sym_me] = ACTIONS(17),
    [anon_sym_msg] = ACTIONS(17),
    [anon_sym_op] = ACTIONS(17),
    [anon_sym_pardon] = ACTIONS(19),
    [anon_sym_pardon_DASHip] = ACTIONS(17),
    [anon_sym_particle] = ACTIONS(17),
    [anon_sym_perf] = ACTIONS(17),
    [anon_sym_place] = ACTIONS(17),
    [anon_sym_playsound] = ACTIONS(17),
    [anon_sym_publish] = ACTIONS(17),
    [anon_sym_random] = ACTIONS(17),
    [anon_sym_recipe] = ACTIONS(17),
    [anon_sym_reload] = ACTIONS(17),
    [anon_sym_return] = ACTIONS(17),
    [anon_sym_ride] = ACTIONS(17),
    [anon_sym_rotate] = ACTIONS(17),
    [anon_sym_save_DASHall] = ACTIONS(17),
    [anon_sym_save_DASHoff] = ACTIONS(17),
    [anon_sym_save_DASHon] = ACTIONS(17),
    [anon_sym_say] = ACTIONS(17),
    [anon_sym_schedule] = ACTIONS(17),
    [anon_sym_scoreboard] = ACTIONS(17),
    [anon_sym_seed] = ACTIONS(17),
    [anon_sym_setblock] = ACTIONS(17),
    [anon_sym_setidletimeout] = ACTIONS(17),
    [anon_sym_setworldspawn] = ACTIONS(17),
    [anon_sym_spawnpoint] = ACTIONS(17),
    [anon_sym_spectate] = ACTIONS(17),
    [anon_sym_spreadplayers] = ACTIONS(17),
    [anon_sym_stop] = ACTIONS(19),
    [anon_sym_stopsound] = ACTIONS(17),
    [anon_sym_summon] = ACTIONS(17),
    [anon_sym_swing] = ACTIONS(17),
    [anon_sym_tag] = ACTIONS(17),
    [anon_sym_team] = ACTIONS(19),
    [anon_sym_teammsg] = ACTIONS(17),
    [anon_sym_teleport] = ACTIONS(17),
    [anon_sym_tell] = ACTIONS(19),
    [anon_sym_tellraw] = ACTIONS(17),
    [anon_sym_test] = ACTIONS(17),
    [anon_sym_time] = ACTIONS(17),
    [anon_sym_title] = ACTIONS(17),
    [anon_sym_tm] = ACTIONS(17),
    [anon_sym_tp] = ACTIONS(17),
    [anon_sym_transfer] = ACTIONS(17),
    [anon_sym_trigger] = ACTIONS(17),
    [anon_sym_version] = ACTIONS(17),
    [anon_sym_w] = ACTIONS(19),
    [anon_sym_waypoint] = ACTIONS(17),
    [anon_sym_weather] = ACTIONS(17),
    [anon_sym_whitelist] = ACTIONS(17),
    [anon_sym_worldborder] = ACTIONS(17),
    [anon_sym_xp] = ACTIONS(17),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
  },
  [STATE(6)] = {
    [sym__statement] = STATE(10),
    [sym_command] = STATE(237),
    [sym__execute_command] = STATE(236),
    [sym__generic_command] = STATE(236),
    [sym_command_identifier] = STATE(130),
    [sym_macro] = STATE(6),
    [aux_sym_source_file_repeat1] = STATE(6),
    [ts_builtin_sym_end] = ACTIONS(55),
    [sym__newline] = ACTIONS(57),
    [aux_sym__statement_token1] = ACTIONS(60),
    [sym_comment] = ACTIONS(63),
    [anon_sym_execute] = ACTIONS(66),
    [sym_macro_indicator] = ACTIONS(69),
    [anon_sym_advancement] = ACTIONS(72),
    [anon_sym_attribute] = ACTIONS(72),
    [anon_sym_ban] = ACTIONS(75),
    [anon_sym_ban_DASHip] = ACTIONS(72),
    [anon_sym_banlist] = ACTIONS(72),
    [anon_sym_bossbar] = ACTIONS(72),
    [anon_sym_clear] = ACTIONS(72),
    [anon_sym_clone] = ACTIONS(72),
    [anon_sym_damage] = ACTIONS(72),
    [anon_sym_data] = ACTIONS(75),
    [anon_sym_datapack] = ACTIONS(72),
    [anon_sym_debug] = ACTIONS(72),
    [anon_sym_defaultgamemode] = ACTIONS(72),
    [anon_sym_deop] = ACTIONS(72),
    [anon_sym_dialog] = ACTIONS(72),
    [anon_sym_difficulty] = ACTIONS(72),
    [anon_sym_effect] = ACTIONS(72),
    [anon_sym_enchant] = ACTIONS(72),
    [anon_sym_experience] = ACTIONS(72),
    [anon_sym_fetchprofile] = ACTIONS(72),
    [anon_sym_fill] = ACTIONS(75),
    [anon_sym_fillbiome] = ACTIONS(72),
    [anon_sym_forceload] = ACTIONS(72),
    [anon_sym_function] = ACTIONS(72),
    [anon_sym_gamemode] = ACTIONS(72),
    [anon_sym_gamerule] = ACTIONS(72),
    [anon_sym_give] = ACTIONS(72),
    [anon_sym_help] = ACTIONS(72),
    [anon_sym_item] = ACTIONS(72),
    [anon_sym_jfr] = ACTIONS(72),
    [anon_sym_kick] = ACTIONS(72),
    [anon_sym_kill] = ACTIONS(72),
    [anon_sym_list] = ACTIONS(72),
    [anon_sym_locate] = ACTIONS(72),
    [anon_sym_loot] = ACTIONS(72),
    [anon_sym_me] = ACTIONS(72),
    [anon_sym_msg] = ACTIONS(72),
    [anon_sym_op] = ACTIONS(72),
    [anon_sym_pardon] = ACTIONS(75),
    [anon_sym_pardon_DASHip] = ACTIONS(72),
    [anon_sym_particle] = ACTIONS(72),
    [anon_sym_perf] = ACTIONS(72),
    [anon_sym_place] = ACTIONS(72),
    [anon_sym_playsound] = ACTIONS(72),
    [anon_sym_publish] = ACTIONS(72),
    [anon_sym_random] = ACTIONS(72),
    [anon_sym_recipe] = ACTIONS(72),
    [anon_sym_reload] = ACTIONS(72),
    [anon_sym_return] = ACTIONS(72),
    [anon_sym_ride] = ACTIONS(72),
    [anon_sym_rotate] = ACTIONS(72),
    [anon_sym_save_DASHall] = ACTIONS(72),
    [anon_sym_save_DASHoff] = ACTIONS(72),
    [anon_sym_save_DASHon] = ACTIONS(72),
    [anon_sym_say] = ACTIONS(72),
    [anon_sym_schedule] = ACTIONS(72),
    [anon_sym_scoreboard] = ACTIONS(72),
    [anon_sym_seed] = ACTIONS(72),
    [anon_sym_setblock] = ACTIONS(72),
    [anon_sym_setidletimeout] = ACTIONS(72),
    [anon_sym_setworldspawn] = ACTIONS(72),
    [anon_sym_spawnpoint] = ACTIONS(72),
    [anon_sym_spectate] = ACTIONS(72),
    [anon_sym_spreadplayers] = ACTIONS(72),
    [anon_sym_stop] = ACTIONS(75),
    [anon_sym_stopsound] = ACTIONS(72),
    [anon_sym_summon] = ACTIONS(72),
    [anon_sym_swing] = ACTIONS(72),
    [anon_sym_tag] = ACTIONS(72),
    [anon_sym_team] = ACTIONS(75),
    [anon_sym_teammsg] = ACTIONS(72),
    [anon_sym_teleport] = ACTIONS(72),
    [anon_sym_tell] = ACTIONS(75),
    [anon_sym_tellraw] = ACTIONS(72),
    [anon_sym_test] = ACTIONS(72),
    [anon_sym_time] = ACTIONS(72),
    [anon_sym_title] = ACTIONS(72),
    [anon_sym_tm] = ACTIONS(72),
    [anon_sym_tp] = ACTIONS(72),
    [anon_sym_transfer] = ACTIONS(72),
    [anon_sym_trigger] = ACTIONS(72),
    [anon_sym_version] = ACTIONS(72),
    [anon_sym_w] = ACTIONS(75),
    [anon_sym_waypoint] = ACTIONS(72),
    [anon_sym_weather] = ACTIONS(72),
    [anon_sym_whitelist] = ACTIONS(72),
    [anon_sym_worldborder] = ACTIONS(72),
    [anon_sym_xp] = ACTIONS(72),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
  },
  [STATE(7)] = {
    [sym_command] = STATE(238),
    [sym__execute_command] = STATE(236),
    [sym__generic_command] = STATE(236),
    [sym_command_identifier] = STATE(130),
    [sym_macro] = STATE(7),
    [sym__newline] = ACTIONS(78),
    [sym_comment] = ACTIONS(80),
    [anon_sym_execute] = ACTIONS(13),
    [sym_macro_indicator] = ACTIONS(15),
    [anon_sym_advancement] = ACTIONS(17),
    [anon_sym_attribute] = ACTIONS(17),
    [anon_sym_ban] = ACTIONS(19),
    [anon_sym_ban_DASHip] = ACTIONS(17),
    [anon_sym_banlist] = ACTIONS(17),
    [anon_sym_bossbar] = ACTIONS(17),
    [anon_sym_clear] = ACTIONS(17),
    [anon_sym_clone] = ACTIONS(17),
    [anon_sym_damage] = ACTIONS(17),
    [anon_sym_data] = ACTIONS(19),
    [anon_sym_datapack] = ACTIONS(17),
    [anon_sym_debug] = ACTIONS(17),
    [anon_sym_defaultgamemode] = ACTIONS(17),
    [anon_sym_deop] = ACTIONS(17),
    [anon_sym_dialog] = ACTIONS(17),
    [anon_sym_difficulty] = ACTIONS(17),
    [anon_sym_effect] = ACTIONS(17),
    [anon_sym_enchant] = ACTIONS(17),
    [anon_sym_experience] = ACTIONS(17),
    [anon_sym_fetchprofile] = ACTIONS(17),
    [anon_sym_fill] = ACTIONS(19),
    [anon_sym_fillbiome] = ACTIONS(17),
    [anon_sym_forceload] = ACTIONS(17),
    [anon_sym_function] = ACTIONS(17),
    [anon_sym_gamemode] = ACTIONS(17),
    [anon_sym_gamerule] = ACTIONS(17),
    [anon_sym_give] = ACTIONS(17),
    [anon_sym_help] = ACTIONS(17),
    [anon_sym_item] = ACTIONS(17),
    [anon_sym_jfr] = ACTIONS(17),
    [anon_sym_kick] = ACTIONS(17),
    [anon_sym_kill] = ACTIONS(17),
    [anon_sym_list] = ACTIONS(17),
    [anon_sym_locate] = ACTIONS(17),
    [anon_sym_loot] = ACTIONS(17),
    [anon_sym_me] = ACTIONS(17),
    [anon_sym_msg] = ACTIONS(17),
    [anon_sym_op] = ACTIONS(17),
    [anon_sym_pardon] = ACTIONS(19),
    [anon_sym_pardon_DASHip] = ACTIONS(17),
    [anon_sym_particle] = ACTIONS(17),
    [anon_sym_perf] = ACTIONS(17),
    [anon_sym_place] = ACTIONS(17),
    [anon_sym_playsound] = ACTIONS(17),
    [anon_sym_publish] = ACTIONS(17),
    [anon_sym_random] = ACTIONS(17),
    [anon_sym_recipe] = ACTIONS(17),
    [anon_sym_reload] = ACTIONS(17),
    [anon_sym_return] = ACTIONS(17),
    [anon_sym_ride] = ACTIONS(17),
    [anon_sym_rotate] = ACTIONS(17),
    [anon_sym_save_DASHall] = ACTIONS(17),
    [anon_sym_save_DASHoff] = ACTIONS(17),
    [anon_sym_save_DASHon] = ACTIONS(17),
    [anon_sym_say] = ACTIONS(17),
    [anon_sym_schedule] = ACTIONS(17),
    [anon_sym_scoreboard] = ACTIONS(17),
    [anon_sym_seed] = ACTIONS(17),
    [anon_sym_setblock] = ACTIONS(17),
    [anon_sym_setidletimeout] = ACTIONS(17),
    [anon_sym_setworldspawn] = ACTIONS(17),
    [anon_sym_spawnpoint] = ACTIONS(17),
    [anon_sym_spectate] = ACTIONS(17),
    [anon_sym_spreadplayers] = ACTIONS(17),
    [anon_sym_stop] = ACTIONS(19),
    [anon_sym_stopsound] = ACTIONS(17),
    [anon_sym_summon] = ACTIONS(17),
    [anon_sym_swing] = ACTIONS(17),
    [anon_sym_tag] = ACTIONS(17),
    [anon_sym_team] = ACTIONS(19),
    [anon_sym_teammsg] = ACTIONS(17),
    [anon_sym_teleport] = ACTIONS(17),
    [anon_sym_tell] = ACTIONS(19),
    [anon_sym_tellraw] = ACTIONS(17),
    [anon_sym_test] = ACTIONS(17),
    [anon_sym_time] = ACTIONS(17),
    [anon_sym_title] = ACTIONS(17),
    [anon_sym_tm] = ACTIONS(17),
    [anon_sym_tp] = ACTIONS(17),
    [anon_sym_transfer] = ACTIONS(17),
    [anon_sym_trigger] = ACTIONS(17),
    [anon_sym_version] = ACTIONS(17),
    [anon_sym_w] = ACTIONS(19),
    [anon_sym_waypoint] = ACTIONS(17),
    [anon_sym_weather] = ACTIONS(17),
    [anon_sym_whitelist] = ACTIONS(17),
    [anon_sym_worldborder] = ACTIONS(17),
    [anon_sym_xp] = ACTIONS(17),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
  },
  [STATE(8)] = {
    [sym_macro] = STATE(8),
    [ts_builtin_sym_end] = ACTIONS(82),
    [sym__newline] = ACTIONS(84),
    [aux_sym__statement_token1] = ACTIONS(84),
    [sym_comment] = ACTIONS(82),
    [anon_sym_execute] = ACTIONS(82),
    [sym_macro_indicator] = ACTIONS(84),
    [anon_sym_advancement] = ACTIONS(82),
    [anon_sym_attribute] = ACTIONS(82),
    [anon_sym_ban] = ACTIONS(84),
    [anon_sym_ban_DASHip] = ACTIONS(82),
    [anon_sym_banlist] = ACTIONS(82),
    [anon_sym_bossbar] = ACTIONS(82),
    [anon_sym_clear] = ACTIONS(82),
    [anon_sym_clone] = ACTIONS(82),
    [anon_sym_damage] = ACTIONS(82),
    [anon_sym_data] = ACTIONS(84),
    [anon_sym_datapack] = ACTIONS(82),
    [anon_sym_debug] = ACTIONS(82),
    [anon_sym_defaultgamemode] = ACTIONS(82),
    [anon_sym_deop] = ACTIONS(82),
    [anon_sym_dialog] = ACTIONS(82),
    [anon_sym_difficulty] = ACTIONS(82),
    [anon_sym_effect] = ACTIONS(82),
    [anon_sym_enchant] = ACTIONS(82),
    [anon_sym_experience] = ACTIONS(82),
    [anon_sym_fetchprofile] = ACTIONS(82),
    [anon_sym_fill] = ACTIONS(84),
    [anon_sym_fillbiome] = ACTIONS(82),
    [anon_sym_forceload] = ACTIONS(82),
    [anon_sym_function] = ACTIONS(82),
    [anon_sym_gamemode] = ACTIONS(82),
    [anon_sym_gamerule] = ACTIONS(82),
    [anon_sym_give] = ACTIONS(82),
    [anon_sym_help] = ACTIONS(82),
    [anon_sym_item] = ACTIONS(82),
    [anon_sym_jfr] = ACTIONS(82),
    [anon_sym_kick] = ACTIONS(82),
    [anon_sym_kill] = ACTIONS(82),
    [anon_sym_list] = ACTIONS(82),
    [anon_sym_locate] = ACTIONS(82),
    [anon_sym_loot] = ACTIONS(82),
    [anon_sym_me] = ACTIONS(82),
    [anon_sym_msg] = ACTIONS(82),
    [anon_sym_op] = ACTIONS(82),
    [anon_sym_pardon] = ACTIONS(84),
    [anon_sym_pardon_DASHip] = ACTIONS(82),
    [anon_sym_particle] = ACTIONS(82),
    [anon_sym_perf] = ACTIONS(82),
    [anon_sym_place] = ACTIONS(82),
    [anon_sym_playsound] = ACTIONS(82),
    [anon_sym_publish] = ACTIONS(82),
    [anon_sym_random] = ACTIONS(82),
    [anon_sym_recipe] = ACTIONS(82),
    [anon_sym_reload] = ACTIONS(82),
    [anon_sym_return] = ACTIONS(82),
    [anon_sym_ride] = ACTIONS(82),
    [anon_sym_rotate] = ACTIONS(82),
    [anon_sym_save_DASHall] = ACTIONS(82),
    [anon_sym_save_DASHoff] = ACTIONS(82),
    [anon_sym_save_DASHon] = ACTIONS(82),
    [anon_sym_say] = ACTIONS(82),
    [anon_sym_schedule] = ACTIONS(82),
    [anon_sym_scoreboard] = ACTIONS(82),
    [anon_sym_seed] = ACTIONS(82),
    [anon_sym_setblock] = ACTIONS(82),
    [anon_sym_setidletimeout] = ACTIONS(82),
    [anon_sym_setworldspawn] = ACTIONS(82),
    [anon_sym_spawnpoint] = ACTIONS(82),
    [anon_sym_spectate] = ACTIONS(82),
    [anon_sym_spreadplayers] = ACTIONS(82),
    [anon_sym_stop] = ACTIONS(84),
    [anon_sym_stopsound] = ACTIONS(82),
    [anon_sym_summon] = ACTIONS(82),
    [anon_sym_swing] = ACTIONS(82),
    [anon_sym_tag] = ACTIONS(82),
    [anon_sym_team] = ACTIONS(84),
    [anon_sym_teammsg] = ACTIONS(82),
    [anon_sym_teleport] = ACTIONS(82),
    [anon_sym_tell] = ACTIONS(84),
    [anon_sym_tellraw] = ACTIONS(82),
    [anon_sym_test] = ACTIONS(82),
    [anon_sym_time] = ACTIONS(82),
    [anon_sym_title] = ACTIONS(82),
    [anon_sym_tm] = ACTIONS(82),
    [anon_sym_tp] = ACTIONS(82),
    [anon_sym_transfer] = ACTIONS(82),
    [anon_sym_trigger] = ACTIONS(82),
    [anon_sym_version] = ACTIONS(82),
    [anon_sym_w] = ACTIONS(84),
    [anon_sym_waypoint] = ACTIONS(82),
    [anon_sym_weather] = ACTIONS(82),
    [anon_sym_whitelist] = ACTIONS(82),
    [anon_sym_worldborder] = ACTIONS(82),
    [anon_sym_xp] = ACTIONS(82),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
  },
  [STATE(9)] = {
    [sym_macro] = STATE(9),
    [ts_builtin_sym_end] = ACTIONS(86),
    [sym__newline] = ACTIONS(88),
    [aux_sym__statement_token1] = ACTIONS(88),
    [sym_comment] = ACTIONS(86),
    [anon_sym_execute] = ACTIONS(86),
    [sym_macro_indicator] = ACTIONS(88),
    [anon_sym_advancement] = ACTIONS(86),
    [anon_sym_attribute] = ACTIONS(86),
    [anon_sym_ban] = ACTIONS(88),
    [anon_sym_ban_DASHip] = ACTIONS(86),
    [anon_sym_banlist] = ACTIONS(86),
    [anon_sym_bossbar] = ACTIONS(86),
    [anon_sym_clear] = ACTIONS(86),
    [anon_sym_clone] = ACTIONS(86),
    [anon_sym_damage] = ACTIONS(86),
    [anon_sym_data] = ACTIONS(88),
    [anon_sym_datapack] = ACTIONS(86),
    [anon_sym_debug] = ACTIONS(86),
    [anon_sym_defaultgamemode] = ACTIONS(86),
    [anon_sym_deop] = ACTIONS(86),
    [anon_sym_dialog] = ACTIONS(86),
    [anon_sym_difficulty] = ACTIONS(86),
    [anon_sym_effect] = ACTIONS(86),
    [anon_sym_enchant] = ACTIONS(86),
    [anon_sym_experience] = ACTIONS(86),
    [anon_sym_fetchprofile] = ACTIONS(86),
    [anon_sym_fill] = ACTIONS(88),
    [anon_sym_fillbiome] = ACTIONS(86),
    [anon_sym_forceload] = ACTIONS(86),
    [anon_sym_function] = ACTIONS(86),
    [anon_sym_gamemode] = ACTIONS(86),
    [anon_sym_gamerule] = ACTIONS(86),
    [anon_sym_give] = ACTIONS(86),
    [anon_sym_help] = ACTIONS(86),
    [anon_sym_item] = ACTIONS(86),
    [anon_sym_jfr] = ACTIONS(86),
    [anon_sym_kick] = ACTIONS(86),
    [anon_sym_kill] = ACTIONS(86),
    [anon_sym_list] = ACTIONS(86),
    [anon_sym_locate] = ACTIONS(86),
    [anon_sym_loot] = ACTIONS(86),
    [anon_sym_me] = ACTIONS(86),
    [anon_sym_msg] = ACTIONS(86),
    [anon_sym_op] = ACTIONS(86),
    [anon_sym_pardon] = ACTIONS(88),
    [anon_sym_pardon_DASHip] = ACTIONS(86),
    [anon_sym_particle] = ACTIONS(86),
    [anon_sym_perf] = ACTIONS(86),
    [anon_sym_place] = ACTIONS(86),
    [anon_sym_playsound] = ACTIONS(86),
    [anon_sym_publish] = ACTIONS(86),
    [anon_sym_random] = ACTIONS(86),
    [anon_sym_recipe] = ACTIONS(86),
    [anon_sym_reload] = ACTIONS(86),
    [anon_sym_return] = ACTIONS(86),
    [anon_sym_ride] = ACTIONS(86),
    [anon_sym_rotate] = ACTIONS(86),
    [anon_sym_save_DASHall] = ACTIONS(86),
    [anon_sym_save_DASHoff] = ACTIONS(86),
    [anon_sym_save_DASHon] = ACTIONS(86),
    [anon_sym_say] = ACTIONS(86),
    [anon_sym_schedule] = ACTIONS(86),
    [anon_sym_scoreboard] = ACTIONS(86),
    [anon_sym_seed] = ACTIONS(86),
    [anon_sym_setblock] = ACTIONS(86),
    [anon_sym_setidletimeout] = ACTIONS(86),
    [anon_sym_setworldspawn] = ACTIONS(86),
    [anon_sym_spawnpoint] = ACTIONS(86),
    [anon_sym_spectate] = ACTIONS(86),
    [anon_sym_spreadplayers] = ACTIONS(86),
    [anon_sym_stop] = ACTIONS(88),
    [anon_sym_stopsound] = ACTIONS(86),
    [anon_sym_summon] = ACTIONS(86),
    [anon_sym_swing] = ACTIONS(86),
    [anon_sym_tag] = ACTIONS(86),
    [anon_sym_team] = ACTIONS(88),
    [anon_sym_teammsg] = ACTIONS(86),
    [anon_sym_teleport] = ACTIONS(86),
    [anon_sym_tell] = ACTIONS(88),
    [anon_sym_tellraw] = ACTIONS(86),
    [anon_sym_test] = ACTIONS(86),
    [anon_sym_time] = ACTIONS(86),
    [anon_sym_title] = ACTIONS(86),
    [anon_sym_tm] = ACTIONS(86),
    [anon_sym_tp] = ACTIONS(86),
    [anon_sym_transfer] = ACTIONS(86),
    [anon_sym_trigger] = ACTIONS(86),
    [anon_sym_version] = ACTIONS(86),
    [anon_sym_w] = ACTIONS(88),
    [anon_sym_waypoint] = ACTIONS(86),
    [anon_sym_weather] = ACTIONS(86),
    [anon_sym_whitelist] = ACTIONS(86),
    [anon_sym_worldborder] = ACTIONS(86),
    [anon_sym_xp] = ACTIONS(86),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
  },
  [STATE(10)] = {
    [sym_macro] = STATE(10),
    [ts_builtin_sym_end] = ACTIONS(90),
    [sym__newline] = ACTIONS(92),
    [aux_sym__statement_token1] = ACTIONS(92),
    [sym_comment] = ACTIONS(90),
    [anon_sym_execute] = ACTIONS(90),
    [sym_macro_indicator] = ACTIONS(92),
    [anon_sym_advancement] = ACTIONS(90),
    [anon_sym_attribute] = ACTIONS(90),
    [anon_sym_ban] = ACTIONS(92),
    [anon_sym_ban_DASHip] = ACTIONS(90),
    [anon_sym_banlist] = ACTIONS(90),
    [anon_sym_bossbar] = ACTIONS(90),
    [anon_sym_clear] = ACTIONS(90),
    [anon_sym_clone] = ACTIONS(90),
    [anon_sym_damage] = ACTIONS(90),
    [anon_sym_data] = ACTIONS(92),
    [anon_sym_datapack] = ACTIONS(90),
    [anon_sym_debug] = ACTIONS(90),
    [anon_sym_defaultgamemode] = ACTIONS(90),
    [anon_sym_deop] = ACTIONS(90),
    [anon_sym_dialog] = ACTIONS(90),
    [anon_sym_difficulty] = ACTIONS(90),
    [anon_sym_effect] = ACTIONS(90),
    [anon_sym_enchant] = ACTIONS(90),
    [anon_sym_experience] = ACTIONS(90),
    [anon_sym_fetchprofile] = ACTIONS(90),
    [anon_sym_fill] = ACTIONS(92),
    [anon_sym_fillbiome] = ACTIONS(90),
    [anon_sym_forceload] = ACTIONS(90),
    [anon_sym_function] = ACTIONS(90),
    [anon_sym_gamemode] = ACTIONS(90),
    [anon_sym_gamerule] = ACTIONS(90),
    [anon_sym_give] = ACTIONS(90),
    [anon_sym_help] = ACTIONS(90),
    [anon_sym_item] = ACTIONS(90),
    [anon_sym_jfr] = ACTIONS(90),
    [anon_sym_kick] = ACTIONS(90),
    [anon_sym_kill] = ACTIONS(90),
    [anon_sym_list] = ACTIONS(90),
    [anon_sym_locate] = ACTIONS(90),
    [anon_sym_loot] = ACTIONS(90),
    [anon_sym_me] = ACTIONS(90),
    [anon_sym_msg] = ACTIONS(90),
    [anon_sym_op] = ACTIONS(90),
    [anon_sym_pardon] = ACTIONS(92),
    [anon_sym_pardon_DASHip] = ACTIONS(90),
    [anon_sym_particle] = ACTIONS(90),
    [anon_sym_perf] = ACTIONS(90),
    [anon_sym_place] = ACTIONS(90),
    [anon_sym_playsound] = ACTIONS(90),
    [anon_sym_publish] = ACTIONS(90),
    [anon_sym_random] = ACTIONS(90),
    [anon_sym_recipe] = ACTIONS(90),
    [anon_sym_reload] = ACTIONS(90),
    [anon_sym_return] = ACTIONS(90),
    [anon_sym_ride] = ACTIONS(90),
    [anon_sym_rotate] = ACTIONS(90),
    [anon_sym_save_DASHall] = ACTIONS(90),
    [anon_sym_save_DASHoff] = ACTIONS(90),
    [anon_sym_save_DASHon] = ACTIONS(90),
    [anon_sym_say] = ACTIONS(90),
    [anon_sym_schedule] = ACTIONS(90),
    [anon_sym_scoreboard] = ACTIONS(90),
    [anon_sym_seed] = ACTIONS(90),
    [anon_sym_setblock] = ACTIONS(90),
    [anon_sym_setidletimeout] = ACTIONS(90),
    [anon_sym_setworldspawn] = ACTIONS(90),
    [anon_sym_spawnpoint] = ACTIONS(90),
    [anon_sym_spectate] = ACTIONS(90),
    [anon_sym_spreadplayers] = ACTIONS(90),
    [anon_sym_stop] = ACTIONS(92),
    [anon_sym_stopsound] = ACTIONS(90),
    [anon_sym_summon] = ACTIONS(90),
    [anon_sym_swing] = ACTIONS(90),
    [anon_sym_tag] = ACTIONS(90),
    [anon_sym_team] = ACTIONS(92),
    [anon_sym_teammsg] = ACTIONS(90),
    [anon_sym_teleport] = ACTIONS(90),
    [anon_sym_tell] = ACTIONS(92),
    [anon_sym_tellraw] = ACTIONS(90),
    [anon_sym_test] = ACTIONS(90),
    [anon_sym_time] = ACTIONS(90),
    [anon_sym_title] = ACTIONS(90),
    [anon_sym_tm] = ACTIONS(90),
    [anon_sym_tp] = ACTIONS(90),
    [anon_sym_transfer] = ACTIONS(90),
    [anon_sym_trigger] = ACTIONS(90),
    [anon_sym_version] = ACTIONS(90),
    [anon_sym_w] = ACTIONS(92),
    [anon_sym_waypoint] = ACTIONS(90),
    [anon_sym_weather] = ACTIONS(90),
    [anon_sym_whitelist] = ACTIONS(90),
    [anon_sym_worldborder] = ACTIONS(90),
    [anon_sym_xp] = ACTIONS(90),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
  },
  [STATE(11)] = {
    [sym_macro] = STATE(11),
    [ts_builtin_sym_end] = ACTIONS(94),
    [sym__newline] = ACTIONS(96),
    [aux_sym__statement_token1] = ACTIONS(96),
    [sym_comment] = ACTIONS(94),
    [anon_sym_execute] = ACTIONS(94),
    [sym_macro_indicator] = ACTIONS(96),
    [anon_sym_advancement] = ACTIONS(94),
    [anon_sym_attribute] = ACTIONS(94),
    [anon_sym_ban] = ACTIONS(96),
    [anon_sym_ban_DASHip] = ACTIONS(94),
    [anon_sym_banlist] = ACTIONS(94),
    [anon_sym_bossbar] = ACTIONS(94),
    [anon_sym_clear] = ACTIONS(94),
    [anon_sym_clone] = ACTIONS(94),
    [anon_sym_damage] = ACTIONS(94),
    [anon_sym_data] = ACTIONS(96),
    [anon_sym_datapack] = ACTIONS(94),
    [anon_sym_debug] = ACTIONS(94),
    [anon_sym_defaultgamemode] = ACTIONS(94),
    [anon_sym_deop] = ACTIONS(94),
    [anon_sym_dialog] = ACTIONS(94),
    [anon_sym_difficulty] = ACTIONS(94),
    [anon_sym_effect] = ACTIONS(94),
    [anon_sym_enchant] = ACTIONS(94),
    [anon_sym_experience] = ACTIONS(94),
    [anon_sym_fetchprofile] = ACTIONS(94),
    [anon_sym_fill] = ACTIONS(96),
    [anon_sym_fillbiome] = ACTIONS(94),
    [anon_sym_forceload] = ACTIONS(94),
    [anon_sym_function] = ACTIONS(94),
    [anon_sym_gamemode] = ACTIONS(94),
    [anon_sym_gamerule] = ACTIONS(94),
    [anon_sym_give] = ACTIONS(94),
    [anon_sym_help] = ACTIONS(94),
    [anon_sym_item] = ACTIONS(94),
    [anon_sym_jfr] = ACTIONS(94),
    [anon_sym_kick] = ACTIONS(94),
    [anon_sym_kill] = ACTIONS(94),
    [anon_sym_list] = ACTIONS(94),
    [anon_sym_locate] = ACTIONS(94),
    [anon_sym_loot] = ACTIONS(94),
    [anon_sym_me] = ACTIONS(94),
    [anon_sym_msg] = ACTIONS(94),
    [anon_sym_op] = ACTIONS(94),
    [anon_sym_pardon] = ACTIONS(96),
    [anon_sym_pardon_DASHip] = ACTIONS(94),
    [anon_sym_particle] = ACTIONS(94),
    [anon_sym_perf] = ACTIONS(94),
    [anon_sym_place] = ACTIONS(94),
    [anon_sym_playsound] = ACTIONS(94),
    [anon_sym_publish] = ACTIONS(94),
    [anon_sym_random] = ACTIONS(94),
    [anon_sym_recipe] = ACTIONS(94),
    [anon_sym_reload] = ACTIONS(94),
    [anon_sym_return] = ACTIONS(94),
    [anon_sym_ride] = ACTIONS(94),
    [anon_sym_rotate] = ACTIONS(94),
    [anon_sym_save_DASHall] = ACTIONS(94),
    [anon_sym_save_DASHoff] = ACTIONS(94),
    [anon_sym_save_DASHon] = ACTIONS(94),
    [anon_sym_say] = ACTIONS(94),
    [anon_sym_schedule] = ACTIONS(94),
    [anon_sym_scoreboard] = ACTIONS(94),
    [anon_sym_seed] = ACTIONS(94),
    [anon_sym_setblock] = ACTIONS(94),
    [anon_sym_setidletimeout] = ACTIONS(94),
    [anon_sym_setworldspawn] = ACTIONS(94),
    [anon_sym_spawnpoint] = ACTIONS(94),
    [anon_sym_spectate] = ACTIONS(94),
    [anon_sym_spreadplayers] = ACTIONS(94),
    [anon_sym_stop] = ACTIONS(96),
    [anon_sym_stopsound] = ACTIONS(94),
    [anon_sym_summon] = ACTIONS(94),
    [anon_sym_swing] = ACTIONS(94),
    [anon_sym_tag] = ACTIONS(94),
    [anon_sym_team] = ACTIONS(96),
    [anon_sym_teammsg] = ACTIONS(94),
    [anon_sym_teleport] = ACTIONS(94),
    [anon_sym_tell] = ACTIONS(96),
    [anon_sym_tellraw] = ACTIONS(94),
    [anon_sym_test] = ACTIONS(94),
    [anon_sym_time] = ACTIONS(94),
    [anon_sym_title] = ACTIONS(94),
    [anon_sym_tm] = ACTIONS(94),
    [anon_sym_tp] = ACTIONS(94),
    [anon_sym_transfer] = ACTIONS(94),
    [anon_sym_trigger] = ACTIONS(94),
    [anon_sym_version] = ACTIONS(94),
    [anon_sym_w] = ACTIONS(96),
    [anon_sym_waypoint] = ACTIONS(94),
    [anon_sym_weather] = ACTIONS(94),
    [anon_sym_whitelist] = ACTIONS(94),
    [anon_sym_worldborder] = ACTIONS(94),
    [anon_sym_xp] = ACTIONS(94),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
  },
  [STATE(12)] = {
    [sym_command] = STATE(242),
    [sym__execute_command] = STATE(236),
    [sym__generic_command] = STATE(236),
    [sym_command_identifier] = STATE(130),
    [sym_macro] = STATE(12),
    [anon_sym_execute] = ACTIONS(13),
    [sym_macro_indicator] = ACTIONS(15),
    [anon_sym_advancement] = ACTIONS(17),
    [anon_sym_attribute] = ACTIONS(17),
    [anon_sym_ban] = ACTIONS(19),
    [anon_sym_ban_DASHip] = ACTIONS(17),
    [anon_sym_banlist] = ACTIONS(17),
    [anon_sym_bossbar] = ACTIONS(17),
    [anon_sym_clear] = ACTIONS(17),
    [anon_sym_clone] = ACTIONS(17),
    [anon_sym_damage] = ACTIONS(17),
    [anon_sym_data] = ACTIONS(19),
    [anon_sym_datapack] = ACTIONS(17),
    [anon_sym_debug] = ACTIONS(17),
    [anon_sym_defaultgamemode] = ACTIONS(17),
    [anon_sym_deop] = ACTIONS(17),
    [anon_sym_dialog] = ACTIONS(17),
    [anon_sym_difficulty] = ACTIONS(17),
    [anon_sym_effect] = ACTIONS(17),
    [anon_sym_enchant] = ACTIONS(17),
    [anon_sym_experience] = ACTIONS(17),
    [anon_sym_fetchprofile] = ACTIONS(17),
    [anon_sym_fill] = ACTIONS(19),
    [anon_sym_fillbiome] = ACTIONS(17),
    [anon_sym_forceload] = ACTIONS(17),
    [anon_sym_function] = ACTIONS(17),
    [anon_sym_gamemode] = ACTIONS(17),
    [anon_sym_gamerule] = ACTIONS(17),
    [anon_sym_give] = ACTIONS(17),
    [anon_sym_help] = ACTIONS(17),
    [anon_sym_item] = ACTIONS(17),
    [anon_sym_jfr] = ACTIONS(17),
    [anon_sym_kick] = ACTIONS(17),
    [anon_sym_kill] = ACTIONS(17),
    [anon_sym_list] = ACTIONS(17),
    [anon_sym_locate] = ACTIONS(17),
    [anon_sym_loot] = ACTIONS(17),
    [anon_sym_me] = ACTIONS(17),
    [anon_sym_msg] = ACTIONS(17),
    [anon_sym_op] = ACTIONS(17),
    [anon_sym_pardon] = ACTIONS(19),
    [anon_sym_pardon_DASHip] = ACTIONS(17),
    [anon_sym_particle] = ACTIONS(17),
    [anon_sym_perf] = ACTIONS(17),
    [anon_sym_place] = ACTIONS(17),
    [anon_sym_playsound] = ACTIONS(17),
    [anon_sym_publish] = ACTIONS(17),
    [anon_sym_random] = ACTIONS(17),
    [anon_sym_recipe] = ACTIONS(17),
    [anon_sym_reload] = ACTIONS(17),
    [anon_sym_return] = ACTIONS(17),
    [anon_sym_ride] = ACTIONS(17),
    [anon_sym_rotate] = ACTIONS(17),
    [anon_sym_save_DASHall] = ACTIONS(17),
    [anon_sym_save_DASHoff] = ACTIONS(17),
    [anon_sym_save_DASHon] = ACTIONS(17),
    [anon_sym_say] = ACTIONS(17),
    [anon_sym_schedule] = ACTIONS(17),
    [anon_sym_scoreboard] = ACTIONS(17),
    [anon_sym_seed] = ACTIONS(17),
    [anon_sym_setblock] = ACTIONS(17),
    [anon_sym_setidletimeout] = ACTIONS(17),
    [anon_sym_setworldspawn] = ACTIONS(17),
    [anon_sym_spawnpoint] = ACTIONS(17),
    [anon_sym_spectate] = ACTIONS(17),
    [anon_sym_spreadplayers] = ACTIONS(17),
    [anon_sym_stop] = ACTIONS(19),
    [anon_sym_stopsound] = ACTIONS(17),
    [anon_sym_summon] = ACTIONS(17),
    [anon_sym_swing] = ACTIONS(17),
    [anon_sym_tag] = ACTIONS(17),
    [anon_sym_team] = ACTIONS(19),
    [anon_sym_teammsg] = ACTIONS(17),
    [anon_sym_teleport] = ACTIONS(17),
    [anon_sym_tell] = ACTIONS(19),
    [anon_sym_tellraw] = ACTIONS(17),
    [anon_sym_test] = ACTIONS(17),
    [anon_sym_time] = ACTIONS(17),
    [anon_sym_title] = ACTIONS(17),
    [anon_sym_tm] = ACTIONS(17),
    [anon_sym_tp] = ACTIONS(17),
    [anon_sym_transfer] = ACTIONS(17),
    [anon_sym_trigger] = ACTIONS(17),
    [anon_sym_version] = ACTIONS(17),
    [anon_sym_w] = ACTIONS(19),
    [anon_sym_waypoint] = ACTIONS(17),
    [anon_sym_weather] = ACTIONS(17),
    [anon_sym_whitelist] = ACTIONS(17),
    [anon_sym_worldborder] = ACTIONS(17),
    [anon_sym_xp] = ACTIONS(17),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
  },
  [STATE(13)] = {
    [sym_command] = STATE(231),
    [sym__execute_command] = STATE(236),
    [sym__generic_command] = STATE(236),
    [sym_command_identifier] = STATE(130),
    [sym_macro] = STATE(13),
    [anon_sym_execute] = ACTIONS(13),
    [sym_macro_indicator] = ACTIONS(15),
    [anon_sym_advancement] = ACTIONS(17),
    [anon_sym_attribute] = ACTIONS(17),
    [anon_sym_ban] = ACTIONS(19),
    [anon_sym_ban_DASHip] = ACTIONS(17),
    [anon_sym_banlist] = ACTIONS(17),
    [anon_sym_bossbar] = ACTIONS(17),
    [anon_sym_clear] = ACTIONS(17),
    [anon_sym_clone] = ACTIONS(17),
    [anon_sym_damage] = ACTIONS(17),
    [anon_sym_data] = ACTIONS(19),
    [anon_sym_datapack] = ACTIONS(17),
    [anon_sym_debug] = ACTIONS(17),
    [anon_sym_defaultgamemode] = ACTIONS(17),
    [anon_sym_deop] = ACTIONS(17),
    [anon_sym_dialog] = ACTIONS(17),
    [anon_sym_difficulty] = ACTIONS(17),
    [anon_sym_effect] = ACTIONS(17),
    [anon_sym_enchant] = ACTIONS(17),
    [anon_sym_experience] = ACTIONS(17),
    [anon_sym_fetchprofile] = ACTIONS(17),
    [anon_sym_fill] = ACTIONS(19),
    [anon_sym_fillbiome] = ACTIONS(17),
    [anon_sym_forceload] = ACTIONS(17),
    [anon_sym_function] = ACTIONS(17),
    [anon_sym_gamemode] = ACTIONS(17),
    [anon_sym_gamerule] = ACTIONS(17),
    [anon_sym_give] = ACTIONS(17),
    [anon_sym_help] = ACTIONS(17),
    [anon_sym_item] = ACTIONS(17),
    [anon_sym_jfr] = ACTIONS(17),
    [anon_sym_kick] = ACTIONS(17),
    [anon_sym_kill] = ACTIONS(17),
    [anon_sym_list] = ACTIONS(17),
    [anon_sym_locate] = ACTIONS(17),
    [anon_sym_loot] = ACTIONS(17),
    [anon_sym_me] = ACTIONS(17),
    [anon_sym_msg] = ACTIONS(17),
    [anon_sym_op] = ACTIONS(17),
    [anon_sym_pardon] = ACTIONS(19),
    [anon_sym_pardon_DASHip] = ACTIONS(17),
    [anon_sym_particle] = ACTIONS(17),
    [anon_sym_perf] = ACTIONS(17),
    [anon_sym_place] = ACTIONS(17),
    [anon_sym_playsound] = ACTIONS(17),
    [anon_sym_publish] = ACTIONS(17),
    [anon_sym_random] = ACTIONS(17),
    [anon_sym_recipe] = ACTIONS(17),
    [anon_sym_reload] = ACTIONS(17),
    [anon_sym_return] = ACTIONS(17),
    [anon_sym_ride] = ACTIONS(17),
    [anon_sym_rotate] = ACTIONS(17),
    [anon_sym_save_DASHall] = ACTIONS(17),
    [anon_sym_save_DASHoff] = ACTIONS(17),
    [anon_sym_save_DASHon] = ACTIONS(17),
    [anon_sym_say] = ACTIONS(17),
    [anon_sym_schedule] = ACTIONS(17),
    [anon_sym_scoreboard] = ACTIONS(17),
    [anon_sym_seed] = ACTIONS(17),
    [anon_sym_setblock] = ACTIONS(17),
    [anon_sym_setidletimeout] = ACTIONS(17),
    [anon_sym_setworldspawn] = ACTIONS(17),
    [anon_sym_spawnpoint] = ACTIONS(17),
    [anon_sym_spectate] = ACTIONS(17),
    [anon_sym_spreadplayers] = ACTIONS(17),
    [anon_sym_stop] = ACTIONS(19),
    [anon_sym_stopsound] = ACTIONS(17),
    [anon_sym_summon] = ACTIONS(17),
    [anon_sym_swing] = ACTIONS(17),
    [anon_sym_tag] = ACTIONS(17),
    [anon_sym_team] = ACTIONS(19),
    [anon_sym_teammsg] = ACTIONS(17),
    [anon_sym_teleport] = ACTIONS(17),
    [anon_sym_tell] = ACTIONS(19),
    [anon_sym_tellraw] = ACTIONS(17),
    [anon_sym_test] = ACTIONS(17),
    [anon_sym_time] = ACTIONS(17),
    [anon_sym_title] = ACTIONS(17),
    [anon_sym_tm] = ACTIONS(17),
    [anon_sym_tp] = ACTIONS(17),
    [anon_sym_transfer] = ACTIONS(17),
    [anon_sym_trigger] = ACTIONS(17),
    [anon_sym_version] = ACTIONS(17),
    [anon_sym_w] = ACTIONS(19),
    [anon_sym_waypoint] = ACTIONS(17),
    [anon_sym_weather] = ACTIONS(17),
    [anon_sym_whitelist] = ACTIONS(17),
    [anon_sym_worldborder] = ACTIONS(17),
    [anon_sym_xp] = ACTIONS(17),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
  },
  [STATE(14)] = {
    [sym__execute_command] = STATE(224),
    [sym__generic_command] = STATE(224),
    [sym_command_identifier] = STATE(130),
    [sym_macro] = STATE(14),
    [anon_sym_execute] = ACTIONS(13),
    [anon_sym_advancement] = ACTIONS(17),
    [anon_sym_attribute] = ACTIONS(17),
    [anon_sym_ban] = ACTIONS(19),
    [anon_sym_ban_DASHip] = ACTIONS(17),
    [anon_sym_banlist] = ACTIONS(17),
    [anon_sym_bossbar] = ACTIONS(17),
    [anon_sym_clear] = ACTIONS(17),
    [anon_sym_clone] = ACTIONS(17),
    [anon_sym_damage] = ACTIONS(17),
    [anon_sym_data] = ACTIONS(19),
    [anon_sym_datapack] = ACTIONS(17),
    [anon_sym_debug] = ACTIONS(17),
    [anon_sym_defaultgamemode] = ACTIONS(17),
    [anon_sym_deop] = ACTIONS(17),
    [anon_sym_dialog] = ACTIONS(17),
    [anon_sym_difficulty] = ACTIONS(17),
    [anon_sym_effect] = ACTIONS(17),
    [anon_sym_enchant] = ACTIONS(17),
    [anon_sym_experience] = ACTIONS(17),
    [anon_sym_fetchprofile] = ACTIONS(17),
    [anon_sym_fill] = ACTIONS(19),
    [anon_sym_fillbiome] = ACTIONS(17),
    [anon_sym_forceload] = ACTIONS(17),
    [anon_sym_function] = ACTIONS(17),
    [anon_sym_gamemode] = ACTIONS(17),
    [anon_sym_gamerule] = ACTIONS(17),
    [anon_sym_give] = ACTIONS(17),
    [anon_sym_help] = ACTIONS(17),
    [anon_sym_item] = ACTIONS(17),
    [anon_sym_jfr] = ACTIONS(17),
    [anon_sym_kick] = ACTIONS(17),
    [anon_sym_kill] = ACTIONS(17),
    [anon_sym_list] = ACTIONS(17),
    [anon_sym_locate] = ACTIONS(17),
    [anon_sym_loot] = ACTIONS(17),
    [anon_sym_me] = ACTIONS(17),
    [anon_sym_msg] = ACTIONS(17),
    [anon_sym_op] = ACTIONS(17),
    [anon_sym_pardon] = ACTIONS(19),
    [anon_sym_pardon_DASHip] = ACTIONS(17),
    [anon_sym_particle] = ACTIONS(17),
    [anon_sym_perf] = ACTIONS(17),
    [anon_sym_place] = ACTIONS(17),
    [anon_sym_playsound] = ACTIONS(17),
    [anon_sym_publish] = ACTIONS(17),
    [anon_sym_random] = ACTIONS(17),
    [anon_sym_recipe] = ACTIONS(17),
    [anon_sym_reload] = ACTIONS(17),
    [anon_sym_return] = ACTIONS(17),
    [anon_sym_ride] = ACTIONS(17),
    [anon_sym_rotate] = ACTIONS(17),
    [anon_sym_save_DASHall] = ACTIONS(17),
    [anon_sym_save_DASHoff] = ACTIONS(17),
    [anon_sym_save_DASHon] = ACTIONS(17),
    [anon_sym_say] = ACTIONS(17),
    [anon_sym_schedule] = ACTIONS(17),
    [anon_sym_scoreboard] = ACTIONS(17),
    [anon_sym_seed] = ACTIONS(17),
    [anon_sym_setblock] = ACTIONS(17),
    [anon_sym_setidletimeout] = ACTIONS(17),
    [anon_sym_setworldspawn] = ACTIONS(17),
    [anon_sym_spawnpoint] = ACTIONS(17),
    [anon_sym_spectate] = ACTIONS(17),
    [anon_sym_spreadplayers] = ACTIONS(17),
    [anon_sym_stop] = ACTIONS(19),
    [anon_sym_stopsound] = ACTIONS(17),
    [anon_sym_summon] = ACTIONS(17),
    [anon_sym_swing] = ACTIONS(17),
    [anon_sym_tag] = ACTIONS(17),
    [anon_sym_team] = ACTIONS(19),
    [anon_sym_teammsg] = ACTIONS(17),
    [anon_sym_teleport] = ACTIONS(17),
    [anon_sym_tell] = ACTIONS(19),
    [anon_sym_tellraw] = ACTIONS(17),
    [anon_sym_test] = ACTIONS(17),
    [anon_sym_time] = ACTIONS(17),
    [anon_sym_title] = ACTIONS(17),
    [anon_sym_tm] = ACTIONS(17),
    [anon_sym_tp] = ACTIONS(17),
    [anon_sym_transfer] = ACTIONS(17),
    [anon_sym_trigger] = ACTIONS(17),
    [anon_sym_version] = ACTIONS(17),
    [anon_sym_w] = ACTIONS(19),
    [anon_sym_waypoint] = ACTIONS(17),
    [anon_sym_weather] = ACTIONS(17),
    [anon_sym_whitelist] = ACTIONS(17),
    [anon_sym_worldborder] = ACTIONS(17),
    [anon_sym_xp] = ACTIONS(17),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
  },
  [STATE(15)] = {
    [sym__subcommand] = STATE(220),
    [sym_subcommand_identifier] = STATE(221),
    [sym_selector] = STATE(221),
    [sym_position] = STATE(221),
    [sym_rotation] = STATE(221),
    [sym_scale] = STATE(221),
    [sym_macro] = STATE(15),
    [sym_operator] = STATE(221),
    [sym_slot] = STATE(221),
    [sym_word] = STATE(221),
    [sym_string] = STATE(221),
    [sym_nbt_compound] = STATE(221),
    [sym_nbt_array] = STATE(221),
    [anon_sym_run] = ACTIONS(98),
    [anon_sym_bossbar] = ACTIONS(100),
    [anon_sym_data] = ACTIONS(100),
    [anon_sym_function] = ACTIONS(100),
    [anon_sym_summon] = ACTIONS(100),
    [anon_sym_align] = ACTIONS(100),
    [anon_sym_all] = ACTIONS(100),
    [anon_sym_anchored] = ACTIONS(100),
    [anon_sym_as] = ACTIONS(100),
    [anon_sym_at] = ACTIONS(100),
    [anon_sym_attacker] = ACTIONS(100),
    [anon_sym_biome] = ACTIONS(100),
    [anon_sym_block] = ACTIONS(100),
    [anon_sym_blocks] = ACTIONS(100),
    [anon_sym_controller] = ACTIONS(100),
    [anon_sym_dimension] = ACTIONS(100),
    [anon_sym_entity] = ACTIONS(100),
    [anon_sym_eyes] = ACTIONS(100),
    [anon_sym_facing] = ACTIONS(100),
    [anon_sym_feet] = ACTIONS(100),
    [anon_sym_if] = ACTIONS(100),
    [anon_sym_in] = ACTIONS(100),
    [anon_sym_items] = ACTIONS(100),
    [anon_sym_leasher] = ACTIONS(100),
    [anon_sym_loaded] = ACTIONS(100),
    [anon_sym_masked] = ACTIONS(100),
    [anon_sym_matches] = ACTIONS(100),
    [anon_sym_max] = ACTIONS(100),
    [anon_sym_on] = ACTIONS(100),
    [anon_sym_origin] = ACTIONS(100),
    [anon_sym_over] = ACTIONS(100),
    [anon_sym_owner] = ACTIONS(100),
    [anon_sym_passengers] = ACTIONS(100),
    [anon_sym_positioned] = ACTIONS(100),
    [anon_sym_predicate] = ACTIONS(100),
    [anon_sym_result] = ACTIONS(100),
    [anon_sym_rotated] = ACTIONS(100),
    [anon_sym_score] = ACTIONS(100),
    [anon_sym_storage] = ACTIONS(100),
    [anon_sym_store] = ACTIONS(100),
    [anon_sym_success] = ACTIONS(100),
    [anon_sym_target] = ACTIONS(100),
    [anon_sym_unless] = ACTIONS(100),
    [anon_sym_value] = ACTIONS(100),
    [anon_sym_vehicle] = ACTIONS(100),
    [sym_selector_identifier] = ACTIONS(23),
    [anon_sym_TILDE] = ACTIONS(25),
    [anon_sym_CARET] = ACTIONS(27),
    [sym_heightmap] = ACTIONS(102),
    [sym_resource] = ACTIONS(102),
    [aux_sym_scale_token1] = ACTIONS(29),
    [aux_sym_scale_token2] = ACTIONS(31),
    [aux_sym_scale_token3] = ACTIONS(31),
    [aux_sym_scale_token4] = ACTIONS(31),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [sym_type] = ACTIONS(102),
    [anon_sym_PLUS_EQ] = ACTIONS(33),
    [anon_sym_DASH_EQ] = ACTIONS(33),
    [anon_sym_STAR_EQ] = ACTIONS(33),
    [anon_sym_SLASH_EQ] = ACTIONS(33),
    [anon_sym_PERCENT_EQ] = ACTIONS(33),
    [anon_sym_GT_LT] = ACTIONS(33),
    [anon_sym_LT] = ACTIONS(33),
    [anon_sym_GT] = ACTIONS(35),
    [aux_sym_slot_token1] = ACTIONS(37),
    [aux_sym_slot_token2] = ACTIONS(37),
    [aux_sym_slot_token3] = ACTIONS(37),
    [aux_sym_slot_token4] = ACTIONS(37),
    [aux_sym_slot_token5] = ACTIONS(37),
    [aux_sym_slot_token6] = ACTIONS(37),
    [aux_sym_slot_token7] = ACTIONS(37),
    [anon_sym_contents] = ACTIONS(37),
    [anon_sym_player_DOTcursor] = ACTIONS(37),
    [aux_sym_slot_token8] = ACTIONS(37),
    [aux_sym_word_token1] = ACTIONS(39),
    [anon_sym_DQUOTE] = ACTIONS(41),
    [anon_sym_SQUOTE] = ACTIONS(43),
    [sym_boolean] = ACTIONS(102),
    [anon_sym_LBRACE] = ACTIONS(45),
    [anon_sym_LBRACK] = ACTIONS(47),
  },
  [STATE(16)] = {
    [sym__subcommand] = STATE(220),
    [sym_subcommand_identifier] = STATE(221),
    [sym_selector] = STATE(221),
    [sym_position] = STATE(221),
    [sym_rotation] = STATE(221),
    [sym_scale] = STATE(221),
    [sym_macro] = STATE(16),
    [sym_operator] = STATE(221),
    [sym_slot] = STATE(221),
    [sym_word] = STATE(221),
    [sym_string] = STATE(221),
    [sym_nbt_compound] = STATE(221),
    [sym_nbt_array] = STATE(221),
    [anon_sym_run] = ACTIONS(104),
    [anon_sym_bossbar] = ACTIONS(100),
    [anon_sym_data] = ACTIONS(100),
    [anon_sym_function] = ACTIONS(100),
    [anon_sym_summon] = ACTIONS(100),
    [anon_sym_align] = ACTIONS(100),
    [anon_sym_all] = ACTIONS(100),
    [anon_sym_anchored] = ACTIONS(100),
    [anon_sym_as] = ACTIONS(100),
    [anon_sym_at] = ACTIONS(100),
    [anon_sym_attacker] = ACTIONS(100),
    [anon_sym_biome] = ACTIONS(100),
    [anon_sym_block] = ACTIONS(100),
    [anon_sym_blocks] = ACTIONS(100),
    [anon_sym_controller] = ACTIONS(100),
    [anon_sym_dimension] = ACTIONS(100),
    [anon_sym_entity] = ACTIONS(100),
    [anon_sym_eyes] = ACTIONS(100),
    [anon_sym_facing] = ACTIONS(100),
    [anon_sym_feet] = ACTIONS(100),
    [anon_sym_if] = ACTIONS(100),
    [anon_sym_in] = ACTIONS(100),
    [anon_sym_items] = ACTIONS(100),
    [anon_sym_leasher] = ACTIONS(100),
    [anon_sym_loaded] = ACTIONS(100),
    [anon_sym_masked] = ACTIONS(100),
    [anon_sym_matches] = ACTIONS(100),
    [anon_sym_max] = ACTIONS(100),
    [anon_sym_on] = ACTIONS(100),
    [anon_sym_origin] = ACTIONS(100),
    [anon_sym_over] = ACTIONS(100),
    [anon_sym_owner] = ACTIONS(100),
    [anon_sym_passengers] = ACTIONS(100),
    [anon_sym_positioned] = ACTIONS(100),
    [anon_sym_predicate] = ACTIONS(100),
    [anon_sym_result] = ACTIONS(100),
    [anon_sym_rotated] = ACTIONS(100),
    [anon_sym_score] = ACTIONS(100),
    [anon_sym_storage] = ACTIONS(100),
    [anon_sym_store] = ACTIONS(100),
    [anon_sym_success] = ACTIONS(100),
    [anon_sym_target] = ACTIONS(100),
    [anon_sym_unless] = ACTIONS(100),
    [anon_sym_value] = ACTIONS(100),
    [anon_sym_vehicle] = ACTIONS(100),
    [sym_selector_identifier] = ACTIONS(23),
    [anon_sym_TILDE] = ACTIONS(25),
    [anon_sym_CARET] = ACTIONS(27),
    [sym_heightmap] = ACTIONS(102),
    [sym_resource] = ACTIONS(102),
    [aux_sym_scale_token1] = ACTIONS(29),
    [aux_sym_scale_token2] = ACTIONS(31),
    [aux_sym_scale_token3] = ACTIONS(31),
    [aux_sym_scale_token4] = ACTIONS(31),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [sym_type] = ACTIONS(102),
    [anon_sym_PLUS_EQ] = ACTIONS(33),
    [anon_sym_DASH_EQ] = ACTIONS(33),
    [anon_sym_STAR_EQ] = ACTIONS(33),
    [anon_sym_SLASH_EQ] = ACTIONS(33),
    [anon_sym_PERCENT_EQ] = ACTIONS(33),
    [anon_sym_GT_LT] = ACTIONS(33),
    [anon_sym_LT] = ACTIONS(33),
    [anon_sym_GT] = ACTIONS(35),
    [aux_sym_slot_token1] = ACTIONS(37),
    [aux_sym_slot_token2] = ACTIONS(37),
    [aux_sym_slot_token3] = ACTIONS(37),
    [aux_sym_slot_token4] = ACTIONS(37),
    [aux_sym_slot_token5] = ACTIONS(37),
    [aux_sym_slot_token6] = ACTIONS(37),
    [aux_sym_slot_token7] = ACTIONS(37),
    [anon_sym_contents] = ACTIONS(37),
    [anon_sym_player_DOTcursor] = ACTIONS(37),
    [aux_sym_slot_token8] = ACTIONS(37),
    [aux_sym_word_token1] = ACTIONS(39),
    [anon_sym_DQUOTE] = ACTIONS(41),
    [anon_sym_SQUOTE] = ACTIONS(43),
    [sym_boolean] = ACTIONS(102),
    [anon_sym_LBRACE] = ACTIONS(45),
    [anon_sym_LBRACK] = ACTIONS(47),
  },
  [STATE(17)] = {
    [sym__subcommand] = STATE(220),
    [sym_subcommand_identifier] = STATE(221),
    [sym_selector] = STATE(221),
    [sym_position] = STATE(221),
    [sym_rotation] = STATE(221),
    [sym_scale] = STATE(221),
    [sym_macro] = STATE(17),
    [sym_operator] = STATE(221),
    [sym_slot] = STATE(221),
    [sym_word] = STATE(221),
    [sym_string] = STATE(221),
    [sym_nbt_compound] = STATE(221),
    [sym_nbt_array] = STATE(221),
    [anon_sym_bossbar] = ACTIONS(100),
    [anon_sym_data] = ACTIONS(100),
    [anon_sym_function] = ACTIONS(100),
    [anon_sym_summon] = ACTIONS(100),
    [anon_sym_align] = ACTIONS(100),
    [anon_sym_all] = ACTIONS(100),
    [anon_sym_anchored] = ACTIONS(100),
    [anon_sym_as] = ACTIONS(100),
    [anon_sym_at] = ACTIONS(100),
    [anon_sym_attacker] = ACTIONS(100),
    [anon_sym_biome] = ACTIONS(100),
    [anon_sym_block] = ACTIONS(100),
    [anon_sym_blocks] = ACTIONS(100),
    [anon_sym_controller] = ACTIONS(100),
    [anon_sym_dimension] = ACTIONS(100),
    [anon_sym_entity] = ACTIONS(100),
    [anon_sym_eyes] = ACTIONS(100),
    [anon_sym_facing] = ACTIONS(100),
    [anon_sym_feet] = ACTIONS(100),
    [anon_sym_if] = ACTIONS(100),
    [anon_sym_in] = ACTIONS(100),
    [anon_sym_items] = ACTIONS(100),
    [anon_sym_leasher] = ACTIONS(100),
    [anon_sym_loaded] = ACTIONS(100),
    [anon_sym_masked] = ACTIONS(100),
    [anon_sym_matches] = ACTIONS(100),
    [anon_sym_max] = ACTIONS(100),
    [anon_sym_on] = ACTIONS(100),
    [anon_sym_origin] = ACTIONS(100),
    [anon_sym_over] = ACTIONS(100),
    [anon_sym_owner] = ACTIONS(100),
    [anon_sym_passengers] = ACTIONS(100),
    [anon_sym_positioned] = ACTIONS(100),
    [anon_sym_predicate] = ACTIONS(100),
    [anon_sym_result] = ACTIONS(100),
    [anon_sym_rotated] = ACTIONS(100),
    [anon_sym_score] = ACTIONS(100),
    [anon_sym_storage] = ACTIONS(100),
    [anon_sym_store] = ACTIONS(100),
    [anon_sym_success] = ACTIONS(100),
    [anon_sym_target] = ACTIONS(100),
    [anon_sym_unless] = ACTIONS(100),
    [anon_sym_value] = ACTIONS(100),
    [anon_sym_vehicle] = ACTIONS(100),
    [sym_selector_identifier] = ACTIONS(23),
    [anon_sym_TILDE] = ACTIONS(25),
    [anon_sym_CARET] = ACTIONS(27),
    [sym_heightmap] = ACTIONS(102),
    [sym_resource] = ACTIONS(102),
    [aux_sym_scale_token1] = ACTIONS(29),
    [aux_sym_scale_token2] = ACTIONS(31),
    [aux_sym_scale_token3] = ACTIONS(31),
    [aux_sym_scale_token4] = ACTIONS(31),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [sym_type] = ACTIONS(102),
    [anon_sym_PLUS_EQ] = ACTIONS(33),
    [anon_sym_DASH_EQ] = ACTIONS(33),
    [anon_sym_STAR_EQ] = ACTIONS(33),
    [anon_sym_SLASH_EQ] = ACTIONS(33),
    [anon_sym_PERCENT_EQ] = ACTIONS(33),
    [anon_sym_GT_LT] = ACTIONS(33),
    [anon_sym_LT] = ACTIONS(33),
    [anon_sym_GT] = ACTIONS(35),
    [aux_sym_slot_token1] = ACTIONS(37),
    [aux_sym_slot_token2] = ACTIONS(37),
    [aux_sym_slot_token3] = ACTIONS(37),
    [aux_sym_slot_token4] = ACTIONS(37),
    [aux_sym_slot_token5] = ACTIONS(37),
    [aux_sym_slot_token6] = ACTIONS(37),
    [aux_sym_slot_token7] = ACTIONS(37),
    [anon_sym_contents] = ACTIONS(37),
    [anon_sym_player_DOTcursor] = ACTIONS(37),
    [aux_sym_slot_token8] = ACTIONS(37),
    [aux_sym_word_token1] = ACTIONS(39),
    [anon_sym_DQUOTE] = ACTIONS(41),
    [anon_sym_SQUOTE] = ACTIONS(43),
    [sym_boolean] = ACTIONS(102),
    [anon_sym_LBRACE] = ACTIONS(45),
    [anon_sym_LBRACK] = ACTIONS(47),
  },
  [STATE(18)] = {
    [sym_macro] = STATE(18),
    [aux_sym__space_token1] = ACTIONS(49),
    [sym_backslash] = ACTIONS(51),
    [anon_sym_run] = ACTIONS(49),
    [anon_sym_bossbar] = ACTIONS(49),
    [anon_sym_data] = ACTIONS(49),
    [anon_sym_function] = ACTIONS(49),
    [anon_sym_summon] = ACTIONS(49),
    [anon_sym_align] = ACTIONS(49),
    [anon_sym_all] = ACTIONS(49),
    [anon_sym_anchored] = ACTIONS(49),
    [anon_sym_as] = ACTIONS(49),
    [anon_sym_at] = ACTIONS(49),
    [anon_sym_attacker] = ACTIONS(49),
    [anon_sym_biome] = ACTIONS(49),
    [anon_sym_block] = ACTIONS(49),
    [anon_sym_blocks] = ACTIONS(49),
    [anon_sym_controller] = ACTIONS(49),
    [anon_sym_dimension] = ACTIONS(49),
    [anon_sym_entity] = ACTIONS(49),
    [anon_sym_eyes] = ACTIONS(49),
    [anon_sym_facing] = ACTIONS(49),
    [anon_sym_feet] = ACTIONS(49),
    [anon_sym_if] = ACTIONS(49),
    [anon_sym_in] = ACTIONS(49),
    [anon_sym_items] = ACTIONS(49),
    [anon_sym_leasher] = ACTIONS(49),
    [anon_sym_loaded] = ACTIONS(49),
    [anon_sym_masked] = ACTIONS(49),
    [anon_sym_matches] = ACTIONS(49),
    [anon_sym_max] = ACTIONS(49),
    [anon_sym_on] = ACTIONS(49),
    [anon_sym_origin] = ACTIONS(49),
    [anon_sym_over] = ACTIONS(49),
    [anon_sym_owner] = ACTIONS(49),
    [anon_sym_passengers] = ACTIONS(49),
    [anon_sym_positioned] = ACTIONS(49),
    [anon_sym_predicate] = ACTIONS(49),
    [anon_sym_result] = ACTIONS(49),
    [anon_sym_rotated] = ACTIONS(49),
    [anon_sym_score] = ACTIONS(49),
    [anon_sym_storage] = ACTIONS(49),
    [anon_sym_store] = ACTIONS(49),
    [anon_sym_success] = ACTIONS(49),
    [anon_sym_target] = ACTIONS(49),
    [anon_sym_unless] = ACTIONS(49),
    [anon_sym_value] = ACTIONS(49),
    [anon_sym_vehicle] = ACTIONS(49),
    [sym_selector_identifier] = ACTIONS(51),
    [anon_sym_EQ] = ACTIONS(49),
    [anon_sym_EQ_BANG] = ACTIONS(51),
    [anon_sym_TILDE] = ACTIONS(51),
    [anon_sym_CARET] = ACTIONS(51),
    [sym_heightmap] = ACTIONS(49),
    [sym_resource] = ACTIONS(49),
    [aux_sym_scale_token1] = ACTIONS(49),
    [aux_sym_scale_token2] = ACTIONS(49),
    [aux_sym_scale_token3] = ACTIONS(49),
    [aux_sym_scale_token4] = ACTIONS(49),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [sym_type] = ACTIONS(49),
    [anon_sym_PLUS_EQ] = ACTIONS(51),
    [anon_sym_DASH_EQ] = ACTIONS(51),
    [anon_sym_STAR_EQ] = ACTIONS(51),
    [anon_sym_SLASH_EQ] = ACTIONS(51),
    [anon_sym_PERCENT_EQ] = ACTIONS(51),
    [anon_sym_GT_LT] = ACTIONS(51),
    [anon_sym_LT] = ACTIONS(51),
    [anon_sym_GT] = ACTIONS(49),
    [aux_sym_slot_token1] = ACTIONS(49),
    [aux_sym_slot_token2] = ACTIONS(49),
    [aux_sym_slot_token3] = ACTIONS(49),
    [aux_sym_slot_token4] = ACTIONS(49),
    [aux_sym_slot_token5] = ACTIONS(49),
    [aux_sym_slot_token6] = ACTIONS(49),
    [aux_sym_slot_token7] = ACTIONS(49),
    [anon_sym_contents] = ACTIONS(49),
    [anon_sym_player_DOTcursor] = ACTIONS(49),
    [aux_sym_slot_token8] = ACTIONS(49),
    [aux_sym_word_token1] = ACTIONS(49),
    [anon_sym_DQUOTE] = ACTIONS(51),
    [anon_sym_SQUOTE] = ACTIONS(51),
    [sym_boolean] = ACTIONS(49),
    [anon_sym_LBRACE] = ACTIONS(51),
    [anon_sym_LBRACK] = ACTIONS(51),
  },
  [STATE(19)] = {
    [sym_macro] = STATE(19),
    [anon_sym_bossbar] = ACTIONS(49),
    [anon_sym_data] = ACTIONS(49),
    [anon_sym_function] = ACTIONS(49),
    [anon_sym_summon] = ACTIONS(49),
    [anon_sym_align] = ACTIONS(49),
    [anon_sym_all] = ACTIONS(49),
    [anon_sym_anchored] = ACTIONS(49),
    [anon_sym_as] = ACTIONS(49),
    [anon_sym_at] = ACTIONS(49),
    [anon_sym_attacker] = ACTIONS(49),
    [anon_sym_biome] = ACTIONS(49),
    [anon_sym_block] = ACTIONS(49),
    [anon_sym_blocks] = ACTIONS(49),
    [anon_sym_controller] = ACTIONS(49),
    [anon_sym_dimension] = ACTIONS(49),
    [anon_sym_entity] = ACTIONS(49),
    [anon_sym_eyes] = ACTIONS(49),
    [anon_sym_facing] = ACTIONS(49),
    [anon_sym_feet] = ACTIONS(49),
    [anon_sym_if] = ACTIONS(49),
    [anon_sym_in] = ACTIONS(49),
    [anon_sym_items] = ACTIONS(49),
    [anon_sym_leasher] = ACTIONS(49),
    [anon_sym_loaded] = ACTIONS(49),
    [anon_sym_masked] = ACTIONS(49),
    [anon_sym_matches] = ACTIONS(49),
    [anon_sym_max] = ACTIONS(49),
    [anon_sym_on] = ACTIONS(49),
    [anon_sym_origin] = ACTIONS(49),
    [anon_sym_over] = ACTIONS(49),
    [anon_sym_owner] = ACTIONS(49),
    [anon_sym_passengers] = ACTIONS(49),
    [anon_sym_positioned] = ACTIONS(49),
    [anon_sym_predicate] = ACTIONS(49),
    [anon_sym_result] = ACTIONS(49),
    [anon_sym_rotated] = ACTIONS(49),
    [anon_sym_score] = ACTIONS(49),
    [anon_sym_storage] = ACTIONS(49),
    [anon_sym_store] = ACTIONS(49),
    [anon_sym_success] = ACTIONS(49),
    [anon_sym_target] = ACTIONS(49),
    [anon_sym_unless] = ACTIONS(49),
    [anon_sym_value] = ACTIONS(49),
    [anon_sym_vehicle] = ACTIONS(49),
    [sym_selector_identifier] = ACTIONS(51),
    [anon_sym_TILDE] = ACTIONS(51),
    [anon_sym_CARET] = ACTIONS(51),
    [sym_heightmap] = ACTIONS(49),
    [sym_resource] = ACTIONS(49),
    [aux_sym_scale_token1] = ACTIONS(49),
    [aux_sym_scale_token2] = ACTIONS(49),
    [aux_sym_scale_token3] = ACTIONS(49),
    [aux_sym_scale_token4] = ACTIONS(49),
    [anon_sym_DOLLAR_LPAREN] = ACTIONS(3),
    [sym_type] = ACTIONS(49),
    [anon_sym_PLUS_EQ] = ACTIONS(51),
    [anon_sym_DASH_EQ] = ACTIONS(51),
    [anon_sym_STAR_EQ] = ACTIONS(51),
    [anon_sym_SLASH_EQ] = ACTIONS(51),
    [anon_sym_PERCENT_EQ] = ACTIONS(51),
    [anon_sym_GT_LT] = ACTIONS(51),
    [anon_sym_LT] = ACTIONS(51),
    [anon_sym_GT] = ACTIONS(49),
    [aux_sym_slot_token1] = ACTIONS(49),
    [aux_sym_slot_token2] = ACTIONS(49),
    [aux_sym_slot_token3] = ACTIONS(49),
    [aux_sym_slot_token4] = ACTIONS(49),
    [aux_sym_slot_token5] = ACTIONS(49),
    [aux_sym_slot_token6] = ACTIONS(49),
    [aux_sym_slot_token7] = ACTIONS(49),
    [anon_sym_contents] = ACTIONS(49),
    [anon_sym_player_DOTcursor] = ACTIONS(49),
    [aux_sym_slot_token8] = ACTIONS(49),
    [aux_sym_word_token1] = ACTIONS(49),
    [anon_sym_DQUOTE] = ACTIONS(51),
    [anon_sym_SQUOTE] = ACTIONS(51),
    [sym_boolean] = ACTIONS(49),
    [anon_sym_LBRACE] = ACTIONS(51),
    [anon_sym_LBRACK] = ACTIONS(51),
  },
};

static const uint16_t ts_small_parse_table[] = {
  [0] = 21,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(106), 1,
      aux_sym__space_token1,
    ACTIONS(108), 1,
      sym_backslash,
    ACTIONS(110), 1,
      sym_selector_identifier,
    ACTIONS(112), 1,
      anon_sym_TILDE,
    ACTIONS(114), 1,
      anon_sym_CARET,
    ACTIONS(118), 1,
      aux_sym_scale_token1,
    ACTIONS(124), 1,
      anon_sym_GT,
    ACTIONS(128), 1,
      aux_sym_word_token1,
    ACTIONS(130), 1,
      anon_sym_DQUOTE,
    ACTIONS(132), 1,
      anon_sym_SQUOTE,
    ACTIONS(134), 1,
      anon_sym_LBRACE,
    ACTIONS(136), 1,
      anon_sym_LBRACK,
    STATE(20), 1,
      sym_macro,
    STATE(22), 1,
      aux_sym_selector_query_repeat1,
    STATE(25), 1,
      sym__space,
    ACTIONS(120), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    ACTIONS(116), 4,
      sym_heightmap,
      sym_resource,
      sym_type,
      sym_boolean,
    ACTIONS(122), 7,
      anon_sym_PLUS_EQ,
      anon_sym_DASH_EQ,
      anon_sym_STAR_EQ,
      anon_sym_SLASH_EQ,
      anon_sym_PERCENT_EQ,
      anon_sym_GT_LT,
      anon_sym_LT,
    ACTIONS(126), 10,
      aux_sym_slot_token1,
      aux_sym_slot_token2,
      aux_sym_slot_token3,
      aux_sym_slot_token4,
      aux_sym_slot_token5,
      aux_sym_slot_token6,
      aux_sym_slot_token7,
      anon_sym_contents,
      anon_sym_player_DOTcursor,
      aux_sym_slot_token8,
    STATE(201), 11,
      sym_selector,
      sym_position,
      sym_rotation,
      sym_scale,
      sym_operator,
      sym_slot,
      sym_word,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
      sym_selector_compound,
  [94] = 21,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(106), 1,
      aux_sym__space_token1,
    ACTIONS(108), 1,
      sym_backslash,
    ACTIONS(110), 1,
      sym_selector_identifier,
    ACTIONS(112), 1,
      anon_sym_TILDE,
    ACTIONS(114), 1,
      anon_sym_CARET,
    ACTIONS(118), 1,
      aux_sym_scale_token1,
    ACTIONS(124), 1,
      anon_sym_GT,
    ACTIONS(128), 1,
      aux_sym_word_token1,
    ACTIONS(130), 1,
      anon_sym_DQUOTE,
    ACTIONS(132), 1,
      anon_sym_SQUOTE,
    ACTIONS(134), 1,
      anon_sym_LBRACE,
    ACTIONS(136), 1,
      anon_sym_LBRACK,
    STATE(21), 1,
      sym_macro,
    STATE(24), 1,
      aux_sym_selector_query_repeat1,
    STATE(25), 1,
      sym__space,
    ACTIONS(120), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    ACTIONS(116), 4,
      sym_heightmap,
      sym_resource,
      sym_type,
      sym_boolean,
    ACTIONS(122), 7,
      anon_sym_PLUS_EQ,
      anon_sym_DASH_EQ,
      anon_sym_STAR_EQ,
      anon_sym_SLASH_EQ,
      anon_sym_PERCENT_EQ,
      anon_sym_GT_LT,
      anon_sym_LT,
    ACTIONS(126), 10,
      aux_sym_slot_token1,
      aux_sym_slot_token2,
      aux_sym_slot_token3,
      aux_sym_slot_token4,
      aux_sym_slot_token5,
      aux_sym_slot_token6,
      aux_sym_slot_token7,
      anon_sym_contents,
      anon_sym_player_DOTcursor,
      aux_sym_slot_token8,
    STATE(201), 11,
      sym_selector,
      sym_position,
      sym_rotation,
      sym_scale,
      sym_operator,
      sym_slot,
      sym_word,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
      sym_selector_compound,
  [188] = 21,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(106), 1,
      aux_sym__space_token1,
    ACTIONS(108), 1,
      sym_backslash,
    ACTIONS(110), 1,
      sym_selector_identifier,
    ACTIONS(112), 1,
      anon_sym_TILDE,
    ACTIONS(114), 1,
      anon_sym_CARET,
    ACTIONS(118), 1,
      aux_sym_scale_token1,
    ACTIONS(124), 1,
      anon_sym_GT,
    ACTIONS(128), 1,
      aux_sym_word_token1,
    ACTIONS(130), 1,
      anon_sym_DQUOTE,
    ACTIONS(132), 1,
      anon_sym_SQUOTE,
    ACTIONS(134), 1,
      anon_sym_LBRACE,
    ACTIONS(136), 1,
      anon_sym_LBRACK,
    STATE(22), 1,
      sym_macro,
    STATE(24), 1,
      aux_sym_selector_query_repeat1,
    STATE(25), 1,
      sym__space,
    ACTIONS(120), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    ACTIONS(138), 4,
      sym_heightmap,
      sym_resource,
      sym_type,
      sym_boolean,
    ACTIONS(122), 7,
      anon_sym_PLUS_EQ,
      anon_sym_DASH_EQ,
      anon_sym_STAR_EQ,
      anon_sym_SLASH_EQ,
      anon_sym_PERCENT_EQ,
      anon_sym_GT_LT,
      anon_sym_LT,
    ACTIONS(126), 10,
      aux_sym_slot_token1,
      aux_sym_slot_token2,
      aux_sym_slot_token3,
      aux_sym_slot_token4,
      aux_sym_slot_token5,
      aux_sym_slot_token6,
      aux_sym_slot_token7,
      anon_sym_contents,
      anon_sym_player_DOTcursor,
      aux_sym_slot_token8,
    STATE(200), 11,
      sym_selector,
      sym_position,
      sym_rotation,
      sym_scale,
      sym_operator,
      sym_slot,
      sym_word,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
      sym_selector_compound,
  [282] = 21,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(106), 1,
      aux_sym__space_token1,
    ACTIONS(108), 1,
      sym_backslash,
    ACTIONS(110), 1,
      sym_selector_identifier,
    ACTIONS(112), 1,
      anon_sym_TILDE,
    ACTIONS(114), 1,
      anon_sym_CARET,
    ACTIONS(118), 1,
      aux_sym_scale_token1,
    ACTIONS(124), 1,
      anon_sym_GT,
    ACTIONS(128), 1,
      aux_sym_word_token1,
    ACTIONS(130), 1,
      anon_sym_DQUOTE,
    ACTIONS(132), 1,
      anon_sym_SQUOTE,
    ACTIONS(134), 1,
      anon_sym_LBRACE,
    ACTIONS(136), 1,
      anon_sym_LBRACK,
    STATE(21), 1,
      aux_sym_selector_query_repeat1,
    STATE(23), 1,
      sym_macro,
    STATE(25), 1,
      sym__space,
    ACTIONS(120), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    ACTIONS(140), 4,
      sym_heightmap,
      sym_resource,
      sym_type,
      sym_boolean,
    ACTIONS(122), 7,
      anon_sym_PLUS_EQ,
      anon_sym_DASH_EQ,
      anon_sym_STAR_EQ,
      anon_sym_SLASH_EQ,
      anon_sym_PERCENT_EQ,
      anon_sym_GT_LT,
      anon_sym_LT,
    ACTIONS(126), 10,
      aux_sym_slot_token1,
      aux_sym_slot_token2,
      aux_sym_slot_token3,
      aux_sym_slot_token4,
      aux_sym_slot_token5,
      aux_sym_slot_token6,
      aux_sym_slot_token7,
      anon_sym_contents,
      anon_sym_player_DOTcursor,
      aux_sym_slot_token8,
    STATE(209), 11,
      sym_selector,
      sym_position,
      sym_rotation,
      sym_scale,
      sym_operator,
      sym_slot,
      sym_word,
      sym_string,
      sym_nbt_compound,
      sym_nbt_array,
      sym_selector_compound,
  [376] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(142), 1,
      aux_sym__space_token1,
    ACTIONS(145), 1,
      sym_backslash,
    STATE(25), 1,
      sym__space,
    STATE(24), 2,
      sym_macro,
      aux_sym_selector_query_repeat1,
    ACTIONS(148), 14,
      sym_selector_identifier,
      anon_sym_TILDE,
      anon_sym_CARET,
      anon_sym_PLUS_EQ,
      anon_sym_DASH_EQ,
      anon_sym_STAR_EQ,
      anon_sym_SLASH_EQ,
      anon_sym_PERCENT_EQ,
      anon_sym_GT_LT,
      anon_sym_LT,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
    ACTIONS(150), 20,
      sym_heightmap,
      sym_resource,
      aux_sym_scale_token1,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
      sym_type,
      anon_sym_GT,
      aux_sym_slot_token1,
      aux_sym_slot_token2,
      aux_sym_slot_token3,
      aux_sym_slot_token4,
      aux_sym_slot_token5,
      aux_sym_slot_token6,
      aux_sym_slot_token7,
      anon_sym_contents,
      anon_sym_player_DOTcursor,
      aux_sym_slot_token8,
      aux_sym_word_token1,
      sym_boolean,
  [431] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(25), 1,
      sym_macro,
    ACTIONS(154), 16,
      sym_backslash,
      sym_selector_identifier,
      anon_sym_EQ,
      anon_sym_TILDE,
      anon_sym_CARET,
      anon_sym_PLUS_EQ,
      anon_sym_DASH_EQ,
      anon_sym_STAR_EQ,
      anon_sym_SLASH_EQ,
      anon_sym_PERCENT_EQ,
      anon_sym_GT_LT,
      anon_sym_LT,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
    ACTIONS(152), 21,
      aux_sym__space_token1,
      sym_heightmap,
      sym_resource,
      aux_sym_scale_token1,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
      sym_type,
      anon_sym_GT,
      aux_sym_slot_token1,
      aux_sym_slot_token2,
      aux_sym_slot_token3,
      aux_sym_slot_token4,
      aux_sym_slot_token5,
      aux_sym_slot_token6,
      aux_sym_slot_token7,
      anon_sym_contents,
      anon_sym_player_DOTcursor,
      aux_sym_slot_token8,
      aux_sym_word_token1,
      sym_boolean,
  [479] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(26), 1,
      sym_macro,
    ACTIONS(51), 15,
      sym_backslash,
      sym_selector_identifier,
      anon_sym_TILDE,
      anon_sym_CARET,
      anon_sym_PLUS_EQ,
      anon_sym_DASH_EQ,
      anon_sym_STAR_EQ,
      anon_sym_SLASH_EQ,
      anon_sym_PERCENT_EQ,
      anon_sym_GT_LT,
      anon_sym_LT,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
    ACTIONS(49), 21,
      aux_sym__space_token1,
      sym_heightmap,
      sym_resource,
      aux_sym_scale_token1,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
      sym_type,
      anon_sym_GT,
      aux_sym_slot_token1,
      aux_sym_slot_token2,
      aux_sym_slot_token3,
      aux_sym_slot_token4,
      aux_sym_slot_token5,
      aux_sym_slot_token6,
      aux_sym_slot_token7,
      anon_sym_contents,
      anon_sym_player_DOTcursor,
      aux_sym_slot_token8,
      aux_sym_word_token1,
      sym_boolean,
  [526] = 18,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(156), 1,
      aux_sym__space_token1,
    ACTIONS(158), 1,
      sym_backslash,
    ACTIONS(160), 1,
      aux_sym_selector_query_token1,
    ACTIONS(162), 1,
      aux_sym_scale_token1,
    ACTIONS(166), 1,
      anon_sym_DQUOTE,
    ACTIONS(168), 1,
      anon_sym_SQUOTE,
    ACTIONS(172), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(174), 1,
      anon_sym_LBRACE,
    ACTIONS(176), 1,
      anon_sym_LBRACK,
    ACTIONS(178), 1,
      anon_sym_RBRACK,
    STATE(27), 1,
      sym_macro,
    STATE(31), 1,
      aux_sym_nbt_array_repeat1,
    STATE(49), 1,
      sym_string,
    ACTIONS(170), 2,
      sym_boolean,
      sym_range,
    STATE(64), 2,
      sym__space,
      sym__nbt_element,
    ACTIONS(164), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    STATE(59), 4,
      sym_scale,
      sym_nbt_identifier,
      sym_nbt_compound,
      sym_nbt_array,
  [588] = 18,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(156), 1,
      aux_sym__space_token1,
    ACTIONS(158), 1,
      sym_backslash,
    ACTIONS(160), 1,
      aux_sym_selector_query_token1,
    ACTIONS(162), 1,
      aux_sym_scale_token1,
    ACTIONS(166), 1,
      anon_sym_DQUOTE,
    ACTIONS(168), 1,
      anon_sym_SQUOTE,
    ACTIONS(172), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(174), 1,
      anon_sym_LBRACE,
    ACTIONS(176), 1,
      anon_sym_LBRACK,
    ACTIONS(180), 1,
      anon_sym_RBRACK,
    STATE(28), 1,
      sym_macro,
    STATE(30), 1,
      aux_sym_nbt_array_repeat1,
    STATE(49), 1,
      sym_string,
    ACTIONS(170), 2,
      sym_boolean,
      sym_range,
    STATE(64), 2,
      sym__space,
      sym__nbt_element,
    ACTIONS(164), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    STATE(59), 4,
      sym_scale,
      sym_nbt_identifier,
      sym_nbt_compound,
      sym_nbt_array,
  [650] = 18,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(156), 1,
      aux_sym__space_token1,
    ACTIONS(158), 1,
      sym_backslash,
    ACTIONS(160), 1,
      aux_sym_selector_query_token1,
    ACTIONS(162), 1,
      aux_sym_scale_token1,
    ACTIONS(166), 1,
      anon_sym_DQUOTE,
    ACTIONS(168), 1,
      anon_sym_SQUOTE,
    ACTIONS(172), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(174), 1,
      anon_sym_LBRACE,
    ACTIONS(176), 1,
      anon_sym_LBRACK,
    ACTIONS(182), 1,
      anon_sym_RBRACK,
    STATE(29), 1,
      sym_macro,
    STATE(30), 1,
      aux_sym_nbt_array_repeat1,
    STATE(49), 1,
      sym_string,
    ACTIONS(170), 2,
      sym_boolean,
      sym_range,
    STATE(64), 2,
      sym__space,
      sym__nbt_element,
    ACTIONS(164), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    STATE(59), 4,
      sym_scale,
      sym_nbt_identifier,
      sym_nbt_compound,
      sym_nbt_array,
  [712] = 17,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(184), 1,
      aux_sym__space_token1,
    ACTIONS(187), 1,
      sym_backslash,
    ACTIONS(190), 1,
      aux_sym_selector_query_token1,
    ACTIONS(193), 1,
      aux_sym_scale_token1,
    ACTIONS(199), 1,
      anon_sym_DQUOTE,
    ACTIONS(202), 1,
      anon_sym_SQUOTE,
    ACTIONS(208), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(211), 1,
      anon_sym_LBRACE,
    ACTIONS(214), 1,
      anon_sym_LBRACK,
    ACTIONS(217), 1,
      anon_sym_RBRACK,
    STATE(49), 1,
      sym_string,
    ACTIONS(205), 2,
      sym_boolean,
      sym_range,
    STATE(30), 2,
      sym_macro,
      aux_sym_nbt_array_repeat1,
    STATE(64), 2,
      sym__space,
      sym__nbt_element,
    ACTIONS(196), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    STATE(59), 4,
      sym_scale,
      sym_nbt_identifier,
      sym_nbt_compound,
      sym_nbt_array,
  [772] = 18,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(156), 1,
      aux_sym__space_token1,
    ACTIONS(158), 1,
      sym_backslash,
    ACTIONS(160), 1,
      aux_sym_selector_query_token1,
    ACTIONS(162), 1,
      aux_sym_scale_token1,
    ACTIONS(166), 1,
      anon_sym_DQUOTE,
    ACTIONS(168), 1,
      anon_sym_SQUOTE,
    ACTIONS(172), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(174), 1,
      anon_sym_LBRACE,
    ACTIONS(176), 1,
      anon_sym_LBRACK,
    ACTIONS(219), 1,
      anon_sym_RBRACK,
    STATE(30), 1,
      aux_sym_nbt_array_repeat1,
    STATE(31), 1,
      sym_macro,
    STATE(49), 1,
      sym_string,
    ACTIONS(170), 2,
      sym_boolean,
      sym_range,
    STATE(64), 2,
      sym__space,
      sym__nbt_element,
    ACTIONS(164), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    STATE(59), 4,
      sym_scale,
      sym_nbt_identifier,
      sym_nbt_compound,
      sym_nbt_array,
  [834] = 18,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(156), 1,
      aux_sym__space_token1,
    ACTIONS(158), 1,
      sym_backslash,
    ACTIONS(160), 1,
      aux_sym_selector_query_token1,
    ACTIONS(162), 1,
      aux_sym_scale_token1,
    ACTIONS(166), 1,
      anon_sym_DQUOTE,
    ACTIONS(168), 1,
      anon_sym_SQUOTE,
    ACTIONS(172), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(174), 1,
      anon_sym_LBRACE,
    ACTIONS(176), 1,
      anon_sym_LBRACK,
    ACTIONS(221), 1,
      anon_sym_RBRACK,
    STATE(32), 1,
      sym_macro,
    STATE(35), 1,
      aux_sym_nbt_array_repeat1,
    STATE(49), 1,
      sym_string,
    ACTIONS(170), 2,
      sym_boolean,
      sym_range,
    STATE(64), 2,
      sym__space,
      sym__nbt_element,
    ACTIONS(164), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    STATE(59), 4,
      sym_scale,
      sym_nbt_identifier,
      sym_nbt_compound,
      sym_nbt_array,
  [896] = 18,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(156), 1,
      aux_sym__space_token1,
    ACTIONS(158), 1,
      sym_backslash,
    ACTIONS(160), 1,
      aux_sym_selector_query_token1,
    ACTIONS(162), 1,
      aux_sym_scale_token1,
    ACTIONS(166), 1,
      anon_sym_DQUOTE,
    ACTIONS(168), 1,
      anon_sym_SQUOTE,
    ACTIONS(172), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(174), 1,
      anon_sym_LBRACE,
    ACTIONS(176), 1,
      anon_sym_LBRACK,
    ACTIONS(223), 1,
      anon_sym_RBRACK,
    STATE(33), 1,
      sym_macro,
    STATE(36), 1,
      aux_sym_nbt_array_repeat1,
    STATE(49), 1,
      sym_string,
    ACTIONS(170), 2,
      sym_boolean,
      sym_range,
    STATE(64), 2,
      sym__space,
      sym__nbt_element,
    ACTIONS(164), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    STATE(59), 4,
      sym_scale,
      sym_nbt_identifier,
      sym_nbt_compound,
      sym_nbt_array,
  [958] = 18,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(156), 1,
      aux_sym__space_token1,
    ACTIONS(158), 1,
      sym_backslash,
    ACTIONS(160), 1,
      aux_sym_selector_query_token1,
    ACTIONS(162), 1,
      aux_sym_scale_token1,
    ACTIONS(166), 1,
      anon_sym_DQUOTE,
    ACTIONS(168), 1,
      anon_sym_SQUOTE,
    ACTIONS(172), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(174), 1,
      anon_sym_LBRACE,
    ACTIONS(176), 1,
      anon_sym_LBRACK,
    ACTIONS(225), 1,
      anon_sym_RBRACK,
    STATE(29), 1,
      aux_sym_nbt_array_repeat1,
    STATE(34), 1,
      sym_macro,
    STATE(49), 1,
      sym_string,
    ACTIONS(170), 2,
      sym_boolean,
      sym_range,
    STATE(64), 2,
      sym__space,
      sym__nbt_element,
    ACTIONS(164), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    STATE(59), 4,
      sym_scale,
      sym_nbt_identifier,
      sym_nbt_compound,
      sym_nbt_array,
  [1020] = 18,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(156), 1,
      aux_sym__space_token1,
    ACTIONS(158), 1,
      sym_backslash,
    ACTIONS(160), 1,
      aux_sym_selector_query_token1,
    ACTIONS(162), 1,
      aux_sym_scale_token1,
    ACTIONS(166), 1,
      anon_sym_DQUOTE,
    ACTIONS(168), 1,
      anon_sym_SQUOTE,
    ACTIONS(172), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(174), 1,
      anon_sym_LBRACE,
    ACTIONS(176), 1,
      anon_sym_LBRACK,
    ACTIONS(227), 1,
      anon_sym_RBRACK,
    STATE(30), 1,
      aux_sym_nbt_array_repeat1,
    STATE(35), 1,
      sym_macro,
    STATE(49), 1,
      sym_string,
    ACTIONS(170), 2,
      sym_boolean,
      sym_range,
    STATE(64), 2,
      sym__space,
      sym__nbt_element,
    ACTIONS(164), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    STATE(59), 4,
      sym_scale,
      sym_nbt_identifier,
      sym_nbt_compound,
      sym_nbt_array,
  [1082] = 18,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(156), 1,
      aux_sym__space_token1,
    ACTIONS(158), 1,
      sym_backslash,
    ACTIONS(160), 1,
      aux_sym_selector_query_token1,
    ACTIONS(162), 1,
      aux_sym_scale_token1,
    ACTIONS(166), 1,
      anon_sym_DQUOTE,
    ACTIONS(168), 1,
      anon_sym_SQUOTE,
    ACTIONS(172), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(174), 1,
      anon_sym_LBRACE,
    ACTIONS(176), 1,
      anon_sym_LBRACK,
    ACTIONS(229), 1,
      anon_sym_RBRACK,
    STATE(30), 1,
      aux_sym_nbt_array_repeat1,
    STATE(36), 1,
      sym_macro,
    STATE(49), 1,
      sym_string,
    ACTIONS(170), 2,
      sym_boolean,
      sym_range,
    STATE(64), 2,
      sym__space,
      sym__nbt_element,
    ACTIONS(164), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    STATE(59), 4,
      sym_scale,
      sym_nbt_identifier,
      sym_nbt_compound,
      sym_nbt_array,
  [1144] = 18,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(156), 1,
      aux_sym__space_token1,
    ACTIONS(158), 1,
      sym_backslash,
    ACTIONS(160), 1,
      aux_sym_selector_query_token1,
    ACTIONS(162), 1,
      aux_sym_scale_token1,
    ACTIONS(166), 1,
      anon_sym_DQUOTE,
    ACTIONS(168), 1,
      anon_sym_SQUOTE,
    ACTIONS(172), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(174), 1,
      anon_sym_LBRACE,
    ACTIONS(176), 1,
      anon_sym_LBRACK,
    ACTIONS(231), 1,
      anon_sym_RBRACK,
    STATE(28), 1,
      aux_sym_nbt_array_repeat1,
    STATE(37), 1,
      sym_macro,
    STATE(49), 1,
      sym_string,
    ACTIONS(170), 2,
      sym_boolean,
      sym_range,
    STATE(64), 2,
      sym__space,
      sym__nbt_element,
    ACTIONS(164), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    STATE(59), 4,
      sym_scale,
      sym_nbt_identifier,
      sym_nbt_compound,
      sym_nbt_array,
  [1206] = 17,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(233), 1,
      aux_sym__space_token1,
    ACTIONS(235), 1,
      sym_backslash,
    ACTIONS(237), 1,
      aux_sym_scale_token1,
    ACTIONS(241), 1,
      anon_sym_DQUOTE,
    ACTIONS(243), 1,
      anon_sym_SQUOTE,
    ACTIONS(247), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(249), 1,
      anon_sym_LBRACE,
    ACTIONS(251), 1,
      anon_sym_LBRACK,
    STATE(38), 1,
      sym_macro,
    STATE(47), 1,
      sym__space,
    STATE(61), 1,
      aux_sym_selector_query_repeat1,
    STATE(146), 1,
      sym_string,
    STATE(148), 1,
      sym__nbt_element,
    ACTIONS(245), 2,
      sym_boolean,
      sym_range,
    ACTIONS(239), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    STATE(134), 4,
      sym_scale,
      sym_nbt_identifier,
      sym_nbt_compound,
      sym_nbt_array,
  [1264] = 17,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(233), 1,
      aux_sym__space_token1,
    ACTIONS(235), 1,
      sym_backslash,
    ACTIONS(237), 1,
      aux_sym_scale_token1,
    ACTIONS(241), 1,
      anon_sym_DQUOTE,
    ACTIONS(243), 1,
      anon_sym_SQUOTE,
    ACTIONS(247), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(249), 1,
      anon_sym_LBRACE,
    ACTIONS(251), 1,
      anon_sym_LBRACK,
    STATE(39), 1,
      sym_macro,
    STATE(47), 1,
      sym__space,
    STATE(61), 1,
      aux_sym_selector_query_repeat1,
    STATE(146), 1,
      sym_string,
    STATE(153), 1,
      sym__nbt_element,
    ACTIONS(245), 2,
      sym_boolean,
      sym_range,
    ACTIONS(239), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    STATE(134), 4,
      sym_scale,
      sym_nbt_identifier,
      sym_nbt_compound,
      sym_nbt_array,
  [1322] = 17,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(233), 1,
      aux_sym__space_token1,
    ACTIONS(235), 1,
      sym_backslash,
    ACTIONS(237), 1,
      aux_sym_scale_token1,
    ACTIONS(241), 1,
      anon_sym_DQUOTE,
    ACTIONS(243), 1,
      anon_sym_SQUOTE,
    ACTIONS(247), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(249), 1,
      anon_sym_LBRACE,
    ACTIONS(251), 1,
      anon_sym_LBRACK,
    STATE(39), 1,
      aux_sym_selector_query_repeat1,
    STATE(40), 1,
      sym_macro,
    STATE(47), 1,
      sym__space,
    STATE(146), 1,
      sym_string,
    STATE(148), 1,
      sym__nbt_element,
    ACTIONS(245), 2,
      sym_boolean,
      sym_range,
    ACTIONS(239), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    STATE(134), 4,
      sym_scale,
      sym_nbt_identifier,
      sym_nbt_compound,
      sym_nbt_array,
  [1380] = 17,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(233), 1,
      aux_sym__space_token1,
    ACTIONS(235), 1,
      sym_backslash,
    ACTIONS(253), 1,
      aux_sym_scale_token1,
    ACTIONS(257), 1,
      anon_sym_DQUOTE,
    ACTIONS(259), 1,
      anon_sym_SQUOTE,
    ACTIONS(263), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(265), 1,
      anon_sym_LBRACE,
    ACTIONS(267), 1,
      anon_sym_LBRACK,
    STATE(41), 1,
      sym_macro,
    STATE(42), 1,
      aux_sym_selector_query_repeat1,
    STATE(47), 1,
      sym__space,
    STATE(70), 1,
      sym_string,
    STATE(121), 1,
      sym__nbt_element,
    ACTIONS(261), 2,
      sym_boolean,
      sym_range,
    ACTIONS(255), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    STATE(112), 4,
      sym_scale,
      sym_nbt_identifier,
      sym_nbt_compound,
      sym_nbt_array,
  [1438] = 17,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(233), 1,
      aux_sym__space_token1,
    ACTIONS(235), 1,
      sym_backslash,
    ACTIONS(253), 1,
      aux_sym_scale_token1,
    ACTIONS(257), 1,
      anon_sym_DQUOTE,
    ACTIONS(259), 1,
      anon_sym_SQUOTE,
    ACTIONS(263), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(265), 1,
      anon_sym_LBRACE,
    ACTIONS(267), 1,
      anon_sym_LBRACK,
    STATE(42), 1,
      sym_macro,
    STATE(47), 1,
      sym__space,
    STATE(61), 1,
      aux_sym_selector_query_repeat1,
    STATE(70), 1,
      sym_string,
    STATE(123), 1,
      sym__nbt_element,
    ACTIONS(261), 2,
      sym_boolean,
      sym_range,
    ACTIONS(255), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    STATE(112), 4,
      sym_scale,
      sym_nbt_identifier,
      sym_nbt_compound,
      sym_nbt_array,
  [1496] = 17,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(233), 1,
      aux_sym__space_token1,
    ACTIONS(235), 1,
      sym_backslash,
    ACTIONS(253), 1,
      aux_sym_scale_token1,
    ACTIONS(257), 1,
      anon_sym_DQUOTE,
    ACTIONS(259), 1,
      anon_sym_SQUOTE,
    ACTIONS(263), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(265), 1,
      anon_sym_LBRACE,
    ACTIONS(267), 1,
      anon_sym_LBRACK,
    STATE(43), 1,
      sym_macro,
    STATE(44), 1,
      aux_sym_selector_query_repeat1,
    STATE(47), 1,
      sym__space,
    STATE(70), 1,
      sym_string,
    STATE(123), 1,
      sym__nbt_element,
    ACTIONS(261), 2,
      sym_boolean,
      sym_range,
    ACTIONS(255), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    STATE(112), 4,
      sym_scale,
      sym_nbt_identifier,
      sym_nbt_compound,
      sym_nbt_array,
  [1554] = 17,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(233), 1,
      aux_sym__space_token1,
    ACTIONS(235), 1,
      sym_backslash,
    ACTIONS(253), 1,
      aux_sym_scale_token1,
    ACTIONS(257), 1,
      anon_sym_DQUOTE,
    ACTIONS(259), 1,
      anon_sym_SQUOTE,
    ACTIONS(263), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(265), 1,
      anon_sym_LBRACE,
    ACTIONS(267), 1,
      anon_sym_LBRACK,
    STATE(44), 1,
      sym_macro,
    STATE(47), 1,
      sym__space,
    STATE(61), 1,
      aux_sym_selector_query_repeat1,
    STATE(70), 1,
      sym_string,
    STATE(125), 1,
      sym__nbt_element,
    ACTIONS(261), 2,
      sym_boolean,
      sym_range,
    ACTIONS(255), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    STATE(112), 4,
      sym_scale,
      sym_nbt_identifier,
      sym_nbt_compound,
      sym_nbt_array,
  [1612] = 17,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(233), 1,
      aux_sym__space_token1,
    ACTIONS(235), 1,
      sym_backslash,
    ACTIONS(237), 1,
      aux_sym_scale_token1,
    ACTIONS(241), 1,
      anon_sym_DQUOTE,
    ACTIONS(243), 1,
      anon_sym_SQUOTE,
    ACTIONS(247), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(249), 1,
      anon_sym_LBRACE,
    ACTIONS(251), 1,
      anon_sym_LBRACK,
    STATE(38), 1,
      aux_sym_selector_query_repeat1,
    STATE(45), 1,
      sym_macro,
    STATE(47), 1,
      sym__space,
    STATE(132), 1,
      sym__nbt_element,
    STATE(146), 1,
      sym_string,
    ACTIONS(245), 2,
      sym_boolean,
      sym_range,
    ACTIONS(239), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    STATE(134), 4,
      sym_scale,
      sym_nbt_identifier,
      sym_nbt_compound,
      sym_nbt_array,
  [1670] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(273), 1,
      aux_sym_scale_token5,
    STATE(46), 1,
      sym_macro,
    ACTIONS(271), 7,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
    ACTIONS(269), 8,
      aux_sym__space_token1,
      aux_sym_scale_token1,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
      sym_boolean,
      sym_range,
      aux_sym_nbt_identifier_token1,
  [1699] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(47), 1,
      sym_macro,
    ACTIONS(154), 7,
      sym_backslash,
      anon_sym_EQ_BANG,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_LBRACE,
      anon_sym_COLON,
      anon_sym_LBRACK,
    ACTIONS(152), 9,
      aux_sym__space_token1,
      anon_sym_EQ,
      aux_sym_scale_token1,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
      sym_boolean,
      sym_range,
      aux_sym_nbt_identifier_token1,
  [1726] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(279), 1,
      aux_sym_scale_token5,
    STATE(48), 1,
      sym_macro,
    ACTIONS(277), 7,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
    ACTIONS(275), 8,
      aux_sym__space_token1,
      aux_sym_scale_token1,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
      sym_boolean,
      sym_range,
      aux_sym_nbt_identifier_token1,
  [1755] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(49), 1,
      sym_macro,
    ACTIONS(283), 7,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
    ACTIONS(281), 8,
      aux_sym__space_token1,
      aux_sym_scale_token1,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
      sym_boolean,
      sym_range,
      aux_sym_nbt_identifier_token1,
  [1781] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(50), 1,
      sym_macro,
    ACTIONS(287), 7,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
    ACTIONS(285), 8,
      aux_sym__space_token1,
      aux_sym_scale_token1,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
      sym_boolean,
      sym_range,
      aux_sym_nbt_identifier_token1,
  [1807] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(51), 1,
      sym_macro,
    ACTIONS(277), 7,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
    ACTIONS(275), 8,
      aux_sym__space_token1,
      aux_sym_scale_token1,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
      sym_boolean,
      sym_range,
      aux_sym_nbt_identifier_token1,
  [1833] = 13,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(253), 1,
      aux_sym_scale_token1,
    ACTIONS(265), 1,
      anon_sym_LBRACE,
    ACTIONS(267), 1,
      anon_sym_LBRACK,
    ACTIONS(289), 1,
      aux_sym__space_token1,
    ACTIONS(291), 1,
      sym_backslash,
    ACTIONS(293), 1,
      aux_sym_scale_token4,
    STATE(52), 1,
      sym_macro,
    STATE(63), 1,
      aux_sym_selector_query_repeat1,
    STATE(66), 1,
      sym__space,
    ACTIONS(255), 2,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
    ACTIONS(295), 2,
      sym_boolean,
      sym_range,
    STATE(114), 3,
      sym_scale,
      sym_nbt_compound,
      sym_nbt_array,
  [1877] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(53), 1,
      sym_macro,
    ACTIONS(299), 7,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
    ACTIONS(297), 8,
      aux_sym__space_token1,
      aux_sym_scale_token1,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
      sym_boolean,
      sym_range,
      aux_sym_nbt_identifier_token1,
  [1903] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(54), 1,
      sym_macro,
    ACTIONS(303), 7,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
    ACTIONS(301), 8,
      aux_sym__space_token1,
      aux_sym_scale_token1,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
      sym_boolean,
      sym_range,
      aux_sym_nbt_identifier_token1,
  [1929] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(55), 1,
      sym_macro,
    ACTIONS(307), 7,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
    ACTIONS(305), 8,
      aux_sym__space_token1,
      aux_sym_scale_token1,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
      sym_boolean,
      sym_range,
      aux_sym_nbt_identifier_token1,
  [1955] = 13,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(253), 1,
      aux_sym_scale_token1,
    ACTIONS(265), 1,
      anon_sym_LBRACE,
    ACTIONS(267), 1,
      anon_sym_LBRACK,
    ACTIONS(289), 1,
      aux_sym__space_token1,
    ACTIONS(291), 1,
      sym_backslash,
    ACTIONS(293), 1,
      aux_sym_scale_token4,
    STATE(56), 1,
      sym_macro,
    STATE(66), 1,
      sym__space,
    STATE(67), 1,
      aux_sym_selector_query_repeat1,
    ACTIONS(255), 2,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
    ACTIONS(309), 2,
      sym_boolean,
      sym_range,
    STATE(120), 3,
      sym_scale,
      sym_nbt_compound,
      sym_nbt_array,
  [1999] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(57), 1,
      sym_macro,
    ACTIONS(313), 7,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
    ACTIONS(311), 8,
      aux_sym__space_token1,
      aux_sym_scale_token1,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
      sym_boolean,
      sym_range,
      aux_sym_nbt_identifier_token1,
  [2025] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(58), 1,
      sym_macro,
    ACTIONS(317), 7,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
    ACTIONS(315), 8,
      aux_sym__space_token1,
      aux_sym_scale_token1,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
      sym_boolean,
      sym_range,
      aux_sym_nbt_identifier_token1,
  [2051] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(59), 1,
      sym_macro,
    ACTIONS(321), 7,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
    ACTIONS(319), 8,
      aux_sym__space_token1,
      aux_sym_scale_token1,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
      sym_boolean,
      sym_range,
      aux_sym_nbt_identifier_token1,
  [2077] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(60), 1,
      sym_macro,
    ACTIONS(51), 7,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
    ACTIONS(49), 8,
      aux_sym__space_token1,
      aux_sym_scale_token1,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
      sym_boolean,
      sym_range,
      aux_sym_nbt_identifier_token1,
  [2103] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(323), 1,
      aux_sym__space_token1,
    ACTIONS(326), 1,
      sym_backslash,
    STATE(47), 1,
      sym__space,
    STATE(61), 2,
      sym_macro,
      aux_sym_selector_query_repeat1,
    ACTIONS(148), 4,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
    ACTIONS(150), 7,
      aux_sym_scale_token1,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
      sym_boolean,
      sym_range,
      aux_sym_nbt_identifier_token1,
  [2135] = 13,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(253), 1,
      aux_sym_scale_token1,
    ACTIONS(265), 1,
      anon_sym_LBRACE,
    ACTIONS(267), 1,
      anon_sym_LBRACK,
    ACTIONS(289), 1,
      aux_sym__space_token1,
    ACTIONS(291), 1,
      sym_backslash,
    ACTIONS(293), 1,
      aux_sym_scale_token4,
    STATE(56), 1,
      aux_sym_selector_query_repeat1,
    STATE(62), 1,
      sym_macro,
    STATE(66), 1,
      sym__space,
    ACTIONS(255), 2,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
    ACTIONS(329), 2,
      sym_boolean,
      sym_range,
    STATE(116), 3,
      sym_scale,
      sym_nbt_compound,
      sym_nbt_array,
  [2179] = 13,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(253), 1,
      aux_sym_scale_token1,
    ACTIONS(265), 1,
      anon_sym_LBRACE,
    ACTIONS(267), 1,
      anon_sym_LBRACK,
    ACTIONS(289), 1,
      aux_sym__space_token1,
    ACTIONS(291), 1,
      sym_backslash,
    ACTIONS(293), 1,
      aux_sym_scale_token4,
    STATE(63), 1,
      sym_macro,
    STATE(66), 1,
      sym__space,
    STATE(67), 1,
      aux_sym_selector_query_repeat1,
    ACTIONS(255), 2,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
    ACTIONS(329), 2,
      sym_boolean,
      sym_range,
    STATE(116), 3,
      sym_scale,
      sym_nbt_compound,
      sym_nbt_array,
  [2223] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(64), 1,
      sym_macro,
    ACTIONS(333), 7,
      sym_backslash,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
      anon_sym_RBRACK,
    ACTIONS(331), 8,
      aux_sym__space_token1,
      aux_sym_scale_token1,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
      sym_boolean,
      sym_range,
      aux_sym_nbt_identifier_token1,
  [2249] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(65), 1,
      sym_macro,
    ACTIONS(51), 5,
      sym_backslash,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
    ACTIONS(49), 8,
      aux_sym__space_token1,
      aux_sym_scale_token1,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
      sym_boolean,
      sym_range,
      aux_sym_nbt_identifier_token1,
  [2273] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(66), 1,
      sym_macro,
    ACTIONS(152), 4,
      aux_sym__space_token1,
      aux_sym_scale_token1,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
    ACTIONS(154), 8,
      sym_backslash,
      anon_sym_EQ,
      aux_sym_scale_token4,
      sym_boolean,
      sym_range,
      anon_sym_LBRACE,
      anon_sym_COLON,
      anon_sym_LBRACK,
  [2296] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(335), 1,
      aux_sym__space_token1,
    ACTIONS(338), 1,
      sym_backslash,
    STATE(66), 1,
      sym__space,
    STATE(67), 2,
      sym_macro,
      aux_sym_selector_query_repeat1,
    ACTIONS(150), 3,
      aux_sym_scale_token1,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
    ACTIONS(148), 5,
      aux_sym_scale_token4,
      sym_boolean,
      sym_range,
      anon_sym_LBRACE,
      anon_sym_LBRACK,
  [2325] = 9,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(341), 1,
      aux_sym__space_token1,
    ACTIONS(343), 1,
      sym_backslash,
    ACTIONS(345), 1,
      anon_sym_EQ,
    ACTIONS(349), 1,
      aux_sym_nbt_identifier_token1,
    STATE(25), 1,
      sym__space,
    STATE(68), 1,
      sym_macro,
    STATE(129), 1,
      aux_sym_selector_query_repeat1,
    ACTIONS(347), 5,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACK,
  [2357] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(69), 1,
      sym_macro,
    ACTIONS(301), 2,
      aux_sym__space_token1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(303), 8,
      sym_backslash,
      anon_sym_EQ,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
      anon_sym_RBRACK,
  [2378] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(70), 1,
      sym_macro,
    ACTIONS(281), 2,
      aux_sym__space_token1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(283), 8,
      sym_backslash,
      anon_sym_EQ,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
      anon_sym_RBRACK,
  [2399] = 12,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(351), 1,
      aux_sym_selector_query_token1,
    ACTIONS(353), 1,
      sym_resource,
    ACTIONS(355), 1,
      anon_sym_DQUOTE,
    ACTIONS(357), 1,
      anon_sym_SQUOTE,
    ACTIONS(359), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(361), 1,
      anon_sym_RBRACE,
    STATE(71), 1,
      sym_macro,
    STATE(80), 1,
      aux_sym_selector_compound_repeat1,
    STATE(97), 1,
      aux_sym_nbt_compound_repeat1,
    STATE(113), 1,
      sym_nbt_identifier,
    STATE(157), 1,
      sym_string,
  [2436] = 11,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(41), 1,
      anon_sym_DQUOTE,
    ACTIONS(43), 1,
      anon_sym_SQUOTE,
    ACTIONS(365), 1,
      sym_backslash,
    ACTIONS(367), 1,
      aux_sym_word_token1,
    ACTIONS(369), 1,
      anon_sym_LBRACK,
    STATE(72), 1,
      sym_macro,
    STATE(88), 1,
      sym_item_compound,
    STATE(135), 1,
      sym_string,
    STATE(187), 1,
      sym_word,
    ACTIONS(363), 2,
      sym__newline,
      aux_sym__space_token1,
  [2471] = 11,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(128), 1,
      aux_sym_word_token1,
    ACTIONS(130), 1,
      anon_sym_DQUOTE,
    ACTIONS(132), 1,
      anon_sym_SQUOTE,
    ACTIONS(363), 1,
      sym_query_identifier,
    ACTIONS(371), 1,
      anon_sym_LBRACK,
    STATE(73), 1,
      sym_macro,
    STATE(87), 1,
      sym_item_compound,
    STATE(133), 1,
      sym_string,
    STATE(182), 1,
      sym_word,
    ACTIONS(365), 2,
      aux_sym_selector_token2,
      aux_sym_selector_query_token1,
  [2506] = 11,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(257), 1,
      anon_sym_DQUOTE,
    ACTIONS(259), 1,
      anon_sym_SQUOTE,
    ACTIONS(263), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(373), 1,
      aux_sym_selector_query_token1,
    ACTIONS(375), 1,
      sym_resource,
    ACTIONS(377), 1,
      anon_sym_RBRACK,
    STATE(68), 1,
      sym_nbt_identifier,
    STATE(70), 1,
      sym_string,
    STATE(74), 1,
      sym_macro,
    STATE(75), 1,
      aux_sym_item_compound_repeat1,
  [2540] = 10,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(379), 1,
      aux_sym_selector_query_token1,
    ACTIONS(382), 1,
      sym_resource,
    ACTIONS(385), 1,
      anon_sym_DQUOTE,
    ACTIONS(388), 1,
      anon_sym_SQUOTE,
    ACTIONS(391), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(394), 1,
      anon_sym_RBRACK,
    STATE(68), 1,
      sym_nbt_identifier,
    STATE(70), 1,
      sym_string,
    STATE(75), 2,
      sym_macro,
      aux_sym_item_compound_repeat1,
  [2572] = 10,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(396), 1,
      aux_sym_selector_query_token1,
    ACTIONS(399), 1,
      sym_resource,
    ACTIONS(402), 1,
      anon_sym_DQUOTE,
    ACTIONS(405), 1,
      anon_sym_SQUOTE,
    ACTIONS(408), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(411), 1,
      anon_sym_RBRACE,
    STATE(149), 1,
      sym_nbt_identifier,
    STATE(157), 1,
      sym_string,
    STATE(76), 2,
      sym_macro,
      aux_sym_selector_compound_repeat1,
  [2604] = 11,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(257), 1,
      anon_sym_DQUOTE,
    ACTIONS(259), 1,
      anon_sym_SQUOTE,
    ACTIONS(263), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(373), 1,
      aux_sym_selector_query_token1,
    ACTIONS(375), 1,
      sym_resource,
    ACTIONS(413), 1,
      anon_sym_RBRACK,
    STATE(68), 1,
      sym_nbt_identifier,
    STATE(70), 1,
      sym_string,
    STATE(74), 1,
      aux_sym_item_compound_repeat1,
    STATE(77), 1,
      sym_macro,
  [2638] = 11,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(257), 1,
      anon_sym_DQUOTE,
    ACTIONS(259), 1,
      anon_sym_SQUOTE,
    ACTIONS(263), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(373), 1,
      aux_sym_selector_query_token1,
    ACTIONS(375), 1,
      sym_resource,
    ACTIONS(415), 1,
      anon_sym_RBRACK,
    STATE(68), 1,
      sym_nbt_identifier,
    STATE(70), 1,
      sym_string,
    STATE(78), 1,
      sym_macro,
    STATE(79), 1,
      aux_sym_item_compound_repeat1,
  [2672] = 11,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(257), 1,
      anon_sym_DQUOTE,
    ACTIONS(259), 1,
      anon_sym_SQUOTE,
    ACTIONS(263), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(373), 1,
      aux_sym_selector_query_token1,
    ACTIONS(375), 1,
      sym_resource,
    ACTIONS(417), 1,
      anon_sym_RBRACK,
    STATE(68), 1,
      sym_nbt_identifier,
    STATE(70), 1,
      sym_string,
    STATE(75), 1,
      aux_sym_item_compound_repeat1,
    STATE(79), 1,
      sym_macro,
  [2706] = 11,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(41), 1,
      anon_sym_DQUOTE,
    ACTIONS(43), 1,
      anon_sym_SQUOTE,
    ACTIONS(353), 1,
      sym_resource,
    ACTIONS(359), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(419), 1,
      aux_sym_selector_query_token1,
    ACTIONS(421), 1,
      anon_sym_RBRACE,
    STATE(76), 1,
      aux_sym_selector_compound_repeat1,
    STATE(80), 1,
      sym_macro,
    STATE(149), 1,
      sym_nbt_identifier,
    STATE(157), 1,
      sym_string,
  [2740] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(118), 1,
      aux_sym_scale_token1,
    STATE(81), 1,
      sym_macro,
    STATE(179), 1,
      sym_scale,
    ACTIONS(120), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    ACTIONS(423), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [2763] = 9,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(425), 1,
      aux_sym_selector_query_token1,
    ACTIONS(428), 1,
      anon_sym_DQUOTE,
    ACTIONS(431), 1,
      anon_sym_SQUOTE,
    ACTIONS(434), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(437), 1,
      anon_sym_RBRACE,
    STATE(140), 1,
      sym_nbt_identifier,
    STATE(157), 1,
      sym_string,
    STATE(82), 2,
      sym_macro,
      aux_sym_nbt_compound_repeat1,
  [2792] = 8,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(31), 1,
      aux_sym_scale_token2,
    ACTIONS(441), 1,
      sym_backslash,
    ACTIONS(443), 1,
      aux_sym_scale_token1,
    STATE(83), 1,
      sym_macro,
    STATE(193), 1,
      sym_scale,
    ACTIONS(439), 2,
      sym__newline,
      aux_sym__space_token1,
    ACTIONS(445), 2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
  [2819] = 8,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(31), 1,
      aux_sym_scale_token2,
    ACTIONS(423), 1,
      sym_backslash,
    ACTIONS(443), 1,
      aux_sym_scale_token1,
    STATE(84), 1,
      sym_macro,
    STATE(176), 1,
      sym_scale,
    ACTIONS(445), 2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    ACTIONS(447), 2,
      sym__newline,
      aux_sym__space_token1,
  [2846] = 10,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(355), 1,
      anon_sym_DQUOTE,
    ACTIONS(357), 1,
      anon_sym_SQUOTE,
    ACTIONS(449), 1,
      aux_sym_selector_query_token1,
    ACTIONS(451), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(453), 1,
      anon_sym_RBRACE,
    STATE(85), 1,
      sym_macro,
    STATE(89), 1,
      aux_sym_nbt_compound_repeat1,
    STATE(140), 1,
      sym_nbt_identifier,
    STATE(157), 1,
      sym_string,
  [2877] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(118), 1,
      aux_sym_scale_token1,
    STATE(86), 1,
      sym_macro,
    STATE(178), 1,
      sym_scale,
    ACTIONS(120), 3,
      aux_sym_scale_token2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
    ACTIONS(441), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [2900] = 9,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(128), 1,
      aux_sym_word_token1,
    ACTIONS(130), 1,
      anon_sym_DQUOTE,
    ACTIONS(132), 1,
      anon_sym_SQUOTE,
    ACTIONS(457), 1,
      sym_query_identifier,
    STATE(87), 1,
      sym_macro,
    STATE(137), 1,
      sym_string,
    STATE(198), 1,
      sym_word,
    ACTIONS(455), 2,
      aux_sym_selector_token2,
      aux_sym_selector_query_token1,
  [2929] = 9,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(41), 1,
      anon_sym_DQUOTE,
    ACTIONS(43), 1,
      anon_sym_SQUOTE,
    ACTIONS(367), 1,
      aux_sym_word_token1,
    ACTIONS(455), 1,
      sym_backslash,
    STATE(88), 1,
      sym_macro,
    STATE(154), 1,
      sym_string,
    STATE(211), 1,
      sym_word,
    ACTIONS(457), 2,
      sym__newline,
      aux_sym__space_token1,
  [2958] = 10,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(355), 1,
      anon_sym_DQUOTE,
    ACTIONS(357), 1,
      anon_sym_SQUOTE,
    ACTIONS(449), 1,
      aux_sym_selector_query_token1,
    ACTIONS(451), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(459), 1,
      anon_sym_RBRACE,
    STATE(82), 1,
      aux_sym_nbt_compound_repeat1,
    STATE(89), 1,
      sym_macro,
    STATE(140), 1,
      sym_nbt_identifier,
    STATE(157), 1,
      sym_string,
  [2989] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(269), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(461), 1,
      aux_sym_scale_token5,
    STATE(90), 1,
      sym_macro,
    ACTIONS(271), 6,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
      anon_sym_RBRACK,
  [3010] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(275), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(463), 1,
      aux_sym_scale_token5,
    STATE(91), 1,
      sym_macro,
    ACTIONS(277), 6,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
      anon_sym_RBRACK,
  [3031] = 10,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(355), 1,
      anon_sym_DQUOTE,
    ACTIONS(357), 1,
      anon_sym_SQUOTE,
    ACTIONS(449), 1,
      aux_sym_selector_query_token1,
    ACTIONS(451), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(465), 1,
      anon_sym_RBRACE,
    STATE(92), 1,
      sym_macro,
    STATE(93), 1,
      aux_sym_nbt_compound_repeat1,
    STATE(140), 1,
      sym_nbt_identifier,
    STATE(157), 1,
      sym_string,
  [3062] = 10,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(355), 1,
      anon_sym_DQUOTE,
    ACTIONS(357), 1,
      anon_sym_SQUOTE,
    ACTIONS(449), 1,
      aux_sym_selector_query_token1,
    ACTIONS(451), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(467), 1,
      anon_sym_RBRACE,
    STATE(82), 1,
      aux_sym_nbt_compound_repeat1,
    STATE(93), 1,
      sym_macro,
    STATE(140), 1,
      sym_nbt_identifier,
    STATE(157), 1,
      sym_string,
  [3093] = 10,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(355), 1,
      anon_sym_DQUOTE,
    ACTIONS(357), 1,
      anon_sym_SQUOTE,
    ACTIONS(449), 1,
      aux_sym_selector_query_token1,
    ACTIONS(451), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(469), 1,
      anon_sym_RBRACE,
    STATE(94), 1,
      sym_macro,
    STATE(95), 1,
      aux_sym_nbt_compound_repeat1,
    STATE(140), 1,
      sym_nbt_identifier,
    STATE(157), 1,
      sym_string,
  [3124] = 10,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(355), 1,
      anon_sym_DQUOTE,
    ACTIONS(357), 1,
      anon_sym_SQUOTE,
    ACTIONS(449), 1,
      aux_sym_selector_query_token1,
    ACTIONS(451), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(471), 1,
      anon_sym_RBRACE,
    STATE(82), 1,
      aux_sym_nbt_compound_repeat1,
    STATE(95), 1,
      sym_macro,
    STATE(140), 1,
      sym_nbt_identifier,
    STATE(157), 1,
      sym_string,
  [3155] = 10,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(355), 1,
      anon_sym_DQUOTE,
    ACTIONS(357), 1,
      anon_sym_SQUOTE,
    ACTIONS(449), 1,
      aux_sym_selector_query_token1,
    ACTIONS(451), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(473), 1,
      anon_sym_RBRACE,
    STATE(96), 1,
      sym_macro,
    STATE(98), 1,
      aux_sym_nbt_compound_repeat1,
    STATE(140), 1,
      sym_nbt_identifier,
    STATE(157), 1,
      sym_string,
  [3186] = 10,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(355), 1,
      anon_sym_DQUOTE,
    ACTIONS(357), 1,
      anon_sym_SQUOTE,
    ACTIONS(449), 1,
      aux_sym_selector_query_token1,
    ACTIONS(451), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(475), 1,
      anon_sym_RBRACE,
    STATE(82), 1,
      aux_sym_nbt_compound_repeat1,
    STATE(97), 1,
      sym_macro,
    STATE(140), 1,
      sym_nbt_identifier,
    STATE(157), 1,
      sym_string,
  [3217] = 10,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(355), 1,
      anon_sym_DQUOTE,
    ACTIONS(357), 1,
      anon_sym_SQUOTE,
    ACTIONS(449), 1,
      aux_sym_selector_query_token1,
    ACTIONS(451), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(477), 1,
      anon_sym_RBRACE,
    STATE(82), 1,
      aux_sym_nbt_compound_repeat1,
    STATE(98), 1,
      sym_macro,
    STATE(140), 1,
      sym_nbt_identifier,
    STATE(157), 1,
      sym_string,
  [3248] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(297), 1,
      aux_sym_nbt_identifier_token1,
    STATE(99), 1,
      sym_macro,
    ACTIONS(299), 6,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
      anon_sym_RBRACK,
  [3266] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(275), 1,
      aux_sym_nbt_identifier_token1,
    STATE(100), 1,
      sym_macro,
    ACTIONS(277), 6,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
      anon_sym_RBRACK,
  [3284] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(285), 1,
      aux_sym_nbt_identifier_token1,
    STATE(101), 1,
      sym_macro,
    ACTIONS(287), 6,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
      anon_sym_RBRACK,
  [3302] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(305), 1,
      aux_sym_nbt_identifier_token1,
    STATE(102), 1,
      sym_macro,
    ACTIONS(307), 6,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
      anon_sym_RBRACK,
  [3320] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(315), 1,
      aux_sym_nbt_identifier_token1,
    STATE(103), 1,
      sym_macro,
    ACTIONS(317), 6,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
      anon_sym_RBRACK,
  [3338] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(311), 1,
      aux_sym_nbt_identifier_token1,
    STATE(104), 1,
      sym_macro,
    ACTIONS(313), 6,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
      anon_sym_RBRACK,
  [3356] = 8,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(479), 1,
      aux_sym__space_token1,
    ACTIONS(481), 1,
      sym_backslash,
    ACTIONS(483), 1,
      anon_sym_EQ,
    ACTIONS(485), 1,
      anon_sym_EQ_BANG,
    STATE(47), 1,
      sym__space,
    STATE(105), 1,
      sym_macro,
    STATE(109), 1,
      aux_sym_selector_query_repeat1,
  [3381] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(490), 1,
      sym_resource,
    ACTIONS(492), 1,
      aux_sym_nbt_identifier_token1,
    STATE(106), 1,
      sym_macro,
    ACTIONS(487), 4,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
  [3400] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(107), 1,
      sym_macro,
    ACTIONS(497), 2,
      sym_query_identifier,
      aux_sym_word_token1,
    ACTIONS(495), 4,
      aux_sym_selector_token2,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
  [3417] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(108), 1,
      sym_macro,
    ACTIONS(501), 2,
      sym_query_identifier,
      aux_sym_word_token1,
    ACTIONS(499), 4,
      aux_sym_selector_token2,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
  [3434] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(148), 1,
      anon_sym_EQ_BANG,
    ACTIONS(150), 1,
      anon_sym_EQ,
    ACTIONS(503), 1,
      aux_sym__space_token1,
    ACTIONS(506), 1,
      sym_backslash,
    STATE(47), 1,
      sym__space,
    STATE(109), 2,
      sym_macro,
      aux_sym_selector_query_repeat1,
  [3457] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(269), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(509), 1,
      aux_sym_scale_token5,
    STATE(110), 1,
      sym_macro,
    ACTIONS(271), 4,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
  [3476] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(275), 1,
      aux_sym_nbt_identifier_token1,
    ACTIONS(511), 1,
      aux_sym_scale_token5,
    STATE(111), 1,
      sym_macro,
    ACTIONS(277), 4,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
  [3495] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(319), 1,
      aux_sym_nbt_identifier_token1,
    STATE(112), 1,
      sym_macro,
    ACTIONS(321), 5,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
  [3512] = 8,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(289), 1,
      aux_sym__space_token1,
    ACTIONS(291), 1,
      sym_backslash,
    ACTIONS(513), 1,
      anon_sym_EQ,
    ACTIONS(515), 1,
      anon_sym_COLON,
    STATE(66), 1,
      sym__space,
    STATE(113), 1,
      sym_macro,
    STATE(117), 1,
      aux_sym_selector_query_repeat1,
  [3537] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(519), 1,
      aux_sym_nbt_identifier_token1,
    STATE(114), 1,
      sym_macro,
    ACTIONS(517), 5,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACK,
  [3554] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(115), 1,
      sym_macro,
    ACTIONS(501), 2,
      sym__newline,
      aux_sym__space_token1,
    ACTIONS(499), 4,
      sym_backslash,
      aux_sym_word_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
  [3571] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(523), 1,
      aux_sym_nbt_identifier_token1,
    STATE(116), 1,
      sym_macro,
    ACTIONS(521), 5,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACK,
  [3588] = 8,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(289), 1,
      aux_sym__space_token1,
    ACTIONS(291), 1,
      sym_backslash,
    ACTIONS(525), 1,
      anon_sym_EQ,
    ACTIONS(527), 1,
      anon_sym_COLON,
    STATE(66), 1,
      sym__space,
    STATE(117), 1,
      sym_macro,
    STATE(126), 1,
      aux_sym_selector_query_repeat1,
  [3613] = 8,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(479), 1,
      aux_sym__space_token1,
    ACTIONS(481), 1,
      sym_backslash,
    ACTIONS(529), 1,
      anon_sym_EQ,
    ACTIONS(531), 1,
      anon_sym_EQ_BANG,
    STATE(47), 1,
      sym__space,
    STATE(105), 1,
      aux_sym_selector_query_repeat1,
    STATE(118), 1,
      sym_macro,
  [3638] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(533), 1,
      aux_sym_nbt_identifier_token1,
    STATE(119), 1,
      sym_macro,
    ACTIONS(490), 5,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
  [3655] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(537), 1,
      aux_sym_nbt_identifier_token1,
    STATE(120), 1,
      sym_macro,
    ACTIONS(535), 5,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACK,
  [3672] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(541), 1,
      aux_sym_nbt_identifier_token1,
    STATE(121), 1,
      sym_macro,
    ACTIONS(539), 5,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
  [3689] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(349), 1,
      aux_sym_nbt_identifier_token1,
    STATE(122), 1,
      sym_macro,
    ACTIONS(347), 5,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACK,
  [3706] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(545), 1,
      aux_sym_nbt_identifier_token1,
    STATE(123), 1,
      sym_macro,
    ACTIONS(543), 5,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
  [3723] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(124), 1,
      sym_macro,
    ACTIONS(497), 2,
      sym__newline,
      aux_sym__space_token1,
    ACTIONS(495), 4,
      sym_backslash,
      aux_sym_word_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
  [3740] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(549), 1,
      aux_sym_nbt_identifier_token1,
    STATE(125), 1,
      sym_macro,
    ACTIONS(547), 5,
      aux_sym_selector_query_token1,
      sym_resource,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      anon_sym_RBRACE,
  [3757] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(335), 1,
      aux_sym__space_token1,
    ACTIONS(338), 1,
      sym_backslash,
    STATE(66), 1,
      sym__space,
    ACTIONS(148), 2,
      anon_sym_EQ,
      anon_sym_COLON,
    STATE(126), 2,
      sym_macro,
      aux_sym_selector_query_repeat1,
  [3778] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(127), 1,
      sym_macro,
    ACTIONS(287), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [3792] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(341), 1,
      aux_sym__space_token1,
    ACTIONS(343), 1,
      sym_backslash,
    ACTIONS(525), 1,
      anon_sym_EQ,
    STATE(25), 1,
      sym__space,
    STATE(128), 1,
      sym_macro,
    STATE(151), 1,
      aux_sym_selector_query_repeat1,
  [3814] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(341), 1,
      aux_sym__space_token1,
    ACTIONS(343), 1,
      sym_backslash,
    ACTIONS(551), 1,
      anon_sym_EQ,
    STATE(25), 1,
      sym__space,
    STATE(129), 1,
      sym_macro,
    STATE(151), 1,
      aux_sym_selector_query_repeat1,
  [3836] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(341), 1,
      aux_sym__space_token1,
    ACTIONS(343), 1,
      sym_backslash,
    ACTIONS(553), 1,
      sym__newline,
    STATE(2), 1,
      sym__space,
    STATE(130), 1,
      sym_macro,
    STATE(152), 1,
      aux_sym__generic_command_repeat1,
  [3858] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(555), 1,
      sym__newline,
    ACTIONS(557), 1,
      aux_sym__space_token1,
    ACTIONS(560), 1,
      sym_backslash,
    STATE(2), 1,
      sym__space,
    STATE(131), 2,
      sym_macro,
      aux_sym__generic_command_repeat1,
  [3878] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(132), 1,
      sym_macro,
    ACTIONS(563), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [3892] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(128), 1,
      aux_sym_word_token1,
    ACTIONS(457), 1,
      sym_query_identifier,
    STATE(133), 1,
      sym_macro,
    STATE(198), 1,
      sym_word,
    ACTIONS(455), 2,
      aux_sym_selector_token2,
      aux_sym_selector_query_token1,
  [3912] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(134), 1,
      sym_macro,
    ACTIONS(321), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [3926] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(367), 1,
      aux_sym_word_token1,
    ACTIONS(455), 1,
      sym_backslash,
    STATE(135), 1,
      sym_macro,
    STATE(211), 1,
      sym_word,
    ACTIONS(457), 2,
      sym__newline,
      aux_sym__space_token1,
  [3946] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(148), 1,
      anon_sym_COLON,
    ACTIONS(335), 1,
      aux_sym__space_token1,
    ACTIONS(338), 1,
      sym_backslash,
    STATE(47), 1,
      sym__space,
    STATE(136), 2,
      sym_macro,
      aux_sym_selector_query_repeat1,
  [3966] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(128), 1,
      aux_sym_word_token1,
    ACTIONS(567), 1,
      sym_query_identifier,
    STATE(137), 1,
      sym_macro,
    STATE(208), 1,
      sym_word,
    ACTIONS(565), 2,
      aux_sym_selector_token2,
      aux_sym_selector_query_token1,
  [3986] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(138), 1,
      sym_macro,
    ACTIONS(569), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [4000] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(139), 1,
      sym_macro,
    ACTIONS(277), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [4014] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(289), 1,
      aux_sym__space_token1,
    ACTIONS(291), 1,
      sym_backslash,
    ACTIONS(515), 1,
      anon_sym_COLON,
    STATE(47), 1,
      sym__space,
    STATE(140), 1,
      sym_macro,
    STATE(150), 1,
      aux_sym_selector_query_repeat1,
  [4036] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(141), 1,
      sym_macro,
    ACTIONS(299), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [4050] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(142), 1,
      sym_macro,
    ACTIONS(301), 2,
      sym__newline,
      aux_sym__space_token1,
    ACTIONS(303), 3,
      sym_backslash,
      anon_sym_EQ,
      aux_sym_word_token1,
  [4066] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(143), 1,
      sym_macro,
    ACTIONS(307), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [4080] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(144), 1,
      sym_macro,
    ACTIONS(317), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [4094] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(145), 1,
      sym_macro,
    ACTIONS(313), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [4108] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(146), 1,
      sym_macro,
    ACTIONS(283), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [4122] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(147), 1,
      sym_macro,
    ACTIONS(303), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [4136] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(148), 1,
      sym_macro,
    ACTIONS(571), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [4150] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(341), 1,
      aux_sym__space_token1,
    ACTIONS(343), 1,
      sym_backslash,
    ACTIONS(513), 1,
      anon_sym_EQ,
    STATE(25), 1,
      sym__space,
    STATE(128), 1,
      aux_sym_selector_query_repeat1,
    STATE(149), 1,
      sym_macro,
  [4172] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(289), 1,
      aux_sym__space_token1,
    ACTIONS(291), 1,
      sym_backslash,
    ACTIONS(527), 1,
      anon_sym_COLON,
    STATE(47), 1,
      sym__space,
    STATE(136), 1,
      aux_sym_selector_query_repeat1,
    STATE(150), 1,
      sym_macro,
  [4194] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(148), 1,
      anon_sym_EQ,
    ACTIONS(573), 1,
      aux_sym__space_token1,
    ACTIONS(576), 1,
      sym_backslash,
    STATE(25), 1,
      sym__space,
    STATE(151), 2,
      sym_macro,
      aux_sym_selector_query_repeat1,
  [4214] = 7,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(341), 1,
      aux_sym__space_token1,
    ACTIONS(343), 1,
      sym_backslash,
    ACTIONS(579), 1,
      sym__newline,
    STATE(2), 1,
      sym__space,
    STATE(131), 1,
      aux_sym__generic_command_repeat1,
    STATE(152), 1,
      sym_macro,
  [4236] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(153), 1,
      sym_macro,
    ACTIONS(581), 5,
      aux_sym_selector_query_token1,
      anon_sym_DQUOTE,
      anon_sym_SQUOTE,
      aux_sym_nbt_identifier_token1,
      anon_sym_RBRACE,
  [4250] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(367), 1,
      aux_sym_word_token1,
    ACTIONS(565), 1,
      sym_backslash,
    STATE(154), 1,
      sym_macro,
    STATE(215), 1,
      sym_word,
    ACTIONS(567), 2,
      sym__newline,
      aux_sym__space_token1,
  [4270] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(583), 1,
      aux_sym__space_token1,
    ACTIONS(586), 1,
      sym_backslash,
    STATE(17), 1,
      sym__space,
    STATE(155), 2,
      sym_macro,
      aux_sym__execute_command_repeat1,
  [4287] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(479), 1,
      aux_sym__space_token1,
    ACTIONS(481), 1,
      sym_backslash,
    STATE(15), 1,
      sym__space,
    STATE(156), 1,
      sym_macro,
    STATE(167), 1,
      aux_sym__execute_command_repeat1,
  [4306] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(281), 1,
      aux_sym__space_token1,
    STATE(157), 1,
      sym_macro,
    ACTIONS(283), 3,
      sym_backslash,
      anon_sym_EQ,
      anon_sym_COLON,
  [4321] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(271), 1,
      sym_backslash,
    ACTIONS(589), 1,
      aux_sym_scale_token5,
    STATE(158), 1,
      sym_macro,
    ACTIONS(269), 2,
      sym__newline,
      aux_sym__space_token1,
  [4338] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(159), 1,
      sym_macro,
    ACTIONS(301), 2,
      sym_query_identifier,
      aux_sym_word_token1,
    ACTIONS(303), 2,
      aux_sym_selector_token2,
      aux_sym_selector_query_token1,
  [4353] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(591), 1,
      aux_sym_selector_token1,
    STATE(160), 1,
      sym_macro,
    ACTIONS(593), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4368] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(301), 1,
      aux_sym__space_token1,
    STATE(161), 1,
      sym_macro,
    ACTIONS(303), 3,
      sym_backslash,
      anon_sym_EQ,
      anon_sym_COLON,
  [4383] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(593), 1,
      sym_backslash,
    ACTIONS(597), 1,
      aux_sym_selector_token1,
    STATE(162), 1,
      sym_macro,
    ACTIONS(595), 2,
      sym__newline,
      aux_sym__space_token1,
  [4400] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(599), 1,
      aux_sym_selector_token2,
    ACTIONS(601), 1,
      sym_query_identifier,
    STATE(163), 1,
      sym_macro,
    STATE(165), 1,
      aux_sym_selector_repeat1,
    STATE(222), 1,
      sym_selector_query,
  [4419] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(601), 1,
      sym_query_identifier,
    ACTIONS(603), 1,
      aux_sym_selector_token2,
    STATE(164), 1,
      sym_macro,
    STATE(166), 1,
      aux_sym_selector_repeat1,
    STATE(222), 1,
      sym_selector_query,
  [4438] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(601), 1,
      sym_query_identifier,
    ACTIONS(605), 1,
      aux_sym_selector_token2,
    STATE(165), 1,
      sym_macro,
    STATE(171), 1,
      aux_sym_selector_repeat1,
    STATE(222), 1,
      sym_selector_query,
  [4457] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(601), 1,
      sym_query_identifier,
    ACTIONS(607), 1,
      aux_sym_selector_token2,
    STATE(166), 1,
      sym_macro,
    STATE(171), 1,
      aux_sym_selector_repeat1,
    STATE(222), 1,
      sym_selector_query,
  [4476] = 6,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(479), 1,
      aux_sym__space_token1,
    ACTIONS(481), 1,
      sym_backslash,
    STATE(16), 1,
      sym__space,
    STATE(155), 1,
      aux_sym__execute_command_repeat1,
    STATE(167), 1,
      sym_macro,
  [4495] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(609), 1,
      aux_sym_scale_token5,
    STATE(168), 1,
      sym_macro,
    ACTIONS(271), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4510] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(611), 1,
      aux_sym_scale_token5,
    STATE(169), 1,
      sym_macro,
    ACTIONS(277), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4525] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(277), 1,
      sym_backslash,
    ACTIONS(613), 1,
      aux_sym_scale_token5,
    STATE(170), 1,
      sym_macro,
    ACTIONS(275), 2,
      sym__newline,
      aux_sym__space_token1,
  [4542] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(615), 1,
      aux_sym_selector_token2,
    ACTIONS(617), 1,
      sym_query_identifier,
    STATE(222), 1,
      sym_selector_query,
    STATE(171), 2,
      sym_macro,
      aux_sym_selector_repeat1,
  [4559] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(620), 1,
      sym_backslash,
    STATE(172), 1,
      sym_macro,
    ACTIONS(555), 2,
      sym__newline,
      aux_sym__space_token1,
  [4573] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(624), 1,
      sym_backslash,
    STATE(173), 1,
      sym_macro,
    ACTIONS(622), 2,
      sym__newline,
      aux_sym__space_token1,
  [4587] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(174), 1,
      sym_macro,
    ACTIONS(624), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4599] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(175), 1,
      sym_macro,
    ACTIONS(626), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4611] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(630), 1,
      sym_backslash,
    STATE(176), 1,
      sym_macro,
    ACTIONS(628), 2,
      sym__newline,
      aux_sym__space_token1,
  [4625] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(626), 1,
      sym_backslash,
    STATE(177), 1,
      sym_macro,
    ACTIONS(632), 2,
      sym__newline,
      aux_sym__space_token1,
  [4639] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(178), 1,
      sym_macro,
    ACTIONS(634), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4651] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(179), 1,
      sym_macro,
    ACTIONS(630), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4663] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(289), 1,
      aux_sym__space_token1,
    ACTIONS(291), 1,
      sym_backslash,
    STATE(12), 1,
      sym__space,
    STATE(180), 1,
      sym_macro,
  [4679] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(277), 1,
      sym_backslash,
    STATE(181), 1,
      sym_macro,
    ACTIONS(275), 2,
      sym__newline,
      aux_sym__space_token1,
  [4693] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(182), 1,
      sym_macro,
    ACTIONS(455), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4705] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(287), 1,
      sym_backslash,
    STATE(183), 1,
      sym_macro,
    ACTIONS(285), 2,
      sym__newline,
      aux_sym__space_token1,
  [4719] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(313), 1,
      sym_backslash,
    STATE(184), 1,
      sym_macro,
    ACTIONS(311), 2,
      sym__newline,
      aux_sym__space_token1,
  [4733] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(185), 1,
      sym_macro,
    ACTIONS(277), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4745] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(638), 1,
      sym_backslash,
    STATE(186), 1,
      sym_macro,
    ACTIONS(636), 2,
      sym__newline,
      aux_sym__space_token1,
  [4759] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(455), 1,
      sym_backslash,
    STATE(187), 1,
      sym_macro,
    ACTIONS(457), 2,
      sym__newline,
      aux_sym__space_token1,
  [4773] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(188), 1,
      sym_macro,
    ACTIONS(287), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4785] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(189), 1,
      sym_macro,
    ACTIONS(307), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4797] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(190), 1,
      sym_macro,
    ACTIONS(317), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4809] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(191), 1,
      sym_macro,
    ACTIONS(313), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4821] = 5,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(289), 1,
      aux_sym__space_token1,
    ACTIONS(291), 1,
      sym_backslash,
    STATE(13), 1,
      sym__space,
    STATE(192), 1,
      sym_macro,
  [4837] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(634), 1,
      sym_backslash,
    STATE(193), 1,
      sym_macro,
    ACTIONS(640), 2,
      sym__newline,
      aux_sym__space_token1,
  [4851] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(194), 1,
      sym_macro,
    ACTIONS(642), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4863] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(644), 1,
      aux_sym_scale_token2,
    STATE(195), 1,
      sym_macro,
    ACTIONS(646), 2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
  [4877] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(196), 1,
      sym_macro,
    ACTIONS(648), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4889] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(651), 1,
      aux_sym_scale_token2,
    STATE(197), 1,
      sym_macro,
    ACTIONS(653), 2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
  [4903] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(198), 1,
      sym_macro,
    ACTIONS(565), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [4915] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(642), 1,
      sym_backslash,
    STATE(199), 1,
      sym_macro,
    ACTIONS(655), 2,
      sym__newline,
      aux_sym__space_token1,
  [4929] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(659), 1,
      aux_sym_selector_query_token1,
    STATE(200), 1,
      sym_macro,
    ACTIONS(657), 2,
      aux_sym_selector_token2,
      sym_query_identifier,
  [4943] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(663), 1,
      aux_sym_selector_query_token1,
    STATE(201), 1,
      sym_macro,
    ACTIONS(661), 2,
      aux_sym_selector_token2,
      sym_query_identifier,
  [4957] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(667), 1,
      sym_backslash,
    STATE(202), 1,
      sym_macro,
    ACTIONS(665), 2,
      sym__newline,
      aux_sym__space_token1,
  [4971] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(669), 1,
      aux_sym_scale_token2,
    STATE(203), 1,
      sym_macro,
    ACTIONS(671), 2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
  [4985] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(307), 1,
      sym_backslash,
    STATE(204), 1,
      sym_macro,
    ACTIONS(305), 2,
      sym__newline,
      aux_sym__space_token1,
  [4999] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(675), 1,
      sym_backslash,
    STATE(205), 1,
      sym_macro,
    ACTIONS(673), 2,
      sym__newline,
      aux_sym__space_token1,
  [5013] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(206), 1,
      sym_macro,
    ACTIONS(675), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [5025] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(677), 1,
      aux_sym_scale_token2,
    STATE(207), 1,
      sym_macro,
    ACTIONS(679), 2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
  [5039] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(208), 1,
      sym_macro,
    ACTIONS(681), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [5051] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(685), 1,
      aux_sym_selector_query_token1,
    STATE(209), 1,
      sym_macro,
    ACTIONS(683), 2,
      aux_sym_selector_token2,
      sym_query_identifier,
  [5065] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(687), 1,
      aux_sym_scale_token2,
    STATE(210), 1,
      sym_macro,
    ACTIONS(689), 2,
      aux_sym_scale_token3,
      aux_sym_scale_token4,
  [5079] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(565), 1,
      sym_backslash,
    STATE(211), 1,
      sym_macro,
    ACTIONS(567), 2,
      sym__newline,
      aux_sym__space_token1,
  [5093] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(317), 1,
      sym_backslash,
    STATE(212), 1,
      sym_macro,
    ACTIONS(315), 2,
      sym__newline,
      aux_sym__space_token1,
  [5107] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(299), 1,
      sym_backslash,
    STATE(213), 1,
      sym_macro,
    ACTIONS(297), 2,
      sym__newline,
      aux_sym__space_token1,
  [5121] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(214), 1,
      sym_macro,
    ACTIONS(691), 3,
      aux_sym_selector_token2,
      sym_query_identifier,
      aux_sym_selector_query_token1,
  [5133] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(681), 1,
      sym_backslash,
    STATE(215), 1,
      sym_macro,
    ACTIONS(693), 2,
      sym__newline,
      aux_sym__space_token1,
  [5147] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(216), 1,
      sym_macro,
    ACTIONS(661), 2,
      aux_sym_selector_token2,
      sym_query_identifier,
  [5158] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(695), 1,
      aux_sym__space_token1,
    ACTIONS(697), 1,
      sym_backslash,
    STATE(217), 1,
      sym_macro,
  [5171] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(218), 1,
      sym_macro,
    ACTIONS(699), 2,
      aux_sym_selector_token2,
      sym_query_identifier,
  [5182] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(219), 1,
      sym_macro,
    ACTIONS(657), 2,
      aux_sym_selector_token2,
      sym_query_identifier,
  [5193] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(701), 1,
      aux_sym__space_token1,
    ACTIONS(703), 1,
      sym_backslash,
    STATE(220), 1,
      sym_macro,
  [5206] = 4,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(705), 1,
      aux_sym__space_token1,
    ACTIONS(707), 1,
      sym_backslash,
    STATE(221), 1,
      sym_macro,
  [5219] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    STATE(222), 1,
      sym_macro,
    ACTIONS(709), 2,
      aux_sym_selector_token2,
      sym_query_identifier,
  [5230] = 3,
    ACTIONS(711), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(713), 1,
      aux_sym_string_token2,
    STATE(223), 1,
      sym_macro,
  [5240] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(715), 1,
      sym__newline,
    STATE(224), 1,
      sym_macro,
  [5250] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(717), 1,
      anon_sym_DQUOTE,
    STATE(225), 1,
      sym_macro,
  [5260] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(717), 1,
      anon_sym_SQUOTE,
    STATE(226), 1,
      sym_macro,
  [5270] = 3,
    ACTIONS(711), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(719), 1,
      aux_sym_string_token2,
    STATE(227), 1,
      sym_macro,
  [5280] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(721), 1,
      ts_builtin_sym_end,
    STATE(228), 1,
      sym_macro,
  [5290] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(723), 1,
      anon_sym_DQUOTE,
    STATE(229), 1,
      sym_macro,
  [5300] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(725), 1,
      anon_sym_DQUOTE,
    STATE(230), 1,
      sym_macro,
  [5310] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(727), 1,
      sym__newline,
    STATE(231), 1,
      sym_macro,
  [5320] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(729), 1,
      aux_sym_macro_token1,
    STATE(232), 1,
      sym_macro,
  [5330] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(731), 1,
      anon_sym_DQUOTE,
    STATE(233), 1,
      sym_macro,
  [5340] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(731), 1,
      anon_sym_SQUOTE,
    STATE(234), 1,
      sym_macro,
  [5350] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(725), 1,
      anon_sym_SQUOTE,
    STATE(235), 1,
      sym_macro,
  [5360] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(733), 1,
      sym__newline,
    STATE(236), 1,
      sym_macro,
  [5370] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(78), 1,
      sym__newline,
    STATE(237), 1,
      sym_macro,
  [5380] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(735), 1,
      sym__newline,
    STATE(238), 1,
      sym_macro,
  [5390] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(737), 1,
      anon_sym_RPAREN,
    STATE(239), 1,
      sym_macro,
  [5400] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(739), 1,
      anon_sym_DQUOTE,
    STATE(240), 1,
      sym_macro,
  [5410] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(739), 1,
      anon_sym_SQUOTE,
    STATE(241), 1,
      sym_macro,
  [5420] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(741), 1,
      sym__newline,
    STATE(242), 1,
      sym_macro,
  [5430] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(743), 1,
      anon_sym_DQUOTE,
    STATE(243), 1,
      sym_macro,
  [5440] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(743), 1,
      anon_sym_SQUOTE,
    STATE(244), 1,
      sym_macro,
  [5450] = 3,
    ACTIONS(711), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(745), 1,
      aux_sym_string_token1,
    STATE(245), 1,
      sym_macro,
  [5460] = 3,
    ACTIONS(711), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(747), 1,
      aux_sym_string_token2,
    STATE(246), 1,
      sym_macro,
  [5470] = 3,
    ACTIONS(711), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(749), 1,
      aux_sym_string_token1,
    STATE(247), 1,
      sym_macro,
  [5480] = 3,
    ACTIONS(711), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(751), 1,
      aux_sym_string_token1,
    STATE(248), 1,
      sym_macro,
  [5490] = 3,
    ACTIONS(711), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(753), 1,
      aux_sym_string_token1,
    STATE(249), 1,
      sym_macro,
  [5500] = 3,
    ACTIONS(711), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(755), 1,
      aux_sym_string_token2,
    STATE(250), 1,
      sym_macro,
  [5510] = 3,
    ACTIONS(711), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(757), 1,
      aux_sym_string_token1,
    STATE(251), 1,
      sym_macro,
  [5520] = 3,
    ACTIONS(711), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(759), 1,
      aux_sym_string_token2,
    STATE(252), 1,
      sym_macro,
  [5530] = 3,
    ACTIONS(711), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(761), 1,
      aux_sym_string_token1,
    STATE(253), 1,
      sym_macro,
  [5540] = 3,
    ACTIONS(711), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(763), 1,
      aux_sym_string_token2,
    STATE(254), 1,
      sym_macro,
  [5550] = 3,
    ACTIONS(3), 1,
      anon_sym_DOLLAR_LPAREN,
    ACTIONS(723), 1,
      anon_sym_SQUOTE,
    STATE(255), 1,
      sym_macro,
  [5560] = 1,
    ACTIONS(765), 1,
      ts_builtin_sym_end,
};

static const uint32_t ts_small_parse_table_map[] = {
  [SMALL_STATE(20)] = 0,
  [SMALL_STATE(21)] = 94,
  [SMALL_STATE(22)] = 188,
  [SMALL_STATE(23)] = 282,
  [SMALL_STATE(24)] = 376,
  [SMALL_STATE(25)] = 431,
  [SMALL_STATE(26)] = 479,
  [SMALL_STATE(27)] = 526,
  [SMALL_STATE(28)] = 588,
  [SMALL_STATE(29)] = 650,
  [SMALL_STATE(30)] = 712,
  [SMALL_STATE(31)] = 772,
  [SMALL_STATE(32)] = 834,
  [SMALL_STATE(33)] = 896,
  [SMALL_STATE(34)] = 958,
  [SMALL_STATE(35)] = 1020,
  [SMALL_STATE(36)] = 1082,
  [SMALL_STATE(37)] = 1144,
  [SMALL_STATE(38)] = 1206,
  [SMALL_STATE(39)] = 1264,
  [SMALL_STATE(40)] = 1322,
  [SMALL_STATE(41)] = 1380,
  [SMALL_STATE(42)] = 1438,
  [SMALL_STATE(43)] = 1496,
  [SMALL_STATE(44)] = 1554,
  [SMALL_STATE(45)] = 1612,
  [SMALL_STATE(46)] = 1670,
  [SMALL_STATE(47)] = 1699,
  [SMALL_STATE(48)] = 1726,
  [SMALL_STATE(49)] = 1755,
  [SMALL_STATE(50)] = 1781,
  [SMALL_STATE(51)] = 1807,
  [SMALL_STATE(52)] = 1833,
  [SMALL_STATE(53)] = 1877,
  [SMALL_STATE(54)] = 1903,
  [SMALL_STATE(55)] = 1929,
  [SMALL_STATE(56)] = 1955,
  [SMALL_STATE(57)] = 1999,
  [SMALL_STATE(58)] = 2025,
  [SMALL_STATE(59)] = 2051,
  [SMALL_STATE(60)] = 2077,
  [SMALL_STATE(61)] = 2103,
  [SMALL_STATE(62)] = 2135,
  [SMALL_STATE(63)] = 2179,
  [SMALL_STATE(64)] = 2223,
  [SMALL_STATE(65)] = 2249,
  [SMALL_STATE(66)] = 2273,
  [SMALL_STATE(67)] = 2296,
  [SMALL_STATE(68)] = 2325,
  [SMALL_STATE(69)] = 2357,
  [SMALL_STATE(70)] = 2378,
  [SMALL_STATE(71)] = 2399,
  [SMALL_STATE(72)] = 2436,
  [SMALL_STATE(73)] = 2471,
  [SMALL_STATE(74)] = 2506,
  [SMALL_STATE(75)] = 2540,
  [SMALL_STATE(76)] = 2572,
  [SMALL_STATE(77)] = 2604,
  [SMALL_STATE(78)] = 2638,
  [SMALL_STATE(79)] = 2672,
  [SMALL_STATE(80)] = 2706,
  [SMALL_STATE(81)] = 2740,
  [SMALL_STATE(82)] = 2763,
  [SMALL_STATE(83)] = 2792,
  [SMALL_STATE(84)] = 2819,
  [SMALL_STATE(85)] = 2846,
  [SMALL_STATE(86)] = 2877,
  [SMALL_STATE(87)] = 2900,
  [SMALL_STATE(88)] = 2929,
  [SMALL_STATE(89)] = 2958,
  [SMALL_STATE(90)] = 2989,
  [SMALL_STATE(91)] = 3010,
  [SMALL_STATE(92)] = 3031,
  [SMALL_STATE(93)] = 3062,
  [SMALL_STATE(94)] = 3093,
  [SMALL_STATE(95)] = 3124,
  [SMALL_STATE(96)] = 3155,
  [SMALL_STATE(97)] = 3186,
  [SMALL_STATE(98)] = 3217,
  [SMALL_STATE(99)] = 3248,
  [SMALL_STATE(100)] = 3266,
  [SMALL_STATE(101)] = 3284,
  [SMALL_STATE(102)] = 3302,
  [SMALL_STATE(103)] = 3320,
  [SMALL_STATE(104)] = 3338,
  [SMALL_STATE(105)] = 3356,
  [SMALL_STATE(106)] = 3381,
  [SMALL_STATE(107)] = 3400,
  [SMALL_STATE(108)] = 3417,
  [SMALL_STATE(109)] = 3434,
  [SMALL_STATE(110)] = 3457,
  [SMALL_STATE(111)] = 3476,
  [SMALL_STATE(112)] = 3495,
  [SMALL_STATE(113)] = 3512,
  [SMALL_STATE(114)] = 3537,
  [SMALL_STATE(115)] = 3554,
  [SMALL_STATE(116)] = 3571,
  [SMALL_STATE(117)] = 3588,
  [SMALL_STATE(118)] = 3613,
  [SMALL_STATE(119)] = 3638,
  [SMALL_STATE(120)] = 3655,
  [SMALL_STATE(121)] = 3672,
  [SMALL_STATE(122)] = 3689,
  [SMALL_STATE(123)] = 3706,
  [SMALL_STATE(124)] = 3723,
  [SMALL_STATE(125)] = 3740,
  [SMALL_STATE(126)] = 3757,
  [SMALL_STATE(127)] = 3778,
  [SMALL_STATE(128)] = 3792,
  [SMALL_STATE(129)] = 3814,
  [SMALL_STATE(130)] = 3836,
  [SMALL_STATE(131)] = 3858,
  [SMALL_STATE(132)] = 3878,
  [SMALL_STATE(133)] = 3892,
  [SMALL_STATE(134)] = 3912,
  [SMALL_STATE(135)] = 3926,
  [SMALL_STATE(136)] = 3946,
  [SMALL_STATE(137)] = 3966,
  [SMALL_STATE(138)] = 3986,
  [SMALL_STATE(139)] = 4000,
  [SMALL_STATE(140)] = 4014,
  [SMALL_STATE(141)] = 4036,
  [SMALL_STATE(142)] = 4050,
  [SMALL_STATE(143)] = 4066,
  [SMALL_STATE(144)] = 4080,
  [SMALL_STATE(145)] = 4094,
  [SMALL_STATE(146)] = 4108,
  [SMALL_STATE(147)] = 4122,
  [SMALL_STATE(148)] = 4136,
  [SMALL_STATE(149)] = 4150,
  [SMALL_STATE(150)] = 4172,
  [SMALL_STATE(151)] = 4194,
  [SMALL_STATE(152)] = 4214,
  [SMALL_STATE(153)] = 4236,
  [SMALL_STATE(154)] = 4250,
  [SMALL_STATE(155)] = 4270,
  [SMALL_STATE(156)] = 4287,
  [SMALL_STATE(157)] = 4306,
  [SMALL_STATE(158)] = 4321,
  [SMALL_STATE(159)] = 4338,
  [SMALL_STATE(160)] = 4353,
  [SMALL_STATE(161)] = 4368,
  [SMALL_STATE(162)] = 4383,
  [SMALL_STATE(163)] = 4400,
  [SMALL_STATE(164)] = 4419,
  [SMALL_STATE(165)] = 4438,
  [SMALL_STATE(166)] = 4457,
  [SMALL_STATE(167)] = 4476,
  [SMALL_STATE(168)] = 4495,
  [SMALL_STATE(169)] = 4510,
  [SMALL_STATE(170)] = 4525,
  [SMALL_STATE(171)] = 4542,
  [SMALL_STATE(172)] = 4559,
  [SMALL_STATE(173)] = 4573,
  [SMALL_STATE(174)] = 4587,
  [SMALL_STATE(175)] = 4599,
  [SMALL_STATE(176)] = 4611,
  [SMALL_STATE(177)] = 4625,
  [SMALL_STATE(178)] = 4639,
  [SMALL_STATE(179)] = 4651,
  [SMALL_STATE(180)] = 4663,
  [SMALL_STATE(181)] = 4679,
  [SMALL_STATE(182)] = 4693,
  [SMALL_STATE(183)] = 4705,
  [SMALL_STATE(184)] = 4719,
  [SMALL_STATE(185)] = 4733,
  [SMALL_STATE(186)] = 4745,
  [SMALL_STATE(187)] = 4759,
  [SMALL_STATE(188)] = 4773,
  [SMALL_STATE(189)] = 4785,
  [SMALL_STATE(190)] = 4797,
  [SMALL_STATE(191)] = 4809,
  [SMALL_STATE(192)] = 4821,
  [SMALL_STATE(193)] = 4837,
  [SMALL_STATE(194)] = 4851,
  [SMALL_STATE(195)] = 4863,
  [SMALL_STATE(196)] = 4877,
  [SMALL_STATE(197)] = 4889,
  [SMALL_STATE(198)] = 4903,
  [SMALL_STATE(199)] = 4915,
  [SMALL_STATE(200)] = 4929,
  [SMALL_STATE(201)] = 4943,
  [SMALL_STATE(202)] = 4957,
  [SMALL_STATE(203)] = 4971,
  [SMALL_STATE(204)] = 4985,
  [SMALL_STATE(205)] = 4999,
  [SMALL_STATE(206)] = 5013,
  [SMALL_STATE(207)] = 5025,
  [SMALL_STATE(208)] = 5039,
  [SMALL_STATE(209)] = 5051,
  [SMALL_STATE(210)] = 5065,
  [SMALL_STATE(211)] = 5079,
  [SMALL_STATE(212)] = 5093,
  [SMALL_STATE(213)] = 5107,
  [SMALL_STATE(214)] = 5121,
  [SMALL_STATE(215)] = 5133,
  [SMALL_STATE(216)] = 5147,
  [SMALL_STATE(217)] = 5158,
  [SMALL_STATE(218)] = 5171,
  [SMALL_STATE(219)] = 5182,
  [SMALL_STATE(220)] = 5193,
  [SMALL_STATE(221)] = 5206,
  [SMALL_STATE(222)] = 5219,
  [SMALL_STATE(223)] = 5230,
  [SMALL_STATE(224)] = 5240,
  [SMALL_STATE(225)] = 5250,
  [SMALL_STATE(226)] = 5260,
  [SMALL_STATE(227)] = 5270,
  [SMALL_STATE(228)] = 5280,
  [SMALL_STATE(229)] = 5290,
  [SMALL_STATE(230)] = 5300,
  [SMALL_STATE(231)] = 5310,
  [SMALL_STATE(232)] = 5320,
  [SMALL_STATE(233)] = 5330,
  [SMALL_STATE(234)] = 5340,
  [SMALL_STATE(235)] = 5350,
  [SMALL_STATE(236)] = 5360,
  [SMALL_STATE(237)] = 5370,
  [SMALL_STATE(238)] = 5380,
  [SMALL_STATE(239)] = 5390,
  [SMALL_STATE(240)] = 5400,
  [SMALL_STATE(241)] = 5410,
  [SMALL_STATE(242)] = 5420,
  [SMALL_STATE(243)] = 5430,
  [SMALL_STATE(244)] = 5440,
  [SMALL_STATE(245)] = 5450,
  [SMALL_STATE(246)] = 5460,
  [SMALL_STATE(247)] = 5470,
  [SMALL_STATE(248)] = 5480,
  [SMALL_STATE(249)] = 5490,
  [SMALL_STATE(250)] = 5500,
  [SMALL_STATE(251)] = 5510,
  [SMALL_STATE(252)] = 5520,
  [SMALL_STATE(253)] = 5530,
  [SMALL_STATE(254)] = 5540,
  [SMALL_STATE(255)] = 5550,
  [SMALL_STATE(256)] = 5560,
};

static const TSParseActionEntry ts_parse_actions[] = {
  [0] = {.entry = {.count = 0, .reusable = false}},
  [1] = {.entry = {.count = 1, .reusable = false}}, RECOVER(),
  [3] = {.entry = {.count = 1, .reusable = true}}, SHIFT(232),
  [5] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_source_file, 0, 0, 0),
  [7] = {.entry = {.count = 1, .reusable = false}}, SHIFT(8),
  [9] = {.entry = {.count = 1, .reusable = false}}, SHIFT(7),
  [11] = {.entry = {.count = 1, .reusable = true}}, SHIFT(237),
  [13] = {.entry = {.count = 1, .reusable = true}}, SHIFT(156),
  [15] = {.entry = {.count = 1, .reusable = false}}, SHIFT(14),
  [17] = {.entry = {.count = 1, .reusable = true}}, SHIFT(186),
  [19] = {.entry = {.count = 1, .reusable = false}}, SHIFT(186),
  [21] = {.entry = {.count = 1, .reusable = false}}, SHIFT(202),
  [23] = {.entry = {.count = 1, .reusable = true}}, SHIFT(162),
  [25] = {.entry = {.count = 1, .reusable = true}}, SHIFT(83),
  [27] = {.entry = {.count = 1, .reusable = true}}, SHIFT(84),
  [29] = {.entry = {.count = 1, .reusable = false}}, SHIFT(195),
  [31] = {.entry = {.count = 1, .reusable = false}}, SHIFT(158),
  [33] = {.entry = {.count = 1, .reusable = true}}, SHIFT(173),
  [35] = {.entry = {.count = 1, .reusable = false}}, SHIFT(173),
  [37] = {.entry = {.count = 1, .reusable = false}}, SHIFT(177),
  [39] = {.entry = {.count = 1, .reusable = false}}, SHIFT(72),
  [41] = {.entry = {.count = 1, .reusable = true}}, SHIFT(248),
  [43] = {.entry = {.count = 1, .reusable = true}}, SHIFT(227),
  [45] = {.entry = {.count = 1, .reusable = true}}, SHIFT(85),
  [47] = {.entry = {.count = 1, .reusable = true}}, SHIFT(34),
  [49] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym__space, 1, 0, 0),
  [51] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym__space, 1, 0, 0),
  [53] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_source_file, 1, 0, 0),
  [55] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0),
  [57] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(8),
  [60] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(7),
  [63] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(237),
  [66] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(156),
  [69] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(14),
  [72] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(186),
  [75] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(186),
  [78] = {.entry = {.count = 1, .reusable = true}}, SHIFT(9),
  [80] = {.entry = {.count = 1, .reusable = true}}, SHIFT(238),
  [82] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym__statement, 1, 0, 0),
  [84] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym__statement, 1, 0, 0),
  [86] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym__statement, 2, 0, 0),
  [88] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym__statement, 2, 0, 0),
  [90] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 1, 0, 0),
  [92] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_source_file_repeat1, 1, 0, 0),
  [94] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym__statement, 3, 0, 0),
  [96] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym__statement, 3, 0, 0),
  [98] = {.entry = {.count = 1, .reusable = false}}, SHIFT(180),
  [100] = {.entry = {.count = 1, .reusable = false}}, SHIFT(217),
  [102] = {.entry = {.count = 1, .reusable = false}}, SHIFT(221),
  [104] = {.entry = {.count = 1, .reusable = false}}, SHIFT(192),
  [106] = {.entry = {.count = 1, .reusable = false}}, SHIFT(26),
  [108] = {.entry = {.count = 1, .reusable = true}}, SHIFT(26),
  [110] = {.entry = {.count = 1, .reusable = true}}, SHIFT(160),
  [112] = {.entry = {.count = 1, .reusable = true}}, SHIFT(86),
  [114] = {.entry = {.count = 1, .reusable = true}}, SHIFT(81),
  [116] = {.entry = {.count = 1, .reusable = false}}, SHIFT(201),
  [118] = {.entry = {.count = 1, .reusable = false}}, SHIFT(207),
  [120] = {.entry = {.count = 1, .reusable = false}}, SHIFT(168),
  [122] = {.entry = {.count = 1, .reusable = true}}, SHIFT(174),
  [124] = {.entry = {.count = 1, .reusable = false}}, SHIFT(174),
  [126] = {.entry = {.count = 1, .reusable = false}}, SHIFT(175),
  [128] = {.entry = {.count = 1, .reusable = false}}, SHIFT(73),
  [130] = {.entry = {.count = 1, .reusable = true}}, SHIFT(253),
  [132] = {.entry = {.count = 1, .reusable = true}}, SHIFT(254),
  [134] = {.entry = {.count = 1, .reusable = true}}, SHIFT(71),
  [136] = {.entry = {.count = 1, .reusable = true}}, SHIFT(33),
  [138] = {.entry = {.count = 1, .reusable = false}}, SHIFT(200),
  [140] = {.entry = {.count = 1, .reusable = false}}, SHIFT(209),
  [142] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_selector_query_repeat1, 2, 0, 0), SHIFT_REPEAT(26),
  [145] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_selector_query_repeat1, 2, 0, 0), SHIFT_REPEAT(26),
  [148] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_selector_query_repeat1, 2, 0, 0),
  [150] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_selector_query_repeat1, 2, 0, 0),
  [152] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_selector_query_repeat1, 1, 0, 0),
  [154] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_selector_query_repeat1, 1, 0, 0),
  [156] = {.entry = {.count = 1, .reusable = false}}, SHIFT(60),
  [158] = {.entry = {.count = 1, .reusable = true}}, SHIFT(60),
  [160] = {.entry = {.count = 1, .reusable = true}}, SHIFT(64),
  [162] = {.entry = {.count = 1, .reusable = false}}, SHIFT(197),
  [164] = {.entry = {.count = 1, .reusable = false}}, SHIFT(46),
  [166] = {.entry = {.count = 1, .reusable = true}}, SHIFT(247),
  [168] = {.entry = {.count = 1, .reusable = true}}, SHIFT(223),
  [170] = {.entry = {.count = 1, .reusable = false}}, SHIFT(59),
  [172] = {.entry = {.count = 1, .reusable = false}}, SHIFT(49),
  [174] = {.entry = {.count = 1, .reusable = true}}, SHIFT(92),
  [176] = {.entry = {.count = 1, .reusable = true}}, SHIFT(32),
  [178] = {.entry = {.count = 1, .reusable = true}}, SHIFT(127),
  [180] = {.entry = {.count = 1, .reusable = true}}, SHIFT(104),
  [182] = {.entry = {.count = 1, .reusable = true}}, SHIFT(184),
  [184] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_nbt_array_repeat1, 2, 0, 0), SHIFT_REPEAT(60),
  [187] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_nbt_array_repeat1, 2, 0, 0), SHIFT_REPEAT(60),
  [190] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_nbt_array_repeat1, 2, 0, 0), SHIFT_REPEAT(64),
  [193] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_nbt_array_repeat1, 2, 0, 0), SHIFT_REPEAT(197),
  [196] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_nbt_array_repeat1, 2, 0, 0), SHIFT_REPEAT(46),
  [199] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_nbt_array_repeat1, 2, 0, 0), SHIFT_REPEAT(247),
  [202] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_nbt_array_repeat1, 2, 0, 0), SHIFT_REPEAT(223),
  [205] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_nbt_array_repeat1, 2, 0, 0), SHIFT_REPEAT(59),
  [208] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_nbt_array_repeat1, 2, 0, 0), SHIFT_REPEAT(49),
  [211] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_nbt_array_repeat1, 2, 0, 0), SHIFT_REPEAT(92),
  [214] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_nbt_array_repeat1, 2, 0, 0), SHIFT_REPEAT(32),
  [217] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_nbt_array_repeat1, 2, 0, 0),
  [219] = {.entry = {.count = 1, .reusable = true}}, SHIFT(145),
  [221] = {.entry = {.count = 1, .reusable = true}}, SHIFT(50),
  [223] = {.entry = {.count = 1, .reusable = true}}, SHIFT(188),
  [225] = {.entry = {.count = 1, .reusable = true}}, SHIFT(183),
  [227] = {.entry = {.count = 1, .reusable = true}}, SHIFT(57),
  [229] = {.entry = {.count = 1, .reusable = true}}, SHIFT(191),
  [231] = {.entry = {.count = 1, .reusable = true}}, SHIFT(101),
  [233] = {.entry = {.count = 1, .reusable = false}}, SHIFT(65),
  [235] = {.entry = {.count = 1, .reusable = true}}, SHIFT(65),
  [237] = {.entry = {.count = 1, .reusable = false}}, SHIFT(203),
  [239] = {.entry = {.count = 1, .reusable = false}}, SHIFT(110),
  [241] = {.entry = {.count = 1, .reusable = true}}, SHIFT(251),
  [243] = {.entry = {.count = 1, .reusable = true}}, SHIFT(252),
  [245] = {.entry = {.count = 1, .reusable = false}}, SHIFT(134),
  [247] = {.entry = {.count = 1, .reusable = false}}, SHIFT(146),
  [249] = {.entry = {.count = 1, .reusable = true}}, SHIFT(94),
  [251] = {.entry = {.count = 1, .reusable = true}}, SHIFT(27),
  [253] = {.entry = {.count = 1, .reusable = false}}, SHIFT(210),
  [255] = {.entry = {.count = 1, .reusable = false}}, SHIFT(90),
  [257] = {.entry = {.count = 1, .reusable = true}}, SHIFT(249),
  [259] = {.entry = {.count = 1, .reusable = true}}, SHIFT(250),
  [261] = {.entry = {.count = 1, .reusable = false}}, SHIFT(112),
  [263] = {.entry = {.count = 1, .reusable = false}}, SHIFT(70),
  [265] = {.entry = {.count = 1, .reusable = true}}, SHIFT(96),
  [267] = {.entry = {.count = 1, .reusable = true}}, SHIFT(37),
  [269] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_scale, 1, 0, 0),
  [271] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_scale, 1, 0, 0),
  [273] = {.entry = {.count = 1, .reusable = false}}, SHIFT(51),
  [275] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_scale, 2, 0, 0),
  [277] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_scale, 2, 0, 0),
  [279] = {.entry = {.count = 1, .reusable = false}}, SHIFT(55),
  [281] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_nbt_identifier, 1, 0, 0),
  [283] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_nbt_identifier, 1, 0, 0),
  [285] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_nbt_array, 2, 0, 0),
  [287] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_nbt_array, 2, 0, 0),
  [289] = {.entry = {.count = 1, .reusable = false}}, SHIFT(4),
  [291] = {.entry = {.count = 1, .reusable = true}}, SHIFT(4),
  [293] = {.entry = {.count = 1, .reusable = true}}, SHIFT(90),
  [295] = {.entry = {.count = 1, .reusable = true}}, SHIFT(114),
  [297] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_nbt_compound, 2, 0, 0),
  [299] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_nbt_compound, 2, 0, 0),
  [301] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_string, 3, 0, 0),
  [303] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_string, 3, 0, 0),
  [305] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_scale, 3, 0, 0),
  [307] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_scale, 3, 0, 0),
  [309] = {.entry = {.count = 1, .reusable = true}}, SHIFT(120),
  [311] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_nbt_array, 3, 0, 0),
  [313] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_nbt_array, 3, 0, 0),
  [315] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_nbt_compound, 3, 0, 0),
  [317] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_nbt_compound, 3, 0, 0),
  [319] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym__nbt_element, 1, 0, 0),
  [321] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym__nbt_element, 1, 0, 0),
  [323] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_selector_query_repeat1, 2, 0, 0), SHIFT_REPEAT(65),
  [326] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_selector_query_repeat1, 2, 0, 0), SHIFT_REPEAT(65),
  [329] = {.entry = {.count = 1, .reusable = true}}, SHIFT(116),
  [331] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_nbt_array_repeat1, 1, 0, 0),
  [333] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_nbt_array_repeat1, 1, 0, 0),
  [335] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_selector_query_repeat1, 2, 0, 0), SHIFT_REPEAT(4),
  [338] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_selector_query_repeat1, 2, 0, 0), SHIFT_REPEAT(4),
  [341] = {.entry = {.count = 1, .reusable = false}}, SHIFT(3),
  [343] = {.entry = {.count = 1, .reusable = true}}, SHIFT(3),
  [345] = {.entry = {.count = 1, .reusable = true}}, SHIFT(52),
  [347] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_item_compound_repeat1, 1, 0, 0),
  [349] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_item_compound_repeat1, 1, 0, 0),
  [351] = {.entry = {.count = 1, .reusable = true}}, SHIFT(106),
  [353] = {.entry = {.count = 1, .reusable = true}}, SHIFT(149),
  [355] = {.entry = {.count = 1, .reusable = true}}, SHIFT(245),
  [357] = {.entry = {.count = 1, .reusable = true}}, SHIFT(246),
  [359] = {.entry = {.count = 1, .reusable = false}}, SHIFT(157),
  [361] = {.entry = {.count = 1, .reusable = true}}, SHIFT(196),
  [363] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_word, 1, 0, 0),
  [365] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_word, 1, 0, 0),
  [367] = {.entry = {.count = 1, .reusable = true}}, SHIFT(72),
  [369] = {.entry = {.count = 1, .reusable = true}}, SHIFT(77),
  [371] = {.entry = {.count = 1, .reusable = true}}, SHIFT(78),
  [373] = {.entry = {.count = 1, .reusable = true}}, SHIFT(122),
  [375] = {.entry = {.count = 1, .reusable = true}}, SHIFT(68),
  [377] = {.entry = {.count = 1, .reusable = true}}, SHIFT(115),
  [379] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_item_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(122),
  [382] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_item_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(68),
  [385] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_item_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(249),
  [388] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_item_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(250),
  [391] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_item_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(70),
  [394] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_item_compound_repeat1, 2, 0, 0),
  [396] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_selector_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(119),
  [399] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_selector_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(149),
  [402] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_selector_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(248),
  [405] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_selector_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(227),
  [408] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_selector_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(157),
  [411] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_selector_compound_repeat1, 2, 0, 0),
  [413] = {.entry = {.count = 1, .reusable = true}}, SHIFT(124),
  [415] = {.entry = {.count = 1, .reusable = true}}, SHIFT(107),
  [417] = {.entry = {.count = 1, .reusable = true}}, SHIFT(108),
  [419] = {.entry = {.count = 1, .reusable = true}}, SHIFT(119),
  [421] = {.entry = {.count = 1, .reusable = true}}, SHIFT(214),
  [423] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_rotation, 1, 0, 0),
  [425] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_nbt_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(138),
  [428] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_nbt_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(245),
  [431] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_nbt_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(246),
  [434] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_nbt_compound_repeat1, 2, 0, 0), SHIFT_REPEAT(157),
  [437] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_nbt_compound_repeat1, 2, 0, 0),
  [439] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_position, 1, 0, 0),
  [441] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_position, 1, 0, 0),
  [443] = {.entry = {.count = 1, .reusable = true}}, SHIFT(195),
  [445] = {.entry = {.count = 1, .reusable = true}}, SHIFT(158),
  [447] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_rotation, 1, 0, 0),
  [449] = {.entry = {.count = 1, .reusable = true}}, SHIFT(138),
  [451] = {.entry = {.count = 1, .reusable = true}}, SHIFT(157),
  [453] = {.entry = {.count = 1, .reusable = true}}, SHIFT(213),
  [455] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_word, 2, 0, 0),
  [457] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_word, 2, 0, 0),
  [459] = {.entry = {.count = 1, .reusable = true}}, SHIFT(212),
  [461] = {.entry = {.count = 1, .reusable = false}}, SHIFT(100),
  [463] = {.entry = {.count = 1, .reusable = false}}, SHIFT(102),
  [465] = {.entry = {.count = 1, .reusable = true}}, SHIFT(53),
  [467] = {.entry = {.count = 1, .reusable = true}}, SHIFT(58),
  [469] = {.entry = {.count = 1, .reusable = true}}, SHIFT(141),
  [471] = {.entry = {.count = 1, .reusable = true}}, SHIFT(144),
  [473] = {.entry = {.count = 1, .reusable = true}}, SHIFT(99),
  [475] = {.entry = {.count = 1, .reusable = true}}, SHIFT(190),
  [477] = {.entry = {.count = 1, .reusable = true}}, SHIFT(103),
  [479] = {.entry = {.count = 1, .reusable = false}}, SHIFT(18),
  [481] = {.entry = {.count = 1, .reusable = true}}, SHIFT(18),
  [483] = {.entry = {.count = 1, .reusable = false}}, SHIFT(20),
  [485] = {.entry = {.count = 1, .reusable = true}}, SHIFT(20),
  [487] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_nbt_compound_repeat1, 1, 0, 0), REDUCE(aux_sym_selector_compound_repeat1, 1, 0, 0),
  [490] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_selector_compound_repeat1, 1, 0, 0),
  [492] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_nbt_compound_repeat1, 1, 0, 0), REDUCE(aux_sym_selector_compound_repeat1, 1, 0, 0),
  [495] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_item_compound, 2, 0, 0),
  [497] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_item_compound, 2, 0, 0),
  [499] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_item_compound, 3, 0, 0),
  [501] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_item_compound, 3, 0, 0),
  [503] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_selector_query_repeat1, 2, 0, 0), SHIFT_REPEAT(18),
  [506] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_selector_query_repeat1, 2, 0, 0), SHIFT_REPEAT(18),
  [509] = {.entry = {.count = 1, .reusable = false}}, SHIFT(139),
  [511] = {.entry = {.count = 1, .reusable = false}}, SHIFT(143),
  [513] = {.entry = {.count = 1, .reusable = true}}, SHIFT(41),
  [515] = {.entry = {.count = 1, .reusable = true}}, SHIFT(45),
  [517] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_item_compound_repeat1, 3, 0, 0),
  [519] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_item_compound_repeat1, 3, 0, 0),
  [521] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_item_compound_repeat1, 4, 0, 0),
  [523] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_item_compound_repeat1, 4, 0, 0),
  [525] = {.entry = {.count = 1, .reusable = true}}, SHIFT(43),
  [527] = {.entry = {.count = 1, .reusable = true}}, SHIFT(40),
  [529] = {.entry = {.count = 1, .reusable = false}}, SHIFT(23),
  [531] = {.entry = {.count = 1, .reusable = true}}, SHIFT(23),
  [533] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_selector_compound_repeat1, 1, 0, 0),
  [535] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_item_compound_repeat1, 5, 0, 0),
  [537] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_item_compound_repeat1, 5, 0, 0),
  [539] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_selector_compound_repeat1, 3, 0, 0),
  [541] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_selector_compound_repeat1, 3, 0, 0),
  [543] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_selector_compound_repeat1, 4, 0, 0),
  [545] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_selector_compound_repeat1, 4, 0, 0),
  [547] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_selector_compound_repeat1, 5, 0, 0),
  [549] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym_selector_compound_repeat1, 5, 0, 0),
  [551] = {.entry = {.count = 1, .reusable = true}}, SHIFT(62),
  [553] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym__generic_command, 1, 0, 0),
  [555] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym__generic_command_repeat1, 2, 0, 0),
  [557] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym__generic_command_repeat1, 2, 0, 0), SHIFT_REPEAT(3),
  [560] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym__generic_command_repeat1, 2, 0, 0), SHIFT_REPEAT(3),
  [563] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_nbt_compound_repeat1, 3, 0, 0),
  [565] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_word, 3, 0, 0),
  [567] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_word, 3, 0, 0),
  [569] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_nbt_compound_repeat1, 1, 0, 0),
  [571] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_nbt_compound_repeat1, 4, 0, 0),
  [573] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_selector_query_repeat1, 2, 0, 0), SHIFT_REPEAT(3),
  [576] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_selector_query_repeat1, 2, 0, 0), SHIFT_REPEAT(3),
  [579] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym__generic_command, 2, 0, 0),
  [581] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_nbt_compound_repeat1, 5, 0, 0),
  [583] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym__execute_command_repeat1, 2, 0, 0), SHIFT_REPEAT(19),
  [586] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym__execute_command_repeat1, 2, 0, 0), SHIFT_REPEAT(19),
  [589] = {.entry = {.count = 1, .reusable = true}}, SHIFT(181),
  [591] = {.entry = {.count = 1, .reusable = true}}, SHIFT(163),
  [593] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_selector, 1, 0, 0),
  [595] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_selector, 1, 0, 0),
  [597] = {.entry = {.count = 1, .reusable = true}}, SHIFT(164),
  [599] = {.entry = {.count = 1, .reusable = true}}, SHIFT(194),
  [601] = {.entry = {.count = 1, .reusable = true}}, SHIFT(118),
  [603] = {.entry = {.count = 1, .reusable = true}}, SHIFT(199),
  [605] = {.entry = {.count = 1, .reusable = true}}, SHIFT(206),
  [607] = {.entry = {.count = 1, .reusable = true}}, SHIFT(205),
  [609] = {.entry = {.count = 1, .reusable = false}}, SHIFT(185),
  [611] = {.entry = {.count = 1, .reusable = false}}, SHIFT(189),
  [613] = {.entry = {.count = 1, .reusable = true}}, SHIFT(204),
  [615] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_selector_repeat1, 2, 0, 0),
  [617] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_selector_repeat1, 2, 0, 0), SHIFT_REPEAT(118),
  [620] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym__generic_command_repeat1, 2, 0, 0),
  [622] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_operator, 1, 0, 0),
  [624] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_operator, 1, 0, 0),
  [626] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_slot, 1, 0, 0),
  [628] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_rotation, 2, 0, 0),
  [630] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_rotation, 2, 0, 0),
  [632] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_slot, 1, 0, 0),
  [634] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_position, 2, 0, 0),
  [636] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_command_identifier, 1, 0, 0),
  [638] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_command_identifier, 1, 0, 0),
  [640] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_position, 2, 0, 0),
  [642] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_selector, 3, 0, 0),
  [644] = {.entry = {.count = 1, .reusable = false}}, SHIFT(170),
  [646] = {.entry = {.count = 1, .reusable = true}}, SHIFT(170),
  [648] = {.entry = {.count = 2, .reusable = true}}, REDUCE(sym_nbt_compound, 2, 0, 0), REDUCE(sym_selector_compound, 2, 0, 0),
  [651] = {.entry = {.count = 1, .reusable = false}}, SHIFT(48),
  [653] = {.entry = {.count = 1, .reusable = true}}, SHIFT(48),
  [655] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_selector, 3, 0, 0),
  [657] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_selector_query, 5, 0, 0),
  [659] = {.entry = {.count = 1, .reusable = true}}, SHIFT(218),
  [661] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_selector_query, 4, 0, 0),
  [663] = {.entry = {.count = 1, .reusable = true}}, SHIFT(219),
  [665] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_argument, 1, 0, 0),
  [667] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_argument, 1, 0, 0),
  [669] = {.entry = {.count = 1, .reusable = false}}, SHIFT(111),
  [671] = {.entry = {.count = 1, .reusable = true}}, SHIFT(111),
  [673] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_selector, 4, 0, 0),
  [675] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_selector, 4, 0, 0),
  [677] = {.entry = {.count = 1, .reusable = false}}, SHIFT(169),
  [679] = {.entry = {.count = 1, .reusable = true}}, SHIFT(169),
  [681] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_word, 4, 0, 0),
  [683] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_selector_query, 3, 0, 0),
  [685] = {.entry = {.count = 1, .reusable = true}}, SHIFT(216),
  [687] = {.entry = {.count = 1, .reusable = false}}, SHIFT(91),
  [689] = {.entry = {.count = 1, .reusable = true}}, SHIFT(91),
  [691] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_selector_compound, 3, 0, 0),
  [693] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_word, 4, 0, 0),
  [695] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_subcommand_identifier, 1, 0, 0),
  [697] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_subcommand_identifier, 1, 0, 0),
  [699] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_selector_query, 6, 0, 0),
  [701] = {.entry = {.count = 1, .reusable = false}}, REDUCE(aux_sym__execute_command_repeat1, 2, 0, 0),
  [703] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym__execute_command_repeat1, 2, 0, 0),
  [705] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym__subcommand, 1, 0, 0),
  [707] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym__subcommand, 1, 0, 0),
  [709] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_selector_repeat1, 1, 0, 0),
  [711] = {.entry = {.count = 1, .reusable = false}}, SHIFT(232),
  [713] = {.entry = {.count = 1, .reusable = false}}, SHIFT(226),
  [715] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_command, 2, 0, 0),
  [717] = {.entry = {.count = 1, .reusable = true}}, SHIFT(54),
  [719] = {.entry = {.count = 1, .reusable = false}}, SHIFT(255),
  [721] = {.entry = {.count = 1, .reusable = true}},  ACCEPT_INPUT(),
  [723] = {.entry = {.count = 1, .reusable = true}}, SHIFT(142),
  [725] = {.entry = {.count = 1, .reusable = true}}, SHIFT(161),
  [727] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym__execute_command, 6, 0, 0),
  [729] = {.entry = {.count = 1, .reusable = true}}, SHIFT(239),
  [731] = {.entry = {.count = 1, .reusable = true}}, SHIFT(69),
  [733] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_command, 1, 0, 0),
  [735] = {.entry = {.count = 1, .reusable = true}}, SHIFT(11),
  [737] = {.entry = {.count = 1, .reusable = true}}, SHIFT(256),
  [739] = {.entry = {.count = 1, .reusable = true}}, SHIFT(147),
  [741] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym__execute_command, 5, 0, 0),
  [743] = {.entry = {.count = 1, .reusable = true}}, SHIFT(159),
  [745] = {.entry = {.count = 1, .reusable = false}}, SHIFT(230),
  [747] = {.entry = {.count = 1, .reusable = false}}, SHIFT(235),
  [749] = {.entry = {.count = 1, .reusable = false}}, SHIFT(225),
  [751] = {.entry = {.count = 1, .reusable = false}}, SHIFT(229),
  [753] = {.entry = {.count = 1, .reusable = false}}, SHIFT(233),
  [755] = {.entry = {.count = 1, .reusable = false}}, SHIFT(234),
  [757] = {.entry = {.count = 1, .reusable = false}}, SHIFT(240),
  [759] = {.entry = {.count = 1, .reusable = false}}, SHIFT(241),
  [761] = {.entry = {.count = 1, .reusable = false}}, SHIFT(243),
  [763] = {.entry = {.count = 1, .reusable = false}}, SHIFT(244),
  [765] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_macro, 3, 0, 0),
};

#ifdef __cplusplus
extern "C" {
#endif
#ifdef TREE_SITTER_HIDE_SYMBOLS
#define TS_PUBLIC
#elif defined(_WIN32)
#define TS_PUBLIC __declspec(dllexport)
#else
#define TS_PUBLIC __attribute__((visibility("default")))
#endif

TS_PUBLIC const TSLanguage *tree_sitter_mcfunction(void) {
  static const TSLanguage language = {
    .abi_version = LANGUAGE_VERSION,
    .symbol_count = SYMBOL_COUNT,
    .alias_count = ALIAS_COUNT,
    .token_count = TOKEN_COUNT,
    .external_token_count = EXTERNAL_TOKEN_COUNT,
    .state_count = STATE_COUNT,
    .large_state_count = LARGE_STATE_COUNT,
    .production_id_count = PRODUCTION_ID_COUNT,
    .supertype_count = SUPERTYPE_COUNT,
    .field_count = FIELD_COUNT,
    .max_alias_sequence_length = MAX_ALIAS_SEQUENCE_LENGTH,
    .parse_table = &ts_parse_table[0][0],
    .small_parse_table = ts_small_parse_table,
    .small_parse_table_map = ts_small_parse_table_map,
    .parse_actions = ts_parse_actions,
    .symbol_names = ts_symbol_names,
    .symbol_metadata = ts_symbol_metadata,
    .public_symbol_map = ts_symbol_map,
    .alias_map = ts_non_terminal_alias_map,
    .alias_sequences = &ts_alias_sequences[0][0],
    .lex_modes = (const void*)ts_lex_modes,
    .lex_fn = ts_lex,
    .primary_state_ids = ts_primary_state_ids,
    .name = "mcfunction",
    .max_reserved_word_set_size = 0,
    .metadata = {
      .major_version = 1,
      .minor_version = 0,
      .patch_version = 0,
    },
  };
  return &language;
}
#ifdef __cplusplus
}
#endif
